...
> [!question]- Почему Vulkan быстрее OpenGL ES для сложных сцен, но может быть медленнее для простых?
> Vulkan дает explicit control над GPU: command buffers, memory management, synchronization. Для сложных сцен это снижает driver overhead (меньше state validation). Но для простых сцен overhead управления памятью и синхронизацией вручную превышает выгоду. OpenGL ES driver автоматизирует эти операции, что эффективнее для простых случаев.

> [!question]- Сценарий: игра на OpenGL ES работает плавно на Pixel, но дергается на Samsung. Почему?
> Разные GPU vendor: Pixel -- Adreno (Qualcomm), Samsung -- Mali (ARM) или Xclipse. Каждый GPU имеет разную производительность для разных операций: texture sampling, shader complexity, fill rate. Решение: profile на целевых устройствах, adaptive quality settings, vendor-specific optimizations.
...
Чем OpenGL ES отличается от Vulkan?
?
OpenGL ES: высокоуровневый, driver управляет памятью/синхронизацией, implicit state machine. Vulkan: низкоуровневый, explicit control, command buffers, лучше для многопоточности. OpenGL ES проще, Vulkan эффективнее для сложных сцен.

Как Android использует GPU для обычного UI?
?
Skia (2D engine) -> HWUI (hardware acceleration) -> OpenGL ES/Vulkan. View.onDraw() записывает команды в DisplayList. RenderThread конвертирует в GPU-команды через Skia backend (GL или Vulkan, выбирается системой).

Что такое EGL в контексте Android?
?
Интерфейс между OpenGL ES и оконной системой Android (Surface). EGLContext -- OpenGL ES context, EGLSurface -- render target. SurfaceView/TextureView предоставляют Surface для прямого GPU рендеринга.

Когда использовать SurfaceView vs TextureView?
?
SurfaceView: отдельный Surface, лучше производительность (нет composition overhead), для games/video. TextureView: часть View hierarchy, поддерживает transforms/alpha, для camera preview, видео в списке.
...
|-------------|------|-------|
| Следующий шаг | [[android-canvas-drawing]] | 2D рисование через Canvas/Skia |
| Углубиться | [[android-window-system]] | SurfaceFlinger и composition |
| Смежная тема | [[cross-graphics-rendering]] | Сравнение GPU API на разных платформах |
