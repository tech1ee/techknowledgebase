...
> **Custom View** — расширение существующего класса через **наследование** (inheritance) — один из четырёх столпов ООП (Booch, *Object-Oriented Analysis and Design*, 1994). Android View System использует **Template Method** паттерн (Gamma et al., 1994): базовый класс `View` определяет алгоритм отрисовки (`measure → layout → draw`), а подклассы переопределяют конкретные шаги (`onMeasure()`, `onLayout()`, `onDraw()`).

Создание Custom View в Android следует парадигме **Framework Inversion of Control** (Johnson/Foote, 1988): разработчик не вызывает методы фреймворка — фреймворк вызывает методы разработчика (`onDraw()`, `onTouchEvent()`). Это «голливудский принцип»: *Don't call us, we'll call you*.

| Подход к расширению UI | Паттерн | Гибкость | Сложность | Пример |
|-----------------------|---------|----------|-----------|--------|
| Наследование от View | Template Method | Максимальная | Высокая | Кастомный график |
| Наследование от существующего View | Specialization | Средняя | Средняя | EditText с валидацией |
| Композиция ViewGroup | Composite + Strategy | Высокая | Средняя | Составной компонент |
| Drawable | Strategy | Только отрисовка | Низкая | Кастомный фон |

> **Система конструкторов** View (`Context`, `Context + AttributeSet`, `Context + AttributeSet + defStyleAttr`, `Context + AttributeSet + defStyleAttr + defStyleRes`) реализует паттерн **Telescoping Constructor**, решающий проблему множественных источников конфигурации. Приоритет атрибутов следует **цепочке разрешения стилей**: inline XML → style → defStyleAttr → defStyleRes → theme — формальная система приоритетов, аналогичная CSS Specificity (W3C).

Концепция `invalidate()` и `requestLayout()` восходит к модели **damage/repair** в оконных системах (Gosling/Rosenthal, *A Window Manager for Bitmapped Displays*, 1986): приложение сообщает системе о «повреждённой» области, а система планирует перерисовку в оптимальный момент (следующий VSYNC). Это реализация принципа **ленивых вычислений** (lazy evaluation) — работа выполняется только когда результат действительно нужен.

**State saving** через `onSaveInstanceState()`/`onRestoreInstanceState()` в Custom View — реализация паттерна **Memento** (Gamma et al., 1994): внутреннее состояние View сериализуется в `Parcelable` объект без нарушения инкапсуляции, позволяя восстановить его после пересоздания.
...
- **Gamma E. et al. (1994). Design Patterns.** — Template Method (lifecycle callbacks), Memento (state saving), Composite (View hierarchy)
- **Booch G. (1994). Object-Oriented Analysis and Design.** — Наследование как механизм расширения
- **Johnson R., Foote B. (1988). Designing Reusable Classes.** — Framework Inversion of Control
- **Gosling J., Rosenthal D. (1986). A Window Manager for Bitmapped Displays.** — Damage/repair модель (invalidate/requestLayout)

### Практические руководства

| Источник | Тип | Вклад |
|----------|-----|-------|
| [Create a view class](https://developer.android.com/develop/ui/views/layout/custom-views/create-view) | Official | Constructors, AttributeSet |
...
| [Optimize custom view](https://developer.android.com/develop/ui/views/layout/custom-views/optimizing-view) | Official | Performance optimization |
| [Make custom views accessible](https://developer.android.com/guide/topics/ui/accessibility/custom-views) | Official | Accessibility requirements |

- **Meier R. (2022)** *Professional Android* — Custom View: конструкторы, AttributeSet, onDraw(), стилизация
- **Phillips B. et al. (2022)** *Android Programming: The Big Nerd Ranch Guide* — создание Custom View с обработкой касаний
...
> [!question]- Почему кастомный View должен поддерживать state saving и как это реализуется?
> При Configuration Change система пересоздает Activity и все View. Стандартные View (EditText, CheckBox) автоматически сохраняют состояние через onSaveInstanceState()/onRestoreInstanceState() если имеют android:id. Кастомный View должен реализовать эти методы сам, создав Parcelable SavedState для сохранения своего состояния.

> [!question]- Сценарий: кастомный View правильно работает программно, но не показывает атрибуты из XML. Что забыли?
> Не реализован конструктор с AttributeSet и не объявлены attrs в res/values/attrs.xml. Нужно: 1) Создать declare-styleable с custom attributes. 2) Реализовать конструктор (context, attrs, defStyleAttr). 3) В init{} получить TypedArray через obtainStyledAttributes() и прочитать значения.
...
Какие конструкторы нужны для кастомного View?
?
View(context) -- создание из кода. View(context, attrs) -- inflate из XML. View(context, attrs, defStyleAttr) -- с default style. View(context, attrs, defStyleAttr, defStyleRes) -- с style resource. Минимум первые два.

Как объявить custom attributes для View?
?
1) res/values/attrs.xml: declare-styleable с attr элементами (format: string, integer, dimension, color, reference). 2) В View: obtainStyledAttributes(attrs, R.styleable.MyView). 3) В XML: app:myAttr='value'.

Что такое onDraw() и правила рисования?
?
Callback для отрисовки View на Canvas. Правила: не создавать объекты (Paint, Path) внутри onDraw() -- только в init. Учитывать padding. Вызывается каждый кадр при invalidate().

Как правильно обработать touch в кастомном View?
?
Override onTouchEvent(). Вернуть true для потребленных событий. Для scrollable -- использовать VelocityTracker и Scroller/OverScroller. Не забыть performClick() для accessibility.

Что такое Drawable и как его использовать в кастомном View?
?
Абстракция для отрисовки: shape, bitmap, vector, selector. Можно назначить как background или рисовать в onDraw() через drawable.setBounds() + drawable.draw(canvas). Поддерживает state (pressed, focused).
...
|-------------|------|-------|
| Следующий шаг | [[android-canvas-drawing]] | Рисование на Canvas детально |
| Углубиться | [[android-touch-handling]] | Touch handling для интерактивных View |
| Смежная тема | [[ios-custom-views]] | Создание кастомных View в iOS (UIView subclass) |
