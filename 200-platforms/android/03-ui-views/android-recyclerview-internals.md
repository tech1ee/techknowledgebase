...
> [!question]- Почему RecyclerView имеет 4 уровня кэша и когда каждый используется?
> Scrap (attached views, текущий layout) -- для перестроения layout без rebind. Cache (по позиции, default 2) -- detached views, не требуют rebind. ViewCacheExtension (custom) -- для специфичных кейсов. RecycledViewPool (по type) -- требуют rebind. Каждый уровень оптимизирует разный сценарий: от перестроения layout до переиспользования view между RecyclerView.

> [!question]- Сценарий: при быстром скролле списка из 10000 элементов приложение дергается. DiffUtil уже используется. Что еще проверить?
> 1) onBindViewHolder() выполняет тяжелую работу (загрузка изображений без кэша, форматирование). 2) Нет setHasStableIds(true) -- RecyclerView не может оптимизировать анимации. 3) GapWorker prefetch отключен или недостаточен. 4) Nested RecyclerView без shared RecycledViewPool. 5) ItemDecoration с тяжелыми Canvas-операциями.
...
Какие 4 уровня кэша в RecyclerView?
?
1) Scrap (mAttachedScrap/mChangedScrap) -- текущие attached views. 2) Cache (mCachedViews, default 2) -- detached, по позиции, без rebind. 3) ViewCacheExtension -- custom cache. 4) RecycledViewPool -- по viewType, требует rebind.

Что такое DiffUtil и как он работает?
?
Утилита для вычисления минимального набора изменений между двумя списками. Использует алгоритм Myers diff (O(N+D^2)). Генерирует точные notify events (insert, remove, move, change) вместо notifyDataSetChanged().

Что такое GapWorker prefetch?
?
Механизм предварительного создания и bind ViewHolder в idle time между кадрами. Использует Choreographer для определения свободного времени. Включен по умолчанию с support library 25.1.0.

Что такое RecycledViewPool и когда его share?
?
Пул ViewHolder по viewType. При scrolle off-screen ViewHolder попадает в pool. Можно share между несколькими RecyclerView с одинаковыми viewType (например, horizontal lists в vertical list).

Как setHasStableIds(true) оптимизирует RecyclerView?
?
Когда ID стабильны, RecyclerView может переиспользовать ViewHolder при изменении позиции (move вместо remove+insert). Уменьшает количество rebind операций и улучшает анимации.
...
|-------------|------|-------|
| Следующий шаг | [[android-compose]] | LazyColumn в Compose как замена RecyclerView |
| Углубиться | [[android-view-rendering-pipeline]] | Как RecyclerView взаимодействует с rendering pipeline |
| Смежная тема | [[ios-scroll-performance]] | Сравнение UITableView и RecyclerView |
