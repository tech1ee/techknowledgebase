...
> **Canvas** в Android — реализация абстракции **2D drawing surface**, восходящей к модели PostScript (Adobe, 1984) и её развитию в PDF/Quartz 2D. Модель «Canvas + Paint» разделяет **геометрию** (что рисовать) и **стиль** (как рисовать) — классический принцип **Separation of Concerns** в графических системах.

Архитектура рисования в Android следует модели **Painter's Algorithm** (Newell et al., 1972): элементы рисуются последовательно, каждый следующий слой перекрывает предыдущий. Порядок вызовов `Canvas.draw*()` в `onDraw()` определяет Z-порядок — в отличие от Z-buffer алгоритма в 3D-графике.

| Концепция | Android (Canvas/Skia) | iOS (Core Graphics) | Web (Canvas 2D) | Происхождение |
|-----------|----------------------|--------------------|--------------------|---------------|
| Drawing surface | `Canvas` | `CGContext` | `CanvasRenderingContext2D` | PostScript (1984) |
| Style object | `Paint` | `CGContext` state | fillStyle/strokeStyle | PostScript gsave/grestore |
| Vector path | `Path` | `CGPath` | `Path2D` | Bézier curves (1962) |
| Compositing | `PorterDuff.Mode` | `CGBlendMode` | `globalCompositeOperation` | Porter-Duff (1984) |
| Transform stack | `save()`/`restore()` | `saveGState()`/`restoreGState()` | `save()`/`restore()` | PostScript (1984) |

> **Compositing modes** (PorterDuff) формализованы в основополагающей статье Thomas Porter и Tom Duff «Compositing Digital Images» (SIGGRAPH, 1984). Они определили 12 операторов комбинирования двух изображений с alpha-каналом (SRC, DST, SRC_OVER, DST_OVER, SRC_IN, DST_IN и др.) — математически полный набор для цифрового композитинга.

Под капотом Android Canvas использует библиотеку **Skia** (Google) — 2D graphics engine, который транслирует вызовы `drawRect()`, `drawPath()` и т.д. в команды GPU через бэкенд OpenGL ES или Vulkan. Skia реализует **display list** (retained mode) оптимизацию: `View.draw()` записывает команды в `RenderNode`, а `RenderThread` воспроизводит их на GPU. Это разделение следует паттерну **Command** (Gamma et al., 1994).

**Кривые Безье** (Bézier, 1962), используемые в `Path.cubicTo()` и `Path.quadTo()`, — основа всей векторной графики: от шрифтов (TrueType, OpenType) до SVG и Android VectorDrawable. Математически это полиномы Бернштейна степени n, обеспечивающие гладкую интерполяцию через контрольные точки.
...
- **Porter T., Duff T. (1984). Compositing Digital Images. SIGGRAPH.** — 12 операторов композитинга (PorterDuff.Mode)
- **Bézier P. (1962). Définition numérique des courbes et surfaces.** — Кривые Безье, основа Path
- **Newell M. et al. (1972). A Solution to the Hidden Surface Problem.** — Painter's Algorithm
- **Adobe (1984). PostScript Language Reference.** — Модель Canvas + Paint (drawing surface + style)

### Практические руководства

- Meier R. (2022). *Professional Android.* — Canvas drawing, custom views, graphics pipeline
- Phillips B. et al. (2022). *Android Programming: The Big Nerd Ranch Guide.* — Custom views и Canvas drawing
- [Create a custom drawing](https://developer.android.com/develop/ui/views/layout/custom-views/custom-drawing) — Canvas/Paint basics
- [Hardware acceleration](https://developer.android.com/develop/ui/views/graphics/hardware-accel) — HW accel limits
- [Canvas API](https://developer.android.com/reference/android/graphics/Canvas) — Drawing methods
- [PorterDuff.Mode](https://developer.android.com/reference/android/graphics/PorterDuff.Mode) — Blending modes
...
> [!question]- Почему объекты Paint нельзя создавать внутри onDraw() и какое влияние это оказывает?
> onDraw() вызывается каждый кадр (~60 раз в секунду). Создание объектов внутри onDraw() генерирует мусор, вызывая частые GC паузы (~2-5ms каждая). При 60 FPS бюджет на кадр -- 16ms. GC пауза + allocation overhead могут вызвать пропуск кадров. Paint, Path, Rect должны создаваться в init{} и переиспользоваться.

> [!question]- Сценарий: Custom View с градиентом и тенями рендерится медленно. Что оптимизировать?
> 1) Использовать hardware layer (setLayerType LAYER_TYPE_HARDWARE) для кэширования сложной отрисовки. 2) Bitmap caching -- рисовать сложную графику в off-screen Bitmap один раз. 3) Избегать Canvas.saveLayer() (создает off-screen buffer). 4) Упростить Path (меньше точек). 5) Использовать hardware-accelerated operations где возможно.
...
Какие основные операции Canvas?
?
drawRect/drawCircle/drawOval (фигуры), drawPath (произвольные контуры), drawBitmap (изображения), drawText (текст), save/restore (сохранение/восстановление трансформаций), translate/rotate/scale (трансформации).

Что такое Paint и какие свойства настраивает?
?
Определяет как рисовать: color, style (FILL/STROKE), strokeWidth, antiAlias, shader (градиент), pathEffect (пунктир), maskFilter (blur), colorFilter. Переиспользуйте -- не создавайте в onDraw().

Что такое Canvas.save() и restore()?
?
save() сохраняет текущую матрицу трансформаций и clip в стек. restore() восстанавливает. Позволяет применять translate/rotate/scale к части рисования без влияния на остальное.

Как работает hardware layer для View?
?
setLayerType(LAYER_TYPE_HARDWARE) кэширует отрисовку View в GPU текстуру. Последующие кадры используют кэш без повторного вызова onDraw(). Нужно invalidate() при изменении содержимого. Используйте для анимаций alpha/translation.
...
|-------------|------|-------|
| Следующий шаг | [[android-animations]] | Анимации с использованием Canvas |
| Углубиться | [[android-graphics-apis]] | GPU rendering: OpenGL ES и Vulkan |
| Смежная тема | [[ios-graphics-fundamentals]] | Core Graphics в iOS -- аналог Canvas |
