...
> **Layout** в теории GUI-фреймворков — это задача **constraint satisfaction** (удовлетворения ограничений): каждому узлу дерева виджетов назначаются размеры и координаты, удовлетворяющие набору ограничений от родителя и содержимого (Borning et al., *Constraint Hierarchies*, 1992).

Система измерений Android (MeasureSpec) реализует **двухпроходный алгоритм layout** (two-pass layout), типичный для tree-based GUI фреймворков. Первый проход (measure) идёт сверху вниз: родитель формирует ограничения (`MeasureSpec`) для каждого ребёнка. Второй проход (layout) снова сверху вниз: каждый узел получает финальную позицию. Это — частный случай обхода дерева с наследуемыми и синтезируемыми атрибутами из теории **атрибутных грамматик** (Knuth, 1968).

| Система layout | Модель ограничений | Сложность одного прохода | Фреймворк |
|---------------|-------------------|-------------------------|-----------|
| MeasureSpec (Android) | Три режима: EXACTLY, AT_MOST, UNSPECIFIED | O(n) для линейных layouts | Android View |
| Auto Layout (iOS) | Cassowary linear solver | O(n) — O(n³) | UIKit |
| Flexbox (Web) | Главная + поперечная ось | O(n) | CSS |
| Compose Layout | Intrinsic measurements + constraints | O(n), single-pass гарантия | Jetpack Compose |

> **Bit-packing** в MeasureSpec (2 бита mode + 30 бит size в одном `int`) — оптимизация из области **data-oriented design** (Fabian, *Data-Oriented Design*, 2018): минимизация аллокаций через упаковку данных в примитивы. Это критично для measure pass, где создание объектов на heap вызвало бы GC pressure пропорционально глубине дерева.

Проблема **double taxation** (двойного измерения) в `LinearLayout` с `weight` и `RelativeLayout` формализуется через теорию сложности обхода деревьев: если каждый уровень иерархии вызывает измерение дочерних элементов дважды, общая сложность растёт как **O(2^d)**, где d — глубина вложенности. Это экспоненциальный рост, объясняющий, почему Google рекомендует `ConstraintLayout`, решающий constraints за **один проход** через алгоритм Cassowary (Badros et al., 2001).

**ConstraintLayout** использует модифицированный **симплекс-метод** из линейного программирования (Dantzig, 1947) через алгоритм Cassowary, изначально разработанный для системы Auto Layout в macOS/iOS (Borning et al., 1997). Все ограничения выражаются как линейные неравенства и решаются за один проход с амортизированной сложностью O(n).
...
- **Borning A. et al. (1992). Constraint Hierarchies.** — Формализация constraint satisfaction для layout систем
- **Badros G. et al. (2001). The Cassowary Linear Arithmetic Constraint Solving Algorithm.** — Алгоритм, используемый в ConstraintLayout и iOS Auto Layout
- **Knuth D. (1968). Semantics of Context-Free Languages.** — Атрибутные грамматики (наследуемые и синтезируемые атрибуты в обходе деревьев)
- **Fabian R. (2018). Data-Oriented Design.** — Bit-packing и минимизация аллокаций (MeasureSpec)
...
- **Meier R. (2022). Professional Android.** — View measurement process, custom ViewGroup, оптимизация layout passes
- **Vasavada N. (2019). Android Internals.** — ViewRootImpl.performMeasure(), алгоритмы обхода view tree
...
> [!question]- Почему measure pass может вызываться несколько раз для одного View в одном кадре?
> Некоторые ViewGroup (LinearLayout с weight, RelativeLayout) выполняют double measure: первый pass для определения размеров children, второй -- для распределения оставшегося пространства. Глубокая вложенность таких контейнеров приводит к экспоненциальному росту measure вызовов. ConstraintLayout решает это одним pass.

> [!question]- Сценарий: кастомный View корректно отображается в layout preview, но неправильно на устройстве. Что проверить?
> 1) MeasureSpec: preview может передать другой mode (EXACTLY vs AT_MOST). 2) Padding и margin: View должен учитывать getPaddingLeft/Right в onMeasure. 3) setMeasuredDimension() должен вызываться с учетом MeasureSpec constraints. 4) isInEditMode() для preview-specific поведения.
...
Что такое MeasureSpec?
?
32-bit int, кодирующий mode (2 бита) + size (30 бит). Три mode: EXACTLY (точный размер, match_parent), AT_MOST (максимальный размер, wrap_content), UNSPECIFIED (без ограничений, ScrollView).

Как правильно реализовать onMeasure()?
?
1) Получить MeasureSpec для width/height. 2) Рассчитать desired size с учетом padding. 3) Вызвать resolveSize() или MeasureSpec.getSize() для соблюдения constraints. 4) Обязательно вызвать setMeasuredDimension().

Почему ConstraintLayout быстрее RelativeLayout?
?
RelativeLayout выполняет double measure pass (горизонтальный + вертикальный). ConstraintLayout решает constraints за один pass используя solver (Cassowary algorithm). При глубокой вложенности разница O(n) vs O(2^n).

Что делает requestLayout()?
?
Помечает View и всех предков как requiring layout. На следующем VSYNC traversal начинается с корня: measure -> layout для всех помеченных View. Частые вызовы requestLayout() замедляют рендеринг.
...
|-------------|------|-------|
| Следующий шаг | [[android-custom-view-fundamentals]] | Создание кастомных View |
| Углубиться | [[android-view-rendering-pipeline]] | Полный pipeline рендеринга |
| Смежная тема | [[cross-ui-imperative]] | Measurement в разных UI фреймворках |
