...
> **Event dispatching** — механизм маршрутизации событий ввода к целевым обработчикам. В теории GUI-систем выделяют два направления обхода: **capturing** (сверху вниз, от корня к листу) и **bubbling** (снизу вверх, от листа к корню) — модель, формализованная в спецификации DOM Level 2 Events (W3C, 2000).

Android touch dispatching реализует паттерн **Chain of Responsibility** (Gamma et al., 1994): цепочка ViewGroup от корня (DecorView) до целевого View последовательно решает, перехватить событие или передать дальше. Каждый узел может либо обработать событие (`onTouchEvent() → true`), либо делегировать следующему в цепочке.

| Фаза | Направление | Метод | Аналог в DOM Events | Аналог в iOS |
|------|------------|-------|---------------------|-------------|
| Dispatch | Сверху вниз | `dispatchTouchEvent()` | Event capturing | `hitTest(_:with:)` |
| Intercept | Сверху вниз | `onInterceptTouchEvent()` | Нет аналога | Нет прямого аналога |
| Handle | Снизу вверх | `onTouchEvent()` | Event bubbling | `touchesBegan(_:with:)` |

Алгоритм **hit testing** (определение, какой View находится под точкой касания) — это задача **point-in-polygon** из вычислительной геометрии (Preparata/Shamos, 1985). Android выполняет обратный обход (reverse order) дочерних View в ViewGroup, проверяя `contains(x, y)` для каждого — последний добавленный (визуально верхний) View получает приоритет.

> **Gesture recognition** формализуется как задача **pattern recognition на временных рядах**: последовательность `MotionEvent` (ACTION_DOWN → ACTION_MOVE* → ACTION_UP) интерпретируется как жест (tap, swipe, pinch) через конечный автомат с пороговыми значениями (touch slop, velocity threshold).

**Touch slop** (~8dp) — реализация принципа **гистерезиса** (hysteresis) из теории управления: порог, устраняющий ложные срабатывания от физического тремора пальца. Значение калибруется с учётом эргономики — исследования Fitts (1954) показали, что точность касания обратно пропорциональна скорости движения (Fitts's Law: `T = a + b · log₂(D/W + 1)`).

**Nested Scrolling Protocol** (Android 5.0+) формализует кооперативную обработку scroll-событий между parent и child через интерфейсы `NestedScrollingParent`/`NestedScrollingChild`. Это решает классическую проблему **вложенных контейнеров** — конфликт между обработчиками одного типа событий на разных уровнях иерархии.
...
- **Gamma E. et al. (1994). Design Patterns.** — Chain of Responsibility (touch dispatch через иерархию ViewGroup)
- **W3C (2000). DOM Level 2 Events Specification.** — Capturing и bubbling фазы event dispatching
- **Preparata F., Shamos M. (1985). Computational Geometry.** — Hit testing как задача point-in-polygon
- **Fitts P. (1954). The Information Capacity of the Human Motor System.** — Fitts's Law, обосновывающий touch slop
...
- **Meier R. (2022). Professional Android.** — Touch dispatching, GestureDetector, nested scrolling
- **Vasavada N. (2019). Android Internals.** — InputDispatcher, InputChannel, полный путь touch event
...
> [!question]- Почему touch events проходят через иерархию View сверху вниз (dispatch), а затем снизу вверх (handle)?
> Dispatch (сверху вниз) позволяет родительским ViewGroup перехватить события до children (onInterceptTouchEvent). Handle (снизу вверх) дает детям приоритет в обработке. Это реализует паттерн Chain of Responsibility: ScrollView может перехватить вертикальный свайп, а Button внутри -- обработать tap.

> [!question]- Сценарий: ScrollView внутри ViewPager. Вертикальный скролл работает, горизонтальный свайп -- нет. Как исправить?
> ViewPager.onInterceptTouchEvent() должен перехватывать горизонтальные свайпы. Проблема: ScrollView потребляет все touch events. Решение: requestDisallowInterceptTouchEvent(true) в ScrollView для вертикального движения. Или использовать NestedScrollView с NestedScrollingParent/Child protocol.
...
Какие три метода участвуют в touch dispatch?
?
dispatchTouchEvent() -- точка входа, вызывается системой. onInterceptTouchEvent() -- только для ViewGroup, решает перехватывать ли события у children. onTouchEvent() -- обработка события самим View.

Что такое touch gesture и как распознать?
?
Последовательность MotionEvent: ACTION_DOWN -> ACTION_MOVE* -> ACTION_UP. GestureDetector распознает: onSingleTapUp, onDoubleTap, onLongPress, onFling, onScroll. ScaleGestureDetector для pinch-to-zoom.

Что такое MotionEvent и какие действия бывают?
?
Объект с координатами и действием: ACTION_DOWN (палец коснулся), ACTION_MOVE (движение), ACTION_UP (палец убран), ACTION_CANCEL (жест отменен родителем), ACTION_POINTER_DOWN/UP (мультитач).

Что такое touch slop?
?
Минимальное расстояние (обычно 8dp) которое палец должен пройти, чтобы движение считалось drag, а не tap. ViewConfiguration.getScaledTouchSlop(). Предотвращает случайные drag при попытке tap.

Как работает requestDisallowInterceptTouchEvent()?
?
Child View говорит родителю: не перехватывай мои события. Родительский ViewGroup перестает вызывать onInterceptTouchEvent(). Сбрасывается при ACTION_DOWN. Используется для разрешения конфликтов вложенного скролла.
...
|-------------|------|-------|
| Следующий шаг | [[android-animations]] | Анимации, управляемые touch событиями |
| Углубиться | [[android-window-system]] | Как WMS dispatch touch events в Activity |
| Смежная тема | [[ios-touch-interaction]] | Сравнение touch handling iOS и Android |
