...
> **Rendering pipeline** — последовательность этапов преобразования описания сцены в растровое изображение на экране. В компьютерной графике это понятие формализовано в модели **graphics pipeline** (Foley et al., *Computer Graphics: Principles and Practice*, 1990), где каждый этап является чистой функцией: `input → transform → output`.

Android Rendering Pipeline реализует классическую архитектуру **двойной буферизации** (double buffering), впервые описанную в контексте растровой графики Sproull/Sutherland (1968). Triple buffering, введённый в Project Butter (Android 4.1, 2012), добавляет третий буфер для устранения pipeline stalls — когда GPU не успевает завершить предыдущий кадр до следующего VSYNC.

| Этап pipeline | Аналог в классической CG | Поток выполнения | Введён в Android |
|--------------|--------------------------|-----------------|-----------------|
| Measure/Layout | Scene graph traversal | Main Thread | 1.0 (2008) |
| Draw (DisplayList) | Display list compilation | Main Thread | 3.0 (2011) |
| Sync & Upload | Texture upload | RenderThread | 5.0 (2014) |
| GPU Render | Rasterization | GPU | 3.0 (2011) |
| Composition | Compositing (Porter-Duff) | SurfaceFlinger | 1.0 (2008) |

Концепция **VSYNC** (Vertical Synchronization) берёт начало из эпохи CRT-мониторов, где электронный луч сканировал экран строка за строкой сверху вниз. Сигнал VSYNC отмечал момент возврата луча в верхний левый угол — безопасный момент для переключения буферов. В современных LCD/OLED дисплеях физического луча нет, но VSYNC сохраняется как механизм синхронизации (обычно 60Hz = 16.67ms, 90Hz = 11.11ms, 120Hz = 8.33ms).

> **Choreographer** (Android 4.1+) реализует паттерн **Mediator** (Gamma et al., 1994): централизует координацию между input events, анимациями и traversal passes, гарантируя их выполнение в правильном порядке на каждом VSYNC-цикле.

**RenderThread** (Android 5.0+) — применение принципа **pipeline parallelism** из теории параллельных вычислений (Hennessy/Patterson, *Computer Architecture*, 2011). Main Thread формирует DisplayList для кадра N+1, пока RenderThread рендерит кадр N на GPU. Это удваивает пропускную способность pipeline при тех же аппаратных ресурсах.

Метрика **overdraw** связана с понятием **fill rate** в GPU-архитектуре — количество пикселей, которые GPU может обработать за единицу времени. Каждый лишний слой отрисовки одного пикселя линейно увеличивает нагрузку на fill rate, что формализуется как `cost = Σ(pixels × layers)` (Akenine-Moller et al., *Real-Time Rendering*, 2018).
...
- **Foley J. et al. (1990). Computer Graphics: Principles and Practice.** — Графический конвейер, double buffering, растеризация
- **Akenine-Moller T. et al. (2018). Real-Time Rendering, 4th ed.** — Fill rate, overdraw, GPU pipeline
- **Hennessy J., Patterson D. (2011). Computer Architecture: A Quantitative Approach.** — Pipeline parallelism (Main Thread ↔ RenderThread)
- **Sproull R., Sutherland I. (1968).** — Основы double buffering для растровых дисплеев
...
- **Vasavada N. (2019). Android Internals.** — ViewRootImpl, Choreographer, RenderThread, SurfaceFlinger
- **Meier R. (2022). Professional Android.** — Rendering optimization на практике
...
> [!question]- Почему Choreographer привязывает рендеринг к VSYNC-сигналу?
> Без синхронизации с VSYNC рендеринг может начаться в середине refresh цикла дисплея, вызывая tearing (часть экрана показывает старый кадр, часть -- новый). Choreographer ждет VSYNC-сигнал и запускает input -> animation -> measure/layout/draw в правильном порядке, обеспечивая плавный 60 FPS.

> [!question]- Сценарий: Systrace показывает, что один кадр занял 32ms. Как определить bottleneck?
> В Systrace проверьте три фазы: measure/layout (слишком глубокая иерархия View), draw (сложные overdraw, тяжелые Canvas операции), GPU completion (сложные шейдеры, большие текстуры). Используйте GPU Overdraw debug option для визуализации. Profile GPU Rendering покажет время каждой фазы.
...
Что такое VSYNC?
?
Vertical Synchronization -- сигнал от дисплея (~60Hz = каждые 16.6ms). Choreographer использует VSYNC для синхронизации рендеринга: input -> animation -> traversal (measure/layout/draw) -> GPU rendering -> display.

Что такое RenderThread?
?
Отдельный поток (с Android 5.0) для GPU-операций. Main Thread создает DisplayList (список команд рисования), RenderThread выполняет их на GPU. Разделение позволяет Main Thread начать следующий кадр пока GPU рисует текущий.

Что такое hardware acceleration?
?
Рендеринг через GPU вместо CPU. Включен по умолчанию с Android 3.0. View.draw() записывает команды в DisplayList, RenderThread отправляет на GPU. Некоторые Canvas операции не поддерживаются (software rendering fallback).

Что такое overdraw?
?
Отрисовка одного пикселя несколько раз за кадр. Каждый View рисует свой background, потом content. Белый background Activity + белый background CardView = 2x overdraw. Цель: максимум 2x для 90% экрана.

Как invalidate() запускает перерисовку?
?
invalidate() помечает View как dirty. На следующем VSYNC Choreographer запускает traversal: dirty View проходят draw(). requestLayout() дополнительно запускает measure/layout. performTraversals() в ViewRootImpl координирует весь процесс.
...
|-------------|------|-------|
| Следующий шаг | [[android-view-measurement]] | Как работает measure pass детально |
| Углубиться | [[android-window-system]] | Window Manager и Surface под капотом |
| Смежная тема | [[ios-view-rendering]] | Сравнение rendering pipeline iOS и Android |
