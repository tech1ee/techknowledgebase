...
> **View hierarchy** — это экземпляр паттерна **Composite** (Gamma et al., 1994): `View` — лист, `ViewGroup` — композит, оба реализуют единый интерфейс отрисовки и обработки событий. Дерево обходится рекурсивно за три прохода: measure → layout → draw.

Концептуально View System Android наследует модель **Immediate Mode vs Retained Mode GUI**. В отличие от immediate mode (OpenGL, ImGui), где приложение перерисовывает весь экран каждый кадр, View System использует **retained mode**: фреймворк хранит дерево объектов и перерисовывает только изменённые поддеревья через `invalidate()`.

| Характеристика | Retained Mode (Android View) | Immediate Mode (ImGui) | Declarative (Compose) |
|---------------|------------------------------|------------------------|-----------------------|
| Состояние UI | Хранится в объектах View | Нет — перерисовка каждый кадр | Функция от State |
| Обновление | `invalidate()` / `requestLayout()` | Полная перерисовка | Recomposition |
| Происхождение | Smalltalk MVC (1979) | SGI GL (1992) | React (2013) |
| Платформы | Android View, UIKit, WPF | OpenGL, Dear ImGui | Compose, SwiftUI, Flutter |

Исторически GUI-фреймворки следуют эволюции от процедурного кода (Xlib, Win32 GDI) через объектно-ориентированные иерархии (MFC, Swing, UIKit, Android View) к декларативным моделям ([[android-compose|Jetpack Compose]], SwiftUI). Android View System (2008) проектировался под влиянием Java Swing и Microsoft WPF, заимствуя паттерн **LayoutInflater** как аналог XAML parser.

> **XML Layout** — реализация принципа **разделения представления и логики** (Separation of Concerns, Dijkstra 1974). Декларативное описание UI в XML отделяет структуру интерфейса от поведения в коде, что позволяет дизайнерам и разработчикам работать параллельно.

Паттерн **Adapter** в RecyclerView формализует связь между доменной моделью и представлением, восходя к определению Gamma et al. (1994): «конвертирует интерфейс класса в другой интерфейс, ожидаемый клиентом». ViewHolder при этом реализует **Object Pool** (Kircher/Jain, 2004) — повторное использование дорогостоящих объектов View вместо их пересоздания.
...
- **Gamma E. et al. (1994). Design Patterns: Elements of Reusable Object-Oriented Software.** — Паттерны Composite (View hierarchy), Adapter (RecyclerView Adapter), Observer (View invalidation), Strategy (LayoutManager)
- **Foley J. et al. (1990). Computer Graphics: Principles and Practice.** — Retained Mode vs Immediate Mode GUI, rendering pipeline
- **Kircher M., Jain P. (2004). Pattern-Oriented Software Architecture, Vol. 3.** — Object Pool pattern, лежащий в основе ViewHolder/RecyclerView
- **Krasner G., Pope S. (1988). A Description of the MVC Framework.** — Smalltalk MVC как исторический предшественник View System
...
- **Phillips B. et al. (2022). Android Programming: The Big Nerd Ranch Guide.** — Лучшее пошаговое введение в View System
- **Meier R. (2022). Professional Android.** — View hierarchy, custom views, RecyclerView optimization
- **Vasavada N. (2019). Android Internals.** — LayoutInflater, View rendering pipeline и window system
...
> [!question]- Почему Android UI toolkit не thread-safe и что происходит при обращении к View из фонового потока?
> View system спроектирован для работы в одном потоке (Main Thread) для простоты и производительности. Добавление синхронизации замедлило бы рендеринг каждого кадра. При обращении из фонового потока -- CalledFromWrongThreadException. Для обновления UI из фонового потока используйте Dispatchers.Main, runOnUiThread() или Handler(Looper.getMainLooper()).

> [!question]- Сценарий: XML layout глубиной вложенности 15 уровней рендерится медленно. Почему и как исправить?
> Каждый уровень вложенности проходит measure->layout->draw. Глубокая иерархия приводит к экспоненциальному росту вызовов measure (O(2^n) в worst case). Решение: ConstraintLayout (плоская иерархия), merge tag для устранения лишних ViewGroup, ViewStub для отложенной инфляции. Или переход на Compose.
...
Какие три фазы рендеринга View?
?
Measure (определение размеров), Layout (определение позиций), Draw (отрисовка на Canvas). Каждая фаза проходит по всей иерархии View сверху вниз. Весь процесс должен уложиться в 16ms для 60 FPS.

Чем ViewBinding отличается от findViewById?
?
ViewBinding генерирует binding-класс с типизированными ссылками на все View с id. Null-safe, type-safe, compile-time проверки. Нет runtime overhead как у findViewById (поиск по дереву View).

Что такое LayoutInflater?
?
Сервис для создания View из XML layout. Парсит XML, создает View через reflection (конструктор с AttributeSet), строит иерархию. Требует Context с темой для resolve атрибутов.

Чем match_parent отличается от wrap_content?
?
match_parent -- View занимает весь доступный размер родителя. wrap_content -- View занимает минимальный размер для своего содержимого. Это MeasureSpec: EXACTLY для match_parent, AT_MOST для wrap_content.

Что такое ViewStub?
?
Легковесный View (0x0), который inflate layout только при setVisibility(VISIBLE) или inflate(). Экономит ресурсы для редко показываемых View (error states, empty states). После inflate заменяется реальным View.
...
|-------------|------|-------|
| Следующий шаг | [[android-view-rendering-pipeline]] | Как View рендерится кадр за кадром |
| Углубиться | [[android-recyclerview-internals]] | RecyclerView -- самый сложный View |
| Смежная тема | [[ios-swiftui-vs-uikit]] | Сравнение Android View system и UIKit |
