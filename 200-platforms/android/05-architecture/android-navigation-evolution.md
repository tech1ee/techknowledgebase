...
> [!question]- Почему Fragment-based навигация уступает Compose Navigation для новых проектов?
> Fragment навигация: lifecycle complexity (Fragment + View lifecycle), XML overhead, FragmentManager state restoration баги, не type-safe аргументы (bundle). Compose Navigation: один lifecycle (composition), type-safe routes (Navigation 2.8+), нет XML, проще тестировать. Fragment нужен только для legacy или interop с View-based UI.

> [!question]- Какую проблему решают third-party навигационные библиотеки (Voyager, Decompose)?
> Jetpack Navigation не поддерживает KMP (привязан к Android). Decompose: навигация в shared code (KMP), собственный lifecycle, state restoration. Voyager: Compose-first, проще API, KMP support. Circuit (Square): MVI + навигация. Выбор зависит от: KMP нужен? Сложность навигации? Размер команды?
...
Какие этапы эволюции навигации в Android?
?
1) Intent для Activity (2008). 2) FragmentTransaction (2011). 3) Navigation Component (2018, Jetpack). 4) Compose Navigation (2021). 5) Type-safe Navigation (2024, Navigation 2.8). Тренд: от императивного к декларативному.

Что такое Decompose?
?
KMP навигационная библиотека от Arkadii Ivanov. Предоставляет: Component lifecycle, child stack/pages/slot, state saving/restoration. Навигационная логика в shared code, UI platform-specific. Используется в крупных KMP проектах.

Что изменилось в Navigation 2.8?
?
Type-safe navigation: routes определяются как Kotlin Serializable классы вместо строк. Аргументы передаются через свойства класса. Compile-time проверки вместо runtime crashes. Нет необходимости в Safe Args plugin.

Почему single-Activity архитектура стала стандартом?
?
Одна Activity + навигация между Fragment/Composable. Преимущества: shared ViewModel, единый NavGraph, проще анимации переходов, один backstack. Multiple Activity оправдан только для разных entry points (notifications, shortcuts).
...
|-------------|------|-------|
| Следующий шаг | [[android-navigation]] | Детальное руководство по навигации |
| Углубиться | [[kmp-navigation]] | Навигация в KMP проектах |
| Смежная тема | [[ios-navigation]] | Эволюция навигации в iOS |
