...
?
Абстракция доступа к данным. Repository решает откуда брать данные: кэш, БД, сеть. ViewModel не знает об источнике. Позволяет менять реализацию (Room -> SQLDelight) без изменения бизнес-логики.
...
|-------------|------|-------|
| Следующий шаг | [[android-viewmodel-internals]] | Как ViewModel работает под капотом |
| Углубиться | [[android-modularization]] | Архитектура модулей для масштабирования |
| Смежная тема | [[ios-architecture-patterns]] | Сравнение архитектуры iOS и Android |
