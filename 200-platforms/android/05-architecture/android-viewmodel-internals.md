...
> **ViewModel** — реализация паттерна **Presentation Model** (Fowler, 2004): объект, моделирующий состояние и логику представления, не зависящий от конкретного UI-фреймворка. На Android ViewModel дополнительно решает проблему **object retention** — сохранения состояния через configuration change.

Механизм выживания ViewModel при rotation основан на `NonConfigurationInstance` — специальном механизме `Activity`, который **не уничтожается** при configuration change. Это реализация паттерна **Memento** (Gamma et al., 1994) на уровне фреймворка: `ViewModelStore` сохраняется как non-configuration instance, а `ViewModel` объекты внутри него остаются в памяти.

| Механизм | Что выживает | Что не выживает | Паттерн |
|----------|-------------|-----------------|---------|
| ViewModel (RAM) | Config change, Fragment recreation | Process death, finish() | Object retention |
| SavedStateHandle (Bundle) | Config change, process death | Uninstall, clear data | Memento / Serialization |
| Room / DataStore (Disk) | Всё, кроме uninstall | Uninstall | Persistence |

> **ViewModelStore** — реализация паттерна **Registry** (Fowler, 2002): хранилище именованных объектов с lifecycle, привязанным к Activity/Fragment/NavBackStackEntry. Ключ — каноническое имя класса ViewModel, значение — экземпляр ViewModel. Это объясняет, почему `by viewModels<MyViewModel>()` возвращает тот же экземпляр после rotation.

**Scoping** ViewModel (Activity scope, Fragment scope, Navigation scope) — формализация **lifetime containment** (Boehm, 2003): время жизни ViewModel ограничено временем жизни его scope owner. Неправильный scope — частая причина багов: Activity-scoped ViewModel живёт дольше Fragment, что может привести к stale data или memory leaks.
...
- **Fowler M. (2004). Presentation Model.** — Оригинальный паттерн, основа ViewModel
- **Gamma E. et al. (1994). Design Patterns.** — Memento (SavedStateHandle), Registry (ViewModelStore)
- **Boehm H.-J. (2003). Destructors, Finalizers, and Synchronization.** — Lifetime containment (scoping)
...
- [ViewModel Source Code](https://cs.android.com/androidx/platform/frameworks/support/+/androidx-main:lifecycle/lifecycle-viewmodel/)
- [Testing ViewModel](https://developer.android.com/training/testing/unit-testing/viewmodel-testing)
- **Phillips B. et al. (2022). Android Programming: The Big Nerd Ranch Guide.** — ViewModel, SavedStateHandle
- **Moskala M. (2021). Effective Kotlin.** — Immutable state, data classes для ViewModel
...
> [!question]- Почему ViewModel переживает Configuration Change, но не Process Death?
> ViewModel хранится в ViewModelStore, который привязан к NonConfigurationInstances -- объекту, который система передает от старой Activity к новой при rotation. При Process Death вся память процесса освобождается, включая ViewModelStore. Для process death нужен SavedStateHandle, который сериализует данные в Bundle и сохраняет через системный механизм.

> [!question]- Сценарий: два Fragment используют shared ViewModel через activityViewModels(). Как данные синхронизируются?
> activityViewModels() привязывает ViewModel к ViewModelStore Activity (не Fragment). Оба Fragment получают одну и ту же instance. При изменении StateFlow в ViewModel оба Fragment получают update. ViewModel живет пока Activity жива. Для communication: Fragment A -> event -> ViewModel -> state -> Fragment B.
...
Как ViewModel переживает rotation?
?
Activity.onRetainNonConfigurationInstances() сохраняет ViewModelStore перед destroy. Новая Activity получает его через getLastNonConfigurationInstance(). ViewModel достается из ViewModelStore по ключу (class name). Памяти не пересоздается.

Что такое SavedStateHandle?
?
Map-like объект для сохранения данных при Process Death. Сериализуется в Bundle. ViewModel получает через конструктор (Hilt инжектит). Поддерживает getLiveData/getStateFlow для реактивного доступа. Лимит ~500KB.

Что такое ViewModelProvider.Factory?
?
Фабрика для создания ViewModel с параметрами конструктора. По умолчанию ViewModel создается через reflection (no-arg constructor). Factory позволяет передать зависимости. Hilt заменяет factory через @HiltViewModel + @Inject constructor.

Как ViewModelScope связан с lifecycle?
?
viewModelScope -- CoroutineScope привязанный к ViewModel. Автоматически отменяется в onCleared(). Использует Dispatchers.Main.immediate. Все корутины в ViewModel запускаются в этом scope.

Что такое CreationExtras?
?
Механизм передачи параметров в ViewModelProvider.Factory без кастомных фабрик. DEFAULT_ARGS_KEY для SavedStateHandle, APPLICATION_KEY для Application. Упрощает создание ViewModel с зависимостями.
...
|-------------|------|-------|
| Следующий шаг | [[android-state-management]] | State management через ViewModel + StateFlow |
| Углубиться | [[android-architecture-patterns]] | Как ViewModel вписывается в архитектуру |
| Смежная тема | [[ios-viewmodel-patterns]] | ViewModel patterns в iOS (ObservableObject) |
