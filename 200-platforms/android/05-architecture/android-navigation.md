...
> [!question]- Почему Navigation Component предпочтительнее ручного FragmentTransaction?
> Ручной FragmentTransaction: легко забыть addToBackStack(), нет type-safety для аргументов, сложная логика вложенных графов, нет deep link support. Navigation Component: декларативный граф, Safe Args (type-safe аргументы), автоматический backstack, deep links, интеграция с BottomNavigation и Toolbar.

> [!question]- Сценарий: приложение должно поддерживать deep link example.com/user/123 и открывать экран профиля. Как реализовать?
> 1) В nav_graph: добавить deepLink с pattern 'example.com/user/{userId}'. 2) В Manifest: intent-filter с VIEW action и BROWSABLE category для Activity. 3) Для App Links: добавить Digital Asset Links файл на сервер. 4) Navigation Component автоматически parse аргумент userId и передаст в destination.
...
Какие подходы к навигации есть в Android?
?
1) Fragment transactions (ручные). 2) Navigation Component (Jetpack, декларативный граф). 3) Compose Navigation (для Compose UI). 4) Third-party: Voyager, Decompose, Circuit. Для новых проектов: Compose Navigation или third-party.

Что такое NavGraph?
?
Декларативный граф экранов и переходов. XML или Kotlin DSL. Содержит destinations (экраны), actions (переходы), arguments (параметры). Поддерживает nested graphs для модуляризации.

Что такое Safe Args?
?
Gradle plugin для Navigation Component. Генерирует type-safe классы для аргументов навигации. Вместо bundle.putString('key', value) -- NavDirections с типизированными параметрами. Ошибки ловятся на этапе компиляции.

Как работает deep linking в Navigation?
?
NavDeepLink определяет URI pattern. При intent с matching URI Navigation Component находит destination в NavGraph, создает синтетический backstack (стартовый экран + целевой), передает аргументы из URI.

Что такое Compose Navigation?
?
Navigation для Compose UI. NavHost с composable destinations. Type-safe routes (с Navigation 2.8+). Нет Fragment overhead. Каждый destination -- composable функция с NavBackStackEntry для аргументов.
...
|-------------|------|-------|
| Следующий шаг | [[android-navigation-evolution]] | История и эволюция навигации в Android |
| Углубиться | [[android-modularization]] | Навигация в многомодульных проектах |
| Смежная тема | [[ios-navigation]] | Навигация в iOS: UINavigationController и SwiftUI |
