...
> **Clean Architecture** (Martin, 2012) — архитектурная модель, основанная на **Dependency Rule**: исходный код зависимостей должен указывать только внутрь, от внешних слоёв (frameworks, UI) к внутренним (entities, use cases). Это обобщение трёх предшествующих концепций: **Hexagonal Architecture** (Cockburn, 2005), **Onion Architecture** (Palermo, 2008) и **BCE** (Jacobson, 1992).

| Год | Автор | Концепция | Ключевая идея |
|-----|-------|-----------|---------------|
| 1992 | Jacobson | BCE (Boundary-Control-Entity) | Три типа объектов по роли |
| 2003 | Evans | Domain-Driven Design | Ubiquitous Language, Bounded Context |
| 2005 | Cockburn | Hexagonal (Ports & Adapters) | Приложение не знает, кто вызывает его |
| 2008 | Palermo | Onion Architecture | Концентрические слои, зависимости внутрь |
| 2012 | Martin | Clean Architecture | Обобщение всех трёх + Dependency Rule |

> **Use Case** (Jacobson, 1992) — формальное описание одного сценария взаимодействия актора с системой. В Clean Architecture Use Case инкапсулирует **одну бизнес-операцию**, реализуя принцип **Single Responsibility** (Martin, 2003). На Android Use Case — это класс с одним публичным методом (`operator fun invoke()`), координирующий вызовы Repository.

Теоретическая основа Dependency Rule — **Dependency Inversion Principle** (Martin, 1996): высокоуровневые модули не должны зависеть от низкоуровневых; оба должны зависеть от абстракций. На практике: Domain Layer определяет интерфейс `UserRepository`, а Data Layer предоставляет реализацию `UserRepositoryImpl`. Это позволяет заменить Retrofit на Ktor или Room на SQLDelight без изменения бизнес-логики.

**Критика:** Clean Architecture на мобильных — часто **over-engineering**. Fowler отмечает, что архитектурные слои оправданы, только когда выгода от абстракции превышает стоимость indirection (*Patterns of Enterprise Application Architecture*, 2002). Для экрана с одним API-вызовом Use Case — это бессмысленный pass-through.
...
| Источник | Тип | Вклад |
|----------|-----|-------|
| Martin R. (2012). "The Clean Architecture" | Блог-пост (оригинал) | Dependency Rule, концентрические слои |
| Martin R. (2017). *Clean Architecture* | Книга | Полное изложение с примерами |
| Cockburn A. (2005). Hexagonal Architecture | Статья | Ports & Adapters — предшественник Clean |
| Jacobson I. (1992). *Object-Oriented Software Engineering* | Книга | BCE (Boundary-Control-Entity), Use Cases |
| Evans E. (2003). *Domain-Driven Design* | Книга | Ubiquitous Language, Bounded Context |
| Fowler M. (2002). *Patterns of Enterprise Application Architecture* | Книга | Критика чрезмерной слоистости |
...
| Fernando Cejas "Architecting Android" (2014) | Блог-пост | [fernandocejas.com](https://fernandocejas.com/2014/09/03/architecting-android-the-clean-way/) |
| UseCase Red Flags (Teknasyon) | Блог-пост | [engineering.teknasyon.com](https://engineering.teknasyon.com/usecase-red-flags-and-best-practices-in-clean-architecture-76e2f6d921eb) |
