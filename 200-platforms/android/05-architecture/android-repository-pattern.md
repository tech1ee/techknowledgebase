...
> [!question]- Почему Repository должен быть единственным источником данных для ViewModel?
> Single Source of Truth: если ViewModel обращается напрямую к API и DB, возникают рассинхронизации (API вернул новые данные, но в DB старые). Repository координирует: запрос к API -> сохранение в DB -> возврат из DB. ViewModel всегда получает consistent данные из одного источника.

> [!question]- Сценарий: приложение должно работать offline. Как Repository реализует offline-first?
> Стратегия: 1) UI подписан на Flow из Room (всегда актуальные данные). 2) При запросе: вернуть кэш из Room + запустить сеть в фоне. 3) При получении из сети: сохранить в Room. 4) Room Flow автоматически уведомит UI. 5) При отсутствии сети: показать кэш + индикатор offline. NetworkBoundResource паттерн.
...
?
Абстракция доступа к данным. Repository скрывает источники (API, DB, cache) от ViewModel. Определяет стратегию: cache-first, network-first, offline-first. ViewModel вызывает repository.getUsers(), не зная об источнике.

Что такое NetworkBoundResource?
?
Паттерн для offline-first: 1) Вернуть кэш из DB. 2) Если stale -- запросить сеть. 3) Сохранить результат в DB. 4) DB уведомляет UI через Flow. Реализуется как inline function или abstract class.

Что такое DataSource?
?
Низкоуровневый класс для работы с одним источником: RemoteDataSource (API через Retrofit/Ktor), LocalDataSource (Room/DataStore). Repository комбинирует несколько DataSource для реализации стратегии.

Как обрабатывать ошибки в Repository?
?
Sealed class Result<T>: Success(data), Error(exception, cachedData?), Loading. Repository оборачивает вызовы в try/catch и возвращает Result. ViewModel маппит Result в UI State. При ошибке сети -- вернуть кэш + ошибку.

Зачем Repository возвращает Flow, а не suspend?
?
Flow обеспечивает реактивные обновления: Room Flow автоматически emit при изменении данных. suspend возвращает один раз. С Flow: UI подписан на поток данных, любое изменение (сеть, другой экран) автоматически отражается в UI.
...
|-------------|------|-------|
| Следующий шаг | [[android-room-deep-dive]] | Room как локальный источник данных |
| Углубиться | [[android-networking]] | Networking: Retrofit, Ktor, OkHttp |
| Смежная тема | [[ios-repository-pattern]] | Repository Pattern в iOS |
