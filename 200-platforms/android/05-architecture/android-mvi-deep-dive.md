...
> **MVI (Model-View-Intent)** — архитектурный паттерн, формализующий UI как **конечный автомат** (FSM): состояние экрана `S` детерминированно переходит в новое состояние `S'` при получении intent `I` через чистую функцию `reduce: (S, I) → S'`. Это прямая реализация **State Machine** (Hopcroft/Ullman, 1979) на уровне архитектуры.

Формально MVI описывается тройкой:
- **Model** — единственный неизменяемый state объект (`data class UiState`)
- **View** — чистая функция отрисовки `render: State → UI`
- **Intent** — замкнутое множество пользовательских действий (`sealed class`)

| Свойство | Формальное определение | Следствие для Android |
|----------|----------------------|----------------------|
| Детерминизм | `reduce(s, i)` всегда даёт один результат | Воспроизводимые баги, time-travel debugging |
| Иммутабельность | State — `data class` с `val` полями | Thread-safety без synchronization |
| Единый источник | Один `StateFlow<State>` на экран | Нет рассинхронизации между полями UI |
| Замкнутость | `sealed class Intent` — конечное множество | Exhaustive `when` — компилятор проверяет все случаи |

> **Reducer** — концепция из функционального программирования: `fold` / `reduce` — свёртка последовательности значений в единый результат. В MVI reducer принимает текущее состояние и intent, возвращая новое состояние. Это тот же `reduce`, что в `List.fold()` Kotlin — применённый к потоку UI-событий.

Теоретические корни MVI лежат в **Elm Architecture** (Czaplicki, 2012): `Model → Update → View` — три чистые функции без побочных эффектов. Redux (Abramov, 2015) популяризировал эту модель для JavaScript. Hannes Dorfmann (2016) адаптировал концепцию для Android как MVI, добавив обработку Android-специфичных side effects.
...
- **Hopcroft J., Ullman J. (1979). Introduction to Automata Theory.** — FSM, формализация state transitions
- **Czaplicki E. (2012). Elm: Concurrent FRP for Functional GUIs.** — Elm Architecture (Model-Update-View)
- **Abramov D. (2015). Redux.** — Reducer pattern, single store, middleware
- **Fowler M. (2005). Event Sourcing.** — State как результат применения событий
- **Staltz A. (2015). Unidirectional User Interface Architectures.** — Обзор UDF-архитектур

### Практические руководства

| # | Название | URL | Тип |
|---|---------|-----|-----|
| 1 | Orbit MVI | [orbit-mvi.org](https://orbit-mvi.org/) | Документация |
| 2 | MVIKotlin | [arkivanov.github.io/MVIKotlin](https://arkivanov.github.io/MVIKotlin/) | Документация |
| 3 | Ballast | [copper-leaf.github.io/ballast](https://copper-leaf.github.io/ballast/) | Документация |
| 4 | FlowMVI | [opensource.respawn.pro/FlowMVI](https://opensource.respawn.pro/FlowMVI/) | Документация |
| 5 | Hannes Dorfmann — MVI | [hannesdorfmann.com/android/mosby3-mvi-1](http://hannesdorfmann.com/android/mosby3-mvi-1/) | Серия статей |
