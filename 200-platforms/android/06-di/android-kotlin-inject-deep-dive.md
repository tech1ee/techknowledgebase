...
> [!question]- Почему kotlin-inject набирает популярность для KMP проектов?
> kotlin-inject: compile-time DI (как Dagger), KSP-based (работает на всех KMP targets), pure Kotlin API (без Java annotations). В отличие от Koin: ошибки графа видны при компиляции. В отличие от Hilt: работает на iOS, Desktop, Web. Для KMP это лучший compile-time DI вариант.

> [!question]- Сценарий: мигрируете с Hilt на kotlin-inject для KMP. Какие основные изменения?
> 1) @Component interface вместо Hilt предопределенных components. 2) @Inject constructor остается. 3) @Provides в companion object интерфейса Component. 4) Нет автоматического Android lifecycle scoping -- нужно управлять вручную. 5) Нет @AndroidEntryPoint -- inject через component instance. 6) Scoping через @Scope annotations.
...
Что такое kotlin-inject?
?
Compile-time DI фреймворк для Kotlin, основанный на KSP. Аналог Dagger для Kotlin. Генерирует код при компиляции. Поддерживает все KMP targets. API через Kotlin interfaces и annotations.

Как объявить Component в kotlin-inject?
?
@Component abstract class AppComponent { abstract val userRepo: UserRepository; companion object { @Provides fun apiService(): ApiService = RetrofitApiService() } }. KSP генерирует реализацию: val component = AppComponent::class.create().

Как kotlin-inject обрабатывает scoping?
?
@Scope annotation создает scope. @Inject @AppScope class UserRepository(...) -- singleton в AppScope. Component с @AppScope создает и хранит scoped instances. Без scope -- новый instance каждый раз.

Чем kotlin-inject отличается от Dagger?
?
kotlin-inject: KSP (быстрее KAPT), pure Kotlin (нет Java annotations), KMP support, simpler API. Dagger: KAPT (только JVM), Java annotations, Android-centric, больше features (multibinding, assisted inject). kotlin-inject достаточен для большинства проектов.

Как тестировать с kotlin-inject?
?
Создать test Component с override bindings. @Component abstract class TestComponent : AppComponent { @Provides fun fakeRepo(): UserRepository = FakeUserRepository() }. Или использовать конструктор injection напрямую в тестах (проще, без DI фреймворка).
...
|-------------|------|-------|
| Следующий шаг | [[android-metro-deep-dive]] | Metro -- новый DI от JetBrains |
| Углубиться | [[android-dagger-deep-dive]] | Dagger для сравнения с kotlin-inject |
| Смежная тема | [[kmp-di-patterns]] | DI patterns в KMP проектах |
