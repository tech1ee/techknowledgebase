...
> **Koin** — runtime DI framework, реализующий паттерн **Service Locator** (Fowler, 2004) с Kotlin DSL для декларативной конфигурации. В отличие от Dagger/Hilt, Koin не использует code generation и annotation processing — зависимости разрешаются в runtime через HashMap-based реестр.

Формально Koin ближе к **Service Locator**, чем к DI: объект запрашивает зависимости из контейнера через `get()` или `inject()` (pull-model), а не получает их через конструктор (push-model). Fowler (2004) считал Service Locator допустимой альтернативой DI для простых приложений.

| Характеристика | Koin (Service Locator) | Hilt/Dagger (DI) |
|---------------|----------------------|-------------------|
| Разрешение зависимостей | Runtime (HashMap lookup) | Compile-time (generated code) |
| Обнаружение ошибок | При запуске / в runtime | При компиляции |
| Code generation | Нет | Да (annotation processing) |
| KMP support | Полная | Hilt: нет, kotlin-inject: да |
| DSL | Kotlin DSL (`module { }`) | Annotations (`@Module`, `@Provides`) |

> **Kotlin DSL** (Domain-Specific Language) — Koin использует **type-safe builders** из Kotlin для декларативной конфигурации: `module { single { UserRepositoryImpl(get()) } }`. Это реализация паттерна **Builder** (Gamma et al., 1994) через Kotlin lambda with receiver, что обеспечивает читаемость без потери type-safety.

Koin `verify()` (Koin 3.3+) — попытка добавить **compile-time-like проверки** к runtime DI: unit-тест вызывает `verify()` для валидации графа зависимостей. Это не эквивалент compile-time safety Dagger (ошибка обнаруживается при запуске тестов, а не при компиляции), но значительно повышает надёжность.
...
- **Fowler M. (2004). Inversion of Control Containers and the DI Pattern.** — Service Locator vs DI
- **Gamma E. et al. (1994). Design Patterns.** — Builder (Kotlin DSL), Factory (module definitions)
...
- **Moskala M.** *Effective Kotlin* (2021) — DI паттерны и DSL-конструкции
- **Meier R.** *Professional Android* (2022) — DI фреймворки в Android
...
> [!question]- Почему Koin проще Hilt, но менее безопасен?
> Koin: runtime DI через DSL (module { single { }, factory { } }). Простой API, нет code generation, нет annotation processing. Но: ошибки только в runtime (NoBeanDefFoundException при запуске). Hilt: compile-time проверки, ошибка до запуска. Koin подходит для: маленькие проекты, KMP, быстрый прототип. Hilt для: production Android-проектов.

> [!question]- Сценарий: Koin выбрасывает NoBeanDefFoundException в production. Как предотвратить?
> 1) checkModules() в unit тестах: verify { checkModules() } -- проверяет весь граф. 2) koinApplication { checkModules() } в CI/CD. 3) Использовать koin-test dependency. 4) Для каждого нового модуля добавлять тест. Это компенсирует отсутствие compile-time проверок.
...
Как объявить зависимости в Koin?
?
module { single { RetrofitClient() } // singleton; factory { UserRepository(get()) } // новый каждый раз; viewModel { HomeViewModel(get()) } // ViewModel scope }. get() -- resolve зависимости из графа.

Чем single отличается от factory в Koin?
?
single: один instance на весь граф (Singleton). factory: новый instance при каждом get(). scoped: один instance в scope (Activity, Fragment). viewModel: привязан к lifecycle ViewModel.

Как инжектить в Android класс с Koin?
?
Activity/Fragment: val viewModel: HomeVM by viewModel(); val repo: UserRepo by inject(). Lazy injection (by inject) или eager (get()). Класс должен implement KoinComponent или использовать KoinAndroidExt.

Поддерживает ли Koin KMP?
?
Да. koin-core -- чистый Kotlin, работает на всех платформах. koin-android -- Android специфика. koin-compose -- Compose Multiplatform. Один граф зависимостей для shared и platform code.

Как тестировать с Koin?
?
koin-test: KoinTest interface, inject() в тестах, checkModules() для верификации графа, declare { } для test overrides. MockProviderRule для автоматического создания mocks.
...
|-------------|------|-------|
| Следующий шаг | [[android-kotlin-inject-deep-dive]] | kotlin-inject -- compile-time KMP альтернатива |
| Углубиться | [[android-hilt-deep-dive]] | Hilt для сравнения подходов |
| Смежная тема | [[kmp-di-patterns]] | DI в KMP проектах |
