...
> [!question]- Почему Dagger генерирует код вместо использования рефлексии?
> Рефлексия: медленная (100-1000x), нет compile-time проверок, ProGuard/R8 может сломать. Code generation: проверка графа при компиляции (отсутствующие зависимости = compilation error), нулевой runtime overhead, оптимизирован для ProGuard. На Android критично: каждая ms экономии при старте важна.

> [!question]- Сценарий: при добавлении новой зависимости в Dagger граф получаете MissingBinding. Как debug?
> 1) Проверить: есть ли @Inject конструктор или @Provides метод для типа. 2) Проверить scope: зависимость в правильном Component? 3) Проверить @Module + @InstallIn: модуль установлен в нужный component. 4) Прочитать полное сообщение ошибки -- Dagger показывает цепочку зависимостей. 5) Для multibinding: проверить @IntoSet/@IntoMap.
...
?
Интерфейс, связывающий consumers (inject) с providers (modules). Dagger генерирует реализацию DaggerXxxComponent. Component определяет scope и lifecycle зависимостей. Иерархия: AppComponent > ActivityComponent > FragmentComponent.

Что такое Subcomponent?
?
Component, наследующий зависимости родителя. ActivitySubcomponent видит все из AppComponent + свои. Реализует scoping: объекты в Subcomponent живут пока жив Subcomponent. Hilt автоматизирует создание Subcomponents.

Как работает @Inject constructor?
?
Dagger видит @Inject constructor и автоматически знает как создать объект. Параметры конструктора -- зависимости, которые Dagger resolve из графа. Не нужен @Provides метод. Самый простой способ регистрации зависимости.

Чем @Provides отличается от @Binds?
?
@Provides: создает объект (return Retrofit.Builder()..build()). @Binds: связывает interface с реализацией (abstract fun bind(impl: UserRepositoryImpl): UserRepository). @Binds эффективнее -- не создает wrapper метод.

Что такое Multibinding в Dagger?
?
Инжекция коллекции объектов: @IntoSet (Set<Plugin>), @IntoMap (Map<Key, Plugin>). Каждый модуль добавляет свой элемент. Полезно для plugin systems, event handlers, feature flags.
...
|-------------|------|-------|
| Следующий шаг | [[android-hilt-deep-dive]] | Hilt -- надстройка над Dagger для Android |
| Углубиться | [[android-dependency-injection]] | Теория DI и сравнение подходов |
| Смежная тема | [[jvm-annotations-processing]] | Annotation processing -- основа Dagger code gen |
