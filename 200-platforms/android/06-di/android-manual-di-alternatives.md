...
> [!question]- Когда manual DI предпочтительнее DI-фреймворка?
> Manual DI оправдан когда: 1) Маленький проект (< 20 классов с зависимостями). 2) Обучение (понять DI перед фреймворком). 3) Библиотека (не навязывать DI-фреймворк потребителям). 4) Минимизация зависимостей. Для средних+ проектов DI-фреймворк экономит время и уменьшает boilerplate.

> [!question]- Почему Service Locator считается антипаттерном, но Koin его использует?
> Service Locator: скрывает зависимости (не видны в конструкторе), сложнее тестировать (нужно настроить глобальный state), нарушает SOLID. Koin формально SL, но: 1) DSL делает зависимости declarative. 2) checkModules() ловит ошибки в тестах. 3) by inject() лаконичнее конструкторов. Pragmatism > purism для простых проектов.
...
Как реализовать manual DI в Android?
?
AppContainer class создается в Application. Содержит фабрики для всех зависимостей. Activity получает AppContainer из (application as MyApp).container. Scoped containers для Activity/Fragment lifecycle.

Что такое Service Locator?
?
Глобальный registry для зависимостей. Объект запрашивает зависимости: ServiceLocator.get<UserRepository>(). Скрытые зависимости (не видны в конструкторе). Просто реализовать, сложно тестировать.

Что такое Factory Pattern для DI?
?
Вместо new UserRepository(api, db) -- UserRepository.Factory.create(). Factory инкапсулирует создание. Можно передать разные Factory для тестов. Промежуточный шаг между manual DI и фреймворком.

Какие плюсы/минусы manual DI?
?
Плюсы: нет зависимостей, полный контроль, понятный код, быстрая компиляция. Минусы: boilerplate растет экспоненциально, нет scoping, нет compile-time проверок графа, нет multibinding.
