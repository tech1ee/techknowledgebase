...
> **Hilt** — **opinionated wrapper** над Dagger 2, реализующий принцип **Convention over Configuration** (Hansson, 2004): вместо ручной настройки Component hierarchy Hilt предоставляет **предопределённые компоненты**, привязанные к Android lifecycle (`SingletonComponent`, `ActivityComponent`, `ViewModelComponent`).

Hilt решает проблему **Composition Root** в Android: место, где собирается граф зависимостей, должно быть как можно ближе к точке входа приложения. В Android это `Application.onCreate()` — отсюда `@HiltAndroidApp`. Hilt автоматически создаёт Component hierarchy, соответствующую Android lifecycle hierarchy.

| Hilt Component | Scope | Lifecycle | Dagger-эквивалент |
|---------------|-------|-----------|-------------------|
| SingletonComponent | @Singleton | Application | AppComponent |
| ActivityRetainedComponent | @ActivityRetainedScoped | ViewModel | — (нужен SubComponent) |
| ViewModelComponent | @ViewModelScoped | ViewModel | — (нужен SubComponent + Factory) |
| ActivityComponent | @ActivityScoped | Activity | ActivitySubComponent |
| FragmentComponent | @FragmentScoped | Fragment | FragmentSubComponent |

> **Bytecode transformation** — Hilt использует Gradle Transform API для модификации bytecode Android-классов. `@AndroidEntryPoint` на Activity генерирует `Hilt_*Activity` базовый класс, инжектирующий зависимости в `onCreate()`. Это реализация паттерна **Template Method** (Gamma et al., 1994) на уровне bytecode.

Hilt решает главную проблему Dagger на Android: **boilerplate для Android-специфичных компонентов**. Без Hilt требуется ~100 строк шаблонного кода для Component + Subcomponents + Factory + inject в Activity/Fragment. С Hilt — 2 аннотации: `@HiltAndroidApp` и `@AndroidEntryPoint`.
...
- **Gamma E. et al. (1994). Design Patterns.** — Template Method (bytecode transformation @AndroidEntryPoint)
- **Hansson D.H. (2004). Convention over Configuration.** — Принцип, реализованный в Hilt через предопределённые компоненты
- **Fowler M. (2004). Inversion of Control Containers and the DI Pattern.** — Composition Root
...
- [Scoping in Android and Hilt — Vivo](https://medium.com/androiddevelopers/scoping-in-android-and-hilt-c2e5222317c0)
- **Bloch J. (2018)** *Effective Java* — Item 5: DI vs hardwiring
- **Meier R. (2022)** *Professional Android* — Hilt + Jetpack в production
...
> [!question]- Почему Hilt рекомендован Google для DI в Android-проектах?
> Hilt упрощает Dagger: 1) Предопределенные components (SingletonComponent, ActivityComponent и т.д.) -- не нужно создавать свои. 2) Автоматический inject в Android classes (@AndroidEntryPoint). 3) Интеграция с ViewModel (@HiltViewModel). 4) Тестирование (HiltTestApplication, @UninstallModules). Dagger = мощный но сложный, Hilt = Dagger с convenience для Android.

> [!question]- Сценарий: нужно подменить Repository в UI тестах. Как сделать с Hilt?
> 1) @UninstallModules(RepositoryModule::class) на тестовом классе -- убирает production binding. 2) @Module @TestInstallIn(components = [SingletonComponent::class], replaces = [RepositoryModule::class]) с @Provides для FakeRepository. 3) Или @BindValue для простых замен: @BindValue val repo: Repository = FakeRepository(). Hilt пересоздает граф с test bindings.
...
Какие предопределенные components есть в Hilt?
?
SingletonComponent (Application), ActivityRetainedComponent (ViewModel), ViewModelComponent, ActivityComponent, FragmentComponent, ViewComponent, ServiceComponent. Иерархия: каждый видит зависимости родителя. @InstallIn определяет куда ставить Module.

Что делает @AndroidEntryPoint?
?
Генерирует Hilt_XxxActivity/Fragment с inject логикой. Activity/Fragment наследуют от Hilt_ версии. Автоматический inject @Inject полей при onCreate/onAttach. Работает с Activity, Fragment, View, Service, BroadcastReceiver.

Что делает @HiltViewModel?
?
Помечает ViewModel для Hilt injection. ViewModel получает зависимости через @Inject constructor. Hilt генерирует ViewModelProvider.Factory автоматически. В Fragment/Activity: val viewModel: MyVM by viewModels(). SavedStateHandle инжектится автоматически.

Что такое @EntryPoint в Hilt?
?
Точка доступа к Hilt графу из классов, которые Hilt не инжектит (ContentProvider, custom class). Интерфейс с @EntryPoint + @InstallIn. Доступ: EntryPointAccessors.fromApplication(context, XxxEntryPoint::class.java).

Как Hilt работает с WorkManager?
?
@HiltWorker + @AssistedInject constructor. HiltWorkerFactory создает Worker с инжектированными зависимостями. @Assisted для Context и WorkerParameters (не из графа). Настройка через Configuration.Builder.setWorkerFactory().
...
|-------------|------|-------|
| Следующий шаг | [[android-koin-deep-dive]] | Koin -- альтернатива для простых проектов и KMP |
| Углубиться | [[android-dagger-deep-dive]] | Dagger 2 под капотом Hilt |
| Смежная тема | [[cross-dependency-injection]] | DI на разных платформах |
