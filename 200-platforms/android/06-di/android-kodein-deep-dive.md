...
> [!question]- Чем Kodein отличается от Koin и когда его выбирать?
> Kodein: type-safe DSL, более строгая типизация через tagged bindings, multiplatform. Koin: проще API, больше community, лучше интеграция с Android. Kodein выбирать когда: 1) Нужна строгая типизация в DI. 2) Проект уже использует Kodein. 3) Предпочитаете функциональный стиль. В целом Koin популярнее для Android.

> [!question]- Почему Kodein теряет популярность в Android-экосистеме?
> 1) Hilt стал официальным решением Google. 2) Koin проще и имеет большее community. 3) kotlin-inject предоставляет compile-time safety для KMP. 4) Документация и примеры Kodein менее обширны. Kodein остается хорошим решением, но market share снижается.
...
Как объявить зависимости в Kodein?
?
val di = DI { bind<UserRepository>() with singleton { UserRepositoryImpl(instance()) }; bind<ApiService>() with provider { ApiServiceImpl() } }. instance() -- resolve зависимости. singleton, provider, factory -- scope.

Какие binding types есть в Kodein?
?
singleton (один на контейнер), provider (новый каждый раз), factory (с параметром), multiton (singleton по ключу), instance (константа). Каждый определяет lifecycle зависимости.

Как Kodein поддерживает KMP?
?
kodein-di -- чистый Kotlin модуль. Работает на JVM, Native, JS. Один DSL для всех платформ. Platform-specific bindings через DI modules.

Чем Kodein DI Framework отличается от Kodein DB?
?
Kodein DI -- dependency injection фреймворк. Kodein DB -- KMP NoSQL база данных (key-value + document). Разные проекты от одной команды (Kodein Koders).
...
|-------------|------|-------|
| Следующий шаг | [[android-koin-deep-dive]] | Koin -- более популярная runtime альтернатива |
| Углубиться | [[android-dependency-injection]] | Теория DI и сравнение фреймворков |
| Смежная тема | [[cross-dependency-injection]] | DI подходы на разных платформах |
