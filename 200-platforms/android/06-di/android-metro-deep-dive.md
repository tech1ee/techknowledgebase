...
> **Metro** — DI framework нового поколения, реализованный как **Kotlin Compiler Plugin** (не annotation processor). Это фундаментальное архитектурное отличие: compiler plugin имеет доступ к полному AST (Abstract Syntax Tree) Kotlin-кода, работает быстрее KAPT/KSP и может генерировать код без промежуточных стадий.

Metro объединяет три ранее несовместимых свойства: compile-time safety (Dagger), module aggregation (Anvil) и KMP support (kotlin-inject). Архитектурно это реализация принципа **Unification** — вместо трёх инструментов, решающих части проблемы, один инструмент решает всю задачу.

| Поколение DI | Инструмент | Модель | Ограничение |
|-------------|-----------|--------|-------------|
| I (2012) | Dagger 1 | Runtime reflection | Производительность |
| II (2015) | Dagger 2 | Annotation processing (KAPT) | Медленная компиляция |
| III (2020) | kotlin-inject | KSP | Нет aggregation |
| IV (2025) | Metro | Kotlin Compiler Plugin | Новый, малый ecosystem |

> **Compiler Plugin** vs **Annotation Processor**: annotation processor (KAPT/KSP) работает как отдельная фаза после парсинга — он видит только аннотации и генерирует новые файлы. Compiler Plugin встроен в компилятор — он может **трансформировать существующий код** (IR transformation), видеть полную семантику и работать инкрементально. Это даёт Metro до **60% ускорения** по сравнению с KAPT-based Dagger.

Metro поддерживает **top-level function injection** и **@Composable injection** — возможности, принципиально недоступные annotation-processing-based фреймворкам (Dagger, kotlin-inject), так как annotation processor не может модифицировать существующие функции.
...
- **Aho A.V., Lam M.S., Sethi R., Ullman J.D.** *Compilers: Principles, Techniques, and Tools* (2006) — формальные основы compiler plugin как IR transformation: AST, промежуточное представление, source-to-source трансформации
- **Gamma E., Helm R., Johnson R., Vlissides J.** *Design Patterns* (1994) — паттерны Factory, Singleton и Command, реализуемые Metro через Kotlin IR codegen вместо annotation processing
- **Cormen T.H. et al.** *Introduction to Algorithms* (2009) — DAG и топологическая сортировка для разрешения графа зависимостей; инкрементальная обработка в compiler plugin
...
- **Moskala M.** *Effective Kotlin* (2021) — best practices Kotlin, включая compiler plugins, DSL-дизайн и паттерны создания объектов, которые помогают понять архитектурные решения Metro
- **Bloch J.** *Effective Java* (2018) — классические принципы API-дизайна и dependency management (Item 5: Prefer dependency injection), которые лежат в основе всех DI-фреймворков включая Metro
- **Leiva A.** *Kotlin for Android Developers* (2017) — практические паттерны Kotlin для Android, включая организацию зависимостей и модульную архитектуру

---

## Проверь себя

> [!question]- Почему JetBrains создали Metro вместо улучшения существующих DI?
> Existing DI не полностью поддерживают KMP: Dagger/Hilt -- только Android (KAPT). Koin -- runtime, нет compile-time safety. kotlin-inject -- хороший, но community-driven. Metro: 1) Official JetBrains support. 2) KSP2-based (быстрее). 3) Designed for KMP from day one. 4) Familiar API (Dagger-like). 5) Integration с Compose/Kotlin экосистемой.

> [!question]- Сценарий: команда выбирает DI для нового KMP проекта. Metro vs kotlin-inject?
> Metro: JetBrains backed, newer, potentially better ecosystem integration, но менее зрелый. kotlin-inject: proven, stable, active community, production-ready. Решение: для новых проектов с долгим горизонтом -- Metro (будет развиваться с Kotlin). Для проектов с запуском скоро -- kotlin-inject (стабильнее сейчас).
...
Что такое Metro?
?
Compile-time DI фреймворк от JetBrains для Kotlin Multiplatform. KSP2-based, Dagger-like API, поддержка всех KMP targets. Альтернатива kotlin-inject с official support. В стадии активной разработки (2025+).

Как Metro сравнивается с Hilt?
?
Metro: KMP (все платформы), KSP2, pure Kotlin. Hilt: только Android, KAPT, Java annotations. Для pure Android: Hilt зрелее. Для KMP: Metro -- future-proof. API похож на Dagger/Hilt для легкой миграции.

Какие преимущества KSP2 для Metro?
?
KSP2: быстрее компиляция, standalone (не зависит от KAPT), better error messages, multiplatform support. Metro использует KSP2 features: incremental processing, symbol resolution на всех targets.

Какой статус Metro?
?
Experimental/Alpha (2025). JetBrains анонсировали как часть Kotlin экосистемы. Не рекомендуется для production пока. Следить за blog.jetbrains.com и GitHub.
...
|-------------|------|-------|
| Следующий шаг | [[android-kotlin-inject-deep-dive]] | kotlin-inject -- production-ready альтернатива |
| Углубиться | [[android-dagger-deep-dive]] | Dagger -- основа API дизайна Metro |
| Смежная тема | [[kmp-di-patterns]] | DI для Kotlin Multiplatform |
