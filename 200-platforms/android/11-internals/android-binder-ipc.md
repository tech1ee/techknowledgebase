...
> **Межпроцессная коммуникация (IPC, Inter-Process Communication)** — механизмы, позволяющие процессам обмениваться данными и синхронизировать выполнение через границы адресных пространств. Классификация IPC включает: разделяемую память, передачу сообщений, каналы (pipes), сокеты и вызовы удалённых процедур (RPC).

### Таксономия IPC-механизмов

| Механизм | Копирований данных | Идентификация вызывающего | Модель | Происхождение |
|----------|-------------------|--------------------------|--------|--------------|
| Unix Pipes | 2 (user→kernel→user) | Нет (только parent-child) | Поток байтов | Thompson, Unix V1 (1971) |
| Unix Domain Sockets | 2 | Через `SO_PEERCRED` (Linux) | Поток / датаграмма | BSD 4.2 (1983) |
| System V Shared Memory | 0 (но нужна синхронизация) | Нет | Общая область памяти | AT&T Unix System V (1983) |
| D-Bus | 2+ (через демон) | String-based, policy XML | Шина сообщений | freedesktop.org (2002) |
| Mach Ports | 1 (kernel copy) | Port rights (capability) | Асинхронные сообщения | Rashid et al., CMU (1987) |
| **Android Binder** | **1** (mmap) | **Kernel-enforced UID/PID** | **Синхронный RPC** | **Be Inc. → Google (2005)** |

> **Object-Capability Model (OCap)** — модель безопасности, где право доступа к ресурсу представлено неподделываемой ссылкой (capability) на этот ресурс. Формализована Mark S. Miller, *Robust Composition: Towards a Unified Approach to Access Control and Concurrency Control* (2006). Binder реализует этот принцип: `IBinder`-ссылка является одновременно адресом объекта и правом доступа к нему — её невозможно подделать из userspace.

> **Proxy Pattern** — GoF, *Design Patterns* (1994). AIDL генерирует Proxy (клиент) и Stub (сервер), обеспечивая прозрачность IPC: вызывающий код не знает, локальный объект или удалённый. Этот же паттерн используется в CORBA, Java RMI, gRPC.

### Формальные свойства Binder

| Свойство | Описание | Теоретическая основа |
|----------|---------|---------------------|
| Синхронность по умолчанию | Вызывающий поток блокируется до получения ответа | Remote Procedure Call (Birrell & Nelson, 1984, *Implementing Remote Procedure Calls*) |
| Однокопийная передача | Данные копируются один раз (sender → kernel buffer, receiver читает через mmap) | Zero-copy идеал из Mach microkernel |
| Kernel-enforced identity | UID/PID вызывающего определяется ядром, не может быть подделан | Reference Monitor (Anderson, 1972, *Computer Security Technology Planning Study*) |
| Reference counting | Binder-объекты удаляются при обнулении счётчика ссылок | Автоматическое управление памятью (Collins, 1960) |
| Death notification | Клиент получает уведомление при гибели удалённого процесса | Erlang-style process monitoring (Armstrong, 1996) |

Сравнение с другими ОС: iOS использует **XPC** (на базе Mach ports + libdispatch), Windows — **COM/DCOM** (на базе LPC/ALPC). Binder ближе всего к модели COM: синхронный RPC с IDL-описанием интерфейсов и reference counting.

Связанные материалы: [[android-kernel-extensions]] (Binder driver в ядре), [[android-system-services]] (использование Binder для системных сервисов), [[android-service-internals]] (Bound Services на уровне приложения).
...
| Источник | Применение |
|----------|-----------|
| Birrell A., Nelson B. *Implementing Remote Procedure Calls* (1984) | RPC-модель, основа синхронного Binder IPC |
| Anderson J. *Computer Security Technology Planning Study* (1972) | Reference Monitor — kernel-enforced identity |
| Miller M. *Robust Composition: Towards a Unified Approach to Access Control and Concurrency Control* (2006) | Object-Capability Model |
| GoF. *Design Patterns* (1994) | Proxy Pattern — AIDL Proxy/Stub |
| Rashid R. et al. *Mach: A New Kernel Foundation for UNIX Development* (1987) | Mach Ports — сравнение IPC-моделей |
| Collins G. *A Method for Overlapping and Erasure of Lists* (1960) | Reference counting |
| Schreiber T. *Android Binder — Android IPC* (Master's thesis, TU Munich, 2011) | Самый подробный академический анализ Binder |
