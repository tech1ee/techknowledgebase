...
> **Процесс загрузки (Boot Process)** — последовательность этапов инициализации аппаратного и программного обеспечения от подачи питания до готовности операционной системы принимать пользовательский ввод.

### Теория процесса загрузки

Загрузка ОС — классическая проблема CS, описанная ещё в контексте IBM System/360 (1964). Термин **bootstrapping** (от "pulling oneself up by one's bootstraps") отражает парадокс: система должна загрузить программу, которая сама должна быть загружена.

| Концепция | Автор / Источник | Применение в Android |
|-----------|-----------------|---------------------|
| Chain of Trust | Trusted Computing Group (TPM, 2003) | AVB (Android Verified Boot): ROM → bootloader → kernel → system — каждый этап проверяет подпись следующего |
| Init System | Dennis Ritchie, Ken Thompson (Unix V1, 1971) | Android init — PID 1, парсит `.rc`-файлы, запускает демоны |
| Process Forking | Ken Thompson, *fork()* (Unix, 1969) | Zygote — шаблонный процесс, от которого fork-ом создаются все приложения |
| Prototype Pattern | GoF, *Design Patterns* (1994) | Zygote = прототип: преинициализированный процесс с загруженными классами копируется для каждого приложения |
| Copy-on-Write (CoW) | IBM CP-67 (1968), формализовано в Mach (Rashid, 1987) | fork() от Zygote использует CoW: страницы памяти копируются только при записи, экономя ~30 МБ на процесс |
| Preloading / Eager Initialization | — | Zygote загружает ~8000 классов и ~200 ресурсов заранее; каждый fork-нутый процесс получает их бесплатно через CoW |

> **Verified Boot** — концепция цепочки доверия (chain of trust), где каждый компонент загрузки проверяет криптографическую подпись следующего. Берёт начало от концепции Trusted Computing (TCG, 2003) и формализована в NIST SP 800-155 (2011, *BIOS Integrity Measurement Guidelines*).

> **Zygote-модель** — уникальное решение Android, не имеющее прямых аналогов в других мобильных ОС. iOS не использует fork() для создания процессов приложений. Windows использует аналогичную концепцию — **Prefetch/Superfetch** (предзагрузка данных), но не fork-модель. Ближайший аналог — **preforking** в серверном ПО (Apache httpd, PostgreSQL).

Связанные материалы: [[android-kernel-extensions]] (патчи ядра для загрузки), [[android-app-startup-performance]] (оптимизация холодного старта), [[android-art-runtime]] (загрузка runtime).
...
| Источник | Применение |
|----------|-----------|
| Ritchie D., Thompson K. *The UNIX Time-Sharing System* (1974) | Процессная модель, fork(), init (PID 1) |
| Rashid R. et al. *Mach: A New Kernel Foundation for UNIX Development* (1987) | Copy-on-Write формализация |
| GoF. *Design Patterns* (1994) | Prototype Pattern → Zygote |
| TCG. *Trusted Platform Module* (2003); NIST SP 800-155 (2011) | Chain of Trust, Verified Boot |
| Corbató F. et al. *An Experimental Time-Sharing System* (1962) | Разделение времени, основы init-систем |
