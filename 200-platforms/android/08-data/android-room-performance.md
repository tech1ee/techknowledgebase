...
> [!question]- Почему SELECT * FROM users WHERE id IN (:ids) может быть медленным для большого списка ids?
> SQLite ограничивает количество bind variables (~999). Для 1000+ ids: Room разбивает на несколько запросов. Каждый запрос -- отдельное SQL execution. Решение: 1) Использовать @RawQuery с временной таблицей. 2) Batch по 500. 3) JOIN с подзапросом вместо IN.

> [!question]- Сценарий: Room Flow из списка с 10000 записей обновляется при каждом INSERT, даже если UI показывает только 20. Как оптимизировать?
> 1) Paging 3 library: PagingSource из Room загружает только видимые страницы. 2) distinctUntilChanged() на Flow -- пропускать если данные не изменились. 3) Более специфичный запрос (LIMIT, WHERE для видимых). 4) InvalidationTracker может быть слишком aggressive -- Room отслеживает таблицу, не строку.
...
Какие основные правила оптимизации Room?
?
1) Индексы на колонки в WHERE/JOIN. 2) Не SELECT * -- только нужные колонки. 3) @Transaction для multi-query операций. 4) Paging для больших списков. 5) WAL mode (по умолчанию). 6) Не создавать БД на Main Thread.

Что такое WAL mode в SQLite?
?
Write-Ahead Logging: записи идут в WAL файл, чтение -- из основной БД + WAL. Позволяет concurrent reads + write. Room включает WAL по умолчанию. Checkpoint мержит WAL в основную БД. Быстрее для read-heavy workloads.

Как Paging 3 работает с Room?
?
@Query возвращает PagingSource<Int, User>. Paging library автоматически загружает страницы при скролле. Room генерирует SQL с LIMIT/OFFSET. RemoteMediator для network + Room. LazyPagingItems для Compose.

Что такое @Transaction в Room?
?
Гарантирует атомарность нескольких операций: все или ничего. BEGIN/COMMIT обертка. Для @Query с @Relation (несколько SELECT). Для batch INSERT/UPDATE. Предотвращает inconsistent reads между запросами.

Как мониторить производительность Room?
?
StrictMode.VmPolicy.detectAll() для обнаружения disk reads на Main Thread. Android Studio Database Inspector. SQLite EXPLAIN QUERY PLAN для проверки индексов. Benchmark: androidx.benchmark для измерения query time.
...
|-------------|------|-------|
| Следующий шаг | [[android-datastore-guide]] | DataStore для key-value хранения |
| Углубиться | [[android-room-deep-dive]] | Room internals для понимания оптимизации |
| Смежная тема | [[database-design-optimization]] | Оптимизация SQL запросов общие принципы |
