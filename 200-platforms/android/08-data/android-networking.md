...
> [!question]- Почему OkHttp использует connection pooling и чем это помогает?
> Установка TCP+TLS соединения: ~200-500ms (DNS + TCP handshake + TLS handshake). Connection pool переиспользует соединения для того же хоста. Default: 5 idle connections, 5 min keepalive. При 10 запросах к API: 1 connection setup + 9 reused = экономия ~2-4 секунд. HTTP/2 multiplexing: один connection для всех запросов к хосту.

> [!question]- Сценарий: API возвращает 429 Too Many Requests. Как реализовать retry с backoff?
> OkHttp Interceptor: при 429 проверить Retry-After header, ждать указанное время. Exponential backoff: 1s, 2s, 4s, 8s с jitter. Максимум 3-5 retries. В Retrofit: Call adapter с retry logic. В Ktor: HttpRequestRetry plugin с configuration. Не retry для 4xx (кроме 429) -- клиентские ошибки не исчезнут.
...
Какие networking библиотеки используются в Android?
?
OkHttp: HTTP client (connection pool, interceptors, HTTP/2). Retrofit: REST client поверх OkHttp (type-safe API, converters). Ktor: Kotlin-first HTTP client (multiplatform, coroutines). Для Android: Retrofit или Ktor. Coil/Glide для изображений.

Что такое Interceptor в OkHttp?
?
Middleware для HTTP запросов/ответов. Application interceptor: before redirect/retry (logging, auth header). Network interceptor: after redirect (caching, compression). Chain pattern: request -> interceptor1 -> interceptor2 -> network -> response.

Как Retrofit генерирует реализацию API?
?
Retrofit использует Proxy (java.lang.reflect) для создания implementation интерфейса в runtime. Annotations (@GET, @POST) + параметры (@Query, @Body) конвертируются в OkHttp Request. Converters (Gson, Moshi, kotlinx.serialization) для body.

Что такое kotlinx.serialization vs Gson/Moshi?
?
kotlinx.serialization: compile-time (KSP), multiplatform, no reflection, fastest. Gson: runtime reflection, только JVM, медленнее. Moshi: codegen + reflection, только JVM. Для новых проектов: kotlinx.serialization.

Как обрабатывать ошибки сети?
?
Sealed class: NetworkResult<T> = Success(data) | Error(code, message) | Exception(throwable). Interceptor для общих ошибок (401 -> refresh token). try/catch для IOException (no network). Retrofit: Response<T>.isSuccessful для HTTP errors.
...
|-------------|------|-------|
| Следующий шаг | [[android-repository-pattern]] | Repository для объединения network + cache |
| Углубиться | [[android-data-persistence]] | Кэширование network данных локально |
| Смежная тема | [[ios-networking]] | Networking в iOS: URLSession и Alamofire |
