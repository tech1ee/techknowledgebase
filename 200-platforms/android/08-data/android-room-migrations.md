...
> [!question]- Почему миграции Room должны быть идемпотентными?
> Пользователь может обновить с версии 1 до 5, пропустив промежуточные. Room выполняет цепочку миграций: 1->2, 2->3, 3->4, 4->5. Если миграция 2->3 зависит от состояния после 1->2, но пользователь уже на версии 3 (partial migration) -- crash. Каждая миграция должна быть self-contained: проверять текущее состояние перед изменением.

> [!question]- Сценарий: после добавления нового столбца приложение крашится у части пользователей. Как debug?
> 1) Проверить fallbackToDestructiveMigration() -- может удалять данные. 2) Проверить все пути миграции (1->3, 2->3, не только 2->3). 3) Room Auto-migration может не покрыть сложный случай (переименование колонки). 4) Тестировать через MigrationTestHelper: exportSchema = true, проверить JSON schema. 5) Логировать версию БД в analytics.
...
?
Migration(fromVersion, toVersion) с SQL-командами. Room сравнивает текущую и ожидаемую schema. При несовпадении -- вызывает миграцию. Цепочка: 1->2->3. addMigrations() в databaseBuilder. fallbackToDestructiveMigration() -- пересоздать при ошибке.

Что такое Auto-migration в Room?
?
Room 2.4+: автоматическая генерация миграций для простых изменений (add column, add table). @AutoMigration(from = 1, to = 2). Для сложных: @AutoMigration(spec = MigrationSpec::class) с @RenameColumn, @DeleteColumn, @RenameTable.

Как тестировать миграции?
?
MigrationTestHelper: 1) Создать БД версии N. 2) Применить миграцию N->N+1. 3) Проверить schema и данные. exportSchema = true в @Database. Schema JSON в schemas/ директории. CI/CD: тест каждого пути миграции.

Что делает fallbackToDestructiveMigration()?
?
При ошибке миграции -- удаляет БД и создает заново. Пользователь теряет данные. Использовать только для кэша/некритичных данных. Для production: написать правильную миграцию и протестировать.

Как обработать переименование колонки?
?
Room Auto-migration: @AutoMigration(spec = Spec::class), class Spec : AutoMigrationSpec { @RenameColumn('table', 'old', 'new') }. Manual: Migration с ALTER TABLE RENAME COLUMN (SQLite 3.25+) или создание новой таблицы + копирование данных.
...
|-------------|------|-------|
| Следующий шаг | [[android-room-performance]] | Оптимизация запросов к Room |
| Углубиться | [[android-room-deep-dive]] | Room internals для контекста миграций |
| Смежная тема | [[databases-fundamentals-complete]] | Schema migrations в SQL базах данных |
