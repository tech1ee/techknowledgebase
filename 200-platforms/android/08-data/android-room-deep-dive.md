...
> [!question]- Почему Room генерирует код вместо использования рефлексии для маппинга?
> Code generation (KSP/KAPT): 1) Compile-time проверка SQL запросов (синтаксис, таблицы, колонки). 2) Нулевой runtime overhead (нет рефлексии). 3) ProGuard-safe (нет строковых имен). Рефлексия: медленная (100x), нет compile-time проверок, ProGuard ломает. Room = type-safe SQL с проверкой при компиляции.

> [!question]- Сценарий: Flow из Room не обновляется после INSERT в другом месте приложения. Почему?
> Room Flow использует InvalidationTracker: при изменении таблицы все Flow запросы к этой таблице переиспускают данные. Возможные причины: 1) INSERT через rawQuery (не trigger invalidation). 2) Другая база данных instance (не singleton). 3) @Query с JOIN -- Room отслеживает все упомянутые таблицы, но может пропустить при сложных запросах.
...
Какие компоненты Room?
?
@Database: abstract class, определяет entities и version. @Entity: таблица (data class). @Dao: interface с @Query, @Insert, @Update, @Delete. Room.databaseBuilder() создает instance. Всегда singleton через DI.

Как Room обеспечивает reactive updates?
?
InvalidationTracker: отслеживает изменения в таблицах. @Query возвращает Flow<List<T>>: при INSERT/UPDATE/DELETE в таблице -- Flow re-emit. Работает через SQLite triggers + callback. Автоматическое уведомление UI.

Что такое @Embedded и @Relation?
?
@Embedded: вложенный объект хранится в той же таблице (flat). @Relation: связь между таблицами (1:N, N:M). @Relation с @Junction для many-to-many. Для сложных запросов лучше custom @Query с JOIN.

Как тестировать Room?
?
In-memory database: Room.inMemoryDatabaseBuilder(). Быстрые, isolated тесты. runBlocking для suspend DAOs. Turbine для testing Flow. allowMainThreadQueries() только для тестов.

Что такое TypeConverter?
?
Конвертер нестандартных типов для хранения в SQLite: Date -> Long, List<String> -> JSON String, Enum -> String. @TypeConverter annotation. Указать в @Database(typeConverters). Kotlin Serialization для сложных типов.
...
|-------------|------|-------|
| Следующий шаг | [[android-room-migrations]] | Миграции базы данных |
| Углубиться | [[android-room-performance]] | Оптимизация Room запросов |
| Смежная тема | [[ios-core-data]] | Core Data в iOS -- аналог Room |
