...
### Таксономия хранения данных

> В теории баз данных различают несколько моделей хранения, каждая с формальными свойствами. Android предоставляет API для каждой модели:

| Модель хранения | Формальное определение | Android реализация | Свойства |
|----------------|----------------------|--------------------|----------|
| **Key-Value Store** | Словарь: `f: Key → Value`, O(1) доступ | DataStore, SharedPreferences | Простота, нет связей, нет запросов |
| **Реляционная БД** | Отношения Кодда (1970): таблицы, SQL, ACID | Room / SQLite | Структурированные данные, JOIN, индексы |
| **Document Store** | JSON/XML документы с произвольной структурой | File Storage (JSON) | Гибкость, нет схемы |
| **Object Store** | Сериализация объектов в бинарный формат | Proto DataStore (Protobuf) | Типобезопасность, backward-compatible |

### ACID-свойства и мобильное хранение

> Edgar Codd (1970, *"A Relational Model of Data for Large Shared Data Banks"*) определил фундамент реляционных БД. Jim Gray (1981, Turing Award) формализовал ACID-свойства транзакций:

| Свойство | Определение | Как реализовано в Room/SQLite |
|----------|------------|-------------------------------|
| **Atomicity** | Транзакция выполняется целиком или не выполняется | `@Transaction`, `withTransaction {}` |
| **Consistency** | БД переходит из одного валидного состояния в другое | Foreign keys, CHECK constraints |
| **Isolation** | Параллельные транзакции не влияют друг на друга | WAL mode: read-write isolation |
| **Durability** | Результат зафиксированной транзакции сохраняется | `fsync()` после commit |

### CAP-теорема для мобильных приложений

> Eric Brewer (2000, *CAP Theorem*): распределённая система не может одновременно обеспечить Consistency, Availability и Partition Tolerance. Мобильное приложение — распределённая система (клиент + сервер), где сеть **всегда** ненадёжна (Partition). Выбор между Consistency (показать только серверные данные) и Availability (показать локальный кэш) — ключевой архитектурный выбор. **Offline-first** подход выбирает Availability + Eventual Consistency через паттерн [[android-repository-pattern|Repository]].

> **Связь**: Реляционная модель → [[database-design-optimization]], ACID → [[android-room-deep-dive]], Offline-first → [[android-repository-pattern]]
...
| Источник | Применение |
|----------|-----------|
| Codd E.F. *A Relational Model of Data for Large Shared Data Banks* (1970) | Реляционная модель → Room/SQLite |
| Haerder T., Reuter A. *Principles of Transaction-Oriented Database Recovery* (1983) | ACID-свойства транзакций |
| Brewer E. *CAP Theorem* (2000, PODC) | Компромиссы в распределённых системах хранения |
| DeCandia G. et al. *Dynamo: Amazon's Highly Available Key-value Store* (2007) | Key-Value Store модель → DataStore |
