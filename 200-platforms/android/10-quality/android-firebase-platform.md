...
### Backend-as-a-Service: архитектурная модель

> **BaaS** (Backend-as-a-Service) — модель cloud computing, в которой серверная логика, хранилище и аутентификация предоставляются как управляемый сервис. Firebase — крупнейшая BaaS-платформа (по числу мобильных приложений). Модель формализуется через **Service-Oriented Architecture** (SOA, W3C, 2004): каждый Firebase-сервис = independent service с API-контрактом.

| Уровень абстракции | Что управляет разработчик | Что управляет платформа | Пример |
|-------------------|--------------------------|------------------------|--------|
| **IaaS** | Виртуальные машины, сети | Физическое железо | GCP Compute Engine |
| **PaaS** | Код приложения | Runtime, scaling, OS | App Engine, Cloud Run |
| **BaaS** | Клиентский код + конфигурация | Серверный код, БД, auth, storage | **Firebase** |
| **SaaS** | Ничего (использование) | Всё | Gmail, Google Analytics |

### Eventual Consistency в Firestore

> Cloud Firestore использует модель **eventual consistency** для multi-region replication (Vogels, *"Eventually Consistent"*, CACM, 2009). Strong consistency гарантируется только для single-document reads после writes. Для queries across documents — eventual consistency с convergence time < 1 секунды. Это следствие **CAP theorem** (Brewer, 2000): Firestore выбирает Availability + Partition Tolerance, жертвуя строгой Consistency для distributed queries.

### Authentication: формальная модель

> Firebase Authentication реализует стандарты **OAuth 2.0** (RFC 6749, Hardt, 2012) и **OpenID Connect** (2014) для федеративной аутентификации. Ключевое понятие — **identity token** (JWT, RFC 7519): self-contained signed claim о личности пользователя, верифицируемый без обращения к серверу авторизации.

> **Связь**: BaaS → [[cloud-fundamentals]], Eventual Consistency → [[database-design-optimization]], OAuth → [[security-concepts]]
...
| Источник | Применение |
|----------|-----------|
| Vogels W. *Eventually Consistent* (2009, ACM Queue) | Eventual Consistency → Firestore offline-first model |
| Hardt D. *RFC 6749: The OAuth 2.0 Authorization Framework* (2012) | OAuth 2.0 → Firebase Auth |
| Sakimura N. et al. *OpenID Connect Core 1.0* (2014) | OIDC → Firebase Auth identity tokens |
| Fox A., Brewer E. *Harvest, Yield, and Scalable Tolerant Systems* (1999) | CAP trade-offs в Firestore |

### Практические руководства
1. [Firebase Documentation](https://firebase.google.com/docs) — официальная документация
2. [Firebase Pricing](https://firebase.google.com/pricing) — тарифы и лимиты
3. [Firebase App Check with Play Integrity](https://firebase.google.com/docs/app-check/android/play-integrity-provider) — App Check
4. [Kotlin Migration from KTX](https://firebase.google.com/docs/android/kotlin-migration) — миграция с KTX
5. [Appwrite vs Supabase vs Firebase](https://uibakery.io/blog/appwrite-vs-supabase-vs-firebase) — сравнение BaaS 2026
