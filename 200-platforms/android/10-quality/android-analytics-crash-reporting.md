...
> **Observability** — свойство системы, позволяющее определить её внутреннее состояние по внешним выходам (Kalman, 1960, теория управления). В software engineering observability реализуется через три столпа (Sridharan, *"Distributed Systems Observability"*, 2018):

| Столп | Определение | Реализация в Android |
|-------|------------|---------------------|
| **Logs** | Дискретные события с timestamp и payload | Crashlytics breadcrumbs, `Log.d/e/w` |
| **Metrics** | Числовые измерения, агрегированные во времени | Crash-free rate, ANR rate, TTID, retention |
| **Traces** | Распределённые цепочки вызовов с тайминг | Firebase Performance traces, Perfetto |

### Event-Driven Analytics: формальная модель

> Аналитика мобильных приложений основана на **event sourcing** (Young, 2010, CQRS): каждое действие пользователя записывается как immutable event. Совокупность событий восстанавливает полную историю взаимодействия. Google Analytics 4 / Firebase Analytics реализуют этот подход: events → parameters → user properties → dimensions для агрегации.

### Crash Symbolication: от обфусцированного стектрейса к читаемому

> **Symbolication** — обратная операция к obfuscation: замена обфусцированных имён (`a.b.c()`) оригинальными (`com.app.UserRepository.loadData()`) через mapping file. Формально это применение **inverse bijection** из mapping: `f^(-1): ObfuscatedName → OriginalName`. Без mapping file стектрейс нечитаем. Crashlytics автоматически применяет mapping при upload через Gradle plugin.

### Sampling и Statistical Significance

> При большом DAU (>100K) аналитические системы используют **sampling** (случайная выборка): не все события отправляются, но статистически значимая выборка даёт корректные агрегаты. Firebase Analytics применяет sampling для BigQuery export. Crash reporting не использует sampling — каждый crash критичен.

> **Связь**: Observability → [[android-performance-profiling]], Event Sourcing → [[android-firebase-platform]], Obfuscation → [[android-proguard-r8]]
...
| Источник | Применение |
|----------|-----------|
| Sigelman B. et al. *Dapper, a Large-Scale Distributed Systems Tracing Infrastructure* (Google, 2010) | Observability: traces — основа Performance Monitoring |
| Fowler M. *Event Sourcing* (martinfowler.com, 2005) | Event Sourcing → analytics event model |
| Vitter J. *Random Sampling with a Reservoir* (1985) | Reservoir Sampling → analytics sampling |
| Shneiderman B. *The Eyes Have It: A Task by Data Type Taxonomy for Information Visualizations* (1996) | Dashboard design → crash/analytics dashboards |

### Практические руководства
1. [Firebase Crashlytics -- Get Started](https://firebase.google.com/docs/crashlytics/android/get-started) -- настройка Crashlytics
2. [Sentry Android SDK](https://docs.sentry.io/platforms/android/) -- документация Sentry
3. [Amplitude Android-Kotlin SDK](https://amplitude.com/docs/sdks/analytics/android/android-kotlin-sdk) -- Amplitude SDK
4. [Google Play -- Crashes and ANR](https://developer.android.com/games/optimize/vitals/crash) -- Android Vitals
5. [GDPR Compliance for Apps](https://gdprlocal.com/gdpr-compliance-for-apps/) -- GDPR guide
