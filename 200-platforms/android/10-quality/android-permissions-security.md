...
> [!question]- Почему Android перешел от install-time к runtime permissions?
> До Android 6: все permissions при установке -- пользователь или принимает все, или не устанавливает. Нет гранулярного контроля. Runtime permissions: запрос в контексте использования (камера при фото, геолокация при карте). Пользователь понимает зачем и может отказать для конкретного permission. Улучшает privacy и trust.

> [!question]- Сценарий: пользователь дважды отказал в permission. shouldShowRequestPermissionRationale() возвращает false. Что делать?
> После двух отказов Android помечает permission как 'Don't ask again'. shouldShowRequestPermissionRationale() = false. Нельзя запросить снова. Решения: 1) Показать dialog с объяснением и кнопкой 'Открыть настройки' (Settings.ACTION_APPLICATION_DETAILS_SETTINGS). 2) Graceful degradation: работать без permission с ограниченным функционалом. 3) Не блокировать всё приложение из-за одного permission.
...
Какие типы permissions?
?
Normal: автоматически, без запроса (INTERNET, VIBRATE). Dangerous: runtime запрос (CAMERA, LOCATION, CONTACTS). Signature: только приложения с тем же signing key. Special: отдельные экраны настроек (SYSTEM_ALERT_WINDOW, MANAGE_EXTERNAL_STORAGE).

Как правильно запрашивать runtime permission?
?
1) Проверить: ContextCompat.checkSelfPermission(). 2) Rationale: shouldShowRequestPermissionRationale(). 3) Запросить: ActivityResultContracts.RequestPermission(). 4) Обработать результат в callback. Запрашивать в контексте действия, не при старте.

Что изменилось в Android 14 для permissions?
?
Photo/video: partial access (выбрать конкретные файлы вместо доступа ко всей галерее). Foreground service types обязательны. Background location требует отдельного запроса. Runtime permission для calendar-based exact alarms.

Что такое Scoped Storage в контексте permissions?
?
Android 10+: приложение видит только свои файлы без permission. Для media: READ_MEDIA_IMAGES/VIDEO/AUDIO (Android 13). Для чужих файлов: Storage Access Framework (SAF). MANAGE_EXTERNAL_STORAGE только для file managers.

Какие security best practices для Android?
?
1) Минимальные permissions. 2) EncryptedSharedPreferences для секретов. 3) Certificate pinning для API. 4) ProGuard/R8 obfuscation. 5) Network Security Config (cleartext traffic). 6) SafetyNet/Play Integrity для tamper detection. 7) Biometric authentication для sensitive operations.
...
|-------------|------|-------|
| Следующий шаг | [[android-notifications]] | Notifications permission (Android 13+) |
| Углубиться | [[mobile-security-owasp]] | OWASP Mobile Security для Android |
| Смежная тема | [[ios-permissions-security]] | Privacy и permissions в iOS |
