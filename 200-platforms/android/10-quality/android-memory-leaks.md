...
> [!question]- Почему memory leaks опаснее на Android чем на сервере?
> 1) Ограниченный heap (128-512MB). 2) Нет swap (в отличие от сервера). 3) Система убьет приложение при high memory pressure (LMK). 4) Каждый leaked Activity = 1-10MB (ViewTree, Bitmaps). 5) Пользователь rotation 5 раз = 5 leaked Activities = 50MB. На сервере: больше памяти, swap file, graceful degradation.

> [!question]- Сценарий: LeakCanary показывает leak через Handler в Activity. Как исправить?
> Handler как inner class имеет implicit reference на Activity. Delayed message в MessageQueue держит Handler -> Activity. Решения: 1) WeakReference<Activity> в Handler. 2) removeCallbacksAndMessages(null) в onDestroy(). 3) Заменить на lifecycleScope.launch { delay(time); action() } -- автоматическая отмена. 4) Static Handler + WeakReference.
...
Какие типичные memory leaks в Android?
?
1) Inner class Handler/AsyncTask (implicit reference на Activity). 2) Static reference на Context/View. 3) Unregistered listeners (LocationManager, SensorManager). 4) Singleton с Activity Context. 5) Non-cancelled coroutines (GlobalScope). 6) Fragment в ViewPager без offscreen limit.

Что такое LeakCanary?
?
Библиотека для автоматического обнаружения memory leaks. Отслеживает GC: если объект не собран после Activity.onDestroy() -- leak. Показывает reference chain: какой объект держит leak. Debug only (no-op в release). Настройка: одна строка dependency.

Как GC работает на Android (ART)?
?
Concurrent Copying GC: паузы ~2-5ms. Generations: young (minor GC, частый), old (major GC, редкий). Large Object Space для Bitmaps. GC не решает leaks: если есть strong reference -- объект не собирается. Weak/SoftReference для кэшей.

Что такое Heap Dump analysis?
?
Snapshot всех объектов в памяти. Android Studio Memory Profiler -> Dump Heap. Показывает: размер объектов, reference chains, retained size. MAT (Eclipse Memory Analyzer) для детального анализа. hprof формат.

Как предотвращать leaks?
?
1) Application Context для синглтонов. 2) WeakReference для callbacks. 3) Unregister listeners в onDestroy/onPause. 4) viewModelScope/lifecycleScope вместо GlobalScope. 5) LeakCanary в debug. 6) Code review правила для Context usage.
...
|-------------|------|-------|
| Следующий шаг | [[android-process-memory]] | Как Android управляет памятью процессов |
| Углубиться | [[android-performance-profiling]] | Инструменты профилирования памяти |
| Смежная тема | [[ios-debugging]] | Memory debugging в iOS: Instruments Leaks |
