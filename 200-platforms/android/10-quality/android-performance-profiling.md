...
**[[android-view-rendering-pipeline]]** — понимание rendering pipeline (measure → layout → draw, VSYNC, Choreographer) критично для интерпретации результатов профилирования UI. Когда CPU Profiler показывает блокировку main thread, а GPU Profiling — пропущенные frames, знание pipeline помогает определить, на каком этапе возникает bottleneck: в layout computation, overdraw или sync с GPU. Изучите rendering pipeline перед глубоким анализом UI-производительности.

**[[android-compose-internals]]** — Compose recomposition создаёт уникальные паттерны производительности, отличные от View system. Profiling Compose требует понимания phases (Composition → Layout → Drawing), skippable vs non-skippable composables и Stability. Layout Inspector и Compose-specific metrics в Perfetto раскрывают проблемы, невидимые в стандартном CPU Profiler. Изучите Compose internals для оптимизации Compose UI.

**[[android-process-memory]]** — Memory Profiler и heap dump analysis требуют понимания heap structure, GC roots и процессной модели Android. Знание LMK priorities, OOM adjustment и trim memory callbacks объясняет, почему приложение убивается в фоне и как оптимизировать использование памяти. PSS vs USS метрики, native memory tracking и GC pause analysis — всё базируется на процессной модели.
...
> [!question]- Почему Android Studio Profiler не заменяет Perfetto/Systrace для production profiling?
> AS Profiler: удобный GUI, но только для debug builds (не оптимизированный код). Production: R8 optimized, другое поведение. Perfetto: работает с release builds, системные traces (scheduler, binder, GPU). Systrace: lightweight, подходит для CI. Макробенчмарки (androidx.benchmark) для автоматизированного profiling release builds.

> [!question]- Сценарий: приложение потребляет 300MB RAM на устройстве с 2GB. Как найти и исправить?
> 1) Memory Profiler: heap dump, найти крупные allocations. 2) LeakCanary для обнаружения memory leaks. 3) Bitmap analysis: unscaled images (5MB bitmap для 100dp ImageView). 4) Coil/Glide с resize для изображений. 5) WeakReference для кэшей. 6) onTrimMemory() callback для освобождения кэшей. 7) Проверить static references на Context/Activity.
...
Какие инструменты профилирования есть?
?
CPU: Android Profiler, Perfetto, Systrace. Memory: Memory Profiler, LeakCanary, MAT. Network: Network Profiler, OkHttp Logging. Rendering: GPU Overdraw, Profile GPU Rendering. Battery: Battery Historian. Benchmark: Macrobenchmark, Microbenchmark.

Что такое Perfetto?
?
System-wide tracing tool. Показывает: CPU scheduling, binder transactions, GPU rendering, memory allocations. Web UI: ui.perfetto.dev. Для Android: record через adb shell perfetto. Интеграция с Jetpack Benchmark для automated tracing.

Что такое Macrobenchmark?
?
Jetpack library для профилирования реальных сценариев: startup time, scroll jank, animation smoothness. Работает с release builds. Генерирует Perfetto traces. CI integration: сравнение с baseline. CompilationMode.Full() для AOT.

Что показывает GPU Overdraw?
?
Debug overlay: цвет показывает сколько раз каждый пиксель отрисован. True color: нет overdraw. Blue: 1x. Green: 2x. Pink: 3x. Red: 4+. Цель: максимум 2x для 90% экрана. Убирать лишние backgrounds.

Что такое Baseline Profiles?
?
Список hot code paths для AOT компиляции. При первом запуске: ART компилирует эти методы. Результат: 30-50% faster startup. Генерируются через Macrobenchmark. profileinstaller library для delivery. Обязательно для production apps.
...
|-------------|------|-------|
| Следующий шаг | [[android-memory-leaks]] | Детальный гайд по memory leaks |
| Углубиться | [[android-app-startup-performance]] | Оптимизация запуска приложения |
| Смежная тема | [[ios-performance-profiling]] | Instruments в iOS: аналог Android Profiler |
