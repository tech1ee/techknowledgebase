...
### Test Pyramid: формальная модель стратегии тестирования

> **Test Pyramid** (Cohn, *"Succeeding with Agile"*, 2009) — модель распределения тестов по уровням: много unit-тестов (быстрые, дешёвые), меньше integration-тестов, ещё меньше UI/E2E-тестов (медленные, дорогие). Google адаптировал эту модель для Android: **70% unit / 20% integration / 10% UI** (Android Testing Guide).

| Уровень | Скорость | Стоимость | Что тестирует | Инструмент |
|---------|----------|-----------|---------------|-----------|
| **Unit** | ~1 ms/тест | Низкая | Один класс в изоляции | JUnit 5, MockK |
| **Integration** | ~100 ms/тест | Средняя | Взаимодействие компонентов | Robolectric, Room in-memory |
| **UI/E2E** | ~1-10 s/тест | Высокая | Полный user flow | Espresso, Compose Testing, UI Automator |

### Test Doubles: таксономия Meszaros

> Gerard Meszaros (*"xUnit Test Patterns"*, 2007) формализовал пять типов test doubles:

| Тип | Определение | Пример в Android |
|-----|------------|-----------------|
| **Dummy** | Передаётся, но не используется | Пустой Context в конструкторе |
| **Stub** | Возвращает предопределённые данные | `every { repo.getUsers() } returns listOf(user)` |
| **Mock** | Проверяет, что метод был вызван | `verify { analytics.logEvent("screen_view") }` |
| **Fake** | Работающая упрощённая реализация | `FakeUserRepository` с in-memory list |
| **Spy** | Обёртка над реальным объектом с возможностью проверки | `spyk(realRepository)` |

### Тестируемость как свойство архитектуры

> Тестируемость — следствие принципов **SOLID** (Martin, 2000): Dependency Inversion позволяет подменять зависимости, Single Responsibility упрощает изоляцию. **Seam** (Feathers, *"Working Effectively with Legacy Code"*, 2004) — точка в коде, где поведение можно изменить без редактирования самого кода. Constructor injection через Hilt создаёт seams автоматически.

> **Связь**: Test Pyramid → [[android-ci-cd]], Test Doubles → [[kotlin-testing]], SOLID → [[android-architecture-patterns]]
...
| Источник | Применение |
|----------|-----------|
| Cohn M. *Succeeding with Agile* (2009) | Test Pyramid — 70% unit, 20% integration, 10% UI |
| Meszaros G. *xUnit Test Patterns* (2007) | Test Double taxonomy: Stub, Mock, Fake, Spy, Dummy |
| Fowler M. *Refactoring* (1999) | Тестируемость как свойство архитектуры |

### Практические руководства
- [Test Apps on Android - Android Developers](https://developer.android.com/training/testing) — официальная документация
- [Test Your ViewModel](https://developer.android.com/topic/libraries/architecture/viewmodel/viewmodel-testing) — тестирование ViewModel
- [Testing Kotlin Coroutines](https://developer.android.com/kotlin/coroutines/test) — тестирование coroutines
- [Espresso](https://developer.android.com/training/testing/espresso) — UI тесты

### Книги
- Phillips B. et al. (2022). *Android Programming: The Big Nerd Ranch Guide.* — unit-тесты, Espresso, ViewModel.
- Bloch J. (2018). *Effective Java.* — тестируемый код: immutability, DI.
- Moskala M. (2022). *Kotlin Coroutines Deep Dive.* — TestDispatcher, runTest, Turbine.
