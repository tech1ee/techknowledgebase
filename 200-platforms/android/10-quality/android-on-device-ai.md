...
### On-Device Inference: формальная модель

> **Inference** (вывод) — применение обученной нейронной сети к новым данным. On-device inference выполняет inference локально на мобильном устройстве, без отправки данных на сервер. Формально: `y = f(x; θ)`, где `f` — архитектура модели, `θ` — обученные веса, `x` — входные данные, `y` — результат. Задача on-device: выполнить `f(x; θ)` за < 100 мс на мобильном процессоре.

| Характеристика | Cloud Inference | On-Device Inference |
|---------------|----------------|---------------------|
| **Latency** | 200-500 мс (network + compute) | 10-200 мс (только compute) |
| **Privacy** | Данные передаются на сервер | Данные не покидают устройство |
| **Availability** | Требует интернет | Работает offline |
| **Cost** | Per-request pricing (API calls) | Бесплатно после загрузки модели |
| **Model size** | Неограничен (GPU clusters) | Ограничен RAM/Storage (1-4 GB) |

### Quantization: сжатие моделей для мобильных устройств

> **Quantization** — уменьшение битовой точности весов модели для снижения размера и ускорения inference. Формально: замена `FP32` (32 бита, ~7 знаков) на `INT8` (8 бит) или `INT4` (4 бита). Jacob et al. (2018, *"Quantization and Training of Neural Networks for Efficient Integer-Arithmetic-Only Inference"*) показали, что INT8 quantization снижает размер модели в 4x с потерей точности < 1%.

| Precision | Размер / параметр | Размер модели 3B params | Потеря точности | Поддержка |
|-----------|-------------------|------------------------|-----------------|-----------|
| **FP32** | 4 bytes | 12 GB | Baseline | CPU |
| **FP16** | 2 bytes | 6 GB | ~0% | GPU |
| **INT8** | 1 byte | 3 GB | < 1% | NPU, GPU |
| **INT4** | 0.5 bytes | 1.5 GB | 1-3% | NPU (Tensor G5, Snapdragon) |

### Neural Processing Unit (NPU): специализированный hardware

> NPU — выделенный процессор для матричных операций (matrix multiply-accumulate, MAC). В отличие от GPU (оптимизирован для графики с параллельными shaders), NPU оптимизирован для **tensor operations** — основы inference нейронных сетей. Современные мобильные SoC (Tensor G5, Snapdragon 8 Elite, Exynos 2500) содержат NPU с производительностью 40-75 TOPS (Tera Operations Per Second).

> **Связь**: ML модели → [[mobile-ai-ml-guide]], Quantization → [[ai-ml-fundamentals]], Hardware → [[android-kernel-extensions]]
...
| Источник | Применение |
|----------|-----------|
| Vaswani A. et al. *Attention Is All You Need* (2017, NeurIPS) | Transformer architecture — основа Gemini Nano |
| Jacob B. et al. *Quantization and Training of Neural Networks for Efficient Integer-Arithmetic-Only Inference* (2018) | Quantization (INT4/INT8) для мобильных устройств |
| Hinton G. et al. *Distilling the Knowledge in a Neural Network* (2015) | Knowledge Distillation → compact on-device models |
| Sze V. et al. *Efficient Processing of Deep Neural Networks* (2017) | NPU hardware, inference optimization |
