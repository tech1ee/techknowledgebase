...
**[[android-compilation-pipeline]]** — ART использует JIT и AOT компиляцию, и Baseline Profiles работают именно через dex2oat, предкомпилируя критические методы при установке. Понимание compilation pipeline объясняет, почему первый запуск после установки медленнее (JIT), и как Baseline Profiles ускоряют cold start за счёт AOT компиляции горячих путей. Изучайте pipeline для понимания механизма оптимизации.

**[[android-process-memory]]** — процесс запуска начинается с fork() от Zygote, и тип запуска (cold/warm/hot) определяется тем, жив ли процесс приложения. LMK может убить фоновый процесс, что превращает следующий запуск в cold start. Понимание модели памяти критично для оптимизации: shared memory от Zygote — причина, почему fork быстрый.

**[[android-activity-lifecycle]]** — startup включает полный lifecycle Activity (onCreate -> onStart -> onResume), и время каждого callback входит в TTID/TTFD метрики. Тяжёлые операции в onCreate непосредственно увеличивают время запуска. Изучение lifecycle помогает понять, какой код выполняется на каком этапе startup.

**[[android-content-provider-internals]]** — ContentProvider.onCreate() вызывается ДО Application.onCreate(), что делает его скрытым bottleneck: каждая библиотека с собственным CP (Firebase, WorkManager, LeakCanary) добавляет 15-30ms к cold start. App Startup library решает это, объединяя инициализацию в один ContentProvider.

**[[android-view-rendering-pipeline]]** — последний этап startup — отрисовка первого кадра (measure/layout/draw). Глубокая иерархия View, тяжёлые drawable ресурсы или сложные ConstraintLayout увеличивают TTID. Понимание rendering pipeline помогает оптимизировать layout первого экрана для быстрого отображения.

**[[android-performance-profiling]]** — Perfetto, CPU profiler и memory profiler — основные инструменты диагностики проблем startup. Perfetto trace показывает точное время каждого этапа (fork, ContentProviders, Application, Activity, first frame), позволяя найти bottleneck. Macrobenchmark автоматизирует измерение startup метрик в CI/CD.
...
> [!question]- Почему cold start -- самый важный performance metric для пользователя?
> Cold start -- первое впечатление. 53% пользователей покидают приложение, загружающееся дольше 3 секунд (Google data). Cold start включает: fork от Zygote, Application.onCreate(), Activity lifecycle, первый кадр. Каждая инициализация в Application замедляет старт. Цель: < 500ms для простых приложений.

> [!question]- Сценарий: Application.onCreate() инициализирует 15 библиотек и занимает 2 секунды. Как оптимизировать?
> 1) App Startup library: lazy initialization, инициализировать только при первом использовании. 2) Background init: Dispatchers.IO для non-UI библиотек. 3) Удалить ненужные ContentProvider (firebase, работа с рекламой -- каждый добавляет ~50ms). 4) Baseline Profiles для AOT компиляции hot paths. 5) Macrobenchmark для измерения. 6) StartupTimingMetric для CI regression detection.
...
Какие типы startup?
?
Cold: процесс не существует, полная инициализация (~500ms+). Warm: процесс жив, Activity уничтожена (~200ms). Hot: Activity в памяти (~50ms). TTID (Time To Initial Display) и TTFD (Time To Full Display) -- ключевые метрики.

Что такое App Startup library?
?
Jetpack библиотека для управления инициализацией. Заменяет ContentProvider-based init (каждый CP = ~2ms + init code). Lazy init: инициализация при первом использовании. Dependency graph между initializers. Merges into single ContentProvider.

Что такое Baseline Profiles?
?
Список методов для AOT компиляции. ART компилирует эти методы при установке/обновлении. Результат: 30-50% faster startup, smoother scrolling. Генерируются через Macrobenchmark. profileinstaller delivers profiles in APK/AAB.

Как измерить startup time?
?
1) adb shell am start-activity -W (TotalTime). 2) Macrobenchmark: StartupTimingMetric. 3) Perfetto trace: reportFullyDrawn(). 4) Firebase Performance: automatic startup trace. 5) Custom: System.nanoTime() в Application.onCreate() до first frame.

Что такое SplashScreen API?
?
Android 12+: системный splash screen (icon + background). SplashScreen compat library для Android 5+. Можно задержать: installSplashScreen().setKeepOnScreenCondition { isLoading }. Заменяет custom splash Activity. Время: ~200ms по умолчанию.
...
|-------------|------|-------|
| Следующий шаг | [[android-performance-profiling]] | Инструменты для профилирования startup |
| Углубиться | [[android-compilation-pipeline]] | Как AOT и Baseline Profiles работают в ART |
| Смежная тема | [[ios-app-startup-performance]] | Startup optimization в iOS: dyld и pre-main |
