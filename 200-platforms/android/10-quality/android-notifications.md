...
**[[android-intent-internals]]** — PendingIntent является ключевым механизмом уведомлений: он делегирует identity вашего приложения системе, позволяя запускать Activity или Service при тапе на notification. Понимание PendingIntent token model, FLAG_IMMUTABLE/FLAG_MUTABLE и request code matching необходимо для правильной работы notification actions, Direct Reply и navigation из уведомлений. С Android 12 требование FLAG_IMMUTABLE делает знание PendingIntent критичным. Изучите Intent internals перед реализацией сложных уведомлений.

**[[android-service-internals]]** — Foreground Service обязан показывать notification (startForeground()), а с Android 14 требует указания конкретного service type с соответствующими permissions. Понимание lifecycle Service (Started vs Bound), timing вызова startForeground() (5-секундное ограничение) и типов FGS критично для фоновых задач с уведомлениями. Без корректного notification Foreground Service крашится с RemoteServiceException.

**[[android-permissions-security]]** — с Android 13 уведомления требуют runtime permission POST_NOTIFICATIONS, без которого они молча отбрасываются системой. Полноэкранные уведомления (звонки, будильники) требуют специального permission USE_FULL_SCREEN_INTENT. NotificationListenerService требует BIND_NOTIFICATION_LISTENER_SERVICE. Понимание runtime permissions модели критично для правильного запроса и обработки отказа. Изучите permissions перед реализацией уведомлений для Android 13+.

**[[android-broadcast-internals]]** — BroadcastReceiver обрабатывает notification actions: Direct Reply, кнопки действий и dismiss callbacks. Система отправляет broadcast при изменении каналов уведомлений пользователем. Понимание ordered vs normal broadcasts, implicit receiver restrictions (Android 8+) и PendingIntent для BroadcastReceiver помогает реализовать интерактивные уведомления. Изучите broadcast internals для notification actions.

**[[android-compose]]** — Compose используется для построения UI permission-запросов, настроек каналов уведомлений и in-app notification preferences. rememberLauncherForActivityResult интегрируется с runtime permission flow для POST_NOTIFICATIONS. Понимание Compose side effects (LaunchedEffect) помогает правильно запрашивать permissions в Compose-based UI.
...
> [!question]- Почему Android требует Notification Channel начиная с Android 8?
> До Android 8: пользователь мог только полностью отключить уведомления приложения. Notification Channels дают гранулярный контроль: отключить marketing уведомления, оставить сообщения. Importance level, звук, вибрация настраиваются per-channel. Приложение без channels не может показывать уведомления на Android 8+.

> [!question]- Сценарий: уведомления не приходят на Xiaomi/Samsung, хотя на Pixel работают. Почему?
> OEM battery optimization: Xiaomi (MIUI), Samsung (OneUI), Huawei (EMUI) агрессивно убивают фоновые процессы. Push через FCM может не доставиться если процесс убит. Решения: 1) Просить пользователя отключить battery optimization. 2) Foreground Service для критичных. 3) High priority FCM messages. 4) Autostart permission (OEM-specific). dontkillmyapp.com для инструкций.
...
Какие компоненты системы уведомлений?
?
NotificationChannel: категория с настройками (importance, sound). NotificationCompat.Builder: создание уведомления (title, text, icon, actions). NotificationManager: показ/отмена. FCM (Firebase Cloud Messaging): push от сервера. PendingIntent: действие при нажатии.

Какие уровни Importance?
?
IMPORTANCE_HIGH: heads-up, звук. IMPORTANCE_DEFAULT: звук, в shade. IMPORTANCE_LOW: без звука, в shade. IMPORTANCE_MIN: без звука, свернутое. IMPORTANCE_NONE: скрытые. Пользователь может переопределить в настройках.

Что такое FCM?
?
Firebase Cloud Messaging: бесплатный push service от Google. Два типа: Notification (показывается системой если app в background) и Data (обрабатывается в onMessageReceived). Требует google-services.json. Token для идентификации устройства.

Что изменилось в Android 13 для notifications?
?
POST_NOTIFICATIONS runtime permission (как камера/микрофон). Приложение должно запросить permission перед показом уведомлений. Без permission -- уведомления не показываются. Рекомендация: запрашивать в контексте (после значимого действия пользователя).

Как показать notification с actions?
?
NotificationCompat.Builder.addAction(icon, title, pendingIntent). До 3 actions. PendingIntent: Activity, BroadcastReceiver, или Service. Reply action: RemoteInput для ввода текста прямо из notification. MessagingStyle для чат-подобных notification.
...
|-------------|------|-------|
| Следующий шаг | [[android-permissions-security]] | Permissions для уведомлений и других API |
| Углубиться | [[android-background-work]] | Background delivery и FCM |
| Смежная тема | [[ios-notifications]] | Push notifications в iOS: APNs |
