...
> [!question]- Почему Android использует компонентную модель вместо единой точки входа main()?
> Мобильные устройства имеют ограниченную память и батарею. Компонентная модель позволяет системе запускать только нужные части приложения, убивать неиспользуемые компоненты для освобождения памяти и интегрировать приложения друг с другом через стандартные механизмы (Intent, ContentProvider).

> [!question]- Что произойдет с данными в ViewModel при process death и почему?
> Данные будут потеряны, потому что ViewModel переживает только configuration changes (через ViewModelStore в Activity), но не process death. При убийстве процесса системой вся память процесса освобождается. Для сохранения данных при process death нужен SavedStateHandle или persistent storage (Room, DataStore).

> [!question]- Сценарий: приложение тормозит при скролле списка. Какой первый показатель вы проверите и почему?
> Frame time на Main Thread. Android рендерит UI с частотой 60 FPS, то есть каждый кадр должен укладываться в 16ms. Если Main Thread заблокирован тяжелой работой (сеть, БД, обработка изображений), кадры пропускаются и пользователь видит jank. Нужно вынести тяжелую работу в фоновые потоки/корутины.

> [!question]- Почему Compose не заменил полностью View system и зачем знать оба подхода?
> Более 70% существующих приложений используют XML или гибридный подход. Знание View system критично для работы с legacy-кодом, interop между Compose и View, понимания RecyclerView internals и кастомных View. Compose работает поверх View system, поэтому глубокое понимание обоих подходов делает разработчика эффективнее.
...
Что такое ART и чем он отличается от JVM?
?
ART (Android Runtime) -- среда выполнения Android-приложений. Использует DEX bytecode (register-based) вместо Java bytecode (stack-based), гибридную AOT+JIT компиляцию и GC с короткими паузами (~2ms) оптимизированный для мобильных устройств.

Что такое Zygote и зачем он нужен?
?
Zygote -- процесс-шаблон, который при загрузке системы инициализирует ART и загружает ~8000 классов framework. Новые приложения создаются через fork() от Zygote, что дает запуск за ~200ms вместо ~2000ms благодаря Copy-on-Write.

Что такое Low Memory Killer (LMK)?
?
Механизм Android, который превентивно убивает процессы ДО критической нехватки памяти. Приоритеты: Foreground > Visible > Service > Cached > Empty. Система всегда остается отзывчивой.

Что такое ANR и когда он возникает?
?
Application Not Responding -- диалог, который система показывает когда Main Thread заблокирован более 5 секунд. Причина: тяжелая работа (сеть, БД) выполняется на UI Thread вместо фоновых потоков.

Какое ограничение по времени на один кадр UI?
?
16ms при 60 FPS. Превышение приводит к пропуску кадров (jank). Тяжелые операции нужно выносить с Main Thread.

Что такое Configuration Change?
?
Изменение конфигурации устройства: поворот экрана, смена языка, изменение размера окна. Приводит к пересозданию Activity, поэтому данные нужно сохранять в ViewModel или SavedStateHandle.

Какой типичный heap limit на Android-устройстве?
?
128-512MB в зависимости от устройства. При превышении -- OutOfMemoryError. Это значительно меньше десктопных приложений, поэтому управление памятью критически важно.
...
|-------------|------|-------|
| Следующий шаг | [[android-architecture]] | Понять архитектуру системы: ART, Zygote, Binder |
| Углубиться | [[android-process-memory]] | Как система управляет памятью и процессами |
| Смежная тема | [[os-processes-threads]] | Процессы и потоки на уровне ОС -- фундамент Android |
