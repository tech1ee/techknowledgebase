...
> [!question]- Почему Android использует компоненты (Activity, Service, BR, CP) вместо единого main()?
> Компонентная модель адаптирована к ограничениям мобильных устройств: мало памяти, батарея, постоянное переключение. Система может запускать только нужные части приложения, убивать неиспользуемые компоненты для освобождения памяти и интегрировать приложения через стандартные Intent.

> [!question]- Сценарий: вам нужно скачать файл размером 500MB. Какой компонент выберете и почему?
> Foreground Service с уведомлением. Это долгая операция, которую пользователь осознает (видит прогресс). WorkManager не подходит, так как система может отложить выполнение. Background Service убьется через минуту на Android 8+. Foreground Service гарантирует высокий приоритет процесса.

> [!question]- Почему сохранение Activity Context в синглтоне приводит к утечке памяти?
> Activity Context привязан к жизненному циклу Activity. Синглтон живет весь жизненный цикл приложения. Если синглтон держит ссылку на Activity Context, GC не может собрать Activity после onDestroy(). Решение: использовать Application Context для синглтонов.

> [!question]- В чем разница между explicit и implicit Intent и когда использовать каждый?
> Explicit Intent указывает конкретный класс компонента -- используется для своих компонентов. Implicit Intent описывает действие (ACTION_VIEW, ACTION_SEND) -- система находит подходящий обработчик. Implicit для интеграции с другими приложениями (открыть URL, отправить email).
...
Какие четыре основных компонента Android-приложения?
?
Activity (экран с UI), Service (фоновая работа без UI), BroadcastReceiver (получение событий), ContentProvider (доступ к данным между приложениями). Каждый регистрируется в AndroidManifest.xml.

Чем Foreground Service отличается от WorkManager?
?
Foreground Service -- для долгих операций с уведомлением, которые пользователь осознает (музыка, навигация). WorkManager -- для отложенных/периодических задач, которые могут подождать. Система сама выберет оптимальное время для WorkManager.

Что возвращает onStartCommand() и зачем?
?
START_NOT_STICKY (не перезапускать), START_STICKY (перезапустить с null Intent), START_REDELIVER_INTENT (перезапустить с последним Intent). Определяет поведение Service после убийства системой.

Что такое PendingIntent?
?
Intent, который будет выполнен позже от имени вашего приложения. Используется в уведомлениях, AlarmManager, виджетах. Android 12+ требует FLAG_IMMUTABLE или FLAG_MUTABLE.

Какие ограничения на BroadcastReceiver появились в Android 8+?
?
Большинство implicit broadcasts больше не доставляются приложениям через Manifest. Работают: BOOT_COMPLETED, LOCALE_CHANGED, explicit broadcasts. Остальные нужно регистрировать динамически в коде.

Зачем нужен ContentProvider?
?
Стандартный интерфейс для доступа к данным между приложениями (контакты, календарь). FileProvider для безопасной передачи файлов. Для внутренних данных приложения лучше использовать Room напрямую.
...
|-------------|------|-------|
| Следующий шаг | [[android-activity-lifecycle]] | Жизненный цикл Activity -- самого сложного компонента |
| Углубиться | [[android-intent-internals]] | Как работает Intent resolution под капотом |
| Смежная тема | [[os-processes-threads]] | Процессы ОС -- основа компонентной модели Android |
