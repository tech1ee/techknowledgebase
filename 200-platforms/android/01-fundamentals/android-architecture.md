...
> [!question]- Почему Binder использует только одно копирование данных, а стандартные IPC-механизмы Linux -- два?
> Binder использует mmap для отображения kernel buffer в адресное пространство получателя. Данные копируются один раз из отправителя в kernel buffer, который уже доступен получателю через mmap. Традиционные IPC (pipes, sockets) копируют данные дважды: user->kernel и kernel->user.

> [!question]- Сценарий: приложение крашится при повороте экрана. Как архитектура Android объясняет эту проблему?
> При повороте экрана происходит Configuration Change. Android убивает и пересоздает Activity (вызывает onDestroy и onCreate). Если данные хранились в полях Activity, они теряются. Это следствие архитектуры: система, а не приложение, контролирует lifecycle. Решение: ViewModel для config changes, SavedStateHandle для process death.

> [!question]- Почему Android использует Profile-Guided Compilation вместо чистого AOT или JIT?
> Чистый AOT дает быстрый запуск, но долгую установку и большой размер на диске. Чистый JIT дает быструю установку, но медленный первый запуск. Profile-Guided Compilation комбинирует оба: первый запуск с JIT создает профиль, фоновая AOT компилирует только hot code. Результат: быстрая установка + быстрый запуск + меньше места.
...
Какие слои входят в архитектуру Android (снизу вверх)?
?
Linux Kernel -> HAL (Hardware Abstraction Layer) -> Native Libraries (C/C++) -> Android Runtime (ART) -> Application Framework -> Applications. Каждый слой предоставляет абстракции для вышестоящего.

Что такое Binder и почему он лучше стандартных IPC?
?
Binder -- механизм IPC Android с одним копированием через mmap (vs 2 для sockets), встроенной идентификацией вызывающего (UID/PID), death notifications и reference counting. Оптимизирован для частых коротких сообщений.

Как Zygote экономит память через Copy-on-Write?
?
Zygote загружает framework (~50MB) один раз. При fork() страницы памяти shared между всеми приложениями и копируются только при записи. 10 приложений используют ~50MB shared + ~20MB private каждое вместо ~70MB * 10.

Что такое System Server и какие сервисы он содержит?
?
Первый процесс от Zygote. Содержит: ActivityManagerService (lifecycle), WindowManagerService (окна), PackageManagerService (установка), PowerManagerService (wakelocks), NotificationManagerService и другие.

Чем отличается Cold, Warm и Hot start?
?
Cold (~500ms+): fork от Zygote, загрузка APK, Application.onCreate(), Activity lifecycle. Warm (~200ms): процесс жив, Activity уничтожена. Hot (~50ms): Activity в памяти, только onResume().

Как ART GC отличается от JVM GC?
?
ART GC (Concurrent Copying) оптимизирован для мобильных: паузы ~2-5ms, throughput ~85%. JVM G1: паузы 50-200ms, throughput ~90%. ART приоритезирует короткие паузы для плавного UI.

Что такое DEX bytecode и чем отличается от Java bytecode?
?
DEX (Dalvik Executable) -- register-based формат (меньше инструкций), компактнее (единая таблица констант), несколько классов в одном файле. Java bytecode -- stack-based. DEX оптимизирован для мобильных устройств.
