...
> [!question]- Почему квалификаторы ресурсов должны идти в строго определенном порядке?
> Android использует алгоритм elimination для выбора ресурсов: система последовательно исключает директории, не соответствующие конфигурации, в порядке приоритета квалификаторов. Нарушение порядка (например, values-night-ru/ вместо values-ru-night/) приведет к тому, что ресурс не будет найден.

> [!question]- Сценарий: после включения nonTransitiveRClass проект перестал компилироваться. Что нужно исправить?
> При nonTransitiveRClass каждый модуль видит только свои ресурсы. Нужно добавить explicit imports для ресурсов из зависимостей: import com.example.core.ui.R as CoreR. Это делает зависимости между модулями явными и ускоряет компиляцию за счет уменьшения размера R класса.

> [!question]- Почему Vector Drawable не всегда лучше растрового изображения?
> Vector Drawable требует CPU для рендеринга при каждом изменении размера. Сложные vectors с множеством path могут быть медленнее pre-rendered PNG. Для фотографий и сложных изображений с градиентами растровые форматы (WebP, PNG) эффективнее. Vectors оптимальны для иконок и простых иллюстраций.
...
Что такое AAPT2 и что он делает?
?
Android Asset Packaging Tool -- компилирует ресурсы в бинарный формат (.flat), валидирует XML, генерирует R класс с ID ресурсов и выполняет resource merging из модулей и библиотек.

Как работает density-independent pixel (dp)?
?
dp -- абстракция над физическими пикселями. Формула: px = dp * (dpi / 160). На xxhdpi (480 dpi): 1dp = 3px. Гарантирует одинаковый физический размер элементов на разных устройствах.

Что такое Smallest Width qualifier (sw)?
?
Минимальная из ширины и высоты экрана в dp. sw320dp -- телефоны, sw600dp -- планшеты 7 дюймов, sw720dp -- планшеты 10 дюймов. Не меняется при повороте экрана.

Как работает shrinkResources?
?
Удаляет неиспользуемые ресурсы из APK. Требует minifyEnabled=true. Анализирует код через R8 и находит используемые ресурсы. Динамически загружаемые ресурсы (getIdentifier) нужно указать в res/raw/keep.xml.

В чем разница между res/raw/ и assets/?
?
raw/ -- доступ через R.raw.filename (compile-time checked). assets/ -- доступ через AssetManager (string path). assets/ поддерживает поддиректории и произвольные имена. raw/ включается в resource merging.

Что такое Per-App Language (Android 13+)?
?
Возможность устанавливать язык приложения независимо от системного. Требует locales_config.xml в манифесте. Пользователь выбирает язык в настройках системы или программно через LocaleManager.
...
|-------------|------|-------|
| Следующий шаг | [[android-ui-views]] | View system -- потребитель ресурсов |
| Углубиться | [[android-compilation-pipeline]] | Как AAPT2 компилирует ресурсы в APK |
| Смежная тема | [[android-apk-aab]] | Как ресурсы оптимизируются в App Bundle |
