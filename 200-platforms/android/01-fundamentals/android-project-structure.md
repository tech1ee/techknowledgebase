...
> [!question]- Почему :core:domain должен быть Kotlin/JVM модулем, а не Android Library?
> Domain содержит бизнес-логику без зависимости от Android. Kotlin/JVM модуль: быстрее компилируется (не нужен Android SDK), проще тестируется (обычные JUnit без эмулятора), переиспользуется в KMP проектах. Это реализует принцип инверсии зависимостей -- domain не знает о платформе.

> [!question]- Сценарий: сборка многомодульного проекта с 20 модулями занимает 5 минут. Какие первые шаги для ускорения?
> 1) Включить Gradle caching и configuration-cache в gradle.properties. 2) Использовать implementation вместо api для зависимостей (уменьшает recompilation scope). 3) Заменить buildSrc на Convention Plugins (инкрементальная компиляция). 4) Проверить, что модули не имеют ненужных зависимостей друг на друга.

> [!question]- Почему Convention Plugins лучше buildSrc?
> buildSrc перекомпилирует весь проект при любом изменении конфигурации. Convention Plugins в composite build компилируются инкрементально и поддерживают Configuration Cache. При изменении одного plugin пересобирается только он, а не весь проект.
...
Какие типы модулей существуют в Android-проекте?
?
Application (com.android.application -> APK/AAB), Library (com.android.library -> AAR), Dynamic Feature (загрузка по требованию), Kotlin/JVM (kotlin('jvm') -- чистый Kotlin без Android, для domain layer).

Что такое Source Set и какие бывают?
?
Набор исходных файлов для варианта сборки: main (общий), debug/release (build type), free/paid (product flavor), test (unit тесты), androidTest (instrumented тесты). Мержатся по приоритету.

Что такое Version Catalog (libs.versions.toml)?
?
Централизованное хранилище версий зависимостей. Единый источник правды, type-safe доступ (libs.retrofit), поддержка bundles и plugins. Избавляет от разных версий одной библиотеки в разных модулях.

Какие три основные dependency configuration в Gradle?
?
implementation (внутренняя, не протекает наружу), api (экспортируемая, видна потребителям), compileOnly (только для компиляции, не включается в runtime).

Что хранится в local.properties и почему не коммитится?
?
Локальные пути (sdk.dir, ndk.dir), keystore для подписи, API ключи. Специфичны для машины разработчика и содержат секреты. Всегда в .gitignore.

Какой порядок приоритета Source Sets для варианта devDebug?
?
src/devDebug/ > src/dev/ (flavor) > src/debug/ (build type) > src/main/ (общий). Ресурсы файлов заменяются, values/*.xml мержатся.
...
|-------------|------|-------|
| Следующий шаг | [[android-gradle-fundamentals]] | Gradle и AGP под капотом |
| Углубиться | [[android-modularization]] | Стратегии модуляризации для масштабных проектов |
| Смежная тема | [[build-systems-theory]] | Теория систем сборки -- DAG, кэширование, инкрементальность |
