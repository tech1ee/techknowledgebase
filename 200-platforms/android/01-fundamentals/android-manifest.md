...
> [!question]- В чем разница между namespace и applicationId и почему они разделены?
> Namespace используется для генерации R класса и относительных имен компонентов -- одинаковый для всех flavors. ApplicationId -- уникальный идентификатор в Play Store, может отличаться для flavors (free, paid). Разделение позволяет иметь один код (R.string.xxx) для разных вариантов приложения.

> [!question]- Почему с Android 12 обязателен атрибут exported для компонентов с intent-filter?
> До Android 12 компоненты с intent-filter были неявно exported=true, что создавало уязвимости: любое приложение могло запустить ваш компонент. Явное указание exported заставляет разработчика осознанно решать, должен ли компонент быть доступен извне, улучшая безопасность.

> [!question]- Сценарий: приложение не появляется в Play Store на устройствах без камеры, хотя камера опциональна. В чем причина?
> Permission CAMERA неявно добавляет uses-feature android.hardware.camera required=true. Play Store фильтрует устройства по обязательным features. Решение: добавить uses-feature android:name='android.hardware.camera' android:required='false' и проверять наличие камеры в runtime.
...
?
Декларативный XML-файл, описывающий приложение системе Android: компоненты, разрешения, requirements к устройству, метаданные. Без него приложение не может быть установлено.

Какие Launch Modes существуют для Activity?
?
standard (новый instance каждый раз), singleTop (переиспользование если на вершине стека), singleTask (один instance в task), singleInstance (один instance в отдельном task).

Чем Deep Link отличается от App Link?
?
Deep Link (custom scheme://) работает без верификации, показывает chooser dialog. App Link (https://) требует Digital Asset Links файл на сервере, открывается напрямую в приложении без chooser.

Как работает Manifest Merger?
?
При сборке объединяет манифесты: build variant > main > library > AAR dependencies. Конфликты решаются через tools:replace, tools:remove, tools:node='remove/replace'.

Какие foregroundServiceType обязательны с Android 14?
?
camera, connectedDevice, dataSync, location, mediaPlayback, mediaProjection, microphone, phoneCall. Каждый требует соответствующего FOREGROUND_SERVICE_* permission.

Что такое Manifest Placeholders?
?
Переменные в манифесте (${VARIABLE}), значения которых задаются в build.gradle.kts через manifestPlaceholders. Позволяют иметь разные значения для debug/release (API keys, icons).
...
|-------------|------|-------|
| Следующий шаг | [[android-project-structure]] | Структура проекта: модули и source sets |
| Углубиться | [[android-permissions-security]] | Система разрешений и безопасность |
| Смежная тема | [[android-navigation]] | Deep Links и App Links в навигации |
