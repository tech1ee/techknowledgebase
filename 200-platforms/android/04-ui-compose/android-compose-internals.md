...
> [!question]- Почему Compose использует Gap Buffer (SlotTable) вместо обычного дерева для хранения composition?
> Gap Buffer обеспечивает O(1) для последовательных вставок/удалений, что идеально для линейного обхода composition tree. Обычное дерево требовало бы O(log n) для каждой операции и больше аллокаций. SlotTable хранит данные в плоском массиве, что улучшает cache locality и уменьшает GC pressure.

> [!question]- Сценарий: composable функция перекомпонуется, хотя её параметры не изменились. Почему?
> Возможные причины: 1) Параметр нестабильный тип (var свойства, List вместо ImmutableList). Compose Compiler не может доказать equality и всегда перекомпонует. 2) Lambda без remember захватывает нестабильную переменную. 3) CompositionLocal изменился выше по дереву. Проверить через Layout Inspector -> recomposition counts или Compose Compiler reports.
...
Что делает Compose Compiler Plugin?
?
Трансформирует @Composable функции: добавляет параметр Composer, вставляет вызовы startGroup/endGroup для tracking, генерирует код сравнения параметров для skip recomposition. Без plugin @Composable -- обычная функция.

Что такое SlotTable в Compose?
?
Gap Buffer -- плоский массив для хранения composition data: groups (composable вызовы), slots (state, параметры). Обеспечивает O(1) sequential access. Две таблицы: текущая (read) и pending (write). После composition swap.

Что такое Snapshot System?
?
Механизм изоляции state changes. Каждый поток видит consistent snapshot данных. При записи в mutableStateOf() создается новая версия. Merge конфликтов при apply. Аналог MVCC в базах данных.

Как Compose определяет что нужно recompose?
?
1) State read tracking: при чтении mutableStateOf() Compose записывает какой scope читает значение. 2) При изменении state -- invalidate всех читающих scopes. 3) Параметры сравниваются через equals(). Stable типы могут быть skipped.

Что такое Stability в Compose?
?
Compose Compiler анализирует типы: Stable (immutable, equals reliable -- можно skip), Unstable (может измениться -- всегда recompose). val + data class = Stable. var или List = Unstable. @Stable/@Immutable аннотации для ручной пометки.
...
|-------------|------|-------|
| Следующий шаг | [[android-state-management]] | State management поверх Snapshot System |
| Углубиться | [[android-view-rendering-pipeline]] | Сравнение rendering pipeline: View vs Compose |
| Смежная тема | [[compilation-pipeline]] | Compiler plugins и code generation |
