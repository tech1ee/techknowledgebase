...
> [!question]- Почему для UI state рекомендуется StateFlow, а не LiveData в Compose?
> StateFlow: 1) Kotlin-first, работает с корутинами. 2) Всегда имеет начальное значение. 3) Использует structural equality для фильтрации дубликатов. 4) Thread-safe из коробки. LiveData: привязан к lifecycle (не нужен в Compose -- collectAsStateWithLifecycle это делает), Java-centric API, не поддерживает операторы flow.

> [!question]- Сценарий: при повороте экрана показывается Snackbar повторно, хотя событие уже обработано. Как исправить?
> Проблема: событие хранится в StateFlow и при resubscribe после rotation доставляется повторно. Решение: использовать Channel с receiveAsFlow() для one-time events. Channel FIFO -- событие доставляется один раз. Альтернатива: SharedFlow(replay=0) или Event wrapper с consumed flag.

> [!question]- Почему MVI предпочтительнее MVVM для сложных экранов?
> MVI: single state object, unidirectional data flow (Intent -> Reducer -> State -> UI). Преимущества для сложных экранов: 1) Все состояние в одном месте -- проще debug. 2) State transitions предсказуемы (pure reducer). 3) Time-travel debugging возможен. 4) Легче тестировать. Недостаток: больше boilerplate для простых экранов.
...
?
StateFlow: всегда имеет значение (replay=1), conflated (новое заменяет старое), structural equality. SharedFlow: настраиваемый replay, extraBufferCapacity, onBufferOverflow. StateFlow для state, SharedFlow для events.

Что такое collectAsStateWithLifecycle()?
?
Extension для безопасного сбора Flow в Compose. Привязан к lifecycle: собирает когда UI видим (STARTED+), останавливает когда невидим. Экономит ресурсы. Заменяет collectAsState() + repeatOnLifecycle.

Что такое UDF (Unidirectional Data Flow)?
?
Паттерн: данные текут в одном направлении. UI -> Event -> ViewModel -> State -> UI. Упрощает отладку: всегда понятно откуда пришло состояние. Основа MVI и Compose state management.

Как организовать UI State в ViewModel?
?
Один sealed interface/data class для всего состояния экрана. StateFlow<ScreenState> в ViewModel. UI подписывается через collectAsStateWithLifecycle(). Events через sealed class + Channel или SharedFlow.

Что такое derived state в Compose?
?
derivedStateOf {} -- вычисляемое значение, которое обновляется только при изменении зависимостей. Аналог computed property. Снижает ненужные recomposition. Пример: derivedStateOf { list.filter { it.isActive } }.
...
|-------------|------|-------|
| Следующий шаг | [[android-viewmodel-internals]] | Как ViewModel хранит state через config changes |
| Углубиться | [[android-compose-internals]] | Snapshot System -- основа state в Compose |
| Смежная тема | [[ios-state-management]] | State management в SwiftUI и iOS |
