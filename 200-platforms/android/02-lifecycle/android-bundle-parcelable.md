...
> [!question]- Почему Bundle ограничен ~1MB и как это связано с архитектурой Binder?
> Bundle передается через Binder IPC при передаче между процессами (Intent, savedInstanceState). Binder имеет общий буфер ~1MB на процесс для всех параллельных транзакций. Превышение -- TransactionTooLargeException. Практический лимит еще меньше (~500KB), так как буфер shared между всеми вызовами.

> [!question]- Почему Parcelable быстрее Serializable и когда это не имеет значения?
> Parcelable записывает данные напрямую в Parcel (бинарный буфер) без reflection. Serializable использует ObjectOutputStream с reflection для обнаружения полей. Parcelable в ~10x быстрее для IPC. Но для сохранения на диск разница незначительна -- overhead I/O доминирует над сериализацией.
...
?
Контейнер key-value пар на базе ArrayMap для передачи данных между компонентами. Поддерживает примитивы, String, Parcelable, Serializable. Используется в Intent extras, savedInstanceState, Fragment arguments.

Что такое Parcelable и чем отличается от Serializable?
?
Parcelable -- Android-специфичный интерфейс сериализации. Записывает поля в Parcel напрямую (без reflection). В ~10x быстрее Serializable. @Parcelize в Kotlin генерирует реализацию автоматически.

Что такое Parcel?
?
Бинарный буфер для сериализации данных при IPC через Binder. Содержит данные в формате type-length-value. Не предназначен для persistent storage -- формат может меняться между версиями Android.

Как работает @Parcelize?
?
Kotlin compiler plugin (kotlin-parcelize) генерирует writeToParcel() и createFromParcel() на основе primary constructor. Все свойства в constructor должны быть Parcelable-совместимых типов. Заменяет ручную реализацию Parcelable.

Какой практический лимит Bundle при использовании с savedInstanceState?
?
Технически ~1MB (Binder buffer), но практически ~500KB. Google рекомендует хранить минимум: ID объектов, позиции скролла, состояние форм. Большие данные -- в ViewModel + persistent storage.
...
|-------------|------|-------|
| Следующий шаг | [[android-activity-lifecycle]] | Как Bundle используется в savedInstanceState |
| Углубиться | [[android-intent-internals]] | Передача данных через Intent и Parcel |
| Смежная тема | [[memory-layout-marshalling]] | Маршаллинг данных при межпроцессном взаимодействии |
