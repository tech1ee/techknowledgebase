...
> [!question]- Почему Foreground Service с Android 14 обязан указывать foregroundServiceType?
> До Android 14 Foreground Service мог делать что угодно, маскируясь под уведомление. Типизация заставляет декларировать реальную цель (camera, location, mediaPlayback). Система может ограничить Service, если тип не соответствует поведению. Это повышает прозрачность для пользователя и контроль системы над ресурсами.

> [!question]- Сценарий: Bound Service продолжает работать после unbind всех клиентов. Почему?
> Service был одновременно Started (startService) и Bound (bindService). При unbind всех клиентов Service остается Started. Для полной остановки нужно вызвать stopService() или stopSelf() внутри Service. Started Service живет до явной остановки, Bound -- до unbind всех клиентов.
...
?
Started (startService -> onStartCommand), Bound (bindService -> onBind -> IBinder), Foreground (startForeground + notification). Service может быть одновременно Started и Bound.

Что возвращает onStartCommand()?
?
START_NOT_STICKY (не перезапускать), START_STICKY (перезапустить с null Intent), START_REDELIVER_INTENT (перезапустить с последним Intent). Определяет поведение после kill системой.

На каком потоке работает Service?
?
Main Thread! Service не создает свой поток. Тяжелая работа должна выполняться в корутинах или фоновых потоках внутри Service. Иначе -- ANR через 20 секунд (для foreground) или 200 секунд (для background).

Что такое AIDL и когда он нужен?
?
Android Interface Definition Language -- для описания IPC-интерфейсов через Binder. Нужен для Bound Service, доступного из другого процесса. Для in-process -- достаточно LocalBinder.

Какие foregroundServiceType существуют с Android 14?
?
...
|-------------|------|-------|
| Следующий шаг | [[android-background-work]] | WorkManager и другие способы фоновой работы |
| Углубиться | [[android-handler-looper]] | Main Thread и механизм сообщений |
| Смежная тема | [[ios-background-execution]] | Сравнение фоновой работы Android и iOS |
