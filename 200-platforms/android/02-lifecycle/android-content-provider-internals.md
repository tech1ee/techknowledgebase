...
> [!question]- Почему ContentProvider.onCreate() вызывается раньше Application.onCreate() и как это используется?
> ContentProvider инициализируется при старте процесса, до Application.onCreate(). Это эксплуатирует App Startup library и Firebase для автоинициализации: они регистрируют ContentProvider, чей onCreate() инициализирует SDK. Недостаток: каждый ContentProvider замедляет cold start на ~2ms.

> [!question]- Сценарий: ContentProvider вашего приложения возвращает Cursor с данными другому приложению. Пользователь видит краш. Что произошло?
> Cursor содержит данные, которые передаются через Binder IPC. Binder transaction ограничен ~1MB. Если Cursor содержит много данных (BLOB, большие строки), транзакция превышает лимит -- TransactionTooLargeException. Решение: использовать CursorWindow с лимитом, пагинацию, или FileProvider для больших данных.
...
Как ContentResolver взаимодействует с ContentProvider?
?
ContentResolver (клиент) -> ActivityManagerService (через Binder) -> AMS находит provider по authority -> AMS запускает процесс provider если нужно -> ContentProvider обрабатывает запрос -> результат возвращается через Binder.

Что такое UriMatcher?
?
Helper для mapping URI patterns к операциям. Например: content://authority/notes -> TABLE_NOTES, content://authority/notes/42 -> NOTE_BY_ID. Используется в query/insert/update/delete для определения типа запроса.

Что такое FileProvider и зачем он нужен?
?
ContentProvider для безопасного sharing файлов между приложениями через content:// URI вместо file://. Файл остается в sandbox приложения, а доступ предоставляется через FLAG_GRANT_READ_URI_PERMISSION.

Что такое App Startup library?
?
Jetpack библиотека для замены множества ContentProvider initializers одним. Уменьшает overhead cold start. Определяет Initializer с dependencies, вызывает их в правильном порядке через единый ContentProvider.

Как ContentProvider обеспечивает thread safety?
?
ContentProvider может вызываться из любого потока (Binder thread pool). Разработчик отвечает за thread safety: synchronized блоки, Room (thread-safe), или ContentProvider.call() с SerialExecutor.
...
|-------------|------|-------|
| Следующий шаг | [[android-data-persistence]] | Хранение данных: Room, DataStore |
| Углубиться | [[android-app-startup-performance]] | Оптимизация cold start и ContentProvider |
| Смежная тема | [[database-design-optimization]] | Проектирование БД для ContentProvider |
