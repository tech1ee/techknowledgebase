...
> [!question]- Почему у Fragment два lifecycle -- свой и View lifecycle, и когда они расходятся?
> Fragment может существовать без View (например, в back stack или retained fragment). View lifecycle начинается в onCreateView и заканчивается в onDestroyView, но Fragment lifecycle продолжается. При добавлении в back stack: onDestroyView вызывается (View уничтожен), но Fragment жив (onDestroy не вызван). При возврате из back stack View пересоздается.

> [!question]- Сценарий: вы собираете Flow в onViewCreated с lifecycleScope. При повороте экрана данные загружаются заново. Как исправить?
> Проблема: lifecycleScope привязан к Fragment lifecycle, а не View lifecycle. Но данные нужно привязать к ViewModel, который переживает config change. Решение: собирать Flow в viewLifecycleOwner.lifecycleScope с repeatOnLifecycle(STARTED), а данные хранить в ViewModel как StateFlow.

> [!question]- Почему commitNow() не работает с addToBackStack()?
> commitNow() выполняет транзакцию синхронно в текущем потоке. addToBackStack() требует, чтобы транзакция была записана в back stack менеджером, что происходит асинхронно через Handler. Синхронное выполнение и запись в back stack несовместимы. Используйте commit() (async) для транзакций с back stack.
...
Какие callback'и Fragment lifecycle?
?
onAttach -> onCreate -> onCreateView -> onViewCreated -> onStart -> onResume -> onPause -> onStop -> onDestroyView -> onDestroy -> onDetach. View lifecycle: onCreateView -- onDestroyView.

Чем commit() отличается от commitNow() и commitAllowingStateLoss()?
?
commit() -- async, ставит в очередь Handler. commitNow() -- sync, выполняет сразу (нельзя с back stack). commitAllowingStateLoss() -- как commit(), но не выбрасывает исключение после onSaveInstanceState.

Что такое viewLifecycleOwner?
?
LifecycleOwner привязанный к View Fragment (onCreateView -- onDestroyView). Используйте для подписок на данные в UI, чтобы они отменялись при уничтожении View, а не Fragment.

Что такое FragmentFactory?
?
Механизм создания Fragment с зависимостями через конструктор вместо пустого конструктора + setArguments(). Регистрируется в FragmentManager перед использованием.

Почему Fragment должен иметь пустой конструктор?
?
Система пересоздает Fragment через reflection при config change и process death. Без пустого конструктора -- InstantiationException. Данные передаются через setArguments(Bundle), а не через конструктор.

Что такое DialogFragment и зачем он нужен?
?
Fragment, отображаемый как диалог. Преимущество перед AlertDialog: переживает config change, имеет lifecycle, можно показать как обычный Fragment или как диалог.
...
|-------------|------|-------|
| Следующий шаг | [[android-navigation]] | Навигация между Fragment и Activity |
| Углубиться | [[android-viewmodel-internals]] | ViewModel и Fragment scope |
| Смежная тема | [[ios-viewcontroller-lifecycle]] | Сравнение Fragment и UIViewController |
