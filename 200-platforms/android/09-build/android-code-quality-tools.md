...
### Статический анализ: формальная основа

> **Статический анализ** — анализ программы без её выполнения. Теоретический фундамент — **abstract interpretation** (Cousot & Cousot, 1977, *"Abstract Interpretation: A Unified Lattice Model for Static Analysis of Programs"*): анализ выполняется на абстрактной модели программы, аппроксимируя все возможные состояния. Практические инструменты (Detekt, Lint) используют упрощённые формы: паттерн-матчинг на AST (Abstract Syntax Tree), dataflow analysis, type checking.

| Метрика | Определение | Инструмент | Пороговое значение |
|---------|------------|------------|-------------------|
| **Cyclomatic Complexity** (McCabe, 1976) | Число линейно независимых путей через код | Detekt `CyclomaticComplexMethod` | ≤10 (рекомендовано) |
| **Cognitive Complexity** (SonarSource, 2016) | Сложность восприятия кода человеком | Detekt `CognitiveComplexMethod` | ≤15 |
| **Lines of Code** | Длина метода/файла | Detekt `LongMethod`, `LargeClass` | ≤40 строк метод, ≤600 класс |
| **Code Duplication** | Повторяющиеся блоки кода | Detekt `DuplicateCode` | 0 дубликатов |

### Lint как формальная верификация

> Android Lint выходит за рамки стилистических проверок: он реализует **lightweight formal verification** — проверку соблюдения API-контрактов платформы. Примеры: использование `@RequiresApi(26)` метода на `minSdk 21`, отсутствие `FLAG_IMMUTABLE` в PendingIntent (Android 12+), hardcoded strings вместо string resources. Это частный случай **contract-based design** (Meyer, *"Design by Contract"*, 1986).

### Formatting: Bikeshedding Prevention

> Автоматическое форматирование (ktlint, Spotless) решает **закон тривиальности** (Parkinson's Law of Triviality, 1957): без единого стандарта команда тратит непропорционально много времени на обсуждение стиля кода. Автоформатирование исключает субъективность: `ktlint --format` приводит код к единому стилю без обсуждений на code review.

> **Связь**: Статический анализ → [[android-ci-cd]], Метрики → [[android-testing]], Code review → [[android-gradle-fundamentals]]
...
| Источник | Применение |
|----------|-----------|
| McCabe T. *A Complexity Measure* (1976, IEEE TSE) | Cyclomatic Complexity — метрика, используемая Detekt |
| Halstead M. *Elements of Software Science* (1977) | Halstead Metrics — сложность кода |
| Johnson S. *Lint, a C Program Checker* (1978, Bell Labs) | Lint — первый статический анализатор кода |
| Cousot P., Cousot R. *Abstract Interpretation* (1977) | Formal verification — основа static analysis |
