...
**[[android-gradle-fundamentals]]** — Gradle является текущей системой сборки Android, пришедшей на смену Ant и Maven. Понимание эволюции сборки объясняет, почему Gradle настроен именно так: convention-over-configuration от Maven, гибкость скриптинга от Ant, а Android Gradle Plugin добавляет Android-специфичные задачи. Изучайте Gradle fundamentals как следующий шаг после эволюции.

**[[android-compilation-pipeline]]** — pipeline сборки (kotlinc -> D8/R8 -> APK) является частью Gradle build process. Понимание эволюции систем сборки показывает, как pipeline усложнялся: от простого javac + dx в Ant до multi-stage pipeline с incremental compilation, KAPT/KSP и R8 optimization в современном Gradle.

**[[android-proguard-r8]]** — ProGuard был отдельным инструментом, интегрированным в Ant и Maven builds, а R8 стал частью D8 dexer в Gradle. Эволюция от ProGuard к R8 параллельна эволюции систем сборки: от отдельных инструментов к integrated pipeline.

**[[android-apk-aab]]** — формат пакета эволюционировал вместе с системой сборки: Ant генерировал только APK, Gradle добавил AAB support через bundletool, а AGP автоматизировал split APK generation. Понимание форматов пакетов необходимо для настройки правильного build pipeline.

**[[android-dependencies]]** — управление зависимостями прошло путь от ручного копирования JAR (Ant) через Maven Central (Maven) к Gradle dependency resolution с version catalogs и BOM. Каждое поколение систем сборки улучшало dependency management, и текущие best practices (TOML catalogs) — результат этой эволюции.
...
> [!question]- Почему Android перешел от Ant к Gradle, а не к Maven?
> Maven: convention over configuration, rigid structure, XML. Не подходит для Android: нужны product flavors, build types, resource processing. Gradle: flexible (Groovy/Kotlin DSL), plugins, task graph. AGP -- мощный plugin для Android-specific tasks. Ant был слишком низкоуровневый (manual task definition).

> [!question]- Какие проблемы решил переход от KAPT к KSP?
> KAPT: 1) Генерирует Java stubs для Kotlin -> медленно. 2) Только JVM (не multiplatform). 3) Не инкрементальный (полная re-process). KSP: 1) Работает с Kotlin symbols напрямую (2x быстрее). 2) Multiplatform support. 3) Инкрементальная обработка. 4) Лучше error messages.
...
Какие этапы эволюции build в Android?
?
Ant + Eclipse (2008-2013) -> Gradle + Android Studio (2013-2018) -> Kotlin DSL (2018-2022) -> Version Catalog + Convention Plugins (2022+). Каждый этап: больше автоматизации, лучше кэширование, строже типизация.

Как менялась компиляция Kotlin в Android?
?
KAPT (2016, Java stubs, медленно) -> KSP (2021, native Kotlin, 2x быстрее) -> KSP2 (2024, standalone, incremental). Annotation processing: Dagger, Room, Moshi переходят на KSP.

Как менялся формат публикации?
?
APK (2008-2021) -> AAB (2021+ обязательный для Play Store). Splits: per-ABI, per-density, per-language. Dynamic Feature Modules. Instant Apps. Тренд: меньше размер, гибче доставка.

Что изменилось в AGP 8.x?
?
Kotlin DSL по умолчанию, R8 Full Mode, Version Catalog recommended, Non-transitive R classes, Namespace в build.gradle (не Manifest), KSP вместо KAPT. Build speed improvements.
...
|-------------|------|-------|
| Следующий шаг | [[android-gradle-fundamentals]] | Современный Gradle для Android |
| Углубиться | [[android-compilation-pipeline]] | Детальный pipeline компиляции |
| Смежная тема | [[cross-build-systems]] | Build systems на разных платформах |
