...
**[[android-compilation-pipeline]]** — R8 является частью Android compilation pipeline, работая после D8 (dexing). Понимание полного pipeline (javac/kotlinc → .class → D8 → .dex → R8 → optimized .dex) объясняет, почему R8 может выполнять оптимизации, недоступные ProGuard: R8 работает напрямую с DEX bytecode, объединяя shrinking, obfuscation и dexing в один проход. Без понимания pipeline невозможно диагностировать проблемы сборки. Изучите compilation pipeline первым.

**[[android-apk-aab]]** — результаты работы R8 (optimized .dex, mapping.txt) упаковываются в APK или AAB. Понимание структуры APK/AAB объясняет, как R8 влияет на размер приложения (shrinking), как mapping файл используется для деобфускации crash reports, и как AAB формат позволяет дополнительную оптимизацию через Play Feature Delivery. R8 и APK/AAB — последние этапы перед доставкой приложения пользователю.

**[[android-gradle-fundamentals]]** — конфигурация R8 выполняется в build.gradle.kts через блок buildTypes (isMinifyEnabled, proguardFiles). Понимание Gradle build variants, product flavors и dependency configurations критично для правильной настройки keep rules, включения full mode и управления consumer-proguard-rules в библиотечных модулях. Без Gradle knowledge невозможно эффективно конфигурировать R8.

**[[android-build-evolution]]** — R8 является результатом эволюции от ProGuard (2002) через DX (2008) к единому инструменту (2018). Понимание исторического контекста объясняет, почему R8 использует ProGuard-формат keep rules, почему существуют compatibility и full mode, и почему некоторые legacy-библиотеки требуют специфических keep правил.
...
> [!question]- Почему R8 может сломать приложение при включении minifyEnabled и как предотвратить?
> R8 удаляет 'unused' код через static analysis. Проблема: reflection, serialization, JNI вызывают классы по строке -- R8 не видит использование и удаляет. Решение: -keep rules в proguard-rules.pro для классов используемых через reflection. Библиотеки включают consumer proguard rules. Тестирование release build обязательно.

> [!question]- Сценарий: после включения R8 Retrofit API вызовы возвращают null. Почему?
> R8 obfuscated data classes: поля переименованы (a, b, c). JSON parser (Gson/Moshi) не может маппить 'userName' в поле 'a'. Решение: 1) @Keep annotation на data classes. 2) -keepclassmembers для DTO. 3) kotlinx.serialization (compile-time, не рефлексия). 4) Moshi codegen (compile-time).
...
Что делает R8?
?
Три функции: Shrinking (удаление unused code/resources), Optimization (inline, dead code elimination), Obfuscation (переименование классов/методов). Замена ProGuard (быстрее, лучше оптимизация). minifyEnabled = true в build.gradle.

Что такое -keep rules?
?
Правила для R8: не удалять/переименовывать указанные классы. -keep class com.example.Model { *; } -- сохранить класс и все members. -keepclassmembers -- только members. -dontwarn -- подавить warnings. Consumer rules: в AAR библиотеке.

Что такое shrinkResources?
?
Удаление неиспользуемых ресурсов (drawables, layouts, strings). Требует minifyEnabled=true. R8 определяет используемые ресурсы через код. Динамические ресурсы (getIdentifier) указать в res/raw/keep.xml. Экономия: 5-30% размера.

Что такое R8 Full Mode?
?
Агрессивная оптимизация: enum unboxing, null checks removal, class merging. Включен по умолчанию в AGP 8.0+. Может сломать reflection-heavy код. android.enableR8.fullMode=true в gradle.properties.

Как debug R8 проблемы?
?
1) -printusage usage.txt -- что удалено. 2) -printmapping mapping.txt -- маппинг имен. 3) -printseeds seeds.txt -- что сохранено. 4) Retrace для deobfuscation stack traces. 5) APK Analyzer в Android Studio для визуального анализа.
...
|-------------|------|-------|
| Следующий шаг | [[android-ci-cd]] | CI/CD для автоматизации release builds |
| Углубиться | [[android-compilation-pipeline]] | R8 как часть compilation pipeline |
| Смежная тема | [[ios-code-signing]] | Code signing и optimization в iOS |
