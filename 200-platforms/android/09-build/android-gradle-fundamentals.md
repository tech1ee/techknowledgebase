...
> [!question]- Почему Gradle использует DAG (Directed Acyclic Graph) для tasks?
> DAG определяет порядок выполнения и зависимости между tasks. Преимущества: 1) Параллельное выполнение independent tasks. 2) Инкрементальная сборка: пропуск up-to-date tasks. 3) Детерминированный порядок: одинаковый результат при одинаковых inputs. Без DAG: sequential execution, невозможно кэширование, невозможна параллелизация.

> [!question]- Сценарий: clean build проекта занимает 3 минуты, incremental -- 30 секунд, но иногда incremental ломается. Что проверить?
> 1) Configuration Cache: gradle.properties -> org.gradle.configuration-cache=true. 2) Build Cache: org.gradle.caching=true. 3) Tasks с нестабильными outputs (timestamp в файлах). 4) buildSrc вместо Convention Plugins (invalidates cache). 5) ./gradlew --scan для анализа bottlenecks. 6) Annotations на custom tasks (@Input, @Output) для правильного up-to-date check.
...
Какие фазы сборки Gradle?
?
1) Initialization: settings.gradle.kts, определение модулей. 2) Configuration: build.gradle.kts, создание task graph. 3) Execution: выполнение tasks по DAG. Configuration Cache кэширует фазу 2. Build Cache кэширует outputs фазы 3.

Что такое AGP (Android Gradle Plugin)?
?
Plugin от Google, добавляющий Android-specific tasks: compile (Java/Kotlin -> DEX), resource processing (AAPT2), packaging (APK/AAB), signing. Версия AGP привязана к Android Studio. AGP 8.x для Kotlin DSL и KSP.

Что такое Gradle Wrapper?
?
gradlew / gradlew.bat: скрипт, загружающий конкретную версию Gradle. gradle-wrapper.properties: distributionUrl с версией. Обеспечивает одинаковую версию Gradle для всех разработчиков. Коммитится в git.

Что такое Configuration Cache?
?
Кэширует результат Configuration фазы (task graph). При повторной сборке без изменений build scripts -- пропускает parsing build.gradle.kts. Экономит 5-30 секунд. Ограничения: не все plugins совместимы.

Чем Kotlin DSL отличается от Groovy DSL?
?
Kotlin DSL (.kts): type-safe, IDE completion, compile-time checks, единый язык. Groovy DSL (.gradle): dynamic, менее verbose, быстрее evaluation. Google рекомендует Kotlin DSL для новых проектов. Миграция: постепенная, файл за файлом.
...
|-------------|------|-------|
| Следующий шаг | [[android-dependencies]] | Управление зависимостями в Gradle |
| Углубиться | [[android-compilation-pipeline]] | Полный pipeline компиляции Android |
| Смежная тема | [[build-systems-theory]] | Теория систем сборки: DAG, кэширование |
