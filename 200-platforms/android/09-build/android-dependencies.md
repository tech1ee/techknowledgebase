...
**[[android-gradle-fundamentals]]** — Управление зависимостями является подсистемой Gradle build system. Понимание фаз Gradle (configuration, execution), task graph и plugin architecture необходимо для осмысленной работы с dependency configurations (implementation, api, compileOnly). Без базовых знаний Gradle невозможно диагностировать проблемы с разрешением версий. Рекомендуется изучить gradle-fundamentals первым.

**[[android-build-evolution]]** — Эволюция систем управления зависимостями (от Ant+Ivy через Maven к Gradle) объясняет, почему современный Gradle использует именно такую модель: GAV координаты из Maven, но с более мощным conflict resolution. Знание истории помогает понять решения в текущем API. Изучайте для контекста.

**[[android-compilation-pipeline]]** — Зависимости являются входом для pipeline компиляции: KAPT/KSP обрабатывают annotation processors из dependencies, R8 оптимизирует весь граф зависимостей, а DEX-компилятор преобразует все библиотеки в байткод Dalvik. Понимание pipeline помогает выбирать правильный configuration scope.
...
- **Meier R. (2022)** *Professional Android* — практическое руководство по конфигурации Gradle dependencies, включая multi-module проекты, Version Catalogs и BOM.
- **Phillips B. et al. (2022)** *Android Programming: The Big Nerd Ranch Guide* — введение в Gradle dependencies для начинающих с пошаговыми примерами добавления библиотек.
...
> [!question]- Почему Version Catalog (libs.versions.toml) лучше ext переменных для управления зависимостями?
> Version Catalog: 1) Type-safe access (libs.retrofit вместо строки). 2) IDE autocomplete. 3) Bundles для группировки. 4) Единый файл для всех модулей. 5) Поддержка plugins. ext переменные: нет type safety, нет completion, разбросаны по файлам. Version Catalog -- стандарт для новых проектов.

> [!question]- Сценарий: после обновления библиотеки 100 тестов упали. Как безопасно обновлять зависимости?
> 1) Dependabot/Renovate для автоматических PR. 2) Обновлять по одной зависимости. 3) CI прогоняет все тесты на каждый PR. 4) Semantic versioning: minor/patch обычно safe, major -- breaking. 5) Changelog библиотеки перед обновлением. 6) gradle dependency-locking для фиксации transitive deps.
...
Как устроен Version Catalog?
?
libs.versions.toml: [versions] -- версии, [libraries] -- зависимости (group:artifact:version), [bundles] -- группы библиотек, [plugins] -- Gradle plugins. Доступ: libs.retrofit, libs.bundles.compose. Централизованное управление версиями.

Чем implementation отличается от api?
?
implementation: зависимость НЕ видна потребителям модуля (compilation isolation). api: транзитивно видна. implementation уменьшает recompilation scope. Правило: всегда implementation, api только для public API модуля.

Что такое BOM (Bill of Materials)?
?
Файл, определяющий совместимые версии группы библиотек. Compose BOM: одна версия определяет все Compose artifacts. platform(libs.compose.bom) -- не нужно указывать версию каждого Compose модуля. Firebase, OkHttp имеют BOM.

Что такое dependency resolution strategy?
?
Как Gradle разрешает конфликты версий: latest wins (по умолчанию), force (принудительная версия), failOnVersionConflict() (ошибка). resolutionStrategy в configurations. ./gradlew dependencies для анализа дерева.

Что такое KSP vs KAPT?
?
KAPT: annotation processing через Java stubs (медленно, JVM only). KSP: Kotlin Symbol Processing (быстрее 2x, multiplatform). Room, Hilt, Moshi переходят на KSP. Для новых проектов: всегда KSP. KAPT deprecated в roadmap.
...
|-------------|------|-------|
| Следующий шаг | [[android-compilation-pipeline]] | Как зависимости компилируются в DEX |
| Углубиться | [[android-gradle-fundamentals]] | Gradle механизмы для управления зависимостями |
| Смежная тема | [[dependency-resolution]] | Теория разрешения зависимостей |
