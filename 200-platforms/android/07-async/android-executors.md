...
> [!question]- Почему Executors предпочтительнее прямого создания Thread?
> Thread: нет переиспользования (создание ~1ms), нет ограничения количества, нет queue management. Executor: thread pool (переиспользование потоков), bounded (ограниченное число потоков), queue для задач, rejection policy. Coroutines используют Executor под капотом (Dispatchers.IO = shared thread pool).

> [!question]- Сценарий: пул из 4 потоков перегружен 100 задачами. Что контролирует поведение?
> 1) Queue type: LinkedBlockingQueue (unbounded -- все задачи в очереди), ArrayBlockingQueue(N) (bounded). 2) RejectionPolicy: AbortPolicy (exception), CallerRunsPolicy (выполнить в вызывающем потоке), DiscardPolicy (тихо отбросить). 3) corePoolSize vs maxPoolSize: сколько потоков создавать при нагрузке.
...
Какие типы Executors есть в Java?
?
newFixedThreadPool(n) -- фиксированный пул. newCachedThreadPool() -- динамический (0-MAX). newSingleThreadExecutor() -- один поток (sequential). newScheduledThreadPool(n) -- с таймером. Для Android: Dispatchers.IO и Dispatchers.Default заменяют прямое использование.

Что такое Future и ListenableFuture?
?
Future: результат асинхронной операции. get() блокирует до завершения. ListenableFuture (Guava): addListener() для non-blocking callback. В Android: androidx.concurrent.futures. Корутины заменяют: suspend fun = Future + callback без блокировки.

Как Dispatchers.IO связан с Executors?
?
Dispatchers.IO использует shared thread pool (до 64 потоков). limitedParallelism(n) создает view с ограниченным параллелизмом. Под капотом -- ScheduledExecutorService. Coroutines добавляют structured concurrency поверх Executors.

Что такое ExecutorService?
?
Расширение Executor с lifecycle: submit() возвращает Future, shutdown() останавливает, awaitTermination() ждет завершения. В Android: используйте Coroutines вместо прямого ExecutorService для lifecycle safety.
