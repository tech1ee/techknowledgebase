...
> [!question]- Почему RxJava теряет популярность в Android, несмотря на мощность?
> 1) Kotlin Coroutines + Flow решают те же задачи с меньшей сложностью. 2) Steep learning curve (Observable/Flowable/Single/Maybe/Completable). 3) Cryptic error messages и сложный debugging. 4) Большой размер библиотеки (~2.5MB). 5) Нет structured concurrency (утечки подписок). Coroutines + Flow: native для Kotlin, lifecycle-aware, проще API.

> [!question]- Сценарий: проект на RxJava. Стоит ли мигрировать на Coroutines/Flow?
> Зависит от: 1) Активная разработка? Если да -- постепенная миграция (новый код на Flow). 2) Команда знает RxJava лучше Coroutines? Обучение нужно. 3) Используются сложные операторы (combineLatest, debounce)? Flow имеет все аналоги. Стратегия: rxjava3-bridge для interop, новые фичи на Flow, миграция при рефакторинге.
...
Какие основные типы в RxJava?
?
Observable (0..n items, no backpressure), Flowable (0..n items, backpressure), Single (1 item), Maybe (0..1 item), Completable (0 items, только success/error). В Flow: все это один тип Flow<T>.

Что такое backpressure в RxJava?
?
Механизм замедления producer когда consumer не успевает. Flowable поддерживает стратегии: BUFFER, DROP, LATEST, ERROR, MISSING. Observable не имеет backpressure -- MissingBackpressureException. Flow: buffered по умолчанию.

Как RxJava операторы маппятся на Flow?
?
map -> map, flatMap -> flatMapConcat, switchMap -> flatMapLatest, combineLatest -> combine, debounce -> debounce, zip -> zip. Разница: Flow sequential по умолчанию, RxJava concurrent.

Что такое CompositeDisposable?
?
Контейнер для Disposable (подписок). disposable.add(subscription). disposable.clear() в onDestroy() отменяет все подписки. Аналог structured concurrency: viewModelScope автоматически отменяет корутины.

Что такое Scheduler в RxJava?
?
Аналог Dispatcher: Schedulers.io() (IO пул), Schedulers.computation() (CPU), AndroidSchedulers.mainThread() (Main). subscribeOn() -- где создать. observeOn() -- где обработать. В Coroutines: flowOn() для upstream, withContext() для блока.
