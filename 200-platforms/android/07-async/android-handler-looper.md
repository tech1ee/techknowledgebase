...
> [!question]- Почему Handler/Looper -- основа всей модели threading в Android?
> Main Thread = Looper.getMainLooper(). Все lifecycle callbacks (onCreate, onResume), touch events, рендеринг -- сообщения в MessageQueue Main Thread. Handler позволяет отправлять Message/Runnable в любую MessageQueue. postDelayed(), runOnUiThread(), View.post() -- все используют Handler под капотом. Без Looper Android не может обрабатывать события.

> [!question]- Сценарий: Handler создан в Activity и используется для delayed task. Activity уничтожена. Что произойдет?
> Memory leak: Handler -> MessageQueue -> Message -> Handler -> Activity (через implicit reference из inner class). Delayed Runnable держит reference на Activity. GC не может собрать Activity. Решение: WeakReference, removeCallbacksAndMessages(null) в onDestroy(), или заменить на lifecycleScope.launch + delay().
...
Что такое Looper?
?
Бесконечный цикл, читающий Message из MessageQueue и dispatching на целевой Handler. Один Looper на поток. Main Thread имеет Looper по умолчанию (Looper.getMainLooper()). HandlerThread -- поток с готовым Looper.

Что такое Handler?
?
Отправляет и обрабатывает Message/Runnable через MessageQueue. Привязан к Looper (и его потоку). Handler(Looper.getMainLooper()) -- отправка на Main Thread. post(Runnable) и sendMessage(Message) -- два способа.

Что такое MessageQueue?
?
FIFO очередь Message, привязанная к Looper. Message обрабатываются по одному, последовательно. when timestamp <= now -- dispatch. Нативная реализация с epoll (Linux) для эффективного ожидания.

Чем Message отличается от Runnable в Handler?
?
Message: объект с what (int id), obj (data), arg1/arg2. Обрабатывается handleMessage(). Переиспользуется через Message.obtain() (pool). Runnable: lambda/anonymous class. post(runnable). Проще, но тяжелее для GC (allocation).

Почему new Handler() deprecated?
?
Без explicit Looper неясно к какому потоку привязан Handler. Молчаливо использует Looper текущего потока, что может быть background thread. Handler(Looper.getMainLooper()) -- explicit. Или Looper.myLooper() для текущего.

Что такое HandlerThread?
?
Thread с подготовленным Looper. val thread = HandlerThread('bg'); thread.start(); Handler(thread.looper). Для последовательного выполнения задач в фоне. IntentService (deprecated) использовал HandlerThread.
