...
> [!question]- Почему AsyncTask deprecated и какие проблемы он создавал?
> 1) Memory leak: inner class AsyncTask держит reference на Activity. 2) Crash при rotation: onPostExecute() обновляет уже уничтоженную Activity. 3) Serial execution по умолчанию (Android 3+): одна задача блокирует остальные. 4) Нет cancellation support: cancel() не прерывает doInBackground(). Замена: Coroutines с viewModelScope.

> [!question]- Сценарий: legacy-проект с 50 AsyncTask. Как мигрировать?
> Поэтапно: 1) Новый код только на Coroutines. 2) Создать extension: AsyncTask -> suspend fun маппинг. 3) При рефакторинге экрана: заменить AsyncTask на viewModelScope.launch { withContext(Dispatchers.IO) { } }. 4) Приоритет: сначала критичные (с memory leaks), потом остальные.
...
Какие методы имел AsyncTask?
?
doInBackground(Params) -- фоновый поток. onPreExecute() -- UI перед запуском. onPostExecute(Result) -- UI после завершения. onProgressUpdate(Progress) -- обновление прогресса на UI. publishProgress() из doInBackground.

Почему AsyncTask -- memory leak?
?
AsyncTask как inner class держит implicit reference на Activity. Activity уничтожена при rotation, но AsyncTask продолжает работу. onPostExecute() пытается обновить мертвую Activity = crash или leak. Решение: ViewModel + Coroutines.

Чем заменить AsyncTask?
?
1) Coroutines + viewModelScope (рекомендуемый). 2) RxJava (legacy). 3) Executors + LiveData. Пример: viewModelScope.launch { val result = withContext(Dispatchers.IO) { loadData() }; _state.value = result }

Когда AsyncTask был Serial, когда Parallel?
?
Android 1.6-2.3: serial (один поток). Android 3.0: parallel (пул). Android 3.2+: serial по умолчанию (SERIAL_EXECUTOR). executeOnExecutor(THREAD_POOL_EXECUTOR) для parallel. Непредсказуемость -- еще одна причина deprecation.
