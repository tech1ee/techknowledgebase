...
### Формальная модель потоков и UI

> **Java Memory Model (JMM, JSR-133)** — формальная спецификация, определяющая правила видимости записей между потоками. Принята в 2004 году (Manson, Pugh, Adve: *"The Java Memory Model"*, POPL 2005). JMM вводит отношение **happens-before**: если операция A happens-before B, то результат A гарантированно виден B. Без happens-before компилятор и процессор могут переупорядочивать инструкции произвольно.

Модель однопоточного UI имеет формальное обоснование, выходящее за рамки Android:

| Концепция | Формальное определение | Первоисточник |
|-----------|----------------------|---------------|
| **Single-threaded apartment (STA)** | Поток владеет объектами; доступ из других потоков — только через очередь сообщений | COM Threading Model (Microsoft, 1993) |
| **Event Dispatch Thread (EDT)** | Единственный поток для обработки событий и обновления Swing-компонентов | Java Swing Architecture (Eckstein et al., 1998) |
| **Thread confinement** | Ограничение доступа к объекту одним потоком устраняет необходимость синхронизации | Goetz B. *Java Concurrency in Practice*, §3.3 (2006) |
| **Run loop / Message loop** | Бесконечный цикл обработки сообщений из очереди — основа всех GUI-фреймворков | NeXTSTEP NSRunLoop (1989), Windows Message Pump (1985) |

> **Связь с Android:** Main Thread Android — это реализация STA-паттерна, где [[android-handler-looper|Looper]] играет роль message pump, а Handler — роль диспетчера сообщений. CalledFromWrongThreadException — это runtime-проверка thread confinement.

### Модель Happens-Before в контексте Android Dispatchers

Kotlin Coroutines Dispatchers обеспечивают happens-before между suspend points:

| Переход | Happens-before гарантия | Механизм |
|---------|------------------------|----------|
| `withContext(Dispatchers.IO) { ... }` → возврат на Main | Да — результат виден на Main | `ContinuationInterceptor` через `Handler.post()` |
| `viewModelScope.launch { }` → child coroutine | Да — parent видит результат child | Structured concurrency (Elizarov, 2018) |
| Два параллельных `async { }` → `await()` | Да — `await()` устанавливает happens-before | `CompletableDeferred` + atomic state machine |

### Structured Concurrency: формальный базис

> **Structured Concurrency** — модель, в которой время жизни конкурентных задач ограничено лексической областью видимости (scope). Формализована Nathaniel J. Smith в 2018 году (*"Notes on structured concurrency, or: Go statement considered harmful"*). Kotlin стал первым mainstream-языком, встроившим structured concurrency в корутины (Roman Elizarov, KotlinConf 2018).

Принципы structured concurrency, применяемые в Android:
- **viewModelScope** и **lifecycleScope** — реализация принципа «задача не переживает своего владельца»
- Отмена parent → отмена всех children (cooperative cancellation через `isActive`)
- Исключение в child → отмена parent (если не `supervisorScope`) — fail-fast семантика

> **Связь**: JMM → [[jvm-memory-model]], Thread confinement → [[os-synchronization]], Structured concurrency → [[kotlin-coroutines]]
...
- **Manson J., Pugh W., Adve S.V. (2005).** *The Java Memory Model.* POPL '05. — Формальная спецификация JSR-133, определяющая happens-before и правила видимости между потоками.
- **Goetz B. (2006).** *Java Concurrency in Practice.* Addison-Wesley. — Фундаментальная книга по многопоточности на JVM: thread safety, visibility, ordering, concurrent collections. Обязательна для глубокого понимания threading.
- **Smith N.J. (2018).** *Notes on structured concurrency, or: Go statement considered harmful.* — Формализация structured concurrency; повлияла на дизайн Kotlin coroutines.
- **Moskala M. (2022).** *Kotlin Coroutines Deep Dive.* — Полное руководство по корутинам: от suspend-функций до structured concurrency и Dispatchers.
...
- **Meier R. (2022).** *Professional Android.* — Практическое покрытие threading: Main Thread rule, background processing, WorkManager и интеграция корутин с lifecycle.
