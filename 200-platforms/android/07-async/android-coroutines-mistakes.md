...
> [!question]- Почему GlobalScope -- антипаттерн в Android?
> GlobalScope: 1) Не привязан к lifecycle -- корутина продолжает работу после onDestroy(). 2) Memory leak -- держит references на объекты. 3) Невозможно отменить все корутины при навигации. viewModelScope автоматически отменяется при onCleared(). lifecycleScope -- при onDestroy(). Structured concurrency: parent scope отменяет children.

> [!question]- Сценарий: корутина в viewModelScope делает сетевой запрос на Dispatchers.IO. При повороте экрана запрос отменяется. Почему?
> viewModelScope НЕ отменяется при rotation (ViewModel переживает config change). Проблема в другом: возможно используется lifecycleScope (привязан к Activity). Или: collectAsState() в Compose отменяет collection при recomposition с новым ключом. Проверить: какой scope используется и кто trigger cancellation.
...
?
Принцип: корутины организованы в иерархию через scope. Parent scope отменяет все children при cancellation. Child exception propagates к parent. viewModelScope, lifecycleScope -- structured scopes. GlobalScope нарушает принцип.

Какие типичные ошибки с корутинами?
?
1) GlobalScope вместо viewModelScope. 2) Забыть withContext(Dispatchers.IO) для сети/БД. 3) Catch CancellationException (нарушает cancellation). 4) suspend fun без реального suspension (blocking call). 5) Мутация shared state без synchronization.

Как правильно обрабатывать ошибки в корутинах?
?
1) try/catch внутри launch {}. 2) CoroutineExceptionHandler для uncaught. 3) SupervisorJob -- child failure не отменяет siblings. 4) Не catch CancellationException (rethrow). 5) Result<T> sealed class для domain errors.

Что такое SupervisorJob?
?
Job, где failure одного child не отменяет остальных. Обычный Job: one child fails -> все siblings cancelled. SupervisorJob: isolated failures. viewModelScope использует SupervisorJob по умолчанию.

Чем launch отличается от async?
?
launch: fire-and-forget, возвращает Job. async: возвращает Deferred<T>, result через await(). launch для side effects (UI update, save to DB). async для parallel computations (await multiple results).

Что такое flowOn и почему не withContext для Flow?
?
flowOn(Dispatchers.IO) меняет dispatcher для upstream операторов. withContext внутри flow {} нарушает контракт Flow (не разрешен). flowOn создает отдельную корутину для collection, обеспечивая channel-based communication.
...
|-------------|------|-------|
| Основы | [[android-coroutines-guide]] | Практический гайд по корутинам — паттерны и best practices |
| Flow | [[android-flow-guide]] | Flow в Android — типичные ошибки с Flow тоже здесь |
| Тестирование | [[android-async-testing]] | Как тестировать корутины и Flow |
| Следующий шаг | [[android-background-work]] | WorkManager для long-running задач |
| Углубиться | [[kotlin-coroutines]] | Корутины Kotlin в деталях |
| Смежная тема | [[ios-concurrency-mistakes]] | Типичные ошибки concurrency в iOS |
