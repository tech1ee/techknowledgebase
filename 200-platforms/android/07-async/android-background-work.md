...
> [!question]- Почему WorkManager предпочтительнее Foreground Service для периодических задач?
> WorkManager: 1) Гарантированное выполнение (даже после перезагрузки). 2) Оптимизирован для батареи (батчинг, Doze-aware). 3) Constraints (сеть, зарядка). 4) Не требует уведомления. Foreground Service: для задач, которые пользователь осознает (музыка, навигация). Периодический sync -- WorkManager. Streaming музыки -- Foreground Service.

> [!question]- Сценарий: приложение должно синхронизировать данные каждые 15 минут, даже без запуска. Как реализовать?
> PeriodicWorkRequest с минимальным интервалом 15 минут. Constraints: NetworkType.CONNECTED. setBackoffCriteria для retry. ExistingPeriodicWorkPolicy.KEEP для предотвращения дубликатов. Тестирование: WorkManagerTestRule в androidTest. Мониторинг: getWorkInfoByIdLiveData().
...
Какие ограничения фоновой работы в Android?
?
Android 8+: background service limits (убивается через 1 мин). Android 12+: foreground service launch restrictions из background. Android 13+: exact alarms требуют permission. Android 14+: foreground service type обязателен. Тренд: всё строже.

Когда использовать WorkManager vs другие API?
?
WorkManager: deferrable, guaranteed (sync data, upload logs). AlarmManager: exact timing (будильник). Foreground Service: long-running user-aware (музыка, навигация). Coroutines: immediate, lifecycle-bound (загрузка экрана).

Что такое Constraints в WorkManager?
?
Условия выполнения: NetworkType (CONNECTED, UNMETERED), requiresCharging, requiresBatteryNotLow, requiresStorageNotLow, requiresDeviceIdle. Worker запускается только когда все constraints выполнены.

Как реализовать WorkManager с Hilt?
?
@HiltWorker class SyncWorker @AssistedInject constructor(@Assisted context: Context, @Assisted params: WorkerParameters, val repo: Repository) : CoroutineWorker(context, params). HiltWorkerFactory в Application. Configuration.Builder.setWorkerFactory().

Что такое CoroutineWorker?
?
Worker для suspend функций: override suspend fun doWork(): Result. Вызывается на Dispatchers.Default. Возвращает Result.success(), Result.failure(), Result.retry(). Замена synchronous Worker для async операций.
...
|-------------|------|-------|
| Следующий шаг | [[android-async-evolution]] | Полная история асинхронности в Android |
| Углубиться | [[android-service-internals]] | Service lifecycle для Foreground Service |
| Смежная тема | [[ios-background-execution]] | Background execution в iOS |
