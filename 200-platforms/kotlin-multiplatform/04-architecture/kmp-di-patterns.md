...
> [!question]- Почему Koin стал де-факто стандартом для DI в KMP, а не Dagger/Hilt?
> Dagger/Hilt используют annotation processing (kapt/KSP), привязанный к JVM. Koin -- runtime DI на чистом Kotlin, работающий на всех платформах без code generation. Это делает его естественным выбором для commonMain.

> [!question]- Вы проектируете DI для KMP-проекта. Как организовать platform-specific зависимости (HTTP engine, database driver)?
> Создать expect val platformModule: Module в commonMain. В androidMain actual модуль содержит OkHttp engine, AndroidSqlDriver. В iosMain -- Darwin engine, NativeSqliteDriver. При инициализации Koin подключаются commonModule + platformModule.

> [!question]- Когда стоит выбрать kotlin-inject вместо Koin для KMP?
> Когда важна compile-time безопасность: kotlin-inject проверяет граф зависимостей при компиляции и выявляет отсутствующие bindings до runtime. Koin обнаруживает ошибки только при запуске. Для больших проектов compile-time DI надёжнее.

---

## Ключевые карточки

Какие DI-фреймворки поддерживают KMP?
?
Koin 4.x (runtime DI, самый популярный), kotlin-inject (compile-time, KSP-based), Manual DI (без фреймворка). Dagger/Hilt не поддерживают KMP из-за привязки к JVM annotation processing.

Как Koin организует зависимости в KMP?
?
commonModule в commonMain содержит shared bindings (repositories, use cases). platformModule через expect/actual содержит platform-specific bindings (HTTP engines, drivers). startKoin объединяет оба модуля.

Что такое kotlin-inject и как он работает в KMP?
?
Compile-time DI фреймворк на KSP. Генерирует код инъекции при компиляции, проверяя граф зависимостей. Аннотации @Component, @Inject, @Provides. Поддерживает KMP через KSP multiplatform.

Когда Manual DI предпочтительнее фреймворков?
?
Для небольших проектов или библиотек, где не хочется добавлять зависимость от DI-фреймворка. Реализуется через конструкторы, factory functions и object declarations. Просто, но не масштабируется.

Как тестировать DI-зависимости в KMP?
?
Koin: koinTest модуль с checkModules() для проверки графа. kotlin-inject: граф проверяется при компиляции. В тестах -- подмена через interface + fake implementations или Koin overrides.
...
|-------------|------|-------|
| Следующий шаг | [[kmp-navigation]] | Навигация в кросс-платформенных приложениях |
| Углубиться | [[kmp-architecture-patterns]] | Архитектурные паттерны, использующие DI |
| Смежная тема | [[android-hilt-deep-dive]] | Hilt как reference для понимания DI-концепций |
