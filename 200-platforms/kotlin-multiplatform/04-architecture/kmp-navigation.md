...
> [!question]- Почему навигация в KMP сложнее, чем в чисто Android-приложении?
> На Android навигация привязана к Activity/Fragment lifecycle. В KMP нужно абстрагировать навигацию от платформенного lifecycle: iOS не имеет Activity, Desktop -- Fragment. Shared navigation должен работать на всех платформах с разными lifecycle-моделями.

> [!question]- Вы выбираете между Decompose и Voyager для KMP-проекта. Какие критерии определяют выбор?
> Decompose подходит для сложных приложений: lifecycle-aware components, child stack/pages/slot, deep linking, state preservation. Voyager проще (screen-based navigation), подходит для небольших проектов. Compose Navigation -- если нужна совместимость с Jetpack.

> [!question]- Почему deep linking в KMP требует platform-specific кода, несмотря на shared navigation graph?
> Deep link URI обрабатывается ОС (Android Intent, iOS Universal Links) до запуска навигации. Platform-specific код парсит URI и передаёт destination в shared navigation. Сам navigation graph и обработка -- в commonMain.

---

## Ключевые карточки

Какие навигационные библиотеки поддерживают KMP?
?
Compose Navigation (Jetpack, multiplatform с 2.9+), Decompose (lifecycle-aware components от Arkadii Ivanov), Voyager (screen-based, простой API). Каждая имеет свои trade-offs.

Что такое Decompose и как он работает?
?
Библиотека для lifecycle-aware component-based навигации. ComponentContext управляет lifecycle, Child Stack -- стеком экранов, Child Pages -- пагинацией. Работает без Compose (logic) + с Compose (UI).

Как Compose Navigation работает в KMP?
?
С версии 2.9+ поддерживает multiplatform. NavHost и NavController размещаются в commonMain. Навигация через route strings. Ограничения: нет полной type-safety, deep linking требует platform setup.

Что такое Voyager и когда его использовать?
?
Screen-based навигация с простым API. Screen -- основная единица, Navigator управляет стеком. Встроенная поддержка tab navigation, bottom sheets. Подходит для простых приложений с линейной навигацией.

Как реализовать state preservation при навигации в KMP?
?
Decompose: StateKeeper сохраняет состояние при configuration changes. Compose Navigation: SavedStateHandle. Voyager: ScreenModel с saved state. На Android -- Bundle, на iOS -- in-memory (нет configuration changes).
...
|-------------|------|-------|
| Следующий шаг | [[kmp-state-management]] | Управление состоянием, работающее с навигацией |
| Углубиться | [[kmp-architecture-patterns]] | Архитектура, в которую вписывается навигация |
| Смежная тема | [[android-navigation]] | Android Navigation Component как reference |
