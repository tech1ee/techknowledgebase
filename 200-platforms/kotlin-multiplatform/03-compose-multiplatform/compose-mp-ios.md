...
> [!question]- Почему Compose Multiplatform на iOS использует Metal для рендеринга, а не CoreGraphics?
> Compose MP использует Skia как графический движок, который на iOS рендерит через Metal (GPU). Metal обеспечивает аппаратное ускорение и 60/120fps на современных устройствах. CoreGraphics -- CPU-based и не подходит для сложного реактивного UI.

> [!question]- iOS-дизайнер заметил, что Compose-приложение не выглядит как нативное iOS. Как решить эту проблему?
> Использовать Material3 Adaptive или Cupertino-стилизованные компоненты. Compose MP позволяет подключить iOS-specific styling: cupertino theme, UIKit interop для нативных элементов (UINavigationBar, UITabBar), iOS-специфичные жесты и haptic feedback.

> [!question]- Почему UIKit interop важен для Compose MP iOS, несмотря на наличие shared Compose UI?
> Не все iOS-компоненты имеют Compose-аналоги (MapKit, WKWebView, AVPlayer). UIKit interop позволяет встраивать нативные iOS-виджеты в Compose-иерархию, обеспечивая полный доступ к платформенным возможностям.

---

## Ключевые карточки

Как Compose Multiplatform рендерит UI на iOS?
?
Через Skia + Metal. Compose-дерево компилируется Kotlin/Native, рендерится Skia в Metal-текстуры, которые отображаются GPU устройства. Это даёт pixel-perfect consistency с другими платформами.

Что такое UIKit interop в Compose MP iOS?
?
Механизм встраивания нативных iOS-виджетов (UIView, UIViewController) в Compose-иерархию через UIKitView composable. Позволяет использовать MapKit, Camera, WebView и другие нативные компоненты внутри Compose UI.

С какой версии Compose MP iOS считается Stable?
?
С версии 1.8.0. До этого iOS был в Alpha/Beta. Stable означает production-ready API, гарантии обратной совместимости и поддержку от JetBrains.

Как обеспечить нативный iOS look-and-feel в Compose MP?
?
Использовать Material3 Adaptive для платформо-специфичной стилизации, подключить Cupertino theme для iOS-компонентов, применять UIKit interop для нативных элементов и реализовать iOS-специфичные жесты.

Какие ограничения имеет Compose MP iOS?
?
Не все iOS accessibility features поддерживаются из коробки, некоторые UIKit-компоненты недоступны напрямую (нужен interop), размер binary больше чем у чистого SwiftUI, и горячая перезагрузка ограничена по сравнению с Xcode Previews.
...
|-------------|------|-------|
| Следующий шаг | [[compose-mp-desktop]] | Compose UI для Desktop-платформы |
| Углубиться | [[kmp-ios-deep-dive]] | Низкоуровневая iOS-интеграция: SKIE, memory, XCFramework |
| Смежная тема | [[kmp-performance-optimization]] | Оптимизация производительности на iOS |
