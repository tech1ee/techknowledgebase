...
> [!question]- Почему K2 compiler даёт 2x ускорение компиляции KMP-проектов?
> K2 -- полностью переписанный frontend Kotlin compiler с новой архитектурой: unified resolution, inline caching, better parallelization. Старый compiler имел legacy overhead. K2 оптимизирован для KMP: быстрее обрабатывает multi-target resolution и expect/actual matching.

> [!question]- Размер iOS binary вашего KMP-приложения -- 50MB. Какие оптимизации помогут уменьшить его?
> Static framework (isStatic = true) вместо dynamic, -Xstrip-debug-info для release, исключение неиспользуемых targets, минимизация exported API в umbrella module, Kotlin/Native binary stripping, ProGuard/R8 для Android-части.

> [!question]- Почему build time -- главная жалоба разработчиков на KMP, и как её минимизировать?
> KMP компилирует для нескольких targets, что мультиплицирует время. Оптимизации: Gradle build cache (local + remote), configuration cache, parallel execution, convention plugins вместо buildSrc, инкрементальная компиляция, отключение ненужных targets в dev.

---

## Ключевые карточки

Какие три области оптимизации важны в KMP?
?
Build time (Gradle cache, K2, parallel), binary size (static frameworks, stripping, R8), runtime performance (inline functions, избегание boxing, оптимизация recomposition). Каждая область имеет свой набор инструментов.

Как K2 compiler улучшает KMP builds?
?
2x быстрее frontend compilation, лучший type inference (меньше explicit types), оптимизированный multi-target resolution. Включается через Settings -> K2 mode или languageVersion = "2.0" в Gradle.

Как уменьшить размер iOS framework?
?
isStatic = true (static linking), -Xstrip-debug-info (убрать debug symbols для release), минимизировать exported API, не экспортировать internal классы. Типичное уменьшение: 30-50%.

Как оптимизировать Gradle build time для KMP?
?
org.gradle.caching=true, org.gradle.parallel=true, configuration cache, convention plugins, кэширование ~/.konan/ в CI, отключение ненужных targets для dev (только Android + JVM).

Какие runtime оптимизации важны для KMP?
?
inline functions для высокочастотных вызовов (избежание boxing), value classes для wrapper types, lazy initialization, efficient collections (kotlinx-collections-immutable), минимизация recomposition в Compose.
...
|-------------|------|-------|
| Следующий шаг | [[kmp-production-checklist]] | Чеклист готовности к production |
| Углубиться | [[kmp-gradle-deep-dive]] | Детальная оптимизация Gradle |
| Смежная тема | [[kmp-memory-management]] | Memory management как часть performance |
