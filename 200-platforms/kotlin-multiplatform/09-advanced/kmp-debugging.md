...
> [!question]- Почему отладка KMP на iOS сложнее, чем на Android?
> На Android shared-код -- JVM bytecode, отлаживается стандартным Android Studio debugger. На iOS -- Kotlin/Native binary, требует LLDB через Xcode. Kotlin-символы не всегда корректно отображаются в LLDB без xcode-kotlin plugin. Stack traces содержат mangled names.

> [!question]- Вы получили crash report с iOS, содержащий только native stack trace без Kotlin-классов. Как найти причину?
> Использовать CrashKiOS для деманглирования Kotlin/Native stack traces. Настроить DSYM-файлы для symbolication. Установить xcode-kotlin plugin для корректного маппинга native frames на Kotlin source code.

> [!question]- Почему Kermit рекомендуется вместо println для logging в KMP?
> println не дифференцирует уровни логов и не использует platform-native logging. Kermit интегрируется с Logcat (Android), OSLog (iOS), console (JS), поддерживает severity levels и tag filtering. В production println не информативен.

---

## Ключевые карточки

Какие инструменты используются для отладки KMP на iOS?
?
xcode-kotlin plugin (Kotlin debugging в Xcode), LLDB (low-level debugger), CrashKiOS (demangling stack traces), Kermit (structured logging). Xcode Instruments для profiling.

Как настроить breakpoints для KMP-кода в Xcode?
?
Установить xcode-kotlin plugin. Открыть iOS-проект в Xcode, поставить breakpoint в Kotlin-файле через Source Editor. Plugin маппит Kotlin source на LLDB symbols. Работает с LLDB command line.

Что такое CrashKiOS?
?
Библиотека от Touchlab для конвертации Kotlin/Native crash stack traces в читаемый формат. Деманглирует native symbols обратно в Kotlin class/function names. Интегрируется с Crashlytics, Sentry, Bugsnag.

Как отлаживать корутины в KMP?
?
На JVM: coroutine debugger в IntelliJ/Android Studio (-Dkotlinx.coroutines.debug). На iOS: отладка сложнее, нужен structured logging (Kermit) и explicit exception handling. Flow debugging через Turbine в тестах.

Как настроить crash reporting для KMP iOS?
?
CrashKiOS + Firebase Crashlytics/Sentry: CrashKiOS перехватывает Kotlin/Native crashes и деманглирует stack traces. Crashlytics/Sentry получает читаемые crash reports с Kotlin class names.
...
|-------------|------|-------|
| Следующий шаг | [[kmp-interop-deep-dive]] | ObjC bridge, Swift Export -- источник многих debug-проблем |
| Углубиться | [[kmp-memory-management]] | Memory-related crashes и их диагностика |
| Смежная тема | [[kmp-troubleshooting]] | Решение типичных проблем |
