...
> [!question]- Почему Kotlin/Native перешёл с freeze-based модели на новый Memory Manager?
> Старая модель требовала freeze() для sharing объектов между потоками, что делало многопоточность сложной и error-prone. Новый Memory Manager (default с Kotlin 1.7.20) использует tracing GC совместимый с ARC, позволяя свободно передавать объекты между потоками.

> [!question]- Как retain cycle может образоваться между Swift и Kotlin/Native объектами?
> Swift closure захватывает Kotlin-объект (strong reference). Kotlin-объект хранит ссылку на callback, который содержит Swift closure. Образуется цикл: Swift -> Kotlin -> callback -> Swift. Решение: [weak self] в Swift или разрыв цикла через SKIE.

> [!question]- Почему JVM GC и Kotlin/Native GC работают по-разному и как это влияет на shared-код?
> JVM использует generational tracing GC (G1, ZGC) с stop-the-world паузами. Kotlin/Native -- tracing GC совместимый с ARC. Shared-код не должен зависеть от конкретного GC behavior (finalizers, weak references работают по-разному).

---

## Ключевые карточки

Как работает Memory Manager в Kotlin/Native?
?
Новый MM (default с 1.7.20) использует tracing GC с поддержкой cycle detection. Совместим с Swift ARC. Объекты можно свободно передавать между потоками. Нет freeze(), нет InvalidMutabilityException.

Что такое retain cycle в контексте KMP?
?
Взаимная сильная ссылка между Swift и Kotlin-объектами: Swift -> Kotlin -> callback -> Swift. GC не может освободить такой цикл. Обнаруживается через Xcode Memory Graph Debugger или Instruments Leaks.

Как GC отличается на разных KMP-платформах?
?
JVM: generational GC (G1, ZGC, Shenandoah). Kotlin/Native: tracing GC + ARC compatibility. JS: браузерный GC. Wasm: WasmGC (браузерный). Shared-код не должен полагаться на GC-специфичное поведение.

Как избежать memory leaks в KMP?
?
Использовать structured concurrency (scope-based lifetime), [weak self] в Swift closures, не хранить long-lived ссылки на UI-объекты в shared-коде, тестировать с Xcode Instruments Leaks.

Что такое autoreleasepool в контексте KMP iOS?
?
Механизм Objective-C/Swift для отложенного освобождения объектов. В KMP iOS может быть нужен в циклах, создающих много временных ObjC-объектов. Без autoreleasepool пиковое использование памяти растёт.
...
|-------------|------|-------|
| Следующий шаг | [[kmp-performance-optimization]] | Оптимизация, включающая memory efficiency |
| Углубиться | [[kmp-interop-deep-dive]] | Interop-механизмы, влияющие на memory |
| Смежная тема | [[garbage-collection-explained]] | CS-фундамент: GC algorithms |
