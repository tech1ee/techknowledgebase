...
> [!question]- Почему expect/actual считается compile-time platform polymorphism, а не runtime polymorphism?
> Компилятор подставляет actual-реализацию на этапе компиляции для каждого target. В runtime нет dispatch, vtable lookup или ветвления. Код исполняется так, как если бы был написан напрямую для платформы, что даёт нулевой runtime overhead.

> [!question]- Вы хотите создать компонент Analytics, который можно подменить в тестах фейковой реализацией. Какой паттерн выбрать: expect object или interface + expect factory?
> Interface + expect factory function. expect object нельзя подменить в тестах, потому что это singleton с фиксированной реализацией. Interface позволяет создать FakeAnalytics для тестов, а expect factory обеспечивает платформо-специфичное создание production-реализации.

> [!question]- Почему typealias для expect class работает только при structural compatibility с platform-типом?
> actual typealias устанавливает эквивалентность типов. Компилятор проверяет, что platform-тип имеет все конструкторы и методы, объявленные в expect class. Если platform-тип не имеет нужных методов (например, NSDate не совпадает по API), typealias не сработает.

> [!question]- Почему expect-декларация не может содержать реализацию (тело функции)?
> expect -- это контракт, аналог abstract-декларации. Если бы expect содержал реализацию, common-код стал бы привязан к конкретной логике, нарушая принцип разделения между объявлением контракта и его платформенной реализацией.
...
?
Механизм compile-time platform polymorphism: expect объявляет контракт в common-коде, actual предоставляет реализацию для каждой платформы. Компилятор гарантирует наличие actual для всех targets.

Какие конструкции поддерживают expect/actual?
?
Functions, properties, objects, classes (Beta, требуют -Xexpect-actual-classes), enums, annotations и typealias для маппинга на существующие platform-типы.

Что такое паттерн Factory Function + Interface?
?
Лучший паттерн для большинства случаев: interface в commonMain определяет контракт, expect fun создаёт экземпляр. Actual factory в platform source sets возвращает платформо-специфичную реализацию. Обеспечивает testability и loose coupling.

В чём разница между expect/actual и interfaces?
?
expect/actual -- compile-time platform selection (выбор реализации при компиляции). Interfaces -- runtime polymorphism (выбор реализации в runtime). Рекомендуется использовать вместе: interface для контракта + expect factory для создания.

Когда actual можно определить в intermediate source set?
?
Когда все targets этого intermediate source set имеют одинаковую реализацию. Например, один actual в iosMain работает для iosArm64, iosX64 и iosSimulatorArm64 без дублирования.

Что такое PlatformContext паттерн?
?
Паттерн для передачи платформо-специфичного контекста в common-код: expect class PlatformContext + actual typealias = android.content.Context на Android, пустой класс на iOS. Используется для инициализации платформы.
...
|-------------|------|-------|
| Следующий шаг | [[kmp-android-integration]] | Как KMP интегрируется с Android и Jetpack |
| Углубиться | [[kmp-interop-deep-dive]] | ObjC bridge, Swift Export, cinterop, SKIE |
| Смежная тема | [[type-systems-theory]] | CS-фундамент: contracts, structural typing |
