...
> [!question]- Почему commonMain не может импортировать код из androidMain или iosMain?
> Это реализация принципа Dependency Inversion: common-код определяет контракты (интерфейсы, expect-декларации), а platform-код их реализует. Если бы common видел platform, он перестал бы быть портируемым и не мог бы компилироваться для всех платформ.

> [!question]- Вы добавили зависимость androidx.core:core-ktx в commonMain.dependencies и получили ошибку компиляции на iOS. Почему?
> AndroidX -- это Android-специфичная библиотека, недоступная на iOS. Зависимости, работающие только на одной платформе, должны добавляться в соответствующий platform source set (androidMain.dependencies), а не в commonMain.

> [!question]- Почему создание кастомного intermediate source set mobileMain (для Android + iOS) имеет меньше смысла, чем appleMain?
> В mobileMain нет общих platform API -- Android и iOS не имеют shared subset API. Код в mobileMain может быть только чистым Kotlin. В appleMain же доступны Foundation, UIKit и другие Apple-фреймворки, общие для iOS и macOS.

> [!question]- Какая разница между `by getting` и `by creating` при объявлении source sets?
> `by getting` используется для доступа к уже существующим source sets (commonMain, androidMain), а `by creating` -- для создания новых кастомных source sets. Использование creating для существующего source set вызовет ошибку компиляции.

---

## Ключевые карточки

Из каких трёх компонентов состоит Source Set?
?
Исходные файлы (src/<name>/kotlin/), dependencies (implementation, api, compileOnly) и compiler options (languageVersion, apiVersion, freeCompilerArgs). Эти три элемента определяют, что компилируется, с чем связывается и как.

Чем implementation отличается от api при объявлении зависимостей?
?
implementation -- внутренняя зависимость, скрытая от модулей-потребителей. api -- публичная зависимость, видимая всем зависящим модулям. Неправильное использование может привести к compilation errors в зависимых модулях.

Что такое intermediate source set и какую проблему он решает?
?
Source set, общий для подмножества платформ (iosMain для всех iOS-архитектур). Решает проблему дублирования: без iosMain один и тот же код пришлось бы копировать в iosArm64Main, iosX64Main и iosSimulatorArm64Main.

Как Default Hierarchy Template определяет, какие intermediate source sets создавать?
?
На основе объявленных targets. Если есть iosX64 и iosArm64 -- создаётся iosMain. Если добавить macosX64 -- появится appleMain и nativeMain. Template автоматически строит иерархию dependsOn.

Почему test source sets требуют явного объявления зависимостей?
?
Test source sets видят код из main source sets, но не наследуют их dependencies. Нужно явно добавить kotlin("test"), coroutines-test и другие тестовые библиотеки в commonTest.dependencies.

Как BOM помогает избежать проблем с версиями зависимостей?
?
BOM (Bill of Materials) задаёт согласованные версии для группы библиотек. Например, ktor-bom:3.0.3 гарантирует совместимость ktor-client-core, ktor-client-okhttp и ktor-client-darwin без ручного указания версий.
...
|-------------|------|-------|
| Следующий шаг | [[kmp-expect-actual]] | Механизм expect/actual, работающий внутри source set hierarchy |
| Углубиться | [[kmp-gradle-deep-dive]] | Продвинутая конфигурация Gradle и optimization |
| Смежная тема | [[module-systems]] | CS-фундамент: encapsulation, visibility, cohesion |
