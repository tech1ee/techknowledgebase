...
> [!question]- Почему runTest из kotlinx-coroutines-test необходим для тестирования suspend-функций в KMP?
> runTest предоставляет TestCoroutineScheduler, который позволяет контролировать виртуальное время: advanceTimeBy пропускает delay без реального ожидания. Без runTest тесты с delay были бы медленными или flaky.

> [!question]- Как Turbine упрощает тестирование Flow в KMP?
> Turbine предоставляет test() extension для Flow: awaitItem() ожидает следующее значение, awaitComplete() -- завершение, expectNoEvents() -- отсутствие событий. Без Turbine тестирование Flow требует ручного collect и timeout management.

> [!question]- Почему assertEquals из kotlin.test предпочтительнее JUnit assertEquals для KMP?
> kotlin.test -- мультиплатформенная библиотека, работающая в commonTest на всех targets. JUnit 4/5 -- только JVM. kotlin.test делегирует JUnit на JVM, XCTest на Native, и Jasmine на JS, обеспечивая единый API.

---

## Ключевые карточки

Какие assertion-библиотеки доступны для KMP unit-тестов?
?
kotlin.test (базовые: assertEquals, assertTrue), Kotest assertions (rich: shouldBe, shouldContain, shouldThrow), kotlin-test-annotations (@Test, @BeforeTest). Kotest рекомендуется для выразительных тестов.

Как тестировать корутины в KMP?
?
Через runTest из kotlinx-coroutines-test: управляет виртуальным временем, TestDispatcher заменяет реальные dispatchers. advanceTimeBy для delay, advanceUntilIdle для завершения pending coroutines.

Как тестировать Flow с Turbine?
?
flow.test { awaitItem() shouldBe expected; cancelAndIgnoreRemainingEvents() }. Turbine поддерживает hot и cold Flow, timeout для зависших тестов, expectMostRecentItem() для StateFlow.

Как организовать test doubles в KMP?
?
Interface + Fake implementation (рекомендуется для KMP). Mokkery для mock/spy через KSP. Fake в commonTest для shared тестов, mock -- когда нужно проверять вызовы и аргументы.

Как запускать unit-тесты для конкретной платформы?
?
./gradlew :shared:jvmTest (JVM), ./gradlew :shared:iosSimulatorArm64Test (iOS), ./gradlew :shared:jsTest (JS). ./gradlew :shared:allTests -- для всех платформ.
...
|-------------|------|-------|
| Следующий шаг | [[kmp-integration-testing]] | Integration-тесты с MockEngine и SQLDelight |
| Углубиться | [[kmp-testing-strategies]] | Общая стратегия тестирования KMP |
| Смежная тема | [[kotlin-testing]] | Фундаментальные принципы тестирования на Kotlin |
