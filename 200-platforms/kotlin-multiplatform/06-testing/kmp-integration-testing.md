...
> [!question]- Почему integration-тесты в KMP обычно размещаются в commonTest, а не в platform-specific test source sets?
> Shared бизнес-логика (repositories, use cases) живёт в commonMain. Integration-тесты проверяют взаимодействие этих компонентов с MockEngine (Ktor) и in-memory DB (SQLDelight), которые доступны в commonTest. Тестирование в commonTest гарантирует работу на всех платформах.

> [!question]- Как тестировать Repository, который использует и Ktor, и SQLDelight, в integration-тесте?
> Создать MockEngine с предопределёнными HTTP-ответами, in-memory SQLDelight database. Инициализировать Repository с обеими fake-зависимостями. Вызвать метод Repository и проверить, что данные сохранены в DB и корректно возвращены.

> [!question]- Почему Fakes предпочтительнее Mocks для integration-тестов в KMP?
> Fakes реализуют interface с реальной (упрощённой) логикой, что делает тесты более реалистичными. Mocks проверяют вызовы методов, но не поведение. В KMP Fakes работают в commonTest без reflection, которого нет на Kotlin/Native.

---

## Ключевые карточки

Как MockEngine используется в integration-тестах?
?
MockEngine подменяет реальный HTTP engine Ktor. Respond handler возвращает предопределённые JSON-ответы. Можно проверять URL, headers, body запросов. Тесты работают без сети, быстро и детерминированно.

Как тестировать SQLDelight с in-memory базой?
?
JdbcSqliteDriver(IN_MEMORY) создаёт базу в памяти. Schema.create(driver) инициализирует таблицы. Тесты выполняют queries и проверяют результаты. База уничтожается после теста автоматически.

Что такое Fake pattern в integration-тестах?
?
Fake -- реализация interface с упрощённой логикой (in-memory storage вместо реальной DB, static data вместо API). Fakes определяются в commonTest и переиспользуются во всех тестах. Более реалистичны, чем mocks.

Как организовать test fixtures для integration-тестов?
?
Создать test factory-функции для domain-моделей, shared MockEngine configurations, base test classes с setup/teardown. Размещать в commonTest/kotlin/fixtures/ для переиспользования между тестами.

Как тестировать error scenarios в integration-тестах?
?
MockEngine может возвращать error status codes (404, 500), timeout exceptions, malformed JSON. SQLDelight -- schema violations, constraint errors. Тесты проверяют корректную обработку ошибок в Repository/UseCase.
...
|-------------|------|-------|
| Следующий шаг | [[kmp-gradle-deep-dive]] | Build system для запуска тестов |
| Углубиться | [[kmp-unit-testing]] | Фундамент: unit-тесты, на которых строятся integration |
| Смежная тема | [[kmp-ktor-networking]] | Ktor Client, тестируемый через MockEngine |
