...
> [!question]- Почему миграция с Flutter на KMP может быть мотивирована проблемами с нативным look-and-feel?
> Flutter рендерит собственный UI через Skia, эмулируя нативные компоненты. KMP позволяет использовать настоящий нативный UI (Compose для Android, SwiftUI для iOS), обеспечивая идеальное соответствие платформенным гайдлайнам и accessibility.

> [!question]- Какие части Flutter-приложения проще всего мигрировать на KMP?
> Business logic, data models, repository layer, networking -- эти слои не зависят от UI-фреймворка. Dart-код переписывается на Kotlin (языки похожи: null safety, async/await, data classes). UI -- самая сложная часть миграции.

> [!question]- Почему команда Dart-разработчиков может столкнуться с проблемами при переходе на KMP?
> KMP требует знания Kotlin + нативных платформ (Android SDK, iOS/Xcode). Dart-разработчики часто не имеют опыта с нативной разработкой. Нужно обучение Kotlin (1-2 недели) и базовое понимание Android/iOS (2-4 недели).

---

## Ключевые карточки

Чем KMP отличается от Flutter по архитектуре?
?
Flutter: единый Dart runtime + собственный rendering engine (Skia) на всех платформах. KMP: нативная компиляция для каждой платформы (JVM/Native/JS) + нативный или Compose MP UI. KMP ближе к платформе.

Какие эквиваленты Dart-конструкций есть в Kotlin?
?
Dart async/await -> Kotlin suspend/coroutines, Dart Stream -> Kotlin Flow, Dart null safety -> Kotlin null safety, Dart data class -> Kotlin data class, Dart Provider -> Kotlin Koin/kotlin-inject.

Когда стоит мигрировать с Flutter на KMP?
?
Когда нужен настоящий нативный UI, максимальная платформенная интеграция, команда знает Kotlin, или Flutter-приложение имеет проблемы с performance/platform fidelity.

Как организовать поэтапную миграцию с Flutter?
?
1) Выделить business logic из Flutter, 2) Переписать на Kotlin в shared-модуле, 3) Интегрировать shared-модуль в новое Android/iOS приложение, 4) Мигрировать UI-экраны последовательно.

Какие библиотеки Flutter заменяются в KMP?
?
http/dio -> Ktor, sqflite -> SQLDelight, shared_preferences -> DataStore KMP, provider/riverpod -> Koin, go_router -> Decompose/Voyager, json_serializable -> kotlinx-serialization.
...
|-------------|------|-------|
| Следующий шаг | [[kmp-migration-from-rn]] | Миграция с React Native на KMP |
| Углубиться | [[kmp-architecture-patterns]] | Архитектура shared-слоя после миграции |
| Смежная тема | [[cross-decision-guide]] | Сравнение кросс-платформенных решений |
