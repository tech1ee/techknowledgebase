...
> [!question]- Почему миграция с нативных Android + iOS на KMP рекомендуется начинать с networking/data layer?
> Networking/data layer имеет минимальную связь с UI и платформенными API. Models, DTOs, repository interfaces -- чистый Kotlin без Android/iOS зависимостей. Это позволяет получить быструю выгоду (единый код) с минимальным риском для существующего приложения.

> [!question]- iOS-команда сопротивляется внедрению KMP. Какие аргументы и подход помогут?
> Начать с SKIE + Swift Export, чтобы KMP-код выглядел как нативный Swift. Внедрить один небольшой модуль (utility/networking) как proof of concept. Показать, что iOS-разработчики продолжают писать на Swift, а KMP -- просто ещё одна Swift library.

> [!question]- Почему постепенная миграция (модуль за модулем) предпочтительнее полной переписки?
> Полная переписка -- высокий риск: месяцы работы без production value, регрессии, feature freeze. Постепенная миграция даёт измеримую выгоду на каждом этапе, позволяет откатиться и не блокирует feature development.

---

## Ключевые карточки

Какой порядок миграции рекомендуется для native приложений?
?
1) Models/DTOs, 2) Networking (Ktor), 3) Repository layer, 4) Use cases/business logic, 5) ViewModel (опционально), 6) UI (опционально, Compose MP). Снизу вверх, от data к presentation.

Как интегрировать KMP shared-модуль в существующее Android-приложение?
?
Добавить :shared модуль в settings.gradle.kts, implementation(project(":shared")) в app module. Постепенно заменять Android-специфичные реализации на shared-код. Не требует переписки -- модуль добавляется рядом.

Как интегрировать KMP shared-модуль в существующее iOS-приложение?
?
Через Direct Integration, CocoaPods или SPM. KMP генерирует .framework, который iOS-проект импортирует. SKIE трансформирует API в идиоматичный Swift. iOS-код вызывает shared-функции как обычные Swift-методы.

Какие метрики отслеживать при миграции?
?
Процент shared-кода (цель: 60-80%), build time (не должен сильно расти), crash rate (не должен расти), code duplication (должен уменьшаться), time-to-feature (должен уменьшаться).

Когда НЕ стоит мигрировать на KMP?
?
Маленькое приложение на одной платформе, команда без Kotlin-опыта, критический deadline, приложение в maintenance mode. KMP даёт ROI при активной разработке двух+ платформ.
...
|-------------|------|-------|
| Следующий шаг | [[kmp-migration-from-flutter]] | Миграция с Flutter на KMP |
| Углубиться | [[kmp-architecture-patterns]] | Архитектура shared-слоя для миграции |
| Смежная тема | [[kmp-production-checklist]] | Чеклист готовности к production |
