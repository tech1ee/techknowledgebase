...
> [!question]- Почему React Native разработчики мотивированы мигрировать на KMP?
> RN имеет проблемы: JS bridge overhead, сложность нативных модулей, зависимость от community packages, частые breaking changes. KMP компилируется в нативный код без bridge, имеет стабильный API и лучшую производительность.

> [!question]- Какая главная сложность миграции с React Native на KMP для JavaScript-команды?
> Команда должна изучить Kotlin и нативные платформы. JavaScript-разработчики обычно не имеют опыта с typed languages, compilation, Gradle. Переход требует 4-8 недель обучения и смены mental model.

> [!question]- Как Kotlin/JS может облегчить миграцию с React Native?
> Kotlin/JS позволяет писать Kotlin-код, компилируемый в JavaScript. Можно постепенно заменять JS-модули на Kotlin в существующем RN-приложении, а затем мигрировать на нативный UI с тем же shared-кодом.

---

## Ключевые карточки

Чем KMP отличается от React Native по архитектуре?
?
RN: JavaScript runtime + bridge к нативным компонентам (New Architecture: JSI + Fabric). KMP: нативная компиляция без bridge. KMP -- нативный код, RN -- интерпретируемый JS с нативным UI.

Какие эквиваленты RN-конструкций есть в KMP?
?
fetch/axios -> Ktor, AsyncStorage -> DataStore KMP, Redux/MobX -> MVI/StateFlow, React Navigation -> Decompose/Voyager, TypeScript interfaces -> Kotlin data classes/sealed classes.

Когда стоит мигрировать с React Native на KMP?
?
Когда нужна лучшая производительность, стабильность, type safety. Когда команда готова инвестировать в Kotlin. Когда RN community packages не покрывают нужды. Когда приложение performance-critical.

Какие части RN-приложения мигрируются первыми?
?
API layer (fetch -> Ktor), data models (TypeScript -> Kotlin), state management (Redux -> MVI). UI мигрируется последней, так как требует максимальных изменений.

Какие преимущества KMP над React Native?
?
Нативная компиляция (нет JS bridge), type safety (Kotlin vs JS), лучшая IDE поддержка, стабильный API без breaking changes, нативный UI (Compose/SwiftUI), меньше runtime crashes.
...
|-------------|------|-------|
| Следующий шаг | [[kmp-debugging]] | Отладка KMP-приложений |
| Углубиться | [[kmp-migration-from-native]] | Миграция с нативных приложений |
