...
> [!question]- Почему для KMP CI/CD обязательно нужен macOS runner?
> iOS-таргет требует Xcode для компиляции через LLVM. Xcode доступен только на macOS. Без macOS runner невозможно собрать iOS framework, запустить iOS-тесты или создать iOS binary. JVM и JS тесты можно запускать на Linux.

> [!question]- Как оптимизировать CI pipeline для KMP, чтобы не тратить дорогие macOS-минуты на JVM-тесты?
> Использовать matrix strategy: JVM/JS тесты на дешёвом ubuntu runner, iOS тесты -- только на macOS. Caching ~/.konan/, ~/.gradle/, build/ между runs. Parallel execution для независимых targets.

> [!question]- Почему кэширование ~/.konan/ критически важно для CI KMP-проектов?
> ~/.konan/ содержит Kotlin/Native compiler и platform libraries (~2-3 GB). Без кэша каждый CI run скачивает и распаковывает их заново, добавляя 5-10 минут к build time. С кэшем -- секунды.

---

## Ключевые карточки

Какие CI-системы поддерживают KMP?
?
GitHub Actions (рекомендуется, бесплатные macOS runners для open source), Bitrise (мобильно-ориентированная), TeamCity (от JetBrains), GitLab CI, CircleCI. Все требуют macOS для iOS builds.

Как организовать GitHub Actions workflow для KMP?
?
Matrix strategy: ubuntu-latest для JVM/JS tests, macos-latest для iOS tests. Steps: checkout, setup-java, gradle-build-action (с кэшем), ./gradlew allTests. Кэширование ~/.gradle/ и ~/.konan/.

Как Fastlane интегрируется с KMP?
?
Fastlane автоматизирует iOS-specific tasks: signing, screenshots, App Store upload. KMP Gradle build создаёт framework, Fastlane собирает iOS-app с этим framework и публикует. Complementary tools, не замена.

Какие артефакты нужно кэшировать в CI для KMP?
?
~/.gradle/caches/ (Gradle dependencies), ~/.gradle/wrapper/ (Gradle distribution), ~/.konan/ (Kotlin/Native), build/ (build cache). Суммарно ускоряет CI на 50-70%.

Как настроить автоматический release в KMP?
?
Semantic versioning в gradle.properties. CI trigger на tag push. Gradle tasks: build, test, publish. Для Android: bundleRelease + Play Store upload. Для iOS: Fastlane + App Store Connect. Для Maven: publishAllPublicationsToMavenRepository.
...
|-------------|------|-------|
| Следующий шаг | [[kmp-publishing]] | Публикация KMP-библиотек |
| Углубиться | [[kmp-gradle-deep-dive]] | Оптимизация Gradle для ускорения CI |
| Смежная тема | [[ci-cd-pipelines]] | Общие принципы CI/CD |
