...
> [!question]- Почему KMP-библиотека публикуется как несколько артефактов, а не один?
> KMP-библиотека компилируется для каждого target отдельно: -jvm.jar, -iosx64.klib, -js.klib. Gradle metadata (.module) описывает все варианты. Потребитель автоматически получает правильный артефакт для своего target через Gradle dependency resolution.

> [!question]- Как iOS-разработчик может использовать опубликованную KMP-библиотеку без Gradle?
> Через XCFramework (SPM/CocoaPods) или KMMBridge. KMMBridge автоматизирует: builds XCFramework, публикует на GitHub Releases/S3, генерирует podspec/Package.swift. iOS-разработчик добавляет зависимость через SPM или CocoaPods как обычную iOS-библиотеку.

> [!question]- Почему публикация на Maven Central требует GPG signing?
> Maven Central -- публичный репозиторий с требованиями безопасности: GPG signing подтверждает авторство артефакта, предотвращая supply chain attacks. Без подписи Sonatype/Maven Central отклоняет публикацию.

---

## Ключевые карточки

Как опубликовать KMP-библиотеку на Maven Central?
?
Через vanniktech/gradle-maven-publish-plugin: настроить coordinates (groupId, artifactId), GPG signing, Sonatype credentials. ./gradlew publishAllPublicationsToMavenCentralRepository. Автоматически публикует все target-артефакты.

Что такое KMMBridge?
?
Инструмент от Touchlab для публикации KMP-библиотек как iOS frameworks. Автоматизирует: build XCFramework, upload на GitHub Releases/S3/Maven, генерация podspec и Package.swift для SPM.

Как работает XCFramework для iOS distribution?
?
XCFramework объединяет бинарники для всех iOS архитектур (arm64 device, arm64 simulator, x86_64 simulator) в один пакет. Распространяется через SPM, CocoaPods или manual integration.

Что такое Gradle metadata (.module файл)?
?
JSON-файл, описывающий все варианты KMP-артефакта: targets, dependencies, capabilities. Позволяет Gradle автоматически выбирать правильный артефакт для target потребителя. Публикуется вместе с .jar/.klib.

Как настроить version management для KMP-библиотеки?
?
Semantic versioning в gradle.properties или через git tags. vanniktech plugin читает версию автоматически. CI публикует при tag push. SNAPSHOT-версии для development, release -- для стабильных.
...
|-------------|------|-------|
| Следующий шаг | [[kmp-migration-from-native]] | Миграция существующих приложений на KMP |
| Углубиться | [[kmp-ci-cd]] | CI/CD для автоматизации публикации |
| Смежная тема | [[kmp-gradle-deep-dive]] | Gradle конфигурация для publishing |
