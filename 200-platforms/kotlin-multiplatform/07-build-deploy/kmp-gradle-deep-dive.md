...
> [!question]- Почему Gradle build cache критически важен для KMP-проектов?
> KMP компилирует код для нескольких targets (JVM, Native, JS), что многократно увеличивает время сборки. Build cache сохраняет результаты tasks и переиспользует их при неизменённых inputs. Без cache каждый build пересобирает все targets с нуля.

> [!question]- Вы заметили, что Gradle sync занимает 3+ минуты в KMP-проекте с 50 модулями. Какие оптимизации помогут?
> Convention plugins (вместо дублирования конфигурации), configuration cache, parallel execution, отключение ненужных targets для development, org.gradle.configureondemand=true, минимизация buildscript dependencies.

> [!question]- Почему Convention Plugins предпочтительнее buildSrc для shared build logic в KMP?
> buildSrc invalidирует весь build cache при любом изменении. Convention plugins в build-logic/ -- отдельный included build, изменения которого не инвалидируют cache остальных модулей. Это критично для performance.

---

## Ключевые карточки

Какие Gradle optimization techniques важны для KMP?
?
Build cache (local + remote), configuration cache, parallel execution, convention plugins, incremental compilation, task avoidance API. Вместе могут сократить build time на 50-80%.

Что такое Convention Plugin в контексте KMP?
?
Gradle plugin в build-logic/, содержащий стандартную конфигурацию KMP-модуля: targets, compiler options, dependencies. Применяется через plugins { id("kmp-library-convention") }. Избавляет от дублирования.

Как работает Kotlin/Native compilation caching?
?
~/.konan/ хранит скомпилированные klibs и LLVM tools (~2-3 GB). Первая сборка медленная (скачивание + компиляция stdlib). Последующие используют cache. В CI cache нужно сохранять между runs.

Какие Gradle properties оптимизируют KMP builds?
?
kotlin.mpp.stability.nowarn, org.gradle.caching=true, org.gradle.parallel=true, kotlin.native.cacheKind=static, org.gradle.configuration-cache=true. Каждый property влияет на конкретный аспект build performance.

Как configuration cache помогает в KMP?
?
Сериализует результат configuration phase и переиспользует при неизменённых build scripts. В KMP с множеством targets configuration phase дорогая. Cache пропускает её при повторных builds.
...
|-------------|------|-------|
| Следующий шаг | [[kmp-ci-cd]] | CI/CD для автоматизации builds |
| Углубиться | [[kmp-project-structure]] | Структура проекта, определяющая Gradle конфигурацию |
| Смежная тема | [[build-systems-theory]] | CS-фундамент: DAG, task graph, caching theory |
