...
> **Software Distribution** — процесс доставки программного обеспечения от разработчика к конечному пользователю. iOS реализует модель **walled garden** (закрытый сад): единственный авторизованный канал — App Store, контролируемый Apple, с обязательной проверкой (App Review) каждого приложения.

### Академический контекст

Модель дистрибуции iOS основана на нескольких теоретических концепциях:

| Концепция | Автор / год | Суть | Проявление в iOS |
|-----------|-------------|------|-------------------|
| Walled Garden | Термин телекоммуникаций, 1990-е | Закрытая экосистема с контролируемым доступом | App Store как единственный канал для пользователей |
| Gatekeeper Model | Access Control, 1970-е | Централизованная проверка перед допуском | App Review — ручная + автоматическая проверка |
| Digital Rights Management | Content Scramble System, 1996 | Контроль копирования и распространения | FairPlay DRM для App Store приложений |
| Staged Rollout | Canary Releases, 2000-е | Постепенное развёртывание для снижения рисков | TestFlight → Phased Release (1-100%) → Full Release |
| Trust but Verify | Reagan Doctrine, 1980-е | Доверяй разработчику, но проверяй код | Code signing + App Review + runtime sandboxing |

### Каналы дистрибуции и их ограничения

| Канал | Аудитория | Лимит устройств | App Review | Срок действия |
|-------|-----------|----------------|------------|---------------|
| Development | Разработчик | 100 UDID | Нет | 7 дней (free) / 1 год |
| Ad Hoc | Тестировщики | 100 UDID | Нет | 1 год |
| TestFlight Internal | Команда | 100 | Нет | 90 дней |
| TestFlight External | Бета-тестеры | 10,000 | Beta Review | 90 дней |
| App Store | Все пользователи | Без лимита | Full Review | Бессрочно |
| Enterprise | Сотрудники компании | Без лимита | Нет | 1 год (сертификат) |

> **Модель walled garden** создаёт **двусторонний рынок** (Rochet & Tirole, 2003): Apple выступает платформой, соединяющей разработчиков и пользователей. Комиссия 15-30% — это плата за доступ к аудитории, инфраструктуру дистрибуции и доверие (App Review как гарант качества).

### Связь с CS-фундаментом

- [[ios-code-signing]] — криптографическая основа дистрибуции (сертификаты, profiles)
- [[ios-ci-cd]] — автоматизация upload в TestFlight/App Store
- [[ios-permissions-security]] — Privacy Review как часть App Review
- [[ios-market-trends-2026]] — DMA, sideloading, альтернативные магазины в ЕС
- [[cross-distribution]] — сравнение App Store vs Google Play дистрибуции
