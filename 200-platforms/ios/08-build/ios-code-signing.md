...
**[[ios-app-distribution]]** — Code signing является фундаментом для всех методов дистрибуции iOS-приложений. Development certificate нужен для запуска на устройстве, Distribution certificate — для TestFlight и App Store, а Enterprise certificate — для внутрикорпоративных приложений. Без понимания связи между сертификатами, provisioning profiles и entitlements невозможно корректно опубликовать приложение. Рекомендуется изучить code signing перед дистрибуцией.

**[[ios-ci-cd]]** — Автоматизация code signing в CI/CD — одна из самых сложных задач iOS-разработки. Fastlane match хранит сертификаты и profiles в зашифрованном Git-репозитории, а Xcode Cloud использует cloud signing с хранением credentials на серверах Apple. Понимание ручного процесса signing необходимо для отладки проблем в автоматизированных pipeline. Изучите code signing вручную, затем автоматизируйте.

**[[cross-code-signing]]** — Кросс-платформенное сравнение подписания кода (iOS certificates + provisioning profiles vs Android keystore) показывает, что iOS использует значительно более сложную многоуровневую систему безопасности. Понимание различий помогает при планировании multi-platform release процессов и объясняет, почему iOS CI/CD требует больше усилий по настройке.
...
> [!question]- Почему Apple требует code signing для каждого iOS-приложения, и что именно подписывается?
> Code signing гарантирует: идентификацию разработчика (кто подписал), целостность кода (не изменен после подписи). Подписывается: исполняемый файл, все ресурсы (hash каждого файла в _CodeSignature/CodeResources), embedded frameworks. При нарушении целостности iOS откажется запускать приложение. Защита от модификации malware.

> [!question]- В чем разница между Development и Distribution provisioning profile, и когда нужен каждый?
> Development: для отладки на конкретных устройствах (UDID в профиле), подписан Development certificate, включает debug entitlements. Distribution: для App Store/TestFlight/Enterprise, подписан Distribution certificate, без привязки к устройствам (кроме Ad Hoc). Development для разработки, Distribution для релиза.

> [!question]- Сценарий: CI/CD сервер не может подписать приложение, ошибка "No signing certificate matching team ID found". Как решить?
> 1) Установить Distribution certificate (.p12) и private key в Keychain сервера. 2) Убедиться, что provisioning profile соответствует certificate и Bundle ID. 3) Использовать Fastlane match (хранит certificates/profiles в git repo). 4) Проверить, что Keychain unlocked для CI (security unlock-keychain). 5) Automatic signing выключен, manual signing с указанием конкретного profile.
...
?
Файл (.mobileprovision), связывающий: App ID (Bundle Identifier), Certificate (кто подписывает), Entitlements (capabilities), Device UDIDs (для Development/Ad Hoc). Встраивается в .app bundle. iOS проверяет profile при установке и запуске. Создается в Apple Developer Portal.

Чем Development Certificate отличается от Distribution?
?
Development: для отладки на устройствах, можно иметь несколько в команде, менее строгие ограничения. Distribution: для App Store/TestFlight/Enterprise, обычно один на команду/организацию. Оба основаны на public/private key pair, private key хранится в Keychain.

Что такое Entitlements в iOS?
?
Plist-файл с правами приложения: push notifications, iCloud, App Groups, HealthKit, Sign in with Apple. Включаются через Capabilities в Xcode. Должны совпадать в entitlements файле, provisioning profile и App ID на портале. Несовпадение = ошибка подписи.

Как Fastlane Match упрощает code signing для команды?
?
Match хранит certificates и profiles в зашифрованном git-репозитории. Вся команда использует одни certificates (не создает свои). CI/CD клонирует repo и устанавливает. Решает: "works on my machine", истечение профилей, лимит certificates, onboarding новых разработчиков.
...
|-------------|------|-------|
| Следующий шаг | [[ios-app-distribution]] | Дистрибуция подписанного приложения |
| Углубиться | [[ios-ci-cd]] | Автоматизация подписи в CI/CD pipeline |
| Смежная тема | [[cross-code-signing]] | Code signing на Android и других платформах |
