...
- **[[android-gradle-fundamentals]]** — Gradle в Android выполняет ту же роль, что Xcode build system: modules соответствуют targets, build variants (debug/release + flavors) аналогичны configurations + schemes, а build.gradle файлы — это текстовый аналог project.pbxproj. Сравнение двух систем сборки показывает, что Gradle более гибок через скриптовой DSL, тогда как Xcode предоставляет GUI-ориентированный подход. Понимание обеих систем критично для кросс-платформенных команд.

- **[[ios-compilation-pipeline]]** — Xcode project определяет что собирать, а compilation pipeline определяет как: Swift compiler (swiftc) компилирует source files из target, линкер связывает объектные файлы с linked frameworks, а code signing подписывает готовый bundle. Понимание compile sources, link binary и copy resources build phases необходимо для диагностики build errors и оптимизации времени сборки.

- **[[cross-build-systems]]** — Кросс-платформенные системы сборки (CMake, Bazel, Buck) решают те же задачи, что Xcode project + Gradle: определение targets, зависимостей и конфигураций. Инструменты генерации проектов (XcodeGen, Tuist) устраняют проблему merge-конфликтов в project.pbxproj, генерируя Xcode project из декларативных YAML/Swift-описаний. Эти знания особенно важны для больших команд и монорепозиториев.
...
> [!question]- В чем разница между Target и Scheme в Xcode, и почему для CI/CD важно правильно настроить Scheme?
> Target -- определяет продукт (app, framework, tests) с его исходниками, ресурсами и build settings. Scheme -- определяет какие targets строить, с какой конфигурацией (Debug/Release), какие тесты запускать. CI/CD использует scheme для xcodebuild: неправильная scheme = неправильная конфигурация, отсутствующие тесты, wrong signing.

> [!question]- Почему xcconfig-файлы предпочтительнее настройки Build Settings через GUI Xcode?
> xcconfig: текстовые файлы, версионируются в git, легко diff, можно шаблонизировать (base.xcconfig + debug.xcconfig), CI/CD может переопределять через include. GUI: изменения в .pbxproj (бинарный формат, merge conflicts), нет наследования, сложно отслеживать историю изменений.

> [!question]- Сценарий: файл есть в проекте, но при сборке получаете "Use of unresolved identifier". В чем причина?
> Файл не добавлен в Target Membership нужного target. Проверить: File Inspector -> Target Membership (галочки). Другие причины: файл в другом модуле без import, access level (internal по умолчанию -- не виден из других модулей), файл добавлен в project, но не в compile sources Build Phase.

---

## Ключевые карточки

Что такое Xcode Target?
?
Описание конкретного продукта сборки: приложение, фреймворк, тесты, extension. Содержит: список source files, resources, build settings, dependencies, build phases (Compile Sources, Copy Bundle Resources, Link Binary). Один проект может иметь множество targets.

Что такое Xcode Scheme?
?
Конфигурация запуска: какие targets строить (Build), с какой конфигурацией запускать (Run), какие тесты выполнять (Test), как профилировать (Profile), как архивировать (Archive). Scheme может быть shared (для команды) или private (локальный).

Чем Build Configuration отличается от Build Settings?
?
Build Configuration -- именованный набор settings (Debug, Release, custom). Build Settings -- конкретные параметры (optimization level, signing, preprocessor macros). Каждый setting может иметь разные значения для разных configurations. xcconfig файлы задают settings для configuration.

Что такое Build Phase в Xcode?
?
Этап сборки target: Compile Sources (компиляция .swift/.m), Copy Bundle Resources (копирование ресурсов), Link Binary With Libraries (линковка), Run Script (кастомные скрипты: SwiftLint, код-генерация). Порядок важен: dependencies первыми.
...
|-------------|------|-------|
| Следующий шаг | [[ios-code-signing]] | Code signing -- следующий шаг после сборки |
| Углубиться | [[ios-compilation-pipeline]] | Внутренний процесс компиляции Swift |
| Смежная тема | [[android-gradle-fundamentals]] | Gradle -- система сборки Android для сравнения |
