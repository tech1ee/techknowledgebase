...
> **Персистентность данных** (data persistence) — свойство данных сохраняться между сеансами выполнения программы, выходя за пределы времени жизни процесса. Формально: данные персистентны, если они доступны после завершения создавшего их процесса (Atkinson et al., 1983 — «Orthogonal Persistence»).

### Академический контекст

Теория хранения данных в мобильных системах опирается на несколько фундаментальных концепций:

| Концепция | Автор / год | Суть | Проявление в iOS |
|-----------|-------------|------|-------------------|
| Иерархия памяти | Wilkes, 1965 | Многоуровневое хранение: регистры → кэш → RAM → диск | RAM → UserDefaults → FileManager → SQLite |
| Sandbox model | Saltzer & Schroeder, 1975 | Принцип наименьших привилегий для изоляции | App Sandbox с Documents/, Library/, tmp/ |
| Key-Value Store | DeCandia et al., 2007 (Dynamo) | Простейшая модель хранения: ключ → значение | UserDefaults, iCloud KVS |
| Orthogonal Persistence | Atkinson et al., 1983 | Персистентность не зависит от типа данных | SwiftData @Model — любой класс персистентен |
| Data Protection | NIST SP 800-111, 2007 | Шифрование данных at rest | File Protection API, Keychain encryption |

### Классификация механизмов хранения

С точки зрения теории баз данных, механизмы iOS-персистентности классифицируются по модели данных:

- **Key-Value** (UserDefaults, Keychain, iCloud KVS) — простейшая модель, O(1) доступ по ключу
- **Файловая** (FileManager) — иерархическая модель, навигация по путям
- **Реляционная** (SQLite через Core Data/SwiftData) — табличная модель, SQL-запросы
- **Объектно-графовая** (Core Data) — граф объектов с отношениями, faulting и change tracking

> **Принцип Saltzer & Schroeder** (1975): «Every program and every privileged user should operate using the least set of privileges necessary.» iOS sandbox реализует этот принцип — каждое приложение имеет доступ только к своей песочнице, а Keychain хранит данные вне песочницы с аппаратным шифрованием.

### Связь с CS-фундаментом

- [[ios-architecture]] — sandbox как реализация принципа наименьших привилегий
- [[ios-process-memory]] — иерархия памяти: RAM → persistent storage
- [[database-internals-complete]] — теория хранения данных на уровне СУБД
- [[ios-core-data]] — объектно-графовый подход к персистентности
- [[ios-permissions-security]] — Data Protection и шифрование at rest
...
**[[ios-core-data]]** — Core Data является специализированным фреймворком для объектно-графового хранения сложных данных с relationships, faulting и undo/redo. В отличие от UserDefaults и FileManager, описанных в данной заметке, Core Data управляет графом объектов в памяти с автоматическим отслеживанием изменений. Рекомендуется сначала изучить базовые механизмы хранения (UserDefaults, Keychain, FileManager), затем переходить к Core Data для сложных объектных моделей.

**[[ios-swiftdata]]** — SwiftData (iOS 17+) является modern-Swift заменой Core Data, использующей @Model macro вместо NSManagedObject. SwiftData значительно упрощает persistence layer, но построен поверх Core Data и SQLite. Понимание базовых механизмов хранения из данной заметки помогает принять осознанное решение: SwiftData для новых проектов с iOS 17+, Core Data для совместимости, UserDefaults/Keychain для простых данных.

**[[android-data-persistence]]** — Сравнение хранения данных на iOS (UserDefaults, Keychain, FileManager) и Android (SharedPreferences, EncryptedSharedPreferences, Internal/External Storage) выявляет схожие паттерны, но с разными API. iOS Keychain обеспечивает hardware-backed encryption, тогда как Android EncryptedSharedPreferences использует программное шифрование. Параллельное изучение помогает при кросс-платформенной разработке и на собеседованиях.

**[[ios-permissions-security]]** — Безопасное хранение данных тесно связано с системой разрешений и File Protection: NSFileProtectionComplete шифрует файлы до первой разблокировки устройства, а Keychain с kSecAttrAccessibleWhenUnlockedThisDeviceOnly ограничивает доступ к секретам. Понимание уровней защиты данных необходимо для соответствия требованиям безопасности и App Store Review Guidelines.

## Источники и дальнейшее чтение

### Теоретические основы
- Atkinson M., Bailey P., Chisholm K., Cockshott P., Morrison R. (1983). *An Approach to Persistent Programming.* — формализация концепции ортогональной персистентности, независимости типа данных от способа хранения
- Saltzer J., Schroeder M. (1975). *The Protection of Information in Computer Systems.* — принцип наименьших привилегий, теоретическая основа sandbox-модели iOS
- DeCandia G. et al. (2007). *Dynamo: Amazon's Highly Available Key-Value Store.* — теория key-value хранилищ, применимая к UserDefaults и iCloud KVS
- NIST SP 800-111 (2007). *Guide to Storage Encryption Technologies.* — стандарты шифрования данных at rest, основа File Protection API

### Практические руководства
- Neuburg M. (2023). *iOS 17 Programming Fundamentals with Swift.* — описывает UserDefaults, FileManager, Keychain и File Protection с практическими примерами и рекомендациями по выбору правильного механизма
- Keur C., Hillegass A. (2020). *iOS Programming: The Big Nerd Ranch Guide, 7th Edition.* — пошаговое введение в persistence с проектами-упражнениями, включая архивирование объектов и работу с файловой системой
- Eidhof C. et al. (2019). *Advanced Swift.* — продвинутые паттерны Codable для сериализации данных и protocol-oriented подход к абстракции storage layer

---

## Проверь себя

> [!question]- Почему хранить токены авторизации в UserDefaults небезопасно, даже если устройство защищено паролем?
> UserDefaults хранит данные в незашифрованном plist-файле в песочнице приложения. При jailbreak или backup на компьютер без шифрования данные читаемы. Keychain шифрует данные отдельным ключом, привязанным к Secure Enclave, и поддерживает access control (kSecAttrAccessibleWhenUnlockedThisDeviceOnly). Keychain данные не попадают в незашифрованные бэкапы.

> [!question]- В чем разница между File Protection уровнями NSFileProtectionComplete и NSFileProtectionCompleteUntilFirstUserAuthentication?
> Complete: файл зашифрован, когда устройство заблокировано (недоступен фоновым процессам). CompleteUntilFirstUserAuthentication: файл зашифрован до первой разблокировки после перезагрузки, затем доступен даже при заблокированном экране. Второй подходит для фоновых загрузок и push-уведомлений, первый -- для максимальной защиты (медицинские данные).

> [!question]- Сценарий: приложению нужно хранить настройки пользователя, кэш изображений и историю транзакций. Какой механизм выбрать для каждого?
> Настройки: UserDefaults (простые key-value, < 1MB, быстрый доступ). Кэш изображений: FileManager + URLCache (большие файлы, автоматическая очистка, директория Caches). История транзакций: SwiftData/CoreData (структурированные данные с отношениями, запросы, сортировка) + Keychain для чувствительных данных (суммы, номера карт).

---

## Ключевые карточки

Какие основные механизмы хранения данных в iOS?
?
UserDefaults (настройки, < 1MB), FileManager (файлы, документы, кэш), Keychain (секреты, токены, пароли), Core Data/SwiftData (структурированные объекты, отношения), iCloud KVS (синхронизация настроек), App Groups (данные между приложениями/extensions).

Что такое Keychain и когда его использовать?
?
Зашифрованное хранилище, защищенное Secure Enclave. Для: паролей, токенов, сертификатов, криптоключей. Данные сохраняются при удалении приложения (если не указано kSecAttrAccessibleWhenUnlockedThisDeviceOnly). API через Security.framework (SecItem).

Какие директории доступны приложению в песочнице?
?
Documents (пользовательские данные, бэкапится), Library/Caches (кэш, не бэкапится, может удаляться системой), Library/Preferences (UserDefaults), tmp (временные файлы, очищается системой). Application Support (данные приложения, бэкапится).

Что такое App Groups и зачем они нужны?
?
App Groups позволяют нескольким приложениям и extensions (Widget, Share Extension) делить данные. Создается shared container через UserDefaults(suiteName:) или FileManager.containerURL(forSecurityApplicationGroupIdentifier:). Необходимо для Widget, которому нужны данные основного приложения.

Чем iCloud Key-Value Storage отличается от CloudKit?
?
iCloud KVS: простые key-value пары (как UserDefaults), лимит 1MB, автосинхронизация, без запросов. CloudKit: полноценная БД с записями, зонами, подписками на изменения, push-уведомлениями. KVS для настроек, CloudKit для структурированных данных.
...
|-------------|------|-------|
| Следующий шаг | [[ios-swiftdata]] | Современный фреймворк персистентности для iOS 17+ |
| Углубиться | [[ios-core-data]] | Core Data для сложных объектных моделей |
| Смежная тема | [[android-data-persistence]] | Сравнить подходы к хранению данных в Android |
