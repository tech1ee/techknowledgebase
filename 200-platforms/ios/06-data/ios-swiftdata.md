...
**[[ios-core-data]]** — Core Data является предшественником SwiftData и до сих пор остается актуальным фреймворком для персистентности данных в iOS. Понимание Core Data критически важно для миграции существующих проектов на SwiftData, а также для случаев, когда SwiftData ещё не поддерживает нужную функциональность (abstract entities, batch operations, сложные конфигурации persistent stores). SwiftData внутренне использует Core Data как backend, поэтому знание архитектуры NSPersistentContainer и NSManagedObjectContext помогает глубже понять принципы работы ModelContainer и ModelContext.

**[[ios-swiftui]]** — SwiftUI является основным потребителем SwiftData, и эти два фреймворка спроектированы для тесной интеграции. Макрос @Query автоматически обновляет SwiftUI-вью при изменении данных, а ModelContainer внедряется через модификатор .modelContainer(). Реактивная природа SwiftUI (Observable, @Bindable) естественно сочетается с @Model-макросом SwiftData, что делает связку SwiftUI + SwiftData наиболее продуктивным подходом к разработке data-driven приложений на iOS 17+.

**[[ios-cloudkit]]** — CloudKit обеспечивает облачную синхронизацию данных между устройствами пользователя через iCloud. SwiftData интегрируется с CloudKit через параметр cloudKitDatabase в ModelConfiguration, что позволяет включить синхронизацию буквально одной строкой кода. Понимание ограничений CloudKit (квоты, конфликты, поддерживаемые типы данных) критично для проектирования моделей SwiftData, которые будут корректно синхронизироваться между устройствами.
...
> [!question]- Почему SwiftData использует macro @Model вместо наследования от NSManagedObject, и какие преимущества это дает?
> @Model -- Swift macro, генерирующий код конформности к PersistentModel на этапе компиляции. Не требует наследования (работает с обычными классами). Преимущества: чистый Swift-синтаксис без Objective-C наследия, автоматическое отслеживание изменений, type-safe свойства (не @NSManaged dynamic), лучшая интеграция с SwiftUI, меньше boilerplate.

> [!question]- В чем разница между @Query в SwiftData и NSFetchedResultsController в Core Data?
> @Query -- property wrapper для SwiftUI View, декларативно определяющий запрос (предикат, сортировка). Автоматически обновляет View при изменении данных (реактивный). NSFetchedResultsController -- delegate-based, требует ручной обработки изменений (controllerDidChangeContent), настройки секций, и не интегрирован с SwiftUI напрямую. @Query значительно проще в использовании.

> [!question]- Сценарий: у вас Core Data проект на iOS 15. Стоит ли мигрировать на SwiftData?
> Зависит от контекста. Мигрировать если: планируете поддерживать только iOS 17+, код активно развивается, используете SwiftUI. Не мигрировать если: нужна поддержка iOS < 17, сложные custom migrations, NSFetchedResultsController с UIKit, специфичные Core Data фичи (derived attributes, abstract entities). SwiftData и Core Data могут сосуществовать через общий persistent store.

---

## Ключевые карточки

Что такое @Model macro в SwiftData?
?
Swift macro, превращающий обычный класс в persistent модель. Генерирует конформность PersistentModel, отслеживание изменений, схему БД. Не нужен NSManagedObject, @NSManaged, .xcdatamodeld файл. Свойства определяются как обычные Swift properties.

Как @Query работает в SwiftUI?
?
Property wrapper для декларативного запроса к SwiftData. Определяет filter (Predicate), sort (SortDescriptor), animation. Автоматически обновляет View при изменении данных в ModelContext. Аналог NSFetchedResultsController, но декларативный и реактивный.

Что такое ModelContainer и ModelContext?
?
ModelContainer -- контейнер схемы и хранилища (аналог NSPersistentContainer). Конфигурируется через Schema и ModelConfiguration. ModelContext -- рабочее пространство для CRUD операций (аналог NSManagedObjectContext). Передается через SwiftUI environment (.modelContainer).

Как SwiftData обрабатывает миграции?
?
Легкие миграции автоматически (добавление поля, переименование). Для сложных: VersionedSchema + SchemaMigrationPlan с этапами (stages). Каждый stage определяет трансформации между версиями. Менее гибко, чем Core Data custom migrations, но проще в настройке.
...
|-------------|------|-------|
| Следующий шаг | [[ios-core-data]] | Core Data для понимания основ, на которых построен SwiftData |
| Углубиться | [[ios-repository-pattern]] | Repository Pattern для абстракции data layer |
| Смежная тема | [[kmp-sqldelight-database]] | SQLDelight -- кросс-платформенная альтернатива |
