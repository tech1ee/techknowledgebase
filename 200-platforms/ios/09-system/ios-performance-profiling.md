...
**[[android-performance-profiling]]** — Android Profiler (CPU, Memory, Network) предоставляет аналогичные возможности, но с другими инструментами: Android Studio Profiler vs Instruments, LeakCanary vs Memory Graph Debugger, Systrace vs System Trace. Сравнение позволяет понять универсальные принципы профилирования (sampling vs tracing, flame graphs, allocation tracking) и оценить сильные стороны каждой платформы. Для кросс-платформенных разработчиков знание обоих наборов инструментов необходимо.

**[[ios-debugging]]** — debugging и profiling являются взаимодополняющими навыками: debugging находит конкретный баг через breakpoints и LLDB, а profiling выявляет системные проблемы производительности через статистический анализ. Memory Graph Debugger (из debugging) помогает найти конкретный retain cycle, а Allocations instrument (из profiling) показывает общую картину роста памяти. Рекомендуется сначала освоить debugging, затем переходить к profiling.

**[[cross-performance-profiling]]** — кросс-платформенное сравнение инструментов профилирования (iOS Instruments, Android Profiler, Flutter DevTools, Chrome DevTools) раскрывает общие метрики и подходы к оптимизации производительности. Понимание того, что 60 FPS = 16.67ms budget является универсальным для всех платформ, помогает применять одни и те же ментальные модели при работе с разными инструментами.
...
> [!question]- Почему профилирование на Simulator недостаточно, и какие метрики отличаются от реального устройства?
> Simulator работает на x86/ARM Mac CPU (не A-series чип), не имеет GPU ограничений iOS-устройства, имеет значительно больше RAM, не подвержен thermal throttling. Отличия: CPU performance (другая архитектура), GPU rendering (нет off-screen rendering penalty), memory (не отражает реальные лимиты), battery impact (невозможно измерить). Всегда профилировать на реальном устройстве, желательно на самом слабом поддерживаемом.

> [!question]- Как MetricKit помогает мониторить производительность в production, и чем он отличается от Instruments?
> MetricKit собирает агрегированные метрики с реальных устройств пользователей: CPU time, memory peaks, disk I/O, hang rate, launch time, battery impact. Данные приходят раз в 24 часа. Instruments -- для локального профилирования с детальной информацией (call tree, allocations). MetricKit для трендов, Instruments для root cause analysis.

> [!question]- Сценарий: пользователи жалуются, что приложение "зависает" на 2-3 секунды при открытии экрана профиля. Как найти причину с помощью Instruments?
> 1) Time Profiler: найти самые тяжелые функции на main thread при открытии экрана. 2) Main Thread Checker: проверить, что нет I/O или сетевых вызовов на main thread. 3) System Trace: анализ thread scheduling и блокировок. 4) Проверить viewDidLoad/viewWillAppear на тяжелые операции. 5) Hang Detection в Xcode Organizer для production данных.

---

## Ключевые карточки

Какие основные Instruments доступны в Xcode?
?
Time Profiler (CPU sampling), Allocations (память: объемы, leaks), Leaks (retain cycles), Core Animation (FPS, off-screen rendering), Network (HTTP трафик), Energy Log (батарея), System Trace (threading, scheduling), File Activity (disk I/O). Запускаются через Product -> Profile.

Что показывает Time Profiler и как его интерпретировать?
?
Sampling-профайлер: каждые 1ms записывает call stack. Показывает: Self Time (время в самой функции), Total Time (включая вызываемые). Heaviest Stack Trace -- самый дорогой путь. Фокус: main thread для UI-проблем. Invert Call Tree для bottom-up анализа. Charge filter по thread.

Что такое Memory Graph Debugger?
?
Инструмент Xcode (Debug Memory Graph): показывает все живые объекты и их ссылки. Находит retain cycles (фиолетовые предупреждения), leaked objects, unexpected strong references. Работает в runtime: пауза приложения, анализ heap. Дополнение к Instruments Leaks.

Что такое MetricKit?
?
Framework для сбора production-метрик (iOS 13+). MXMetricManager.shared: CPU, memory, disk, network, hang rate, launch time, battery drain. MXDiagnosticPayload (iOS 14+): crash logs, hang reports, disk write exceptions. Данные приходят 1 раз в 24 часа. Не влияет на производительность.

Как измерить launch time приложения?
?
Cold launch: приложение не в памяти (DYLD_PRINT_STATISTICS, os_signpost). Warm launch: процесс в памяти. Instruments: App Launch template. MetricKit: MXAppLaunchMetric. Цель: < 400ms для cold launch. Оптимизации: lazy initialization, уменьшение linked frameworks, отложенная загрузка ViewControllers.
...
|-------------|------|-------|
| Следующий шаг | [[ios-debugging]] | Отладка найденных performance-проблем |
| Углубиться | [[ios-view-rendering]] | Render loop для оптимизации отрисовки |
| Смежная тема | [[android-performance-profiling]] | Android Profiler для кросс-платформенного сравнения |
