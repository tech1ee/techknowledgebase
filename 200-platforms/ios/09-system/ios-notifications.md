...
**[[ios-background-execution]]** — push-уведомления тесно связаны с фоновым выполнением: silent push (content-available) пробуждает приложение для обновления данных, а background fetch позволяет регулярно обновлять контент. Понимание background modes необходимо для реализации надёжной системы уведомлений, где данные готовы к моменту, когда пользователь открывает уведомление. Рекомендуется изучать обе темы вместе для построения production-ready notification системы.

**[[android-notifications]]** — Android предоставляет более гибкую систему уведомлений (Notification Channels, foreground services, WorkManager), тогда как iOS более ограничена (все идёт через APNs, строгие лимиты background выполнения). Сравнение помогает понять, какие паттерны универсальны (rich media, action buttons, grouping) и какие специфичны для платформы (Live Activities на iOS, ongoing notifications на Android). Полезно для кросс-платформенных команд.

**[[ios-permissions-security]]** — уведомления требуют явного разрешения пользователя через UNUserNotificationCenter.requestAuthorization, а App Tracking Transparency влияет на таргетинг push-уведомлений. Неправильный запрос разрешений (при запуске вместо контекстного) снижает opt-in rate на 40-60%. Понимание системы разрешений iOS критично для максимизации доставки уведомлений.
...
> [!question]- Как APNs (Apple Push Notification service) доставляет push-уведомления, и почему сервер не может гарантировать доставку?
> Путь: Server -> APNs -> Device. Сервер отправляет JSON payload в APNs с device token. APNs хранит только последнее уведомление для оффлайн-устройства (best effort delivery). Потеря: устройство выключено долго, payload > 4KB (отклонен), невалидный token, пользователь отключил уведомления. APNs не подтверждает доставку на устройство серверу.

> [!question]- В чем разница между Local Notification и Remote (Push) Notification, и когда выбирать каждый?
> Local: создается и планируется самим приложением (напоминания, таймеры, геофенсы). Не нужен сервер. Remote: приходит с сервера через APNs (новые сообщения, обновления). Нужен backend + APNs certificate/key. Local для: алармы, напоминания на основе времени/локации. Remote для: чат-сообщения, обновления контента, маркетинг.

> [!question]- Сценарий: вы хотите показать изображение в push-уведомлении и добавить кнопки действий. Какие компоненты нужны?
> Notification Service Extension: перехватывает уведомление, загружает изображение, модифицирует content (mutable-content: 1 в payload). Notification Content Extension: кастомный UI для уведомления. UNNotificationCategory с UNNotificationAction для кнопок. Регистрация категорий при запуске приложения. payload включает category identifier.

---

## Ключевые карточки

Как работает APNs (Apple Push Notification service)?
?
Сервер отправляет HTTP/2 запрос в APNs с device token и JSON payload. APNs маршрутизирует на устройство через persistent connection. Device token уникален для app + device (обновляется). Аутентификация: JWT (p8 key, рекомендуется) или TLS certificate (p12). Payload максимум 4KB.

Что такое UNUserNotificationCenter?
?
Центральный объект для управления уведомлениями (iOS 10+). Запрос разрешений (requestAuthorization), планирование local notifications (add), обработка действий пользователя (delegate), управление доставленными уведомлениями (getDeliveredNotifications, removeDeliveredNotifications).

Что такое Live Activities и Dynamic Island?
?
Live Activities (iOS 16.1+): обновляемый виджет на Lock Screen для real-time информации (доставка, спорт, таймер). Dynamic Island (iPhone 14 Pro+): компактное/расширенное представление Live Activity вокруг выреза камеры. Обновление через ActivityKit (локально) или push через APNs.

Какие типы Notification Extensions существуют?
?
Notification Service Extension: перехватывает remote notification до показа, позволяет загрузить медиа, модифицировать контент, расшифровать payload. Notification Content Extension: кастомный UI для отображения уведомления (3D Touch/long press). Оба -- отдельные targets в Xcode с собственным lifecycle.

Silent Push vs Alert Push -- в чем разница?
?
Alert Push: показывает уведомление пользователю (баннер, звук, badge). Silent Push: content-available: 1, без alert/sound. Пробуждает приложение в фоне для обновления данных (~30 секунд). Система контролирует частоту (throttling). Не гарантирует доставку при low battery.
...
|-------------|------|-------|
| Следующий шаг | [[ios-background-execution]] | Фоновая обработка по push-триггеру |
| Углубиться | [[ios-permissions-security]] | Разрешения на уведомления |
| Смежная тема | [[android-notifications]] | Система уведомлений Android для сравнения |
