...
**[[android-testing]]** — Тестирование в Android и iOS имеет много общих принципов (пирамида тестов, DI для моков, AAA-паттерн), но различается в инструментарии. В iOS используется XCTest с XCTAssert-семейством, в Android — JUnit с Espresso для UI-тестов. Сравнение подходов помогает кроссплатформенным разработчикам быстрее переключаться между экосистемами и переносить best practices: например, mock-injection через протоколы (iOS) аналогичен интерфейсам в Kotlin/Java, а XCUITest сопоставим с Espresso по возможностям автоматизации.

**[[ios-architecture-patterns]]** — Архитектура приложения напрямую определяет его тестируемость. MVVM позволяет тестировать ViewModel изолированно от UI, Clean Architecture обеспечивает тестирование use case-ов без зависимости от фреймворков, а Coordinator-паттерн делает навигацию тестируемой. Без правильной архитектуры (с dependency injection через протоколы) невозможно эффективно мокировать зависимости, что превращает unit-тесты в интеграционные и делает их медленными и хрупкими.
...
> [!question]- Почему Testing Pyramid (много unit, меньше integration, мало UI тестов) важна для iOS, и какие проблемы возникают при инвертировании?
> Unit тесты: быстрые (мс), стабильные, изолированные. UI тесты: медленные (секунды), хрупкие (ломаются при изменении UI), зависят от simulator. Инвертирование (много UI, мало unit): долгий CI (30+ минут), flaky тесты (нестабильные), сложная отладка падений. Правило: 70% unit, 20% integration, 10% UI. Unit для бизнес-логики, UI для критических user flows.

> [!question]- Как dependency injection делает код тестируемым, и что такое протокольные моки в Swift?
> DI позволяет передать mock-зависимость вместо реальной. Протокольный мок: определить protocol NetworkServiceProtocol, ViewModel зависит от протокола. В тестах: создать MockNetworkService, реализующий протокол, с предопределенными ответами. Проверить: правильный URL вызван, UI обновлен, ошибки обработаны. Без DI -- невозможно изолировать тестируемый код.

> [!question]- Сценарий: ваш UI-тест для экрана логина периодически падает на CI, но работает локально. Как сделать его стабильным?
> 1) Использовать accessibilityIdentifier вместо text для поиска элементов (не ломается при локализации). 2) Добавить явные ожидания: XCTAssertTrue(element.waitForExistence(timeout: 5)). 3) Reset app state перед каждым тестом (launchArguments для mock data). 4) Не зависеть от сети: использовать URLProtocol mock. 5) Проверить simulator state (keyboard, orientation). 6) Увеличить timeouts для CI (медленнее локальной машины).

---

## Ключевые карточки

Какие типы тестов поддерживает XCTest?
?
Unit Tests (XCTestCase: тестирование логики), UI Tests (XCUIApplication: автоматизация интерфейса), Performance Tests (measure {}: замер времени выполнения). Также: Swift Testing (новый framework, iOS 16+), snapshot tests (через сторонние библиотеки: SnapshotTesting).

Как тестировать async/await код в XCTest?
?
Тестовый метод помечается async: `func testFetch() async throws`. Внутри: let result = try await viewModel.fetchData(). XCTAssertEqual(result.count, 5). Для timeout: Task с cancellation. Для mock: protocol + mock с async методами. withCheckedContinuation для обратной совместимости с callback API.

Что такое @testable import?
?
Директива, открывающая internal declarations модуля для тестового target. Без нее: доступны только public/open. С ней: тестируются internal классы, методы, свойства. Работает только для Debug builds (не Release). Не открывает private/fileprivate.

Какие инструменты для мокирования есть в iOS?
?
Ручные моки через протоколы (рекомендуется). Swift Testing framework (встроенный). Сторонние: OCMock (ObjC runtime, мощный), Cuckoo (кодогенерация моков), Mockingbird (compile-time). URLProtocol для сетевых моков. UserDefaults(suiteName:) для изолированного storage в тестах.

Что такое Snapshot Testing?
?
Сравнение отрендеренного UI с эталонным скриншотом. Первый запуск: сохраняет reference image. Последующие: сравнивает pixel-by-pixel. Библиотека: PointFree SnapshotTesting. Для: визуальная регрессия, проверка layout на разных размерах экранов. Минус: хрупкие при изменении дизайна.
...
|-------------|------|-------|
| Следующий шаг | [[ios-ci-cd]] | Автоматический запуск тестов в CI pipeline |
| Углубиться | [[ios-architecture-patterns]] | Архитектура влияет на тестируемость |
| Смежная тема | [[android-testing]] | Тестирование Android для кросс-платформенного сравнения |
