...
## Теоретические основы: формальный базис Swift Concurrency

### Structured Concurrency (Smith, 2018)

> **Structured Concurrency** — дисциплина, при которой время жизни конкурентной задачи ограничено лексическим scope родителя. Формальный инвариант: **ни одна дочерняя задача не переживает родительскую**. Введена Nathaniel J. Smith (2018) по аналогии со structured programming Дейкстры (1968).

Swift Task tree реализует structured concurrency: `async let` и `TaskGroup` создают дочерние задачи, которые гарантированно завершаются до выхода из scope. Unstructured tasks (`Task { }`, `Task.detached { }`) — сознательный отказ от этой гарантии.

| Свойство | Structured (async let, TaskGroup) | Unstructured (Task { }) |
|----------|----------------------------------|------------------------|
| Lifetime | Ограничен scope родителя | Не ограничен |
| Cancellation | Пропагируется от parent к children | Ручное управление |
| Error propagation | Автоматическое (throws) | Ручное |
| Resource safety | Гарантирована (no leaks) | На ответственности разработчика |

### Actor Model (Hewitt, Bishop, Steiger, 1973)

> **Actor** — конкурентная сущность, которая: (1) имеет приватное изменяемое состояние, (2) принимает сообщения последовательно, (3) может создавать новых actors, отправлять сообщения, менять своё состояние. Введена Carl Hewitt et al. (1973).

Swift actors — это **restricted actors**: компилятор статически проверяет actor isolation через типовую систему (`Sendable`, `@MainActor`, `nonisolated`). В отличие от Erlang/Akka actors, Swift actors не имеют mailbox и не отправляют сообщения — вместо этого используется `await` для cross-actor calls, что формально является **synchronized access through suspension**.

### Continuation-Passing Style (CPS)

Под капотом Swift async/await использует CPS-трансформацию: каждая `async` функция компилируется в state machine с continuation. Формально:

```
// Высокоуровневый
func f() async -> T

// CPS-трансформация (концептуально)
func f(continuation: (T) -> Void)
```

Swift использует **restricted single-shot continuations**: `CheckedContinuation` и `UnsafeContinuation` для bridging с callback-based API. В отличие от full continuations (Scheme call/cc), Swift continuations можно resume ровно один раз.

> **Связь**: Structured Concurrency → [[concurrency-fundamentals]], Actor Model → [[concurrency-fundamentals]], CPS → [[kotlin-coroutines-internals]]
...
- Smith N.J. (2018). *Notes on structured concurrency, or: Go statement considered harmful.* — Формулировка structured concurrency и аналогия со structured programming Дейкстры.
- Hewitt C., Bishop P., Steiger R. (1973). *A Universal Modular ACTOR Formalism for Artificial Intelligence.* — Определение Actor Model, на которой основаны Swift actors.
- SE-0304 (2021). *Structured Concurrency.* Swift Evolution Proposal. — Формальная спецификация Task tree, cancellation propagation и child-parent lifetime guarantees в Swift.
...
> [!question]- Почему async/await в Swift не создает новый поток при каждом вызове async функции, в отличие от GCD?
> Swift async/await использует cooperative thread pool, ограниченный числом ядер CPU. При suspension point (await) поток освобождается для других задач, а не блокируется. При возобновлении задача может выполниться на другом потоке. Это исключает thread explosion и снижает overhead context switching.

> [!question]- В чем разница между Task, TaskGroup и detached Task, и когда использовать каждый?
> Task -- структурированная задача, наследует контекст (actor, priority) от родителя. TaskGroup -- для параллельного выполнения динамического числа задач с общей обработкой результатов. Detached Task -- неструктурированная, не наследует контекст, используется для фоновых операций, не связанных с текущим scope. Предпочитайте структурированные Task для автоматической отмены.

> [!question]- Сценарий: вы переносите код с completion handlers на async/await. Функция вызывает два API последовательно. Как сделать вызовы параллельными?
> Использовать async let для параллельного выполнения: `async let result1 = api1()`, `async let result2 = api2()`, затем `let (r1, r2) = await (result1, result2)`. Или TaskGroup для динамического числа вызовов. Без async let вызовы с await выполняются последовательно.

> [!question]- Что такое @MainActor и почему он заменяет DispatchQueue.main.async?
> @MainActor -- глобальный actor, гарантирующий выполнение кода на Main Thread на уровне компилятора. В отличие от DispatchQueue.main.async, ошибка обнаруживается при компиляции, а не в runtime. Применяется к классам (UIViewController), методам или свойствам. В Swift 6 компилятор требует явную аннотацию для UI-кода.

---

## Ключевые карточки

Что такое suspension point в Swift async/await?
?
Suspension point (await) -- место, где функция может приостановить выполнение, освободив поток. При возобновлении выполнение может продолжиться на другом потоке. Между suspension points код выполняется атомарно на одном потоке.

Чем structured concurrency отличается от unstructured?
?
Structured: задачи привязаны к scope (TaskGroup, async let), автоматически отменяются при выходе из scope, наследуют контекст. Unstructured (Task {}, Task.detached): живут независимо от scope, требуют ручного управления lifecycle и отменой.

Что такое Actor в Swift и какую проблему решает?
?
Actor -- reference type с изолированным состоянием. Доступ к свойствам/методам actor требует await (кроме nonisolated). Компилятор гарантирует, что только один вызов одновременно изменяет состояние. Решает проблему data races без ручных блокировок.

Что делает протокол Sendable?
?
Sendable гарантирует, что тип безопасен для передачи между потоками/actors. Value types (struct, enum) обычно Sendable автоматически. Reference types должны быть final class без мутабельного состояния или actor. Swift 6 требует Sendable для всех межпоточных передач.

Как работает async let для параллельного выполнения?
?
async let создает child task, которая начинает выполняться немедленно и параллельно. Результат получается через await при обращении к переменной. Если scope завершается раньше -- child task автоматически отменяется. Это structured concurrency для фиксированного числа параллельных задач.
...
|-------------|------|-------|
| Следующий шаг | [[ios-combine]] | Реактивное программирование как дополнение к async/await |
| Углубиться | [[ios-concurrency-mistakes]] | Типичные ошибки при работе с конкурентностью |
| Смежная тема | [[kotlin-coroutines]] | Корутины Kotlin -- аналогичная модель конкурентности |
