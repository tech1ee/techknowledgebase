...
## Теоретические основы: формальный базис GCD

### Thread Pool и оптимальный размер (Amdahl's Law)

> **Thread Pool** — паттерн, при котором фиксированный набор рабочих потоков обрабатывает очередь задач. GCD управляет глобальным thread pool, динамически регулируя количество потоков.

Оптимальный размер пула определяется характером задач:
- **CPU-bound**: `N_threads = N_cores` (закон Амдала — добавление потоков сверх числа ядер не ускоряет вычисления, а добавляет overhead на context switch)
- **I/O-bound**: `N_threads = N_cores × (1 + W/S)`, где W — время ожидания I/O, S — время вычислений (формула Литтла)

GCD ограничивает thread pool cooperative pool width = 64 потока (по умолчанию), чтобы избежать **thread explosion** — ситуации, когда сотни заблокированных потоков потребляют память (1MB stack каждый) без полезной работы.

### Quality of Service как Priority Scheduling

> **QoS (Quality of Service)** в GCD — это реализация **priority scheduling** на уровне ОС. Каждый QoS-класс маппится на приоритеты XNU-ядра Darwin.

| QoS | XNU Priority Band | Семантика | Пример |
|-----|-------------------|-----------|--------|
| `.userInteractive` | 47 (highest) | Непосредственное взаимодействие с пользователем | Анимации, рендеринг |
| `.userInitiated` | 37 | Пользователь ждёт результата | Открытие файла |
| `.default` | 31 | Общая работа | Таймеры |
| `.utility` | 20 | Длительные задачи с прогрессом | Загрузка данных |
| `.background` | 9 (lowest) | Невидимая работа | Синхронизация, бэкапы |

GCD реализует **priority inheritance** (инверсия приоритетов): если высокоприоритетная задача ждёт низкоприоритетную (через DispatchSemaphore/barrier), приоритет низкоприоритетной временно повышается. Это решает проблему **priority inversion** (Mars Pathfinder bug, 1997).

### Producer-Consumer и FIFO-очереди

GCD основан на паттерне **producer-consumer** (Dijkstra, 1965): dispatch queues — это FIFO-буферы между producer'ами (код, вызывающий `async {}`) и consumer'ами (потоки пула). Serial queue гарантирует **mutual exclusion** без explicit locks — это формально эквивалентно monitor'у (Hoare, 1974).

> **Связь**: Thread pools → [[concurrency-fundamentals]], Priority scheduling → [[os-scheduling]], Producer-Consumer → [[concurrency-fundamentals]]
...
- Amdahl G. (1967). *Validity of the single processor approach to achieving large scale computing capabilities.* — Закон Амдала: ограничение ускорения от параллелизма, определяющее оптимальный размер thread pool.
- Hoare C.A.R. (1974). *Monitors: An Operating System Structuring Concept.* — Определение monitor'а; serial dispatch queue — формальный эквивалент monitor.
- Dijkstra E.W. (1965). *Cooperating Sequential Processes.* — Producer-consumer problem; GCD queues решают эту задачу через FIFO-буферы.
...
> [!question]- Почему DispatchQueue.global() может создать больше потоков, чем ядер CPU, и как это контролировать?
> Global concurrent queue не ограничивает число потоков. Если задачи блокируются (I/O, semaphore), GCD создает новые потоки для поддержания concurrency, что ведет к thread explosion. Контроль: использовать OperationQueue с maxConcurrentOperationCount, или перейти на cooperative thread pool (async/await), который ограничен числом ядер.

> [!question]- Как DispatchWorkItem.cancel() работает на самом деле и почему это не мгновенная отмена?
> cancel() устанавливает флаг isCancelled = true, но не прерывает выполняющуюся задачу. Код внутри workItem должен сам проверять isCancelled и выходить. Это cooperative cancellation -- аналогично CancellationToken в других платформах. Задача в очереди, еще не начавшая выполнение, будет пропущена.

> [!question]- Сценарий: вам нужно загрузить 100 изображений параллельно и показать UI только после загрузки всех. Какие примитивы GCD использовать?
> DispatchGroup: enter() перед каждой загрузкой, leave() после завершения, notify(queue: .main) для обновления UI. Альтернативно: concurrent queue + dispatch_barrier для финального действия. Важно ограничить параллелизм через OperationQueue (maxConcurrent = 4-6), чтобы избежать thread explosion.

> [!question]- В чем разница между dispatch_barrier и DispatchSemaphore для синхронизации доступа к shared resource?
> Barrier обеспечивает эксклюзивный доступ в рамках concurrent queue (readers-writer lock): обычные задачи выполняются параллельно, barrier-задача ждет завершения всех и выполняется монопольно. Semaphore -- универсальный примитив для ограничения числа одновременных доступов (count). Barrier эффективнее для read-heavy сценариев, semaphore -- для ограничения concurrency.

---

## Ключевые карточки

Какие типы очередей GCD существуют?
?
Main queue (serial, UI), Global queues (concurrent, 4 уровня QoS), Private queues (serial или concurrent, создаются разработчиком). Main queue привязана к Main Thread, global queues используют shared thread pool.

Что такое DispatchGroup и для чего он нужен?
?
DispatchGroup отслеживает завершение группы асинхронных задач. enter() увеличивает счетчик, leave() уменьшает. Когда счетчик = 0, вызывается notify() или разблокируется wait(). Используется для ожидания нескольких параллельных операций.

Как работает dispatch_barrier?
?
Barrier-задача в concurrent queue ждет завершения всех ранее добавленных задач, выполняется монопольно (эксклюзивно), затем возобновляется параллельное выполнение. Реализует readers-writer lock: чтение параллельно, запись эксклюзивно.

Что такое DispatchSource и когда его использовать?
?
DispatchSource -- механизм для мониторинга системных событий: таймеры (DispatchSourceTimer), файловые изменения (DispatchSourceFileSystemObject), сигналы процесса. Эффективнее, чем polling, потому что использует kernel events (kqueue на macOS/iOS).

Чем опасен вызов semaphore.wait() на main queue?
?
semaphore.wait() блокирует текущий поток. На main queue это заморозит UI. Если сигнал семафора зависит от задачи, которая должна выполниться на main queue -- deadlock. Правило: никогда не блокировать main thread синхронными ожиданиями.

Что такое target queue в GCD?
?
Target queue определяет, где фактически выполняются задачи из custom queue. Позволяет создавать иерархии очередей. Например, несколько serial queues с общим target concurrent queue. Используется для организации приоритетов и ограничения параллелизма.
...
|-------------|------|-------|
| Следующий шаг | [[ios-async-await]] | Современная альтернатива GCD на базе async/await |
| Углубиться | [[ios-concurrency-mistakes]] | Типичные ошибки при работе с GCD |
| Смежная тема | [[android-handler-looper]] | Handler/Looper -- аналог GCD в Android |
