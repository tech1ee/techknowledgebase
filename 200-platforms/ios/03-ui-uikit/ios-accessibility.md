...
> **Определение:** Доступность (accessibility, a11y) — свойство продукта, позволяющее людям с различными формами ограничений (зрение, слух, моторика, когнитивные) полноценно использовать его функциональность. Формализовано в стандарте WCAG (Web Content Accessibility Guidelines, W3C, 1999).

### Нормативная база и стандарты

| Стандарт / Закон | Год | Регион | Требования |
|-----------------|-----|--------|-----------|
| **ADA** (Americans with Disabilities Act) | 1990 | США | Обязательная доступность для digital services |
| **Section 508** | 1998/2017 | США (госсектор) | Соответствие WCAG 2.0 AA |
| **WCAG 2.1** (W3C) | 2018 | Международный | 4 принципа: Perceivable, Operable, Understandable, Robust |
| **European Accessibility Act** | 2019 (вступает 2025) | ЕС | Обязательно для apps и websites |
| **Apple Accessibility Guidelines** | Постоянно | Apple ecosystem | Рекомендации для iOS/macOS/watchOS |

### Четыре принципа WCAG (POUR)

> **POUR Model (W3C, 2008):** Контент должен быть **P**erceivable (воспринимаемым), **O**perable (управляемым), **U**nderstandable (понятным) и **R**obust (устойчивым к изменениям технологий).

| Принцип | Значение | Реализация в iOS |
|---------|----------|-----------------|
| **Perceivable** | Информация доступна через разные каналы | VoiceOver labels, Dynamic Type, цветовой контраст |
| **Operable** | Можно управлять разными способами | Switch Control, Voice Control, AssistiveTouch |
| **Understandable** | Интерфейс предсказуем и понятен | Accessibility hints, consistent navigation |
| **Robust** | Работает с вспомогательными технологиями | Semantic traits, accessibility API |

### Curb-Cut Effect (Blackwell, 2016)

> **Curb-Cut Effect** — явление, при котором технологии, разработанные для людей с ограничениями, приносят пользу всем пользователям. Термин происходит от скошенных бордюров (curb cuts), установленных для инвалидных колясок, но используемых велосипедистами, родителями с колясками и путешественниками.

Примеры в iOS: Dynamic Type полезен всем при чтении в транспорте; VoiceOver используется зрячими водителями; Reduce Motion помогает при укачивании.

### Связь с CS-фундаментом

- [[ios-uikit-fundamentals]] — UIView accessibility API как расширение UIKit
- [[ios-swiftui]] — декларативный accessibility в SwiftUI
- [[ios-testing]] — accessibility audit и автоматизированное тестирование a11y
...
**[[ios-swiftui]]** — SwiftUI предоставляет встроенные модификаторы доступности (.accessibilityLabel, .accessibilityHint, .accessibilityElement), которые делают создание accessible UI значительно проще, чем в UIKit. Понимание SwiftUI accessibility API — обязательное условие для создания доступных современных iOS-приложений. Рекомендуется сначала изучить базовые концепции accessibility, затем перейти к SwiftUI-специфичным модификаторам.

**[[ios-swiftui-vs-uikit]]** — UIKit и SwiftUI реализуют accessibility по-разному: UIKit требует явной настройки свойств (isAccessibilityElement, accessibilityLabel, accessibilityTraits), тогда как SwiftUI автоматически генерирует accessibility tree на основе view hierarchy. Сравнение подходов помогает выбрать оптимальную стратегию для конкретного проекта и понять, как мигрировать accessibility-код между фреймворками.

**[[ios-testing]]** — автоматизированное тестирование accessibility через XCTest UI позволяет проверять наличие labels, traits и корректный порядок навигации VoiceOver. Accessibility тесты являются частью общей стратегии тестирования и помогают предотвратить регрессии при изменениях UI. Начните с accessibility-чеклиста, затем автоматизируйте проверки через XCUITest.
...
### Теоретические основы
- W3C (2018). *Web Content Accessibility Guidelines (WCAG) 2.1.* — международный стандарт доступности
- Blackwell A. G. (2016). *The Curb-Cut Effect.* Stanford Social Innovation Review — теория универсальной пользы от accessibility
- Shneiderman B. (2000). *Universal Usability.* Communications of the ACM — принципы универсального дизайна

### Практические руководства
- Neuburg M. (2023). *iOS 17 Programming Fundamentals with Swift.* — accessibility API в UIKit и SwiftUI
- [Apple Accessibility Programming Guide](https://developer.apple.com/accessibility/) — официальная документация
...
> [!question]- Почему accessibility важна не только для пользователей с ограничениями, но и для бизнеса?
> 1) Юридические требования: ADA compliance в US, European Accessibility Act в EU. 2) 15-20% населения имеют disability. 3) App Store review может отклонить app без accessibility. 4) Accessibility улучшает UX для всех: Dynamic Type полезен всем, VoiceOver используют и зрячие (за рулём).

> [!question]- В чём разница между accessibilityLabel и accessibilityHint?
> accessibilityLabel -- краткое описание элемента ("Кнопка отправить"). accessibilityHint -- описание результата действия ("Двойное нажатие отправит сообщение"). VoiceOver читает label сразу, hint -- с задержкой. Label обязателен, hint -- опционален и не должен дублировать label.

> [!question]- Как протестировать accessibility приложения без физического устройства?
> 1) Accessibility Inspector в Xcode (Xcode -> Open Developer Tool). 2) VoiceOver в Simulator (Settings -> Accessibility). 3) Accessibility audit в Xcode (Debug -> Accessibility -> Run Audit). 4) XCTest: XCUIElement.isAccessibilityElement, .label, .traits для автоматизации.

---

## Ключевые карточки

Что такое VoiceOver и как он работает?
?
Screen reader от Apple, читающий содержимое экрана вслух. Пользователь навигирует свайпами (следующий/предыдущий элемент) и двойным тапом (активация). Разработчик должен обеспечить: accessibilityLabel, accessibilityTraits, правильный порядок фокуса.

Что такое Dynamic Type?
?
Система масштабирования шрифтов в iOS. Пользователь выбирает размер текста в Settings. Приложение должно использовать UIFont.preferredFont(forTextStyle:) или .font(.body) в SwiftUI. adjustsFontForContentSizeCategory = true для автоматического масштабирования.

Какие accessibilityTraits существуют?
?
.button (кнопка), .link (ссылка), .header (заголовок секции), .image (изображение), .selected (выбрано), .staticText (текст), .adjustable (слайдер), .notEnabled (отключено). Traits помогают VoiceOver объяснить тип и состояние элемента.

Как сделать custom view доступным?
?
isAccessibilityElement = true (для листовых элементов). Установить accessibilityLabel, accessibilityTraits. Для группы -- accessibilityElements массив в правильном порядке. Для custom actions -- accessibilityCustomActions. В SwiftUI: .accessibilityLabel(), .accessibilityAddTraits().

Что такое Accessibility Audit в Xcode?
?
Инструмент автоматической проверки accessibility (Debug -> Accessibility -> Run Audit). Находит: отсутствующие labels, слишком маленькие tap targets (<44pt), недостаточный контраст, отсутствующие traits. Также доступен через XCTest framework.
...
|-------------|------|-------|
| Следующий шаг | [[ios-testing]] | Автоматизация accessibility тестирования |
| Углубиться | [[ios-custom-views]] | Создание accessible custom компонентов |
| Смежная тема | [[cross-ui-declarative]] | Accessibility в кросс-платформенных фреймворках |
| Обзор | [[ios-overview]] | Вернуться к карте раздела |
- Keur C., Hillegass A. (2020). *iOS Programming: The Big Nerd Ranch Guide, 7th Edition.* — практические примеры создания доступных интерфейсов с пошаговыми упражнениями
- Eidhof C. et al. (2020). *Thinking in SwiftUI.* — раскрывает ментальную модель accessibility tree в SwiftUI и объясняет, как декларативный подход упрощает доступность
