...
- Neuburg (2021). *iOS Programming Fundamentals with Swift*. --- систематическое изложение UIKit от view hierarchy до Auto Layout с акцентом на "почему так устроено". Лучшая книга для понимания архитектуры UIKit и взаимосвязей между UIView, UIViewController, responder chain. Объясняет не только API, но и дизайнерские решения Apple.
- Apple Documentation: [UIKit Framework Reference](https://developer.apple.com/documentation/uikit). --- первоисточник по всем классам UIKit, включая UIView, UIStackView, Auto Layout guides. Содержит WWDC-сессии с визуализациями drawing cycle и hit-testing, которые невозможно найти в сторонних источниках. Обязательно для глубокого понимания layout engine.
- Sadun (2016). *Core iOS Developer's Cookbook*. --- практические рецепты для UIKit: custom views, gesture recognizers, scroll views, table views с пошаговыми реализациями. Хотя написана для более ранних версий iOS, фундаментальные паттерны UIKit (responder chain, drawing cycle, coordinate systems) не изменились и остаются актуальными.
