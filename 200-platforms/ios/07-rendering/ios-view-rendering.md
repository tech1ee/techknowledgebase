...
> [!question]- Почему Render Server (backboardd) работает в отдельном процессе, и что произойдет, если ваше приложение зависнет?
> Render Server -- системный процесс, композирующий слои всех приложений на GPU. Отдельный процесс обеспечивает: стабильность (зависшее приложение не замораживает весь экран), безопасность (приложение не имеет прямого доступа к framebuffer), эффективность (анимации продолжаются даже если main thread занят). При зависании app -- последний закоммиченный кадр остается на экране.

> [!question]- Из каких фаз состоит Render Loop и что происходит в каждой?
> 1) Layout: вычисление размеров и позиций (layoutSubviews, Auto Layout constraints). 2) Display: растеризация содержимого (draw(_:rect:), Core Graphics в backing store). 3) Commit: CATransaction пакует layer tree и отправляет в Render Server через IPC. Все три фазы должны уложиться в бюджет кадра (16.67ms при 60fps).

> [!question]- Сценарий: в вашем приложении UITableView скроллится с 45fps вместо 60fps. Instruments показывает "Time Profiler: layoutSubviews 12ms". Как оптимизировать?
> 12ms на layout -- слишком много. Решения: 1) Кэшировать высоты ячеек (предрассчитать, не использовать UITableView.automaticDimension). 2) Упростить constraint hierarchy (меньше вложенных StackView). 3) Использовать frame-based layout для сложных ячеек. 4) Избегать layoutIfNeeded() в scrollViewDidScroll. 5) Prefetch данные через UITableViewDataSourcePrefetching.

---

## Ключевые карточки

Что такое Render Loop в iOS?
?
Цикл рендеринга, синхронизированный с VSync дисплея: Layout (вычисление размеров) -> Display (растеризация) -> Commit (отправка в Render Server) -> GPU Compositing (композиция слоев). Бюджет: 16.67ms при 60fps, 8.33ms при 120fps ProMotion.

Что такое backing store в контексте CALayer?
?
Bitmap-буфер в памяти, хранящий растеризованное содержимое слоя. Создается при первом draw(_:rect:) или при установке contents (UIImage). Потребляет width * height * 4 байт (RGBA). Не создается для слоев без содержимого (только backgroundColor/borderColor).

Что такое off-screen rendering и как его обнаружить?
?
GPU создает временный буфер для промежуточного результата (маски, тени, clipsToBounds + cornerRadius). Обнаружение: Simulator -> Debug -> Color Off-screen Rendered (желтый цвет). Instruments -> Core Animation -> offscreen renders. Решение: shouldRasterize, предварительная обрезка изображений.

Как CATransaction управляет обновлениями слоев?
?
CATransaction группирует изменения свойств CALayer в atomic commit. Implicit transaction создается автоматически в текущем run loop iteration. Explicit: CATransaction.begin()/commit(). Все изменения применяются одновременно в commit phase, не по одному.

Что такое setNeedsLayout vs layoutIfNeeded?
?
setNeedsLayout -- маркирует view для пересчета layout в следующем render loop (асинхронно, batched). layoutIfNeeded -- принудительно выполняет layout немедленно (синхронно). Для анимаций: изменить constraints, затем layoutIfNeeded внутри UIView.animate. Избегать layoutIfNeeded в scrollViewDidScroll.
...
|-------------|------|-------|
| Следующий шаг | [[ios-scroll-performance]] | Оптимизация scroll performance на практике |
| Углубиться | [[ios-graphics-fundamentals]] | Графический стек: Core Graphics, Core Animation, Metal |
| Смежная тема | [[android-view-rendering-pipeline]] | Render pipeline Android для сравнения |
