...
- **[[android-viewmodel-internals]]** — Android ViewModel привязан к lifecycle через ViewModelStore и переживает configuration changes (поворот экрана), тогда как iOS ViewModel — обычный объект без встроенной lifecycle-привязки. Сравнение двух подходов показывает, как @StateObject в SwiftUI решает аналогичную проблему сохранения состояния. Понимание Android-паттернов полезно при проектировании кросс-платформенной архитектуры.

- **[[ios-state-management]]** — Управление состоянием — ключевая ответственность ViewModel: @Published свойства автоматически уведомляют View об изменениях через Combine, @Observable (iOS 17+) обеспечивает гранулярные обновления на уровне отдельных свойств, а @State/@Binding обрабатывают локальное состояние View. Правильный выбор механизма state management определяет производительность и тестируемость приложения.

- **[[ios-architecture-patterns]]** — ViewModel является центральным элементом MVVM-архитектуры, но также используется в вариациях MVI, Clean Architecture и VIPER. Понимание границ ответственности ViewModel (подготовка данных для отображения, обработка пользовательского ввода, координация с сервисами) критично для предотвращения Massive ViewModel антипаттерна и обеспечения тестируемости.
...
> [!question]- Почему @StateObject и @ObservedObject ведут себя по-разному при пересоздании View, и когда использовать каждый?
> @StateObject создает и владеет ViewModel (сохраняется при пересоздании View body). @ObservedObject не владеет -- при пересоздании parent View создается новый экземпляр, теряя состояние. Правило: @StateObject для создания ViewModel (обычно в parent View), @ObservedObject для передачи уже созданного ViewModel в child View.

> [!question]- В чем преимущество Input-Output ViewModel паттерна перед классическим @Published подходом?
> Input-Output явно разделяет: Input (enum с действиями пользователя: .tapButton, .textChanged) и Output (struct с состоянием: isLoading, items, errorMessage). Преимущества: все возможные действия видны в одном месте, проще тестировать (отправить Input, проверить Output), нет неявных side-effects. Классический @Published менее структурирован.

> [!question]- Сценарий: ваш ViewModel для экрана профиля содержит 15 @Published свойств, 10 методов и 200 строк. Как рефакторить?
> 1) Разбить на sub-ViewModels: ProfileHeaderViewModel, ProfileSettingsViewModel, ProfileStatsViewModel. 2) Использовать enum State { case loading, loaded(Profile), error(Error) } вместо множества @Published. 3) Вынести бизнес-логику в UseCase/Interactor. 4) С iOS 17+ перейти на @Observable (убирает @Published boilerplate).

---

## Ключевые карточки

Чем @Observable (iOS 17) отличается от ObservableObject?
?
@Observable -- macro, автоматически отслеживает доступ к свойствам. Не нужен @Published. Granular observation: View обновляется только при изменении свойств, которые она читает. ObservableObject уведомляет всех подписчиков при любом @Published изменении (coarse-grained).

Что такое Input-Output ViewModel паттерн?
?
ViewModel определяет enum Input (действия: .viewDidLoad, .tapRetry, .searchTextChanged(String)) и struct Output (состояние: isLoading, items, error). Метод transform(input:) -> Output маппит действия в состояние. Четкий контракт, легко тестировать.

Зачем нужен protocol-based ViewModel?
?
Протокол определяет интерфейс ViewModel (свойства, методы). View зависит от протокола, не от конкретного класса. Позволяет: подставить MockViewModel в тестах и preview, заменить реализацию без изменения View, соблюдать Dependency Inversion Principle (SOLID).

Какие ошибки типичны при проектировании ViewModel?
?
God ViewModel (слишком много ответственности), прямой доступ к UI-фреймворку (import UIKit в ViewModel), business logic в ViewModel (вместо UseCase), отсутствие error handling, retain cycles (self в closure без weak), мутабельное состояние без синхронизации.

Как правильно управлять жизненным циклом ViewModel в SwiftUI?
?
@StateObject в parent View (создание и владение). @ObservedObject в child View (получение извне). @EnvironmentObject для shared state (передача через иерархию). С iOS 17: @State + @Observable (проще, нет разницы StateObject/ObservedObject). ViewModel живет пока жив его owner View.
...
|-------------|------|-------|
| Следующий шаг | [[ios-state-management]] | Управление состоянием -- ключевая роль ViewModel |
| Углубиться | [[ios-architecture-patterns]] | ViewModel в контексте разных архитектур |
| Смежная тема | [[android-viewmodel-internals]] | Android ViewModel для кросс-платформенного сравнения |
