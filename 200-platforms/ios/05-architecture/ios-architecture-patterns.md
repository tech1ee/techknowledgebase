...
**[[android-architecture-patterns]]** — сравнение архитектурных паттернов iOS (MVC, MVVM, VIPER, TCA) с Android-подходами (MVI, MVVM, Clean Architecture) демонстрирует общие принципы разделения ответственности, но с разными реализациями из-за особенностей платформ. На iOS MVVM стал стандартом благодаря SwiftUI + Combine, тогда как на Android MVI набирает популярность благодаря Jetpack Compose. Параллельное изучение углубляет понимание архитектурных решений и помогает на кросс-платформенных собеседованиях.

**[[ios-state-management]]** — управление состоянием является ключевым аспектом любого архитектурного паттерна: в MVC state разбросан по контроллерам, в MVVM инкапсулирован в ViewModel через @Published, а в TCA централизован в Store. Понимание паттернов управления состоянием (@State, @StateObject, @EnvironmentObject) необходимо для осознанного выбора архитектуры. Рекомендуется изучить архитектурные паттерны для формирования концептуального понимания, затем углубиться в конкретные механизмы state management.

**[[ios-combine]]** — Combine является реактивным движком для MVVM-архитектуры на iOS, обеспечивая data binding между View и ViewModel через @Published и Publishers. Без понимания Combine невозможно эффективно реализовать MVVM с UIKit, хотя SwiftUI абстрагирует часть этой работы. Рекомендуется изучить Combine после освоения базовых архитектурных паттернов, чтобы понять, как реактивность решает проблему синхронизации состояния.

**[[ios-testing]]** — тестируемость является главным критерием выбора архитектуры: MVC практически не тестируем из-за tight coupling с UIKit, MVVM позволяет unit-тестировать ViewModel изолированно, а VIPER и TCA предоставляют полную изоляцию каждого компонента. Изучение тестирования после архитектуры позволяет на практике оценить преимущества разделения ответственности.
...
> [!question]- Почему Apple рекомендовала MVC, но разработчики массово перешли на MVVM для SwiftUI-проектов?
> MVC в iOS приводит к Massive View Controller: UIViewController совмещает роли View и Controller, накапливая логику навигации, форматирования данных и обработки событий. MVVM разделяет эти обязанности: ViewModel берет на себя логику трансформации данных и состояние, а SwiftUI View становится чисто декларативным описанием UI. @Published и Combine обеспечивают нативную реактивную привязку.

> [!question]- В чем ключевое отличие VIPER от Clean Architecture, и когда VIPER избыточен?
> VIPER -- конкретная реализация Clean Architecture для iOS с фиксированными ролями (View, Interactor, Presenter, Entity, Router). Clean Architecture -- абстрактный принцип (Dependency Rule, слои). VIPER избыточен для маленьких экранов и прототипов: 5+ файлов на модуль создают boilerplate без пропорциональной выгоды. Подходит для крупных команд с четкими зонами ответственности.

> [!question]- Сценарий: ваша команда начинает новый SwiftUI-проект (iOS 17+). Какой архитектурный паттерн выбрать и почему?
> MVVM с @Observable macro (iOS 17+). Причины: нативная поддержка SwiftUI (автоматическое отслеживание изменений без @Published), минимальный boilerplate, хорошая тестируемость через protocol-based ViewModels. Для сложных фич -- TCA (The Composable Architecture) для предсказуемого управления состоянием. Для простых экранов достаточно @State без ViewModel.

> [!question]- Что такое Dependency Rule в Clean Architecture и почему внутренние слои не должны знать о внешних?
> Dependency Rule: зависимости направлены только внутрь (UI -> Presentation -> Domain -> Entity). Внутренние слои (Domain) не импортируют внешние (UI, Network). Это обеспечивает: тестируемость (Domain тестируется без UI/сети), заменяемость (смена БД не затрагивает бизнес-логику), переиспользование (Domain работает на iOS, macOS, watchOS).

---

## Ключевые карточки

Что такое Massive View Controller и как его избежать?
?
Антипаттерн MVC, где UIViewController содержит UI-логику, бизнес-логику, навигацию и форматирование. Решение: вынести логику в ViewModel (MVVM), Presenter (MVP), Interactor (VIPER). В SwiftUI проблема меньше благодаря декларативному UI.

Какие 5 компонентов составляют VIPER?
?
View (отображение), Interactor (бизнес-логика), Presenter (форматирование для View), Entity (модели данных), Router (навигация). Каждый компонент имеет протокол-интерфейс. Presenter связывает View и Interactor, Router управляет переходами.

Чем TCA отличается от MVVM?
?
TCA (The Composable Architecture) использует однонаправленный поток данных: State (состояние), Action (события), Reducer (обработка), Effect (сайд-эффекты). MVVM двунаправлен (View <-> ViewModel). TCA предсказуемее, лучше тестируется, но имеет больший порог входа и boilerplate.

Что такое @Observable macro в iOS 17+ и чем он лучше ObservableObject?
?
@Observable -- macro, автоматически генерирующий отслеживание изменений свойств. Не требует @Published для каждого свойства. SwiftUI обновляет только View, использующие конкретное измененное свойство (granular observation), а не все View подписчики, как с ObservableObject.

Какой паттерн выбрать для iOS в 2026?
?
Простые экраны: @State + View. Средние: MVVM с @Observable. Сложные фичи: TCA или Clean Architecture. Legacy UIKit: MVP или MVVM с Combine. Критерии: размер команды, требования к тестируемости, минимальная iOS версия, сложность навигации и состояния.
...
|-------------|------|-------|
| Следующий шаг | [[ios-viewmodel-patterns]] | Детальные паттерны ViewModel для SwiftUI и UIKit |
| Углубиться | [[ios-modularization]] | Модульная архитектура для масштабных проектов |
| Смежная тема | [[android-architecture-patterns]] | Сравнить архитектурные подходы Android и iOS |
