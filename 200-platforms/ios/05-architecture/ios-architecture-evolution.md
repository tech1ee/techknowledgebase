...
### [[android-architecture-evolution]]
Сравнение эволюции архитектуры iOS и Android показывает параллельные пути решения одних и тех же проблем. Android прошёл от Activity-based MVC через MVP/MVVM к Jetpack Compose с unidirectional data flow — аналогично iOS-пути от Massive ViewController через MVVM к SwiftUI. Понимание обоих платформ позволяет увидеть, что проблемы (separation of concerns, state management, testability) универсальны, а различаются лишь конкретные API и инструменты.

### [[ios-architecture-patterns]]
Этот файл описывает историческое *почему*, а ios-architecture-patterns — практическое *как*. Знание эволюции объясняет, почему MVVM стал де-факто стандартом (реакция на Massive VC), почему VIPER потерял популярность (over-engineering для большинства проектов), и почему @Observable + SwiftUI — логичное следствие стремления к декларативности. Без исторического контекста выбор архитектуры превращается в слепое следование трендам.
...
> [!question]- Почему Massive View Controller стал главной проблемой Apple MVC и какие архитектурные решения появились в ответ?
> Apple MVC объединял Controller и View в UIViewController, который накапливал логику: форматирование, навигация, сетевые вызовы. Ответные решения: MVP (отделение Presenter), MVVM (ViewModel с реактивным binding), VIPER (полное разделение на 5 компонентов). Каждое решение выносило часть ответственности из ViewController.

> [!question]- Как появление SwiftUI в 2019 году изменило подходы к архитектуре iOS приложений?
> SwiftUI сделал MVVM естественным выбором: декларативный UI + @Published + Combine обеспечили нативный data binding. Отпала необходимость в сложных биндинг-фреймворках (RxSwift). Появился TCA для функционального подхода. @Observable (iOS 17) еще больше упростил MVVM, убрав необходимость в @Published.

> [!question]- Сценарий: вы поддерживаете приложение, начатое в 2016 году на MVC + UIKit. Как планировать миграцию архитектуры?
> Поэтапно: 1) Новые экраны на SwiftUI + MVVM, интеграция через UIHostingController. 2) Выносить логику из существующих ViewController в ViewModel/Presenter. 3) Общую логику выделять в модули (SPM). 4) Не переписывать работающий UIKit-код без бизнес-необходимости. Полная миграция на SwiftUI -- годы, гибридный подход -- реальность.

---

## Ключевые карточки

Какие основные вехи эволюции архитектуры iOS?
?
2008: Apple MVC. 2012-14: MVP, MVVM (ReactiveCocoa). 2015: VIPER, Clean Architecture. 2017: Coordinator Pattern. 2019: SwiftUI + MVVM. 2021: async/await + actors. 2023: @Observable. 2024: TCA 1.0, Swift 6 strict concurrency.

Что такое Coordinator Pattern и какую проблему он решает?
?
Coordinator выносит навигацию из ViewController/View в отдельный объект. Решает проблему связности (coupling): экраны не знают друг о друге, навигацией управляет Coordinator. Особенно полезен для UIKit; в SwiftUI частично заменяется NavigationStack.

Почему @Observable (iOS 17) считается революцией для MVVM?
?
@Observable автоматически отслеживает обращения к свойствам (granular observation). Не нужен @Published для каждого поля. SwiftUI обновляет только View, читающие конкретное свойство, а не все подписчики. Меньше boilerplate, лучше производительность, проще код.
...
|-------------|------|-------|
| Следующий шаг | [[ios-architecture-patterns]] | Детальный разбор каждого архитектурного паттерна |
| Углубиться | [[ios-dependency-injection]] | DI как ключевой элемент любой архитектуры |
| Смежная тема | [[android-architecture]] | Эволюция архитектуры Android для сравнения |
