...
> [!question]- Почему iOS-приложение не может прочитать файлы другого приложения, даже зная точный путь?
> Из-за sandbox isolation. Каждое приложение работает в изолированной среде на уровне ядра. Kernel запрещает доступ к чужим директориям. Единственный способ обмена данными -- через App Groups с shared container (требует entitlement) или через system APIs (UIDocumentPickerViewController).

> [!question]- Вы заметили, что приложение работает медленно при обработке изображений. На каком слое архитектуры искать решение и почему?
> Нужно спуститься на уровень Media Layer или Core OS. Core Image использует GPU через Metal для аппаратного ускорения. Core Graphics работает на CPU. Для максимальной производительности можно использовать Accelerate framework (SIMD операции) или напрямую Metal. Чем ниже слой, тем больше контроля и производительности, но сложнее код.

> [!question]- Почему Swift автоматически bridge-ит Core Foundation types, но иногда нужен takeRetainedValue()?
> Swift автоматически управляет большинством CF types через toll-free bridging. Однако при работе с unmanaged types (C APIs, которые возвращают Unmanaged<T>), Swift не знает ownership семантику. takeRetainedValue() означает "я владею объектом, ARC должен его освободить", а takeUnretainedValue() -- "я не владею, не нужно release".

---

## Ключевые карточки

Из каких четырёх слоёв состоит iOS?
?
Core OS (Security, Accelerate, Bluetooth) -> Core Services (Foundation, Core Data, CloudKit) -> Media (Core Animation, AVFoundation, Metal) -> Cocoa Touch (UIKit, SwiftUI, MapKit). Чем выше слой, тем удобнее API, но дальше от hardware.

Что такое XNU и из чего он состоит?
?
XNU (X is Not Unix) -- гибридное ядро iOS/macOS. Состоит из Mach (микроядро: tasks, threads, ports, VM), BSD (Unix APIs: VFS, sockets, processes) и IOKit (объектно-ориентированные драйверы устройств).

Что такое sandbox в iOS и как он работает?
?
Каждое приложение изолировано: имеет доступ только к своим директориям (Documents/, Library/, tmp/), системные frameworks доступны read-only, а доступ к hardware и system services контролируется через entitlements.

Что такое entitlements и зачем они нужны?
?
Entitlements -- пропуска (capabilities), дающие приложению доступ к системным функциям: push notifications, iCloud, HealthKit, Keychain sharing. Без entitlement код работает в simulator, но падает на устройстве.

В чём разница между Foundation и Core Foundation?
?
Core Foundation -- низкоуровневое C API (CFString, CFArray). Foundation -- высокоуровневое ObjC/Swift API (String, Array). Swift автоматически bridge-ит между ними. Используй Foundation если не нужен специфичный C API.
...
|-------------|------|-------|
| Следующий шаг | [[ios-app-components]] | Понять компоненты приложения поверх архитектуры |
| Углубиться | [[ios-process-memory]] | Разобрать XNU Mach VM subsystem и управление памятью |
| Смежная тема | [[android-architecture]] | Сравнить Darwin/XNU с Linux kernel и Android Runtime |
