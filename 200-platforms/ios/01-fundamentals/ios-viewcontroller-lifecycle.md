...
> **Определение:** Жизненный цикл объекта (object lifecycle) — это последовательность состояний и переходов, которые объект проходит от создания до уничтожения. UIViewController lifecycle является реализацией паттерна Template Method (GoF, 1994), где суперкласс определяет скелет алгоритма, а подклассы переопределяют конкретные шаги.

### Формальная модель lifecycle как конечного автомата

UIViewController lifecycle формально описывается как детерминированный конечный автомат (DFA) согласно теории Hopcroft & Ullman (1979):

| Состояние | Инвариант | Доступные операции |
|-----------|-----------|-------------------|
| Initialized | view == nil, parent == nil | Настройка свойств, dependency injection |
| View Loaded | view != nil, не в иерархии | Конфигурация subviews, constraints |
| Appearing | view в иерархии, ещё не видим | Обновление данных, подписки |
| Visible | view на экране, получает события | Полная интерактивность |
| Disappearing | view уходит с экрана | Сохранение состояния, отмена анимаций |
| Disappeared | view не виден, но в памяти | Освобождение тяжёлых ресурсов |

> **Template Method (GoF, 1994):** «Определить скелет алгоритма в операции, делегируя определение некоторых шагов подклассам. Template Method позволяет подклассам переопределять шаги алгоритма, не изменяя его структуру.» В UIKit суперкласс UIViewController вызывает viewDidLoad(), viewWillAppear(_:) и другие методы в фиксированном порядке.

### Историческая эволюция

| Версия iOS | Изменения lifecycle | Причина |
|------------|-------------------|---------|
| iPhone OS 2.0 (2008) | Базовый lifecycle: loadView → viewDidLoad → appear/disappear | Первая публичная версия SDK |
| iOS 3.0 (2009) | viewDidUnload() для освобождения view при memory warning | Устройства с 128 MB RAM |
| iOS 5.0 (2011) | Appearance containment API (addChildViewController) | Поддержка container VC |
| iOS 6.0 (2012) | viewDidUnload() deprecated | Устройства с >= 512 MB RAM |
| iOS 8.0 (2014) | Adaptive layout, viewWillTransition(to:with:) | Унифицированные storyboards |
| iOS 13.0 (2019) | Scene-based lifecycle влияет на VC lifecycle | Multi-window на iPad |

### Сравнение с Android Activity Lifecycle

| Аспект | UIViewController (iOS) | Activity (Android) |
|--------|----------------------|-------------------|
| Паттерн | Template Method | Template Method + Callback |
| Восстановление | Нет автоматического state restoration | onSaveInstanceState / Bundle |
| Уничтожение | Только при dealloc (ARC) | Может быть уничтожена ОС при rotation |
| Вложенность | Child VC через containment API | Fragment с собственным lifecycle |
| Формализм | Линейная последовательность | Граф состояний с обратными переходами |

### Связь с CS-фундаментом

- [[state-machines]] — формальная модель конечных автоматов, применимая к lifecycle
- [[object-lifecycle]] — общие паттерны управления жизненным циклом объектов
- [[event-driven-programming]] — событийная модель обработки lifecycle callbacks
...
### Теоретические основы
- Gamma E. et al. (1994). *Design Patterns: Elements of Reusable Object-Oriented Software.* Addison-Wesley — паттерн Template Method
- Hopcroft J. E., Ullman J. D. (1979). *Introduction to Automata Theory, Languages, and Computation.* — формальная модель конечных автоматов
- Harel D. (1987). *Statecharts: A Visual Formalism for Complex Systems.* — иерархические состояния и переходы
- Meyer B. (1997). *Object-Oriented Software Construction, 2nd ed.* — инварианты объектов и контрактное программирование
...
- WWDC 2011: "Introduction to View Controllers" — классическое введение
- WWDC 2019: [Optimizing Your App's Performance](https://developer.apple.com/videos/play/wwdc2019/417/)
- Neuburg M. (2023). *iOS 17 Programming Fundamentals with Swift.* — lifecycle с диаграммами
- Keur C., Hillegass A. (2020). *iOS Programming: The Big Nerd Ranch Guide.* — практические проекты с lifecycle
- [Hacking with Swift: UIViewController Lifecycle](https://www.hackingwithswift.com/example-code/uikit/what-is-the-uiviewcontroller-lifecycle) — Paul Hudson

---

## Связь с другими темами

- **[[ios-view-hierarchy]]** — Иерархия view напрямую связана с lifecycle: loadView() создаёт корневой view, viewDidLoad() подключает subviews, а viewDidLayoutSubviews() финализирует их размеры. Понимание responder chain и view tree необходимо для правильного управления событиями на каждом этапе lifecycle. Ошибки в иерархии view часто проявляются именно через нарушения lifecycle.

- **[[ios-memory-management]]** — ARC, retain cycles и weak/unowned ссылки критически важны в контексте lifecycle: closures в viewDidLoad без [weak self] создают утечки, таймеры без invalidate в viewDidDisappear удерживают view controller в памяти, а отсутствие вызова deinit — главный индикатор retain cycle. Правильная работа с памятью на каждом этапе lifecycle предотвращает до 85% утечек.

- **[[swiftui-view-lifecycle]]** — SwiftUI использует принципиально другую модель: onAppear/onDisappear вместо viewWillAppear/viewDidDisappear, .task для автоматической отмены async-операций, и декларативное управление состоянием через @State/@StateObject. Сравнение двух подходов помогает выбрать правильные паттерны при работе с гибридными UIKit+SwiftUI приложениями.

- **[[android-activity-lifecycle]]** — Android Activity lifecycle (onCreate, onStart, onResume, onPause, onStop, onDestroy) имеет прямые аналогии с UIViewController, но с важными отличиями: Android может уничтожить Activity при нехватке памяти, требуя сохранения состояния в onSaveInstanceState. Понимание обеих платформ углубляет знание архитектурных решений в mobile lifecycle management.

- **[[ios-navigation-patterns]]** — Navigation controller и tab bar controller имеют собственные lifecycle-паттерны: push вызывает viewWillDisappear у текущего VC, pop вызывает viewWillAppear у предыдущего. Presentation style (.fullScreen vs .pageSheet) влияет на то, получит ли presenting VC события disappear. Эти нюансы критичны для корректного обновления данных при навигации.

- **[[ios-container-view-controllers]]** — Контейнерные view controllers (addChild, willMove, didMove) расширяют lifecycle дочерними событиями: правильный порядок вызовов addChild → addSubview → didMove(toParent:) гарантирует корректную передачу lifecycle-событий от родителя к дочернему VC. Нарушение этого порядка — частая причина багов с appear/disappear.

---

## Источники и дальнейшее чтение

- Neuburg M. (2023). *iOS 17 Programming Fundamentals with Swift.* — Детально разбирает каждый метод UIViewController lifecycle с диаграммами, объясняет порядок вызовов при различных сценариях навигации и presentation styles, включая особенности iOS 15+ sheet presentation.

- Keur C., Hillegass A. (2020). *iOS Programming: The Big Nerd Ranch Guide.* — Пошаговые проекты с практическим применением lifecycle-методов, включая работу с NotificationCenter, таймерами и правильной очисткой ресурсов при переходах между экранами.

- Eidhof C., Javier N., Willeke F. (2020). *Thinking in SwiftUI.* — Сравнение императивного UIViewController lifecycle с декларативным подходом SwiftUI, что помогает понять принципиальные различия и правильно мигрировать lifecycle-логику в современные SwiftUI-приложения.
...
Какие методы lifecycle вызываются ровно один раз за lifetime VC?
?
init -> loadView -> viewDidLoad -> deinit. Все остальные (viewWillAppear, viewDidAppear, viewWillDisappear, viewDidDisappear, viewDidLayoutSubviews) могут вызываться многократно при навигации и изменениях layout.

Почему нельзя обращаться к view в init?
?
В init view ещё не загружен (nil). IBOutlets не подключены. Обращение к self.view в init вызовет loadView преждевременно или приведёт к крашу. Настройка UI должна быть в viewDidLoad.

Почему размеры view неточны в viewDidLoad?
?
В viewDidLoad view только что создан, Auto Layout ещё не применён, safe area insets не учтены. Размеры могут быть из storyboard (iPhone 8), а устройство -- iPhone 15 Pro Max. Точные размеры доступны в viewDidLayoutSubviews.

Как правильно работать с таймерами в lifecycle?
?
Создавать в viewWillAppear/viewDidAppear с [weak self]. Invalidate в viewDidDisappear. Страховочный invalidate в deinit. Хранить ссылку на timer для возможности остановки. Альтернатива: Combine Timer.publish или async/await Task.

Какие lifecycle-события получает presenting VC при modal presentation?
?
При fullScreen: viewWillDisappear и viewDidDisappear. При pageSheet/formSheet: НЕ получает disappear events, так как presenting VC остаётся видимым. При dismiss -- viewWillAppear и viewDidAppear (только для fullScreen).

Почему viewDidLayoutSubviews вызывается многократно?
?
Вызывается при каждом изменении layout: первый показ, rotation (5-10 раз), keyboard, navigation bar, safe area, scroll, dynamic constraints. Оптимизация: кешировать lastLayoutBounds, выполнять тяжёлые операции только при изменении размера.
...
|-------------|------|-------|
| Следующий шаг | [[ios-swiftui]] | Сравнить императивный lifecycle с декларативным onAppear/onDisappear |
| Углубиться | [[ios-process-memory]] | Понять retain cycles в lifecycle -- closures, timers, observers |
| Смежная тема | [[android-activity-lifecycle]] | Сравнить onCreate/onResume с viewDidLoad/viewDidAppear |
