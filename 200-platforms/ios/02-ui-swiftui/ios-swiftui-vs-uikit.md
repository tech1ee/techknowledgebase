...
**[[ios-architecture-patterns]]** — Выбор между SwiftUI и UIKit напрямую влияет на архитектурные паттерны приложения. SwiftUI естественно тяготеет к MVVM благодаря встроенным property wrappers (@StateObject, @ObservedObject), тогда как UIKit поддерживает более широкий спектр паттернов включая VIPER, VIP и Coordinator. При гибридном подходе архитектура должна учитывать мост между двумя мирами — UIHostingController для SwiftUI внутри UIKit и UIViewRepresentable для UIKit внутри SwiftUI. Понимание архитектурных паттернов критично для принятия правильного решения о стратегии миграции.

**[[ios-performance-optimization]]** — Производительность является одним из ключевых факторов при выборе UI-фреймворка. SwiftUI использует автоматический diffing для минимизации перерисовок, но может испытывать проблемы с большими списками и сложными анимациями. UIKit предоставляет более тонкий контроль над производительностью через prefetching, cell reuse и ручное управление Core Animation. Инструменты профилирования (Instruments, View Debugger) работают с обоими фреймворками, но для SwiftUI требуются специфические подходы к диагностике проблем перерисовки.

**[[ios-migration-strategies]]** — Миграция с UIKit на SwiftUI является одной из наиболее частых задач в iOS-разработке 2025-2026 годов, и выбор правильной стратегии миграции критически важен для успеха проекта. Существуют четыре основных подхода: bottom-up (начиная с leaf-компонентов), top-down (новые экраны целиком на SwiftUI), module-by-module (миграция целых модулей) и гибридный. Каждый подход имеет свои преимущества в зависимости от размера команды, сложности проекта и бизнес-требований к срокам.
...
> [!question]- CTO стартапа спрашивает: "Почему нам не переписать всё приложение с UIKit на SwiftUI?" Каков ваш ответ?
> Полное переписывание рискованно: 1) Fortune 500 apps на 60% всё ещё на UIKit -- это проверено. 2) Некоторые UIKit API не имеют SwiftUI аналогов (UICollectionView compositional layout). 3) Оптимальнее инкрементальная миграция: новые экраны на SwiftUI, legacy постепенно через UIHostingController/UIViewRepresentable. 4) Рабочий код не нужно переписывать без бизнес-причины.

> [!question]- Почему UIKit всё ещё требуется в 70-80% job listings, несмотря на рост SwiftUI?
> Потому что большинство production apps имеют UIKit codebase, накопленный за 15+ лет. Migration -- длительный процесс. Enterprise-компании консервативны. Сложные UI (custom transitions, collection view layouts) лучше реализуются в UIKit. Знание UIKit необходимо для debugging SwiftUI (под капотом -- UIKit).

> [!question]- Какой presentation style в iOS определяет, получит ли presenting VC lifecycle events?
> .fullScreen -- presenting VC полностью скрывается и получает viewDidDisappear. .pageSheet/.formSheet (default с iOS 13) -- presenting VC виден сзади и НЕ получает disappear events. Это критично при выборе, где обновлять данные.

---

## Ключевые карточки

В чём ключевое отличие SwiftUI от UIKit?
?
SwiftUI -- декларативный (описываешь "что"), UIKit -- императивный (описываешь "как"). SwiftUI автоматически обновляет UI при изменении состояния, UIKit требует ручного управления. SwiftUI -- value types (struct), UIKit -- reference types (class).

Когда выбрать SwiftUI?
?
Новые проекты iOS 15+, простой/средний UI, быстрое прототипирование, мультиплатформа (watchOS, macOS, tvOS), маленькая команда, декларативный стиль. SwiftUI уменьшает код на 40-60%.

Когда выбрать UIKit?
?
Legacy проекты, сложные кастомные UI, iOS 13-14 support, production-proven, сложные анимации и transitions, UICollectionView compositional layouts, enterprise с accessibility requirements.

Как интегрировать SwiftUI в UIKit проект?
?
UIHostingController оборачивает SwiftUI View для использования в UIKit navigation. UIViewRepresentable позволяет использовать UIKit views внутри SwiftUI. UIViewControllerRepresentable -- для UIKit view controllers в SwiftUI.

Какой гибридный подход оптимален в 2026?
?
Новые экраны на SwiftUI, сложные компоненты на UIKit. UIHostingController для встраивания SwiftUI в UIKit navigation stack. Постепенная миграция без big-bang rewrite. ~70% команд используют гибридный подход.
...
|-------------|------|-------|
| Следующий шаг | [[ios-navigation]] | Освоить навигацию в обоих фреймворках |
| Углубиться | [[ios-architecture-patterns]] | Понять архитектурные паттерны для SwiftUI vs UIKit |
| Смежная тема | [[cross-ui-declarative]] | Кросс-платформенное сравнение декларативных UI подходов |
