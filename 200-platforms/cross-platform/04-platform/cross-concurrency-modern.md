...
> [!question]- Почему Swift actor -- языковая конструкция, а Kotlin решает ту же проблему через Mutex и Channel? Какие trade-offs у каждого подхода?
> Swift actor встроен в язык: compiler гарантирует изоляцию данных, запрещает прямой доступ к mutable state извне actor без await. Это compile-time safety -- ошибки обнаруживаются при компиляции. Kotlin Mutex -- библиотечное решение: coroutine lock, не проверяется компилятором, ответственность на разработчике. Trade-offs: Swift actors безопаснее, но менее гибки (re-entrancy по умолчанию, потенциальные deadlocks); Kotlin Mutex даёт полный контроль, но позволяет забыть lock. Channel -- другой подход: коммуникация вместо shared state.

> [!question]- Сценарий: корутина в viewModelScope выполняет сетевой запрос. Пользователь закрывает экран. Что произойдёт с запросом?
> viewModelScope привязан к lifecycle ViewModel. При закрытии экрана Activity/Fragment уничтожается, но ViewModel может жить дольше (configuration change). При реальном закрытии (back navigation) ViewModel.onCleared() отменяет viewModelScope, что отменяет все дочерние корутины. Сетевой запрос (suspend call) будет отменён на ближайшей suspension point через CancellationException. OkHttp/Ktor корректно прерывают HTTP-запрос. Аналог в Swift: Task отменяется, но нужно проверять Task.isCancelled вручную в async функциях.

> [!question]- Почему structured concurrency считается главным достижением обеих платформ по сравнению с GCD/Handler?
> Structured concurrency гарантирует: 1) все дочерние задачи завершатся до завершения родительской (нет orphaned tasks), 2) отмена распространяется автоматически сверху вниз, 3) ошибки propagate вверх по дереву задач, 4) ресурсы освобождаются предсказуемо. GCD/Handler: fire-and-forget, нет гарантии завершения, ручная отмена, memory leaks при захвате self. Structured concurrency делает async код таким же предсказуемым, как sync.

> [!question]- Как Kotlin Coroutines работают на iOS в KMP-проекте через SKIE?
> SKIE (Swift Kotlin Interface Enhancer) автоматически генерирует Swift wrappers для Kotlin suspend functions и Flow. Suspend function -> Swift async function (с proper cancellation). Flow -> Swift AsyncSequence. Без SKIE: suspend function -> callback-based API (completionHandler), Flow -> ручной wrapper через Closeable. SKIE устраняет impedance mismatch между корутинами и Swift Concurrency, позволяя iOS-разработчикам использовать native async/await синтаксис.

---

## Ключевые карточки

Чем Swift async/await отличается от Kotlin suspend functions?
?
Swift: async/await -- языковая конструкция, Task для запуска, TaskGroup для параллелизма, actor для изоляции. Kotlin: suspend functions через compiler plugin (CPS transform), CoroutineScope для запуска, coroutineScope для structured concurrency, Mutex для изоляции. Оба: cooperative, structured concurrency, cancellation propagation. Отличие: Swift actors -- compile-time safe, Kotlin Mutex -- runtime.

Что такое CoroutineScope и чем SupervisorJob отличается от обычного Job?
?
CoroutineScope: область структурированной конкурентности, определяет lifecycle корутин. Job: при ошибке в дочерней корутине -- отменяются ВСЕ дочерние корутины (fail-fast). SupervisorJob: при ошибке в дочерней корутине -- отменяется только она, остальные продолжают работать. viewModelScope использует SupervisorJob, чтобы ошибка в одном запросе не отменила остальные.

Как работает cancellation в structured concurrency?
?
Swift: Task.cancel() устанавливает флаг, async функции проверяют через Task.checkCancellation() или Task.isCancelled. Kotlin: cancel() выбрасывает CancellationException на ближайшем suspension point. Обе: cancellation cooperative -- код должен проверять отмену. Без проверки -- задача продолжит выполняться. Structured concurrency автоматически отменяет дочерние задачи при отмене родительской.

Что такое Dispatchers в Kotlin и как соотносятся с Swift?
?
Kotlin: Dispatchers.Main (UI thread), Dispatchers.IO (I/O operations, расширяемый пул), Dispatchers.Default (CPU-intensive, потоков = кол-во ядер), Dispatchers.Unconfined (не привязан к потоку). Swift: @MainActor (main thread), global actors, нет явных dispatchers -- runtime автоматически распределяет. withContext(Dispatchers.IO) = нет прямого аналога в Swift (нет необходимости -- async уже на background).

Чем TaskGroup в Swift отличается от coroutineScope в Kotlin?
?
TaskGroup: создаётся через withTaskGroup/withThrowingTaskGroup, добавляет child tasks через group.addTask {}, собирает результаты через for await. coroutineScope: создаёт новый scope, launch/async внутри для child coroutines, ожидает завершения всех. Оба: structured (все дочерние завершаются до выхода), cancel propagation. TaskGroup более типизирован (generic результат), coroutineScope гибче (разные типы через async).
...
|-------------|------|-------|
| Следующий шаг | [[cross-background-work]] | Фоновая работа использует async/await и корутины на практике |
| Углубиться | [[kotlin-coroutines]] | Детальное погружение в Kotlin Coroutines internals |
| Смежная тема | [[ios-async-await]] | Swift Concurrency (async/await, actor) из раздела iOS |
