...
> [!question]- Почему Apple выбрала ARC вместо tracing GC для iOS? Какие технические и бизнес-причины стоят за этим решением?
> Технические: 1) ARC детерминистичен -- объект освобождается мгновенно, когда reference count = 0, нет GC pauses. Для мобильных устройств с 60fps это критично -- GC pause > 16ms = пропущенный кадр. 2) ARC не требует отдельного GC thread, экономя CPU и батарею. 3) Предсказуемое потребление памяти -- нет накопления мусора до GC cycle. Бизнес: Apple контролирует hardware и может оптимизировать ARC под конкретные чипы. Trade-off: ARC не обрабатывает retain cycles автоматически -- разработчик должен использовать weak/unowned.

> [!question]- Сценарий: Android-приложение показывает jank при скролле списка. Профайлер показывает частые GC pauses. Какие оптимизации применить?
> Причина: excessive allocations в onBindViewHolder/composable функциях создают GC pressure. Оптимизации: 1) Object pooling -- переиспользовать объекты вместо создания новых (StringBuilder, Paint, Rect). 2) Избегать autoboxing -- использовать SparseIntArray вместо HashMap<Int, Int>. 3) Использовать @JvmInline value class для оберток. 4) Предаллоцировать коллекции с known capacity. 5) Использовать DiffUtil/LazyColumn key для минимизации rebind. 6) Проверить Baseline Profiles для pre-compilation hot paths.

> [!question]- Почему retain cycle -- главная проблема ARC, и почему GC не подвержен этой проблеме?
> ARC использует reference counting: объект жив пока refcount > 0. Retain cycle: A держит strong ref на B, B держит strong ref на A -- оба refcount = 1, но объекты недостижимы из root. ARC не может обнаружить cycles. GC использует tracing (reachability analysis от GC roots): если объект недостижим -- он мусор, даже если имеет циклические ссылки. GC обходит весь граф объектов, ARC считает только локальные references. Цена: GC требует stop-the-world или concurrent tracing.

> [!question]- Как Kotlin/Native управляет памятью на iOS-таргете в KMP-проектах?
> Kotlin/Native (с new memory manager, default since 1.7.20) использует tracing GC с concurrent mark-and-sweep. Это отличается от Swift ARC: Kotlin объекты в iOS-процессе собираются GC, Swift объекты -- ARC. На границе: Kotlin объект, переданный в Swift, оборачивается в ObjC wrapper с ARC. Когда Swift отпускает wrapper -- Kotlin GC может собрать объект. Потенциальная проблема: Kotlin GC может вызывать паузы на iOS, но на практике паузы минимальны (<1ms) благодаря concurrent collection.

---

## Ключевые карточки

Чем ARC (Automatic Reference Counting) отличается от GC (Garbage Collection)?
?
ARC: компилятор вставляет retain/release, объект освобождается при refcount=0. Детерминистичный, нет pauses, но не обрабатывает retain cycles. GC: runtime периодически сканирует граф объектов от roots, собирает недостижимые. Обрабатывает cycles, но создаёт pauses (Stop-the-World или concurrent overhead). ARC = iOS/Swift, GC = Android/JVM.

Что такое retain cycle и как его предотвратить в Swift?
?
Retain cycle: два объекта держат strong references друг на друга, refcount никогда не станет 0. Предотвращение: weak reference (обнуляется при dealloc, optional тип) для объектов с независимым lifecycle; unowned (не optional, crash при доступе после dealloc) для объектов с гарантированно равным lifecycle. Типичные места: closures (capture list [weak self]), delegate паттерн.

Какие типы GC используются на Android?
?
Android: Concurrent Mark-Sweep (CMS) на старых версиях, G1 (Garbage-First) на новых. G1: делит heap на regions, собирает сначала regions с наибольшим мусором, concurrent marking с STW pause только для remark. Generational: Young Gen (Eden + Survivor) для частых коллекций, Old Gen для долгоживущих объектов. Minor GC (Young) быстрый, Major GC (Full) дорогой.

Как memory warning работает на iOS vs Android?
?
iOS: didReceiveMemoryWarning/applicationDidReceiveMemoryWarning -- приложение должно освободить кэши, иначе jetsam (OOM killer) убьёт процесс. Нет swap на iOS. Android: onTrimMemory(level) с разными уровнями (TRIM_MEMORY_RUNNING_LOW, TRIM_MEMORY_COMPLETE). Android имеет zRAM (compressed swap), поэтому OOM менее агрессивен. Low Memory Killer уничтожает по oom_adj_score.

Как Kotlin/Native new memory manager работает на iOS?
?
New MM (default с Kotlin 1.7.20): concurrent tracing GC, no freeze requirement, shared state между потоками разрешён. GC работает в отдельном потоке, concurrent mark-and-sweep с минимальными паузами (<1ms). Объекты на границе Kotlin-Swift оборачиваются в ObjC wrappers (ARC управляет wrapper, GC управляет Kotlin объектом). Старый MM (deprecated) требовал freeze для shared state.
...
|-------------|------|-------|
| Следующий шаг | [[cross-performance-profiling]] | Профилирование памяти -- ключевой инструмент диагностики |
| Углубиться | [[garbage-collection-explained]] | Алгоритмы GC: mark-sweep, generational, concurrent |
| Смежная тема | [[ios-process-memory]] | Memory management на iOS из раздела iOS |
