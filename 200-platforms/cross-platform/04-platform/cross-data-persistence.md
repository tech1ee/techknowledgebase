...
> [!question]- Почему Core Data использует NSManagedObjectContext с разными concurrency types, а Room предпочитает suspend functions? Какая философия стоит за каждым подходом?
> Core Data (2005) проектировался до modern concurrency: NSManagedObjectContext привязан к потоку (mainQueueConcurrencyType или privateQueueConcurrencyType), доступ из другого потока = undefined behavior. Это thread-confinement модель. Room (2017) создан в эпоху корутин: DAO методы помечены suspend, Room автоматически переключает на background thread. Философия: Core Data даёт контроль (но ответственность на разработчике), Room абстрагирует threading (проще, но менее гибко).

> [!question]- Сценарий: после обновления приложения база данных крашится при запуске. Какие причины и как предотвратить?
> Причина: изменение схемы БД без миграции. Room: если version увеличился, но нет Migration объекта -- crash (IllegalStateException). Core Data: если model не совпадает с store -- crash (NSInternalInconsistencyException). Предотвращение: Room -- addMigrations() или fallbackToDestructiveMigration(). Core Data -- lightweight migration (automatic) для простых изменений, heavyweight (mapping model) для сложных. SwiftData: автоматические миграции через VersionedSchema. Обязательно: тестировать миграции на реальных данных.

> [!question]- Почему SQLDelight предпочтительнее Room для KMP-проектов?
> Room -- Android-only библиотека (зависит от android.database, kapt/ksp). SQLDelight: compile-time type-safe SQL для Kotlin Multiplatform, генерирует Kotlin data classes из .sq файлов, поддерживает iOS (через NativeSqliteDriver), Android, Desktop, JS. SQLDelight использует raw SQL (не ORM), что даёт полный контроль над запросами и лучшую производительность. Trade-off: нет ORM-удобств (relationship mapping, lazy loading), больше SQL-кода.

> [!question]- Чем UserDefaults/SharedPreferences отличаются от Keychain/EncryptedSharedPreferences для хранения токенов?
> UserDefaults/SharedPreferences: plain text XML/plist, доступны при jailbreak/root, не шифрованы. Подходят для настроек, не для секретов. Keychain (iOS): hardware-backed encryption, Secure Enclave, доступ через ACL, переживает удаление приложения. EncryptedSharedPreferences (Android): AES-256 шифрование через AndroidKeyStore, но файл остаётся в app sandbox. Правило: токены, пароли, API keys -- только Keychain/EncryptedSharedPreferences.

---

## Ключевые карточки

Чем Room отличается от Core Data?
?
Room: compile-time SQL проверка через annotations (@Entity, @Dao, @Query), suspend functions для async, Flow для reactive queries, lightweight. Core Data: full ORM с NSManagedObject, NSFetchedResultsController для UI-binding, CloudKit sync из коробки, heavyweight. Room ближе к SQL, Core Data ближе к object graph. SwiftData -- современная замена Core Data с @Model macro.

Как работают миграции в Room и Core Data?
?
Room: версионирование через @Database(version = N), Migration объекты с SQL ALTER TABLE. fallbackToDestructiveMigration() для сброса. Auto-migrations (Room 2.4+) для простых изменений. Core Data: lightweight migration (автоматическая, renaming/adding columns), heavyweight (NSMappingModel для сложных трансформаций). Обе: обязательно тестировать на реальных данных.

Что такое SQLDelight и почему он подходит для KMP?
?
SQLDelight: compile-time type-safe SQL generator от Square/CashApp. Пишешь .sq файлы с SQL-запросами, генерируются Kotlin data classes и type-safe API. Поддерживает все KMP targets через platform-specific drivers (AndroidSqliteDriver, NativeSqliteDriver для iOS). Не ORM -- полный контроль SQL. Альтернатива Room для KMP-проектов.

Чем DataStore отличается от SharedPreferences на Android?
?
SharedPreferences: синхронное API (commit() блокирует UI thread), apply() асинхронный но без гарантий, XML-файл, нет type safety. DataStore: полностью асинхронное (Kotlin Flow), Preferences DataStore (key-value) и Proto DataStore (Protocol Buffers с type safety), транзакционный API, no ANR risk. DataStore -- официальная замена SharedPreferences.

Какие данные хранить в Keychain (iOS) vs UserDefaults?
?
Keychain: tokens, passwords, API keys, certificates, cryptographic keys -- всё секретное. Hardware-backed encryption, Secure Enclave, kSecAttrAccessible для контроля доступа, переживает удаление приложения. UserDefaults: настройки, preferences, non-sensitive flags. Plain text plist, доступен при jailbreak. Правило: если данные нельзя показать пользователю -- Keychain.
...
|-------------|------|-------|
| Следующий шаг | [[cross-networking]] | Networking -- основной источник данных для persistence layer |
| Углубиться | [[android-data-persistence]] | Room, DataStore, EncryptedSharedPreferences из раздела Android |
| Смежная тема | [[mobile-databases-complete]] | SQLite, Realm, SQLDelight -- выбор БД из раздела Databases |
