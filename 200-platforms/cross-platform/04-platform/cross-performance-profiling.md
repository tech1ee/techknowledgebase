...
> **Профилирование производительности** — процесс измерения использования ресурсов (CPU, память, I/O, GPU) программой с целью выявления узких мест (bottlenecks) и оптимизации (Knuth, 1974: «We should forget about small efficiencies, about 97% of the time»).

### Метрики производительности

| Метрика | Определение | iOS инструмент | Android инструмент |
|---------|-------------|----------------|-------------------|
| **Startup time** | Время от tap до first frame | Instruments (App Launch) | Android Vitals, Macrobenchmark |
| **Frame rate** | Кадров в секунду (цель: 60/120) | Core Animation FPS | GPU rendering profiling |
| **Memory** | RSS / heap allocation | Instruments (Allocations) | Android Profiler (Memory) |
| **CPU** | % использования процессора | Instruments (CPU Profiler) | CPU Profiler |
| **Battery** | Энергопотребление | Instruments (Energy Log) | Battery Historian |
| **Network** | Latency, bandwidth | Instruments (Network) | Network Profiler |

### Amdahl's Law для мобильных приложений

```
Speedup = 1 / ((1 - P) + P/S)
P = доля оптимизируемого кода, S = ускорение этой доли
```

Следствие: оптимизация 10% кода (hot path) даёт больше пользы, чем оптимизация 90% cold path. Профилирование определяет P — какой код оптимизировать.

### Кросс-платформенные особенности

В KMP-приложении один и тот же shared-код может иметь **разную производительность** на разных targets:

| Фактор | JVM (Android) | Native (iOS) |
|--------|--------------|-------------|
| **JIT compilation** | Да (hot methods) | Нет (AOT only) |
| **GC impact** | GC pauses | Tracing GC (K/N) |
| **Inline functions** | Bytecode inlining | LLVM inlining |
| **Value classes** | Possible escape | Always inlined |

> **CS-фундамент:** Профилирование связано с [[cross-memory-management]] (memory profiling) и [[kmp-performance-optimization]] (KMP-оптимизации). Теоретическая база — Amdahl's Law (1967), Profiling (Knuth, 1974).
...
- **Knuth D. (1974).** *Structured Programming with go to Statements.* — «Premature optimization is the root of all evil» — профилируй перед оптимизацией.
- **Amdahl G. (1967).** *Validity of the Single Processor Approach.* — Amdahl's Law для оценки эффективности оптимизаций.

### Практические руководства

- [Instruments Guide](https://developer.apple.com/instruments/) — iOS профилирование.
- [Android Profiler](https://developer.android.com/studio/profile) — Android профилирование.
