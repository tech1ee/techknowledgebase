...
> [!question]- Почему вызов sync на текущей serial dispatch queue в GCD вызывает deadlock, и есть ли аналогичная проблема в Android Handler/Looper?
> GCD deadlock: sync блокирует текущий поток до завершения блока, но блок должен выполниться на этой же serial queue, которая занята ожиданием. Результат -- взаимная блокировка. DispatchQueue.main.sync из main thread -- классический пример. Android Handler: Looper обрабатывает messages последовательно. handler.sendMessageSync() из того же Looper thread не вызовет deadlock, потому что Android использует MessageQueue (FIFO), а не блокирующий sync вызов. Но если использовать CountDownLatch для ожидания результата -- аналогичный deadlock возможен.

> [!question]- Сценарий: в legacy iOS-коде используется concurrent queue с barrier для thread-safe чтения/записи словаря. Как реализовать аналогичный паттерн на Android?
> iOS: concurrent queue + dispatch_barrier_async для записи (эксклюзивный доступ) + dispatch_async для чтения (параллельный доступ). Это readers-writers lock на основе GCD. Android аналог: ReentrantReadWriteLock (readLock для чтения, writeLock для записи) или synchronized с ConcurrentHashMap. Другой вариант -- Handler с single thread (все операции последовательны, но проще). Современная альтернатива на обеих платформах: Actor model (Swift actor, Kotlin Mutex/Channel).

> [!question]- Почему AsyncTask был deprecated на Android и какие проблемы он вызывал?
> AsyncTask: 1) привязан к Activity lifecycle -- при повороте экрана Activity уничтожается, но AsyncTask продолжает работать и пытается обновить уничтоженный UI (crash/leak), 2) serial execution по умолчанию на одном потоке (API 11+) -- одна зависшая задача блокирует все, 3) неявное хранение ссылки на Activity (inner class) -- memory leak, 4) нет отмены -- cancel() не прерывает doInBackground, только ставит флаг. Замена: Kotlin Coroutines + viewModelScope (lifecycle-aware, structured cancellation).

> [!question]- Чем OperationQueue на iOS отличается от ExecutorService на Android?
> OperationQueue: объектно-ориентированная обёртка над GCD, поддерживает зависимости между Operation (addDependency), приоритеты, maxConcurrentOperationCount, KVO-наблюдение за состоянием. ExecutorService: Java API, пул потоков (fixed, cached, scheduled), Future для результатов, shutdown для завершения. Ключевое отличие: OperationQueue позволяет строить DAG зависимостей, ExecutorService -- нет (нужен CompletableFuture chain). Оба deprecated в пользу async/await и корутин.

---

## Ключевые карточки

Чем GCD (Grand Central Dispatch) отличается от Handler/Looper на Android?
?
GCD: системный уровень (libdispatch), работает с dispatch queues (serial/concurrent), автоматический thread pool, QoS для приоритетов. Handler/Looper: application level, MessageQueue с FIFO обработкой, привязан к конкретному thread, ручное создание потоков. GCD абстрагирует потоки полностью, Handler/Looper работает поверх конкретных потоков.

Что такое dispatch barrier и зачем он нужен?
?
dispatch_barrier_async на concurrent queue обеспечивает эксклюзивный доступ: все предыдущие блоки завершаются, barrier блок выполняется один, затем последующие блоки возобновляются. Реализует readers-writers lock паттерн: чтение параллельно (dispatch_async), запись эксклюзивно (dispatch_barrier_async). Эффективнее обычного mutex для частого чтения.

Как работает MessageQueue в Android Handler/Looper?
?
Looper.loop() бесконечно извлекает Message из MessageQueue и передаёт в Handler.handleMessage(). Messages упорядочены по when (timestamp). Handler.post(Runnable) оборачивает Runnable в Message. sendMessageDelayed добавляет с задержкой. Main Looper создаётся ActivityThread при старте приложения. Один Looper -- один поток.

Почему OperationQueue и ExecutorService считаются legacy подходами?
?
OperationQueue: verbose API (подклассы Operation, KVO), не интегрирован с Swift Concurrency, callback-based results. ExecutorService: callback через Future.get() (блокирующий), нет structured concurrency, нет автоматической отмены. Современные аналоги: Swift TaskGroup (structured, async/await), Kotlin coroutineScope (structured, suspend). Оба legacy API остаются в production-коде, знание необходимо для поддержки.

Что такое thread explosion и как GCD его предотвращает?
?
Thread explosion -- создание слишком многих потоков (сотни), каждый потребляет ~512KB stack. GCD ограничивает thread pool (обычно ~64 потока). Но можно вызвать thread explosion через concurrent queue + blocking sync calls (каждый блокированный поток создаёт новый). Решение: использовать serial queues для I/O, ограничивать maxConcurrentOperationCount, избегать sync вызовов на concurrent queues.
...
|-------------|------|-------|
| Следующий шаг | [[cross-concurrency-modern]] | Современные async/await и корутины -- эволюция GCD/Handler |
| Углубиться | [[ios-gcd-deep-dive]] | Детальное погружение в GCD internals из раздела iOS |
| Смежная тема | [[android-handler-looper]] | Handler/Looper/MessageQueue internals из раздела Android |
