...
> [!question]- Почему Dispatchers.IO не существует на iOS и как решить эту проблему в KMP?
> Kotlin/Native (iOS target) не имеет пула фоновых потоков как JVM. Dispatchers.IO -- JVM-специфичная реализация. Решение: создать expect val ioDispatcher в commonMain, в androidMain использовать actual val ioDispatcher = Dispatchers.IO, в iosMain -- actual val ioDispatcher = Dispatchers.Default.

> [!question]- Когда использовать expect/actual, а когда interface + DI для платформенных абстракций?
> expect/actual -- для простых утилит (getPlatformName, UUID, Dispatchers), где не нужно мокать в тестах. interface + DI -- для бизнес-логики и сервисов (Repository, Analytics, FileSystem), где тестируемость критична. Правило: если класс будет мокаться в тестах, используйте interface + DI.

> [!question]- Сценарий: вы пишете KMP-модуль без SKIE. Как Swift-код увидит sealed class Result?
> Без SKIE Swift увидит обычные Kotlin-классы без exhaustive switch. Нельзя будет использовать switch-case с проверкой компилятора на полноту. Придётся писать ручные wrapper-функции или использовать if/else цепочки, что теряет type safety.

> [!question]- Почему CoroutineScope без отмены на iOS приводит к memory leaks?
> На iOS нет автоматической привязки scope к lifecycle (как viewModelScope на Android). Если CoroutineScope создан в KMP-модуле и не отменяется при deinit ViewController, корутины продолжат работать, удерживая ссылки на объекты. Решение: привязать scope.cancel() к deinit UIViewController.
...
?
expect -- декларация контракта в commonMain (что нужно), actual -- платформенная реализация (как работает на Android/iOS). Компилятор гарантирует наличие actual для каждого expect на каждом таргете.

Что делает SKIE для Swift-разработчиков?
?
SKIE (Swift-Kotlin Interface Enhancer) от Touchlab автоматически преобразует: sealed class -> Swift enum с exhaustive switch, suspend -> async/await, Flow -> AsyncSequence. Делает KMP API нативным для Swift без ручных wrappers.

Как actual typealias используется в KMP?
?
actual typealias позволяет в androidMain использовать существующий JVM-класс как реализацию expect-декларации: actual typealias AtomicInt = java.util.concurrent.atomic.AtomicInteger. На iosMain нужна своя actual class реализация.

Какая модель слоёв абстракции рекомендуется для KMP?
?
Три слоя: Platform Layer (Swift/Android) -- минимум логики, Bridge Layer (expect/actual, SKIE) -- только адаптация, Shared Layer (commonMain) -- максимум логики. Логика движется вниз, платформенные детали остаются наверху.

Как интегрировать Compose Multiplatform UI на iOS?
?
В commonMain создаётся @Composable App(), в iosMain -- fun MainViewController() = ComposeUIViewController { App() }. В Swift -- UIViewControllerRepresentable wrapper, который создаёт MainViewController. Рендеринг на iOS идёт через Skia, не через UIKit.
...
|-------------|------|-------|
| Следующий шаг | [[cross-lifecycle]] | Lifecycle-привязка для KMP на разных платформах |
| Углубиться | [[kmp-interop-deep-dive]] | Низкоуровневый interop Kotlin/Native и ObjC |
| Смежная тема | [[kotlin-coroutines]] | Корутины как основа async-паттернов в KMP |
