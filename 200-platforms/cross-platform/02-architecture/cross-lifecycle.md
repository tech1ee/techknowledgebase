...
**[[android-activity-lifecycle]]** — Activity lifecycle является центральной концепцией Android-разработки, и текущий файл сравнивает его с iOS-аналогами. Понимание того, почему Android пересоздаёт Activity при повороте экрана (configuration changes), а iOS — нет, объясняет архитектурные решения обеих платформ. В заметке по Activity lifecycle подробно разобраны все callback-методы, SavedStateHandle и взаимодействие с ViewModel, что дополняет кросс-платформенное сравнение.

**[[ios-viewcontroller-lifecycle]]** — UIViewController lifecycle построен на принципиально другой философии: view-driven подход, где UI определяет состояние. Детальная заметка раскрывает нюансы loadView, viewDidLoad, viewWillAppear и их связь с memory warnings. Сравнение с Android в текущем файле показывает, что отсутствие process death callback на iOS делает архитектуру восстановления состояния совершенно иной.

**[[processes-threads-fundamentals]]** — Жизненный цикл приложения неразрывно связан с тем, как операционная система управляет процессами и потоками. На iOS jetsam убивает процесс молча, а на Android система отправляет onSaveInstanceState перед уничтожением — эти различия коренятся в разных моделях управления процессами. Понимание process scheduling и memory pressure помогает объяснить, почему платформы выбрали столь разные подходы к lifecycle.
...
> [!question]- Почему iOS убивает приложение молча (jetsam), а Android даёт шанс сохранить состояние через onSaveInstanceState?
> iOS проектировалась для одного активного приложения -- переключение было редким, и Apple полагала, что приложение должно само сохранять важные данные (UserDefaults, Core Data). Android проектировался для многозадачности на устройствах с ограниченной памятью, где переключение между приложениями -- норма, поэтому система встроила механизм сохранения состояния в lifecycle.

> [!question]- Сценарий: пользователь заполняет форму, переключается на другое приложение, через 5 минут возвращается. Как обеспечить сохранность данных на обеих платформах?
> iOS: сохранять данные в viewWillDisappear через UserDefaults или @AppStorage, восстанавливать в viewDidLoad. Android: использовать SavedStateHandle в ViewModel -- данные автоматически сохранятся в onSaveInstanceState и восстановятся в onCreate. В Compose -- rememberSaveable.

> [!question]- Почему критическую логику нельзя размещать в deinit (iOS) или onDestroy (Android)?
> deinit может не вызваться из-за retain cycle или jetsam. onDestroy не вызывается при process death. Оба не гарантированы. Критические операции (сохранение данных, аналитика) нужно выполнять в viewWillDisappear/onStop, а не откладывать до уничтожения.

> [!question]- Как Essenty решает проблему кросс-платформенного lifecycle в KMP?
> Essenty предоставляет общий Lifecycle API в commonMain: doOnStart, doOnStop, doOnDestroy. StateKeeper для сохранения/восстановления состояния (аналог SavedStateHandle). InstanceKeeper для объектов, переживающих configuration changes (аналог ViewModel). Платформенные интеграции маппят native lifecycle на общий API.

---

## Ключевые карточки

Чем view-driven lifecycle (iOS) отличается от system-driven lifecycle (Android)?
?
iOS: жизненный цикл контроллера привязан к его view -- viewWillAppear когда view появляется, deinit когда нет ссылок. Android: система контролирует lifecycle -- может убить Activity в любой момент, даёт onSaveInstanceState для сохранения, пересоздаёт при configuration changes.

Что происходит при Process Death на каждой платформе?
?
iOS (jetsam): приложение убивается молча, перезапускается полностью, все данные в памяти потеряны. Android (OOM Killer): onSaveInstanceState вызывается перед смертью, Bundle сохраняется, при возврате onCreate получает savedInstanceState.

Какие данные выживают Process Death на iOS?
?
UserDefaults, Keychain, Core Data, файлы на диске. Не выживают: всё состояние в памяти, view controllers, navigation stack, in-memory cache, активные сетевые запросы.

Что такое SavedStateHandle и зачем он нужен?
?
SavedStateHandle -- механизм в Android ViewModel для автоматического сохранения/восстановления состояния при process death. getStateFlow() возвращает Flow, который автоматически персистируется в Bundle. Решает проблему: обычный ViewModel выживает configuration changes, но не process death.

Почему поворот экрана пересоздаёт Activity на Android, но не UIViewController на iOS?
?
Android трактует поворот как configuration change -- ресурсы (layouts, strings) могут зависеть от ориентации, поэтому Activity пересоздаётся с новыми ресурсами. iOS использует Auto Layout и size classes -- UIViewController адаптируется без пересоздания, получая viewWillTransition(to:with:).

Что такое Essenty в контексте KMP?
?
Essenty -- библиотека Arkadii Ivanov для унификации lifecycle в KMP. Предоставляет Lifecycle (doOnStart/Stop/Destroy), StateKeeper (сохранение состояния), InstanceKeeper (объекты, переживающие config changes). Работает на обеих платформах через ComponentContext.
...
|-------------|------|-------|
| Следующий шаг | [[cross-state-management]] | Управление состоянием с учётом lifecycle |
| Углубиться | [[android-activity-lifecycle]] | Детальный lifecycle Activity из раздела Android |
| Смежная тема | [[processes-threads-fundamentals]] | Как ОС управляет процессами -- основа lifecycle |
