...
**[[android-state-management]]** — Jetpack Compose использует MutableState, StateFlow и SavedStateHandle для управления состоянием на разных уровнях (composable, ViewModel, process death). Заметка детально разбирает remember vs rememberSaveable, State Hoisting, side effects и интеграцию с Hilt. Понимание Compose-подхода к state помогает оценить, почему KMP-фреймворки (Decompose, MVIKotlin) выбрали реактивную модель, близкую к Android.

**[[ios-state-management]]** — SwiftUI использует property wrappers (@State, @Binding, @StateObject, @EnvironmentObject) для декларативного управления состоянием. Заметка раскрывает Combine, ObservableObject, @Published и TCA (The Composable Architecture). Сравнение с Android в текущем файле показывает, что Swift property wrappers более лаконичны, но менее гибки, чем Kotlin-аналоги, что влияет на проектирование shared state в KMP.

**[[kmp-state-management]]** — Управление состоянием в KMP — ключевая архитектурная задача, потому что iOS и Android используют разные реактивные примитивы (Combine vs Flow). Заметка описывает shared ViewModel patterns, KMP-MVVM, MVIKotlin и Decompose для unified state management. Это прямое продолжение кросс-платформенного сравнения из текущего файла с фокусом на практическую реализацию.
...
> [!question]- Почему SwiftUI использует property wrappers (@State, @Binding) для управления состоянием, а Compose -- функции (remember, mutableStateOf)?
> Это отражает разницу языковых возможностей. Swift property wrappers -- compile-time feature, которая оборачивает хранение значения и добавляет логику наблюдения. Kotlin не имеет property wrappers, поэтому Compose использует Compose compiler plugin, который отслеживает чтение State объектов во время composition и автоматически перекомпонует при изменении. Оба подхода достигают одного результата (реактивное обновление UI), но через разные механизмы языка.

> [!question]- Сценарий: после поворота экрана Android-приложение теряет введённые пользователем данные, хотя ViewModel сохраняет данные из сети. Где проблема?
> Данные формы хранятся в remember { mutableStateOf() }, который не переживает configuration change (Activity recreation). ViewModel переживает поворот, но remember -- нет. Решение: использовать rememberSaveable для примитивных данных формы, или поднять состояние формы в ViewModel через StateFlow. На iOS аналогичной проблемы нет -- SwiftUI View struct пересоздаётся, но @State автоматически сохраняется фреймворком.

> [!question]- Почему мутабельное состояние в Compose должно быть обёрнуто в MutableState, а не использоваться как обычная переменная?
> Compose compiler отслеживает чтение State<T> объектов во время composition. Если composable читает State, он автоматически подписывается на изменения и перекомпонуется при обновлении значения. Обычная переменная не отслеживается Compose runtime -- изменение не вызовет recomposition. Это фундаментальное отличие от imperative UI, где разработчик вручную обновляет View при изменении данных.

> [!question]- Какие проблемы возникают при проектировании shared state layer в KMP, если iOS использует Combine, а Android -- Kotlin Flow?
> Kotlin Flow работает в commonMain, но iOS не может нативно подписаться на Flow -- нужен wrapper (SKIE генерирует Swift async sequences, или ручной CFlow/CommonFlow). Combine Publisher и Flow имеют разную семантику отмены и backpressure. Shared ViewModel должен экспонировать StateFlow (immutable), а платформенный слой адаптирует: Android подписывается через collectAsState(), iOS через SKIE-обёртку или Combine adapter. Альтернатива -- KMP-MVVM библиотеки, абстрагирующие разницу.

---

## Ключевые карточки

Чем @State отличается от @StateObject в SwiftUI?
?
@State -- для простых value types (Int, String, Bool), принадлежит View, хранится SwiftUI. @StateObject -- для reference types (ObservableObject), создаётся один раз при первом appear View. @State пересоздаётся при struct recreation, @StateObject -- нет. Используй @StateObject для ViewModel, @State для UI-состояния.

Чем remember отличается от rememberSaveable в Compose?
?
remember сохраняет значение между recompositions, но теряет при configuration change (поворот экрана) или process death. rememberSaveable дополнительно сохраняет в SavedInstanceState Bundle -- переживает configuration change и process death. Для форм и пользовательского ввода используй rememberSaveable.

Что такое State Hoisting и зачем он нужен?
?
State Hoisting -- подъём состояния из дочернего компонента в родительский. Дочерний компонент становится stateless (принимает state и callbacks как параметры). Это улучшает: 1) тестируемость -- stateless компонент легче тестировать, 2) переиспользуемость -- один компонент для разных источников данных, 3) single source of truth -- состояние управляется в одном месте.

Как StateFlow используется для shared state в KMP?
?
StateFlow в commonMain хранит текущее состояние (hot stream, всегда имеет value). Android подписывается через collectAsState() в Compose. iOS получает доступ через SKIE (автоматически генерирует Swift AsyncSequence) или через CFlow wrapper. StateFlow предпочтительнее SharedFlow для UI state, потому что гарантирует наличие начального значения и conflation.

Что такое Unidirectional Data Flow (UDF) и почему он важен для обеих платформ?
?
UDF -- паттерн, где данные текут в одном направлении: UI Event -> ViewModel -> State -> UI. Пользователь генерирует Intent/Action, ViewModel обрабатывает и обновляет State, UI реагирует на новый State. UDF упрощает отладку (предсказуемый state), тестирование (проверяем state transitions) и соответствует модели SwiftUI/Compose (декларативный UI = f(state)).

Чем MVI отличается от MVVM в контексте state management?
?
MVVM: ViewModel хранит несколько observable свойств, View подписывается на каждое отдельно. MVI: единый sealed State class, единый Intent (события), Reducer (state + intent -> new state). MVI строже -- один state object, предсказуемые transitions. MVVM гибче -- проще для простых экранов. В KMP MVI удобнее, так как один shared state класс экспортируется на обе платформы.
...
|-------------|------|-------|
| Следующий шаг | [[cross-ui-declarative]] | Декларативный UI -- основной потребитель state management |
| Углубиться | [[kmp-state-management]] | Практическая реализация shared state в KMP-проектах |
| Смежная тема | [[ios-state-management]] | Детальное погружение в SwiftUI property wrappers из раздела iOS |
