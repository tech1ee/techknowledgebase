...
**[[android-testing]]** — Android-тестирование включает JUnit 4/5, Mockk, Espresso для UI, Robolectric для unit-тестов с Android SDK и Compose Testing API. Заметка детально разбирает Test Rules, AndroidJUnitRunner, Hilt-тестирование и coverage через JaCoCo. Понимание Android-тестирования помогает оценить, какие тесты можно вынести в commonTest (kotlin.test), а какие остаются platform-specific.
...
> [!question]- Почему XCTest -- монолитный фреймворк (unit + UI), а Android использует набор отдельных библиотек (JUnit + Mockk + Espresso + Robolectric)? Какие последствия для тестовой архитектуры?
> Apple контролирует всю экосистему и предоставляет единый инструмент: XCTest для unit-тестов, XCUITest для UI, интеграция с Xcode (coverage, test navigator). Android -- open ecosystem: JUnit из Java-мира, Mockk -- community, Espresso -- Google, Robolectric -- third-party. Последствия: iOS -- проще настроить, но менее гибко (Protocol-based mocking вместо библиотечного). Android -- больше выбора и гибкости, но сложнее начальная настройка и версионные конфликты между библиотеками. В KMP: commonTest использует kotlin.test, платформенные тесты используют нативные фреймворки.

> [!question]- Сценарий: нужно протестировать ViewModel, который зависит от Repository с сетевым вызовом. Как организовать тест на обеих платформах?
> Общий подход: Repository -- interface/protocol, в тесте подменяется на mock/fake. iOS: создать MockRepository, реализующий протокол, передать в ViewModel через initializer injection. XCTest + XCTestExpectation для async. Android: Mockk (mockk<UserRepository>(), every { }, coEvery { }), передать в ViewModel constructor. JUnit5 + Turbine для Flow testing, runTest для корутин. KMP: commonTest -- kotlin.test + fake Repository в commonMain, тестирует без платформенных зависимостей.

> [!question]- Почему snapshot-тестирование популярно на iOS (swift-snapshot-testing) и менее распространено на Android?
> iOS: swift-snapshot-testing от Point-Free -- записывает screenshot View, при следующем запуске сравнивает pixel-by-pixel. Популярно потому что: 1) SwiftUI Preview + snapshot = быстрый визуальный регресс, 2) ограниченное количество устройств (iPhone models), 3) детерминистичный рендеринг. Android: Paparazzi (от Square) набирает популярность, но: фрагментация устройств (тысячи моделей, разные density/size), разные OEM-шрифты -- snapshot'ы ломаются чаще. Compose Preview + Screenshot Testing API (Jetpack) улучшают ситуацию.

> [!question]- Какие тесты в KMP-проекте можно вынести в commonTest, а какие остаются платформенными?
> commonTest: бизнес-логика (use cases, repositories с fake data source), маппинг данных, валидация, state machine логика -- всё, что не зависит от платформенных API. Используется kotlin.test. Платформенные: UI-тесты (XCUITest/Espresso/Compose Testing), тесты platform-specific кода (CoreData, Room, platform permissions), integration-тесты с реальными HTTP-клиентами. Правило: чем больше кода в commonMain, тем больше тестов в commonTest.

---

## Ключевые карточки

Чем XCTest отличается от JUnit для мобильного тестирования?
?
XCTest: монолитный фреймворк Apple (unit + UI + performance), интеграция с Xcode, XCTestExpectation для async, Protocol-based mocking (нет reflection). JUnit: Java-фреймворк для unit-тестов, используется с Mockk/Mockito для mocking, Espresso для UI, Robolectric для unit с Android SDK. XCTest проще начать, JUnit + ecosystem гибче.

Как тестировать async код на обеих платформах?
?
iOS: XCTestExpectation + wait(for:timeout:), или async test methods (Swift 5.5+: func testSomething() async throws). Android: runTest { } для корутин (kotlinx-coroutines-test), Turbine для Flow testing (app.cash.turbine), TestDispatcher для контроля времени. KMP commonTest: runTest + TestDispatcher работает для shared async кода.

Что такое Robolectric и зачем он нужен?
?
Robolectric: JVM-реализация Android SDK, позволяет запускать unit-тесты без эмулятора. Симулирует Android классы (Context, SharedPreferences, SQLite) на JVM. Быстрее instrumented tests (секунды vs минуты). Ограничения: не 100% совместим с реальным Android, некоторые API не поддерживаются. Аналог на iOS: нет необходимости -- XCTest запускается на Mac с полным iOS SDK.

Как kotlin.test работает в KMP commonTest?
?
kotlin.test: мультиплатформенная тестовая библиотека. Аннотации @Test, @BeforeTest, @AfterTest. Assertions: assertEquals, assertTrue, assertFails. Под капотом: на JVM -> JUnit, на Native -> kotlin.test.native, на JS -> Karma/Mocha. Пишешь тесты один раз в commonTest, запускаются на всех target platforms. Ограничение: нет mocking framework -- используй fakes/stubs.

Какие уровни тестирования используются в мобильной разработке?
?
Unit: изолированная логика, mock зависимости (kotlin.test/JUnit/XCTest). Integration: взаимодействие компонентов, real DB + fake network (Room InMemory, Core Data in-memory). UI: user interactions (Espresso, Compose Testing, XCUITest). Snapshot: визуальный регресс (Paparazzi, swift-snapshot-testing). E2E: полный user flow на реальном устройстве. Testing Pyramid: много unit, меньше integration, мало UI/E2E.
