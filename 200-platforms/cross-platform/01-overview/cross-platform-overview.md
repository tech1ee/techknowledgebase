...
> [!question]- Почему iOS использует ARC, а Android -- GC, и как это влияет на паттерны разработки?
> iOS пришла из мира NeXTSTEP, где детерминированное управление памятью было приоритетом -- отсюда reference counting с предсказуемым освобождением ресурсов. Android пришёл из мира Java, где GC автоматически разрешает циклические ссылки. На практике это означает, что iOS-разработчик обязан вручную разрывать retain cycles через weak/unowned, а Android-разработчик полагается на GC, но получает непредсказуемые паузы.

> [!question]- Вам нужно спроектировать KMP-модуль для приложения с offline-first синхронизацией. Какие слои вы вынесете в shared, а какие оставите платформенными?
> В shared следует вынести бизнес-логику (use cases, валидацию), модели данных, сетевой слой (Ktor), базу данных (SQLDelight) и state management. Платформенными останутся UI (SwiftUI/Compose), push-уведомления, platform-specific APIs и часть storage (Keychain/KeyStore для секретов). Ключевой принцип -- максимум логики в commonMain, минимум в expect/actual.

> [!question]- Сценарий: команда из 3 Android-разработчиков запускает KMP-проект. Какие риски вы видите для iOS-части?
> Главные риски: iOS-сборка будет ломаться без понимания Xcode toolchain; отладка на iOS сложнее из-за непрозрачного Kotlin/Native debugging; SKIE-конфигурация может требовать Swift-знаний; производительность Kotlin/Native может отличаться от JVM. Решение -- заложить время на обучение iOS-специфике и привлечь хотя бы консультанта с iOS-опытом.

> [!question]- Почему утверждение "SwiftUI и Compose -- это одно и то же" является заблуждением?
> Хотя оба фреймворка декларативные, их реализация фундаментально различается. SwiftUI View -- это struct (value type), Compose @Composable -- это функция. SwiftUI использует diffing Views для обновления, Compose -- positional memoization. SwiftUI построен поверх UIKit, Compose -- полностью новый рендеринг-движок. На iOS Compose Multiplatform рендерит через Skia, а не через UIKit, что влияет на поведение анимаций и жестов.

---

## Ключевые карточки

Какая фундаментальная философия лежит в основе iOS-платформы?
?
Детерминизм и контроль. Разработчик контролирует lifecycle, память освобождается предсказуемо через ARC, один vendor обеспечивает консистентность. Компилятор знает всё о целевом устройстве.

Какая фундаментальная философия лежит в основе Android-платформы?
?
Гибкость и адаптивность. Система контролирует lifecycle, GC освобождает память когда удобно, код должен работать на тысячах разных устройств. Байткод JVM обеспечивает универсальность.

Чем struct в Swift отличается от data class в Kotlin?
?
struct -- это value type (копируется при присваивании, хранится на стеке), а data class -- reference type (передаётся по ссылке, хранится в куче). Swift struct в 10x быстрее для простых данных. В KMP обычно выбирают immutable подход.

Что такое deinit в iOS и почему на него можно полагаться?
?
deinit -- деструктор Swift-объекта, гарантированно вызывается при обнулении последней ссылки (ARC). В отличие от Android onDestroy(), который может не вызваться при process death, deinit вызывается детерминированно -- если нет retain cycle.

Когда KMP НЕ подходит как решение?
?
KMP не подходит для простых приложений, MVP/прототипов, команд из одного разработчика без Kotlin-опыта. Также KMP не решает проблемы платформенного UI (до стабилизации Compose Multiplatform на iOS), увеличивает build time и размер приложения.

Какие три вопроса задаёт каждый файл в разделе Cross-Platform?
?
1) ЧТО -- конкретные различия в API, синтаксисе, подходах. 2) ПОЧЕМУ -- исторические и технические причины различий. 3) КАК -- практические паттерны для KMP-разработки.

В чём разница между ARC (iOS) и GC (Android) по trade-offs?
?
ARC: предсказуемая задержка, низкий overhead, нет пауз, но retain cycles -- ваша проблема. GC: автоматически решает циклы, проще для разработчика, но непредсказуемые паузы, память освобождается "когда-то", больше потребление памяти.

Какие три категории готовности проверяет чеклист кросс-платформенного разработчика?
?
Фундамент (ARC vs GC, lifecycle, async, типы), UI (SwiftUI, Compose, state management, navigation) и KMP (expect/actual, настройка проекта, библиотеки Ktor/SQLDelight/Koin, ограничения Kotlin/Native).
...
|-------------|------|-------|
| Следующий шаг | [[cross-decision-guide]] | Применить знания к выбору технологического стека |
| Углубиться | [[cross-memory-management]] | Детальное сравнение ARC vs GC с примерами кода |
| Смежная тема | [[kmp-architecture-patterns]] | Архитектурные паттерны KMP из раздела Kotlin Multiplatform |
| Обзор | [[cross-platform-overview]] | Вернуться к карте раздела |
