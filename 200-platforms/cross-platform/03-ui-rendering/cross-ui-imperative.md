...
**[[android-ui-views]]** — Android View system построена на иерархии View → ViewGroup с XML-разметкой и программным созданием. Заметка детально разбирает measure/layout/draw pipeline, RecyclerView с ViewHolder pattern, custom views, touch handling и ViewBinding. Понимание imperative Android UI необходимо для работы с legacy-кодом (80%+ существующих приложений) и для оценки преимуществ перехода на Compose.

**[[ios-uikit-fundamentals]]** — UIKit — это фундамент iOS-разработки, построенный на UIView/UIViewController с Auto Layout и UITableView/UICollectionView. Заметка раскрывает storyboards, XIB, программную вёрстку, UIGestureRecognizer и Core Animation. Сравнение с Android Views в текущем файле показывает, что UIKit более объектно-ориентирован (делегаты, datasources), тогда как Android использует паттерн Adapter.

**[[ios-custom-views]]** — Создание кастомных views на iOS через подклассы UIView с переопределением draw(_:), layoutSubviews() и intrinsicContentSize. Заметка раскрывает Core Graphics, CALayer, hit testing и accessibility. Это дополняет кросс-платформенное сравнение, показывая, что iOS custom views более тесно интегрированы с графическим стеком (Core Animation/Metal), чем Android custom views с Canvas.
...
> [!question]- Почему UIKit использует delegate/datasource паттерн для UITableView, а Android использует Adapter для RecyclerView? Какие trade-offs у каждого подхода?
> UIKit delegate/datasource разделяет ответственность: datasource предоставляет данные (количество строк, ячейки), delegate обрабатывает взаимодействие (выбор, swipe). Это следует паттерну Protocol-Oriented Programming Swift. Android Adapter объединяет обе роли в одном классе, но через ViewHolder pattern отделяет создание View от привязки данных. Trade-offs: UIKit -- более модульный (можно заменить delegate отдельно), Android -- проще для понимания (один класс), но ViewHolder обязателен для производительности.

> [!question]- Сценарий: UITableView показывает неправильные изображения при быстрой прокрутке. Какая причина и как исправить?
> Причина -- переиспользование ячеек (cell reuse). При быстрой прокрутке ячейка получает новые данные, но предыдущий асинхронный загрузчик изображения всё ещё загружает старую картинку и устанавливает её. Исправления: 1) в prepareForReuse() отменять предыдущий запрос загрузки и очищать imageView, 2) при установке изображения проверять, что URL всё ещё соответствует текущей ячейке, 3) использовать библиотеки (Kingfisher/SDWebImage на iOS, Coil/Glide на Android), которые обрабатывают это автоматически.

> [!question]- Почему Android требует ViewHolder pattern для RecyclerView, тогда как UIKit обходится без аналогичного требования?
> RecyclerView разделяет создание View (onCreateViewHolder -- дорого) и привязку данных (onBindViewHolder -- дёшево). ViewHolder кэширует ссылки на дочерние View, избегая повторного findViewById() при каждом bind. UIKit решает эту проблему иначе: dequeueReusableCell возвращает уже настроенную ячейку, а IBOutlet/property доступ к подвидам кэшируется на уровне UIView иерархии. Android подход более явный и оптимизированный для сложных списков.

> [!question]- Какие преимущества Auto Layout (iOS) имеет перед ConstraintLayout (Android) и наоборот?
> Auto Layout: интеграция с Interface Builder, intrinsicContentSize автоматически определяет размер контента, приоритеты constraints (1-1000) для flexible layouts. ConstraintLayout: Barrier, Guideline, Chain -- мощнее для сложных layouts без вложенности, MotionLayout для анимаций, flat hierarchy (лучше производительность). Auto Layout может создавать глубокую иерархию constraints, ConstraintLayout решает всё в одном уровне. Оба используют Cassowary-подобные алгоритмы решения constraints.

---

## Ключевые карточки

Чем отличается UIView lifecycle от Android View lifecycle?
?
UIView: init -> willMove(toSuperview:) -> didMoveToSuperview -> layoutSubviews -> draw(_:). Android View: constructor -> onAttachedToWindow -> onMeasure -> onLayout -> onDraw. Ключевое отличие: Android явно разделяет measure и layout фазы, UIKit объединяет их в layoutSubviews. Android требует setMeasuredDimension() в onMeasure.

Как работает cell reuse в UITableView и RecyclerView?
?
UITableView: dequeueReusableCell(withIdentifier:) возвращает существующую ячейку из пула или создаёт новую. prepareForReuse() сбрасывает состояние. RecyclerView: onCreateViewHolder() создаёт ViewHolder, onBindViewHolder() привязывает данные. onViewRecycled() -- аналог prepareForReuse(). Оба: переиспользуют View для экономии памяти.

Что такое measure/layout/draw pipeline в Android?
?
Три фазы рендеринга: 1) Measure -- каждый View определяет свой размер (onMeasure, MeasureSpec), 2) Layout -- каждый ViewGroup позиционирует дочерние View (onLayout), 3) Draw -- отрисовка на Canvas (onDraw). Фазы идут сверху вниз по дереву. invalidate() вызывает только draw, requestLayout() -- все три фазы.

Чем storyboard отличается от программной вёрстки в UIKit?
?
Storyboard: визуальный редактор, auto layout constraints в Interface Builder, segues для навигации. Плюсы: наглядность, быстрое прототипирование. Минусы: merge conflicts в XML, runtime crashes при опечатках в identifier. Программная вёрстка: NSLayoutConstraint/LayoutAnchors в коде. Плюсы: type safety, лучше для code review, нет merge conflicts. Минусы: verbose, сложнее визуализировать.

Какие паттерны проектирования лежат в основе imperative UI обеих платформ?
?
Composite (view hierarchy -- View содержит дочерние View), Template Method (lifecycle методы: layoutSubviews/onMeasure), Observer (delegate/datasource в UIKit, OnClickListener в Android), Adapter (RecyclerView.Adapter), Strategy (LayoutManager в RecyclerView, UICollectionViewLayout). Понимание этих паттернов помогает проектировать custom components.
...
|-------------|------|-------|
| Следующий шаг | [[cross-ui-declarative]] | Декларативный UI -- эволюция imperative подхода |
| Углубиться | [[android-recyclerview-internals]] | Детальное погружение в RecyclerView internals |
| Смежная тема | [[ios-custom-views]] | Создание кастомных View в UIKit из раздела iOS |
