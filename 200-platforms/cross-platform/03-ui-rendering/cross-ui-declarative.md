...
> [!question]- Почему SwiftUI пересчитывает весь body, а Compose перекомпонует только изменённые composable functions? Какие последствия это имеет для производительности?
> SwiftUI body -- computed property, Swift вычисляет его целиком при каждом изменении @State. Но SwiftUI использует structural diffing для определения, какие View реально изменились. Compose работает иначе: compiler plugin отслеживает чтение State объектов в каждой composable function и перекомпонует только те функции, которые читали изменённый State. Следствие: в Compose мелкая гранулярность (одна функция), в SwiftUI -- крупная (весь body). Для производительности Compose требует меньше оптимизаций «из коробки», но SwiftUI компенсирует это через AttributeGraph.

> [!question]- Сценарий: после добавления анимации список из 1000 элементов в Compose начал заметно тормозить. Какие причины и как исправить?
> Причины: 1) LazyColumn без key -- Compose не может отследить identity элементов при анимации, перекомпонует все. 2) Анимация применяется к элементу, вызывающему recomposition родителя. 3) Нестабильные (unstable) классы данных в items -- каждый recomposition считает их изменёнными. Исправления: добавить key = { item.id } в items {}, использовать @Stable/@Immutable аннотации для data classes, применять animateItem() modifier вместо ручной анимации, профилировать через Layout Inspector recomposition counts.

> [!question]- Почему порядок модификаторов в SwiftUI и Compose меняет результат, и чем механизмы отличаются?
> В SwiftUI каждый modifier возвращает новый View, оборачивая предыдущий (как матрёшка). .padding().background(.red) -- padding внутри красного фона. .background(.red).padding() -- красный фон, затем прозрачный padding снаружи. В Compose Modifier -- это цепочка объектов, применяемых последовательно к Layout Node. Результат аналогичен, но механизм другой: SwiftUI создаёт новые View structs, Compose модифицирует properties одного Layout Node.

> [!question]- Какие преимущества даёт Compose Multiplatform по сравнению с отдельными SwiftUI + Compose на нативных платформах?
> CMP позволяет писать shared UI на Kotlin, что даёт: 1) единый UI-код для Android, iOS, Desktop, Web -- меньше дублирования, 2) единый design system -- гарантия визуальной консистентности, 3) shared state management через ViewModel в commonMain. Trade-offs: iOS-пользователи могут заметить отличия от нативного поведения (scroll physics, haptics), Apple-специфичные API (Share Sheet, Shortcuts) требуют platform bridges, и community/tooling пока меньше, чем у нативных фреймворков.

---

## Ключевые карточки

Чем отличается recomposition в Compose от body re-evaluation в SwiftUI?
?
Compose: compiler plugin отслеживает чтение State в каждой composable function, перекомпонует только изменённые функции (мелкая гранулярность). SwiftUI: пересчитывает весь body computed property, затем structural diffing определяет реальные изменения (крупная гранулярность). Оба подхода в итоге обновляют только изменённые UI-элементы.

Что такое @ViewBuilder в SwiftUI и @Composable в Compose?
?
@ViewBuilder -- result builder, который позволяет писать несколько View выражений в теле closure и автоматически объединяет их в TupleView/Group. @Composable -- annotation, обрабатываемая Compose compiler plugin, которая трансформирует функцию для поддержки positional memoization и recomposition. Оба -- DSL для декларативного описания UI.

Как работает remember {} в Compose и есть ли аналог в SwiftUI?
?
remember {} сохраняет значение между recompositions -- при recomposition значение не пересоздаётся. Ближайший аналог в SwiftUI -- @State, который тоже сохраняет значение между body re-evaluations. Но @State привязан к identity View, а remember -- к позиции в Composition tree (positional memoization).

Почему нельзя вызывать suspend/async функции напрямую в body/composable?
?
body SwiftUI и composable functions вызываются фреймворком многократно при каждом изменении состояния. Сайд-эффекты (сетевые запросы, запись в БД) не должны повторяться при каждом re-evaluation. Compose: используй LaunchedEffect/SideEffect. SwiftUI: используй .task {} modifier или .onAppear {}. Это обеспечивает контролируемый lifecycle сайд-эффектов.

Что такое Modifier chain в Compose и View modifier chain в SwiftUI?
?
Compose Modifier -- цепочка объектов (Modifier.padding().background()), каждый добавляет поведение к Layout Node. Порядок важен: применяются последовательно. SwiftUI modifier -- функция, возвращающая новый View (обёртку). .padding().background() создаёт ModifiedContent<ModifiedContent<View, PaddingModifier>, BackgroundModifier>. Оба: порядок влияет на результат.
...
|-------------|------|-------|
| Следующий шаг | [[cross-graphics-rendering]] | Графический pipeline -- то, что стоит за декларативным UI |
| Углубиться | [[android-compose]] | Детальное погружение в Jetpack Compose internals |
| Смежная тема | [[ios-swiftui]] | Детальное погружение в SwiftUI из раздела iOS |
