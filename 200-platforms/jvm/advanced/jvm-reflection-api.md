...
> [!question]- Почему Dagger использует compile-time code generation вместо Reflection для DI, и когда это оправдано?
> Dagger генерирует код через annotation processor, что дает: (1) производительность как у обычного кода (без 7-100x overhead Reflection); (2) ошибки DI-графа обнаруживаются при компиляции; (3) совместимость с ProGuard/R8 (obfuscation не ломает DI). Оправдано на Android, где startup time критичен и каждая миллисекунда на счету. Spring использует Reflection, потому что гибкость runtime-конфигурации важнее startup time на серверах.

> [!question]- Сценарий: Benchmark показывает, что Reflection-вызов в 225x медленнее прямого. Но с кэшированием Method объекта — только 7.5x. Почему такая разница и как оптимизировать дальше?
> Без кэширования: каждый вызов включает getMethod() — проход по всем методам класса, String-сравнение имен, проверка parameter types (~450ns). С кэшированием: только invoke() — security checks, boxing аргументов, native call (~15ns). Дальнейшая оптимизация: MethodHandles (JIT может инлайнить), ByteBuddy (генерирует прямой байткод), или VarHandle для доступа к полям.

> [!question]- Почему Dynamic Proxy в Java работает только с интерфейсами? Как создать proxy для класса?
> Сгенерированный proxy наследуется от java.lang.reflect.Proxy. Java не поддерживает множественное наследование классов — proxy не может одновременно наследовать Proxy и целевой класс. Для proxy классов: CGLIB генерирует подкласс целевого класса; ByteBuddy предоставляет более современный API с поддержкой final классов (через Advice). Spring использует JDK Dynamic Proxy для интерфейсов и CGLIB/ByteBuddy для классов.

> [!question]- В чем разница между getMethod() и getDeclaredMethod()? Почему это частый источник ошибок?
> getMethod() возвращает только public методы, включая inherited от суперклассов и интерфейсов. getDeclaredMethod() возвращает все методы (public, protected, private, package-private), но только объявленные в данном классе, без inherited. Частая ошибка: getDeclaredMethod() для inherited метода вернет NoSuchMethodException. Для private inherited: нужно подняться по hierarchy через getSuperclass().

---

## Ключевые карточки

Какие три способа получить объект Class в Java?
?
1. Через .class литерал: User.class (compile-time, тип Class<User>). 2. Через getClass() на instance: user.getClass() (runtime, тип Class<?>). 3. Через Class.forName("com.example.User") (полностью динамически, имя может быть из конфига). Первый способ самый быстрый, третий самый гибкий.

Почему Reflection в 7-100x медленнее прямого вызова?
?
Прямой вызов: invokevirtual — 1-2 инструкции через vtable (~2-5ns). Reflection: (1) поиск метода по имени в таблице методов, (2) security checks, (3) boxing/unboxing аргументов в Object[], (4) native JNI-вызов invoke(), (5) проверка типов в runtime, (6) exception handling, (7) unboxing результата. С кэшированием Method объекта шаги 1-2 пропускаются, остается 7-10x overhead.

Что такое InvocationHandler и как он связан с Dynamic Proxy?
?
InvocationHandler — интерфейс с методом invoke(proxy, method, args), перехватывающим ВСЕ вызовы методов proxy-объекта. Dynamic Proxy (Proxy.newProxyInstance) генерирует класс, реализующий указанные интерфейсы, где каждый метод делегирует вызов InvocationHandler. Spring AOP использует это для @Transactional: handler добавляет begin/commit/rollback вокруг вызова реального метода.

Чем MethodHandles лучше Reflection и в чем их ограничения?
?
MethodHandles быстрее: JIT может оптимизировать MethodHandle.invoke() лучше, чем Method.invoke() (меньше dynamic dispatch). Type-safe через MethodType. Лучше интегрируется с invokedynamic (лямбды, string concatenation). Ограничения: более сложный API, меньше flexibility — нет introspection (нельзя перечислить все методы класса), нет getDeclaredFields(). Для discovery используйте Reflection, для вызовов — MethodHandles.

Как setAccessible(true) обходит инкапсуляцию и какие есть ограничения?
?
setAccessible(true) отключает access checks для Field/Method/Constructor: позволяет читать/менять private поля, вызывать private методы. Ограничения: (1) JPMS модульная система — InaccessibleObjectException если package не открыт через opens; (2) SecurityManager (deprecated) мог блокировать; (3) final поля: значение может быть inlined JIT-компилятором, изменение через Reflection не обновит все использования.
...
|-------------|------|-------|
| Следующий шаг | [[jvm-annotations-processing]] | Понять compile-time альтернативу: APT генерирует код без runtime overhead |
| Углубиться | [[jvm-bytecode-manipulation]] | Изучить ByteBuddy как замену Reflection для proxy-генерации |
| Связанная тема | [[jvm-module-system]] | Понять как JPMS ограничивает Reflection через opens/exports |
| Смежная область | [[android-dagger-deep-dive]] | Увидеть compile-time DI как альтернативу Reflection-based Spring DI |
