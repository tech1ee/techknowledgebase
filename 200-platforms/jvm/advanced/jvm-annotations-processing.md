...
> [!question]- Почему Lombok не может быть реализован через стандартный Annotation Processing API (JSR-269)?
> Стандартный APT может только генерировать НОВЫЕ файлы — он не может модифицировать существующий исходный код или AST. Lombok работает через хаки javac internals: получает доступ к внутреннему представлению AST компилятора через non-public API и модифицирует его напрямую. В Eclipse Lombok использует java agent для инъекции кода в парсер IDE. Это объясняет, почему Lombok ломается при обновлении JDK и требует отдельных плагинов для каждой IDE.

> [!question]- Сценарий: У вас проект с Lombok, MapStruct и Dagger. При сборке MapStruct не видит getter-методы, сгенерированные Lombok. Как решить проблему и почему она возникает?
> Проблема в порядке annotation processors: MapStruct пытается обработать классы до того, как Lombok сгенерировал getter/setter методы. Решение: (1) в annotationProcessorPaths указать Lombok ПЕРВЫМ, MapStruct ВТОРЫМ; (2) добавить lombok-mapstruct-binding зависимость для корректной интеграции. Порядок критичен, потому что процессоры работают в rounds — MapStruct должен видеть уже готовые методы.

> [!question]- В чем принципиальная разница между compile-time (@Retention(SOURCE)) и runtime (@Retention(RUNTIME)) аннотациями с точки зрения производительности? Когда каждый подход предпочтительнее?
> Compile-time аннотации обрабатываются один раз при компиляции — сгенерированный код работает как обычный Java без overhead. Runtime аннотации читаются через reflection при каждом обращении — это 20-200x медленнее. Compile-time предпочтительнее когда типы известны заранее (MapStruct маппинг, Dagger DI граф). Runtime необходим когда конфигурация зависит от runtime данных (Spring контекст с профилями, Hibernate с dynamic schema).

> [!question]- Annotation processor генерирует файл с аннотацией, которую сам же обрабатывает. Что произойдет и как это предотвратить?
> Возникнет бесконечный цикл: процессор генерирует файл, компилятор обнаруживает новый файл с аннотацией, запускает новый round, процессор генерирует ещё один файл, и так далее. Предотвращение: (1) использовать узкий @SupportedAnnotationTypes вместо "*"; (2) сгенерированные файлы не должны содержать обрабатываемую аннотацию; (3) возвращать true из process() для claim аннотации; (4) проверять roundEnv.processingOver() для финального round.

---

## Ключевые карточки

Какие три Retention Policy существуют для аннотаций и когда каждая доступна?
?
SOURCE — только в исходном коде, отбрасывается компилятором (@Override, @SuppressWarnings). CLASS — сохраняется в .class файле, но недоступна через reflection в runtime (по умолчанию). RUNTIME — доступна через reflection в runtime (@Entity, @Transactional).

Что делает метод process() в AbstractProcessor и что означает возвращаемое значение boolean?
?
process() вызывается в каждом round обработки аннотаций. Получает набор TypeElement (обрабатываемые аннотации) и RoundEnvironment (доступ к аннотированным элементам). Возвращает true если аннотации "claimed" (другие процессоры не будут их обрабатывать), false если другие процессоры тоже могут обработать.

В чем разница между KAPT и KSP для обработки аннотаций в Kotlin?
?
KAPT (Kotlin Annotation Processing Tool) генерирует Java stubs из Kotlin кода и запускает стандартные Java annotation processors — совместим со всеми Java процессорами, но медленный (2x overhead). KSP (Kotlin Symbol Processing) — нативный Kotlin API, 2-3x быстрее KAPT, полная поддержка Kotlin features, но требует специальной поддержки от библиотеки.

Почему compile-time code generation (APT) в 20-200x быстрее runtime reflection?
?
APT генерирует обычный Java-код один раз при компиляции — далее JIT оптимизирует его как обычный вызов (~5-10ns). Reflection каждый раз выполняет dynamic dispatch: поиск метода по имени, проверка доступа, boxing/unboxing аргументов (~100-1000ns). Cached reflection быстрее uncached (20x vs 200x), но все равно медленнее generated кода.

Как правильно зарегистрировать annotation processor для javac?
?
Два способа: (1) Создать файл META-INF/services/javax.annotation.processing.Processor с полным именем класса процессора. (2) Использовать Google AutoService — аннотация @AutoService(Processor.class), которая сама генерирует META-INF/services файл. В обоих случаях процессор должен быть в classpath или annotationProcessorPath при компиляции.

Что такое processing round в annotation processing и зачем нужны несколько rounds?
?
Round — один проход обработки аннотаций компилятором. В каждом round процессоры обрабатывают аннотированные элементы и могут генерировать новые файлы. Новые файлы проверяются в следующем round — вдруг они тоже содержат обрабатываемые аннотации. Rounds продолжаются пока новых файлов не генерируется. Финальный round (processingOver()=true) — сигнал для cleanup.

Какие ограничения у стандартного APT по сравнению с Lombok?
?
APT может только генерировать НОВЫЕ .java файлы — не может модифицировать существующий исходный код, не может изменять AST, не может добавлять методы в существующие классы. Lombok обходит эти ограничения через хаки javac internal API (com.sun.tools.javac), получая доступ к AST и модифицируя его напрямую. Это делает Lombok хрупким при обновлениях JDK.
...
|-------------|------|-------|
| Следующий шаг | [[jvm-reflection-api]] | Понять runtime-обработку аннотаций через reflection |
| Углубиться | [[jvm-bytecode-manipulation]] | Изучить альтернативу APT: модификацию байткода |
| Связанная тема | [[jvm-service-loader-spi]] | Понять механизм регистрации процессоров через META-INF/services |
| Смежная область | [[android-hilt-deep-dive]] | Увидеть annotation processing в действии: Hilt генерирует DI-код через APT |
