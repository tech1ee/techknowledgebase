...
Java Instrumentation Agents реализуют концепцию **программной инструментации** (software instrumentation) — автоматического добавления наблюдательного кода в программу без изменения её исходного текста.

> **Определение:** *Software instrumentation — техника внедрения дополнительного кода (probes) в программу для сбора информации о её поведении во время выполнения.*

| Теоретическая концепция | Автор / Источник | Применение в Java Agents |
|------------------------|-----------------|-------------------------|
| **Software instrumentation** | Ball & Larus, 1994 | Внедрение probes для профилирования и мониторинга → ClassFileTransformer |
| **Aspect-Oriented Programming** | Kiczales et al., 1997 | Cross-cutting concerns (логирование, метрики) → agent как аспект |
| **Bytecode weaving** | AspectJ, 2001 | Модификация байткода при загрузке → load-time weaving через agents |
| **Dynamic binary instrumentation** | Nethercote & Seward (Valgrind), 2007 | Инструментация работающей программы → dynamic attach через Attach API |
| **Observability** | Distributed systems, 2010-е | Traces, metrics, logs → APM agents (Datadog, New Relic) |

> **Ключевой принцип (java.lang.instrument):** Agent перехватывает загрузку классов через `ClassFileTransformer` и модифицирует байткод **до** его выполнения. Это позволяет добавлять метрики, трассировку и проверки без изменения исходного кода приложения и без перекомпиляции.

Два режима подключения: **premain** (agent загружается при старте JVM через `-javaagent`) и **agentmain** (agent подключается к работающей JVM через Attach API). Premain гарантирует инструментацию всех классов; agentmain позволяет подключать диагностику к production-системам на лету.

Связанные темы: [[jvm-bytecode-manipulation]] (ASM/ByteBuddy как инструменты трансформации внутри agent), [[jvm-class-loader-deep-dive]] (ClassFileTransformer перехватывает фазу Loading), [[jvm-profiling]] (agents как основа APM-инструментов).
...
- Ball T., Larus J. (1994). *Optimally Profiling and Tracing Programs*. — Формальные основы software instrumentation: методы эффективного размещения probes для минимизации overhead при максимальном покрытии.
- Kiczales G. et al. (1997). *Aspect-Oriented Programming*. — AOP как теоретический фундамент agent-based инструментации: разделение cross-cutting concerns и бизнес-логики.
- Lindholm T. et al. (2014). *The Java Virtual Machine Specification*, Chapter 5: Loading, Linking, and Initializing. — Процесс загрузки классов, в который встраивается ClassFileTransformer.
- Oracle (2004). *JSR-163: Java Platform Profiling Architecture*. — Спецификация, которая ввела `java.lang.instrument` и JVMTI в Java 5.

### Практические руководства

- Winterhalter R. (2020). *The Definitive Guide to Java Agents*. infoq.com. — Практическое руководство от создателя ByteBuddy по разработке agent'ов с AgentBuilder и Advice API.
- Oaks S. (2020). *Java Performance*, 2nd Edition. — Главы о мониторинге и диагностике: использование agent'ов для сбора метрик и их влияние на JIT-компиляцию.
- Venners B. (2000). *Inside the Java Virtual Machine*, 2nd Edition. — Подробное описание class loading и верификации байткода, перехватываемых agent'ами.
