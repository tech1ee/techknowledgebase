...
> [!question]- Почему ConcurrentHashMap не допускает null для ключей и значений, в отличие от HashMap?
> get(key) возвращает null, когда ключа нет. Если null-значения разрешены, невозможно атомарно отличить "ключ отсутствует" от "ключ есть, значение null". В однопоточном HashMap можно вызвать containsKey() отдельно, но в многопоточном между containsKey() и get() другой поток может изменить map.

> [!question]- Сценарий: команда заменила HashMap на ConcurrentHashMap, но race condition остался. Код: if (!map.containsKey(key)) map.put(key, value). Почему замена коллекции не помогла?
> ConcurrentHashMap гарантирует потокобезопасность отдельных операций, но не составных. Между containsKey() и put() другой поток может вставить значение. Решение: использовать атомарную операцию putIfAbsent(key, value), которая выполняет проверку и вставку как одно неделимое действие.

> [!question]- Почему CopyOnWriteArrayList идеален для слушателей событий, но катастрофичен для частых записей?
> Каждый add/remove создаёт полную копию внутреннего массива (O(n)). Для слушателей событий: добавление редко (при инициализации), итерация постоянно (при каждом событии), итератор работает по snapshot без блокировок. Для частых записей: 1000 add в список из 10000 элементов = 10 миллионов копирований.

> [!question]- Сценарий: при нагрузке 10000 req/s producer заполняет BlockingQueue быстрее, чем consumer обрабатывает. Память растёт до OutOfMemoryError. Какой тип очереди использовался и как исправить?
> Использовалась LinkedBlockingQueue без явного размера (лимит Integer.MAX_VALUE). Решение: заменить на ArrayBlockingQueue(N) с явным размером и использовать put() для backpressure -- producer заблокируется, когда очередь полная, автоматически замедляя приём запросов.

---

## Ключевые карточки

Как ConcurrentHashMap в Java 8 обеспечивает параллелизм?
?
CAS для вставки в пустые bucket'ы (без блокировки), synchronized на первом узле для непустых bucket'ов. Каждый bucket -- отдельная точка синхронизации. Чтение полностью lock-free через volatile.

Что такое treeification в ConcurrentHashMap?
?
При > 8 элементах в одном bucket связный список преобразуется в красно-чёрное дерево. Сложность поиска меняется с O(n) на O(log n). Защищает от HashDoS-атак. Обратное преобразование при < 6 элементах.

Чем SynchronousQueue отличается от других BlockingQueue?
?
Размер всегда 0, ничего не хранит. put() блокируется до вызова take() другим потоком -- прямая передача из рук в руки. Используется в CachedThreadPool: задача либо передаётся свободному потоку, либо создаётся новый.

В чём разница ArrayBlockingQueue и LinkedBlockingQueue?
?
ArrayBlockingQueue: один lock на put и take, фиксированный размер, предсказуемое потребление памяти. LinkedBlockingQueue: раздельные locks для put и take (выше пропускная способность), больше потребление памяти из-за узлов связного списка.

Почему computeIfAbsent может вызвать deadlock?
?
Лямбда внутри computeIfAbsent выполняется под lock на bucket. Если лямбда обращается к другому bucket той же ConcurrentHashMap, может возникнуть deadlock. Лямбда должна быть быстрой и без побочных эффектов.

Что такое weakly consistent итератор?
?
Итератор видит состояние на момент создания, но может пропустить параллельные изменения. Не бросает ConcurrentModificationException. Используется в ConcurrentHashMap и CopyOnWriteArrayList. Для строгого snapshot нужно копировать в обычную коллекцию.
...
|-------------|------|-------|
| Следующий шаг | [[jvm-executors-futures]] | ThreadPoolExecutor использует BlockingQueue внутри |
| Углубиться | [[jvm-synchronization]] | CAS, volatile, locks -- строительные блоки concurrent collections |
| Связанная тема | [[java-modern-features]] | Virtual Threads меняют подход к concurrency |
| Смежная область | [[kotlin-collections]] | Коллекции Kotlin и их потокобезопасность на JVM |
