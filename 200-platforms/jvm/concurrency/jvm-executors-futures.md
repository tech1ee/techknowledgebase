...
> [!question]- Почему ForkJoinPool.commonPool() опасен для I/O-задач, и как это связано с parallel streams?
> commonPool имеет размер N_cores - 1. Если все потоки заблокированы на I/O (HTTP-запросы, JDBC), commonPool парализуется. Parallel streams тоже используют commonPool, поэтому I/O-блокировка в CompletableFuture.supplyAsync без явного executor замораживает и parallel streams. Решение: всегда передавать отдельный executor для I/O-задач.

> [!question]- Сценарий: веб-сервис обрабатывает 5000 запросов/сек, каждый запрос делает HTTP-вызов (200мс ожидания, 10мс вычислений). Сервер имеет 8 ядер. Как рассчитать размер пула потоков и какой тип executor выбрать?
> По формуле Little's Law: 8 * (1 + 200/10) = 168 потоков. Это I/O-bound задача, подойдёт FixedThreadPool. Но лучше использовать ThreadPoolExecutor с bounded queue и CallerRunsPolicy для backpressure. С Java 21 оптимальнее Virtual Threads: по одному на запрос без пула вообще.

> [!question]- Почему CallerRunsPolicy часто лучше AbortPolicy в production?
> CallerRunsPolicy создаёт естественный backpressure: при перегрузке вызывающий поток сам выполняет задачу, замедляя приём новых запросов. AbortPolicy бросает RejectedExecutionException, требуя явной обработки. CallerRunsPolicy не теряет данные и не требует catch-блоков, а замедление producer'а предотвращает каскадную перегрузку.

> [!question]- Сценарий: после миграции на Virtual Threads приложение стало медленнее. Код активно использует synchronized блоки. Почему это произошло и как исправить?
> Virtual threads при входе в synchronized блок "закрепляются" (pinning) на carrier thread и не могут быть отсоединены. Если много virtual threads блокируются на synchronized, carrier threads исчерпываются. Решение: заменить synchronized на java.util.concurrent.locks.ReentrantLock, который не вызывает pinning.

---

## Ключевые карточки

Какой оптимальный размер пула для CPU-bound задач?
?
N_cores + 1. Дополнительный поток компенсирует page fault или GC pause. Больше потоков ухудшает производительность из-за context switching.

Чем отличается scheduleAtFixedRate от scheduleWithFixedDelay?
?
scheduleAtFixedRate отсчитывает интервал от начала предыдущего запуска (для метрик). scheduleWithFixedDelay -- от завершения предыдущего запуска (для синхронизации). При scheduleAtFixedRate задача не запускается параллельно с предыдущей.

Что такое work stealing в ForkJoinPool?
?
У каждого потока своя deque задач. Свободный поток "крадёт" задачу из хвоста deque занятого соседа (FIFO). Владелец берёт из головы (LIFO). Разные концы deque минимизируют конкуренцию.

Почему исключения в submit() "проглатываются"?
?
submit() оборачивает исключение в Future. Если никто не вызовет get()/join(), ошибка потеряна навсегда. execute() печатает исключение в stderr. Решение: всегда обрабатывать Future или переопределить afterExecute().

Когда Virtual Threads не дают преимущества?
?
Для CPU-bound задач (virtual thread не может отсоединиться от carrier при непрерывных вычислениях), при использовании synchronized (pinning), при ThreadLocal-кэшировании (миллион копий). Для CPU-bound используйте ForkJoinPool.

Что делает CompletableFuture.exceptionally()?
?
Аналог catch для асинхронного кода. Перехватывает исключение из предыдущего шага цепочки и возвращает fallback-значение. Без exceptionally/handle исключение запаковывается в CompletableFuture и молчит.

Почему Executors.newFixedThreadPool опасен в production?
?
Использует LinkedBlockingQueue без лимита (Integer.MAX_VALUE). При перегрузке очередь растёт бесконечно до OutOfMemoryError. Безопаснее: явный ThreadPoolExecutor с ArrayBlockingQueue и rejection policy.
...
|-------------|------|-------|
| Следующий шаг | [[jvm-concurrent-collections]] | Понять BlockingQueue, на которой построен ThreadPoolExecutor |
| Углубиться | [[java-modern-features]] | Virtual Threads и Structured Concurrency как эволюция executors |
| Связанная тема | [[jvm-synchronization]] | Примитивы синхронизации внутри executor framework |
| Смежная область | [[kotlin-coroutines]] | Корутины как альтернативный подход к асинхронности на JVM |
