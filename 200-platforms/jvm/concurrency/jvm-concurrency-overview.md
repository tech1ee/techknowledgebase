...
Concurrent programming опирается на формальную теорию параллельных процессов, разработанную в 1960--1970-х годах и развиваемую по сей день.

> **Определение (Dijkstra, 1965):** *Concurrent programs — множество последовательных процессов, которые взаимодействуют через разделяемые ресурсы и должны координировать доступ к этим ресурсам для обеспечения корректности.*

| Теоретическая концепция | Автор / Источник | Значение для JVM |
|------------------------|-----------------|-----------------|
| **Семафоры** | Dijkstra, 1965 | Первый формальный примитив синхронизации; `java.util.concurrent.Semaphore` |
| **Мониторы** | Hoare, 1974; Hansen, 1973 | Mutex + condition variable в одной конструкции → `synchronized` + `wait()`/`notify()` |
| **CSP** (Communicating Sequential Processes) | Hoare, 1978 | Модель «процессы обмениваются сообщениями через каналы» → Go channels, Kotlin channels |
| **Actor model** | Hewitt, 1973 | Модель «акторы обмениваются асинхронными сообщениями» → Akka, Erlang/OTP |
| **Linearizability** | Herlihy & Wing, 1990 | Формальный критерий корректности concurrent-структур данных |
| **Закон Амдала** | Amdahl, 1967 | Предел ускорения: последовательная доля ограничивает параллелизм |

> **Java Memory Model (JSR-133, Manson, Pugh & Adve, 2005):** формальная спецификация, определяющая *happens-before* — отношение порядка между операциями в разных потоках. Без JMM невозможно определить, когда запись одного потока видна чтению другого. JMM — контракт между программистом и JVM: программист использует `synchronized`/`volatile`, JVM гарантирует видимость и порядок.

Формализация **lock-free progress** (Herlihy, 1991) ввела иерархию гарантий прогресса: **wait-free** (каждый поток завершает операцию за конечное число шагов), **lock-free** (хотя бы один поток делает прогресс), **obstruction-free** (поток делает прогресс в отсутствие contention). `AtomicInteger` — lock-free, `LongAdder` — практически wait-free при достаточном числе ячеек.

Связанные темы: [[jvm-synchronization]] (примитивы: synchronized, volatile, Atomic*), [[jvm-memory-model]] (JMM и happens-before), [[kotlin-coroutines]] (кооперативная многозадачность поверх JVM threads).
...
- **Dijkstra E.W. (1965). Cooperating Sequential Processes.** — формализация mutual exclusion, семафоры, проблема обедающих философов.
- **Hoare C.A.R. (1974). Monitors: An Operating System Structuring Concept.** — мониторы: mutex + condition variable → основа `synchronized`.
- **Hoare C.A.R. (1978). Communicating Sequential Processes.** — CSP: процессы обмениваются сообщениями через каналы.
- **Herlihy M., Wing J. (1990). Linearizability: A Correctness Condition for Concurrent Objects.** — формальный критерий корректности concurrent-структур.
- **Manson J., Pugh W., Adve S. (2005). The Java Memory Model.** — JSR-133: формальная спецификация happens-before для JVM.
- **Lu S. et al. (2008). Learning from Mistakes.** — анализ 97% реальных concurrency-багов: четыре категории.

### Практические руководства

- **Goetz B. et al. (2006). Java Concurrency in Practice.** — каноническое руководство по Java concurrency.
- **Herlihy M., Shavit N. (2012). The Art of Multiprocessor Programming.** — теория lock-free алгоритмов и linearizability.
- **Lea D. (2000). Concurrent Programming in Java, 2nd ed.** — design rationale пакета java.util.concurrent.
- **Pike R. (2012). Concurrency is not Parallelism (talk).** — каноническое объяснение разницы concurrency vs parallelism.
