...
> [!question]- Почему LongAdder в 100 раз быстрее AtomicLong при high contention, и в каком случае LongAdder не подходит?
> AtomicLong при high contention вызывает CAS contention: 1000 потоков делают CAS, успевает только один, 999 делают retry. CPU тратит циклы на бесполезные попытки. LongAdder использует striped counters -- массив ячеек по разным cache lines. Каждый поток пишет в свою ячейку, конфликтов почти нет. Не подходит для точных мгновенных чтений: sum() не атомарен, пока суммируем, кто-то может добавить в ячейку.

> [!question]- У вас два потока захватывают lockA и lockB в разном порядке, вызывая deadlock. Какие стратегии предотвращения вы рассмотрите?
> Три стратегии, каждая разрывает одно из условий Коффмана: (1) Lock ordering -- всегда захватывать lockA перед lockB во всей программе (разрывает circular wait). (2) tryLock с timeout через ReentrantLock -- если не удалось захватить второй lock за 100ms, отпустить первый и попробовать позже (разрывает hold and wait). (3) Один глобальный lock вместо двух (разрывает circular wait, ценой параллелизма).

> [!question]- Почему synchronized внутри virtual thread создаёт проблему pinning, и каково решение?
> synchronized привязывает virtual thread к carrier thread (platform thread) -- pinning. Пока virtual thread внутри synchronized, carrier thread не может обслуживать другие virtual threads. Если внутри synchronized есть I/O -- carrier thread блокируется на уровне ОС. Решение: использовать ReentrantLock вместо synchronized, потому что ReentrantLock корректно поддерживает unmounting virtual thread при блокировке.

> [!question]- Объясните, почему синхронизация на String литерале опасна и как это исправить.
> String interning может привести к тому, что два независимых модуля синхронизируются на одном и том же объекте String. Например, synchronized("lock") в двух разных компонентах будет использовать один и тот же interned String объект -- неожиданная взаимная блокировка. Решение: private final Object lock = new Object() -- гарантированно уникальный объект для синхронизации.

---

## Ключевые карточки

Чем отличаются synchronized, volatile и Atomic по гарантиям?
?
synchronized: mutual exclusion + visibility + atomicity (блокирует поток). volatile: visibility + запрет reordering, но НЕ atomicity (count++ -- race condition). Atomic*: visibility + atomicity через CAS (lock-free, без блокировки). Правило: используйте наименее мощный инструмент, который решает задачу.

Что такое CAS (Compare-And-Swap) и как работает AtomicInteger?
?
CAS -- аппаратная инструкция CPU (CMPXCHG на x86): атомарно сравнивает значение с ожидаемым и заменяет на новое, если совпало. AtomicInteger.incrementAndGet() -- retry loop: читает текущее значение, вычисляет новое, делает CAS. Если CAS failed (другой поток успел изменить) -- пробует снова.

Когда ReentrantLock предпочтительнее synchronized?
?
Когда нужен tryLock (попытка без блокировки), timeout (ограниченное ожидание), interruptible waiting (прерываемое ожидание), fair mode (FIFO порядок), или при работе с virtual threads (нет pinning). Цена: обязательный unlock() в finally, иначе deadlock.

Что такое ABA problem и как её решить?
?
CAS проверяет только текущее значение, не историю. Значение может измениться A->B->A, CAS не заметит. Опасно для lock-free структур данных: ссылка "та же", но объект уже другой. Решение: AtomicStampedReference -- CAS проверяет и значение, и версионный stamp.

Какие четыре условия Коффмана необходимы для deadlock?
?
(1) Mutual exclusion -- ресурсы эксклюзивны. (2) Hold and wait -- поток держит ресурсы, ожидая другие. (3) No preemption -- ресурсы нельзя отобрать. (4) Circular wait -- циклическая зависимость. Разорвите любое -- deadlock невозможен.
...
|-------------|------|-------|
| Следующий шаг | [[jvm-concurrent-collections]] | Concurrent-коллекции построены на примитивах из этого файла |
| Углубиться | [[jvm-executors-futures]] | Высокоуровневые абстракции, заменяющие ручную синхронизацию |
| Связанная тема | [[jvm-concurrency-overview]] | Общая карта concurrency: потоки, состояния, JMM |
| Смежная область | [[synchronization-primitives]] | Синхронизация на уровне ОС -- mutex, semaphore, futex |
