...
Концепция виртуальной машины формализована в академической литературе как механизм абстракции ISA (Instruction Set Architecture):

> **Определение:** «A virtual machine is an efficient, isolated duplicate of a real machine.» — Popek G.J., Goldberg R.P., *Formal Requirements for Virtualizable Third Generation Architectures*, Communications of the ACM, 1974.

Popek и Goldberg (1974) сформулировали три требования к виртуализации: **эквивалентность** (программа работает так же, как на реальной машине), **управление ресурсами** (VM-монитор контролирует все ресурсы), **эффективность** (большинство инструкций выполняются напрямую на hardware). Эти требования определили таксономию, которую Smith & Nair (2005) расширили до двух основных классов:

| Класс VM | Примеры | Что виртуализирует | Granularity |
|----------|---------|-------------------|-------------|
| **System VM** | VMware, KVM, Xen, VirtualBox | Весь компьютер (hardware ISA) | Полная ОС |
| **Process VM** | JVM, CLR, V8, CPython | Один процесс (application ISA) | Одна программа |

JVM относится к process VM с **высокоуровневым ISA**: байткод JVM --- не эмуляция реального процессора, а абстрактный набор инструкций, спроектированный для объектно-ориентированных языков.

**Стековая vs регистровая архитектура.** VM делятся на стековые (операнды на стеке: JVM, CLR, P-code) и регистровые (операнды в виртуальных регистрах: Dalvik/ART, Lua VM). Стековая архитектура даёт более компактный байткод (Davis et al., 2003), но требует больше инструкций на операцию. JVM выбрала стековую модель для минимизации размера class-файлов --- критично в 1995 году при передаче апплетов через dial-up.

| Свойство | Стековая (JVM, CLR) | Регистровая (Dalvik, Lua) |
|----------|---------------------|---------------------------|
| Размер инструкции | 1-3 байта | 2-4 байта |
| Количество инструкций | Больше (push/pop overhead) | Меньше (прямые операнды) |
| Размер байткода | Компактнее на ~25% | Крупнее, но меньше dispatch |
| Производительность интерпретации | Медленнее (memory traffic) | Быстрее (меньше loads/stores) |

Исторические корни: P-code (Wirth, ETH Zürich, 1966) для Pascal --- первая широко используемая process VM с промежуточным представлением. UCSD Pascal (1978) сделал P-code переносимым. Smalltalk-80 VM (Goldberg & Robson, 1983) ввела GC и динамическую диспетчеризацию. Java VM (1995) объединила эти идеи с [[jvm-jit-compiler|JIT-компиляцией]] и формальной [[jvm-memory-model|моделью памяти]], создав платформу, на которой работают [[jvm-languages-ecosystem|несколько языков]].
...
- Popek G.J., Goldberg R.P. (1974). *Formal Requirements for Virtualizable Third Generation Architectures.* Communications of the ACM 17(7). --- Формальные требования к виртуализации (эквивалентность, управление ресурсами, эффективность), заложившие теоретический фундамент всех VM.
- Smith J.E., Nair R. (2005). *Virtual Machines: Versatile Platforms for Systems and Processes.* --- Каноническая таксономия: system VM vs process VM, высокоуровневый vs низкоуровневый ISA. Лучший академический источник по теме.
- Craig I. (2006). *Virtual Machines.* --- Систематическое сравнение виртуальных машин: от P-code до JVM и CLR. Исторический контекст и эволюция идей.
- Davis B., Beatty A., Casey K. et al. (2003). *The Case for Virtual Register Machines.* --- Сравнение стековых и регистровых VM; обоснование выбора архитектуры.

### Практические руководства

- Lindholm T. et al. (2014). *The Java Virtual Machine Specification, Java SE 8 Edition.* --- Официальная спецификация JVM: набор инструкций байткода, формат class-файла, верификация и модель памяти.
- Venners B. (2000). *Inside the Java Virtual Machine.* --- Одна из первых книг о внутренностях JVM. Фундаментальные концепции актуальны до сих пор.
...
> [!question]- Почему JVM выбрала стековую, а не регистровую архитектуру?
> В 1995 году байткод загружался через медленные модемы, и компактность была критична. Стековые инструкции не указывают регистры-источники, занимая 1 байт вместо 3-4. После JIT-компиляции разница исчезает, поскольку JIT транслирует байткод в регистровый машинный код.

> [!question]- Сервис стартует за 50 мс на GraalVM Native Image, но через час показывает throughput на 20% ниже, чем при обычном запуске HotSpot. Почему?
> Native Image использует AOT-компиляцию без runtime profiling. HotSpot JIT после warmup применяет profile-guided оптимизации (speculative inlining, escape analysis), недоступные при статической компиляции. Для долгоживущих сервисов JIT обычно даёт лучший пиковый throughput.

> [!question]- Чем контейнер Docker отличается от системной виртуальной машины на фундаментальном уровне?
> System VM эмулирует полный компьютер через гипервизор и содержит собственное ядро ОС. Docker использует cgroups и namespaces для изоляции процессов, разделяя ядро хоста. Контейнеры легче и быстрее стартуют, но не обеспечивают уровень изоляции VM.

> [!question]- Почему BEAM VM (Erlang) может обслуживать миллионы пользователей WhatsApp, хотя её вычислительная производительность в 3-10 раз ниже JVM?
> BEAM спроектирована вокруг лёгких процессов с изолированной памятью и message passing. Каждый процесс имеет собственный GC, а сбой одного не затрагивает другие. Для IO-bound задач (messaging) отказоустойчивость и масштабируемость важнее чистой вычислительной скорости.

---

## Ключевые карточки

Что такое Process VM?
?
Виртуальная машина, создающая виртуальный процессор для выполнения байткода одного приложения (JVM, CLR, V8). Работает как обычный процесс внутри ОС, не требует отдельной ОС.

Чем стековая архитектура JVM отличается от регистровой (Dalvik)?
?
В стековой архитектуре операнды берутся неявно с вершины стека (инструкция 1 байт). В регистровой явно указываются регистры (инструкция 2-6 байт). Стековый байткод компактнее на 25-30%, но интерпретируется медленнее.

Зачем нужен байткод вместо компиляции напрямую в машинный код?
?
Байткод обеспечивает кроссплатформенность (один артефакт для всех ОС), возможность runtime-оптимизации (JIT profile-guided), верификацию безопасности перед выполнением и многоязыковую платформу (Kotlin, Scala компилируются в тот же байткод).

Что такое JIT-компиляция и почему она может быть быстрее AOT?
?
JIT компилирует байткод в машинный код во время выполнения. В отличие от AOT, JIT наблюдает за реальным поведением программы и оптимизирует горячие пути на основе runtime-профиля, что недоступно статическим компиляторам.

Какие четыре причины делают байткод лучше прямой компиляции?
?
Кроссплатформенность (одна сборка для всех ОС), profile-guided оптимизация в runtime, верификация безопасности (проверка перед выполнением), многоязыковая платформа (любой язык с валидным байткодом получает GC, JIT, библиотеки).

Чем System VM отличается от Process VM?
?
System VM (VMware, KVM) эмулирует целый компьютер с ОС, управляется гипервизором, требует гигабайты памяти. Process VM (JVM, CLR) эмулирует процессор для одного приложения, работает как процесс ОС, требует мегабайты.

Что такое P-code machine и почему она важна для истории JVM?
?
P-code machine (1970-е, Niklaus Wirth) -- первая реализация концепции промежуточного кода для языка Pascal. Компилятор генерировал P-code, а интерпретатор на каждой платформе выполнял его. Это прямой предок всех процессных VM, включая JVM.
...
|-------------|------|-------|
| Следующий шаг | [[jvm-basics-history]] | История JVM объясняет, почему VM устроена именно так |
| Углубиться | [[jvm-class-loader-deep-dive]] | Механизм загрузки классов -- ключевая подсистема JVM |
| Смежная тема | [[os-virtualization]] | Системная виртуализация и контейнеры -- другая ветвь виртуализации |
| Обзор | [[jvm-overview]] | Вернуться к карте раздела |

---

*Проверено: 2026-02-13 | Источники: Smith & Nair (2005) Virtual Machines, Lindholm et al. JVM Specification, Craig (2006) Virtual Machines*
