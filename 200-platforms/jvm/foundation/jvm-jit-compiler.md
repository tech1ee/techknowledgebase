...
JIT-компиляция опирается на фундаментальные результаты теории компиляторов, профильной оптимизации и динамической адаптации программ.

> **Определение:** *Just-In-Time compilation — техника трансляции промежуточного представления (байткода) в машинный код целевой платформы во время выполнения программы, с использованием информации о реальном профиле исполнения для управления оптимизациями.*

Теоретический фундамент JIT охватывает несколько ключевых концепций:

| Концепция | Автор / Источник | Суть |
|-----------|-----------------|------|
| **Adaptive optimization** | Hölzle & Ungar, 1994 (Self VM) | Компилятор наблюдает за поведением программы и перекомпилирует горячий код с учётом профиля |
| **Type feedback** | Chambers & Ungar, 1989 | Информация о типах, собранная в runtime, используется для девиртуализации и speculative inlining |
| **Profile-Guided Optimization (PGO)** | Chang et al., 1991 | Оптимизации, управляемые данными о реальном исполнении, а не статическим анализом |
| **Partial evaluation** | Futamura, 1971 | Специализация программы по известным входным данным — теоретическая основа Truffle/Graal |
| **Sea of Nodes IR** | Click & Paleczny, 1995 | Промежуточное представление C2: граф, объединяющий control flow и data flow в единую структуру |

Формально, JIT реализует **проекцию Футамуры** (Futamura projection, 1971): интерпретатор байткода, специализированный по конкретной программе, эквивалентен скомпилированной версии этой программы. GraalVM Truffle доводит эту идею до практической реализации: AST-интерпретатор языка + partial evaluator Graal = JIT-компилятор для любого языка.

> **Ключевое наблюдение (Hölzle, 1994):** *"Dynamic compilation need not be inferior to static compilation; runtime information enables optimizations impossible for any static compiler."* — Адаптивная компиляция не уступает статической, поскольку runtime-информация позволяет применять оптимизации, невозможные для статического компилятора.

Escape Analysis основана на теории **data flow analysis** (Kildall, 1973) и формализована для Java в работе **Choi et al. (1999)** *"Escape Analysis for Java"*. Анализ строит **connection graph** — граф связей между объектами и ссылками — и определяет, достижим ли объект из-за пределов метода или потока.

Speculative optimization и deoptimization формализованы в контексте **optimistic compilation** (Chambers, 1992): компилятор генерирует код на основе текущих наблюдений, вставляя guard-проверки для валидации предположений. При нарушении предположения происходит **on-stack replacement** (OSR) — замена скомпилированного фрейма на интерпретируемый прямо во время выполнения.

Связанные темы: [[jvm-virtual-machine-concept]] (стековый байткод как входные данные JIT), [[jvm-benchmarking-jmh]] (корректное измерение с учётом JIT), [[jvm-profiling]] (связь профилирования и JIT-оптимизаций).
...
- **Chambers C., Ungar D. (1989). Customization: Optimizing Compiler Technology for Self.** — основы adaptive compilation: type feedback, speculative optimization, ставшие фундаментом HotSpot JIT.
- **Futamura Y. (1971). Partial Evaluation of Computation Process.** — теоретическая основа JIT через проекции Футамуры: специализация интерпретатора по программе.
- **Choi J.-D. et al. (1999). Escape Analysis for Java.** — формализация escape analysis через connection graph, определяющий достижимость объектов за пределами метода.
- **Click C., Paleczny M. (1995). A Simple Graph-Based Intermediate Representation.** — Sea of Nodes IR, используемый в C2 HotSpot.
- **Paleczny M. et al. (2001). The Java HotSpot Server Compiler.** — архитектура C2: graph-based оптимизации и design decisions.
- **Würthinger T. et al. (2013). One VM to Rule Them All.** — архитектура GraalVM и Truffle: partial evaluation для поддержки множества языков.

### Практические руководства

- **Oaks S. (2014). Java Performance: The Definitive Guide.** — практическое руководство по JIT tuning: tiered compilation, inlining, escape analysis.
- [JITWatch](https://github.com/AdoptOpenJDK/jitwatch) — визуализация JIT-компиляции: что inline'ено, что скомпилировано, где deoptimization
...
> [!question]- Почему JVM не компилирует код сразу на Level 4 (C2), а использует многоуровневую tiered compilation?
> C2 нуждается в данных профилирования для эффективной работы: type profiles, branch profiles, exception profiles. Без этих данных C2 не может делать speculative inlining и оптимизацию layout кода. Level 3 (C1 с профилированием) -- необходимый этап сбора данных, а не просто промежуточный шаг. Кроме того, C1 даёт быстрый буст (10x к интерпретатору), пока C2 собирает достаточно статистики.

> [!question]- У вас сервис на Spring Boot, и p99 latency первых 30 секунд после деплоя в 5 раз хуже, чем после прогрева. Какие стратегии ускорения warmup вы рассмотрите?
> Три подхода: (1) снизить пороги компиляции (Tier3InvocationThreshold, Tier4InvocationThreshold) для более раннего JIT; (2) прогнать синтетическую нагрузку перед приёмом production-трафика (pre-warming); (3) использовать CDS для ускорения загрузки классов. Для serverless -- рассмотреть GraalVM Native Image или AWS SnapStart. Выбор зависит от trade-off: снижение порогов даёт менее оптимальную C2-компиляцию.

> [!question]- Объясните, как inlining и escape analysis работают синергически для устранения allocation.
> Inlining подставляет тело вызываемого метода в вызывающий код. После этого escape analysis может проанализировать весь объединённый код и определить, что объект, созданный внутри inline-кода, не покидает метод (NoEscape). Тогда JIT применяет scalar replacement -- разбирает объект на отдельные переменные. Без inlining JIT не мог бы увидеть, что объект не "убегает" через вызываемый метод.

> [!question]- Почему наивный бенчмарк без JMH может показать время выполнения 0 наносекунд?
> JIT применяет dead code elimination: если результат вычисления не используется (не выводится, не сохраняется, не возвращается), JIT удаляет всё вычисление целиком, включая циклы и создание объектов. JMH решает это через Blackhole -- объект, который "потребляет" значения, не давая JIT их устранить.
...
?
Многоуровневая компиляция в HotSpot: Level 0 (Interpreter), Level 1 (C1 без профилирования, тривиальные методы), Level 2 (C1 с ограниченным профилированием), Level 3 (C1 с полным профилированием, ~2000 вызовов), Level 4 (C2 с агрессивными оптимизациями, ~15000 вызовов).

Что такое escape analysis и какие оптимизации он позволяет?
?
Анализ, определяющий "убегает" ли объект из метода. Три уровня: NoEscape, ArgEscape, GlobalEscape. Для NoEscape-объектов JIT может применить scalar replacement (разбить на переменные), stack allocation (через scalar replacement) и lock elision (убрать ненужную синхронизацию).

Чем monomorphic call site отличается от megamorphic?
?
Monomorphic -- виртуальный метод вызывается всегда на объектах одного типа, JIT может inline конкретную реализацию. Megamorphic -- три и более типа, inlining невозможен, используется virtual dispatch через vtable. Megamorphic в 3-5 раз медленнее.

Что такое deoptimization и когда она происходит?
?
Откат скомпилированного кода обратно в интерпретатор, когда speculative assumption JIT оказалось неверным. Причины: загрузка нового подкласса (class hierarchy change), ветвь-которая-раньше-не-выполнялась (unstable if), null-ссылка. После deopt метод перекомпилируется с учётом нового поведения.

Что такое GraalVM Native Image и в чём его trade-off по сравнению с JIT?
?
AOT-компиляция Java в standalone native binary. Плюсы: мгновенный старт (<50ms), минимальное потребление памяти, без warmup. Минусы: потеря runtime-оптимизаций, все классы должны быть известны при компиляции, reflection требует конфигурации, dynamic class loading невозможен.

Почему маленькие методы в Java не имеют overhead вызова?
?
JIT inline маленькие методы (до 35 байт байткода, до 325 для частых), подставляя тело метода в вызывающий код. Это устраняет overhead вызова и открывает каскад оптимизаций: constant propagation, constant folding, dead code elimination.
...
|-------------|------|-------|
| Следующий шаг | [[jvm-benchmarking-jmh]] | Научиться правильно измерять производительность с учётом warmup и JIT |
| Углубиться | [[jvm-profiling]] | Найти горячие методы и понять, как JIT их обработал |
| Связанная тема | [[jvm-performance-overview]] | Общая карта оптимизации JVM -- JIT в контексте GC и I/O |
| Смежная область | [[native-compilation-llvm]] | Как работает AOT-компиляция в других языках через LLVM |
