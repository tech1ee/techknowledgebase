...
> [!question]- Почему Гослинг выбрал стековую, а не регистровую модель для байткода --- и какой компонент JVM делает эту "медленную" архитектуру конкурентоспособной по скорости?
> Стековая модель давала компактный байткод (инструкции без указания регистров --- 1 байт вместо 3--4) и простоту портирования на любую архитектуру (не зависит от количества регистров процессора). В 1995 году это было критично: байткод грузился по модему 14.4 Kbps. Платой стала медленная интерпретация (больше обращений к памяти), но JIT-компилятор (появившийся в HotSpot) полностью компенсирует это: он транслирует байткод в регистровый машинный код конкретного процессора, используя profile-guided optimization.

> [!question]- В Java 5 для реализации generics выбрали type erasure вместо reified generics. Какую проблему это решило и какие ограничения создало? Как Kotlin решает эту же проблему на той же JVM?
> Type erasure сохранила backward compatibility: старый код, написанный до Java 5, продолжал работать без изменений, потому что в байткоде `List<String>` и `List<Integer>` --- одинаковые `List`. Но это создало ограничения: невозможно проверить generic-тип в runtime (`instanceof List<String>` невозможен), невозможно создать `new T()`. Kotlin на той же JVM обходит проблему через `reified` type parameters в inline-функциях: компилятор подставляет конкретный тип на этапе компиляции, что даёт доступ к типу в runtime без нарушения совместимости байткода.

> [!question]- Netflix перешёл на Virtual Threads (Java 21) и снизил ошибки с 2000/сек до 100/сек. Почему обычные потоки ОС создавали проблему, и как архитектурно Virtual Threads её решают?
> Каждый поток ОС занимает ~1 MB стека и требует переключения контекста через kernel. При 10 000 одновременных соединений --- 10 GB только на стеки, плюс накладные расходы на переключение. Virtual Threads управляются JVM, а не ОС: тысячи виртуальных потоков мультиплексируются на нескольких потоках ОС (carrier threads). При блокирующей операции виртуальный поток "снимается" с carrier thread, освобождая его для другой работы. Это позволяет масштабировать конкурентность без пропорционального роста потребления памяти.

> [!question]- Сравните подход Java (backward compatibility как "святое правило") и Python (Python 2 -> Python 3 breaking change). Какие конкретные архитектурные решения в JVM стали следствием этого принципа, и где этот подход создал технический долг?
> Backward compatibility означает, что код, скомпилированный для Java 1.0, работает на Java 21. Это породило: type erasure (вместо полноценных reified generics), сохранение `java.util.Date` (мутабельный, с ошибками дизайна) параллельно с `java.time`, raw types, checked exceptions. Python 3 сломал совместимость и мигрировал 15 лет, расколов сообщество. Java выбрала "добавлять, не ломая" --- enterprise доверяет платформе (можно обновить JDK без переписывания), но исторический багаж накапливается и усложняет язык.

> [!question]- Как связаны архитектура Android (ART/Dalvik) и классическая JVM? Почему Google не использовал стандартную JVM на мобильных устройствах, хотя Android-приложения пишутся на Java/Kotlin?
> Стандартная JVM (HotSpot) оптимизирована для серверов: JIT-компиляция требует прогрева и потребляет CPU и память. На мобильных устройствах с ограниченной батареей и RAM это неприемлемо. Google создал Dalvik (регистровая VM, не стековая --- более эффективна для ARM), а затем ART (Ahead-of-Time компиляция при установке). При этом Android использует тот же байткод-принцип (dex вместо class-файлов) и Java/Kotlin как языки, но с другой виртуальной машиной, оптимизированной под мобильные ограничения. Это показывает, что "платформенная независимость через байткод" --- универсальная идея, реализуемая по-разному.
...
?
Промежуточный код между исходным текстом и машинным кодом конкретного процессора. Обеспечивает платформенную независимость (WORA): один .class файл выполняется на любой ОС, где есть JVM.

Чем HotSpot отличается от "просто JVM"?
?
JVM --- это спецификация (набор правил). HotSpot --- конкретная реализация от Oracle с adaptive optimization: VM наблюдает за работающей программой и JIT-компилирует только горячие методы. Существуют и другие реализации: OpenJ9 (IBM), GraalVM, Azul Zing.

Что такое tiered compilation и какие компиляторы участвуют?
?
Двухуровневая компиляция в HotSpot: C1 (client) --- быстрая компиляция с базовыми оптимизациями для быстрого старта; C2 (server) --- агрессивная оптимизация горячих методов для максимальной скорости после прогрева. JVM автоматически выбирает уровень для каждого метода.

Почему JVM использует стековую, а не регистровую модель?
?
Три причины: (1) компактность байткода --- инструкции не указывают регистры, 1 байт вместо 3--4; (2) простота портирования --- не зависит от числа регистров процессора; (3) независимость от архитектуры --- x86 имеет 16 регистров, ARM --- 31, SPARC --- 128.

Что такое type erasure и почему Sun выбрала именно этот подход для generics в Java 5?
?
Type erasure --- стирание generic-типов из байткода после проверки компилятором. `List<String>` и `List<Integer>` в runtime одинаковы. Выбрано ради backward compatibility: миллионы строк существующего кода продолжили работать без изменений.

Что изменил переход на 6-месячный цикл релизов в 2017 году?
?
Вместо "большого релиза раз в 3--5 лет" --- небольшой релиз каждые полгода. Каждый третий год --- LTS-версия для enterprise. Это ускорило доставку фичей: между Java 6 и Java 7 прошло 5 лет, теперь фичи появляются в preview и стабилизируются за 1--2 цикла.

Чем Virtual Threads (Java 21) отличаются от обычных потоков ОС?
?
Обычный поток = поток ОС (~1 MB стека, переключение через kernel). Virtual Thread --- лёгкий поток, управляемый JVM: тысячи виртуальных потоков мультиплексируются на нескольких потоках ОС. Позволяет масштабировать конкурентность без пропорционального роста потребления памяти.

Почему Java Applets были революционны в 1995 году и какую проблему безопасности они создали?
?
Applets загружали код с сервера и выполняли в браузере --- первый интерактивный контент в вебе. Проблема: недоверенный код из интернета выполняется на машине пользователя. Решение --- Security Manager и sandbox, ограничивающие доступ к файлам, сети и процессам.

Назовите ключевое нововведение каждой из LTS-версий: Java 8, 11, 17, 21.
?
Java 8 --- lambda и Stream API (функциональное программирование). Java 11 --- первый LTS нового цикла, HTTP Client, удаление Java EE модулей. Java 17 --- sealed classes, records, pattern matching для instanceof. Java 21 --- Virtual Threads, pattern matching для switch.

Почему OpenJDK стал open source в 2006 году?
?
Не альтруизм: Sun теряла долю рынка и открыла код для привлечения контрибьюторов и ускорения развития. Сегодня OpenJDK развивается усилиями Oracle, Amazon, Red Hat, SAP, Azul и других. Все основные бесплатные дистрибутивы (Corretto, Temurin, Zulu) собираются из OpenJDK.
...
|-------------|------|-------|
| Следующий шаг | [[jvm-memory-model]] | Понять, как устроены Heap, Stack и Metaspace --- куда попадают объекты, созданные вашим кодом |
| Углубление | [[jvm-class-loader-deep-dive]] | Разобрать механизм ленивой загрузки классов --- архитектурное решение из Green Project |
| Углубление | [[jvm-jit-compiler]] | Детально изучить tiered compilation, warmup и profile-guided optimization |
| Практика | [[jvm-gc-tuning]] | Научиться выбирать и настраивать GC (G1, ZGC, Shenandoah) для конкретных задач |
| Кросс-домен | [[bytecode-virtual-machines]] | Сравнить JVM с другими виртуальными машинами (CLR, V8, BEAM) --- общие принципы и различия |
| Кросс-домен | [[android-compilation-pipeline]] | Увидеть, как те же идеи (байткод, VM) реализованы в Android: .dex, ART, AOT vs JIT |
| Языки | [[kotlin-basics]] | Перейти к Kotlin --- современному языку на JVM с null-safety, coroutines и лаконичным синтаксисом |
