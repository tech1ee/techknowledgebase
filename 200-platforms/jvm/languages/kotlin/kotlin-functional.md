...
> [!question]- Почему inline функции устраняют overhead лямбд и когда inline НЕ стоит использовать?
> Каждая лямбда в Kotlin компилируется в анонимный класс с методом invoke(), что создаёт аллокацию объекта в heap. inline копирует тело функции и лямбды непосредственно в место вызова — нет создания объекта, нет вызова invoke(). Не стоит использовать inline когда: (1) функция большая — код дублируется в каждом месте вызова, увеличивая размер bytecode; (2) лямбда сохраняется в переменную или передаётся дальше (crossinline/noinline); (3) функция не принимает лямбды — inline без лямбд не даёт выигрыша и увеличивает bytecode.

> [!question]- Сценарий: у вас есть nullable объект User?, и вам нужно: проверить его на null, если не null — обновить его поле и вернуть результат. Какие scope functions использовать и почему?
> Оптимальный вариант: user?.let { it.name = "Updated"; it } или user?.apply { name = "Updated" }. let подходит когда нужно вернуть результат трансформации (возвращает результат лямбды). apply подходит когда нужно мутировать объект и вернуть его (возвращает сам объект). Обе функции вызываются через safe call ?. и выполняются только если user не null. Не использовать run — он не имеет преимуществ перед let в этом случае. Не использовать also — он для side effects без мутации.

> [!question]- В чём разница между (T) -> R и T.() -> R в сигнатурах Kotlin-функций?
> (T) -> R — обычная лямбда с параметром: { user -> user.name }. T.() -> R — лямбда с receiver (extension lambda): { this.name } или просто { name }. Внутри extension lambda this указывает на receiver (объект типа T), позволяя вызывать его методы напрямую без явного обращения. Это основа для DSL в Kotlin: apply, with, run принимают T.() -> Unit. Gradle buildscripts, Jetpack Compose, Ktor — все используют extension lambdas для создания читаемых DSL.

---

## Ключевые карточки

Какие пять scope functions в Kotlin и чем они отличаются?
?
let: it + возвращает результат (null-safety, трансформация). run: this + возвращает результат (вычисления на объекте). with: this + возвращает результат (настройка объекта, не extension). apply: this + возвращает объект (конфигурация, builder). also: it + возвращает объект (side effects, логирование). Ключевые различия: it vs this (обращение к объекту), возвращает результат vs объект.

Что делает inline и зачем нужен reified?
?
inline копирует тело функции в место вызова, устраняя overhead создания лямбда-объекта. reified сохраняет generic тип в runtime (только в inline функциях): inline fun <reified T> isType(obj: Any) = obj is T. Без reified нельзя проверить generic тип из-за type erasure. Используется в Gson, Kotlin serialization, Android bundle extensions.

Чем trailing lambda отличается от обычного вызова?
?
Если последний параметр функции — лямбда, её можно вынести за скобки: list.map { it * 2 } вместо list.map({ it -> it * 2 }). Если лямбда — единственный аргумент, скобки можно опустить: run { println("hello") }. Это основа DSL-дизайна в Kotlin: Gradle, Compose, Ktor используют trailing lambda для создания декларативного синтаксиса.

Что такое higher-order function (HOF)?
?
Функция, которая принимает другую функцию как параметр или возвращает функцию: fun <T> List<T>.customFilter(predicate: (T) -> Boolean): List<T>. Примеры из stdlib: map, filter, fold, reduce, flatMap. HOF — основа функционального стиля в Kotlin. Позволяют абстрагировать поведение и создавать переиспользуемые паттерны.

Для чего нужны crossinline и noinline?
?
noinline запрещает инлайнинг конкретной лямбды в inline-функции (когда лямбду нужно сохранить в переменную или передать другой не-inline функции). crossinline запрещает non-local return из лямбды (когда лямбда вызывается из другого контекста выполнения, например из Runnable). Оба модификатора нужны для корректной работы inline функций с нестандартным использованием лямбд.
...
|-------------|------|-------|
| Углубление | [[kotlin-collections]] | Применение HOF и лямбд к коллекциям: map, filter, fold, sequences |
| Углубление | [[kotlin-coroutines]] | Suspend functions — расширение концепции лямбд для асинхронного кода |
| Связь | [[kotlin-type-system]] | Generics и reified — как система типов взаимодействует с inline |
| Кросс-область | [[functional-programming]] | Теоретические основы ФП: монады, функторы, чистые функции |
