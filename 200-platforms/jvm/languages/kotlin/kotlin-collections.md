...
> [!question]- Почему Kotlin List является read-only, но не truly immutable? Какие практические последствия это имеет?
> Kotlin List<T> — это интерфейс без методов мутации (нет add, remove). Но под капотом это может быть java.util.ArrayList, который мутабелен. Если кто-то имеет ссылку на underlying MutableList, он может изменить содержимое, и это будет видно через read-only List. Практические последствия: (1) val list: List<Int> = mutableListOf(1,2,3) — через cast можно мутировать; (2) в многопоточном коде read-only не гарантирует thread-safety; (3) для true immutability используйте kotlinx.collections.immutable (persistent collections).

> [!question]- Сценарий: у вас есть список из 10 миллионов элементов, к которому нужно применить map, filter и take(10). Что быстрее — обычные коллекции или Sequence, и почему?
> Sequence будет значительно быстрее. Обычные коллекции (eager evaluation): создают промежуточный список 10М элементов после map, затем фильтруют в новый список, затем берут 10. Это 2-3 полных прохода и 2-3 промежуточных аллокации. Sequence (lazy evaluation): обрабатывает элементы по одному — для каждого элемента выполняет map, filter, и если элемент прошёл — добавляет в результат. После нахождения 10 элементов останавливается. Нет промежуточных коллекций, обработано может быть всего 100-1000 элементов вместо 10М.

> [!question]- В чём разница между fold и reduce, и когда reduce может бросить исключение?
> fold принимает начальное значение и аккумулятор: list.fold(0) { acc, item -> acc + item }. reduce использует первый элемент как начальное значение: list.reduce { acc, item -> acc + item }. reduce бросает UnsupportedOperationException если коллекция пуста (нет первого элемента для инициализации). fold всегда безопасен — вернёт начальное значение для пустой коллекции. Также fold позволяет менять тип результата (например, List<Int> -> String), а reduce возвращает тот же тип, что и элементы коллекции.

---

## Ключевые карточки

Чем List отличается от MutableList в Kotlin?
?
List<T> — read-only интерфейс без методов мутации (add, remove, set). MutableList<T> расширяет List<T> и добавляет методы мутации. listOf() создаёт List, mutableListOf() создаёт MutableList. Read-only не означает immutable — это API ограничение, underlying реализация может быть мутабельной.

Когда использовать Sequence вместо обычных коллекций?
?
Sequence (lazy) вместо Collection (eager) когда: (1) большой dataset (>10000 элементов); (2) цепочка из 3+ операций (map, filter, take); (3) нужно раннее завершение (first, take). Sequence обрабатывает элементы по одному без промежуточных коллекций. Не использовать для маленьких коллекций — overhead lazy evaluation не окупится.

Какие ключевые операторы коллекций в Kotlin?
?
Трансформация: map (преобразование), flatMap (преобразование + flatten), zip (объединение двух коллекций). Фильтрация: filter (по предикату), filterNotNull, take/drop. Агрегация: fold/reduce (свёртка), groupBy (группировка), associate (в Map). Проверки: any, all, none (предикаты), count, sum, average.

Чем chunked отличается от windowed?
?
chunked(n) разбивает коллекцию на куски размера n без перекрытия: [1,2,3,4,5].chunked(2) = [[1,2],[3,4],[5]]. windowed(n) создаёт скользящее окно с перекрытием: [1,2,3,4,5].windowed(3) = [[1,2,3],[2,3,4],[3,4,5]]. windowed поддерживает step (шаг) и partialWindows (неполные окна в конце).

Как groupBy и associateBy различаются?
?
groupBy создаёт Map<K, List<V>> — группирует элементы по ключу, значения в списках: users.groupBy { it.city } -> Map<City, List<User>>. associateBy создаёт Map<K, V> — один элемент на ключ (последний выигрывает): users.associateBy { it.id } -> Map<Id, User>. groupBy для группировки, associateBy для индексации.
...
|-------------|------|-------|
| Углубление | [[kotlin-flow]] | Flow — асинхронные последовательности, расширение концепции Sequence |
| Углубление | [[kotlin-coroutines]] | Suspend functions для асинхронных операций над коллекциями |
| Связь | [[kotlin-functional]] | HOF основы, на которых строятся операторы коллекций |
| Кросс-область | [[jvm-concurrent-collections]] | Thread-safe коллекции JVM для многопоточного доступа |
