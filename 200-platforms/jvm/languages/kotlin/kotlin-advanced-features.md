...
## Теоретические основы: PL-теория за advanced features

### Extension Functions: Open Classes (CLOS, 1988)

> **Extension function** — формально это **static dispatch функция** с implicit receiver. В PL-теории это реализация **open methods** (generic functions): методы, определённые вне класса, но вызываемые через dot-notation.

Прецеденты в истории PL:
- **CLOS** (Common Lisp Object System, 1988) — generic functions, dispatch по типу первого аргумента
- **C# Extension Methods** (2007) — static methods с `this` modifier
- **Haskell Type Classes** (Wadler & Blott, 1989) — ad-hoc polymorphism без изменения типа

Kotlin extensions компилируются в **static methods** с первым параметром-receiver: `fun String.isEmail()` → `static boolean isEmail(String $this)`. Следствие: extensions **не поддерживают virtual dispatch** (разрешаются статически по типу переменной, а не по runtime-типу объекта).

### Property Delegates: Strategy Pattern + Метапрограммирование

> **Property delegation** — реализация **Strategy pattern** (GoF, 1994) на уровне свойств: логика get/set вынесена в отдельный объект (delegate), определяемый через `operator fun getValue/setValue`.

Формально это **метапрограммирование на уровне свойств**: компилятор генерирует код, перенаправляющий доступ к свойству через delegate-объект. `by lazy {}` — lazy evaluation (нормальный порядок редукции в λ-исчислении, Church 1936); `by observable {}` — Observer pattern (GoF, 1994).

### DSL: Function Types with Receiver = Context-Oriented Programming

> **Function type with receiver** (`A.() -> B`) — это **λ-абстракция с implicit context**: внутри блока доступны все методы receiver'а без квалификации. Формально это restricted форма **Reader monad** (`Reader<A, B> = A → B`).

Kotlin DSL строится на трёх механизмах:
1. **Trailing lambda** — синтаксический сахар для последнего lambda-параметра
2. **Function type with receiver** — implicit context (this = receiver)
3. **@DslMarker** — контроль scope resolution для предотвращения accidental captures

> **Связь**: Extension functions → [[kotlin-type-system]], Delegates → [[kotlin-coroutines]] (by lazy как пример), DSL → [[functional-programming]] (Reader monad)
...
- Wadler P., Blott S. (1989). *How to Make ad-hoc Polymorphism Less ad hoc.* POPL. — Определение type classes, теоретическая основа extension functions как ad-hoc polymorphism.
- Gamma E. et al. (1994). *Design Patterns: Elements of Reusable Object-Oriented Software.* — Strategy, Observer, Builder паттерны, которые Kotlin delegates и DSL реализуют на уровне языка.
...
> [!question]- Почему extension functions разрешаются статически, а не динамически, и какие практические последствия это имеет?
> Extension functions компилируются в static методы: fun String.isPalindrome() становится StringKt.isPalindrome(String). Dispatch по типу receiver определяется на этапе компиляции, не runtime. Последствие: если объявить extension для open class Base и вызвать через переменную типа Base, которая содержит Derived — вызовется extension для Base, а не Derived. Extension не поддерживает полиморфизм. Для полиморфного поведения используйте обычные virtual methods или interface. Extension идеален для utility-операций, DSL, работы с чужими классами.

> [!question]- Сценарий: вам нужно создать DSL для конфигурации HTTP-клиента: httpClient { timeout = 5000; retry { maxAttempts = 3 } }. Какие языковые механизмы Kotlin используются?
> Механизмы: (1) Function type with receiver (T.() -> Unit) — позволяет this внутри лямбды ссылаться на builder-объект. (2) Trailing lambda — последний параметр-лямбда выносится за скобки. (3) @DslMarker — аннотация для контроля scope (запрещает обращение к outer receiver из inner lambda). (4) Builder pattern — каждая вложенная лямбда создаёт builder-объект. Реализация: fun httpClient(block: HttpClientBuilder.() -> Unit): HttpClient; class HttpClientBuilder { var timeout = 0; fun retry(block: RetryBuilder.() -> Unit) }.

> [!question]- Чем property delegate by lazy {} отличается от property с backing field и custom getter, и когда lazy не стоит использовать?
> lazy {} вычисляет значение при первом обращении и кэширует: val value by lazy { compute() }. Custom getter вычисляет каждый раз: val value get() = compute(). lazy создаёт объект Lazy<T> с синхронизацией (по умолчанию SYNCHRONIZED). Не стоит использовать lazy когда: (1) значение быстро вычисляется — overhead на Lazy объект и синхронизацию не оправдан; (2) значение зависит от mutable состояния — lazy кэширует первый результат навсегда; (3) в performance-critical пути — synchronized lock на каждый первый доступ. Альтернативы: LazyThreadSafetyMode.NONE для single-thread, lateinit var для mutable.

---

## Ключевые карточки

Как extension functions компилируются в bytecode?
?
fun String.isPalindrome(): Boolean компилируется в static метод: public static boolean isPalindrome(String $this$isPalindrome). Первый параметр — receiver. В Java: StringKt.isPalindrome("abba"). Extension не может обращаться к private members receiver. Разрешение (dispatch) статическое, не виртуальное — определяется типом переменной, не runtime типом объекта.

Какие стандартные property delegates в Kotlin?
?
lazy {} — ленивая инициализация (вычисляет при первом доступе, кэширует). observable {} — callback при изменении: var name by Delegates.observable("") { _, old, new -> log("$old -> $new") }. vetoable {} — callback с возможностью отмены изменения. notNull() — не-null delegate для var без начального значения. map — delegation к Map для конфигураций.

Что такое @DslMarker и зачем он нужен?
?
@DslMarker — мета-аннотация, ограничивающая scope в DSL. Без неё: inner lambda имеет доступ к receiver outer lambda (this@outer), что приводит к ошибкам. С @DslMarker: доступ к outer receiver запрещён без явного this@label. Пример: в Compose DSL Column { Row { ... } } — внутри Row нельзя случайно вызвать методы Column.

Как создать custom property delegate?
?
Реализовать operator fun getValue(thisRef: Any?, property: KProperty<*>): T для val. Дополнительно operator fun setValue(thisRef: Any?, property: KProperty<*>, value: T) для var. Или реализовать интерфейсы ReadOnlyProperty<R, T> / ReadWriteProperty<R, T>. Пример: delegate для SharedPreferences: var name by stringPreference("key", "default").

Что такое operator overloading и какие операторы можно перегрузить?
?
Kotlin позволяет определить поведение операторов: operator fun plus(other: T): T для +, operator fun get(index: Int): T для [], operator fun invoke(): T для (). operator fun compareTo(other: T): Int для <, >, <=, >=. Destructuring: operator fun component1(): T. Iterator: operator fun iterator(): Iterator<T>. Все операторы — именованные функции с модификатором operator.
...
|-------------|------|-------|
| Углубление | [[kotlin-best-practices]] | Когда использовать и не использовать продвинутые фичи |
| Углубление | [[kotlin-type-system]] | Generics и reified — основа для продвинутых extensions и delegates |
| Связь | [[kotlin-functional]] | Extension lambdas — основа DSL design |
| Кросс-область | [[android-compose]] | Jetpack Compose — крупнейший DSL на Kotlin в production |
