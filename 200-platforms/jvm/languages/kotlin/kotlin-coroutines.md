...
## Теоретические основы: что это и как связано

> Этот раздел даёт формальные определения ключевых концепций корутин.
> Каждая концепция отвечает на три вопроса: **что это?** — **зачем существует?** — **как связано с остальными?**
> Реализация подробно описана в [[kotlin-coroutines-internals]], история async-моделей — в [[async-models-overview]].

### Корутина

> **Корутина** — обобщение подпрограммы (subroutine), допускающее множественные точки входа и выхода с сохранением локального состояния между приостановками.

Термин ввёл Мелвин Конвей (Melvin Conway) в 1958 г.; первое формальное описание — в *"Design of a Separable Transition-Diagram Compiler"* (1963). В CS корутина определяется двумя свойствами (Marlin, 1980): (1) значения локальных данных сохраняются между вызовами; (2) выполнение приостанавливается при выходе и продолжается с того же места при следующем входе.

"Kotlin coroutine" — это два уровня: **компилятор** трансформирует `suspend`-функцию в state machine (CS-концепция), а библиотека **kotlinx.coroutines** добавляет structured concurrency, dispatchers и channels (runtime-слой).

Kotlin coroutines — **asymmetric** (управление возвращается вызывающему), **stackless** (нет собственного стека, ~100-200 байт state machine в heap), **constrained** (компилятор управляет Continuation, пользователь не работает с ним напрямую). Подробная таксономия и сравнение с thread/fiber/goroutine/virtual thread — в [[async-models-overview]], раздел "Таксономия корутин".

### Suspend-функция

> **`suspend`** — маркер типовой системы, объявляющий: "эта функция может приостановить выполнение вызывающей корутины в определённых точках приостановки (suspension points)."

`suspend` — это не аннотация и не модификатор доступа. Он **меняет calling convention**: компилятор добавляет параметр `Continuation<T>` через CPS-трансформацию (см. раздел "Как корутины работают внутри" ниже).

Сравнение с аналогами в других языках:

| Механизм | Язык | Суть | Отличие от Kotlin `suspend` |
|----------|------|------|-----------------------------|
| `yield` | Python, JS | Генерирует значение, приостанавливает | Только производит значения; `suspend` может и потреблять, и производить |
| `async/await` | JS, Python, C# | `async` помечает функцию, `await` — точка ожидания | В Kotlin нет `await` — любой вызов suspend-функции — неявная потенциальная точка приостановки |
| `call/cc` | Scheme | Захватывает continuation как first-class value | `call/cc` захватывает весь стек; Kotlin — только state machine (дешевле, безопаснее, но менее мощно) |
| algebraic effects | Koka, OCaml 5 | Обобщение `suspend` на произвольные эффекты | `suspend` — ограниченная форма: единственный эффект — "пауза" |

Ключевая разница: **suspension point vs preemption point.** В корутинах программист определяет, ГДЕ может произойти приостановка (кооперативно). В потоках ОС решает, КОГДА прервать (вытесняюще). Поэтому между двумя suspension points в корутине гарантированно нет гонок.

### Continuation

> **Continuation** — представление "остатка вычисления" (the rest of the computation) от заданной точки в программе.

В CS это фундаментальное понятие из денотационной семантики (Christopher Strachey, Christopher Wadsworth, 1970-е). Если программа находится в точке P, continuation — это функция, принимающая значение в P и производящая финальный результат.

```
fun process() {
    val a = step1()
    val b = step2(a)      // ← в этой точке continuation =
    val c = step3(b)      //   "взять результат step2, передать в step3,
    return c              //    вернуть результат step3"
}
```

Continuation впервые стали доступны программисту через `call/cc` в Scheme (Sussman & Steele, 1975). Kotlin использует **restricted single-shot continuation**: компилятор управляет ей, пользователь не может захватить или повторно вызвать. Это безопаснее и эффективнее.

Kotlin `Continuation<T>` — интерфейс с двумя членами: `context: CoroutineContext` (окружение выполнения) и `resumeWith(Result<T>)` (способ передать результат). По сути это **callback с контекстом**. Иерархия реализаций — в [[kotlin-coroutines-internals]], раздел "Continuation и его реализация".

### CoroutineContext

> **CoroutineContext** — гетерогенное иммутабельное индексированное множество (heterogeneous immutable indexed set): персистентная структура данных, отображающая типизированные ключи в типизированные значения.

В CS это реализация концепции **окружения** (environment) — маппинга имён в значения, который сопровождает вычисление. Аналоги в других языках:

| Язык | Аналог | Отличие от CoroutineContext |
|------|--------|-----------------------------|
| Go | `context.Context` | Похожая мотивация, но Go context хранит key-value через `interface{}` без типизации |
| Java | `ThreadLocal` | Привязан к потоку, мутабельный; CoroutineContext — иммутабельный, путешествует с корутиной |
| Haskell | Reader monad | Та же идея: пронести конфигурацию через вычисление неявно |
| Lisp | Dynamic scope | Динамическое окружение, сопровождающее выполнение |

**Почему иммутабельный?** Корутины переключаются между потоками — мутабельный контекст потребовал бы синхронизации. Иммутабельность даёт thread safety by construction. Операция `plus()` создаёт новый контекст, а не изменяет старый.

**Почему Composite pattern?** Каждый `Element` одновременно является `CoroutineContext`. Это значит, что одиночный `Job()` можно использовать как полноценный контекст: `launch(Job())` — валидный код. API единообразен для одного элемента и для коллекции.

**Ключевые инварианты:** не более одного элемента на ключ; `plus` имеет семантику set (последний с тем же Key побеждает); Job всегда новый при наследовании (фундамент structured concurrency). Реализация (CombinedContext, fold) — в [[kotlin-coroutines-internals]], раздел "CoroutineContext: архитектура".

### CoroutineScope

> **CoroutineScope** — интерфейс с единственным свойством `val coroutineContext: CoroutineContext`. Это **граница владения** (ownership boundary) для structured concurrency.

Зачем отдельная сущность, если есть CoroutineContext? Дизайн-решение от Roman Elizarov: без CoroutineScope можно было бы вызвать `launch` откуда угодно, имея только контекст. CoroutineScope делает владение явным: "я — сущность, владеющая корутинами и контролирующая их жизненный цикл". Все coroutine builders (`launch`, `async`) — extension-функции на CoroutineScope.

**Аналогия со structured programming:** scope = блок `{}` в структурном программировании. Как блок определяет время жизни локальных переменных, так CoroutineScope определяет время жизни запущенных в нём корутин.

```
CoroutineScope { coroutineContext: CoroutineContext }
       │
       ├── launch {} ──> child coroutine (новый child Job)
       ├── async {}  ──> child coroutine (новый child Job)
       └── cancel()  ──> отмена всех children
       │
       ▼
  CoroutineContext = Job + Dispatcher + Name + ExceptionHandler + ...
                      │
                      ▼
                 Job hierarchy (parent → children tree)
```

**Context = конфигурация ("где, как, с какими метаданными"). Scope = владелец ("кто контролирует жизненный цикл").**

### Structured Concurrency

> **Structured Concurrency** — принцип организации конкурентных вычислений, при котором время жизни дочерних задач ограничено временем жизни родительского scope.

Концепцию сформулировал Натаниэль Дж. Смит (Nathaniel J. Smith) в статье *"Notes on structured concurrency, or: Go statement considered harmful"* (2018). Он провёл прямую аналогию с Эдсгером Дейкстрой (Edsger Dijkstra), который в 1968 г. написал знаменитое письмо *"Go To Statement Considered Harmful"*: как `goto` создаёт произвольный поток управления, так `go`/`GlobalScope.launch` создаёт произвольный поток конкурентности. Оба делают невозможным рассуждение о поведении программы.

**Формальные инварианты:**

| Инвариант | Что гарантирует |
|-----------|-----------------|
| **Lifetime scoping** | Дочерние корутины не могут пережить родительский scope |
| **Downward cancellation** | Отмена scope рекурсивно отменяет всех потомков |
| **Upward error propagation** | Необработанная ошибка в child пропагируется к parent |
| **Completion waiting** | Scope не завершается, пока не завершатся все children |

**Дерево управления ресурсами.** Structured concurrency создаёт дерево владения (Jobs), аналогичное RAII в C++ или try-with-resources в Java. Дерево гарантирует, что все ресурсы (корутины) освобождаются при завершении владеющего scope. Это решает проблему "orphan coroutines" — забытых корутин, работающих после уничтожения компонента.

Kotlin реализует structured concurrency через иерархию Job: каждый `launch`/`async` создаёт child Job, привязанный к parent Job из scope. Практические примеры — в разделе "Structured Concurrency" ниже.

### Job

> **Job** — lifecycle handle корутины: first-class представление единицы конкурентной работы, отвечающее на вопросы "работает ли?", "завершилась ли?", "отменена ли?".

Job имеет **двойную роль**:
1. **CoroutineContext.Element** — элемент контекста, путешествующий с корутиной; parent-child иерархия устанавливается через наследование контекста (`context[Job]` находит parent)
2. **Lifecycle state machine** — конечный автомат с состояниями New → Active → Completing → Completed (или Cancelling → Cancelled)

**Аналогия с Erlang/OTP.** Job hierarchy напоминает деревья супервизии Erlang: parent Job — супервизор, child Jobs — управляемые процессы. Отмена распространяется вниз (parent отменяет children), ошибки — вверх (child уведомляет parent). Ключевое отличие: Kotlin использует лексическую привязку (structured concurrency), Erlang — динамические деревья супервизии.

**Tree invariant:** Jobs формируют строгое дерево (не DAG). У каждого Job ровно один parent (кроме root Jobs). Это гарантирует однозначные пути владения и очистки. Детали state machine — в [[kotlin-coroutines-internals]], раздел "Job и Cancellation internals".

### Dispatcher

> **Dispatcher** — `ContinuationInterceptor`, определяющий политику выполнения: ГДЕ и КОГДА корутина выполняется. Это отделение "что делать" (логика корутины) от "где делать" (поток или пул потоков).

Это реализация **Strategy pattern**: корутина определяет алгоритм, dispatcher определяет стратегию выполнения. Аналоги в других системах:

| Система | Аналог Dispatcher |
|---------|-------------------|
| Java | `Executor` / `ExecutorService` |
| JavaScript | Event Loop (single-threaded dispatcher) |
| RxJava | `Scheduler` |
| .NET | `SynchronizationContext` / `TaskScheduler` |
| Go | Runtime scheduler (неявный, без контроля пользователя) |

Механизм: Dispatcher — это `ContinuationInterceptor`, специальный элемент `CoroutineContext`. При возобновлении корутины interceptor оборачивает Continuation в `DispatchedContinuation`, которая направляет выполнение на нужный поток. Это **Decorator pattern**, применённый к continuation. Подробности — в [[kotlin-coroutines-internals]], раздел "ContinuationInterceptor и Dispatchers".

### Карта концепций: как всё связано

```
                    ┌──────────────────────────────────┐
                    │     КОРУТИНА (Conway, 1958)       │
                    │  = тело suspend-функции           │
                    │  = state machine (компилятор)     │
                    └──────────┬───────────────────────┘
                               │ "выполняется в"
                    ┌──────────▼───────────────────────┐
                    │   COROUTINE SCOPE                │
                    │   = граница жизненного цикла      │
                    │   = владеет CoroutineContext      │
                    └──────────┬───────────────────────┘
                               │ "содержит"
                    ┌──────────▼───────────────────────┐
                    │   COROUTINE CONTEXT              │
                    │   = immutable indexed set         │
          ...
          │  = tree node  │  │  = ContinuationInterceptor│
          │  (аналог      │  │  (Strategy pattern)      │
          │   Erlang sup.)│  └──────────────────────────┘
          └───────────────┘
               │ "иерархия обеспечивает"
     ┌─────────▼──────────────────┐
     │  STRUCTURED CONCURRENCY    │
     │  (Smith, 2018)             │
     │  = lifetime + cancel + err │
     │  аналогия: goto → struct   │
     └────────────────────────────┘

 SUSPEND FUNCTION               CONTINUATION
 = маркер типовой системы       = "остаток вычисления"
 = CPS-трансформация            = callback + context
 = точки входа в корутину       = state machine объект
```
...
Корутина — обобщение подпрограммы (Conway, 1958): функция с множественными точками входа, сохраняющая состояние между приостановками. В Kotlin корутины — stackless, asymmetric, compiler-managed.
...
| Термин | Формально | Простыми словами | Аналогия |
|--------|-----------|------------------|----------|
| **Корутина** | Обобщение подпрограммы с множественными точками входа (Conway, 1958) | Легковесный "поток" — объект в heap (~сотни байт), не OS thread | Задача в списке дел — переключаешься между ними |
| **suspend** | Маркер типовой системы: функция может приостановить корутину в suspension points | Функция, которая может "замереть" без блокировки потока | Официант: ставит заказ, идёт к другому столику |
| **Continuation** | Представление "остатка вычисления" — callback с контекстом (Strachey, 1970-е) | Объект, хранящий состояние приостановленной корутины | Закладка в книге — помнит где остановился |
| **CoroutineContext** | Гетерогенное иммутабельное индексированное множество (environment) | Набор настроек корутины: поток, имя, Job, обработчик ошибок | Набор документов для командировки |
| **CoroutineScope** | Ownership boundary: интерфейс с единственным свойством `coroutineContext` | Область жизни корутин — управляет отменой | Менеджер проекта — закрывает проект, все задачи отменяются |
| **Dispatcher** | ContinuationInterceptor — execution policy (Strategy pattern) | Определяет на каком потоке выполняется корутина | Диспетчер такси — направляет машины на заказы |
| **Job** | Lifecycle handle корутины: элемент контекста + state machine | Представление жизненного цикла корутины | Контракт на работу — активен, выполнен или расторгнут |
| **SupervisorJob** | Job с переопределённым `childCancelled()` → false | Job, где failure дочерней корутины не отменяет siblings | Директор школы — один заболел, остальные учатся |
| **Deferred\<T\>** | Job + `suspend fun await(): T` — результат конкурентного вычисления | Job с результатом, получается через await() | Квитанция на химчистку — позже заберёшь результат |
| **Structured Concurrency** | Принцип: lifetime дочерних задач ≤ lifetime родительского scope (Smith, 2018) | Корутины образуют иерархию: parent ждёт children | Семья в поездке — родители не уедут без детей |
...
- Smith N.J. (2018). *Notes on structured concurrency, or: Go statement considered harmful*. — манифест structured concurrency; прямая аналогия с Dijkstra "Go To Statement Considered Harmful" (1968)
- Dijkstra E.W. (1968). *Go To Statement Considered Harmful*. Communications of the ACM, 11(3), pp. 147–148. — параллель, на которую опирается концепция structured concurrency
- Elizarov R. et al. (2021). *Kotlin coroutines: design and implementation*. ACM SIGPLAN Onward! — формальное описание дизайна корутин от их автора
- Marlin C. (1980). *Coroutines: A Programming Methodology, a Language Design and an Implementation*. — систематизация корутин; формулировка двух фундаментальных свойств
