...
Идиоматичный Kotlin-код основывается на принципах, формализованных в software engineering и теории языков программирования.

### Immutability и Referential Transparency

> **Формально:** **referential transparency** (Quine, 1960; Strachey, 1967) — свойство выражения, при котором его можно заменить результатом без изменения поведения программы. `val` в Kotlin обеспечивает **неизменяемость ссылки** (referential immutability), что является предпосылкой для referential transparency.

Принцип `val` по умолчанию следует из наблюдений Bloch (*Effective Java*, Item 17, 2001): «Minimize mutability» — иммутабельные объекты проще для reasoning, thread-safe по определению и не требуют защитного копирования.

### Principle of Least Surprise

> **Закон наименьшего удивления** (Principle of Least Astonishment, Saltzer, 1974) — API должен вести себя так, как ожидает пользователь. В контексте Kotlin best practices:

| Принцип | Антипаттерн | Идиоматичный Kotlin |
|---------|------------|---------------------|
| Предсказуемость | `!!` — неожиданный NPE | `?.let {}`, `?: return` — явная обработка |
| Минимализм | `var` когда хватит `val` | `val` по умолчанию |
| Явность | Magic numbers | `const val MAX_RETRIES = 3` |
| Конвенция | Java-стиль getters | Kotlin properties |

### Expression-Oriented Programming

Kotlin следует традиции **expression-oriented languages** (ML, 1973; Scala): `if`, `when`, `try` — выражения, возвращающие значения. Это снижает количество мутабельного состояния:

```kotlin
// Statement-oriented (Java-стиль)
var result: String
...
Формально expression-oriented стиль уменьшает число **side effects** (Strachey, 1967) и упрощает **equational reasoning** — возможность рассуждать о программе путём подстановки.

### Scope Functions как Design Patterns

Пять scope functions Kotlin формализуют типичные паттерны работы с объектами:

| Function | Pattern | Формальное свойство |
|----------|---------|---------------------|
| `let` | **Null-safe pipeline** | `T?.let { f(it) }` — монадический bind для nullable |
| `apply` | **Builder pattern** | Возвращает `this` — fluent interface (Fowler, 2005) |
| `also` | **Tee combinator** | Side effect без изменения значения (Unix `tee`) |
| `run` | **Execute with receiver** | Вычисление в контексте объекта |
| `with` | **Context configuration** | Аналог `run`, но не extension function |

> **Правило одной scope function** (Moskala, *Effective Kotlin*, 2024): не вкладывать scope functions друг в друга. Вложенность `let` внутри `apply` внутри `run` создаёт ambiguity между `this` и `it` — нарушение принципа наименьшего удивления.

См. также: [[kotlin-functional]] — теоретические основы FP в Kotlin, [[kotlin-basics]] — базовые конструкции языка.
...
- Strachey C. (1967). *Fundamental Concepts in Programming Languages*. — Формализация referential transparency, L-values/R-values, first-class values; теоретическая основа для `val` и expression-oriented стиля.
- Saltzer J. (1974). *Protection and the Control of Information Sharing in Multics*. — Principle of Least Surprise (Astonishment); мотивация для предсказуемого API.
- Bloch J. (2001/2018). *Effective Java*. Items 15-17. — «Minimize mutability», «Favor immutability» — принципы, воплощённые в `val` по умолчанию и data class.
- Fowler M. (2005). *FluentInterface*. — Формализация fluent interface pattern; теоретическая основа для `apply {}` и builder DSL.
