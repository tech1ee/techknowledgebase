...
> [!question]- Почему Kotlin различает nullable (String?) и non-nullable (String) типы, и чем это лучше Java-подхода с Optional?
> Kotlin встраивает null-safety в систему типов: String гарантированно не null, String? может быть null. Компилятор заставляет обрабатывать null до запуска программы. В Java Optional — это обёртка-контейнер, которая: (1) сама может быть null; (2) предназначена только для return types (не для полей, параметров); (3) создаёт runtime overhead на аллокацию объекта. Kotlin подход: zero overhead (проверки на уровне компилятора, в bytecode null-check вставляется как if), работает везде (поля, параметры, return types), невозможно "забыть" проверку.

> [!question]- Сценарий: вы объявили переменную как val list = mutableListOf(1, 2, 3). Можно ли изменять содержимое списка? Можно ли переназначить переменную?
> val означает, что переменную нельзя переназначить (list = другойСписок выдаст ошибку компиляции). Однако содержимое списка можно менять: list.add(4), list.remove(1) — это работает, потому что mutableListOf возвращает MutableList. val защищает только ссылку, не содержимое объекта. Для полной иммутабельности нужно использовать listOf(), который возвращает read-only List без методов add/remove.

> [!question]- Почему в Kotlin рекомендуется использовать when вместо цепочки if-else if, и при каком условии when проверяет exhaustiveness?
> when — более читаемый и мощный аналог switch/if-else: поддерживает pattern matching, диапазоны (in 1..10), проверку типов (is String), множественные значения (1, 2, 3). Exhaustiveness checking (проверка полноты) включается когда: (1) when используется как выражение (val result = when...); (2) аргумент — sealed class или enum. Компилятор требует обработать все варианты или добавить else. Это предотвращает ошибки при добавлении новых подтипов.

> [!question]- Объясните разницу между == и === в Kotlin. Когда результат отличается?
> == в Kotlin вызывает equals() — сравнение по содержимому (structural equality). === сравнивает ссылки — это тот же объект в памяти (referential equality). Результат отличается когда: два разных объекта имеют одинаковое содержимое (data class User("John") == data class User("John") вернёт true, но === вернёт false). Для примитивных типов (Int, Boolean) === работает как == из-за boxing/caching. Важно: в Java == сравнивает ссылки для объектов, что является частой ошибкой.

---

## Ключевые карточки

Чем val отличается от var в Kotlin?
?
val — неизменяемая ссылка (read-only), аналог final в Java. var — изменяемая ссылка. Рекомендация: всегда начинать с val, менять на var только при необходимости. val защищает ссылку, но не содержимое объекта (mutableList через val всё равно можно модифицировать).

Что такое safe call (?.) и Elvis operator (?:)?
?
Safe call (?.) — если объект null, возвращает null вместо NullPointerException: name?.length вернёт null если name == null. Elvis operator (?:) — значение по умолчанию если null: name?.length ?: 0 вернёт 0 если name null. Цепочка: user?.address?.city ?: "Unknown" безопасно обрабатывает null на каждом уровне.

Что генерирует data class автоматически?
?
equals() (сравнение по всем свойствам), hashCode() (для HashMap/HashSet), toString() (читаемое представление), copy() (создание копии с возможностью изменить поля), componentN() (для деструктуризации). Заменяет 50+ строк Java boilerplate одной строкой.

В чём разница между String и String? в Kotlin?
?
String — non-nullable тип, гарантированно не null. String? — nullable тип, может быть null. Присвоение null в String вызовет ошибку компиляции. Для работы с String? используются: safe call (?.), Elvis (?:), not-null assertion (!!). Компилятор принуждает обработать null-case до runtime.

Какие основные типы функций в Kotlin?
?
Top-level функции (без класса), extension functions (добавляют методы к существующим классам), infix functions (вызов без точки: 1 to "one"), operator functions (перегрузка операторов: operator fun plus), local functions (функция внутри функции). Все функции — first-class citizens.
...
|-------------|------|-------|
| Углубление | [[kotlin-oop]] | Классы, sealed classes, delegation — ООП возможности Kotlin |
| Углубление | [[kotlin-functional]] | Лямбды, scope functions, inline — функциональная сторона Kotlin |
| Связь | [[kotlin-type-system]] | Generics, variance, reified — продвинутая система типов |
| Кросс-область | [[jvm-languages-ecosystem]] | Сравнить Kotlin с Scala, Clojure, Groovy — контекст JVM-экосистемы |
