...
> [!question]- Почему в Kotlin классы по умолчанию final и как это связано с принципом "Effective Java"?
> Блох в "Effective Java" (Item 17) рекомендует "Design and document for inheritance, or prohibit it". Большинство классов не проектируются для наследования, и неконтролируемое наследование ведёт к хрупким базовым классам (fragile base class problem). Kotlin следует этому принципу: все классы final по умолчанию. Для разрешения наследования нужно явно пометить класс как open. Это заставляет разработчика осознанно принимать решение о наследовании и документировать контракт.

> [!question]- Сценарий: вам нужен класс, представляющий валюту в приложении (USD, EUR, KZT). Значения известны на этапе компиляции, каждая валюта имеет код и символ. Что лучше использовать: enum class, sealed class или value class?
> enum class — лучший выбор для фиксированного набора значений: enum class Currency(val code: String, val symbol: String) { USD("USD", "$"), EUR("EUR", "E"), KZT("KZT", "T") }. Enum гарантирует: фиксированный набор на compile-time, каждый экземпляр singleton, поддержка when exhaustiveness, встроенные name/ordinal. Sealed class был бы overengineering — он для случаев когда варианты имеют разную структуру данных. Value class не подходит — он для type-safe обёрток примитивов (inline class Amount(val value: Double)).

> [!question]- Чем delegation (by) принципиально лучше наследования для композиции поведения?
> Delegation by позволяет делегировать реализацию интерфейса другому объекту без наследования: class LoggingList<T>(list: MutableList<T>) : MutableList<T> by list. Преимущества: (1) нет проблемы fragile base class — делегат можно заменить; (2) класс может делегировать несколько интерфейсов разным объектам (множественное "наследование"); (3) можно переопределить только нужные методы; (4) тестируемость — делегат можно подменить mock. Наследование создаёт жёсткую связь: изменение базового класса ломает все наследники.

---

## Ключевые карточки

Что такое sealed class и чем он отличается от enum?
?
Sealed class — закрытая иерархия: все подклассы известны на compile-time, но каждый может иметь разную структуру. Enum — фиксированный набор singleton-экземпляров с одинаковой структурой. Sealed class: sealed class Result { data class Success(val data: String) : Result(), data class Error(val exception: Exception) : Result() }. when проверяет exhaustiveness для обоих.

Что такое companion object и чем он отличается от static в Java?
?
companion object — объект-компаньон внутри класса, заменяет Java static. Отличия: companion object может реализовывать интерфейсы, может быть расширен extension functions, может иметь имя. Является полноценным объектом (singleton), а не просто набором статических методов. Доступ: ClassName.method() или ClassName.Companion.method().

Чем data class отличается от обычного class?
?
data class автоматически генерирует: equals() (сравнение по свойствам), hashCode(), toString(), copy() (копирование с изменениями), componentN() (деструктуризация). Ограничения: primary constructor должен иметь хотя бы один параметр, все параметры val/var, не может быть abstract/open/sealed/inner. Свойства вне primary constructor не участвуют в equals/hashCode.

Что такое value class (inline class) и когда его использовать?
?
value class — обёртка типа без runtime overhead: @JvmInline value class UserId(val id: Long). Компилятор заменяет обёртку на примитив в bytecode. Использовать для type safety: UserId(1) нельзя случайно передать как OrderId(1). Ограничения: один параметр в primary constructor, нельзя наследовать, нет identity (=== не гарантирован).

Как работает delegation by в Kotlin?
?
Компилятор генерирует методы-делегаты: class Printer(val writer: Writer) : Writer by writer. Все вызовы методов Writer перенаправляются на writer. Можно переопределить конкретные методы. Это compile-time генерация, не рефлексия — zero overhead. Поддерживает делегирование свойств: val name by lazy { compute() }.
