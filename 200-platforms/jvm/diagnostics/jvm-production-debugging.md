...
> [!question]- Почему remote debugging категорически нельзя использовать в production?
> Remote debugging останавливает JVM на breakpoint'ах, блокируя ВСЕ потоки. Один breakpoint замораживает весь сервис: пользователи получают timeout, load balancer начинает переключать трафик, session state теряется. Это не "замедление", а полная остановка обработки запросов.

> [!question]- Сценарий: сервис тормозит в 3 AM, CPU 100%. Как провести диагностику, не создавая downtime? Какие инструменты и в каком порядке использовать?
> 1) Thread dump (jstack, ~100мс пауза, безопасно) -- 3 dump'а с интервалом 5-10 секунд для выявления паттерна. 2) Если потоки RUNNABLE на одном методе -- async-profiler (CPU mode, 30 сек, <1% overhead). 3) Если потоки BLOCKED -- lock contention, анализировать thread dump. 4) JFR dump (если continuous recording включён) для ретроспективного анализа. Heap dump -- только как последний resort, на off-peak.

> [!question]- Почему один thread dump недостаточен для диагностики, и что даёт серия из трёх dump'ов?
> Один dump -- случайный snapshot момента, может быть нерепрезентативен. Серия из 3 dump'ов с интервалом 5-10 секунд выявляет паттерн: если поток "застрял" на одном методе во всех трёх dump'ах -- проблема реальная. Если позиция меняется -- поток работает, просто медленно.

> [!question]- Сценарий: Eclipse MAT показывает 2.4 млн экземпляров ProductDTO, занимающих 5.2 GB. Path to GC Roots ведёт к static HashMap в CacheService. Почему GC не может собрать эти объекты?
> static поля являются GC root. HashMap -- strong reference. Пока CacheService жив (а static fields живут до выгрузки класса), все 2.4 млн ProductDTO reachable от GC root через HashMap. GC обязан считать их живыми. Решение: Caffeine/Guava cache с maximumSize и expireAfterWrite.

---

## Ключевые карточки

Что показывают состояния потоков RUNNABLE, BLOCKED и WAITING в thread dump?
?
RUNNABLE -- поток выполняет код (или I/O). BLOCKED -- ждёт вход в synchronized блок, захваченный другим потоком. WAITING -- вызвал wait()/park() без timeout, ждёт явного пробуждения. TIMED_WAITING -- ожидание с timeout (sleep, poll).

Чем retained size отличается от shallow size в heap dump?
?
Shallow size -- размер самого объекта (заголовок + поля). Retained size -- shallow size + size всех объектов, которые станут unreachable при удалении данного объекта. HashMap с shallow 64 байта может иметь retained 2 GB (все entries).

Какой минимальный набор JVM-флагов нужен для production?
?
HeapDumpOnOutOfMemoryError (dump при OOM), HeapDumpPath (куда сохранять), Xlog:gc* (GC логи), StartFlightRecording (JFR continuous). Overhead <2%, но при инциденте данные уже собраны.

Как JFR continuous recording помогает при инцидентах?
?
JFR пишет в кольцевой буфер постоянно (overhead <2%). При инциденте jcmd JFR.dump сохраняет данные за последние часы: CPU sampling, GC events, lock contention, allocations. Не нужно воспроизводить проблему -- данные уже есть.

Чем pool exhaustion отличается от deadlock?
?
Deadlock -- потоки блокируют друг друга (JVM детектирует автоматически). Pool exhaustion -- все connections/threads заняты медленными операциями, новые запросы ждут. JVM не детектирует, потому что нет циклической зависимости. Диагностика: thread dump + анализ что держат connections.
...
|-------------|------|-------|
| Следующий шаг | [[jvm-gc-tuning]] | Настройка GC после диагностики memory-проблем |
| Углубиться | [[jvm-profiling]] | Систематическое профилирование для проактивной оптимизации |
| Связанная тема | [[jvm-memory-model]] | Фундамент для понимания heap dump'ов |
| Смежная область | [[devops-incident-management]] | Процессы реагирования на инциденты в production |
