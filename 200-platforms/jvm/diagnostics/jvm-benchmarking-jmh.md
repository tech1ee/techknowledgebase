...
JMH построен на принципах **экспериментальной методологии** и статистического анализа, адаптированных для среды с агрессивной JIT-компиляцией и недетерминированным GC.

> **Определение:** *Microbenchmark — контролируемый эксперимент, измеряющий производительность изолированного фрагмента кода с устранением систематических погрешностей (bias) и статистической обработкой результатов.*

| Теоретическая концепция | Автор / Источник | Применение в JMH |
|------------------------|-----------------|-----------------|
| **Dead Code Elimination** | Appel, 1998 (Modern Compiler Implementation) | JIT удаляет код с неиспользуемым результатом → Blackhole предотвращает DCE |
| **Constant Folding** | Aho et al., 1986 (Dragon Book) | JIT вычисляет константные выражения → @State предотвращает через runtime-значения |
| **Statistical significance** | Fisher, 1925 | Confidence interval (99.9%) через t-распределение → ±error в результатах JMH |
| **Observer effect** | Heisenberg principle (applied to SW) | Instrumentation изменяет поведение программы → sampling минимизирует вмешательство |
| **Tiered compilation** | Paleczny et al. (C2), 1999 | Interpreter → C1 → C2: warmup iterations обеспечивают пиковую JIT-оптимизацию |

> **Методологический принцип:** Каждый fork JMH запускает новый JVM-процесс, что обеспечивает **статистическую независимость** измерений. Разные fork'и могут показать различную производительность из-за разного layout'а объектов в памяти (cache effects), разных решений JIT-компилятора, и различного GC-поведения. Множественные fork'и + confidence interval дают статистически достоверный результат.

Связанные темы: [[jvm-jit-compiler]] (DCE, Constant Folding, Inlining — оптимизации, от которых JMH защищает), [[jvm-profiling]] (профилирование находит bottleneck, JMH измеряет варианты fix'а), [[jvm-gc-tuning]] (GC interference как источник шума в бенчмарках).
...
- Aho A., Sethi R., Ullman J. (1986). *Compilers: Principles, Techniques, and Tools* (Dragon Book). -- Формальное описание Dead Code Elimination, Constant Folding и других оптимизаций компилятора, от которых JMH защищает бенчмарки.
- Herlihy M., Shavit N. (2012). *The Art of Multiprocessor Programming.* -- Теоретические основы измерения производительности concurrent-кода и проектирование экспериментов с воспроизводимыми результатами.
- Fisher R. (1925). *Statistical Methods for Research Workers.* -- Основы статистической значимости и confidence intervals, на которых построена интерпретация результатов JMH.

### Практические руководства

- Shipilev A. (2013--2023). *JMH documentation, samples, and conference talks.* -- Первоисточник от автора JMH: каждый pitfall микробенчмаркинга объяснён разработчиком HotSpot JVM.
- Oaks S. (2014). *Java Performance: The Definitive Guide.* -- Методология измерения производительности Java-кода, статистическая интерпретация результатов.
- Evans B., Gough J. (2014). *Optimizing Java.* -- JIT-компиляция, GC и их влияние на бенчмарки с практическими примерами JMH.
...
> [!question]- Почему бенчмарк с void-методом и Math.sqrt(42) показывает ~1 нс вместо реальных ~15 нс?
> JIT-компилятор применяет Dead Code Elimination: результат Math.sqrt(42) никем не используется, поэтому JIT удаляет вычисление целиком. Бенчмарк измеряет пустой вызов метода. Решение: вернуть значение из @Benchmark-метода или использовать Blackhole.consume().

> [!question]- Сценарий: два бенчмарка показывают 100 +/- 15 ns/op и 110 +/- 20 ns/op. Коллега утверждает, что первый вариант быстрее. Прав ли он?
> Нет. Confidence intervals перекрываются: [85, 115] и [90, 130]. Разница статистически незначима -- она может быть случайной. Для достоверного вывода нужно либо увеличить количество fork'ов для сужения интервалов, либо признать, что варианты равноценны по производительности.

> [!question]- Почему данные для бенчмарка должны приходить через @State, а не через static final поля?
> JIT может применить Constant Folding к static final полям, вычислив результат на этапе компиляции. @State-поля создаются через reflection, и JIT не может доказать их неизменность -- вынужден выполнять вычисление при каждом вызове. Это гарантирует, что бенчмарк измеряет реальное вычисление, а не возврат константы.

> [!question]- Почему Fork=1 опасен для бенчмарков?
> Один fork -- один конкретный JVM-процесс с конкретным layout объектов в памяти, конкретными решениями JIT, конкретным поведением GC. Другой fork может дать другие результаты из-за разного расположения данных в cache lines. Минимум 3-5 fork'ов нужны для статистически значимого результата.

---

## Ключевые карточки

Какие три JIT-оптимизации искажают наивные бенчмарки?
?
Dead Code Elimination (удаляет код с неиспользуемым результатом), Constant Folding (вычисляет константные выражения при компиляции), Loop Optimization (развёртка, векторизация циклов). JMH защищает от первых двух через Blackhole и @State.

Зачем нужен warmup в JMH?
?
HotSpot использует tiered compilation: интерпретатор (50-100x медленнее) -> C1 (5x) -> C2 (пиковая). Без warmup измеряется интерпретатор, а не скомпилированный код. 5 warmup итераций по 1 секунде достаточно для C2-компиляции в большинстве случаев.

Чем Throughput отличается от SampleTime в JMH?
?
Throughput -- ops/sec, сколько операций за секунду (пропускная способность). SampleTime -- распределение latency с percentiles (p50, p90, p99). Для latency-sensitive систем SampleTime критичнее: средняя 1мс бессмысленна, если p99 = 200мс.

Что такое Blackhole в JMH?
?
Механизм предотвращения Dead Code Elimination. Blackhole "поглощает" результат вычисления через volatile-семантику и @CompilerControl. JIT вынужден вычислить значение, но без реального side-effect. Альтернатива -- вернуть значение из @Benchmark-метода.

Когда JMH НЕ подходит?
?
Макробенчмарки (целое приложение -- используй Gatling/JMeter), поиск bottleneck (используй profiling), бенчмарки с I/O (latency определяет внешняя система). JMH для изолированных вопросов: "A быстрее B?" в контролируемых условиях.
...
|-------------|------|-------|
| Следующий шаг | [[jvm-profiling]] | Профилирование находит bottleneck, JMH измеряет варианты fix'а |
| Углубиться | [[jvm-jit-compiler]] | Понимание JIT необходимо для написания корректных бенчмарков |
| Связанная тема | [[jvm-performance-overview]] | Общая картина performance engineering |
| Смежная область | [[testing-fundamentals]] | Бенчмарки как часть стратегии тестирования производительности |
