...
> [!question]- Почему -Xmx определяет не всю память JVM, и к чему это приводит в контейнерах?
> -Xmx определяет только heap. Native memory (Metaspace, thread stacks, JIT code cache, GC structures, direct buffers) не входит в Xmx. В контейнере с лимитом 4GB и -Xmx4g JVM будет убита OOM Killer, потому что суммарное потребление превысит лимит. Правило: heap должен занимать не более 60-75% от container memory limit, остальное -- для native memory.

> [!question]- Объясните, почему volatile не делает операцию counter++ атомарной, и какое решение корректно.
> counter++ -- это три операции: read, increment, write. volatile гарантирует, что каждое чтение/запись видно всем потокам, но между read и write другой поток может вклиниться. Два потока прочитают одно значение (скажем, 5), оба инкрементируют до 6, оба запишут 6 -- один инкремент потерян. Решение: AtomicInteger.incrementAndGet(), использующий CAS -- аппаратно-атомарную операцию.

> [!question]- У вас приложение в Kubernetes падает с OOM Kill, но в логах нет Java OutOfMemoryError. Где искать причину?
> OOM Kill от ОС означает, что суммарная память процесса (heap + native) превысила cgroup limit контейнера. Java OOM не логируется, потому что убивает ОС, а не JVM. Диагностика: включить NMT (-XX:NativeMemoryTracking=summary), проверить jcmd VM.native_memory, проверить количество потоков (1000 потоков * 1MB = 1GB), проверить direct ByteBuffers. Решение: уменьшить -Xmx или увеличить container memory limit.

> [!question]- Почему Virtual Threads требуют осторожного использования ThreadLocal?
> С platform threads ThreadLocal имеет десятки-сотни копий (по числу потоков в пуле). С virtual threads каждый из миллиона потоков получает свою копию ThreadLocal -- это миллион экземпляров ExpensiveObject в памяти. Решение: использовать ScopedValue (Java 21+) или shared structures с синхронизацией.

> [!question]- Как Double-Checked Locking может вернуть полусконструированный объект без volatile?
> instance = new Singleton() -- три шага: (1) выделить память, (2) вызвать конструктор, (3) присвоить ссылку. CPU/JIT может переставить шаги 2 и 3. Другой поток увидит ненулевой instance (шаг 3 выполнен), но конструктор ещё не завершён (шаг 2 не выполнен). volatile запрещает reordering: запись в instance не произойдёт до завершения конструктора.

---

## Ключевые карточки

Чем отличаются Heap и Stack в JVM?
?
Heap -- общая для всех потоков область, управляется GC, хранит объекты (new). Stack -- приватный для каждого потока, хранит локальные переменные (примитивы целиком, ссылки на объекты) и stack frames. Stack не требует GC -- память освобождается автоматически при возврате из метода.

Что такое Generational Hypothesis и почему heap разделён на поколения?
?
90-98% объектов живут очень недолго (наблюдение Ungar, 1984). Heap разделён на Young Gen (короткоживущие, собирается часто и быстро) и Old Gen (долгоживущие, собирается редко). Это избегает сканирования долгоживущих объектов при каждой сборке.

Что такое TLAB и почему new в Java быстрее malloc в C?
?
TLAB (Thread-Local Allocation Buffer) -- приватный буфер в Eden для каждого потока. Создание объекта -- просто сдвиг указателя без синхронизации между потоками. malloc требует lock на глобальную heap structure и поиск свободного блока.

Какие типы OutOfMemoryError существуют и чем они различаются?
?
Java heap space (heap переполнен), Metaspace (слишком много загруженных классов), StackOverflowError (глубокая рекурсия), unable to create new native thread (лимит потоков ОС). Каждый тип имеет свои причины и решения.

Что такое happens-before и зачем нужен volatile?
?
Happens-before -- гарантия JMM: если A happens-before B, все изменения до A видны в B. volatile обеспечивает happens-before между записью и последующими чтениями. Без volatile поток может бесконечно видеть старое значение из-за CPU cache и reordering.
...
|-------------|------|-------|
| Следующий шаг | [[jvm-gc-tuning]] | Выбор и настройка GC для heap, описанного в этом файле |
| Углубиться | [[jvm-production-debugging]] | Heap dump, thread dump -- инструменты диагностики проблем с памятью |
| Связанная тема | [[jvm-performance-overview]] | JVM Performance как система: memory + JIT + GC + I/O |
| Смежная область | [[memory-model-fundamentals]] | Модели памяти за пределами JVM -- сравнение подходов |
