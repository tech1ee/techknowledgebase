...
<details>
<summary>1. Почему polling может быть лучше interrupts при высокой нагрузке?</summary>

**Ответ:** При высокой нагрузке interrupts создают overhead — каждый interrupt = context switch + interrupt handler. При тысячах пакетов в секунду CPU тратит больше времени на обработку прерываний чем на полезную работу (interrupt livelock). Polling избегает этого overhead, просто проверяя устройство в цикле.

</details>

<details>
<summary>2. Что такое DMA и зачем он нужен?</summary>

**Ответ:** DMA (Direct Memory Access) — механизм передачи данных между устройством и RAM без участия CPU. Без DMA: CPU читает байт из устройства → пишет в RAM → повторяет. С DMA: CPU говорит контроллеру "перенеси 1MB туда", контроллер делает пока CPU занят другим. Критично для высокоскоростных устройств (диски, сеть, GPU).

</details>

<details>
<summary>3. Почему базы данных используют O_DIRECT?</summary>

**Ответ:** O_DIRECT обходит page cache ОС. БД имеют свой buffer pool, оптимизированный для паттернов доступа БД. Двойная буферизация (page cache + buffer pool) — трата RAM. Плюс БД лучше знает что кэшировать. PostgreSQL и MySQL используют O_DIRECT для datafiles.

</details>

<details>
<summary>4. Чем отличается блочное устройство от символьного?</summary>

**Ответ:**
- **Блочное:** Данные читаются/пишутся блоками фиксированного размера. Можно seek (произвольный доступ). Примеры: HDD, SSD.
- **Символьное:** Поток байтов, нет seek. Примеры: терминал, serial port, /dev/null.
