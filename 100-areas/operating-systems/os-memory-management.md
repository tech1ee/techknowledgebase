...
<details>
<summary>1. Почему malloc(1GB) может вернуть успех, даже если свободной памяти 100MB?</summary>

**Ответ:** Linux использует overcommit по умолчанию. `malloc` резервирует виртуальное адресное пространство, но физическая память выделяется только при первом обращении (demand paging). Система "надеется", что программа не использует всю запрошенную память. Если при реальном обращении памяти не хватит — OOM Killer убьёт процесс.

</details>

<details>
<summary>2. Что такое TLB и почему context switch между процессами дороже, чем между потоками?</summary>

**Ответ:** TLB (Translation Lookaside Buffer) — кэш трансляций виртуальных адресов в физические. При switch между процессами TLB нужно сбросить (flush), потому что у нового процесса другие page tables. После flush — TLB misses, каждый требует page table walk (~100-1000 cycles). Между потоками одного процесса page table общая, TLB остаётся валидным.

</details>

<details>
<summary>3. Чем отличается minor page fault от major page fault?</summary>

**Ответ:**
- **Minor:** Страница есть в памяти, но не в page table процесса. Пример: первое обращение после malloc. Ядро просто обновляет page table. Время: ~1µs.
- **Major:** Страницы нет в RAM, она в swap на диске. Ядро должно прочитать с диска. Время: ~0.1ms (SSD) — 10ms (HDD). Разница в 100-10000 раз!

</details>

<details>
<summary>4. Почему JVM heap не должен уходить в swap?</summary>

**Ответ:** Full GC сканирует весь heap. Если часть heap в swap, каждое обращение к swapped страницам = major page fault = 0.1-10ms задержки. Full GC на 8GB heap может занять 500ms в RAM, но 10+ секунд если часть в swap. Это приводит к "stop the world" паузам, убивающим latency приложения.
