...
Пользователь зарегистрировался через Google (alex@example.com). Через месяц входит через GitHub с тем же email. Автоматическая привязка опасна: если GitHub не верифицирует email, злоумышленник может получить доступ к чужому аккаунту. Безопасный подход: требовать подтверждение привязки и всегда проверять `email_verified`.
