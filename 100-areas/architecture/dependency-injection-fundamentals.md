...
**1. В чём разница между DIP и DI?**

<details>
<summary>Ответ</summary>

**DIP (Dependency Inversion Principle)** — это ПРИНЦИП из SOLID:
- Высокоуровневые модули не должны зависеть от низкоуровневых
- Оба должны зависеть от абстракций

**DI (Dependency Injection)** — это ТЕХНИКА:
- Способ передачи зависимостей в объект извне
- Конкретная реализация IoC

DIP говорит ЗАЧЕМ (зависеть от абстракций), DI говорит КАК (передавать их).

</details>

**2. Почему Constructor Injection предпочтительнее Field Injection?**

<details>
<summary>Ответ</summary>

**Constructor Injection:**
- Явные зависимости (видно из сигнатуры)
- Иммутабельность (final поля)
- Легко тестировать (передать mock в конструктор)
- Полная инициализация или ошибка

**Field Injection:**
- Скрытые зависимости
- Нет иммутабельности
- Требует reflection для тестов
- Объект может быть в неполном состоянии

</details>

**3. Что такое Composition Root?**

<details>
<summary>Ответ</summary>

**Composition Root** — единственное место в приложении, где собирается весь граф объектов.

Правило: использовать `new` для сервисов ТОЛЬКО в Composition Root.

Примеры:
- `main()` в консольном приложении
- `@Configuration` классы в Spring
- `@HiltAndroidApp` + `@Module` в Android

</details>

**4. Когда Service Locator допустим?**

<details>
<summary>Ответ</summary>

Service Locator допустим:
1. **Legacy код** — постепенный рефакторинг
2. **Инфраструктура** — middleware, interceptors
3. **Фреймворки** — когда DI технически невозможен
4. **Cross-cutting concerns** — logging (LoggerFactory)

Но для бизнес-логики DI предпочтительнее.

</details>

**5. Назови 3 проблемы кода без DI**

<details>
<summary>Ответ</summary>

1. **Невозможно тестировать** — нельзя подменить зависимости на mock
2. **Жёсткая связность** — изменение реализации требует изменения всех классов
3. **Скрытые зависимости** — не видно из API что нужно классу
4. **Нет переиспользования** — класс привязан к конкретным реализациям
5. **Нарушение SRP** — класс знает КАК создавать зависимости
