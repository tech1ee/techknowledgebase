...
### 1. В чём разница между TTFT и TPOT?

**TTFT (Time To First Token)** - время до первого токена, включает обработку промпта (prefill фаза). Важно для perceived responsiveness.

**TPOT (Time Per Output Token)** - время генерации каждого последующего токена (decode фаза). Определяет "скорость печатания".

### 2. Почему continuous batching даёт 2-23x throughput?

Static batching ждёт завершения ВСЕХ запросов в batch, даже если некоторые закончились раньше. GPU простаивает.

Continuous batching добавляет новые запросы сразу как освобождается место - GPU всегда загружен на максимум.

### 3. Какую квантизацию выбрать для H100?

**FP8** - нативная поддержка в H100/H200/B200, минимальная потеря качества (~99.5%), 2x speedup.

Для GPU без FP8 поддержки - AWQ 4-bit (лучшее качество среди 4-bit методов).

### 4. Зачем нужен speculative decoding?

Decode фаза последовательна и медленна. Speculative decoding использует маленькую модель для предсказания нескольких токенов, которые большая модель верифицирует параллельно.

Если draft угадала - получаем несколько токенов за цену одного forward pass. Speedup 2-3.5x без потери качества.

### 5. Когда SGLang лучше vLLM?

SGLang с RadixAttention лучше для:
- Multi-turn conversations (общий chat history кэшируется)
- Structured workloads (few-shot learning, common prefixes)
- Когда нужна стабильная, предсказуемая latency

vLLM лучше для quick start и OpenAI API compatibility.

### 6. Почему PagedAttention экономит 60-80% памяти?

Традиционные системы выделяют память под максимальную длину последовательности заранее. Если max_length=4096, а реальный output=100 токенов, 97.5% памяти потрачено зря.

PagedAttention выделяет блоки по требованию (как виртуальная память в ОС), потери только в последнем блоке (~4%).
...
- [LMSYS: SGLang and RadixAttention](https://lmsys.org/blog/2024-01-17-sglang/)
- [Anyscale: Continuous Batching](https://www.anyscale.com/blog/continuous-batching-llm-inference)
- [FlashAttention-3 Paper](https://tridao.me/blog/2024/flash3/)
...
- [H100 vs H200 vs B200 Guide](https://www.introl.io/blog/h100-vs-h200-vs-b200-choosing-the-right-nvidia-gpus-for-your-ai-workload)
- [Ollama vs vLLM Benchmarks](https://developers.redhat.com/articles/2025/08/08/ollama-vs-vllm-deep-dive-performance-benchmarking)
- [vLLM V1 Engine Performance](https://developers.redhat.com/articles/2025/04/28/performance-boosts-vllm-081-switching-v1-engine)

### Production Case Studies
- [ZenML: 457 LLMOps Case Studies](https://www.zenml.io/blog/llmops-in-production-457-case-studies-of-what-actually-works)
- [a16z: Enterprise AI 2025](https://a16z.com/ai-enterprise-2025/)
- [GenAI Case Studies Collection](https://github.com/themanojdesai/genai-llm-ml-case-studies)
...
- [[local-llms-self-hosting]] - Локальный запуск LLM
- [[ai-cost-optimization]] - Оптимизация затрат
- [[ai-devops-deployment]] - DevOps для AI
- [[rag-architectures]] - RAG системы

---

## Связь с другими темами

**[[mobile-ai-ml-guide]]** — Оптимизация инференса на мобильных устройствах — это экстремальный случай задач, описанных в данном материале. Техники квантизации (GGUF, AWQ), описанные здесь для серверного deployment, применяются на мобильных устройствах в ещё более агрессивных формах (INT4, weight palettization). NPU-ускорение на чипах Apple и Qualcomm — мобильный аналог GPU-оптимизаций H100/B200, описанных в разделе о Hardware.

**[[local-llms-self-hosting]]** — Self-hosting LLM — практическое применение техник оптимизации, описанных в данном материале. Выбор между vLLM, SGLang и TensorRT-LLM, настройка квантизации, PagedAttention и continuous batching — всё это напрямую используется при развёртывании собственных LLM-серверов. Данный материал даёт теоретическую базу, а self-hosting гайд — пошаговую инструкцию.

---

## Источники и дальнейшее чтение

- **Huyen, C. (2022). *Designing Machine Learning Systems*. O'Reilly.** — Практическое руководство по проектированию ML-систем для production, включая разделы об оптимизации inference, батчинге, мониторинге и масштабировании. Идеально дополняет технические детали данного материала системным взглядом на ML-инфраструктуру.

- **Goodfellow, I., Bengio, Y. & Courville, A. (2016). *Deep Learning*. MIT Press.** — Глава о вычислительных аспектах глубокого обучения объясняет фундаментальные причины memory-bound и compute-bound bottleneck'ов, которые определяют стратегии оптимизации инференса LLM.

- **Bishop, C. M. (2006). *Pattern Recognition and Machine Learning*. Springer.** — Классический учебник, дающий математическую основу для понимания квантизации, аппроксимации и trade-off'ов между точностью и вычислительной эффективностью моделей.
