...
<details>
<summary><strong>1. Чем Matryoshka embeddings отличаются от обычных? Какую проблему решают?</strong></summary>

**Matryoshka Representation Learning (MRL):**
- При обучении loss применяется к разным срезам вектора (768, 512, 256, 128...)
- Модель "front-loads" важную информацию в первые dimensions
- Можно использовать любой префикс: `embedding[:256]` работает без потери качества

**Решаемые проблемы:**
- До 14x экономия storage (1024d → 64d при 96% качества)
- Адаптивный trade-off: скорость vs качество в runtime
- Не требует переобучения для разных размерностей

**Модели с MRL:** OpenAI v3-large, Voyage-3-large, Nomic-embed-v1.5, Jina v3
</details>

<details>
<summary><strong>2. Когда использовать Binary Quantization, а когда Scalar (int8)?</strong></summary>

| Тип | Экономия | Потеря качества | Когда использовать |
|-----|----------|-----------------|-------------------|
| **Binary (1-bit)** | 32x | 5-10% (без rescoring) | Первичный поиск по миллионам векторов |
| **Scalar (int8)** | 4x | 1-3% | Rescoring top-1000, production default |
| **Float32** | 0x | 0% | Финальное ранжирование top-10 |

**Production pipeline:**
```
Binary search (in-memory, top-1000)
  → Scalar rescore (top-100)
    → Float32 rerank (top-10)
```

**С rescoring binary достигает 96%+ исходного качества.**
</details>

<details>
<summary><strong>3. Что такое Late Chunking и почему это лучше традиционного chunking?</strong></summary>

**Традиционный chunking:**
- Разбить документ на chunks
- Embed каждый chunk отдельно
- Проблема: chunk теряет контекст документа

**Late Chunking (Jina AI):**
1. Пропустить **весь документ** через transformer
2. Получить token-level embeddings с полным контекстом
3. Применить mean pooling к каждому chunk **после** encoding

**Результаты:**
- Similarity: 82-84% vs 70-75% (традиционный)
- Каждый chunk "помнит" о чём весь документ

**API:**
```python
client.embed(model="jina-v3", input=chunks, late_chunking=True)
```
</details>

<details>
<summary><strong>4. Voyage-3 vs OpenAI text-embedding-3: когда выбрать какую модель?</strong></summary>

| Критерий | OpenAI v3-large | Voyage-3-large |
|----------|-----------------|----------------|
| **Quality** | Baseline | +9.74% retrieval |
| **Context** | 8K токенов | 32K токенов |
| **MRL** | ✅ | ✅ |
| **Цена** | $0.13/1M | ~$0.12/1M |
| **Мультимодальность** | ❌ | ✅ voyage-multimodal-3 |
| **Domain-specific** | ❌ | ✅ (law, finance, code) |

**Рекомендации:**
- **OpenAI:** если уже используете OpenAI API, простая интеграция
- **Voyage:** лучшее качество retrieval, длинные документы, specialized domains
- **Бюджет:** voyage-3-lite или text-embedding-3-small (оба ~$0.02/1M)
</details>

<details>
<summary><strong>5. Когда fine-tuning embeddings оправдан, а когда это waste of time?</strong></summary>

**Fine-tuning НУЖЕН когда:**
- Специфическая терминология (медицина, право, финансы)
- Off-the-shelf модели показывают <70% retrieval accuracy
- Есть 5000+ качественных query-document пар
- Данные НЕ пересекаются с pre-training корпусом

**Fine-tuning НЕ НУЖЕН когда:**
- Проблема в chunking, а не в модели (проверьте сначала!)
- Нужен exact keyword matching → используйте hybrid search
- Мало обучающих данных (<1000 пар)
- General-purpose retrieval на типичных документах

**Ожидаемые результаты:**
- ~7% improvement с 6.3K samples
- 3 минуты обучения на consumer GPU
- Contrastive learning + hard negatives = best approach
</details>
