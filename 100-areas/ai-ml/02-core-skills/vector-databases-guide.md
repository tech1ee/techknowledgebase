...
**Почему B-tree индексы не подходят для vector search?**

<details>
<summary>Ответ</summary>

B-tree индексы оптимизированы для **упорядоченных данных** и **точных запросов**. Вектор из 1536 измерений нельзя "упорядочить" в традиционном смысле.

Vector search требует найти "похожие" записи в многомерном пространстве -- это фундаментально другая задача, требующая ANN-алгоритмов (HNSW, IVF).
</details>

### Вопрос 2

**Объясни ключевые параметры HNSW: M, ef_construction, ef_search. Как они влияют на производительность?**

<details>
<summary>Ответ</summary>

**M** -- количество связей на узел:
- Больше M = лучше recall, больше памяти
- Типичные значения: 12-48

**ef_construction** -- глубина поиска при построении:
- Больше = качественнее граф, дольше build
- Типичные значения: 100-500

**ef_search** -- глубина поиска при запросе:
- Больше = лучше recall, медленнее запрос
- Типичные значения: 50-200

**Trade-off:** Memory/build-time vs query-speed vs recall.
</details>

### Вопрос 3

**Когда использовать Binary Quantization?**

<details>
<summary>Ответ</summary>

Binary Quantization эффективна когда:
- **Высокоразмерные векторы** (>=1024 dimensions)
- **Центрированное распределение** компонент
- Нужно **32x сжатие памяти** или **40x ускорение поиска**

Примеры хорошо работающих моделей:
- OpenAI text-embedding-ada-002 (1536d): 0.98 recall с 4x oversampling
- Cohere embed-english-v2.0 (4096d): 0.98 recall с 2x oversampling

**Не подходит** для низкоразмерных (<1024d) embeddings.
</details>

### Вопрос 4

**Что такое DiskANN и когда его использовать?**

<details>
<summary>Ответ</summary>

**DiskANN** -- алгоритм от Microsoft для billion-scale поиска на SSD.

**Когда использовать:**
- Миллиарды векторов
- HNSW требует слишком много RAM ($10k+/месяц на 1B векторов)
- Приемлема latency 10-20ms (vs <5ms для in-memory)

**Преимущества:**
- 5-10x больше точек на машину
- 15-50x меньше RAM чем HNSW
- Single-node deployment для миллиардов
</details>

### Вопрос 5

**Почему Hybrid Search важен и как работает RRF?**

<details>
<summary>Ответ</summary>

**Почему важен:**
- Vector search понимает семантику, но плох для exact matches ("SKU-12345")
- BM25 находит точные совпадения, но не понимает смысл
- Вместе покрывают оба сценария
...
```
- Объединяет результаты по рангам, не по scores
- Scores из разных систем несопоставимы
- k=60 -- стандартное сглаживающее значение
</details>

### Вопрос 6

**Как выбрать chunking strategy для RAG?**

<details>
<summary>Ответ</summary>

**Рекомендации:**
1. **Размер:** 200-500 tokens с 50-100 overlap
2. **Стратегия:** Recursive (по параграфам -> предложениям) для большинства случаев
3. **Match** chunk size к training data embedding модели

**Advanced:**
- Semantic chunking для coherent blocks
- LLM-based chunking для complex documents
- Contextual Retrieval (Anthropic) для сохранения контекста

**Тестируйте** разные стратегии на своих данных -- нет universal solution.
</details>
...
| 11 | [Reddit Vector DB Case Study](https://milvus.io/blog/choosing-a-vector-database-for-ann-search-at-reddit.md) | Case Study | Billion-scale выбор |
...
- **Bishop C.M. (2006). Pattern Recognition and Machine Learning. Springer.** — фундаментальное изложение методов кластеризации, ближайших соседей и снижения размерности, что является математической основой алгоритмов поиска в векторных БД
- **Goodfellow I., Bengio Y., Courville A. (2016). Deep Learning. MIT Press.** — глубокое рассмотрение representation learning и пространств эмбеддингов, что необходимо для понимания семантического поиска в векторных базах
- **Huyen C. (2022). Designing Machine Learning Systems. O'Reilly.** — практические паттерны работы с данными в ML-системах, включая выбор хранилищ, индексирование и масштабирование, применимые к продакшен-деплойменту векторных БД
