...
### Вопрос 1: Что такое Constrained Decoding?

<details>
<summary>Ответ</summary>

**Constrained Decoding** — это техника ограничения множества допустимых токенов на каждом шаге генерации.

Вместо выбора из всех 50,000+ токенов словаря, модель может выбирать только те токены, которые ведут к валидному результату по заданной схеме.

**Пример для JSON:**
- После `{` допустимы только: `"`, `}`, whitespace
- После `"name":` допустимы: `"`, `null`, цифры (в зависимости от схемы)
- Для enum `["USD", "EUR"]` после `"` допустимы только: `U`, `E`

**Реализации:**
- **XGrammar**: Pushdown automaton, до 100x ускорение
- **Outlines**: FSM-based, pioneer в open source
- **LM Format Enforcer**: Динамическая проверка
- **llguidance (Microsoft)**: ~50μs per token

Это гарантирует 100% соответствие схеме на уровне inference.

</details>

### Вопрос 2: Почему LLM НЕ выполняет функции при Function Calling?

<details>
<summary>Ответ</summary>

Потому что LLM — это языковая модель, работающая в изолированной среде. Она:

1. **Не имеет доступа** к внешним системам (API, базы данных, файлы)
2. **Не может выполнять код** — только генерировать текст
3. **Статична** — её веса не меняются при inference

LLM при Function Calling:
- Анализирует запрос пользователя
- Определяет, какая функция нужна
- Генерирует JSON с именем функции и аргументами
- Возвращает этот JSON как часть ответа

**Твой код** должен:
- Распарсить tool_call
- Выполнить реальную функцию
- Вернуть результат обратно LLM

Это разделение — ключевой аспект безопасности и архитектуры.

</details>

### Вопрос 3: Чем JSON Mode отличается от Structured Outputs?

<details>
<summary>Ответ</summary>

| Аспект | JSON Mode | Structured Outputs |
|--------|-----------|-------------------|
| Валидный JSON синтаксис | Да | Да |
| Соответствие схеме | Нет | Да |
| Required поля | Не гарантировано | Гарантировано |
| Типы данных | Не гарантированы | Точно по схеме |
| Enum значения | Любые | Только из списка |
| Механизм | Constrained decoding для синтаксиса | Constrained decoding для схемы |
| Success rate для схемы | ~90-95% | 100% |

**JSON Mode** гарантирует только `{...}` — валидный JSON.
**Structured Outputs** гарантирует `{"name": str, "age": int}` — точную структуру.

</details>

### Вопрос 4: Что делает Instructor и почему он популярен?

<details>
<summary>Ответ</summary>

**Instructor** — Python библиотека (3M+ downloads/month) для работы со structured outputs.

**Что делает:**
1. Конвертирует Pydantic models в JSON Schema автоматически
2. Патчит API клиенты (OpenAI, Anthropic, etc.) для поддержки `response_model`
3. Автоматически парсит и валидирует ответы через Pydantic
4. Ретраит запросы при ошибках валидации
5. Поддерживает streaming partial results

**Почему популярен:**
- Единый API для 15+ провайдеров
- Минимум boilerplate кода
- Type safety и IDE autocomplete
- Надёжность через automatic retries
- Активное развитие и поддержка

```python
# Без Instructor: ~50 строк кода
# С Instructor: 5 строк
...
### Вопрос 5: Какие основные ошибки при работе с tools?

<details>
<summary>Ответ</summary>

1. **Плохие описания tools**
   - LLM не понимает когда использовать
   - Решение: детальные descriptions с примерами

2. **Слишком много tools (>15)**
   - LLM путается в выборе
   - Решение: группировка, RAG-based selection, Tool Search Tool

3. **Отсутствие валидации аргументов**
   - Security риски (injection)
   - Решение: Pydantic validation, allowlists

4. **Огромные tool results**
   - Переполнение context window
   - Решение: пагинация, суммаризация

5. **Игнорирование ошибок**
   - LLM не может адаптироваться
   - Решение: информативные error messages

6. **Strict format снижает reasoning quality**
   - Модель фокусируется на формате
   - Решение: двухэтапный подход (reasoning → structuring)

</details>

### Вопрос 6: Когда использовать какую библиотеку?

<details>
<summary>Ответ</summary>

**Instructor**: Extraction из hosted моделей (OpenAI, Anthropic). Нужны retries, streaming, multi-provider.

**Pydantic AI**: Agents с type-safe tools. Интеграция с Pydantic ecosystem.

**Outlines**: Self-hosted модели. Regex, grammar constraints. Research/experimentation.

**XGrammar**: Production vLLM/SGLang. Максимальная производительность.

**BAML**: Сложные schemas, где format constraints снижают качество. Multi-language support.

**LangChain**: Full agentic framework. Много integrations. Enterprise features.

**Marvin**: Простые AI functions. Minimal code. Декларативный стиль.
