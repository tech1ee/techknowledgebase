...
1. **Q: Какой паттерн retry использовать для rate limit errors?**
   A: Exponential backoff с jitter. Начать с 1 сек, удваивать до max 60 сек, добавить random jitter для предотвращения thundering herd. Уважать Retry-After header если есть.

2. **Q: Как снизить costs на 50% без потери качества?**
   A: Prompt caching (90% скидка на cached tokens) + model routing (дешёвые модели для простых задач) + Batch API (50% скидка). В сумме 60-80% экономии.

3. **Q: Чем LiteLLM лучше прямых SDK?**
   A: Unified interface для 100+ провайдеров, автоматический fallback, cost tracking, load balancing. Один код — любой провайдер.

4. **Q: Как обеспечить 100% соответствие JSON schema в ответах?**
   A: OpenAI Structured Outputs с Pydantic моделями. SDK автоматически конвертирует схему, валидирует ответ, и парсит в typed объект.

5. **Q: Streaming vs обычные запросы — когда что?**
   A: Streaming для chat interfaces (мгновенный UX). Обычные запросы для background processing, когда нужен полный ответ сразу (pipelines, batch).
...
- [Google GenAI SDK](https://github.com/google-gemini/cookbook)

### Best Practices & Guides
- [LLM API Integration Best Practices](https://www.21medien.de/en/blog/llm-api-integration-best-practices)
- [API Rate Limits Explained 2025](https://orq.ai/blog/api-rate-limit)
- [LLM Cost Optimization Guide](https://ai.koombea.com/blog/llm-cost-optimization)
- [Prompt Caching Deep Dive](https://ngrok.com/blog/prompt-caching/)
- [Error Handling for Production LLM](https://markaicode.com/llm-error-handling-production-guide/)
...
- [[local-llms-self-hosting]] - Локальные LLM и Self-Hosting
- [[structured-outputs-tools]] - Structured Outputs подробно
- [[mcp-model-context-protocol]] - MCP для интеграций
...
*Создано: 2025-12-28 | Обновлено: 2025-12-28*

[[ai-engineering-moc|← AI Engineering MOC]] | [[local-llms-self-hosting|← Local LLMs]] | [[mcp-model-context-protocol|MCP →]]
