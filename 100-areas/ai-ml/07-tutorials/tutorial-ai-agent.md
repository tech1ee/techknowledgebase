...
1. **Что такое StateGraph и зачем он нужен?**
   > StateGraph -- граф состояний LangGraph. Управляет workflow агента, хранит состояние (messages, iteration) и передает его между узлами.

2. **Как работает human-in-the-loop?**
   > Инструменты, требующие подтверждения, помечены в APPROVAL_REQUIRED. При их вызове граф прерывается через interrupt(), ожидает ответа пользователя и продолжает с Command(resume=...).

3. **Какие виды guardrails существуют?**
...
4. **Зачем нужен MAX_ITERATIONS?**
   > Защита от бесконечных циклов и runaway costs. Без лимита агент может зациклиться, расходуя API-кредиты.

5. **Short-term vs Long-term memory?**
   > Short-term: контекст текущего разговора, как RAM.
   > Long-term: persistent storage между сессиями через vector store, как HDD.

---

## Актуальность 2025: Выбор фреймворка

### Сравнение фреймворков (декабрь 2025)
...
|  - Multi-role collaboration → CrewAI                                      |
|  - OpenAI stack + velocity → OpenAI Agents SDK                            |
|  - Iterative reasoning → AutoGen                                          |
|  - Gemini + multimodal → Google ADK                                       |
|                                                                            |
+----------------------------------------------------------------------------+
```

### OpenAI Agents SDK: Быстрый старт
...
# Определяем tool через декоратор
@function_tool
def get_weather(city: str) -> str:
    """Get current weather for a city."""
    # Pydantic validation автоматически
...
writer = Agent(
    ...
    handoffs=[researcher]  # Может делегировать researcher'у
)
...
orchestrator = Agent(
    ...
    - For writing → delegate to Writer""",
    handoffs=[researcher, writer]
)

# Запуск с orchestrator
result = Runner.run_sync(
    orchestrator,
    messages=[{
        "role": "user",
...
### Когда что выбрать (Decision Tree)
...
```
            ...
            ┌─────────────────────┼─────────────────────┐
            ▼                     ▼                     ▼
    Нужен контроль?         Multi-role?          OpenAI only?
            │                     │                     │
            ▼                     ▼                     ▼
    ┌───────────────┐    ┌───────────────┐    ┌───────────────┐
    │   LangGraph   │    │    CrewAI     │    │ OpenAI Agents │
    │               │    │               │    │     SDK       │
    │ + Full control│    │ + Easy teams  │    │ + Fast start  │
    │ + Stateful    │    │ + Autonomous  │    │ + Tracing     │
    │ - Verbose     │    │ - Less control│    │ - Lock-in     │
    └───────────────┘    └───────────────┘    └───────────────┘
```
...
def send_email(to: str, subject: str, body: str) -> str:
    """Отправляет email. Требует подтверждения пользователя."""
    # Реализация
    return f"Email sent to {to}"

# Добавить в APPROVAL_TOOLS в tools/__init__.py
```
...
Этот tutorial даёт практические навыки создания агента, а ai-agents-advanced раскрывает теорию и продвинутые архитектуры. После прохождения tutorial рекомендуется изучить multi-agent patterns (supervisor, hierarchical, collaborative), advanced memory systems и planning strategies. Практический опыт из tutorial'а позволяет глубже понять абстрактные концепции: ReAct loop, tool use protocol и state management обретают конкретный смысл, когда вы уже написали работающего агента.

### [[prompt-engineering-guide]]

System prompts — основной механизм управления поведением агента, и их качество определяет, насколько надёжно агент следует инструкциям, выбирает правильные tools и избегает hallucinations. Техники из prompt engineering (chain-of-thought, few-shot examples, structured output instructions) напрямую применяются при написании agent system prompts. Итерационное улучшение промптов на основе debugging traces — ключевой skill для AI engineer.

### [[langchain-ecosystem]]

LangGraph, использованный в tutorial'е, является частью большей экосистемы LangChain, включающей orchestration, observability (LangSmith), deployment (LangGraph Cloud) и community ресурсы. Понимание экосистемы помогает выбрать правильные инструменты для каждого этапа: LangSmith для debugging, LangGraph Platform для deployment, LangChain Hub для sharing промптов и chains. Знание альтернатив (CrewAI, OpenAI SDK, Pydantic AI) позволяет сделать informed decision о выборе стека.

---

**Связанные материалы:**
- [[ai-agents-advanced]] -- глубже про multi-agent архитектуры
- [[prompt-engineering-guide]] -- как писать эффективные system prompts
- [[tutorial-rag-chatbot]] -- RAG система для расширения знаний агента
- [[langchain-ecosystem]] -- обзор LangChain/LangGraph экосистемы
