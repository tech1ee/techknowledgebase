...
1. **Jemerov D., Isakova S. (2017).** *Kotlin in Action.* — Понимание основ Kotlin необходимо для работы с Kotlin/Wasm: система типов, корутины и мультиплатформенные модули определяют, как Kotlin-код компилируется в WebAssembly и взаимодействует с браузерными API.
2. **Moskala M. (2021).** *Effective Kotlin.* — Оптимизация Kotlin-кода критически важна для Web: размер бандла, минимизация аллокаций и эффективное использование корутин напрямую влияют на производительность веб-приложения и время загрузки.
