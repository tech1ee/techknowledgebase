...
1. **Moskala M. (2022).** *Kotlin Coroutines: Deep Dive.* — Корутины на iOS работают через Kotlin/Native с особенностями: main dispatcher привязан к Main RunLoop, freeze/unfreeze (в старых версиях). Книга помогает понять корутинную модель, критически важную для iOS-интеграции.
2. **Moskala M. (2021).** *Effective Kotlin.* — Практические рекомендации по написанию Kotlin-кода, совместимого с iOS: управление памятью, обработка null safety при interop с Objective-C, и паттерны, дружественные к Swift Export.
3. **Martin R. (2017).** *Clean Architecture.* — Принципы boundary objects и dependency inversion определяют, как проектировать интерфейс между shared KMP-модулем и iOS-приложением. Помогает минимизировать связанность и упростить iOS-специфичные адаптации.
