...
**[[android-compose]]** — Jetpack Compose — это декларативный UI-фреймворк Google, построенный на Kotlin-компиляторе с plugin-архитектурой. Заметка детально разбирает Composition, recomposition, remember, side effects, Modifier chains и интеграцию с Navigation и ViewModel. Понимание внутренних механизмов Compose (slot table, gap buffer) помогает объяснить, почему recomposition работает иначе, чем SwiftUI body re-evaluation.

**[[ios-swiftui]]** — SwiftUI — это декларативный UI-фреймворк Apple, тесно интегрированный с Swift через property wrappers и result builders. Заметка раскрывает @State/@Binding/@StateObject, view identity (structural vs explicit), Environment, Preferences и анимации. Сравнение с Compose в текущем файле показывает, что SwiftUI оценивает весь body целиком, а Compose — только изменённые composable functions, что влияет на производительность.

**[[compose-mp-overview]]** — Compose Multiplatform расширяет Jetpack Compose на iOS, Desktop и Web, позволяя писать shared UI на Kotlin. Заметка описывает архитектуру CMP, ComposeUIViewController для iOS, platform-specific composables и текущие ограничения. Это практическое продолжение теоретического сравнения SwiftUI vs Compose из текущего файла — как объединить лучшее из обоих миров.

---

## Источники и дальнейшее чтение

- **Moskala M. (2021). *Effective Kotlin*.** — Kotlin-специфичные паттерны (DSL builders, inline functions, receivers) лежат в основе Compose API. Понимание этих конструкций помогает писать идиоматичные composable functions и custom modifiers.
- **Neuburg M. (2023). *iOS Programming Fundamentals*.** — Раскрывает Swift language features (property wrappers, result builders), которые являются основой SwiftUI. Помогает понять, почему SwiftUI API выглядит так, а не иначе, и чем он отличается от Compose.
- **Gamma E. et al. (1994). *Design Patterns*.** — Composite, Decorator и Observer — это паттерны, которые декларативные UI-фреймворки реализуют «под капотом». Понимание этих паттернов помогает осознанно проектировать переиспользуемые UI-компоненты в обеих экосистемах.
