...
**[[ios-process-memory]]** — Управление памятью на iOS основано на ARC и тесно связано с понятием ownership. Заметка раскрывает работу Instruments (Leaks, Allocations, Memory Graph), диагностику retain cycles и паттерны weak/unowned. Понимание iOS-специфичных memory warnings и jetsam-механизма критически важно для кросс-платформенной разработки, где один и тот же код может вести себя по-разному на разных таргетах.

**[[android-process-memory]]** — Android использует garbage collection (G1/CMS на современных версиях), что создаёт принципиально другие проблемы: GC pauses, Stop-the-World, фрагментация heap. Заметка подробно разбирает Android Profiler, LeakCanary и техники минимизации аллокаций. Сравнение с ARC в текущем файле показывает, что детерминизм iOS (deinit вызывается предсказуемо) vs недетерминизм Android (finalize deprecated) влияет на проектирование ресурсоёмких компонентов.

**[[garbage-collection-explained]]** — Теоретические основы сборки мусора: mark-and-sweep, generational GC, concurrent collectors. Эта заметка даёт фундамент для понимания, почему GC на Android может вызывать janky frames, и почему Apple выбрала ARC вместо tracing GC. Знание алгоритмов GC помогает оптимизировать Kotlin-код, который работает на JVM (Android) и через Kotlin/Native (iOS) с разными моделями памяти.

---

## Источники и дальнейшее чтение

- **Neuburg M. (2023). *iOS Programming Fundamentals*.** — Объясняет ARC, retain cycles, weak/unowned references и инструменты диагностики утечек памяти в Xcode. Книга даёт практическое понимание ownership model, которая лежит в основе всей iOS-разработки.
- **Meier R. (2022). *Professional Android*.** — Раскрывает работу garbage collector на Android, memory profiling, LeakCanary и оптимизацию аллокаций. Помогает понять, как избежать GC pauses и OutOfMemoryError в production-приложениях.
- **Moskala M. (2021). *Effective Kotlin*.** — Содержит рекомендации по эффективному использованию памяти в Kotlin: избегание лишних аллокаций, работа с sequences, inline-классы. Особенно ценно для KMP-разработки, где один код работает на платформах с разными моделями памяти.
