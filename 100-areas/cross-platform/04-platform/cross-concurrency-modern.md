...
[[ios-async-await]] — Swift async/await (с Swift 5.5) — это structured concurrency с компиляторной проверкой потокобезопасности через Sendable и actor isolation. Заметка глубоко разбирает Task, TaskGroup, AsyncSequence, @MainActor, а также подводные камни: suspension points, task cancellation, actor reentrancy. Понимание Swift Concurrency необходимо для корректного сравнения с Kotlin Coroutines и для интеграции KMP-кода на стороне iOS через SKIE.

[[kotlin-coroutines]] — Kotlin Coroutines — основа современной конкурентности на Android и в KMP. Заметка охватывает suspend functions, CoroutineScope, structured concurrency, Dispatchers, SupervisorJob, exception handling и Flow. Корутины реализуют cooperative multitasking через continuation passing, что принципиально отличается от preemptive модели потоков. Глубокое понимание корутин необходимо для сравнения с Swift async/await и для написания shared-кода в KMP-проектах.

[[async-models-overview]] — Обзор моделей асинхронного программирования: callbacks, Promises/Futures, Rx, async/await и actor model. Заметка помещает Swift и Kotlin подходы в контекст эволюции асинхронных моделей — от callback hell через reactive streams к structured concurrency. Это фундаментальное знание помогает понять, почему обе платформы пришли к похожим решениям и в чём концептуальные отличия их реализаций.

---

## Источники и дальнейшее чтение

- Moskala M. (2021). *Effective Kotlin: Best Practices.* — Содержит главы о корутинах, Flow и best practices асинхронного программирования на Kotlin. Практические рекомендации по structured concurrency, обработке ошибок и тестированию асинхронного кода.
- Neuburg M. (2023). *iOS Programming Fundamentals with Swift.* — Разбирает Swift Concurrency (async/await, Task, actor), а также legacy-подходы (GCD, Operation). Помогает понять, как iOS-разработчики мигрируют с callback-based кода на structured concurrency.
- Skeen J. et al. (2019). *Kotlin Programming: The Big Nerd Ranch Guide.* — Вводит в Kotlin с нуля, включая корутины и асинхронное программирование. Полезна как базовый справочник по языковым конструкциям, используемым в корутинах.
