...
### Вопрос 1: Порядок stages

Расположите stages iOS CI/CD pipeline в правильном порядке:
A) Upload to TestFlight
B) Run tests
C) Code signing
D) Build archive
E) Checkout code
...
**Правильный порядок: E → D → B → C → A**

1. **E) Checkout code** - получаем исходный код
2. **D) Build archive** - компилируем приложение (можно без signing для тестов)
3. **B) Run tests** - проверяем качество кода
4. **C) Code signing** - подписываем для дистрибуции
5. **A) Upload to TestFlight** - загружаем в App Store Connect

Примечание: иногда тесты запускаются на simulator builds (без signing), а archive+sign делается параллельно или после.
</details>

### Вопрос 2: Match vs Manual signing

Какой подход к code signing в CI лучше использовать и почему?

A) Хранить .p12 и .mobileprovision в репозитории проекта
B) Использовать match с отдельным git репозиторием
C) Каждый раз генерировать новые сертификаты
D) Использовать Xcode Automatic Signing

<details>
<summary>Ответ</summary>

**Правильный ответ: B) Использовать match с отдельным git репозиторием**

Объяснение:
- **A) Неправильно** - сертификаты нельзя хранить в публичном или даже приватном репозитории проекта (security risk)
- **B) Правильно** - match шифрует сертификаты, хранит в отдельном репозитории, синхронизирует между командой
- **C) Неправильно** - есть лимит на количество сертификатов, plus это долго
- **D) Неправильно** - Automatic Signing не работает в headless CI environment

Преимущества match:
- Единый источник истины для всей команды
- Шифрование с паролем
- Автоматическое обновление
- Поддержка readonly mode для CI
</details>

### Вопрос 3: Оптимизация build time

Ваш iOS CI pipeline занимает 45 минут. Какие три техники помогут сократить время?
...
1. **Кэширование зависимостей**
   ...
   - DerivedData для инкрементальных builds
   - Используйте `hashFiles('Podfile.lock')` для cache key

2. **Параллельные jobs**
   - Lint, Unit Tests, UI Tests могут работать параллельно
   - Разделите UI тесты по device types
   - Matrix builds для разных iOS версий

3. **Incremental builds**
   - Не делайте `clean` без необходимости
   - Используйте DerivedData cache между runs
   - Build-for-testing + Test-without-building separation

Дополнительно:
- Используйте Apple Silicon runners (M1/M2) - в 2-3 раза быстрее Intel
- Минимизируйте вывод (`xcpretty`, `silent: true`)
- Оптимизируйте сами тесты (mock network, in-memory DB)
</details>

### Вопрос 4: Диагностика

TestFlight upload падает с ошибкой "No suitable application records were found". Что проверить?
...
1. **Bundle ID**
   - Bundle ID в Xcode совпадает с App Store Connect?
   - Нет опечаток: `com.company.app` vs `com.company.App`

2. **App Store Connect**
   - Приложение создано в App Store Connect?
   - Статус не "Ready for Sale" с замороженной версией?
   - Нет pending submissions которые блокируют?

3. **API Key / Credentials**
   - Правильный Team ID?
   - API Key имеет права Admin или App Manager?
   - API Key не отозван?

4. **Version / Build Number**
   - Version number валидный (x.y.z)?
   - Build number уникальный и больше предыдущего?
   - Не используется build number который уже был uploaded?

5. **Provisioning Profile**
   - Profile type = App Store (не AdHoc, не Development)?
   - Profile не истёк?
   - Profile для правильного Bundle ID?
...
- [[ios-code-signing]] - Как работает подписание кода в iOS
- [[ios-app-distribution]] - Способы распространения iOS приложений

### Related (связанные темы)
- [[android-ci-cd]] - CI/CD для Android (сравнение подходов)
- [[ios-testing]] - Тестирование iOS приложений
- [[git-workflows]] - Git branching стратегии для CI/CD

### Next Steps (изучить после)
- [[ios-feature-flags]] - Управление фичами и A/B тесты
- [[ios-crash-reporting]] - Интеграция с Crashlytics/Sentry
- [[ios-performance-monitoring]] - Мониторинг производительности
