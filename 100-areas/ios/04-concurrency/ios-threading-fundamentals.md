...
- **Neuburg M. (2023). *iOS 17 Programming Fundamentals with Swift.* O'Reilly.** — Охватывает как классический GCD (DispatchQueue, QoS, sync/async), так и современный Swift Concurrency (async/await, actors). Подробно разбирает Main Thread и RunLoop, что является необходимой базой для понимания многопоточности в iOS.
- **Eidhof C., Airspeed Velocity, et al. (2019). *Advanced Swift.* objc.io.** — Глубокое погружение в модель памяти Swift, value types vs reference types и их влияние на потокобезопасность. Понимание copy-on-write, ARC и capture semantics критически важно для написания корректного многопоточного кода без утечек памяти и retain cycles в async-блоках.
- **Keur A., Hillegass A. (2020). *iOS Programming: The Big Nerd Ranch Guide.* Big Nerd Ranch.** — Практическое руководство с примерами использования GCD для фоновой загрузки данных, обработки изображений и обновления UI. Паттерны из этой книги (background fetch + main thread update) являются каноническими для iOS-разработки.
