...
**Почему нельзя создавать DateFormatter в cellForRowAt?**

<details>
<summary>Ответ</summary>

DateFormatter — очень дорогой объект для создания. Его инициализация занимает несколько миллисекунд. При скролле cellForRowAt вызывается десятки раз в секунду, и создание DateFormatter каждый раз:
- Блокирует main thread
- Может превысить бюджет 16.67мс на кадр
- Приводит к дёрганью скролла

**Решения:**
1. Создать DateFormatter один раз как static property
2. Предвычислять отформатированные даты при создании модели
3. Использовать кэш для форматированных строк
...
**В чём разница между reloadData() и apply(snapshot)?**
...
| reloadData() | apply(snapshot) |
|--------------|-----------------|
| Перезагружает ВСЕ ячейки | Обновляет только изменённые |
| Нет анимаций | Автоматические анимации |
| Мерцание экрана | Плавные переходы |
| Может crash при несинхронизации | Безопасно, crash невозможен |
| Простой, но неэффективный | Сложнее, но производительнее |

Diffable Data Source автоматически вычисляет разницу между старым и новым состоянием, и анимирует только изменения. Это как git diff для UI.
</details>

### Вопрос 3

**Когда вызывается prepareForReuse() и что в нём делать?**

<details>
<summary>Ответ</summary>

`prepareForReuse()` вызывается когда ячейка:
1. Уходит за пределы экрана
2. Возвращается в reuse pool
3. Готовится к переиспользованию для других данных

**Что делать в prepareForReuse():**
- Отменять незавершённые сетевые запросы
- Сбрасывать изображения в nil
- Останавливать анимации
- Сбрасывать прогресс загрузки

**Что НЕ делать:**
- Удалять subviews (они переиспользуются)
- Менять constraints
- Тяжёлые вычисления
...
**Почему в SwiftUI List важно использовать стабильный id?**

<details>
<summary>Ответ</summary>

SwiftUI использует `id` для определения **identity** элемента — является ли это тем же самым элементом или новым.

**Нестабильный id (плохо):**
```swift
List(items, id: \.title)  // Если title изменится — view пересоздаётся!
```

**Проблемы:**
- При изменении title SwiftUI думает что это новый элемент
- View полностью пересоздаётся вместо обновления
- Теряется состояние (scroll position, text field input)
- Анимации ломаются

**Стабильный id (хорошо):**
```swift
struct Item: Identifiable {
    let id: UUID  // Никогда не меняется для существующего item
    var title: String
}

List(items) { item in  // Использует item.id
    ItemRow(item: item)
}
```

Теперь при изменении title SwiftUI знает что это тот же элемент и просто обновляет его содержимое.
</details>
...
- [[ios-view-rendering]] — Как iOS рендерит views, Core Animation, CALayer
- [[ios-uikit-fundamentals]] — Основы UIView, UIViewController, Auto Layout

### Related (связанные темы)
- [[ios-performance-profiling]] — Instruments, Time Profiler, Core Animation
- [[ios-swiftui]] — Декларативный UI, List, LazyVStack
- [[ios-combine]] — Reactive programming для данных
- [[ios-memory-management]] — ARC, retain cycles в ячейках

### See also (для углубления)
- [[ios-core-animation]] — Анимации, CALayer, implicit/explicit animations
- [[ios-networking]] — Загрузка изображений, кэширование
...
| 2020 | [Lists in UICollectionView](https://developer.apple.com/videos/play/wwdc2020/10026/) | List Configuration |
...
| 2023 | [What's new in UIKit](https://developer.apple.com/videos/play/wwdc2023/10055/) | Latest UIKit updates |
