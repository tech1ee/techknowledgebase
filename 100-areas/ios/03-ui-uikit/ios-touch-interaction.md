...
class CustomSuperview: UIView {
    override func touchesBegan(_ touches: Set<UITouch>, with event: UIEvent?) {
        print("CustomSuperview")
        // super НЕ вызывается
    }
}
```
**Что напечатается при касании CustomView, если она subview CustomSuperview?**

A) `CustomView`
B) `CustomView` → `CustomSuperview`
C) `CustomSuperview` → `CustomView`
D) Ничего не напечатается
...
Responder chain идет от child к parent. CustomView печатает "CustomView" и вызывает `super`, который передает событие по chain вверх к CustomSuperview.
</details>

### Вопрос 2: Gesture Dependencies
```swift
...
```
**Что происходит при double tap на view?**

A) Срабатывают оба gesture
B) Срабатывает только `tap`
C) Срабатывает только `doubleTap`
D) Не срабатывает ни один
...
`tap.require(toFail: doubleTap)` означает, что `tap` ждет, пока `doubleTap` не провалится. При double tap `doubleTap` распознается успешно, поэтому `tap` не срабатывает.
</details>

### Вопрос 3: @GestureState
```swift
struct TestView: View {
    ...
    var body: some View {
        Circle()
            .offset(x: finalOffset.width + offset.width,
                    y: finalOffset.height + offset.height)
            .gesture(
                DragGesture()
...
```
**Что происходит после окончания drag gesture?**

A) Circle остается на месте окончания drag
B) Circle возвращается на позицию `finalOffset`
C) Circle возвращается в начальную позицию (0, 0)
D) Circle исчезает
...
`@GestureState` автоматически сбрасывается в `.zero` после окончания жеста. Без `.onEnded` для обновления `finalOffset`, круг возвращается на последнюю сохраненную позицию в `@State`.
</details>

### Вопрос 4: Hit Testing
```swift
...
```
**Что происходит с touch events на ParentView и его subviews?**

A) Обрабатываются только subviews
B) Обрабатывается только ParentView
C) Все touch events игнорируются
D) Touch events передаются в responder chain
...
Возвращая `nil` из `hitTest`, ParentView сообщает, что ни она, ни её subviews не должны получать touch events. Это эффективно делает всю иерархию "неосязаемой".
</details>

### Вопрос 5: Gesture Priority
```swift
struct TestView: View {
    var body: some View {
        Text("Hello")
            .gesture(
                TapGesture().onEnded { print("A") }
            )
            .simultaneousGesture(
                TapGesture().onEnded { print("B") }
            )
            .highPriorityGesture(
                TapGesture().onEnded { print("C") }
            )
    }
}
```
**Что напечатается при tap на Text?**
...
`highPriorityGesture` (C) срабатывает первым и блокирует обычный `.gesture` (A), но `simultaneousGesture` (B) работает параллельно и не блокируется.
</details>

### Вопрос 6: Multi-Touch
```swift
...
// isMultipleTouchEnabled НЕ установлен
```
**Сколько касаний можно получить одновременно?**

A) 0 (multi-touch отключен полностью)
B) 1 (только первое касание)
C) До 5 касаний
D) Неограниченно
...
По умолчанию `isMultipleTouchEnabled = false`, что означает, что view получает только первое касание. Последующие пальцы игнорируются, пока первый не поднимется.
</details>

### Вопрос 7: Gesture Composition
```swift
struct TestView: View {
    var body: some View {
        Rectangle()
...
```
**Когда напечатается "Completed"?**

A) После 1 секунды long press
B) После окончания drag
C) После 1 секунды long press И drag
D) После любого из жестов
...
`.sequenced` требует последовательного выполнения: сначала должен завершиться long press (1 секунда), затем drag. `.onEnded` срабатывает только после завершения обоих.
</details>

### Вопрос 8: Touch Phases
```swift
...
```
**Что не так с этим кодом при interruptions (звонок)?**

A) Ничего, код корректный
B) touchCount может стать отрицательным
C) touchCount не сбросится до 0
D) touchCount будет удваиваться
...
При interruption (звонок, notification) вызывается `touchesCancelled`, а не `touchesEnded`. Без обработки `touchesCancelled` счетчик останется > 0, что приведет к багам.
</details>

## Связанные темы

- [[uikit-views]] - UIView иерархия и lifecycle
- [[swiftui-views]] - View композиция в SwiftUI
- [[ios-animations]] - Анимации и transitions
- [[uikit-gesture-recognizers]] - Детали UIGestureRecognizer
- [[swiftui-state-management]] - @State, @GestureState, @Binding
- [[ios-accessibility]] - VoiceOver и assistive touch
- [[ios-haptics]] - Тактильная обратная связь
- [[core-animation]] - CALayer и низкоуровневые анимации
- [[ios-responder-chain]] - Подробно про responder chain
- [[swift-concurrency]] - Async gesture handling

---

## Связь с другими темами

- **[[ios-uikit-fundamentals]]** — UIKit предоставляет базовую инфраструктуру для обработки касаний: UIView, UIResponder, UIWindow. Без понимания иерархии view и responder chain невозможно корректно маршрутизировать touch events. Hit testing проходит по дереву view сверху вниз, а responder chain поднимается снизу вверх — эти механизмы являются фундаментом touch handling в UIKit.

- **[[ios-custom-views]]** — Создание custom views напрямую связано с обработкой касаний, поскольку кастомные view часто требуют переопределения touchesBegan/Moved/Ended, кастомного hit testing через hitTest(_:with:) и point(inside:with:), а также реализации собственных gesture recognizers. Понимание draw(_:) и setNeedsDisplay важно для визуальной обратной связи при жестах.

- **[[android-touch-handling]]** — Android использует аналогичную, но отличающуюся модель обработки касаний: dispatchTouchEvent вместо hitTest, onTouchEvent вместо touchesBegan, и ViewGroup.onInterceptTouchEvent для перехвата событий родителем. Сравнение двух платформ углубляет понимание архитектурных решений в touch handling и помогает при кросс-платформенной разработке.

---

## Источники и дальнейшее чтение

- Neuburg M. (2023). *iOS 17 Programming Fundamentals with Swift.* — Подробно разбирает UIResponder, responder chain и gesture recognizers с нуля, объясняя как iOS маршрутизирует touch events через иерархию view.

- Keur C., Hillegass A. (2020). *iOS Programming: The Big Nerd Ranch Guide.* — Практические примеры реализации custom gesture recognizers и multi-touch обработки, с пошаговыми проектами для закрепления навыков.

- Eidhof C., Airspeed Velocity, Javier N. (2019). *Advanced Swift.* — Глубокое понимание Swift-протоколов и generics, необходимое для создания типобезопасных абстракций над gesture handling, включая композицию жестов в SwiftUI.

---

**Практика:** Создайте приложение с draggable, rotatable, scalable cards. Реализуйте в UIKit и SwiftUI, добавьте multi-touch support для одновременного взаимодействия с несколькими cards. Добавьте haptic feedback и accessibility support.