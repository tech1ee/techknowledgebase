...
<details>
<summary><strong>Вопрос 1:</strong> В чём разница между @State и @StateObject?</summary>

**Ответ:**
- `@State` — для **value types** (Int, String, struct)
- `@StateObject` — для **reference types** (class, ObservableObject)

`@State` создаёт хранилище для простых значений внутри SwiftUI.
`@StateObject` создаёт и владеет экземпляром класса, сохраняя его между перерисовками.

```swift
@State private var count = 0 // Value type
@StateObject private var viewModel = ViewModel() // Reference type
```
</details>

<details>
<summary><strong>Вопрос 2:</strong> Почему этот код работает неправильно?</summary>
...
**Ответ:**
При каждой перерисовке `ParentView` создаётся **новый экземпляр** `MyViewModel()`, теряя предыдущее состояние.
...
<details>
<summary><strong>Вопрос 3:</strong> Какой порядок модификаторов даст синий фон вокруг padding?</summary>

a) `.padding().background(.blue)`
b) `.background(.blue).padding()`

**Ответ:** **a)** `.padding().background(.blue)`

Модификаторы применяются снизу вверх. Сначала добавляется padding, затем фон покрывает всё (включая padding).
</details>

<details>
<summary><strong>Вопрос 4:</strong> В чём разница между implicit и explicit анимациями?</summary>
...
Circle()
    .scaleEffect(scale)
    .animation(.spring(), value: scale) // Анимирует только scale
```
Анимирует **конкретное свойство**.
...
```
Анимирует **все изменения состояния** в блоке.
</details>

<details>
<summary><strong>Вопрос 5:</strong> Когда использовать LazyVStack вместо VStack?</summary>

**Ответ:**
- **VStack**: когда элементов мало (< 20) и все помещаются на экране
- **LazyVStack**: когда элементов много и нужна прокрутка

`LazyVStack` создаёт views **on-demand** (только видимые), экономя память.
...
<details>
<summary><strong>Вопрос 6:</strong> Что произойдёт при вызове этого кода?</summary>
...
struct ChildView: View {
    ...
    var body: some View { Text(store.data) }
}
```

**Ответ:** Всё работает корректно! `@EnvironmentObject` получит `MyStore` из environment.

**Краш был бы, если:**
```swift
struct ContentView: View {
    var body: some View {
        ChildView() // ❌ Нет .environmentObject()
    }
}
```
</details>
...
<details>
<summary><strong>Вопрос 7:</strong> Почему @ViewBuilder позволяет писать if без return?</summary>

**Ответ:**
`@ViewBuilder` — это **result builder**, который трансформирует код компилятором.
...
var content: some View {
    ViewBuilder.buildBlock(
        ViewBuilder.buildIf(condition ? Text("A") : nil),
        Text("B")
    )
}
```

Result builder позволяет использовать declarative синтаксис (if/else/for) внутри view.
</details>
