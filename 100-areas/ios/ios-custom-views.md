...
### Вопрос 1: Инициализаторы UIView

**Вопрос:** Почему UIView требует реализации двух инициализаторов - `init(frame:)` и `init(coder:)`? Можно ли обойтись одним?

<details>
<summary>Ответ</summary>

UIView имеет два **designated initializer'а**:

1. **`init(frame:)`** - используется при программном создании:
   ```swift
   let view = CustomView(frame: .zero)
   ```

2. **`init(coder:)`** - используется при загрузке из Storyboard/XIB:
   ```swift
   @IBOutlet weak var customView: CustomView! // Вызовется init(coder:)
   ```

**Обойтись одним НЕЛЬЗЯ**, потому что:

- Swift требует реализации всех designated initializers родительского класса
- `init(coder:)` помечен как `required` в UIView - вы обязаны его реализовать
- Если не реализуете - код из Storyboard не будет работать

**Решение - commonInit pattern:**
```swift
override init(frame: CGRect) {
    super.init(frame: frame)
    commonInit()
...
### Вопрос 2: setNeedsLayout vs layoutIfNeeded

**Вопрос:** В чем разница между `setNeedsLayout()` и `layoutIfNeeded()`? Когда использовать каждый из них?

<details>
<summary>Ответ</summary>

**setNeedsLayout():**
- Помечает view как "нуждающийся в layout"
- Layout произойдет в **следующем** update cycle (асинхронно)
- Множественные вызовы батчатся в один `layoutSubviews()`
- Более производительно для множества изменений

```swift
// Три вызова → один layoutSubviews() позже
view.setNeedsLayout()
anotherView.setNeedsLayout()
thirdView.setNeedsLayout()
```

**layoutIfNeeded():**
- Выполняет layout **немедленно** (синхронно)
- Если view не нуждается в layout - ничего не делает
- Необходим для анимаций constraints

```swift
// Анимация изменения constraint
heightConstraint.constant = 200
UIView.animate(withDuration: 0.3) {
    self.superview?.layoutIfNeeded() // Layout ВНУТРИ animation block
}
```
...
| Ситуация | Метод |
|----------|-------|
| Обычные изменения UI | `setNeedsLayout()` |
| Анимация constraints | `layoutIfNeeded()` внутри animate |
| Нужен актуальный frame сразу | `layoutIfNeeded()` |
| Batch изменений | `setNeedsLayout()` |

</details>

### Вопрос 3: intrinsicContentSize

**Вопрос:** Что такое `intrinsicContentSize` и когда нужно вызывать `invalidateIntrinsicContentSize()`?

<details>
<summary>Ответ</summary>

**intrinsicContentSize** - это "естественный" размер view, который он хочет иметь на основе своего содержимого.

Примеры:
- `UILabel`: размер текста
- `UIImageView`: размер изображения (если задано)
- `UIButton`: размер title + padding
...
**invalidateIntrinsicContentSize()** - сообщает Auto Layout, что intrinsicContentSize изменился и нужно пересчитать layout.

**ОБЯЗАТЕЛЬНО вызывать когда:**
- Изменилось содержимое (текст, изображение)
- Изменились padding/margins
- Любое изменение, влияющее на "естественный" размер

```swift
var text: String = "" {
    didSet {
        label.text = text
        invalidateIntrinsicContentSize() // ВАЖНО!
    }
}
```

**Если забыть:**
- Auto Layout не узнает об изменении
- View останется старого размера
- UI будет выглядеть сломанным

</details>

### Вопрос 4: draw(_:) vs CAShapeLayer

**Вопрос:** Когда лучше использовать `draw(_:)` для кастомной графики, а когда `CAShapeLayer`?

<details>
<summary>Ответ</summary>

**draw(_:) - CPU рендеринг:**

Плюсы:
- Полный контроль над каждым пикселем
- Простой API (UIBezierPath)
- Не создает дополнительных CALayer

Минусы:
- Работает на CPU (медленнее)
- Каждый setNeedsDisplay() = полная перерисовка
- Анимация = много перерисовок

**Когда использовать draw(_:):**
- Статическая сложная графика
- Pixel-perfect рендеринг
- Когда не нужна анимация path

---

**CAShapeLayer - GPU рендеринг:**

Плюсы:
- Работает на GPU (быстрее)
- Встроенная анимация (strokeEnd, path)
- Кэширование на GPU

Минусы:
- Дополнительный layer в иерархии
- Менее гибкий API для сложной графики

**Когда использовать CAShapeLayer:**
- Анимация path (loading indicators, progress)
- Простые формы с анимацией
- Performance-критичные сценарии

```swift
// CAShapeLayer с анимацией
...
**Правило:** Если нужна анимация → CAShapeLayer. Если статика → draw(_:).
...
- [[ios-uikit-fundamentals]] - UIView hierarchy, Auto Layout, coordinate systems
- [[ios-graphics-fundamentals]] - Core Graphics, Core Animation, CALayer

### Related (связанные темы)

- [[android-custom-view-fundamentals]] - кастомные View на Android (onDraw, onMeasure, onLayout)
- [[ios-swiftui]] - декларативный подход к построению UI
- [[ios-view-rendering]] - render loop, off-screen rendering
- [[ios-scroll-performance]] - оптимизация scrolling с custom cells

### Next steps (куда двигаться дальше)

- [[ios-animations]] - Core Animation, UIView animations
- [[ios-gesture-recognizers]] - обработка жестов в custom views
- [[ios-accessibility-custom-views]] - VoiceOver для custom компонентов
