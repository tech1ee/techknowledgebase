...
<details>
<summary>1. Какой тип EC2 instance выбрать для web server?</summary>

**Ответ:** T-family (t3, t4g) для типичных web серверов:
- Burstable performance (накапливает credits)
- Хорошее соотношение цена/производительность
- t4g (Graviton ARM) — ещё дешевле

Для high-traffic: M-family (m6i) — consistent performance.

</details>

<details>
<summary>2. Чем Multi-AZ отличается от Read Replicas?</summary>

**Ответ:**
- **Multi-AZ:** Для High Availability. Standby синхронно реплицируется, автоматический failover, standby не для чтения.
- **Read Replicas:** Для scaling reads. Async репликация, можно читать, до 15 реплик.

Можно использовать оба: Multi-AZ primary + Read Replicas.

</details>

<details>
<summary>3. Когда использовать S3 Glacier?</summary>

**Ответ:** Для данных с редким доступом и долгосрочного хранения:
- Бэкапы старше 30-90 дней
- Compliance архивы (HIPAA, GDPR)
- Логи для аудита

Glacier: retrieval от минут до часов.
Glacier Deep Archive: retrieval 12+ часов, самый дешёвый.

</details>

<details>
<summary>4. Почему IAM Role лучше Access Keys?</summary>

**Ответ:**
- **Access Keys:** Долгоживущие credentials, нужно ротировать, риск утечки
- **IAM Roles:** Временные credentials, автоматическая ротация, нет secrets в коде

EC2, Lambda, ECS используют roles — credentials инжектятся автоматически.

</details>

<details>
<summary>5. Какие ограничения у Lambda?</summary>

**Ответ:**
- **Timeout:** 15 минут max
- **Memory:** 128MB - 10GB
- **Package:** 50MB zip / 250MB unzipped
- **Concurrency:** 1000 default (можно увеличить)
- **Payload:** 6MB sync / 256KB async

Для долгих задач: Step Functions или ECS/Fargate.
