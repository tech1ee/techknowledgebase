...
**Q1:** В чём разница между explicit и implicit Intent?

<details>
<summary>Ответ</summary>

**Explicit Intent** указывает конкретный компонент (class name): `Intent(context, MyActivity::class.java)`. Используется для внутренней навигации в приложении.

**Implicit Intent** описывает действие (action) и/или данные (data) без указания компонента: `Intent(Intent.ACTION_VIEW, Uri.parse("https://..."))`. Система находит подходящий компонент через IntentFilter matching. Используется для взаимодействия с другими приложениями.

</details>

**Q2:** Зачем нужна категория DEFAULT в IntentFilter?

<details>
<summary>Ответ</summary>

Когда Activity запускается через `startActivity()` с implicit Intent, система автоматически добавляет категорию `CATEGORY_DEFAULT`. Если IntentFilter не содержит `<category android:name="android.intent.category.DEFAULT"/>`, Activity не будет найдена для implicit Intent (за исключением LAUNCHER и некоторых системных категорий).

</details>

### Вопросы уровня "Продвинутое"

**Q3:** Объясните алгоритм Intent resolution для implicit Intent.

<details>
<summary>Ответ</summary>

Intent resolution проходит три теста для каждого IntentFilter:

1. **Action test**: Intent action должен совпадать с одним из action в filter. Если в filter нет actions, проходят все. Если в Intent нет action, проходит.

2. **Category test**: ВСЕ категории из Intent должны присутствовать в filter. Filter может иметь дополнительные категории. `startActivity()` автоматически добавляет `CATEGORY_DEFAULT`.

3. **Data test**: Сложный matching по scheme, host, port, path, mimeType. Если filter указывает только mimeType, Intent должен иметь тот же тип. Если указаны URI parts, они матчатся иерархически.

Компонент проходит resolution если все три теста пройдены хотя бы для одного IntentFilter.

</details>

**Q4:** В чём разница между Deep Link и App Link?

<details>
<summary>Ответ</summary>

**Deep Link** — любой URI scheme, который открывает приложение. Может использовать custom scheme (`myapp://`) или http/https. Не верифицирован — несколько приложений могут зарегистрировать один и тот же link, появится disambiguation dialog.

**App Link** (Android 6.0+) — верифицированный http/https deep link. Требует:
1. `android:autoVerify="true"` в IntentFilter
2. Файл `assetlinks.json` на сервере по пути `/.well-known/assetlinks.json`
3. Файл содержит SHA-256 fingerprint signing certificate приложения

После верификации система открывает ваше приложение напрямую без dialog.

</details>

### Вопросы уровня "Экспертное"

**Q5:** Как работает PendingIntent identity? Когда два PendingIntent считаются одинаковыми?

<details>
<summary>Ответ</summary>

Два PendingIntent считаются идентичными если:
1. Одинаковый тип операции (getActivity, getBroadcast, getService)
2. Одинаковый request code
3. Intent-ы "equivalent" по Intent.filterEquals():
   - Одинаковый action
   - Одинаковый data (URI)
   - Одинаковый type (mimeType)
   - Одинаковый package
   - Одинаковый component
   - Одинаковые categories

**Extras НЕ участвуют в сравнении!** Это важно для notifications: если создать два PendingIntent с разными extras но одинаковым request code — второй заменит первый (или обновит его extras при FLAG_UPDATE_CURRENT).

</details>

**Q6:** Объясните Intent Redirection vulnerability и как защититься.

<details>
<summary>Ответ</summary>

**Intent Redirection** — уязвимость когда приложение использует данные из недоверенного Intent для создания и запуска нового Intent.

**Атака:**
1. Атакующий отправляет Intent с `target_class="SensitiveInternalActivity"`
2. Уязвимое приложение создаёт `Intent().setClassName(pkg, target_class)`
3. Запускает internal Activity, которая не должна быть доступна извне

**Защита:**
1. **Whitelist** разрешённых targets — проверять target по списку
2. **Не передавать Intent напрямую** — не использовать `startActivity(getIntent().getParcelableExtra("intent"))`
3. **Validation** — проверять все данные из Intent
4. **Principle of least privilege** — минимизировать exported компоненты
