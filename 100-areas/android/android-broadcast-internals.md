...
### Вопрос 1
**Q:** Почему этот receiver не получает broadcast на Android 14?
```kotlin
...
На Android 14 (targetSdk 34) **обязательно** указать `RECEIVER_EXPORTED` или `RECEIVER_NOT_EXPORTED` для всех не-protected broadcasts. Без флага — `SecurityException`. Исправление:
```kotlin
ContextCompat.registerReceiver(context, receiver,
    IntentFilter("MY_ACTION"),
    ContextCompat.RECEIVER_NOT_EXPORTED)
```
Для protected broadcasts (ACTION_BATTERY_CHANGED, ACTION_SCREEN_ON и др.) флаг не требуется.
</details>

### Вопрос 2
**Q:** Чем отличается goAsync() от запуска корутины без goAsync()?

<details>
<summary>Ответ</summary>

**Без goAsync():** система считает receiver завершённым после return из onReceive(). BroadcastQueue вызывает finishReceiver() автоматически. Процесс может быть убит LMK (receiver больше не "живёт"), корутина может не доработать.

**С goAsync():** mPendingResult отсоединяется от receiver. Система ждёт вызова PendingResult.finish(). Процесс получает повышенный priority пока receiver "активен". Корутина получает шанс завершиться.

**Но:** ANR timeout (~10s fg / ~60s bg) всё ещё действует. goAsync() НЕ снимает timeout — только позволяет уйти с main thread.

Для задач > 10 секунд: запускайте WorkManager или Service из onReceive(), а не используйте goAsync().
</details>

### Вопрос 3
**Q:** Почему manifest receiver для CONNECTIVITY_CHANGE не работает на Android 8+, но BOOT_COMPLETED работает?

<details>
<summary>Ответ</summary>

Android 8.0 (API 26) ввёл **implicit broadcast restrictions**: manifest-registered receivers **не могут** получать большинство implicit broadcasts. Это решает "thundering herd" проблему — 50+ приложений пробуждались при каждом изменении сети.

`CONNECTIVITY_CHANGE` — **НЕ** в списке исключений. При каждом изменении сети пробуждение 50 процессов неприемлемо. Замена: `ConnectivityManager.NetworkCallback` или dynamic receiver.

`BOOT_COMPLETED` — **В** списке исключений (manifest-exempt). Причина: однократное событие (1 раз при загрузке), критично для инициализации (alarm rescheduling, WorkManager initialization). Пробуждение N процессов один раз допустимо.

Полный список исключений: [broadcast-exceptions](https://developer.android.com/develop/background-work/background-tasks/broadcasts/broadcast-exceptions)
</details>

### Вопрос 4
**Q:** Чем BroadcastQueueModernImpl (Android 14) отличается от старой BroadcastQueue?

<details>
<summary>Ответ</summary>

Старая `BroadcastQueue`:
- Две отдельные очереди (foreground/background)
- Линейная обработка ordered broadcasts
- Простой timeout (10s/60s фиксированный)
- Нет deferral для cached processes

Новая `BroadcastQueueModernImpl`:
- Единая очередь с per-process scheduling
- Параллельная доставка в разные процессы (даже для ordered broadcasts, где возможно)
- Адаптивные timeouts с учётом загруженности системы
- Broadcast deferral для cached processes (API 35+): broadcast задерживается пока app в cached state
- Batching: несколько broadcast для одного процесса группируются в один Binder call
- Urgent queue для критичных системных broadcasts
</details>

### Вопрос 5
**Q:** Почему нельзя передавать Bitmap через broadcast extras?

<details>
<summary>Ответ</summary>

1. **Binder transaction buffer limit**: 1 MB на процесс (shared между всеми IPC). Bitmap 1080x1920 ARGB_8888 = ~8 MB → `TransactionTooLargeException`.

2. **Parcel serialization overhead**: Bitmap сериализуется в Parcel (byte array), передаётся через Binder, десериализуется. Двойное копирование + IPC latency.

3. **Multiple receivers**: При N receivers bitmap копируется N раз (в Parcel для каждого Binder call).

Правильно: передать URI (content:// или File URI через FileProvider) или ID для загрузки из кэша/базы.

```kotlin
// ✅ Передать URI вместо Bitmap
context.sendBroadcast(Intent("com.example.IMAGE_READY").apply {
    putExtra("image_uri", imageUri.toString()) // URI, не Bitmap!
})
```
</details>
