...
<details>
<summary>1. Что такое recomposition и когда она происходит?</summary>

**Ответ:** Recomposition — процесс перевызова composable функций при изменении state. Происходит когда:
- Изменился State, который читается в composable
- Изменились параметры composable функции

Compose умный — перекомпозиция ТОЛЬКО затронутых частей UI, не всего дерева. Для этого используется Stable annotation и структурное сравнение параметров.

</details>

<details>
<summary>2. Чем remember отличается от rememberSaveable?</summary>

**Ответ:**
- `remember{}` — сохраняет значение между recompositions. НЕ переживает configuration change (поворот экрана).
- `rememberSaveable{}` — сохраняет в savedInstanceState Bundle. Переживает configuration change и process death.

Используй rememberSaveable для UI state, который должен восстанавливаться (scroll position, текст в поле ввода).

</details>

<details>
<summary>3. Почему composable функции должны быть idempotent?</summary>

**Ответ:** Compose может вызвать composable функцию любое количество раз и в любом порядке. Если функция имеет side effects (сетевой запрос, запись в БД), эти effects будут выполняться непредсказуемо. Side effects — только через LaunchedEffect, SideEffect, DisposableEffect.

</details>

<details>
<summary>4. Что такое State Hoisting и зачем он нужен?</summary>

**Ответ:** State Hoisting — паттерн подъёма state из composable наружу. Composable принимает state как параметр и callback для его изменения. Это делает composable:
- **Stateless** — проще тестировать
- **Reusable** — не привязан к конкретному источнику state
- **Controlled** — родитель управляет состоянием

Пример: TextField принимает `value` и `onValueChange` вместо внутреннего state.
