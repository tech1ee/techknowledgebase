...
### Вопрос 1: View vs Property Animation
**Q:** Почему View Animation (TranslateAnimation) — плохой выбор для анимации кнопки? Что произойдёт при нажатии?

**A:** View Animation изменяет только визуальное отображение (draw matrix), не меняя реальных свойств View. translationX/Y остаются = 0. Click target (hitRect) остаётся на исходном месте. Пользователь нажмёт на "новое" положение кнопки — клик не сработает. Нажмёт на пустое место "старого" положения — сработает. Property Animation (ObjectAnimator) изменяет РЕАЛЬНЫЕ свойства, поэтому click target перемещается вместе с View.

---

### Вопрос 2: Choreographer
**Q:** В каком порядке Choreographer обрабатывает callbacks? Почему этот порядок важен для анимаций?

**A:** INPUT → ANIMATION → INSETS_ANIMATION → TRAVERSAL → COMMIT. Порядок критичен: сначала обрабатывается input (touch события), затем animation callbacks обновляют значения свойств, и только потом TRAVERSAL выполняет measure/layout/draw с уже обновлёнными значениями. Если бы traversal был раньше animation — рисовался бы кадр с устаревшими значениями.

---

### Вопрос 3: Compose recomposition в анимациях
**Q:** Почему `Modifier.graphicsLayer { alpha = animatedValue }` лучше чем `Modifier.alpha(animatedValue)` для анимаций?

**A:** `Modifier.alpha(value)` — reading state в Composition phase. Каждое изменение value вызывает recomposition composable → пересоздание modifier chain → потенциально лишняя работа. `Modifier.graphicsLayer { alpha = value }` — reading state в Draw phase. Изменение value обновляет только draw, без recomposition. Для анимации с 60 updates/sec это разница между 60 recompositions и 0 recompositions в секунду.

---

### Вопрос 4: Spring vs Tween
**Q:** Когда spring-анимация предпочтительнее tween? Приведите конкретный сценарий.

**A:** Spring предпочтителен когда анимация начинается с ненулевой скорости (gesture-driven). Пример: пользователь перетаскивает карточку и отпускает. С tween: анимация начнётся с 0 скорости → визуальный рывок (скорость пальца ≠ 0, а анимации = 0). С spring: `setStartVelocity(gestureVelocity)` → плавный переход от gesture к анимации. Также spring не имеет фиксированной длительности — для малых расстояний анимация короче, для больших — длиннее, что выглядит физически натурально.

---

### Вопрос 5: Hardware Layer
**Q:** Когда и как использовать hardware layer для анимаций? Какие есть ограничения?

**A:**
**Когда использовать:**
- Анимация translationX/Y, rotation, scaleX/Y, alpha
- View сложный (много children, тени, rounded corners)
- Продолжительность анимации > 200ms
...
animator.start()
animator.doOnEnd {
    view.setLayerType(View.LAYER_TYPE_NONE, null)  // Освободить GPU память!
}
```

**Ограничения:**
- Занимает GPU память (bitmap размера View)
- Не подходит для анимации содержимого View (текст, цвет)
- Если забыть убрать layer — память не освободится
- На некоторых устройствах hardware layer медленнее software

---

### Вопрос 6: Shared Element Problems
**Q:** Какие типичные проблемы возникают с Shared Element transitions и как их решать?

**A:**
1. **Изображение не загружено до начала transition:**
   - Решение: `postponeEnterTransition()` + `startPostponedEnterTransition()` после загрузки

2. **transitionName не уникален:**
   - Решение: Добавить ID к transitionName: `"image_${item.id}"`

3. **RecyclerView items переиспользуются:**
   - Решение: Устанавливать transitionName в onBindViewHolder

4. **Return transition не работает:**
   - Решение: Проверить что targetView существует в обоих направлениях

5. **Различные aspect ratio:**
   - Решение: ChangeImageTransform в TransitionSet + centerCrop/fitCenter

---

### Вопрос 7: Animation Cancellation
**Q:** Что происходит если не отменить анимацию при уничтожении View/Activity?

**A:**
1. **Memory Leak:** AnimatorUpdateListener держит ссылку на View → View держит Activity Context → Activity не собирается GC

2. **Crash:** Анимация пытается обновить destroyed View → `IllegalStateException` или `NullPointerException`

3. **Wasted resources:** Animator продолжает работать в background, потребляя CPU
...
// Compose: автоматически — анимации отменяются при disposal
```

---

### Вопрос 8: Compose Animation Performance
**Q:** Как измерить и оптимизировать производительность Compose анимаций?

**A:**
**Измерение:**
1. Layout Inspector → Component Tree → Recomposition counts
2. Compose compiler metrics: `composeCompiler { metricsDestination = ... }`
3. Perfetto trace с `trace("AnimationName") { }`

**Оптимизация:**
1. **graphicsLayer{}** вместо прямых Modifier (alpha, offset, scale)
2. **derivedStateOf** для вычисляемых значений
3. **remember** для AnimationSpec чтобы не пересоздавать
4. **key()** в LazyColumn для stable identity
5. Минимизировать объекты в lambda capture

---

### Вопрос 9: MotionLayout vs Compose
**Q:** Когда выбрать MotionLayout, а когда Compose animations?

**A:**
**MotionLayout выбрать когда:**
- Сложная координированная анимация многих элементов
- Анимация между разными ConstraintSets
- Нужен визуальный редактор (Motion Editor в AS)
- Команда знакома с ConstraintLayout
- Анимация зависит от scroll/swipe прогресса

**Compose animations выбрать когда:**
- Простые state-driven анимации
- Анимация одного элемента
- Нужен полный контроль через код
- Проект уже на Compose
- Shared Element transitions (Navigation 2.8+)

**Можно комбинировать:** MotionLayout внутри ComposeView для сложных transitions.

---

### Вопрос 10: 60fps Budget
**Q:** Какой бюджет времени на кадр для 60fps? Что включает этот бюджет?

**A:**
**Бюджет: 16.67ms** (1000ms / 60fps)

**Что включает:**
1. **Input handling:** ~2ms (touch events)
2. **Animation callbacks:** ~1-2ms (ValueAnimator updates)
3. **Layout:** ~2-5ms (measure + layout для animated Views)
4. **Draw:** ~5-10ms (Canvas operations, RenderNode)
5. **Sync to GPU:** ~2ms (buffer swap)

**Итого:** ~12-19ms — близко к границе!

**При 120Hz:** бюджет всего **8.3ms** — оптимизация критична.

**Jank происходит когда:** общее время > бюджет → кадр пропущен → visually заметно.
