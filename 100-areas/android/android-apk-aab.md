...
1. **Что такое APK и какова его структура?**
   - ZIP архив с конкретной структурой
   - classes.dex (код), resources.arsc (ресурсы), AndroidManifest.xml (binary), res/, assets/, lib/, META-INF/

2. **Чем AAB отличается от APK?**
   - AAB — публикационный формат, APK — установочный
   - AAB конвертируется в оптимизированные Split APKs для каждого устройства
   - AAB использует text XML и protobuf вместо binary formats

3. **Какие существуют signing schemes и их отличия?**
   - v1 (JAR): подпись файлов, медленная верификация
   - v2 (Full APK): весь APK как blob, быстрая верификация
   - v3: key rotation support
   - v4: incremental install, Merkle tree

4. **Что такое Split APKs?**
   - Отдельные APK для каждой конфигурации (ABI, density, language)
   - Устанавливаются вместе как один app
   - Уменьшают download size

5. **Зачем нужен zipalign?**
   - Выравнивание uncompressed данных на 4-byte boundary
   - Позволяет эффективное использование mmap()
   - Меньше RAM, быстрее запуск

6. **Что такое Play Asset Delivery?**
   - Система доставки больших assets (игры)
   - Три режима: install-time, fast-follow, on-demand
   - Лимит: 2 GB для asset packs

7. **Зачем нужен APK Analyzer?**
   - Анализ размера компонентов
   - DEX viewer для методов и классов
...
- **[[android-proguard-r8]]** — R8 shrinking влияет на размер DEX
- **[[android-compilation-pipeline]]** — pipeline генерирует DEX и APK
- **[[android-resources-system]]** — ресурсы компилируются в APK
- **[[android-gradle-fundamentals]]** — конфигурация сборки APK/AAB
