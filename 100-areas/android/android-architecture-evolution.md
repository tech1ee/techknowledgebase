...
1. **Почему MVP устарел с появлением Architecture Components?**
   - Presenter держит reference на View → lifecycle issues
   - ViewModel автоматически управляет lifecycle
   - LiveData/StateFlow заменили callbacks

2. **В чём главное преимущество Unidirectional Data Flow?**
   - Предсказуемость: всегда ясно откуда пришло изменение
   - Отладка: можно логировать каждый Intent
   - Нет race conditions: одна точка обновления state

3. **Когда MVI избыточен?**
   - Простые CRUD экраны
   - Прототипы
   - Маленькие приложения (< 5 экранов)

4. **Почему Compose хорошо сочетается с MVI?**
   - Оба основаны на immutable state
   - UDF встроен в Compose (state down, events up)
   - Stateless composables = чистые функции

5. **Какая главная проблема God Activity?**
   - Configuration change уничтожает всё состояние
