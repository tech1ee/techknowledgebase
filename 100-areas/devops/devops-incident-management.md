...
<details>
<summary>1. Почему "blameless" важен для post-mortems?</summary>

**Ответ:**

**Если искать виноватых:**
- Люди скрывают ошибки
- Меньше прозрачности
- Не учимся на проблемах
- Культура страха

**Blameless подход:**
- Фокус на системных проблемах, не людях
- Все свободно делятся информацией
- Ошибки = возможность улучшить систему
- Psychological safety → лучшая инженерия

**Правило:** "The person who made the mistake is often the best person to fix the system that allowed it."

</details>

<details>
<summary>2. Как структурировать on-call rotation?</summary>

**Ответ:**

**Базовые правила:**
1. **Длительность:** 1 неделя (не больше)
2. **Overlap:** Handoff meeting при смене
3. **Backup:** Всегда есть secondary on-call
4. **Rest:** После тяжёлого дежурства — day off

**Структура:**
```
Primary → Secondary → Manager (escalation)
```

**Healthy on-call:**
- < 2 pages per shift в среднем
- Compensate time or pay
- Avoid hero culture (один человек всё знает)
- Regular rotation through all team members

**Red flags:**
- > 5 pages/shift → alert fatigue
- Same person always on-call → bus factor
- No escalation path → burnout

</details>

<details>
<summary>3. Когда восстанавливать vs когда искать root cause?</summary>

**Ответ:**

**Правило:** Сначала MITIGATION, потом root cause.

**Приоритет во время инцидента:**
1. **Restore service** (rollback, scale, failover)
2. **Stabilize** (monitor, prevent cascade)
3. **Communicate** (users, stakeholders)
4. Root cause — после восстановления!

**Почему:**
- Пользователи не ждут, пока ты дебажишь
- Длительный outage = больше impact
- Root cause analysis требует времени и ясной головы

**Исключение:** Если mitigation невозможен без понимания причины (e.g., security breach — нужно знать что закрывать).

</details>

<details>
<summary>4. Что должно быть в хорошем runbook?</summary>

**Ответ:**

**Обязательные секции:**

1. **Overview** — что это за проблема
2. **Severity** — насколько срочно
3. **Detection** — как обнаружить (alerts, symptoms)
4. **Impact** — что ломается для пользователей
5. **Quick Diagnosis** — команды для проверки
6. **Resolution Steps** — пошаговая инструкция
7. **Rollback** — как откатить если хуже
8. **Escalation** — кого звать если не получается

**Качества хорошего runbook:**
- Можно выполнить в 3 AM полусонным
- Конкретные команды, не "проверьте логи"
- Актуальный (review каждые 3 месяца)
- Тестируется на game days
