...
<details>
<summary>1. Какие метрики БД критично мониторить?</summary>
...
- **Replication:** Lag в секундах, replica status
- **Resources:** Disk space, CPU, memory, I/O
- **Locks:** Deadlocks, long-running transactions, lock waits

Минимум: pg_stat_statements + disk space + replication lag.

</details>

<details>
<summary>2. Что такое pg_stat_statements и зачем он нужен?</summary>

**Ответ:** Extension для сбора статистики по запросам:
- Количество вызовов каждого запроса
- Общее и среднее время выполнения
- Количество прочитанных/записанных блоков
- Cache hit ratio

Позволяет найти:
- Самые медленные запросы
- Самые частые запросы
- Запросы с плохим cache hit

</details>

<details>
<summary>3. Как работает Row-Level Security (RLS)?</summary>

**Ответ:** RLS — механизм фильтрации строк на уровне БД.

1. Включаем RLS: `ALTER TABLE orders ENABLE ROW LEVEL SECURITY`
2. Создаём policy с условием: `USING (tenant_id = current_setting('app.tenant'))`
3. Устанавливаем контекст: `SET app.tenant = 'acme'`
4. Все запросы автоматически фильтруются

Преимущества: нельзя забыть фильтр, защита от SQL injection, централизованный контроль.

</details>

<details>
<summary>4. Какие уровни encryption существуют для PostgreSQL?</summary>

**Ответ:**
1. **In transit (SSL/TLS):** Шифрование соединения между клиентом и сервером
2. **At rest (filesystem):** LUKS, EBS encryption — прозрачно для PostgreSQL
3. **Column-level (pgcrypto):** Шифрование отдельных полей, ключи в приложении
4. **TDE:** Transparent Data Encryption — enterprise решение

Рекомендация: SSL обязателен + filesystem encryption для at rest.

</details>

<details>
<summary>5. Что такое принцип Least Privilege для БД?</summary>

**Ответ:** Каждый пользователь должен иметь минимально необходимые права.

Практика:
- **app_user:** SELECT, INSERT, UPDATE, DELETE — для приложения
- **report_user:** только SELECT — для отчётов
- **migration_user:** DDL права — для миграций
- **backup_user:** pg_read_all_data — для бэкапов

Никогда: `GRANT ALL PRIVILEGES TO app_user`
