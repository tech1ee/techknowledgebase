...
*Последнее обновление: 2026-01-09 — Добавлены педагогические секции: 5 аналогий (K8s cluster как корпоративный офис, Service как виртуальный ресепшен, типы Services как способы связи с внешним миром, CNI как подрядчик по коммуникациям, Network Policy как пропускная система), 6 типичных ошибок с СИМПТОМ/РЕШЕНИЕ (NetworkPolicy без DNS egress, Flannel + NetworkPolicy = не работает, Service без endpoints, хардкодинг Pod IP, kube-proxy iptables при 1000+ services, Ingress без Ingress Controller), 5 ментальных моделей (четыре уровня коммуникации, Service = iptables/IPVS правила, DNS иерархия, дерево выбора способа доступа, troubleshooting checklist)*
