...
> [!question]- Почему репликация не заменяет бэкапы?
> Репликация копирует ВСЕ изменения, включая ошибочные: DROP TABLE на мастере мгновенно удалит данные на реплике. Бэкап — это snapshot на определённый момент, позволяющий откатиться к состоянию до ошибки. PITR (Point-in-Time Recovery) через WAL-архивы позволяет восстановить БД на любой момент в прошлом. Репликация для HA, бэкапы для DR.

> [!question]- Чем отличается RTO от RPO и как они определяют стратегию бэкапов?
> RPO (Recovery Point Objective) — максимально допустимая потеря данных (сколько данных можно потерять). RTO (Recovery Time Objective) — максимально допустимое время восстановления. RPO 0 = streaming replication + synchronous commit. RPO 1 час = hourly WAL archiving. RTO 5 мин = hot standby. RTO 1 час = cold restore from backup. Стоимость растёт экспоненциально при снижении RPO/RTO.

> [!question]- Почему правило 3-2-1 критично для стратегии бэкапов?
> 3-2-1: 3 копии данных, на 2 разных типах носителей, 1 копия off-site (в другом физическом месте). Защищает от: аппаратного сбоя (разные носители), стихийного бедствия/пожара (off-site), человеческой ошибки (несколько копий), ransomware (offline копия). Без off-site бэкапа пожар в дата-центре уничтожит все данные.

> [!question]- Когда использовать logical backup (pg_dump) а когда physical (pg_basebackup)?
> Logical backup (pg_dump): портируемый SQL, можно выбрать таблицы, переносить между версиями PostgreSQL, но медленный для больших БД (TB). Physical backup (pg_basebackup): копия файлов БД, быстрее для больших объёмов, поддерживает PITR через WAL, но привязан к конкретной версии и платформе.
...
?
Восстановление БД на любой момент в прошлом. Работает через базовый бэкап + replay WAL-архивов до нужного момента. Позволяет откатить случайный DROP TABLE, восстановив данные на секунду до ошибки.

Что такое WAL archiving?
?
Непрерывное копирование WAL-сегментов (журнала изменений) в безопасное хранилище (S3, NFS). Каждый сегмент 16 MB. В связке с base backup позволяет PITR. Команда: archive_command в postgresql.conf.

Что такое правило 3-2-1 для бэкапов?
?
3 копии данных, 2 разных типа носителей (SSD + S3), 1 копия off-site (другой регион/дата-центр). Защищает от любого одиночного сбоя: железо, пожар, ransomware, человеческая ошибка.

Чем отличается Hot Standby от Cold Standby?
?
Hot Standby: реплика постоянно работает, принимает read-запросы, мгновенный failover (секунды). Cold Standby: сервер выключен, при сбое нужно запустить и восстановить из бэкапа (минуты-часы). Hot дороже, но RTO значительно ниже.

Что делает pgBackRest?
?
Enterprise-инструмент для бэкапов PostgreSQL: full/incremental/differential backup, параллельное сжатие, WAL archiving в S3/GCS, PITR, верификация бэкапов. Замена pg_basebackup для production.
...
|-------------|------|-------|
| Следующий шаг | [[databases-monitoring-security]] | Мониторинг и безопасность БД |
| Углубиться | [[cloud-databases-complete]] | Managed backups в AWS RDS, Aurora, GCP Cloud SQL |
| Смежная тема | [[cloud-disaster-recovery]] | DR-стратегии в облаке, multi-region failover |
| Смежная тема | [[devops-incident-management]] | Incident response при сбое БД |
