...
> [!question]- Почему Read Committed — уровень изоляции по умолчанию в PostgreSQL, а не Serializable?
> Serializable гарантирует полную изоляцию, но с высокой ценой: больше блокировок, serialization failures требующих retry, ниже throughput. Read Committed — оптимальный компромисс для большинства приложений: предотвращает dirty reads, достаточно производителен, и non-repeatable reads редко вызывают проблемы в типичных CRUD-операциях.

> [!question]- Как возникает deadlock и как его предотвратить?
> Deadlock: транзакция A блокирует строку 1 и ждёт строку 2, транзакция B блокирует строку 2 и ждёт строку 1 — обе ждут бесконечно. Предотвращение: всегда блокировать ресурсы в одном и том же порядке (например, по ID), использовать короткие транзакции, применять SELECT FOR UPDATE NOWAIT для быстрого обнаружения. БД автоматически убивает одну из транзакций.

> [!question]- В чём разница между оптимистичной и пессимистичной блокировкой и когда какую применять?
> Пессимистичная (SELECT FOR UPDATE) — блокирует строку при чтении, другие ждут. Оптимистичная (version/updated_at) — не блокирует, но при UPDATE проверяет, не изменилась ли версия. Пессимистичная — для высокого contention (мало строк, много конкурирующих транзакций). Оптимистичная — для низкого contention (много строк, редкие конфликты).

> [!question]- Почему длинные транзакции — антипаттерн в PostgreSQL?
> Длинные транзакции держат блокировки, мешая другим запросам. В PostgreSQL они также блокируют VACUUM: мёртвые строки не удаляются, таблица раздувается (bloat). Реплики могут отставать. Правило: транзакция должна быть максимально короткой — выполнить только необходимые SQL-операции без пауз и внешних вызовов.

---

## Ключевые карточки

Что означает каждая буква ACID?
?
Atomicity — всё или ничего (транзакция целиком или откат). Consistency — данные всегда удовлетворяют constraints. Isolation — параллельные транзакции не видят промежуточных состояний друг друга. Durability — после COMMIT данные сохранены даже при сбое.

Какие аномалии возникают при недостаточной изоляции?
?
Dirty read — чтение незакоммиченных данных. Non-repeatable read — повторное чтение даёт другой результат. Phantom read — появляются новые строки при повторном запросе. Lost update — параллельные UPDATE теряют изменения.

Что такое MVCC (Multi-Version Concurrency Control)?
?
Механизм, при котором каждая транзакция видит свой snapshot данных. Читатели не блокируют писателей, писатели не блокируют читателей. PostgreSQL хранит несколько версий строки с xmin/xmax метками видимости.

Какие уровни изоляции определяет SQL-стандарт?
?
Read Uncommitted (dirty reads), Read Committed (no dirty reads), Repeatable Read (snapshot), Serializable (полная изоляция). Каждый уровень устраняет больше аномалий, но снижает concurrency.

Что такое SELECT FOR UPDATE?
?
Пессимистичная блокировка: SELECT ... FOR UPDATE захватывает exclusive lock на выбранные строки до конца транзакции. Другие транзакции ждут. Используется когда нужно гарантировать, что данные не изменятся между чтением и записью.

Что такое Savepoint в транзакции?
?
Точка сохранения внутри транзакции: SAVEPOINT sp1; ... ROLLBACK TO sp1. Позволяет откатить часть транзакции без отката всей. Полезно для обработки ошибок внутри длинных транзакций.
...
|-------------|------|-------|
| Следующий шаг | [[database-design-optimization]] | Оптимизация производительности запросов |
| Следующий шаг | [[databases-replication-sharding]] | Распределённые транзакции, 2PC, Saga |
| Углубиться | [[database-internals-complete]] | WAL, MVCC, ARIES под капотом |
| Смежная тема | [[architecture-distributed-systems]] | Распределённые транзакции, Saga pattern |
