...
> [!question]- Почему OFFSET-based пагинация деградирует на больших страницах и чем её заменить?
> OFFSET 100000 заставляет БД просканировать и отбросить 100000 строк перед возвратом результата — O(offset + limit). Cursor-based пагинация использует WHERE id > last_seen_id ORDER BY id LIMIT 20 — всегда O(limit), потому что начинает с индекса. Для пользовательских интерфейсов с бесконечным скроллом cursor pagination обязательна.

> [!question]- Как N+1 проблема убивает производительность и как её диагностировать?
> N+1: загрузить список из N элементов (1 запрос) и для каждого загрузить связанные данные (N запросов). Например, 100 заказов + 100 отдельных запросов на клиента = 101 запрос вместо 1 JOIN. Диагностика: pgBadger для анализа логов, APM-инструменты для трассировки. Решение: JOIN, batch loading (WHERE id IN (...)), eager loading в ORM.

> [!question]- Когда денормализация оправдана, а когда она — технический долг?
> Оправдана при read-heavy нагрузке, когда JOIN стоит слишком дорого: предвычисленные агрегаты (total_orders в таблице users), материализованные представления, CQRS read models. Технический долг — когда денормализуют без измерений, просто «для скорости»: данные рассинхронизируются, усложняется обновление, появляются баги.

> [!question]- Что покажет EXPLAIN ANALYZE и на какие метрики обращать внимание?
> EXPLAIN ANALYZE выполняет запрос и показывает реальный план: тип сканирования (Seq Scan vs Index Scan), estimated vs actual rows, actual time для каждого узла, sort method. Красные флаги: Seq Scan на большой таблице, большое расхождение estimated/actual rows (устаревшая статистика), Nested Loop с большим outer count, disk-based sort.
...
?
B-Tree (по умолчанию, сравнения и диапазоны), Hash (только =), GIN (массивы, JSONB, full-text), GiST (геоданные, диапазоны), BRIN (физически упорядоченные данные, даты). Выбор зависит от типа запросов.

Что такое Covering Index?
?
Индекс, содержащий все колонки запроса: CREATE INDEX ON orders(user_id) INCLUDE (total, status). БД отвечает только из индекса, не обращаясь к таблице (Index Only Scan). Значительно ускоряет read-heavy запросы.

Что такое нормальные формы и зачем нужна 3NF?
?
1NF — атомарные значения. 2NF — нет частичных зависимостей от составного ключа. 3NF — нет транзитивных зависимостей (каждый неключевой атрибут зависит только от PK). 3NF устраняет аномалии обновления и дублирование данных.

Как решить проблему N+1 запросов?
?
JOIN — загрузить данные одним запросом. Batch loading: WHERE user_id IN (1,2,3...) вместо N отдельных запросов. В ORM: eager loading (include/prefetch). DataLoader — batch + cache для GraphQL.

Что такое партиционирование таблиц?
?
Разделение большой таблицы на части по ключу: по дате (logs_2024_01), по диапазону, по хэшу. PostgreSQL поддерживает declarative partitioning. Ускоряет запросы (partition pruning) и упрощает обслуживание (удаление старых партиций).

Что такое Materialized View?
?
Предвычисленный результат запроса, сохранённый как таблица. CREATE MATERIALIZED VIEW sales_summary AS SELECT ... Быстрое чтение, но требует REFRESH MATERIALIZED VIEW для обновления. Идеален для тяжёлых отчётов, дашбордов.
...
|-------------|------|-------|
| Следующий шаг | [[databases-monitoring-security]] | Мониторинг производительности, pg_stat_statements |
| Углубиться | [[database-internals-complete]] | B-Tree, Buffer Pool, Query Optimizer под капотом |
| Углубиться | [[databases-replication-sharding]] | Масштабирование когда оптимизация одного сервера исчерпана |
| Смежная тема | [[caching-strategies]] | Redis-кэш для снижения нагрузки на БД |
| Смежная тема | [[api-design]] | Cursor pagination, batch endpoints, field selection |
