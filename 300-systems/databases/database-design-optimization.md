...
## Теоретические основы: формальный базис оптимизации БД

### B-Tree: формальный инвариант (Bayer & McCreight, 1970)

> **B-Tree** порядка `m` — сбалансированное дерево поиска со следующими инвариантами:
> 1. Каждый узел содержит от `⌈m/2⌉ - 1` до `m - 1` ключей (кроме корня)
> 2. Все листья находятся на одном уровне (идеальная балансировка)
> 3. Операции search, insert, delete — `O(log_m N)` по количеству disk I/O

На практике SQL-базы используют **B+ Tree** (вариант, где данные хранятся только в листьях, а листья связаны в linked list для эффективного range scan). PostgreSQL `btree` index, MySQL/InnoDB primary key — всё B+ Tree.

Почему `O(log_m N)` так эффективно: при `m = 200` (типичный page size) и `N = 100M` строк — глубина дерева = `log_200(100M) ≈ 3-4`. То есть **любая строка находится за 3-4 disk I/O**.

### Normal Forms: формальная теория (Codd, 1970-1974)

> **Нормализация** — процесс декомпозиции отношений для устранения **функциональных зависимостей**, нарушающих определённые формальные свойства.

| Нормальная форма | Формальное условие | Устраняет |
|------------------|-------------------|-----------|
| **1NF** | Все атрибуты атомарны (нет повторяющихся групп) | Повторяющиеся группы |
| **2NF** | 1NF + нет частичных зависимостей от составного ключа | Частичные зависимости |
| **3NF** | 2NF + нет транзитивных зависимостей (A→B→C) | Транзитивные зависимости |
| **BCNF** | Каждый детерминант — суперключ | Все функциональные зависимости |

**Денормализация** — сознательное нарушение NF для ускорения read-запросов. Формально: добавление **управляемой избыточности** (redundancy) ради уменьшения числа JOIN. Trade-off: быстрее SELECT, медленнее INSERT/UPDATE, риск data inconsistency.

### Cost-Based Query Optimization

> **Cost-Based Optimizer (CBO)** — компонент СУБД, который перебирает возможные планы выполнения запроса и выбирает план с минимальной **оценочной стоимостью**. Стоимость = f(I/O pages, CPU operations, memory).

Оптимизатор принимает решения на основе **статистик** (histogram'ы, ndistinct, correlation), хранимых в pg_statistic (PostgreSQL) или information_schema (MySQL). ANALYZE / ANALYZE TABLE обновляет эти статистики.

Ключевое ограничение: CBO оценивает **selectivity** (долю строк, удовлетворяющих предикату). При неточных статистиках или коррелированных предикатах оценка может быть ошибочной — отсюда необходимость `EXPLAIN ANALYZE` для верификации.

> **Связь**: B-Tree → [[sql-databases-complete]], Normal Forms → [[databases-fundamentals-complete]], Query Optimization → [[sql-databases-complete]]
...
- Bayer R., McCreight E. (1970). *Organization and Maintenance of Large Ordered Indices.* — Оригинальная работа, определяющая B-Tree — структуру данных, лежащую в основе всех SQL-индексов.
- Codd E.F. (1970). *A Relational Model of Data for Large Shared Data Banks.* — Определение реляционной модели и нормальных форм.
- Selinger P.G. et al. (1979). *Access Path Selection in a Relational Database Management System.* ACM SIGMOD. — Первый cost-based query optimizer (System R), архитектура которого используется в современных СУБД.
