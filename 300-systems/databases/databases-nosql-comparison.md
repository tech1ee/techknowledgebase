...
> [!question]- Как выбрать между MongoDB и DynamoDB для нового проекта?
> MongoDB — гибкая схема, мощные запросы ($lookup, aggregation pipeline), self-hosted или Atlas. DynamoDB — serverless, предсказуемая latency, автомасштабирование, глубокая интеграция с AWS. Если проект на AWS и основной паттерн — key-value с предсказуемыми запросами, DynamoDB. Если нужны сложные запросы, агрегации и облачная независимость — MongoDB.

> [!question]- Почему Polyglot Persistence лучше единой БД для крупных систем?
> Разные данные имеют разные паттерны доступа: заказы требуют ACID (PostgreSQL), сессии — быстрый TTL (Redis), каталог товаров — гибкую схему (MongoDB), поиск — full-text (Elasticsearch). Одна БД не оптимальна для всех паттернов. Polyglot Persistence использует правильный инструмент для каждой задачи, как разная посуда для разных блюд.

> [!question]- Почему нельзя игнорировать CAP-теорему при выборе NoSQL-решения?
> CAP определяет поведение системы при сетевых сбоях (которые неизбежны). Если выбрать CP-систему (MongoDB) — при partition некоторые узлы будут недоступны. Если AP-систему (Cassandra) — ответы могут быть устаревшими. Незнание CAP-характеристик приводит к неожиданному поведению в production при сбоях.

> [!question]- В чём ключевая разница между Redis и Memcached?
> Redis — структуры данных (lists, sets, sorted sets, hashes), persistence (RDB/AOF), pub/sub, Lua-скрипты, кластеризация. Memcached — только строки, нет persistence, проще. Redis заменил Memcached в большинстве сценариев благодаря богатым типам данных и встроенным фичам.

---

## Ключевые карточки

Когда выбирать Document DB (MongoDB)?
?
Для данных с переменной структурой, быстрого прототипирования, content management, каталогов товаров с разными атрибутами. Не подходит для сильно связанных данных с частыми JOIN.

Когда выбирать Wide-Column (Cassandra)?
?
Для write-heavy нагрузки (логи, метрики, IoT), высокой доступности без single point of failure, линейного горизонтального масштабирования. Требует проектирования схемы от запросов (query-driven design).

Когда выбирать Key-Value (Redis)?
?
Для кэширования, сессий, rate limiting, pub/sub, leaderboard (sorted sets). Данные должны помещаться в RAM. Не подходит для сложных запросов и данных больше объёма памяти.

Что такое Polyglot Persistence?
?
Использование разных типов баз данных в одном проекте для разных задач: PostgreSQL для транзакций, Redis для кэша, MongoDB для каталога, Elasticsearch для поиска. Каждая БД оптимальна для своего паттерна.

Чем отличается CP от AP системы (CAP)?
?
CP (Consistency + Partition tolerance): при сбое жертвует доступностью (MongoDB, HBase). AP (Availability + Partition tolerance): при сбое жертвует консистентностью (Cassandra, DynamoDB). CA без Partition — невозможно в реальных сетях.

Что такое DynamoDB и его ключевое преимущество?
?
Managed key-value/document DB от AWS. Serverless, автомасштабирование, single-digit millisecond latency, pay-per-request. Глубокая интеграция с Lambda, S3, streams. Ограничение — query только по partition key + sort key.
...
|-------------|------|-------|
| Следующий шаг | [[nosql-databases-complete]] | Глубокое погружение в каждый тип NoSQL |
| Следующий шаг | [[databases-replication-sharding]] | Масштабирование и распределённые системы |
| Углубиться | [[database-design-optimization]] | Оптимизация запросов в SQL и NoSQL |
| Смежная тема | [[caching-strategies]] | Стратегии кэширования с Redis |
| Смежная тема | [[architecture-distributed-systems]] | CAP, consensus, eventual consistency |
