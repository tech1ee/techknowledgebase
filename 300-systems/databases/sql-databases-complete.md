...
> [!question]- Почему PostgreSQL чаще рекомендуют для новых проектов по сравнению с MySQL?
> PostgreSQL поддерживает полный ACID с MVCC без блокировки читателей, расширяемые типы данных (JSONB, массивы, hstore), мощные оконные функции, полнотекстовый поиск, CTE с рекурсией, partial и expression индексы. MySQL проще в настройке и имеет обширное сообщество, но исторически уступает в стандартоисполнении SQL и расширяемости.

> [!question]- В каком сценарии SQLite лучший выбор, а когда его использовать нельзя?
> SQLite идеален для мобильных приложений, встроенных систем, прототипирования и single-writer приложений — он serverless, zero-config, весь файл БД. Нельзя использовать при параллельной записи от нескольких процессов, высоком write-concurrency, сетевом доступе к БД и когда нужна репликация — для этого нужен PostgreSQL или MySQL.

> [!question]- Почему MVCC в PostgreSQL работает иначе, чем в MySQL/InnoDB, и как это влияет на vacuum?
> PostgreSQL хранит старые версии строк в той же таблице (tuple versioning), поэтому требует VACUUM для очистки мёртвых строк. MySQL/InnoDB хранит старые версии в undo log — отдельном пространстве. Подход PostgreSQL упрощает reads, но создаёт bloat без регулярного vacuum. InnoDB не требует отдельной очистки, но undo log может расти при длинных транзакциях.

> [!question]- Когда использовать JSONB в PostgreSQL вместо отдельных таблиц?
> JSONB подходит для полуструктурированных данных с переменной схемой (настройки пользователя, метаданные), когда структура меняется часто и JOIN по этим данным не нужен. Для данных с фиксированной структурой, частыми JOIN и необходимостью FK — используй обычные таблицы. JSONB поддерживает GIN-индексы для быстрого поиска.

---

## Ключевые карточки

Какой storage engine использует PostgreSQL?
?
PostgreSQL использует единый storage engine на основе heap-таблиц с MVCC (Multi-Version Concurrency Control). Каждая строка хранит xmin/xmax для управления видимостью версий. Индексы — B-Tree по умолчанию.

Чем отличаются InnoDB и MyISAM в MySQL?
?
InnoDB — транзакционный движок с ACID, row-level locking, MVCC, foreign keys. MyISAM — нетранзакционный, table-level locking, быстрее для read-only. InnoDB — стандарт с MySQL 5.5, MyISAM практически не используется.

Что такое WAL (Write-Ahead Logging)?
?
Механизм, при котором изменения сначала записываются в журнал (WAL), а затем применяются к данным. Обеспечивает durability: при сбое можно восстановить данные из WAL. Используется в PostgreSQL и SQLite (WAL mode).

В чём особенность connection pooling (pgBouncer)?
?
pgBouncer держит пул постоянных соединений к PostgreSQL и распределяет их между клиентами. Создание TCP-соединения стоит 20-50 мс, из пула — 0.1-1 мс. Критично для серверов приложений с сотнями процессов.

Что такое Partial Index в PostgreSQL?
?
Индекс, покрывающий только часть строк по условию: CREATE INDEX idx ON orders(status) WHERE status = 'pending'. Экономит место и быстрее обновляется. Полезен когда запросы фильтруют по конкретному подмножеству.

Чем PostgreSQL JSONB отличается от JSON?
?
JSONB хранит данные в бинарном формате — быстрее чтение и поиск, поддержка GIN-индексов, операторы @> и ?. JSON хранит текст as-is — дешевле запись, но без индексов и медленнее поиск.

Какие уровни изоляции поддерживает PostgreSQL?
?
Read Committed (по умолчанию), Repeatable Read, Serializable. PostgreSQL не поддерживает Read Uncommitted — он работает как Read Committed. Serializable использует SSI (Serializable Snapshot Isolation).
...
|-------------|------|-------|
| Следующий шаг | [[database-design-optimization]] | EXPLAIN ANALYZE, индексы, оптимизация запросов |
| Следующий шаг | [[database-internals-complete]] | B-Tree, WAL, MVCC, Buffer Pool под капотом |
| Углубиться | [[nosql-databases-complete]] | Альтернативный подход: MongoDB, Redis, Cassandra |
| Смежная тема | [[caching-strategies]] | Redis как кэш перед PostgreSQL, стратегии инвалидации |
