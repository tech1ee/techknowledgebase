...
> [!question]- Почему Terraform state -- это "единая точка отказа" и как от этого защититься?
> State связывает код с реальными ресурсами. Без state Terraform не знает, что уже создано, и может попытаться создать дубликаты или потерять контроль. Два одновременных apply могут повредить state. Защита: remote state в S3 с encryption, DynamoDB-блокировка (предотвращает параллельный apply), версионирование bucket, регулярные бэкапы. Никогда не хранить state в Git.

> [!question]- Сравните декларативный (Terraform) и императивный (скрипты) подходы к управлению инфраструктурой. Когда каждый уместен?
> Декларативный: описываешь "что хочу" (3 сервера, VPC, база) -- Terraform сам определяет шаги. Императивный: описываешь "как сделать" (создай VPC, потом subnet, потом EC2). Декларативный лучше для стабильной инфраструктуры: идемпотентность, drift detection, plan перед apply. Императивный -- для одноразовых миграций, сложной логики (if/else), или когда нужен точный контроль порядка операций.

> [!question]- Что такое drift и почему это опасно для production-инфраструктуры?
> Drift -- расхождение между кодом (desired state) и реальностью. Происходит когда кто-то изменяет ресурс вручную через консоль. Terraform plan показывает "0 changes", создавая ложное чувство безопасности. При следующем apply Terraform может перезаписать ручные изменения или сломать зависимости. Решения: запретить ручные изменения (политика), регулярный `terraform plan -refresh-only`, drift detection в CI.

> [!question]- Почему Terraform хранит секреты в state и как с этим бороться?
> State содержит ВСЕ атрибуты ресурсов, включая пароли БД, API-ключи, private keys. Это архитектурное решение Terraform -- state должен быть полным. Защита: encrypt state at rest (S3 encryption), ограничить доступ к bucket через IAM, не коммитить state в Git, использовать secret managers (Vault, AWS Secrets Manager) для чувствительных значений вместо tfvars.
...
?
Подход, при котором инфраструктура описывается кодом, хранится в Git и применяется автоматически. Обеспечивает воспроизводимость, версионирование, code review и аудит изменений. 47 кликов в консоли заменяются одной командой terraform apply.

Чем Terraform отличается от Pulumi?
?
Terraform использует декларативный HCL (специальный язык). Pulumi позволяет описывать инфраструктуру на обычных языках (Python, Go, TypeScript). Terraform имеет наибольшую экосистему (1000+ providers). Pulumi удобнее для разработчиков, привыкших к программированию.

Что такое Terraform State?
?
Файл (terraform.tfstate), связывающий код с реальными ресурсами. Содержит ID, атрибуты и зависимости всех управляемых ресурсов. Без state Terraform не знает что уже создано. Хранить в remote backend с блокировкой.

Чем terraform plan отличается от terraform apply?
?
plan показывает что БУДЕТ изменено (предпросмотр), не трогая реальные ресурсы. apply реально создаёт/изменяет/удаляет ресурсы. Всегда запускать plan перед apply. В CI: plan при PR, apply после merge в main.

Что такое Terraform Module?
?
Переиспользуемый набор ресурсов с входными переменными и выходными значениями. Аналог функции в программировании. Модули можно хранить локально или брать из Terraform Registry. Уменьшают дублирование кода.

Как разделять environments в Terraform?
?
Рекомендуемый подход -- директории: environments/dev/, staging/, prod/ с отдельными state-файлами. Полная изоляция, разные права доступа, отдельные pipelines. Workspaces проще, но один state = больший риск.

Что такое OpenTofu?
?
Open source форк Terraform под лицензией MPL 2.0 (Linux Foundation, CNCF Sandbox). 100% совместим с HCL и существующими Terraform-конфигурациями. Создан после смены лицензии Terraform на BSL.
...
|-------------|------|-------|
| Следующий шаг | [[observability]] | Мониторинг инфраструктуры, созданной через IaC |
| Углубиться | [[kubernetes-advanced]] | Terraform + K8s: управление кластерами и ресурсами через код |
| Смежная тема | [[cloud-aws-core-services]] | Ресурсы AWS, которыми управляет Terraform: EC2, S3, VPC, RDS |
