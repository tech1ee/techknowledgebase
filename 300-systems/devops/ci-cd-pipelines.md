...
> [!question]- В чём разница между Continuous Delivery и Continuous Deployment, и почему это важно для выбора стратегии?
> Continuous Delivery доводит код до staging автоматически, но деплой в production требует ручного approve (кнопка). Continuous Deployment полностью автоматизирует цепочку до production. Выбор зависит от зрелости команды: без хороших автотестов, мониторинга и быстрого rollback Continuous Deployment опасен. Большинство команд начинают с Delivery и переходят к Deployment по мере роста уверенности в pipeline.

> [!question]- Почему кэширование зависимостей в CI так критично, и что произойдёт если его не настроить?
> Без кэша каждый запуск pipeline скачивает все зависимости заново (npm install, pip install) -- это может занимать 30-60 секунд вместо 5. При 50+ push'ах в день это часы потерянного времени. Со временем pipeline разрастается: 3 минуты превращаются в 45, разработчики перестают ждать результатов и мержат без CI. Кэширование по hashFiles('package-lock.json') гарантирует пересборку только при изменении зависимостей.

> [!question]- Компания имеет public GitHub-репозиторий. Безопасно ли использовать self-hosted runners? Почему?
> Нет, это серьёзный риск безопасности. Злоумышленник может создать Pull Request с вредоносным кодом, который выполнится на вашем self-hosted runner. Это даёт доступ к вашей инфраструктуре, секретам и сети. Для public repos нужно использовать только GitHub-hosted runners. Self-hosted runners безопасны только для private repos с контролируемым доступом.

> [!question]- Почему "100% тестовое покрытие и зелёный pipeline" не гарантируют качество продукта?
> Покрытие измеряет какой код выполняется тестами, а не что именно проверяется. Тесты могут проверять тривиальные случаи, игнорируя edge cases и интеграционные сценарии. Зелёный pipeline означает лишь что текущие тесты проходят -- если они не покрывают реальные пользовательские сценарии, production всё равно упадёт. Важно качество тестов, а не их количество.
...
?
Автоматический build и запуск тестов при каждом push в репозиторий. Цель -- быстро обнаруживать ошибки интеграции и поддерживать код в рабочем состоянии.

Что такое DORA метрики?
?
Четыре метрики эффективности DevOps: Deployment Frequency (как часто деплоим), Lead Time (время от коммита до production), Change Failure Rate (% деплоев с ошибкой), MTTR (время восстановления). Elite команды деплоят в 973x чаще.

Что такое Quality Gate?
?
Порог качества, который код должен пройти для продолжения pipeline. Примеры: тестовое покрытие > 80%, размер бандла < 500KB, отсутствие high-severity уязвимостей. Брак не пропускается на следующий этап.

Что такое Matrix Build?
?
Параллельный запуск тестов на нескольких комбинациях версий и ОС. Например, тестирование на Node 18, 20, 22 одновременно на Ubuntu и macOS. fail-fast: false позволяет увидеть все проблемы сразу.

Чем отличается Reusable Workflow от обычного?
?
Reusable workflow определяется через `workflow_call` и может быть вызван из других workflow с параметрами (inputs, secrets). Это DRY-принцип для CI/CD: один deploy-workflow, переиспользуемый для staging и production.

Что такое flaky test и почему это проблема для CI?
?
Тест, который то проходит, то падает без изменений в коде. Подрывает доверие к pipeline -- разработчики начинают игнорировать красные сборки. Решение: изолировать и починить или удалить flaky-тесты.

Какие security-проверки должны быть в pipeline?
?
SAST (статический анализ кода, например CodeQL), dependency scanning (проверка уязвимостей в зависимостях), сканирование секретов (Gitleaks). В 2024 году более 10 миллионов секретов найдено в публичных репозиториях.
...
|-------------|------|-------|
| Следующий шаг | [[docker-for-developers]] | Контейнеры -- основа современных pipeline: build, test и deploy в Docker |
| Углубиться | [[gitops-argocd-flux]] | GitOps -- эволюция CD: Git как единый source of truth для деплоя |
| Смежная тема | [[observability]] | Мониторинг pipeline и production -- без него CI/CD слепой |
