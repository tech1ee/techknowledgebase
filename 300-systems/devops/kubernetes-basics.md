...
> **Kubernetes** (K8s) — open-source система оркестрации контейнеров, разработанная Google на основе внутренней системы **Borg** (Burns et al., 2016). Передана в CNCF (2015). Реализует **декларативную модель**: пользователь описывает **desired state**, система автоматически приводит **actual state** к нему через **control loops**.

### Декларативная модель: desired vs actual state

```
Desired State (YAML)          Actual State (Runtime)
┌───────────────────┐         ┌───────────────────┐
│ replicas: 3       │   ──►   │ Running pods: 2   │  ← Controller видит разницу
│ image: app:v2     │  sync   │ image: app:v1     │  ← Запускает reconciliation
└───────────────────┘         └───────────────────┘
```

**Reconciliation loop**: observe → diff → act → observe... (бесконечный цикл)

### Архитектурные принципы K8s

| Принцип | Описание |
|---------|----------|
| **Declarative configuration** | Описываешь ЧТО хочешь, не КАК достичь |
| **Control loops** | Контроллеры непрерывно сверяют desired ↔ actual |
| **Immutable infrastructure** | Не меняй Pod — удали и создай новый |
| **Self-healing** | При сбое — автоматический restart/reschedule |
| **Loose coupling** | Компоненты общаются через API, заменяемы |

### Историческая хронология

| Год | Событие |
|-----|---------|
| 2003 | Google Borg — внутренняя система оркестрации |
| 2013 | Docker — стандартизация контейнеров |
| 2014 | Google открывает Kubernetes (проект Seven) |
| 2015 | Kubernetes 1.0, передача в CNCF |
| 2018 | Kubernetes де-факто стандарт оркестрации |

> **См. также**: [[docker-for-developers]] — контейнеры, [[cloud-overview]] — облачные платформы
...
### Теоретические основы
- Burns B. et al. (2016). *Borg, Omega, and Kubernetes*. — Эволюция систем оркестрации в Google
- Verma A. et al. (2015). *Large-scale Cluster Management at Google with Borg*. — Оригинальная Borg paper
...
> [!question]- Почему livenessProbe и readinessProbe -- это разные механизмы, и что произойдёт, если использовать только один из них?
> livenessProbe проверяет "жив ли контейнер" -- при провале Pod перезапускается. readinessProbe проверяет "готов ли к трафику" -- при провале Pod убирается из Service endpoints, но не рестартуется. Если использовать только liveness: Pod с временной проблемой (БД недоступна) будет бесконечно рестартоваться вместо ожидания. Если только readiness: зависший процесс (deadlock) никогда не будет перезапущен.

> [!question]- Команда из 3 человек запускает стартап. Стоит ли им использовать Kubernetes? Почему?
> Нет, для маленькой команды без DevOps-экспертизы K8s -- overhead. Минимальный production setup требует Ingress Controller, Certificate Manager, мониторинг, RBAC, Network Policies -- это недели настройки и $300-1000+/мес. Лучше использовать Docker Compose на одном сервере, Cloud Run, Heroku или Serverless. K8s оправдан при масштабировании, микросервисах и команде 5+ человек.

> [!question]- Сравните requests и limits для ресурсов Pod'а. Что произойдёт если limits слишком низкие или отсутствуют?
> requests -- гарантированный минимум: scheduler не поставит Pod на ноду без этих ресурсов. limits -- жёсткий максимум. Слишком низкий memory limit -- OOMKilled при пиковой нагрузке. Отсутствие limits -- один Pod может потребить все ресурсы ноды и убить другие Pod'ы. Рекомендация: requests = типичное потребление, limits = пиковое с запасом.

> [!question]- Как Kubernetes обеспечивает rolling update без downtime? Что гарантируют maxSurge и maxUnavailable?
> При обновлении K8s создаёт новый ReplicaSet и постепенно заменяет Pod'ы. maxSurge=1 позволяет создать +1 Pod сверх replicas. maxUnavailable=0 гарантирует, что минимальное число Pod'ов всегда доступно. Процесс: создать новый Pod -> дождаться readinessProbe -> убрать старый -> повторить. Без readinessProbe K8s может отправить трафик на неготовый Pod.

> [!question]- Почему нельзя создавать "naked pods" напрямую?
> Naked pod (Pod без Deployment/ReplicaSet) не будет пересоздан при падении ноды или OOM. Deployment гарантирует desired state: если Pod умер -- создаст новый. Также Deployment обеспечивает rolling updates, rollback, scaling -- всё это недоступно для naked pods.
...
?
Минимальная единица деплоя -- один или несколько контейнеров с общей сетью (IP-адрес) и storage. Pod эфемерен -- может быть убит в любой момент. Обычно один контейнер + опциональные sidecars.

Какую роль выполняет etcd в кластере?
?
Distributed key-value store, который хранит всё состояние кластера: какие Pod'ы существуют, на каких нодах, какие Service'ы настроены. Без etcd кластер не может функционировать. Это единственное stateful-хранилище в Control Plane.

Чем ClusterIP отличается от LoadBalancer?
?
ClusterIP -- Service доступен только внутри кластера по DNS-имени. LoadBalancer -- создаёт облачный балансировщик с публичным IP, доступным извне. Для HTTP-трафика обычно используют Ingress поверх ClusterIP.

Что такое sidecar-контейнер?
?
Вспомогательный контейнер в Pod'е рядом с основным. Примеры: log shipper (отправка логов), proxy (Envoy/Istio для service mesh), secrets sync (подтягивание из Vault). Делит с основным контейнером сеть (localhost) и volumes.

Как Deployment управляет Pod'ами?
?
Deployment создаёт ReplicaSet, который поддерживает заданное количество Pod'ов. Связь через labels и selector. При обновлении создаётся новый ReplicaSet, старые Pod'ы постепенно заменяются (rolling update).

Что такое ConfigMap и Secret?
?
ConfigMap -- key-value конфигурация (не секретная): DATABASE_HOST, LOG_LEVEL. Secret -- конфиденциальные данные (пароли, API-ключи), хранятся в base64 (не зашифрованы!). Оба передаются в Pod через env или volume mount.

Что такое HPA (HorizontalPodAutoscaler)?
?
Контроллер, который автоматически масштабирует количество Pod'ов на основе метрик (CPU, memory, custom). Например: если CPU > 70% -- добавить реплики. Требует metrics-server в кластере.

В чём разница между 128Mi и 128M?
?
128Mi (Mebibytes) = 128 x 1024^2 = 134,217,728 байт. 128M (Megabytes) = 128,000,000 байт. Mi -- двоичная система (используется в K8s), M -- десятичная. Разница ~5%.
...
|-------------|------|-------|
| Следующий шаг | [[kubernetes-advanced]] | RBAC, Network Policies, Operators -- безопасность и расширяемость K8s |
| Углубиться | [[infrastructure-as-code]] | Terraform для создания K8s-кластеров и облачной инфраструктуры |
| Смежная тема | [[microservices-vs-monolith]] | Архитектурные паттерны, для которых K8s наиболее полезен |
