...
> **Послойная диагностика (Layer-by-Layer Troubleshooting)** — методология, основанная на модели OSI: проблему локализуют, двигаясь снизу вверх от физического уровня (L1) к прикладному (L7). Каждый уровень проверяется независимо: если нижний уровень работает корректно, причина проблемы находится выше. Этот подход формализован в модели ISO/IEC 7498-1 (1994).

### Методология: OSI layer-by-layer

| Уровень | Что проверяем | Инструмент | Ожидаемый результат |
|---------|---------------|------------|---------------------|
| L1 (Physical) | Кабель, порт, сигнал | `ip link show`, `ethtool` | Interface UP, link detected |
| L2 (Data Link) | MAC, ARP, VLAN | `arp -a`, `ip neigh` | ARP-запись для gateway |
| L3 (Network) | IP-связность, маршрутизация | `ping`, `traceroute`, `ip route` | ICMP reply, корректный маршрут |
| L4 (Transport) | TCP/UDP порт, соединение | `nc -zv`, `ss -tulnp` | Connection established |
| L5-6 (Session/Presentation) | TLS handshake, сертификат | `openssl s_client` | Valid certificate chain |
| L7 (Application) | HTTP, DNS, протокол | `curl -v`, `dig` | HTTP 200, DNS resolved |

### Типичные паттерны отказов

| Симптом | Вероятный уровень | Типичная причина | Первый шаг |
|---------|-------------------|------------------|------------|
| Нет ответа (timeout) | L3 или firewall | Неверный IP, firewall DROP | `ping`, `traceroute` |
| Connection Refused | L4 | Сервис не запущен, порт закрыт | `ss -tulnp`, `nc -zv` |
| SSL/TLS ошибка | L5-6 | Просроченный сертификат, неполная цепочка | `openssl s_client` |
| HTTP 5xx | L7 | Ошибка приложения, backend down | `curl -v`, логи сервера |
| DNS resolution failed | L7 (DNS) | Неверный DNS-сервер, просроченный TTL | `dig`, `/etc/resolv.conf` |

### Принципы диагностики

- **Снизу вверх** (bottom-up) — начинаем с L1, поднимаемся. Если `ping` не работает, нет смысла проверять HTTP
- **Divide and conquer** — делим путь пополам: проблема на клиенте? на сервере? в сети между ними?
- **Одно изменение за раз** — меняем один параметр, проверяем результат, документируем
- **Baseline** — знание нормального состояния позволяет быстро определить аномалию

> **Правило "Connection Refused vs Timeout"** — мгновенный отказ (RST-пакет) означает: хост доступен, но порт закрыт; длительный timeout означает: пакеты не доходят (firewall DROP, неверный маршрут, хост выключен). Это разграничение — ключевой навык диагностики.

**См. также:** [[network-tools-reference]] (справочник инструментов), [[network-troubleshooting-advanced]] (сложные сценарии), [[network-tcpdump-wireshark]] (глубокий анализ пакетов)
...
> [!question]- Ping до сервера работает, но curl возвращает timeout. Какой уровень стека нужно проверять?
> Ping проверяет только L3 (ICMP). Timeout на curl означает проблему на L4 (TCP) или выше. Вероятные причины: firewall блокирует TCP-порт (разрешён ICMP, заблокирован TCP), приложение не слушает на порту, Security Group без правила для нужного порта. Проверка: telnet host port или nc -zv host port.

> [!question]- Почему "гуглить ошибку" --- плохая стратегия первого шага при сетевой проблеме?
> Без диагностики невозможно определить уровень проблемы. Одинаковый симптом ("не работает") может быть вызван десятками причин на разных уровнях. Системный подход (снизу вверх: ping -> dig -> curl -v) за 5 минут сужает область до конкретного уровня. Гугл полезен после локализации проблемы, а не вместо неё.

> [!question]- Как быстро отличить Connection Refused от Connection Timeout, и что это говорит о проблеме?
> Refused --- мгновенный ответ от сервера: IP доступен, но порт закрыт (RST-пакет). Timeout --- долгое ожидание: пакеты "теряются" (firewall DROP, сервер выключен, неверный IP). Refused --- проблема на целевом хосте (сервис не запущен). Timeout --- проблема в сети или firewall.

---

## Ключевые карточки

Какой порядок диагностики сетевых проблем?
?
Снизу вверх: 1) ping --- L3 связность. 2) dig --- DNS резолвинг. 3) curl -v --- TCP/TLS/HTTP. 4) Проверка заголовков и статус-кодов. 5) Логи приложения. Каждый шаг сужает область поиска. Если ping не работает --- не нужно проверять HTTP.

Что показывает команда dig?
?
dig (DNS lookup) показывает: какой IP-адрес соответствует домену, TTL записи, тип записи (A, CNAME, MX), авторитативный DNS-сервер. dig @8.8.8.8 domain --- запрос через конкретный резолвер. Полезно для сравнения ответов разных DNS.

Что показывает curl -v?
?
curl -v (verbose) показывает весь процесс запроса: DNS resolution, TCP connection, TLS handshake, отправленные заголовки, полученные заголовки, статус-код, тело ответа. Позволяет определить на каком именно этапе проблема.

Что означает traceroute?
?
traceroute показывает маршрут пакетов: каждый маршрутизатор на пути до цели с временем ответа. Помогает определить, где на пути пакет "застревает" или теряется. Звёздочки (*) означают timeout на данном хопе (может быть firewall, а не проблема).

Какие основные метрики в DevTools Network Tab?
?
DNS Lookup (резолвинг домена), Initial Connection (TCP handshake), SSL (TLS handshake), TTFB (Time To First Byte --- серверная обработка), Content Download (загрузка тела). TTFB > 500ms --- проблема сервера. DNS > 100ms --- проблема DNS.
...
|-------------|------|-------|
| Следующий шаг | [[network-tools-reference]] | Полный справочник всех сетевых инструментов |
| Углубиться | [[network-tcpdump-wireshark]] | Глубокий анализ пакетов |
| Смежная тема | [[network-troubleshooting-advanced]] | Сложные сценарии: MTU, intermittent issues |
| Обзор | [[networking-overview]] | Вернуться к карте раздела |

---

*Последнее обновление: 2026-01-09 --- Добавлены педагогические секции: 5 аналогий, 6 типичных ошибок диагностики, 5 ментальных моделей*
