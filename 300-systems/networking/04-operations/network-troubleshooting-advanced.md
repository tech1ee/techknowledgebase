...
> **Root Cause Analysis (RCA)** — систематическая методология определения первопричины инцидента, а не его симптомов. В сетевом контексте RCA предотвращает "лечение симптомов" (например, перезагрузку вместо исправления конфигурации). Формализованные подходы: 5 Whys (Toyoda, Toyota Production System), Ishikawa diagram (fishbone), Fault Tree Analysis (Bell Labs, 1962).

### Методология Fault Domain Isolation

| Домен отказа | Что включает | Как изолировать | Инструменты |
|-------------|--------------|-----------------|-------------|
| **Клиент** | Приложение, ОС, DNS resolver | Воспроизвести с другого клиента | curl, dig, browser DevTools |
| **Локальная сеть** | Switch, VLAN, WiFi AP, firewall | Проверить с другого устройства в сети | ping gateway, arp -a, ip route |
| **WAN / ISP** | Маршрутизация, transit | traceroute/mtr между клиентом и сервером | mtr, traceroute -T |
| **Load Balancer** | L4/L7 балансировщик | Обратиться к backend напрямую | curl --resolve, nc |
| **Сервер** | Приложение, ОС, ресурсы | Проверить localhost, логи, ss | ss -tulnp, journalctl, top |
| **DNS** | Резолвинг, кэширование | Сравнить dig с разных серверов | dig @8.8.8.8, dig @resolver |

### Гипотезо-ориентированная диагностика (Hypothesis-Driven Troubleshooting)

Метод основан на научном подходе:

1. **Наблюдение** — собрать симптомы (что именно не работает, когда началось, кого затрагивает)
2. **Гипотеза** — сформулировать предположение о причине (конкретное, проверяемое)
3. **Предсказание** — определить, что должен показать тест, если гипотеза верна
4. **Тест** — выполнить один диагностический шаг
5. **Анализ** — подтвердить или опровергнуть гипотезу
6. **Итерация** — повторить с новой гипотезой или углубить текущую

### Паттерны отказов и их корневые причины

| Паттерн | Характеристика | Типичные причины | RCA-подход |
|---------|---------------|------------------|------------|
| **Постоянный отказ** | 100% запросов не работают | Misconfiguration, сервис down | Проверка конфигурации, логов |
| **Intermittent** | Случайные сбои | Resource exhaustion, race condition | Корреляция с метриками по timeline |
| **Периодический** | Сбои по расписанию | Cron jobs, backup, log rotation | Корреляция со временем |
| **Под нагрузкой** | Деградация при росте трафика | Capacity, connection pool | Load testing, ss -s |
| **Нарастающий** | Постепенная деградация | Memory leak, conntrack overflow | Мониторинг трендов |

### Принцип Divide and Conquer

> **Бинарный поиск в сетевой диагностике** — делим путь запроса пополам и проверяем каждую половину. Каждый шаг отсекает 50% возможных причин. Пример: работает ли запрос с localhost? Если да — проблема в сети. Если нет — проблема на сервере. Работает ли напрямую к backend? Если да — проблема в load balancer.

**См. также:** [[network-debugging-basics]] (базовая диагностика), [[network-tcpdump-wireshark]] (доказательства на уровне пакетов), [[network-observability]] (baseline и исторические данные)
...
[[network-debugging-basics]] — базовый troubleshooting является обязательным фундаментом перед изучением advanced-методик. Там описаны ping, dig, curl, traceroute — инструменты, которые используются как первый шаг в любой диагностике. Advanced troubleshooting начинается там, где базовые инструменты перестают давать ответы. Рекомендуется изучить базовый материал первым.

[[network-tcpdump-wireshark]] — захват и анализ пакетов является ключевым навыком для advanced troubleshooting. Когда гипотеза требует доказательства на уровне пакетов (retransmissions, zero window, MTU issues), tcpdump и Wireshark дают объективные данные вместо предположений. Этот материал дополняет текущий, давая инструментарий для проверки гипотез.

[[observability]] — мониторинг, метрики и alerting критически важны для advanced troubleshooting, потому что без baseline и исторических данных невозможно определить «что изменилось». Prometheus, Grafana, distributed tracing — всё это превращает troubleshooting из реактивного в проактивный. Материалы дополняют друг друга и могут изучаться параллельно.

[[network-transport-layer]] — глубокое понимание TCP state machine, congestion control и QUIC необходимо для диагностики сложных проблем: TIME_WAIT accumulation, retransmission storms, zero window conditions. Без этих знаний симптомы транспортного уровня выглядят как «application timeout». Рекомендуется изучить TCP internals перед advanced troubleshooting.

[[devops-incident-management]] — incident response и post-mortem процессы являются организационной стороной troubleshooting. Техническая диагностика (этот материал) и процесс управления инцидентами (incident management) работают вместе: один даёт методологию поиска root cause, другой — framework для координации команды и документирования.
...
### Теоретические основы
- Toyoda S. (1950s). "5 Whys" — Toyota Production System (методология RCA)
- Vesely W. et al. (1981). "Fault Tree Handbook" — NUREG-0492, U.S. Nuclear Regulatory Commission
- **Kurose J., Ross K. (2021). Computer Networking: A Top-Down Approach, 8th Edition.** — Академический учебник с глубоким объяснением сетевых протоколов, которые являются предметом диагностики.

### Практические руководства
- **Fall K., Stevens W.R. (2011). TCP/IP Illustrated, Volume 1: The Protocols, 2nd Edition.** — Классический справочник с реальными трассировками пакетов, который учит «читать» поведение TCP/IP стека. Незаменим для анализа дампов tcpdump и Wireshark при advanced troubleshooting.

- **Peterson L., Davie B. (2011). Computer Networks: A Systems Approach, 5th Edition.** — Системный подход к сетевым технологиям, который помогает понять взаимосвязи между уровнями стека. Полезен для develop divide-and-conquer мышления при сложной диагностике.

---

## Проверь себя

> [!question]- Микросервис A вызывает микросервис B. Каждый десятый запрос уходит в timeout (30 секунд). CPU и память обоих сервисов в норме. Как ты будешь систематически искать причину?
> 1. Сформулировать гипотезу: intermittent = не воспроизводится стабильно, значит проблема скорее всего в ресурсах или timing. 2. Собрать baseline: мониторинг latency P50 vs P99 — если P50 норма, а P99 взлетает, проблема в хвостовых запросах. 3. tcpdump между A и B: `tcpdump -i any port <B_port> -w capture.pcap` — посмотреть, уходит ли SYN от A, приходит ли SYN-ACK от B, есть ли retransmissions. 4. Проверить connection pool: `ss -tn | grep <B_port> | wc -l` — не исчерпан ли пул. 5. Distributed tracing (Jaeger/Zipkin): где именно тратится время — DNS, connect, wait, transfer. 6. Корреляция по времени: совпадает ли с GC паузами, cron jobs, другим трафиком.

> [!question]- После деплоя новой версии TLS-сертификата часть клиентов получают ошибку "SSL: CERTIFICATE_VERIFY_FAILED", но браузер открывает сайт без проблем. Почему это происходит и как диагностировать?
> Браузер автоматически достраивает неполные цепочки сертификатов (скачивает intermediate CA по ссылке в AIA extension), а библиотеки (curl, Java, Python requests) этого не делают. Диагностика: 1. `openssl s_client -connect host:443 -showcerts` — проверить, что сервер отдает полную цепочку (server + intermediate). 2. Если depth=0 только server cert — incomplete chain, нужно добавить intermediate. 3. `curl -v https://host` — покажет "unable to get local issuer certificate". 4. Проверить через `ssllabs.com/ssltest/` — покажет "Extra download" для intermediate. Решение: объединить server cert + intermediate в один файл для nginx/apache.

> [!question]- Пользователи жалуются на медленную загрузку. mtr показывает 40% packet loss на hop 5, но итоговый хост отвечает без потерь. Это реальная проблема или ложное срабатывание?
> Это, скорее всего, ложное срабатывание. Многие маршрутизаторы намеренно ограничивают обработку ICMP (rate limiting) для защиты control plane, при этом данные пересылают нормально. Если последний hop показывает 0% loss и нормальный RTT — промежуточные потери ICMP не являются причиной проблемы. Реальная потеря на промежуточном hop приводит к потерям на всех последующих hop, включая финальный. Для проверки: сравнить TCP-трафик (iperf3, curl timing) — если throughput и latency в норме, ICMP loss на промежуточном hop можно игнорировать.
...
?
Метод диагностики по шагам: 1) Собрать симптомы, 2) Сформулировать гипотезу, 3) Определить тест для проверки, 4) Выполнить тест, 5) Подтвердить или опровергнуть. Повторять пока root cause не найден. Противоположность хаотичному перебору.

Divide and conquer в сетевой диагностике
?
Разделить путь запроса пополам и проверить каждую половину. Работает ли localhost? Работает ли с соседнего хоста? Через LB или напрямую? Каждый шаг отсекает половину возможных причин, как бинарный поиск.

Baseline
?
Нормальное состояние системы, зафиксированное заранее. Без baseline невозможно сказать "стало хуже" — только "сейчас плохо". Включает: типичный latency (P50/P99), retransmission rate, connection count, DNS resolution time.

Connection pool exhaustion
?
Все соединения из пула заняты, новые запросы ждут в очереди или получают timeout. Причины: connection leak (не закрывается), slow queries (долго держат), маленький pool size. Диагностика: `ss -tn | grep PORT | wc -l` + логи пула.

Intermittent failure паттерны
?
Периодические сбои: совпадают с cron/scheduled tasks. Случайные: race condition, resource exhaustion. Под нагрузкой: capacity issue. Ключ к диагностике — корреляция с метриками (CPU, memory, disk I/O, network) по timeline.

Post-mortem документация
?
Отчет после инцидента: симптомы, timeline, root cause, resolution, prevention. Цель — не обвинить, а предотвратить повторение. Включает action items с owners и deadlines.
...
|-------------|------|-------|
| Базовая диагностика | [[network-debugging-basics]] | Фундамент перед advanced методиками |
| Анализ пакетов | [[network-tcpdump-wireshark]] | Доказательства на уровне пакетов |
| Мониторинг | [[network-observability]] | Baseline и исторические данные для сравнения |
| TCP internals | [[network-transport-layer]] | Понимание retransmissions и zero window |
| Incident management | [[devops-incident-management]] | Организационная сторона troubleshooting |
