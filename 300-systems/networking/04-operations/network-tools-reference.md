...
> **Таксономия сетевых инструментов** основана на уровнях модели OSI: каждый инструмент работает на определённом уровне сетевого стека и предоставляет видимость соответствующих протоколов. Выбор инструмента определяется уровнем, на котором предполагается проблема.

### Классификация инструментов по уровню OSI

| Уровень OSI | Инструмент | Что проверяет | Тип |
|-------------|------------|---------------|-----|
| L1 (Physical) | `ethtool`, `ip link` | Состояние интерфейса, скорость, duplex | Диагностика |
| L2 (Data Link) | `arp`, `ip neigh`, `bridge` | ARP-таблица, MAC-адреса, VLAN | Диагностика |
| L3 (Network) | `ping`, `traceroute`, `mtr` | ICMP-связность, маршрутизация | Связность |
| L3 (Network) | `ip route`, `nmap` | Таблица маршрутизации, сканирование | Конфигурация |
| L4 (Transport) | `ss`, `nc`, `iperf3` | TCP/UDP соединения, throughput | Состояние / Бенчмарк |
| L4 (Transport) | `tcpdump`, `Wireshark` | Захват и анализ пакетов | Анализ трафика |
| L5-6 (TLS) | `openssl s_client`, `curl -v` | TLS handshake, сертификаты | Безопасность |
| L7 (Application) | `curl`, `dig`, `nslookup` | HTTP, DNS | Прикладной уровень |
| Cross-layer | `mtr`, `tcptraceroute` | Комбинированная диагностика | Комплексный |

### Методология диагностики

Выбор инструмента следует принципу **"сверху вниз по симптому, снизу вверх по проверке":**

1. **Определи симптом** (L7): "сайт не открывается", "API возвращает 500"
2. **Проверяй снизу** (L3 -> L4 -> L7): ping -> nc/ss -> curl -v -> dig
3. **Углубляйся** при необходимости: tcpdump -> Wireshark -> eBPF

### Категории инструментов

| Категория | Назначение | Примеры |
|-----------|-----------|---------|
| **Connectivity** | Проверка доступности | ping, traceroute, mtr, nc |
| **DNS** | Разрешение имён | dig, nslookup, drill |
| **Traffic Analysis** | Захват и анализ пакетов | tcpdump, Wireshark, tshark |
| **Performance** | Измерение производительности | iperf3, wrk, hey, ab |
| **Configuration** | Просмотр/изменение настроек | ip, ss, sysctl, ethtool |
| **Security** | Сканирование и TLS | nmap, openssl, testssl.sh |

> **Правило "right tool for the right layer"**: использование инструмента не того уровня приводит к ложным выводам. Пример: `ping` (L3) работает, но `curl` (L7) не работает — это НЕ "странная ошибка", а нормальная ситуация, когда проблема на L4-L7 (firewall блокирует TCP, сервис не запущен).

**См. также:** [[network-debugging-basics]] (методология диагностики), [[network-tcpdump-wireshark]] (глубокий анализ пакетов), [[network-troubleshooting-advanced]] (комбинирование инструментов)
...
**[[network-debugging-basics]]** — Debugging basics даёт методологию сетевой диагностики (послойный подход, изоляция проблемы, бритва Оккама), тогда как tools reference предоставляет детальное описание каждого инструмента с флагами, примерами и типичными сценариями. Без методологии набор инструментов превращается в хаотичный перебор, а методология без инструментов остаётся теорией. Сначала изучите системный подход к debugging, затем используйте этот справочник как ежедневную шпаргалку.

**[[network-troubleshooting-advanced]]** — Advanced troubleshooting покрывает сложные сценарии диагностики (intermittent failures, performance degradation, DNS poisoning), где базовых инструментов недостаточно и требуется комбинирование нескольких утилит. Этот справочник даёт детальное описание каждого инструмента, а advanced troubleshooting учит их комбинировать для решения нетривиальных проблем. Рекомендуется освоить инструменты из этого справочника перед переходом к сложным кейсам.

**[[devops-incident-management]]** — Сетевые инструменты из этого справочника являются основным арсеналом при расследовании инцидентов: ping/traceroute для проверки connectivity, tcpdump для захвата трафика, ss/netstat для анализа соединений, curl для тестирования endpoints. Быстрое владение этими инструментами критично для сокращения MTTR (Mean Time To Resolve) во время инцидентов. Знание инструментов — prerequisite для эффективного incident management.
...
### Теоретические основы
- ISO/IEC 7498-1 (1994). Open Systems Interconnection — Basic Reference Model (формальное определение уровней OSI)
- McCanne S., Jacobson V. (1993). "The BSD Packet Filter" — USENIX (теоретическая основа tcpdump)
- **Fall, Stevens (2011).** *TCP/IP Illustrated, Vol. 1 (2nd ed).* — каждый протокол разбирается с примерами работы диагностических инструментов; незаменим для понимания того, что именно показывают tcpdump, traceroute и другие утилиты.

### Практические руководства
- **Hunt (2002).** *TCP/IP Network Administration.* — классическое руководство по сетевому администрированию с детальным описанием диагностических инструментов Unix/Linux; идеален как справочник для повседневной работы.
- **Sanders (2017).** *Practical Packet Analysis with Wireshark.* — углублённое описание tcpdump и Wireshark, которые являются ключевыми инструментами этого справочника; от базового захвата до сложного анализа.

---

## Проверь себя

> [!question]- Сервер отвечает на ping, но curl на порт 443 висит без ответа. Какие инструменты и в каком порядке ты используешь для диагностики?
> 1. `nc -zv host 443` — проверить, открыт ли порт (TCP connect). 2. `ss -tulnp | grep 443` на сервере — слушает ли что-то на порту. 3. `curl -v https://host` — посмотреть, на каком этапе зависает (DNS, TCP connect, TLS handshake, ответ). 4. `traceroute -T -p 443 host` — TCP traceroute до порта. 5. `sudo tcpdump -i any port 443` — захватить пакеты, чтобы увидеть SYN без SYN-ACK (firewall) или зависание на TLS. Порядок от простого к сложному позволяет быстро сузить область проблемы.

> [!question]- Почему `dig example.com` может показывать правильный IP, а браузер при этом не может открыть сайт? Назови минимум 3 причины.
> 1. DNS-кэш браузера или ОС содержит устаревший IP (dig обращается к серверу напрямую, обходя кэш). 2. Файл `/etc/hosts` перенаправляет имя на другой IP, а dig не читает hosts. 3. dig использует другой DNS-сервер, нежели ОС (проверить через `dig example.com` vs `dig example.com @$(cat /etc/resolv.conf | grep nameserver | head -1 | awk '{print $2}')`). 4. Проблема не в DNS, а в TCP/TLS — порт заблокирован, сертификат невалиден, редирект на другой домен.

> [!question]- Команда `ss -tulnp` показывает процесс на порту 8080, но внешние клиенты не могут подключиться. Какие шаги предпримешь?
> 1. Проверить bind address: если процесс слушает на 127.0.0.1:8080, он доступен только локально — нужен 0.0.0.0:8080. 2. Проверить firewall: `iptables -L -n` или `ufw status` — порт может быть закрыт. 3. Проверить Security Group / NACL (если в облаке). 4. `curl -v http://localhost:8080` с самого сервера — работает ли сервис вообще. 5. `tcpdump -i any port 8080` — приходят ли пакеты от клиента.

> [!question]- В чём разница между `traceroute` и `mtr`? В каком сценарии ты выберешь mtr вместо traceroute?
> traceroute делает один снимок маршрута (три пакета на каждый hop). mtr непрерывно отправляет пакеты и показывает статистику (loss%, avg/best/worst latency) для каждого hop в реальном времени. mtr лучше при intermittent packet loss — он покажет на каком hop теряются пакеты и с какой частотой. traceroute покажет только одномоментную картину. Для быстрой проверки маршрута хватит traceroute, для анализа нестабильности — mtr.
...
ping
?
ICMP Echo Request/Reply. Проверяет доступность хоста и базовый RTT. Не проверяет порты и может быть заблокирован firewall (ICMP DROP).

dig vs nslookup
?
dig дает полный DNS-ответ (все секции, TTL, флаги), nslookup — упрощённый вывод. dig не использует /etc/hosts и системный кэш, обращается напрямую к DNS-серверу.

curl -v
?
Verbose-режим curl показывает все этапы HTTP-запроса: DNS resolution, TCP connect, TLS handshake, отправленные заголовки, полученный ответ. Основной инструмент отладки HTTP.

ss -tulnp
?
Показывает все listening сокеты: TCP (-t) и UDP (-u), listening (-l), числовые адреса (-n), имя процесса (-p). Замена устаревшему netstat.

tcpdump -i any port 80
?
Захватывает все пакеты на любом интерфейсе (-i any) с фильтром по порту 80. Для чтения дампа лучше сохранить в файл (-w dump.pcap) и открыть в Wireshark.

nc -zv host port
?
Zero I/O mode (-z) с verbose (-v) — быстрая проверка, открыт ли TCP-порт. Не отправляет данных, только TCP handshake. Для UDP добавить -u.

nmap -sV target
?
Сканирует порты и определяет версии сервисов. SYN scan (-sS, нужен root) быстрее и скрытнее, чем connect scan (-sT). По умолчанию сканирует 1000 популярных портов.

iperf3 -c server
?
Измеряет пропускную способность (throughput) между клиентом и сервером. Для UDP-теста добавить -u. Потребляет всю bandwidth, запускать вне рабочих часов.

mtr target
?
Комбинация ping + traceroute в реальном времени. Показывает loss% и статистику latency на каждом hop. Лучший инструмент для диагностики intermittent packet loss.

openssl s_client -connect host:443
?
Устанавливает TLS-соединение и показывает цепочку сертификатов, версию TLS, используемый cipher suite. Основной инструмент отладки TLS/HTTPS проблем.
...
|-------------|------|-------|
| Методология диагностики | [[network-debugging-basics]] | Системный подход к использованию инструментов |
| Глубокий анализ пакетов | [[network-tcpdump-wireshark]] | tcpdump и Wireshark для сложных случаев |
| Сложные кейсы | [[network-troubleshooting-advanced]] | Комбинирование инструментов в production |
| Оптимизация производительности | [[network-performance-optimization]] | Benchmarking и TCP tuning с iperf3 |
