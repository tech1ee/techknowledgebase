...
**[[network-debugging-basics]]** — Debugging basics даёт системный подход к сетевой диагностике (послойный анализ, изоляция проблемы), тогда как tcpdump и Wireshark — это конкретные инструменты для глубокого анализа на уровне пакетов. Packet analysis применяется, когда стандартные инструменты (ping, traceroute, curl) не позволяют локализовать проблему и нужно увидеть реальное содержимое сетевого трафика. Освойте системный подход к debugging прежде, чем погружаться в packet analysis.

**[[network-transport-layer]]** — TCP и UDP — это протоколы, трафик которых чаще всего анализируется в Wireshark: TCP-handshake, retransmissions, window scaling, RST-пакеты, UDP-потоки. Без понимания TCP state machine и механизмов управления потоком интерпретация пакетных дампов будет поверхностной — вы увидите пакеты, но не поймёте, почему соединение ведёт себя определённым образом. Транспортный уровень — обязательный prerequisite.

**[[security-https-tls]]** — TLS-трафик является одним из самых частых объектов анализа в Wireshark: диагностика handshake-ошибок, проверка cipher suites, анализ certificate chain. При этом TLS 1.3 шифрует большинство метаданных, что существенно ограничивает возможности пассивного анализа. Понимание TLS необходимо для корректной интерпретации зашифрованного трафика и настройки дешифрования в Wireshark через SSLKEYLOGFILE.
...
> [!question]- Вы захватываете трафик tcpdump через SSH-сессию и получаете гигабайты данных за секунды. Что не так?
> tcpdump захватывает и свои собственные SSH-пакеты, создавая петлю: каждый захваченный пакет генерирует SSH-трафик, который тоже захватывается. Решение: исключить SSH из захвата фильтром "not port 22" или "not host your-ip". Всегда фильтруйте при захвате.

> [!question]- В Wireshark видны TCP Retransmissions. Это всегда проблема?
> Нет. Небольшой процент retransmissions (< 1%) --- нормально для любой сети. Проблема, если: процент > 2-3%, retransmissions для конкретного хоста/потока, растут RTO (retransmission timeout). Важно смотреть контекст: единичные retransmissions при высокой нагрузке --- норма, постоянные на пустой сети --- проблема.

> [!question]- Чем отличается capture filter от display filter в Wireshark и когда использовать каждый?
> Capture filter (BPF syntax: "port 80") применяется при захвате --- непрошедшие пакеты не сохраняются. Display filter (Wireshark syntax: "http.response.code == 500") применяется к уже захваченным данным. Capture filter для ограничения объёма данных. Display filter для анализа. Не путать синтаксис: "port 80" (capture) vs "tcp.port == 80" (display).

---

## Ключевые карточки

Базовая команда tcpdump для захвата HTTP-трафика?
?
tcpdump -i any -n port 80 -w capture.pcap. Флаги: -i any (все интерфейсы), -n (не резолвить DNS), port 80 (фильтр), -w (сохранить в файл). Для чтения: tcpdump -r capture.pcap. Для HTTPS (port 443) содержимое зашифровано.

Как использовать Follow TCP Stream в Wireshark?
?
Правый клик на пакет -> Follow -> TCP Stream. Показывает полный диалог клиент-сервер в читаемом виде. Красный --- данные от клиента, синий --- от сервера. Незаменимо для анализа HTTP-запросов и отладки протоколов.

Какие цвета в Wireshark что означают?
?
По умолчанию: зелёный --- HTTP, голубой --- UDP, чёрный --- TCP errors (retransmissions, resets), красный --- bad TCP (RST, unreachable), жёлтый --- warning. Цветовая схема настраивается. Чёрные строки при анализе --- первое место для расследования.

Как расшифровать TLS-трафик в Wireshark?
?
Два способа: 1) Pre-master secret log (SSLKEYLOGFILE) --- браузер записывает ключи, Wireshark использует их. Работает с TLS 1.3. 2) RSA private key сервера --- только для TLS 1.2 без forward secrecy. Способ 1 предпочтителен для отладки.

Что такое BPF и зачем нужны capture filters?
?
BPF (Berkeley Packet Filter) --- язык для фильтрации пакетов в ядре. Примеры: "host 10.0.0.1", "port 443", "tcp and not port 22". Фильтрация на уровне ядра экономит CPU и дисковое пространство. Без фильтра на загруженном сервере pcap-файл вырастет до гигабайт за минуты.
...
|-------------|------|-------|
| Следующий шаг | [[network-troubleshooting-advanced]] | Сложные сценарии: MTU, intermittent issues |
| Углубиться | [[network-transport-layer]] | TCP internals для понимания пакетов |
| Смежная тема | [[network-performance-optimization]] | Анализ производительности через пакеты |
| Обзор | [[networking-overview]] | Вернуться к карте раздела |

---

*Последнее обновление: 2026-01-09 --- Добавлены педагогические секции: 5 аналогий, 6 типичных ошибок packet analysis, 5 ментальных моделей*
