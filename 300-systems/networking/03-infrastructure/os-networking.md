...
[[networking-overview]] — обзорная карта сетевых технологий, которая помещает сетевой стек ОС в контекст всех уровней. Пока обзор объясняет «что» (модель OSI, протоколы), этот материал объясняет «как» (syscalls, sk_buff, netfilter). Рекомендуется изучить обзор для понимания общей архитектуры, затем перейти к реализации в ядре.

[[network-transport-layer]] — TCP/UDP/QUIC реализованы внутри сетевого стека ОС, и системные вызовы socket(), bind(), listen(), accept() являются интерфейсом к этой реализации. Понимание TCP state machine на уровне протокола дополняется знанием того, как ядро управляет sk_buff, congestion control и socket buffers. Эти материалы дополняют друг друга — один теоретический, другой имплементационный.

[[network-ip-routing]] — маршрутизация в Linux реализована через routing table ядра, а netfilter/iptables позволяют фильтровать и манипулировать пакетами на уровне IP. Понимание IP-адресации, подсетей и NAT на концептуальном уровне необходимо перед изучением iptables chains и nftables правил в ядре.

[[docker-for-developers]] — Docker использует Linux network primitives (namespaces, veth pairs, bridges, iptables) для создания изолированных сетей контейнеров. Понимание сетевого стека ОС объясняет, как docker0 bridge работает «под капотом» и почему port mapping реализован через iptables NAT rules. Рекомендуется изучить OS networking перед Docker networking deep dive.

[[network-cloud-modern]] — eBPF и Cilium, описанные в этом материале, являются технологической основой для Service Mesh и cloud-native networking. XDP для DDoS protection, eBPF для observability — всё это реализовано на уровне сетевого стека ОС и используется в Kubernetes networking (Cilium CNI).
...
- **Stevens W.R. (1994). TCP/IP Illustrated, Volume 1: The Protocols.** — Классический справочник, который детально описывает работу TCP/IP стека с точки зрения реализации в ядре. Трассировки пакетов в книге показывают то же поведение, которое можно наблюдать через tcpdump на Linux.

- **Comer D. (2015). Internetworking with TCP/IP, Volume 1, 6th Edition.** — Системный подход к реализации TCP/IP, включая socket API, buffer management и взаимодействие ядра с сетевыми устройствами. Даёт фундамент для понимания Linux networking stack.

- **Peterson L., Davie B. (2011). Computer Networks: A Systems Approach, 5th Edition.** — Книга с системным подходом, которая объясняет реализацию сетевых стеков в операционных системах: от device drivers до socket layer. Полезна для понимания архитектуры sk_buff и netfilter hooks.
...
> [!question]- Почему после перезапуска сервиса он не может занять порт и выдаёт EADDRINUSE, хотя старый процесс уже завершён?
> TCP-соединения после закрытия остаются в состоянии TIME_WAIT (2 минуты по умолчанию) для обработки запоздавших пакетов. Порт считается занятым. Решение: использовать SO_REUSEADDR (позволяет bind к порту в TIME_WAIT) или SO_REUSEPORT (несколько процессов на одном порту для load balancing).

> [!question]- Как eBPF/XDP ускоряет обработку пакетов по сравнению с iptables?
> iptables обрабатывает пакеты в kernel space, но проходит через весь сетевой стек (L2->L3->L4->netfilter chains). XDP перехватывает пакеты на уровне драйвера NIC, до создания sk_buff, что экономит allocation и копирование. XDP может DROP/REDIRECT пакет за наносекунды, iptables --- за микросекунды. Разница 10-100x.

> [!question]- Настроили iptables с политикой INPUT DROP, и потеряли SSH-доступ. Что пошло не так и как предотвратить?
> При смене политики на DROP без предварительного правила ACCEPT для SSH (порт 22) все входящие пакеты отбрасываются, включая SSH-сессию. Правильный порядок: сначала добавить ACCEPT для SSH, затем менять политику. Страховка: cron-задача, снимающая правила через 5 минут, или out-of-band доступ (IPMI/console).

---

## Ключевые карточки

Что такое socket в контексте ОС?
?
Socket --- файловый дескриптор (fd), через который приложение взаимодействует с сетевым стеком ядра. Системные вызовы: socket() создаёт, bind() привязывает к адресу, listen() начинает слушать, accept() принимает соединение, read()/write() обмен данными. "Всё есть файл" в Unix.

Что такое netfilter и iptables?
?
Netfilter --- фреймворк в ядре Linux для обработки пакетов. Iptables --- утилита для настройки правил netfilter. 5 точек перехвата (hooks): PREROUTING, INPUT, FORWARD, OUTPUT, POSTROUTING. Правила в таблицах (filter, nat, mangle) обрабатываются по цепочкам, первое совпадение --- действие.

Что такое network namespace?
?
Network namespace --- изолированный сетевой стек (свои интерфейсы, routing table, iptables, порты). Docker создаёт отдельный namespace для каждого контейнера. Контейнеры не видят сетевые интерфейсы хоста. Связь через veth-пары (виртуальный кабель между namespaces).

Как работает epoll для обработки множества соединений?
?
epoll --- механизм Linux для мониторинга множества fd без polling. Приложение регистрирует fd в epoll, ядро уведомляет о готовности. O(1) для проверки событий вместо O(n) у select/poll. Основа nginx, Node.js, и других высоконагруженных серверов.

Что такое sk_buff?
?
sk_buff (socket buffer) --- структура ядра Linux, описывающая сетевой пакет. Содержит указатели на данные, метаданные (протокол, интерфейс), headroom/tailroom для добавления/удаления заголовков при прохождении через стек. Аллокация sk_buff --- одна из основных затрат при обработке пакетов.
...
|-------------|------|-------|
| Следующий шаг | [[network-docker-deep-dive]] | Как namespaces, veth, bridges используются в Docker |
| Углубиться | [[network-performance-optimization]] | TCP tuning, буферы ядра, sysctl параметры |
| Смежная тема | [[os-processes-threads]] | Процессы, fd, epoll в контексте ОС |
| Обзор | [[networking-overview]] | Вернуться к карте раздела |

---

*Последнее обновление: 2026-01-09 --- Добавлены педагогические секции: 5 аналогий (socket, путь пакета, netfilter/iptables, network namespace, eBPF/XDP), 6 типичных ошибок с СИМПТОМ/РЕШЕНИЕ, 5 ментальных моделей*
