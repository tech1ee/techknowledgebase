...
> [!question]- Почему контейнеры в default bridge network не могут обращаться друг к другу по имени, а в user-defined bridge --- могут?
> Default bridge не имеет встроенного DNS-резолвера для контейнеров. User-defined bridge запускает embedded DNS (127.0.0.11), который резолвит имена контейнеров в IP-адреса. Это основная причина использовать docker-compose (создаёт user-defined network) вместо ручного запуска в default bridge.

> [!question]- UFW настроен блокировать порт 8080, но контейнер с -p 8080:80 доступен извне. Почему?
> Docker добавляет свои правила в iptables PREROUTING/FORWARD до UFW. UFW работает с INPUT chain, а Docker маршрутизирует трафик через FORWARD, минуя UFW. Решение: использовать DOCKER-USER chain для кастомных правил или настраивать iptables напрямую, а не полагаться на UFW.

> [!question]- Когда стоит использовать host network mode вместо bridge?
> Host mode убирает сетевую изоляцию: контейнер использует сетевой стек хоста напрямую. Преимущество --- нет NAT overhead, максимальная производительность. Используют для: высоконагруженных сервисов (latency-sensitive), сетевых утилит (tcpdump), когда нужен доступ к реальному IP клиента. Минус --- нет port isolation между контейнерами.

---

## Ключевые карточки

Какие сетевые драйверы Docker существует и когда какой использовать?
?
bridge (default) --- изолированная сеть, NAT наружу. host --- без изоляции, максимальная производительность. none --- без сети. overlay --- multi-host (Docker Swarm). macvlan --- контейнер получает реальный MAC/IP в физической сети. ipvlan --- L3 аналог macvlan.

Что такое veth pair?
?
veth (virtual ethernet) pair --- виртуальный "кабель" между двумя network namespaces. Один конец в namespace контейнера (eth0), другой подключен к bridge на хосте (docker0). Все пакеты из контейнера проходят через veth в bridge, затем через NAT наружу.

Как работает port mapping (-p) в Docker?
?
-p 8080:80 создаёт iptables DNAT правило: трафик на хост:8080 перенаправляется на IP контейнера:80. Docker добавляет правила в nat/PREROUTING и filter/FORWARD. Контейнер не знает о маппинге --- он слушает свой порт 80 в своём namespace.

Как работает DNS в Docker Compose?
?
Docker Compose создаёт user-defined bridge с embedded DNS (127.0.0.11). Каждый сервис резолвится по имени из docker-compose.yml. Запрос "db" -> DNS -> IP контейнера db. При scale > 1 DNS возвращает round-robin IP всех реплик.

Что такое macvlan и когда его использовать?
?
macvlan даёт контейнеру реальный MAC-адрес и IP в физической сети хоста. Контейнер выглядит как обычное устройство в LAN. Нет NAT, нет port mapping. Используют для: legacy-приложений, требующих реальный IP, DHCP-серверов. Ограничение: контейнер не может общаться с хостом без bridge sub-interface.
...
|-------------|------|-------|
| Следующий шаг | [[network-kubernetes-deep-dive]] | CNI, Services, Network Policies в K8s |
| Углубиться | [[os-networking]] | Linux networking primitives под Docker |
| Смежная тема | [[docker-for-developers]] | Docker basics: образы, volumes, compose |
| Обзор | [[networking-overview]] | Вернуться к карте раздела |

---

*Последнее обновление: 2026-01-09 --- Добавлены педагогические секции: 5 аналогий (контейнеры, veth, типы сетей, port mapping, Docker Compose), 6 типичных ошибок с СИМПТОМ/РЕШЕНИЕ, 5 ментальных моделей*
