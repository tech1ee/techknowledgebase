...
**[[networking-overview]]** --- обзорная карта всей сетевой области, которая систематизирует связи между темами и помогает выстроить последовательность изучения. Данный материал по основам --- отправная точка для всех остальных сетевых тем. После усвоения фундамента (IP, порты, DNS, TCP/UDP, HTTP) можно углубляться в любое направление: транспортный уровень, безопасность, оптимизацию производительности, контейнерные сети.

**[[network-dns-tls]]** --- DNS и TLS, кратко затронутые здесь, подробно раскрываются в отдельном материале. DNS --- не просто "телефонная книга", а сложная иерархическая система с кэшированием на каждом уровне, TTL-политиками, DNSSEC для защиты от подмены и специфичными атаками (DNS spoofing, cache poisoning). TLS 1.3 сократил handshake до одного RTT и добавил 0-RTT resumption, что критично для производительности мобильных приложений.

**[[network-transport-layer]]** --- TCP и UDP, представленные здесь на уровне аналогий, имеют глубокую внутреннюю механику. TCP включает congestion control (slow start, congestion avoidance), flow control через sliding window, механизмы retransmission и SACK. QUIC (основа HTTP/3) работает поверх UDP и решает проблему head-of-line blocking, присущую TCP. Понимание транспортного уровня критично для диагностики проблем с производительностью.

**[[network-http-evolution]]** --- HTTP прошёл путь от простого текстового протокола (HTTP/1.0) через persistent connections и pipelining (HTTP/1.1), binary framing и multiplexing (HTTP/2) до работы поверх QUIC (HTTP/3). Каждая версия решала конкретные проблемы предыдущей: head-of-line blocking, overhead создания соединений, неэффективность текстового формата. Для мобильных разработчиков разница между HTTP/1.1 и HTTP/2 может означать 30-50% ускорения загрузки.

**[[network-debugging-basics]]** --- если данный материал объясняет "что такое сеть", то debugging basics отвечает на вопрос "что делать, когда сеть не работает". Систематический подход к диагностике (снизу вверх по уровням OSI) превращает хаотичный поиск проблемы в методичный процесс: проверка физического уровня, IP-связности, DNS-резолвинга, TLS-handshake, HTTP-ответа.
...
> [!question]- Почему DNS-запрос часто является узким местом при первом обращении к сайту?
> Потому что DNS-резолвинг происходит до установки TCP-соединения и может потребовать обращения к нескольким серверам (Root, TLD, Authoritative). Без кэша это добавляет 20-120ms к каждому первому запросу. Именно поэтому используют DNS prefetch и локальный кэш.

> [!question]- Пользователь жалуется, что API "не работает". В консоли браузера видно CORS-ошибку. На какой стороне проблема и почему Postman работает нормально?
> Проблема на стороне сервера --- он не отправляет CORS-заголовки (Access-Control-Allow-Origin). Postman работает, потому что CORS --- это исключительно браузерная защита. Браузер перед кросс-доменным запросом отправляет preflight OPTIONS-запрос и проверяет ответные заголовки. Curl и Postman этого не делают.

> [!question]- В чём разница между Connection Refused и Connection Timeout, и как это помогает в диагностике?
> Connection Refused (мгновенный ответ) означает, что сервер доступен, но ничто не слушает на указанном порту --- приложение не запущено или порт неверный. Connection Timeout (долгое ожидание) означает, что пакеты уходят в "никуда" --- firewall с DROP-правилом, сервер выключен или неверный IP. Это сразу определяет направление диагностики.

> [!question]- Разработчик поставил fetch без обработки ошибок и timeout. Какие конкретные проблемы возникнут в production?
> Запрос может "висеть" минутами при проблемах с сервером, вызывая ANR на мобильных. При отсутствии интернета пользователь не получит понятного сообщения. При 5xx-ошибках не будет retry. При невалидном Content-Type попытка вызвать .json() бросит исключение. Нужны явные timeout, проверка response.ok, обработка сетевых ошибок.

> [!question]- Почему при переходе с localhost на production приложение часто ломается, и какой чеклист поможет это предотвратить?
> Локально все порты открыты, DNS разрешается в 127.0.0.1, HTTPS не требуется, CORS не актуален. В production другие Security Groups, DNS-записи, обязательный HTTPS, CORS для кросс-доменных запросов. Чеклист: URL (относительные или с переменной окружения), HTTPS настроен, CORS для прод-домена, переменные окружения актуальны, порты и firewall открыты.
...
?
DNS (Domain Name System) переводит человекочитаемые доменные имена (google.com) в IP-адреса (142.250.74.46). Без DNS пришлось бы запоминать числовые адреса всех сайтов.

Какие этапы проходит запрос от ввода URL до получения страницы?
?
7 этапов: парсинг URL, DNS lookup (20-120ms), TCP handshake (1 RTT), TLS handshake (1-2 RTT для TLS 1.3), HTTP request, server response, rendering (DOM, CSS, JS).

В чём разница между TCP и UDP?
?
TCP --- надёжная доставка с подтверждением (handshake, ACK, порядок, retransmission). UDP --- быстрая доставка без гарантий (нет handshake, пакеты могут потеряться). TCP для веб/API, UDP для видеозвонков и игр.

Что означают HTTP-статусы 401 и 403?
?
401 Unauthorized --- "кто ты?", нет или невалидный токен, нужна аутентификация. 403 Forbidden --- "знаю кто ты, но нельзя", пользователь аутентифицирован, но нет прав. 401 показываем форму логина, 403 --- сообщение "нет доступа".

Что такое CORS и почему он работает только в браузере?
?
CORS (Cross-Origin Resource Sharing) --- браузерная защита от кросс-доменных запросов. Браузер проверяет заголовки Access-Control-Allow-Origin от сервера. Curl и Postman не проверяют CORS, поэтому запросы проходят.

Что такое TTFB и о чём он говорит?
?
TTFB (Time To First Byte) --- время от отправки запроса до получения первого байта ответа. Включает DNS, TCP, TLS, серверную обработку. TTFB > 500ms --- сигнал о проблеме на сервере или в сети.

Чем отличается bandwidth от latency?
?
Bandwidth --- толщина трубы (сколько данных проходит за раз). Latency --- длина трубы (как быстро дойдёт первый байт). Для API-запросов важнее latency, для загрузки файлов --- bandwidth.

Что такое сокет и 5-tuple?
?
Сокет = IP + Port + Protocol --- программный endpoint для сетевой коммуникации. 5-tuple --- уникальный идентификатор соединения: Source IP, Source Port, Destination IP, Destination Port, Protocol.

Зачем нужен Three-Way Handshake в TCP?
?
Три шага (SYN, SYN-ACK, ACK) гарантируют, что оба узла готовы к обмену и канал работает в обе стороны. Два шага недостаточно --- отправитель не узнает, получил ли получатель его первое сообщение.

Какие timeout-ы нужно устанавливать для сетевых запросов?
?
Connection timeout (5-10 сек) --- ожидание установки соединения. Read timeout (30 сек для API, 60-120 для файлов) --- ожидание ответа. Никогда не полагаться на дефолтные значения --- они могут быть слишком большими.
...
|-------------|------|-------|
| Следующий шаг | [[network-debugging-basics]] | Научиться систематически диагностировать сетевые проблемы |
| Углубиться | [[network-transport-layer]] | Понять TCP, UDP, QUIC на уровне механик |
| Углубиться | [[network-dns-tls]] | Детально разобрать DNS-резолвинг и TLS handshake |
| Смежная тема | [[network-http-evolution]] | Эволюция HTTP/1.1 -> HTTP/2 -> HTTP/3 |
