...
**[[networking-overview]]** — Обзор сетевых технологий показывает место IP-маршрутизации в общей модели OSI/TCP-IP. IP-адресация и routing — это Layer 3 (сетевой уровень), который соединяет физический/канальный уровни снизу с транспортным уровнем сверху. Без понимания этого контекста сложно осознать, почему маршрутизация необходима и как она отличается от коммутации на L2. Начните с обзора для получения общей картины.

**[[network-transport-layer]]** — TCP, UDP и QUIC работают непосредственно поверх IP, используя IP-адреса для идентификации хостов и добавляя порты для идентификации приложений. Понимание IP-маршрутизации объясняет, как пакеты доставляются между хостами, в то время как транспортный уровень отвечает за надёжность и управление потоком данных. Эти два уровня неразрывно связаны: проблемы маршрутизации проявляются как TCP-таймауты и потери пакетов. Рекомендуется изучать последовательно: сначала IP, затем транспортный уровень.

**[[network-physical-layer]]** — Физический и канальный уровни (Ethernet, WiFi, MAC) обеспечивают доставку кадров внутри одного сегмента сети, тогда как IP-маршрутизация обеспечивает доставку пакетов между сегментами. ARP связывает эти два уровня, преобразуя IP-адреса в MAC-адреса для локальной доставки. Понимание L2 необходимо для диагностики проблем, когда пакеты не покидают локальную сеть. Изучите физический уровень перед IP-маршрутизацией.

**[[network-cloud-modern]]** — Облачные сети (VPC, subnets, route tables) являются виртуализированной версией классической IP-маршрутизации. Все концепции — CIDR, subnetting, NAT, routing tables — применяются в облаке, но реализуются через software-defined networking. Глубокое знание IP-маршрутизации критично для проектирования VPC-архитектур и диагностики проблем с connectivity в облаке.
...
> [!question]- Почему NAT одновременно решает проблему дефицита IPv4-адресов и создаёт трудности для P2P-приложений?
> NAT позволяет множеству устройств выходить в интернет через один публичный IP, решая проблему дефицита адресов. Но NAT ломает сквозное соединение: внешний хост не может инициировать подключение к устройству за NAT, потому что роутер не знает, кому переслать входящий пакет. Для P2P нужны обходные техники: STUN, TURN, hole punching.

> [!question]- Сервер в подсети 10.0.1.0/24 не может достучаться до сервера в 10.0.2.0/24, хотя оба в одной организации. Каковы вероятные причины?
> Серверы в разных подсетях нуждаются в маршрутизаторе. Возможные причины: нет маршрута между подсетями (проверить routing table), default gateway не настроен или указывает не туда, firewall/ACL блокирует трафик между подсетями, VLAN не связаны через L3.

> [!question]- Что произойдёт, если TTL пакета достигнет нуля, и зачем вообще нужен этот механизм?
> Маршрутизатор отбросит пакет и отправит ICMP Time Exceeded обратно отправителю. TTL предотвращает бесконечную циркуляцию пакетов при петлях маршрутизации. Без TTL один ошибочный маршрут мог бы заполнить сеть "бессмертными" пакетами. Traceroute использует этот механизм, последовательно увеличивая TTL.

> [!question]- В чём суть Longest Prefix Match и почему это ключевой принцип маршрутизации?
> Маршрутизатор выбирает маршрут с самым длинным совпадающим префиксом. Например, для пакета к 10.20.30.40 маршрут 10.20.30.0/24 будет выбран вместо 10.0.0.0/8, потому что /24 точнее описывает назначение. Это позволяет иметь общие маршруты и более специфичные исключения одновременно.

---

## Ключевые карточки

Что такое CIDR и что означает /24?
?
CIDR (Classless Inter-Domain Routing) --- нотация для определения размера подсети. /24 означает, что первые 24 бита --- адрес сети, остальные 8 бит --- адреса хостов. /24 = 256 адресов (254 для хостов), /16 = 65536, /8 = 16 миллионов.

Как работает NAT?
?
NAT (Network Address Translation) подменяет внутренний IP и порт на публичный IP и уникальный порт, ведёт таблицу соответствий. Входящие пакеты сопоставляются по порту и перенаправляются нужному внутреннему хосту. Так сотни устройств используют один публичный IP.

Что такое default gateway?
?
Default gateway --- маршрутизатор, которому хост отправляет все пакеты, не предназначенные для локальной сети. Запись 0.0.0.0/0 в таблице маршрутизации --- "для всего неизвестного иди сюда". Без default gateway хост может общаться только в своей подсети.

Чем ARP отличается от DNS?
?
ARP преобразует IP в MAC-адрес в локальной сети (L2). DNS преобразует доменное имя в IP-адрес (L3+). ARP работает через broadcast в пределах подсети, DNS --- через иерархию серверов по всему интернету.

Чем IPv4 отличается от IPv6?
?
IPv4 --- 32 бита (4.3 млрд адресов, исчерпаны). IPv6 --- 128 бит (3.4 x 10^38 адресов). IPv6 не нуждается в NAT, поддерживает автоконфигурацию (SLAAC), встроенный IPsec, более простой заголовок. Пока сосуществуют через dual-stack и туннелирование.

Что делает ICMP и как его используют ping и traceroute?
?
ICMP --- протокол диагностических сообщений. Ping отправляет Echo Request и ждёт Echo Reply для проверки доступности. Traceroute отправляет пакеты с последовательно увеличивающимся TTL: каждый роутер на пути возвращает Time Exceeded, раскрывая маршрут.

Какие IP-адреса зарезервированы для частных сетей?
?
10.0.0.0/8 (16 млн адресов), 172.16.0.0/12 (1 млн), 192.168.0.0/16 (65 тыс.). Эти адреса не маршрутизируются в интернете и могут использоваться в любой локальной сети. Для выхода в интернет требуется NAT.
...
|-------------|------|-------|
| Следующий шаг | [[network-transport-layer]] | TCP, UDP, QUIC --- протоколы поверх IP |
| Углубиться | [[network-cloud-modern]] | VPC, subnets, routing в облаке |
| Смежная тема | [[os-networking]] | Как IP-стек реализован в ядре Linux |
