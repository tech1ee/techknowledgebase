...
[[networking-overview]] — это обзорная карта всего раздела сетевых технологий, в которой транспортный уровень занимает центральное место. Понимание общей архитектуры сетевого стека помогает увидеть, как TCP, UDP и QUIC вписываются в систему «приложение → транспорт → IP → физический уровень». Рекомендуется начать с обзора, затем перейти к транспортному уровню.

[[network-ip-routing]] — IP-уровень обеспечивает маршрутизацию пакетов между хостами, но не гарантирует доставку, порядок или мультиплексирование. Транспортный уровень (TCP/UDP/QUIC) решает именно те проблемы, которые IP оставляет открытыми: порты, надёжность, flow control. Изучение IP-маршрутизации перед транспортным уровнем даёт необходимый фундамент для понимания инкапсуляции и маршрутизации сегментов.

[[network-http-evolution]] — HTTP/3 построен поверх QUIC, что делает понимание QUIC критически важным для работы с современным вебом. Знание различий между TCP и QUIC объясняет, почему HTTP/3 решает проблему Head-of-Line blocking и обеспечивает 0-RTT соединения. Рекомендуется сначала изучить транспортный уровень, затем переходить к эволюции HTTP.

[[os-processes-threads]] — сокеты реализованы как файловые дескрипторы в ОС, а механизмы epoll/kqueue позволяют эффективно обрабатывать тысячи TCP-соединений. Понимание процессов и потоков объясняет, почему блокирующие сокеты требуют по потоку на соединение и как асинхронный I/O решает проблему C10K. Эти знания дополняют друг друга и могут изучаться в любом порядке.

[[os-synchronization]] — механизмы синхронизации (мьютексы, семафоры) напрямую связаны с concurrent обработкой TCP-соединений в серверных приложениях. Когда несколько потоков обрабатывают запросы на общих ресурсах, возникают race conditions, которые требуют тех же примитивов синхронизации, что описаны в этом материале.

[[kotlin-coroutines]] — корутины предоставляют элегантный способ работы с сетевым I/O: suspend-функции позволяют писать асинхронный код линейно, а Dispatchers.IO обеспечивает пул потоков для блокирующих сокетных операций. Понимание транспортного уровня объясняет, почему сетевые вызовы являются blocking и зачем нужна асинхронность.
...
- **Fall K., Stevens W.R. (2011). TCP/IP Illustrated, Volume 1: The Protocols, 2nd Edition.** — Обновлённое издание, включающее IPv6, современные расширения TCP и актуальные реалии сетевого стека. Рекомендуется как основной учебник по транспортному уровню.

- **Kurose J., Ross K. (2021). Computer Networking: A Top-Down Approach, 8th Edition.** — Академический учебник, который объясняет транспортный уровень сверху вниз: от приложения к протоколам. Отличные визуализации congestion control, sliding window и сравнение TCP/UDP.

- **Grigorik I. (2013). High Performance Browser Networking.** — Практическое руководство по оптимизации сетевой производительности, включая детальный разбор QUIC, TCP tuning и 0-RTT. Бесплатно доступна онлайн — идеальна для разработчиков, работающих с HTTP/3.
...
> [!question]- Почему QUIC решает проблему head-of-line blocking, которая есть в TCP, хотя оба обеспечивают надёжную доставку?
> В TCP все потоки мультиплексированы через одно соединение --- потеря одного пакета блокирует все потоки, пока пакет не будет повторно доставлен. QUIC работает поверх UDP и реализует независимые потоки: потеря пакета в одном потоке не влияет на другие. Каждый поток имеет свой контроль последовательности.

> [!question]- Приложение отправляет данные через TCP, send() возвращает успех, но получатель данные не видит. Почему это возможно?
> send() возвращает успех, когда данные помещены в буфер отправки ОС, а не когда получатель их обработал. Данные могут быть потеряны при обрыве соединения после буферизации, или приложение получателя может не успеть вызвать recv(). Для критичных данных нужен application-level ACK.

> [!question]- В каких сценариях UDP предпочтительнее TCP, несмотря на отсутствие гарантий доставки?
> UDP лучше для: видеозвонков (лучше пропустить кадр, чем задержать), онлайн-игр (устаревшие координаты бесполезны), DNS-запросов (быстрый request-response без overhead handshake), IoT-телеметрии (частые обновления, потеря одного некритична). Общий принцип: когда скорость важнее точности и свежие данные вытесняют старые.

> [!question]- Что такое congestion control и почему TCP slow start начинает с маленького окна?
> Congestion control предотвращает перегрузку сети. Slow start начинает с малого окна (обычно 10 MSS) и экспоненциально увеличивает его, потому что сеть --- общий ресурс. Если сразу отправить много данных, можно перегрузить промежуточные маршрутизаторы. Slow start "нащупывает" реальную пропускную способность.
...
?
SYN, SYN-ACK, ACK --- три сообщения гарантируют, что оба узла готовы и канал работает в обе стороны. Случайные sequence numbers защищают от подделки пакетов. Два шага недостаточно: отправитель не знал бы, что получатель его слышит.

Что такое Flow Control и Window Size?
?
Flow Control --- механизм, не позволяющий отправителю переполнить буфер получателя. Window Size --- сколько байт можно отправить без подтверждения. Получатель рекламирует свой window size в каждом ACK, динамически увеличивая или уменьшая.

Чем отличается congestion control от flow control?
?
Flow control защищает получателя от переполнения (window size получателя). Congestion control защищает сеть от перегрузки (congestion window отправителя). Реальное окно = min(receiver window, congestion window).

Что такое QUIC и какие проблемы он решает?
?
QUIC --- транспортный протокол поверх UDP от Google. Решает: HOL blocking (независимые потоки), медленный handshake (0-RTT при повторном подключении), connection migration (сохранение соединения при смене IP/сети), встроенное TLS 1.3.

Что такое эфемерный порт?
?
Эфемерный (ephemeral) порт --- временный порт, назначаемый ОС клиенту для исходящего соединения (диапазон 49152-65535). Сервер слушает на фиксированном порту (443, 80), клиент получает случайный. 5-tuple (src IP, src port, dst IP, dst port, protocol) уникально идентифицирует соединение.

Когда TCP retransmission ухудшает ситуацию?
?
При высоком packet loss TCP aggressively retransmits, заполняя и без того перегруженную сеть повторными пакетами. Exponential backoff замедляет retransmission, но добавляет latency. В таких условиях QUIC лучше: потеря в одном потоке не блокирует другие.
...
|-------------|------|-------|
| Следующий шаг | [[network-http-evolution]] | HTTP/1.1, HTTP/2, HTTP/3 поверх TCP и QUIC |
| Углубиться | [[network-performance-optimization]] | TCP tuning, буферы, congestion control в деталях |
| Смежная тема | [[os-networking]] | Реализация сокетов и TCP-стека в ядре Linux |
