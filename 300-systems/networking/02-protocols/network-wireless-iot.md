...
> **IoT-протоколы** — беспроводные протоколы, оптимизированные для устройств с ограниченными ресурсами (батарея, CPU, RAM). Ключевой trade-off: **дальность × скорость × энергопотребление** — улучшение одного параметра ухудшает остальные.

### Классификация по дальности и скорости

| Категория | Дальность | Скорость | Примеры |
|-----------|----------|----------|---------|
| **PAN** (Personal Area Network) | 1-100 м | 1-3 Mbps | Bluetooth, Zigbee, Thread |
| **LAN** (Local Area Network) | 10-100 м | 1-600 Mbps | Wi-Fi, Matter |
| **LPWAN** (Low Power Wide Area) | 1-15 км | 0.3-50 Kbps | LoRa, Sigfox, NB-IoT |
| **NFC** (Near Field) | < 10 см | 424 Kbps | NFC (payments, tags) |

### Топологии сетей

| Топология | Протокол | Плюсы | Минусы |
|-----------|---------|-------|--------|
| **Star** | BLE, NFC | Простая, низкая latency | Single point of failure |
| **Mesh** | Zigbee, Thread, BLE Mesh | Self-healing, масштабируемость | Сложная маршрутизация, latency |
| **Star-of-stars** | LoRaWAN | Дальность + gateway aggregation | Зависимость от gateways |

### Matter (2022, CSA): унификация IoT

> **Matter** — application-layer протокол (over Thread/Wi-Fi/Ethernet), унифицирующий экосистемы Apple HomeKit, Google Home, Amazon Alexa. Основан на IPv6 (Thread), IP-native.

> **См. также**: [[network-bluetooth]] — Bluetooth/BLE, [[network-cellular]] — NB-IoT/LTE-M
...
### Теоретические основы
- IEEE 802.15.4 (2003). *Low-Rate Wireless Personal Area Networks*. — Физический и MAC уровень для Zigbee, Thread
- Sornin N. et al. (2015). *LoRaWAN Specification v1.0*. — Формальная спецификация LPWAN-протокола
- Shelby Z. et al. (2014). *The Constrained Application Protocol (CoAP)* (RFC 7252). — Lightweight REST для IoT
