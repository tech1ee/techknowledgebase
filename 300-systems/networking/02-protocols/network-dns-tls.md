...
**[[networking-overview]]** — Обзор сетевых технологий даёт карту всего раздела и показывает, где DNS и TLS находятся в общей архитектуре интернета. DNS работает поверх UDP/TCP на прикладном уровне, а TLS располагается между транспортным и прикладным уровнями, обеспечивая шифрование. Без понимания общей картины сложно осознать, почему DNS-резолюция происходит до TLS-handshake и как они взаимодействуют. Начните с обзора, затем углубляйтесь в DNS и TLS.

**[[network-transport-layer]]** — TCP и UDP — это транспортные протоколы, поверх которых работают DNS (традиционно UDP:53, но также TCP) и TLS (исключительно TCP, а в QUIC — UDP). Понимание TCP-handshake критично для осознания того, почему TLS-handshake добавляет дополнительные round-trip и как TLS 1.3 минимизирует эту задержку. Знание транспортного уровня является обязательным prerequisite для глубокого понимания DNS и TLS.

**[[network-http-evolution]]** — HTTPS — это буквально HTTP поверх TLS, поэтому эволюция HTTP напрямую связана с развитием TLS. HTTP/2 требует TLS на практике (хотя не в спецификации), а HTTP/3 (QUIC) интегрирует TLS 1.3 непосредственно в транспортный протокол, объединяя handshake. Изучение HTTP-эволюции без понимания TLS невозможно, и наоборот. Рекомендуется изучать параллельно.

**[[web-security-owasp]]** — TLS является фундаментальным элементом веб-безопасности, защищая данные в транзите от перехвата и модификации. OWASP Top 10 включает уязвимости, связанные с неправильной конфигурацией TLS: слабые cipher suites, отсутствие HSTS, смешанный контент. Понимание TLS из этой заметки даёт техническую базу для реализации рекомендаций OWASP.

**[[authentication-authorization]]** — TLS client certificates (mTLS) используются для аутентификации клиентов на уровне транспорта, особенно в service-to-service коммуникации и Zero Trust архитектурах. Понимание PKI и сертификатов из раздела TLS необходимо для реализации mTLS-аутентификации. Рекомендуется сначала освоить основы TLS, затем изучать аутентификацию на основе сертификатов.
...
- **Grigorik (2013).** *High Performance Browser Networking.* — практическое руководство по оптимизации DNS-резолюции и TLS-соединений в браузерах, включая dns-prefetch, OCSP stapling и session resumption.
...
> [!question]- Почему "It's always DNS" стало мемом среди инженеров? Назовите три причины, почему DNS так часто оказывается виноват.
> DNS --- первый шаг каждого запроса, без него ничего не работает. DNS кэшируется на множестве уровней (браузер, ОС, роутер, ISP) с разными TTL, создавая рассинхронизацию. ISP может игнорировать TTL и кэшировать записи дольше. Разные резолверы могут возвращать разные ответы одновременно.

> [!question]- Вы сменили IP-адрес сервера в DNS-записи с TTL=60 сек, но через 2 часа часть пользователей всё ещё идёт на старый IP. Почему?
> ISP DNS-резолверы часто игнорируют низкий TTL и кэшируют записи значительно дольше (до 24 часов). Роутеры пользователей также могут иметь свой DNS-кэш. Для критичных миграций нужно заранее снизить TTL за 24-48 часов и использовать DNS-based failover или load balancer перед серверами.

> [!question]- Чем TLS 1.3 лучше TLS 1.2 с точки зрения производительности и безопасности?
> Производительность: TLS 1.3 --- 1-RTT handshake (вместо 2-RTT), поддержка 0-RTT при повторном подключении. Безопасность: убраны устаревшие шифры (RC4, 3DES, CBC), обязательный forward secrecy (ECDHE), шифрование сертификата сервера. Меньше round-trips = быстрее, меньше шифров = безопаснее.

> [!question]- Что такое Certificate Transparency и почему это критически важно для безопасности?
> CT --- публичный лог всех выданных сертификатов. Позволяет владельцам доменов обнаружить мошеннические сертификаты, выданные скомпрометированным CA. Без CT злоумышленник с доступом к CA мог бы выпустить валидный сертификат для чужого домена незаметно. Браузеры требуют SCT (Signed Certificate Timestamp) для доверия.

---

## Ключевые карточки

Какие основные типы DNS-записей и для чего они?
?
A --- IPv4-адрес, AAAA --- IPv6-адрес, CNAME --- alias на другое имя, MX --- почтовый сервер, NS --- авторитативный DNS-сервер, TXT --- произвольные данные (SPF, DKIM, verification), SRV --- сервис с портом и приоритетом.

Как работает DNS-резолвинг?
?
Кэш браузера -> кэш ОС -> роутер -> ISP DNS -> рекурсивный резолвер -> Root DNS -> TLD DNS (.com) -> Authoritative DNS. Каждый уровень кэширует ответ на время TTL. Результат --- IP-адрес для запрошенного домена.

Что такое DNSSEC?
?
DNSSEC добавляет криптографические подписи к DNS-записям, позволяя проверить, что ответ не подменён. Цепочка доверия от root zone до домена. Защищает от DNS spoofing и cache poisoning. Не шифрует запросы --- для этого нужен DoH/DoT.

Что такое DoH и DoT?
?
DNS over HTTPS (DoH, порт 443) и DNS over TLS (DoT, порт 853) --- шифрование DNS-запросов. Без них ISP видит все запрашиваемые домены. DoH сложнее блокировать (выглядит как обычный HTTPS). DoT проще фильтровать (отдельный порт).

Что даёт TLS и из чего состоит handshake?
?
TLS обеспечивает три вещи: конфиденциальность (шифрование), целостность (данные не изменены), аутентификацию (сервер подлинный). Handshake: ClientHello (шифры, SNI) -> ServerHello + Certificate -> Key Exchange -> Finished. TLS 1.3 = 1 RTT.

Что такое цепочка доверия сертификатов?
?
Root CA (предустановлен в ОС/браузере) подписывает Intermediate CA, который подписывает сертификат сайта. Браузер проверяет всю цепочку до доверенного root. Если любое звено скомпрометировано, вся цепочка недоверенна. Let's Encrypt --- бесплатный CA с автоматизацией.
...
|-------------|------|-------|
| Следующий шаг | [[network-http-evolution]] | Как TLS интегрируется в HTTP/2 и HTTP/3 |
| Углубиться | [[network-security-fundamentals]] | Zero Trust, DDoS, firewall на основе TLS |
| Смежная тема | [[authentication-authorization]] | OAuth и JWT поверх HTTPS |
