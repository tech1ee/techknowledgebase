...
> **GraphQL** — декларативный язык запросов для API, основанный на типизированной схеме (type system), где клиент специфицирует точную форму ответа, а сервер гарантирует соответствие через статическую валидацию.

### Формальная модель

GraphQL формально определяется через три компонента:
1. **Type System** — описание возможных данных (схема)
2. **Query Language** — синтаксис запросов (selection sets)
3. **Execution Engine** — алгоритм разрешения запроса (resolver chain)

Запрос GraphQL — это **проекция на типизированный граф данных**. В отличие от REST (ресурсы с фиксированной структурой) и SQL (произвольные join-ы), GraphQL позволяет клиенту выбрать подграф определённой формы.

### Историческая хронология

| Год | Событие | Значение |
|-----|---------|----------|
| 1984 | Birrell & Nelson — RPC | Формализация удалённых вызовов |
| 2000 | Fielding — REST | Архитектурный стиль на основе ресурсов |
| 2012 | Facebook создаёт GraphQL | Решение проблемы N+1 roundtrips в мобильном приложении |
| 2015 | Open source релиз GraphQL | Спецификация + reference implementation (graphql-js) |
| 2016 | GitHub переходит на GraphQL | Первый крупный публичный API на GraphQL |
| 2020 | Apollo Federation v1 | Распределённая схема для микросервисов |
| 2022 | Apollo Federation v2 | @shareable, @override — гибкая граница владения |

### Ключевые теоретические концепции

| Концепция | Формальное определение | Практическое следствие |
|-----------|----------------------|----------------------|
| **Schema-first** | API определяется типами до реализации (Design by Contract, Meyer 1988) | Клиент и сервер могут развиваться независимо |
| **Декларативность** | Клиент описывает ЧТО нужно, не КАК получить | Оптимизация выполнения — ответственность сервера |
| **Introspection** | Схема — метаданные, доступные через сам API | Автоматическая генерация документации и tooling |
| **Type safety** | Каждое поле имеет тип, валидируемый до выполнения | Ошибки обнаруживаются на этапе компиляции запроса |

### Фундаментальная проблема: over-fetching и under-fetching

REST возвращает фиксированные структуры → два антипаттерна:
- **Over-fetching**: GET /users возвращает 50 полей, клиенту нужно 3
- **Under-fetching**: для экрана нужно 5 последовательных запросов к разным endpoint-ам

GraphQL решает оба: клиент запрашивает ровно нужные поля в одном запросе. Цена — **сложность выполнения** (query complexity analysis, N+1 problem через DataLoader).

> **См. также**: [[api-design]] — обзор парадигм API, [[api-rest-deep-dive]] — REST в деталях, [[api-grpc-deep-dive]] — альтернатива для межсервисной коммуникации
...
### Теоретические основы
- **Lee B., Voss N., Schäfer M. et al. (2015-2024). GraphQL Specification.** — формальное определение type system, execution model, validation rules
- **Birrell A., Nelson B. (1984). Implementing Remote Procedure Calls. ACM TOCS.** — формализация RPC, на фоне которой GraphQL предлагает декларативную альтернативу
- **Hartig O., Pérez J. (2018). Semantics and Complexity of GraphQL. WWW Conference.** — первый формальный анализ семантики и вычислительной сложности GraphQL
