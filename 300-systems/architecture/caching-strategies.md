...
> [!question]- Black Friday: ваш интернет-магазин, 10,000 RPS. Самый популярный товар (iPhone) — 50% всех запросов. TTL истекает — 5000 запросов одновременно идут в БД. Какие 3 механизма защиты вы комбинируете и почему?
> 1. **Distributed Lock:** Только один запрос идёт в БД, остальные ждут результат (SET key value NX PX 30000). 2. **TTL Jitter:** Добавить ±10-20% рандомизации к TTL всех ключей, чтобы они не истекали одновременно. 3. **L1 Local Cache (Caffeine/LRU) + L2 Redis:** Горячие ключи хранить в памяти процесса (0ms, без сети), Redis как L2. Дополнительно: Stale-While-Revalidate — отдавать stale данные мгновенно, обновлять в фоне. Для iPhone это приемлемо: цена не менялась за последние 5 секунд.

> [!question]- Пользователь обновил email в профиле, но в ленте друзей его имя показывается со старым email ещё час. Какая проблема кэширования здесь и какие 2 подхода к инвалидации вы бы скомбинировали?
> Проблема: **stale data** — кэш ленты содержит денормализованные данные пользователя, при обновлении профиля инвалидируется только user:123, но не feed:* где встроены данные пользователя. Комбинация: (1) **Event-based инвалидация** с картой зависимостей: при обновлении user инвалидировать user:*, feed:*, team:*:members. (2) **TTL как safety net:** даже если забыли зависимость, через TTL данные обновятся. Паттерн CacheInvalidator с регистрацией зависимостей entity→keys решает системно. Pub/Sub для уведомления всех инстансов в распределённой системе.

> [!question]- Ваш API показывает Cache-Control: no-cache для профиля пользователя. Коллега говорит: "no-cache значит не кэшировать". Он прав? Объясните разницу между no-cache, no-store и private.
> Коллега НЕ прав. **no-cache**: можно кэшировать, но ОБЯЗАТЕЛЬНО проверять свежесть перед использованием (If-None-Match / ETag). Браузер спросит "изменилось?" перед каждым использованием. **no-store**: НИКОГДА не сохранять — для sensitive данных (пароли, токены, баланс). **private**: кэшировать ТОЛЬКО в браузере пользователя, НЕ на CDN/proxy — для персональных данных (профиль, настройки). Для профиля пользователя правильно: `private, max-age=300` (5 мин, только в браузере) или `private, no-cache` (кэш с обязательной ревалидацией через ETag).

> [!question]- Write-Behind кэширование для лайков Instagram — данные пишутся в Redis и асинхронно в БД. Какой риск это создаёт и для каких типов данных Write-Behind категорически недопустим?
> Риск: если Redis упадёт до записи в БД — **данные ПОТЕРЯНЫ навсегда**. Нет автоматического recovery. Для лайков это приемлемо (потерять несколько лайков не критично). **Категорически недопустим для:** финансовых транзакций (деньги), заказов (e-commerce), медицинских записей, аутентификации (сессии). Для денег используется Write-Through (синхронная запись в оба хранилища) или даже БД-first с инвалидацией кэша. Правило: если потеря данных = юридические/финансовые последствия — Write-Through или Cache-Aside.

---

## Ключевые карточки

4 стратегии кэширования — когда какую?
?
Cache-Aside (90% случаев): read-heavy, app управляет кэшем и БД. Write-Through: консистентность критична (банки). Write-Behind: некритичные данные, высокая частота записи (лайки, метрики). Read-Through: упрощение кода, кэш сам ходит в БД.

Что такое Cache Stampede (Thundering Herd) и 3 решения?
?
Популярный ключ истекает → тысячи запросов одновременно в БД → БД падает. Решения: (1) Distributed Lock — один обновляет, остальные ждут. (2) Stale-While-Revalidate — отдаём старое, обновляем в фоне. (3) TTL Jitter — рандомизация ±10% чтобы ключи не истекали одновременно.

Cache Penetration — что это и как защититься?
?
Запросы несуществующих данных всегда проходят в БД (кэш не помогает). Решения: (1) Кэшировать NULL с коротким TTL (5 мин). (2) Bloom Filter — вероятностная структура, говорит "точно нет" за O(1) без обращения к сети.

Redis vs Memcached — ключевые различия (2025)?
?
Redis: 10+ типов данных, persistence (RDB+AOF), Pub/Sub, Lua scripting, single-threaded + I/O threads. Лицензия AGPLv3 с v8.0. Memcached: только strings, нет persistence, full multi-threaded, BSD лицензия. Альтернативы Redis: Valkey (Linux Foundation), KeyDB, Dragonfly.

L1 + L2 кэширование — зачем два уровня?
?
L1 = in-process cache (Caffeine/LRU), 0ms, без сети, ограничен памятью процесса. L2 = Redis, 1-2ms, shared между инстансами. L1 решает Hot Key проблему: 10K RPS на один Redis shard → bottleneck. С L1 большинство запросов не покидают процесс.

ETag + If-None-Match — как экономят трафик?
?
Первый запрос: сервер отдаёт данные + ETag (хеш контента). Второй запрос: клиент отправляет If-None-Match с ETag. Если данные не изменились → 304 Not Modified (тело НЕ передаётся). Экономия: не передаём полный ответ, только заголовки.

Cache-Control: immutable — когда использовать?
?
Для файлов с хешем в имени: app.abc123.js. Файл НИКОГДА не изменится — при изменении кода генерируется НОВЫЙ файл с НОВЫМ хешем. Можно кэшировать на 1 год. ETag не нужен. Браузер автоматически загрузит новый файл по новому URL.

Почему при обновлении данных лучше DELETE из кэша, а не SET?
?
DELETE проще (не нужно дублировать логику формирования кэша) и безопаснее (меньше шансов на inconsistency между кэшем и БД). Следующий GET подгрузит свежие данные из БД. SET при обновлении создаёт риск race condition между параллельными записями.
...
|-------------|------|-------|
| Следующий шаг | [[performance-optimization]] | Кэширование — один из инструментов оптимизации |
| Углубиться | [[architecture-distributed-systems]] | Consistency модели влияют на стратегию кэширования |
| HTTP кэширование | [[network-http-evolution]] | Детали Cache-Control, ETag, CDN |
| Память | [[jvm-memory-model]] | Как in-process кэш (Caffeine/Guava) работает с GC |
| Мониторинг | [[observability]] | Метрики кэша: hit rate, latency, eviction |
| Смежная тема | [[architecture-rate-limiting]] | Rate limiting часто реализуется через Redis |
