...
> **Dependency Injection (DI)** — реализация принципа Inversion of Control (IoC), при которой зависимости объекта передаются ему извне (через конструктор, метод или свойство), вместо создания внутри.

Формально: если класс `A` зависит от интерфейса `B`, то `A` не создаёт реализацию `B`, а получает её как параметр: `A(b: B)`.

### Историческая атрибуция

| Концепция | Автор | Год | Вклад |
|-----------|-------|-----|-------|
| **Inversion of Control** | Johnson & Foote | 1988 | "Designing Reusable Classes" — IoC как принцип фреймворков |
| **Dependency Inversion Principle** | Martin, R.C. (Uncle Bob) | 1996 | "D" в SOLID: зависеть от абстракций, не от конкретных классов |
| **IoC Containers** | Apache Avalon | 1999 | Первый IoC-контейнер для Java |
| **Spring Framework** | Johnson, R. | 2003 | DI-контейнер для Java, ставший стандартом |
| **"DI" term coined** | Fowler, M. | 2004 | "Inversion of Control Containers and the DI Pattern" — назвал паттерн |
| **Dagger 2** | Google | 2015 | Compile-time DI для Android (без reflection) |
| **Koin** | Arnaud Giuliani | 2017 | Kotlin-first DI: DSL, no code generation |

### Три формы DI (Fowler, 2004)

| Форма | Механизм | Пример (Kotlin) |
|-------|----------|-----------------|
| **Constructor Injection** | Зависимость передаётся в конструктор | `class Service(val repo: Repository)` |
| **Setter Injection** | Зависимость устанавливается через setter | `service.repo = RealRepo()` |
| **Interface Injection** | Зависимость передаётся через интерфейс | `fun inject(repo: Repository)` |

> **Рекомендация:** Constructor Injection — предпочтительная форма (immutable, explicit, easy to test). Setter Injection — для опциональных зависимостей.

### DI и SOLID: формальная связь

DI — техническая реализация двух SOLID-принципов:
...
- **Open/Closed Principle (OCP):** DI позволяет расширять поведение (подставляя другую реализацию) без модификации кода.

### Compile-time vs Runtime DI

| Свойство | Compile-time (Dagger 2) | Runtime (Koin, Spring) |
|----------|------------------------|----------------------|
| **Когда resolve** | При компиляции | При запуске |
| **Ошибки** | Compile error | Runtime crash |
| **Performance** | Нулевой runtime overhead | Reflection/lookup overhead |
| **Code generation** | Да (annotation processor) | Нет (Koin) / Да (Spring AOT) |
...
### Теоретические основы
- Johnson, R. & Foote, B. (1988). "Designing Reusable Classes" — JOOP; первая формализация IoC
- Martin, R.C. (1996). "The Dependency Inversion Principle" — оригинальная статья о DIP
- [Fowler, M. (2004). "Inversion of Control Containers and the DI Pattern"](https://martinfowler.com/articles/injection.html) — каноническое определение DI

### Практические руководства
- Seemann, M. & van Deursen, S. *Dependency Injection Principles, Practices, and Patterns* — лучшая книга по DI
- Martin, R.C. *Clean Architecture* — DI в контексте архитектуры
- [Seemann, M. (2010). "Service Locator is an Anti-Pattern"](https://blog.ploeh.dk/2010/02/03/ServiceLocatorisanAnti-Pattern/) — критика Service Locator
