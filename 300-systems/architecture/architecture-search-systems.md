...
> [!question]- Почему в Elasticsearch фильтры (filter context) работают быстрее, чем must, хотя оба отсеивают документы?
> Filter context не вычисляет relevance score для каждого документа — он работает бинарно (да/нет). Это позволяет Elasticsearch кэшировать результаты фильтров в виде bitset. При повторном запросе с тем же фильтром (например, `category: "footwear"`) ES не пересчитывает результат, а берёт из кэша. Must же каждый раз пересчитывает BM25 score. Поэтому правило: всё, что не влияет на порядок результатов (точные совпадения, диапазоны), — выносим в filter.

> [!question]- Интернет-магазин с 50 млн товаров получает жалобы: поиск "кроссовки для бега" не находит товары с описанием "беговая обувь". Как бы вы решили эту проблему, если BM25 и анализаторы уже настроены?
> Это ограничение keyword-based поиска (BM25): "кроссовки" и "обувь" — разные токены. Решения по нарастающей: (1) добавить synonym filter в анализатор (`кроссовки => обувь, кеды`); (2) внедрить vector search (semantic search) через dense_vector поле с embeddings от языковой модели — вектора "кроссовки для бега" и "беговая обувь" будут близки в пространстве; (3) гибридный поиск (BM25 + KNN), где BM25 ловит точные совпадения, а vector search — семантическую близость. Вариант 3 даёт лучший результат на практике.

> [!question]- Как связаны inverted index в Elasticsearch и индексы в RAG-системах (Retrieval-Augmented Generation)? В чём принципиальное различие подходов к поиску?
> И inverted index, и vector store в RAG решают одну задачу — быстрый поиск релевантных документов. Inverted index ищет по точным токенам (лексический поиск): "кошка" найдёт только документы с этим словом. Vector store в RAG хранит embeddings и ищет по семантической близости: "кошка" найдёт и "домашний питомец". Современные системы комбинируют оба подхода: Elasticsearch поддерживает dense_vector для KNN-поиска, а RAG-пайплайны часто используют hybrid retrieval (BM25 + embeddings). Ключевое различие: BM25 — статистический алгоритм на основе частоты слов, vector search — поиск в пространстве значений, созданном нейросетью.

> [!question]- Команда запустила Elasticsearch-кластер с 30 шардами для индекса размером 15 ГБ. Поиск работает медленнее ожидаемого. Оцените архитектурное решение и предложите исправление.
> Over-sharding: 15 ГБ / 30 шардов = 0.5 ГБ на шард, что далеко ниже рекомендуемых 10-50 ГБ. Каждый шард — это отдельный Lucene-индекс с накладными расходами на файловые дескрипторы, память, thread pool. При scatter-gather запрос уходит на все 30 шардов, coordinating node собирает 30 ответов — overhead координации превышает выигрыш от параллелизма. Решение: пересоздать индекс с 1-2 primary shards (15 ГБ укладывается в один шард). Количество шардов нельзя изменить после создания — потребуется reindex.

---

## Ключевые карточки

Inverted index — что это и какую задачу решает?
?
Структура данных, которая сопоставляет каждое уникальное слово (term) со списком документов, содержащих это слово. Позволяет искать за O(1) вместо полного сканирования O(n*m), как делает SQL LIKE.

Какие три фактора учитывает BM25 при расчёте релевантности?
?
Term Frequency (частота слова в документе, с насыщением), Inverse Document Frequency (редкие слова важнее частых) и Document Length normalization (короткие документы с тем же количеством совпадений релевантнее длинных).

Чем отличаются типы полей text и keyword в Elasticsearch?
?
Text проходит через анализатор (tokenization, lowercase, stemming) и используется для полнотекстового поиска. Keyword хранится as-is без анализа и используется для точных совпадений, фильтров, сортировки и агрегаций.

Что такое scatter-gather и как работает поиск в кластере Elasticsearch?
?
Coordinating node рассылает запрос на все шарды (scatter), каждый шард возвращает свой локальный top-N, затем coordinating node объединяет результаты и возвращает финальный top-N клиенту (gather).

Почему filter context быстрее must в bool-запросе Elasticsearch?
?
Filter не вычисляет relevance score (работает бинарно: да/нет), что позволяет кэшировать результаты в виде bitset. При повторных запросах с тем же фильтром результат берётся из кэша без пересчёта.

В чём преимущество hybrid search (BM25 + vector) над каждым подходом по отдельности?
?
BM25 точно находит документы с конкретными ключевыми словами, а vector search находит семантически похожие документы без совпадения слов. Комбинация покрывает оба сценария: "кроссовки Nike" (точное совпадение) и "недорогой ноутбук" → "бюджетный лэптоп" (семантика).

Какой рекомендуемый размер шарда в Elasticsearch и почему?
?
10-50 ГБ на шард. Слишком маленькие шарды (<1 ГБ) создают overhead координации и расходуют ресурсы на каждый Lucene-индекс. Слишком большие (>100 ГБ) замедляют recovery и перебалансировку кластера.

Какие три этапа проходит текст в Text Analysis Pipeline перед попаданием в inverted index?
?
Character filters (очистка HTML, замена символов), tokenizer (разбиение на токены/слова), token filters (lowercase, удаление стоп-слов, стемминг, синонимы). Тот же pipeline применяется к поисковому запросу для согласованности.
...
|-------------|------|-------|
| Следующий шаг | [[caching-strategies]] | Кэширование результатов поиска критично для production-нагрузок |
| Глубже в тему | [[databases-replication-sharding]] | Понять шардирование и репликацию на уровне баз данных — те же принципы в ES |
| Смежная тема | [[embeddings-complete-guide]] | Разобраться в embeddings, которые стоят за vector search в Elasticsearch |
| Кросс-домен | [[rag-advanced-techniques]] | RAG использует те же принципы retrieval (BM25 + vector) для AI-приложений |
| Кросс-домен | [[vector-databases-guide]] | Специализированные vector DB (Pinecone, Weaviate) vs встроенный KNN в ES |
| Практика | [[architecture-distributed-systems]] | Elasticsearch — распределённая система; понять CAP, consensus, partition tolerance |
| Обзор | [[architecture-overview]] | Вернуться к общей карте архитектурных тем |
