...
> [!question]- Компания разрабатывает публичный API для платёжного сервиса. Архитектор предлагает хранить сессии пользователей на сервере (server-side sessions) для "удобства". Почему это нарушает REST и к каким конкретным проблемам приведёт при масштабировании с 1 до 50 серверов?
> Хранение сессий на сервере нарушает constraint Statelessness. При масштабировании возникают три проблемы: (1) sticky sessions — балансировщик должен направлять клиента на конкретный сервер, где хранится его сессия; (2) при падении сервера все сессии теряются; (3) добавление нового сервера не даёт линейного прироста, потому что нужно перебалансировать сессии. Правильный подход — JWT или OAuth-токены, которые путешествуют с каждым запросом. Любой из 50 серверов обработает любой запрос без знания о предыдущих.

> [!question]- API интернет-магазина возвращает заказ: `GET /orders/42 → {"id": 42, "status": "pending", "total": 1500}`. Клиентское приложение зашивает URL `DELETE /orders/42` для отмены и `POST /orders/42/payment` для оплаты. Оцените этот подход с точки зрения Richardson Maturity Model. Какой уровень? Что потеряно по сравнению с Level 3?
> Это Level 2 — используются правильные HTTP-методы и отдельные URI для ресурсов, но URL действий зашиты в клиенте. Потеряно: (1) сервер не может динамически управлять доступными действиями — если заказ уже оплачен, клиент всё равно попытается вызвать `payment`; (2) при изменении URL на сервере все клиенты сломаются; (3) бизнес-логика «что можно делать с заказом» дублируется между клиентом и сервером. На Level 3 (HATEOAS) сервер возвращал бы `_links` с доступными действиями, и ссылка `payment` исчезала бы после оплаты.

> [!question]- Разработчик утверждает, что PATCH и PUT — это одно и то же, просто PATCH «удобнее». Приведите сценарий, где замена PUT на PATCH приведёт к багу, связанному с идемпотентностью.
> PUT идемпотентен всегда: полная замена ресурса. Повторный PUT с теми же данными = тот же результат. PATCH может быть не идемпотентным. Сценарий: PATCH с операцией инкремента `{"operation": "increment", "field": "login_count", "value": 1}`. Первый вызов: login_count = 6. Сетевой таймаут, клиент повторяет. Второй вызов: login_count = 7. Третий retry: login_count = 8. С PUT такого не произойдёт — `{"login_count": 6}` при повторе запишет 6 снова. Вывод: PATCH требует дополнительных мер (Idempotency-Key), если операция не является простой заменой поля.

> [!question]- Микросервис обрабатывает платежи. После успешного списания $100 ответ не дошёл до клиента (504 Gateway Timeout). Клиент повторяет POST-запрос. Как Idempotency-Key предотвращает двойное списание? Почему нельзя кэшировать ответы с кодом 500?
> При первом запросе сервер сохраняет Idempotency-Key в Redis вместе с результатом (success, $100 списано). При повторном запросе с тем же ключом сервер находит сохранённый результат и возвращает его без повторного списания. Ответы 500 нельзя кэшировать, потому что ошибка может быть временной: сервер мог не завершить операцию (транзакция откатилась, база была недоступна). Если закэшировать 500, повторный запрос вернёт ошибку, хотя операция могла бы успешно выполниться. Кэшируются только 2xx (успех) и 409 (конфликт — запрос ещё обрабатывается).

---

## Ключевые карточки

REST — это протокол или архитектурный стиль? В чём разница?
?
REST — архитектурный стиль, не протокол. Протокол (HTTP, SOAP) определяет конкретный формат сообщений и правила обмена. Архитектурный стиль — набор ограничений (constraints), которые можно реализовать поверх разных протоколов. REST описывает *принципы*, а не байты на проводе.

Перечисли 6 REST constraints Филдинга.
?
1) Client-Server — разделение ответственности. 2) Statelessness — каждый запрос самодостаточен. 3) Cacheability — ответы помечаются как кэшируемые. 4) Uniform Interface — единый интерфейс для всех ресурсов. 5) Layered System — клиент не знает о промежуточных слоях. 6) Code-on-Demand (опциональный) — сервер может отправить исполняемый код.

Что такое HATEOAS и какую проблему он решает?
?
HATEOAS (Hypermedia As The Engine Of Application State) — принцип, при котором сервер в каждом ответе возвращает ссылки на доступные действия. Решает проблему жёсткой связности: клиенту не нужно зашивать URL, он следует ссылкам из ответа. Если заказ оплачен — ссылка «оплатить» исчезает, появляется «вернуть».

Какие HTTP-методы идемпотентны, а какие нет? Почему это важно?
?
Идемпотентны: GET, HEAD, PUT, DELETE — повторный вызов даёт тот же результат. Не идемпотентны: POST (создаёт новый ресурс при каждом вызове), PATCH (зависит от реализации). Это критично для retry-логики в ненадёжных сетях: идемпотентные запросы можно безопасно повторять при таймауте.

Назови 4 уровня Richardson Maturity Model.
?
Level 0 (Swamp of POX) — один URL, один метод (POST), как SOAP. Level 1 (Resources) — разные URI, но один HTTP-метод. Level 2 (HTTP Verbs) — правильные методы (GET/POST/PUT/DELETE) и статус-коды. Level 3 (HATEOAS) — гиперссылки в ответах управляют навигацией. 95% production API работают на Level 2.

Чем Strong ETag отличается от Weak ETag?
?
Strong ETag (`"abc123"`) гарантирует побайтовую идентичность — два ответа с одинаковым Strong ETag полностью идентичны. Weak ETag (`W/"abc123"`) гарантирует лишь семантическую эквивалентность — содержимое может отличаться пробелами или порядком полей, но смысл тот же.

Как работает Idempotency-Key для POST-запросов?
?
Клиент генерирует уникальный UUID и отправляет в заголовке `Idempotency-Key`. Сервер проверяет ключ в хранилище (Redis): если ключ новый — обрабатывает запрос и сохраняет результат; если ключ уже есть — возвращает сохранённый ответ без повторной обработки. TTL обычно 24 часа. Кэшируются только 2xx и 409, но не 500.

Почему constraint Statelessness критичен для горизонтального масштабирования?
?
Без состояния на сервере любой сервер может обработать любой запрос — не нужны sticky sessions или синхронизация сессий между серверами. Добавление нового сервера = просто запуск ещё одного инстанса. Балансировка — round-robin. При падении сервера клиент просто идёт на другой.
...
|-------------|------|-------|
| Следующий шаг | [[api-graphql-deep-dive]] | Альтернатива REST для сложных запросов, решает over/under-fetching |
| Углубление | [[api-grpc-deep-dive]] | Бинарный протокол для межсервисной коммуникации, в 5-7 раз быстрее REST |
| Современные паттерны | [[api-modern-patterns]] | tRPC, Webhooks, API Gateway, BFF — что выходит за рамки REST |
| Протокол под капотом | [[network-http-evolution]] | HTTP/1.1 → HTTP/2 → HTTP/3: как эволюция протокола влияет на API |
| Смежная тема | [[caching-strategies]] | Redis, CDN, HTTP-кэширование — один из 6 REST constraints подробно |
| Безопасность | [[security-api-protection]] | Rate limiting, API keys, input validation — защита REST API |
| Обзор | [[architecture-overview]] | Карта всех архитектурных тем: от API до распределённых систем |
