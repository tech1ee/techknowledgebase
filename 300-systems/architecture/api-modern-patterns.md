...
> [!question]- Почему tRPC не подходит для публичного API, даже если весь бэкенд написан на TypeScript?
> Потому что потребители публичного API — это внешние команды и сервисы, которые могут использовать любой язык (Python, Go, Java, Swift). tRPC требует TypeScript-клиент и доступ к типам роутера через import. Для public API нужен language-agnostic контракт — REST + OpenAPI или GraphQL, — чтобы любой разработчик мог сгенерировать клиент на своём языке.

> [!question]- Webhook-endpoint вашего сервиса принимает POST-запросы от Stripe, но не проверяет HMAC-подпись. Какие конкретные атаки становятся возможными и почему `timingSafeEqual` важен при сравнении подписей?
> Без проверки HMAC любой злоумышленник может отправить поддельный POST на ваш webhook-URL — например, фейковое событие `payment.succeeded`, чтобы получить товар без оплаты. Также возможна replay-атака: перехваченный легитимный запрос отправляется повторно. `timingSafeEqual` нужен потому, что обычное сравнение строк (`===`) прерывается на первом несовпавшем символе, и по времени ответа атакующий может побайтово подобрать правильную подпись (timing attack).

> [!question]- Команда разрабатывает мобильное приложение (iOS/Android) и веб-панель администратора для e-commerce. Мобильному клиенту нужны агрегированные данные одного экрана за один запрос, а веб-панели — полные таблицы с фильтрацией и пагинацией. Оцените два варианта: (а) единый REST API для обоих клиентов, (б) BFF-паттерн с отдельным бэкендом для каждого клиента.
> Вариант (а) приведёт к «компромиссному API»: мобильный клиент получает лишние данные и делает лишние запросы, а веб-панель ограничена форматом, заточенным под мобилку. Вариант (б) — BFF — позволяет Mobile BFF агрегировать 3-5 вызовов микросервисов в один компактный ответ с нужными полями, а Web BFF отдавать полные данные с поддержкой pagination и фильтров. Минус BFF — дополнительная кодовая база и инфраструктура, но для e-commerce с разными клиентами это оправдано.

> [!question]- В архитектуре с 8 микросервисами команда рассматривает два варианта: клиенты обращаются к сервисам напрямую или через API Gateway. При каких условиях прямые вызовы допустимы и когда Gateway становится необходимостью?
> Прямые вызовы допустимы при 1-2 сервисах, одном типе клиента и внутреннем использовании (без внешних потребителей). API Gateway становится необходим, когда: (1) несколько клиентов (mobile, web, IoT) — иначе auth, rate limiting и CORS дублируются в каждом сервисе; (2) нужна централизованная аутентификация — без gateway каждый сервис должен сам проверять токены; (3) нужен request aggregation — клиент не должен знать внутреннюю топологию из 8 адресов; (4) нужны cross-cutting concerns: логирование, мониторинг, SSL termination — всё в одной точке вместо 8.

---

## Ключевые карточки

tRPC обеспечивает type safety между сервером и клиентом. Какой механизм это делает возможным без кодогенерации?
?
TypeScript inference: сервер экспортирует тип роутера (`type AppRouter = typeof appRouter`), клиент импортирует только этот тип (`import type`). TypeScript выводит типы input/output каждой процедуры — 0 байт в bundle, ошибки ловятся в compile-time.

Что такое HMAC-SHA256 в контексте webhooks и от чего он защищает?
?
HMAC-SHA256 — это подпись payload с использованием общего секрета (signing secret). Provider вычисляет `HMAC-SHA256(secret, timestamp + "." + payload)` и отправляет подпись в заголовке. Получатель повторяет вычисление и сравнивает. Защищает от подделки webhook-запросов и replay-атак (timestamp проверяется на свежесть).

Чем API Gateway отличается от reverse proxy?
?
Reverse proxy (Nginx) выполняет маршрутизацию трафика и балансировку нагрузки. API Gateway добавляет поверх этого: аутентификацию/авторизацию, rate limiting, request/response transformation, аналитику, developer portal и кэширование.

В чём главная идея паттерна BFF (Backend for Frontend)?
?
Создание отдельного бэкенд-слоя для каждого типа клиента. Mobile BFF агрегирует данные и отдаёт компактные ответы, Web BFF — полные данные с пагинацией. Бизнес-логика остаётся в микросервисах, BFF только адаптирует формат.

Почему при обработке webhooks нужна идемпотентность?
?
Потому что webhooks используют retry-механизм: если получатель не ответил 200 OK, provider отправляет событие повторно (Stripe — до 8 раз за 3 дня). Без проверки `event_id` одно событие может быть обработано несколько раз — например, двойное списание платежа.

Чем North-South трафик отличается от East-West и какие инструменты используются для каждого?
?
North-South — трафик между внешними клиентами и сервисами (обрабатывает API Gateway: Kong, AWS API Gateway). East-West — трафик между микросервисами внутри кластера (обрабатывает Service Mesh: Istio, Linkerd). В продакшене используют оба вместе.

Когда tRPC — правильный выбор, а когда нет?
?
tRPC подходит для TypeScript-монорепо, одной fullstack-команды, внутренних инструментов и admin-панелей. Не подходит для: публичных API (нужен REST + OpenAPI), мобильных клиентов на Swift/Kotlin, multi-language бэкенда и сложных графов данных (лучше GraphQL).

Почему webhook-обработчик должен отвечать быстро (< 5 секунд) и что делать с долгой обработкой?
?
Если обработчик не ответит 200 OK в течение таймаута, provider считает доставку неудачной и включает retry — это создаёт дубликаты событий и нагрузку. Правильный подход: сразу ответить 200, а само событие поставить в очередь (Redis, SQS, RabbitMQ) для асинхронной обработки.
...
| Основа | [[api-design]] | Обзор REST, GraphQL, gRPC — когда что выбрать |
| Углубление | [[api-rest-deep-dive]] | REST constraints, HATEOAS, идемпотентность подробно |
| Углубление | [[api-graphql-deep-dive]] | Schema, federation, subscriptions — альтернатива tRPC для сложных графов |
| Следующий шаг | [[event-driven-architecture]] | Event-driven как альтернатива sync API: Kafka, RabbitMQ, Saga |
| Контекст | [[microservices-vs-monolith]] | Когда нужны API Gateway и BFF, а когда монолит проще |
| Кросс-домен | [[network-http-evolution]] | HTTP/2, HTTP/3 — транспорт, на котором работают все эти паттерны |
