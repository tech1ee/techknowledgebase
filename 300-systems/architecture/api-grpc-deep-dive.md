...
> **RPC (Remote Procedure Call)** — парадигма межпроцессного взаимодействия, при которой вызов удалённой процедуры синтаксически неотличим от локального вызова. Формализована Birrell & Nelson (1984) в работе "Implementing Remote Procedure Calls".

### Формальная модель RPC

Birrell & Nelson определили 5 компонентов RPC:
1. **Caller** (client stub) — маршалинг аргументов
2. **Communication module** — передача по сети
3. **Callee** (server stub) — демаршалинг и вызов
4. **Binding** — обнаружение сервера
5. **Interface Definition** — контракт между сторонами

gRPC реализует эту модель с современными оптимизациями: Protocol Buffers как IDL + сериализация, HTTP/2 как транспорт, code generation для stubs.

### Историческая хронология

| Год | Система | Вклад |
|-----|---------|-------|
| 1976 | RFC 707 | Первое описание RPC-концепции |
| 1984 | Birrell & Nelson | Формальная модель RPC (Xerox PARC) |
| 1988 | Sun RPC / ONC | Первая широко используемая реализация |
| 1991 | CORBA | IDL + объектная модель, language-agnostic |
| 1998 | XML-RPC → SOAP | RPC через HTTP + XML |
| 2001 | Google Stubby | Внутренняя RPC-система Google (10B calls/sec) |
| 2008 | Protocol Buffers | Open source (использовался в Google с 2001) |
| 2015 | gRPC 1.0 | Open source переработка Stubby |
| 2023 | Connect Protocol | Buf: gRPC + gRPC-Web + JSON в одном фреймворке |

### Ключевые теоретические концепции

| Концепция | Формальное определение | gRPC реализация |
|-----------|----------------------|-----------------|
| **IDL (Interface Definition Language)** | Формальный язык описания контракта, независимый от языка реализации | Protocol Buffers (.proto файлы) |
| **Сериализация** | Преобразование структуры данных в последовательность байт (маршалинг) | Protobuf: TLV-encoding, varint, zigzag |
| **Мультиплексирование** | Множество логических потоков в одном соединении | HTTP/2 streams |
| **Contract-first** | API определяется контрактом до реализации | .proto → codegen → stubs |

### Четыре паттерна коммуникации

gRPC формализует 4 типа RPC, покрывающих все паттерны взаимодействия:

```
Unary:             Client ──req──▸ Server ──resp──▸ Client
Server streaming:  Client ──req──▸ Server ══resp══▸ Client (N сообщений)
Client streaming:  Client ══req══▸ Server ──resp──▸ Client (N сообщений)
Bidirectional:     Client ══req══▸ Server ══resp══▸ Client (N × M)
```

### Восемь заблуждений распределённых систем (Deutsch 1994)

gRPC проектировался с учётом **Fallacies of Distributed Computing**: сеть ненадёжна, латентность ненулевая, пропускная способность конечна. Поэтому: deadline propagation, retry policies, circuit breakers, load balancing — встроены в framework.

> **См. также**: [[api-design]] — обзор парадигм API, [[api-rest-deep-dive]] — REST как альтернатива, [[ffi-foreign-function-interface]] — FFI как локальный аналог RPC
...
### Теоретические основы
- **Birrell A., Nelson B. (1984). Implementing Remote Procedure Calls. ACM TOCS.** — фундаментальная работа по RPC: 5-компонентная модель, stub generation, binding
- **Slee M., Agarwal A., Kwiatkowski M. (2007). Thrift: Scalable Cross-Language Services Implementation. Facebook.** — предшественник gRPC, IDL + binary serialization
- **RFC 707 (1976). A High-Level Framework for Network-Based Resource Sharing.** — первое описание RPC-концепции
- **Deutsch P. (1994). The Eight Fallacies of Distributed Computing.** — фундаментальные ограничения, которые gRPC учитывает в дизайне
...
> [!question]- Почему Protobuf даёт выигрыш в 70-90% по размеру payload по сравнению с JSON, но при этом проигрывает в удобстве отладки? Какие архитектурные trade-offs это создаёт при выборе формата для публичного API vs внутренней межсервисной связи?
> Protobuf использует бинарное кодирование (varint, TLV), где передаются только номера полей и значения — без имён, без пробелов, без кавычек. JSON передаёт имена полей текстом, плюс синтаксический overhead (скобки, двоеточия). Для публичных API это проблема: разработчики не могут прочитать payload в DevTools, нужен protoc или grpcurl. Для внутренней связи — нечитаемость не проблема, зато экономия трафика и CPU критична при тысячах RPC/секунду. Поэтому на практике используют оба: REST/JSON снаружи, gRPC/Protobuf внутри.

> [!question]- Мобильное приложение должно получать обновления цен в реальном времени. Сравните три подхода: REST polling, WebSocket и gRPC Server Streaming. Какой выберете для условий нестабильной мобильной сети и почему?
> REST polling создаёт избыточный трафик (заголовки каждого запроса, пустые ответы), что критично на мобильной сети. WebSocket работает, но использует отдельное TCP-соединение и не имеет встроенного сжатия заголовков. gRPC Server Streaming оптимален: HTTP/2 multiplexing через одно TCP-соединение, HPACK-сжатие заголовков, Protobuf для компактных сообщений (до 90% экономии), встроенные deadline для контроля зависших соединений. На нестабильной сети gRPC также выигрывает за счёт меньшего объёма передаваемых данных при переподключениях.

> [!question]- В Kubernetes кластере с 3 pod'ами gRPC-сервиса один pod обрабатывает 95% запросов, два других простаивают. Что происходит и как это исправить?
> Стандартный Kubernetes Service (ClusterIP) выполняет L4-балансировку — распределяет TCP-соединения, а не запросы. Поскольку gRPC использует HTTP/2 с мультиплексированием, все RPC-вызовы идут через одно долгоживущее соединение к одному pod'у. Решения: (1) Headless Service + client-side LB — клиент получает IP всех pod'ов через DNS и балансирует сам; (2) Service Mesh (Istio/Linkerd) — sidecar-прокси рядом с каждым pod'ом выполняет L7-балансировку на уровне запросов; (3) L7-прокси (Envoy) — парсит HTTP/2 фреймы и распределяет отдельные RPC-вызовы.

> [!question]- Команда развернула Istio service mesh и все gRPC-сервисы работают через Envoy sidecar. Как interceptors на уровне приложения и Envoy-фильтры дополняют друг друга? Может ли service mesh полностью заменить application-level interceptors?
> Service mesh (Envoy sidecar) обеспечивает инфраструктурные concerns: mTLS-шифрование, L7-балансировку, retry-политики, circuit breaker, distributed tracing (заголовки), метрики (latency, error rate). Это прозрачно для приложения и language-agnostic. Но application-level interceptors незаменимы для бизнес-логики: валидация авторизации с проверкой ролей, обогащение metadata бизнес-контекстом, кастомное логирование с business-метриками, трансформация запросов. Полная замена невозможна — mesh отвечает за «как доставить», interceptors — за «что проверить/добавить на уровне домена».

---

## Ключевые карточки

gRPC использует HTTP/1.1 или HTTP/2?
?
Только HTTP/2. Это обязательное требование, так как gRPC использует multiplexing, binary framing и trailers из HTTP/2.

Какие 4 типа RPC поддерживает gRPC?
?
Unary (запрос-ответ), Server Streaming (один запрос → поток ответов), Client Streaming (поток запросов → один ответ), Bidirectional Streaming (потоки в обе стороны независимо).

Чем deadline отличается от timeout в gRPC?
?
Timeout — относительное время ожидания («жди 5 секунд»). Deadline — абсолютное время («ответь до 14:30:05»). Deadline автоматически propagates по цепочке микросервисов, уменьшаясь с каждым hop'ом.

Почему L4 load balancer не работает для gRPC?
?
gRPC использует долгоживущие HTTP/2 соединения с мультиплексированием. L4 балансирует TCP-соединения, а не запросы. Все RPC-вызовы уходят через одно соединение на один сервер. Нужен L7 балансировщик или service mesh.

Что такое wire type в Protobuf и зачем он нужен?
?
Wire type определяет способ кодирования значения поля: 0 — varint (int, bool, enum), 1 — 64-bit fixed, 2 — length-delimited (string, bytes, вложенные message), 5 — 32-bit fixed. Парсер использует wire type для определения длины поля без знания схемы.

Почему нельзя переиспользовать номер удалённого поля в .proto?
?
Номер поля — это ключ в бинарном формате. Старый клиент с предыдущей версией .proto будет парсить новое поле как старое, получая повреждённые данные. Удалённые номера нужно помечать как reserved.

Какие ограничения имеет gRPC-Web по сравнению с нативным gRPC?
?
gRPC-Web поддерживает только Unary и Server Streaming. Client Streaming и Bidirectional Streaming не работают. Требуется прокси (Envoy) между браузером и сервером. В text mode используется Base64 с overhead ~33%.

Чем Connect Protocol от Buf отличается от gRPC-Web?
?
Connect поддерживает три протокола на одном сервере: нативный gRPC, gRPC-Web и простой HTTP/JSON. Не требует Envoy-прокси. Позволяет обращаться к gRPC-сервисам через curl с обычным JSON.
...
|-------------|------|-------|
| Следующий шаг | [[api-modern-patterns]] | tRPC, Webhooks, API Gateway, BFF — современные паттерны поверх gRPC/REST |
| Альтернатива | [[api-rest-deep-dive]] | REST constraints Филдинга, идемпотентность — для сравнения подходов |
| Альтернатива | [[api-graphql-deep-dive]] | GraphQL schema, federation, caching — другая модель запросов |
| Вглубь | [[architecture-distributed-systems]] | Паттерны распределённых систем, где gRPC выступает транспортом |
| Вглубь | [[network-http-evolution]] | HTTP/2 multiplexing, HPACK, QUIC — транспорт, на котором построен gRPC |
| Смежная тема | [[event-driven-architecture]] | Когда streaming недостаточно: events, message brokers, CQRS |
| Обзор | [[architecture-overview]] | Карта всех архитектурных тем и связей между ними |
