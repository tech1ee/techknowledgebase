...
> [!question]- Команда разрабатывает банковское Android-приложение. Как бы вы определили, какой MAS Testing Profile (L1, L2, R, P) нужен, и какие категории MASVS будут критичными в первую очередь?
> Для банковского приложения необходим полный профиль L1+L2+R+P. Выбор определяется через threat modeling: приложение обрабатывает финансовые транзакции (L2 + R для защиты от fraud), хранит PII и финансовые данные (P для privacy), содержит критичную бизнес-логику (R для anti-tampering). Критичные категории в первую очередь: MASVS-AUTH (биометрия привязана к криптографии, step-up auth для транзакций), MASVS-STORAGE (шифрование tokens/credentials через Android Keystore), MASVS-NETWORK (TLS 1.2+ с certificate pinning). Затем MASVS-RESILIENCE (root detection, anti-debug, integrity checks) и MASVS-PRIVACY (consent, data minimization).

> [!question]- Почему UI-based биометрическая аутентификация (без привязки к криптографической операции) считается небезопасной с точки зрения MASVS-AUTH-1?
> Потому что UI-based биометрия лишь показывает диалог и возвращает boolean-результат — «пользователь подтвердил» или «нет». Атакующий с Frida может перехватить вызов callback и подменить результат на true, полностью обойдя проверку. Криптографическая биометрия привязывает аутентификацию к реальной операции с ключом в TEE/Secure Enclave: без успешной биометрии Cipher/SecKey не выполнит decrypt/sign. Даже перехватив callback, атакующий не получит доступ к данным, потому что без аппаратного подтверждения криптографическая операция невозможна. Именно поэтому MASVS-AUTH-1 требует CryptoObject при работе с BiometricPrompt (Android) и kSecUseAuthenticationContext при доступе к Keychain (iOS).

> [!question]- В CI/CD пайплайне настроен MobSF для статического анализа APK. Какие категории MASVS можно проверить автоматически на этом этапе, а какие требуют ручного тестирования с Frida/Objection и почему?
> Автоматически (static analysis) покрываются: MASVS-CODE (debuggable flag, signing, ProGuard), частично MASVS-STORAGE (hardcoded secrets, незашифрованные SharedPreferences в коде), MASVS-CRYPTO (обнаружение слабых алгоритмов DES/MD5/ECB в исходниках), MASVS-NETWORK (cleartext в манифесте, HTTP URLs), MASVS-PLATFORM (exported components, WebView настройки в коде). Ручное тестирование необходимо для: MASVS-AUTH (реальный bypass биометрии через Frida), MASVS-RESILIENCE (попытки обхода root detection, anti-debug — нужна runtime среда), MASVS-STORAGE runtime (данные в памяти, clipboard, background screenshots), MASVS-NETWORK runtime (реальный bypass pinning через mitmproxy). Static analysis находит потенциальные проблемы в коде, а dynamic analysis подтверждает их эксплуатируемость в рабочем приложении.

> [!question]- Приложение реализует certificate pinning через OkHttp CertificatePinner. Проанализируйте, от каких сценариев атак это защищает, а от каких — нет, и какие дополнительные меры из MASVS-NETWORK рекомендуется применить.
> Certificate pinning через OkHttp защищает от: перехвата трафика на скомпрометированном Wi-Fi с подменным сертификатом, корпоративного прокси с собственным CA, теоретической компрометации удостоверяющего центра. Не защищает от: атакующего с root-доступом и Frida (bypass через hook CertificatePinner.check), скомпрометированного серверного endpoint, malware на устройстве с доступом к памяти процесса, ошибок в самом бэкенде. Дополнительные меры по MASVS-NETWORK: обязательно включить backup pins для ротации сертификатов, настроить Network Security Config с запретом cleartext и user CA в production, ограничить cipher suites до TLS 1.2+/1.3, рассмотреть mutual TLS для критичных endpoints, мониторить Certificate Transparency логи для обнаружения неавторизованных сертификатов.

---

## Ключевые карточки

MASVS 2.0 — сколько категорий и какие? Что изменилось по сравнению с 1.x?
?
8 категорий: STORAGE, CRYPTO, AUTH, NETWORK, PLATFORM, CODE, RESILIENCE, PRIVACY. Главные изменения: убраны уровни L1/L2 в пользу MAS Testing Profiles, добавлена категория PRIVACY, все требования стали атомарными (одно требование = один тест).

Чем отличаются MASVS и MASTG? Какова их роль в MAS?
?
MASVS определяет ЧТО проверять (стандарт с атомарными требованиями безопасности). MASTG описывает КАК проверять (руководство с конкретными тест-кейсами, инструментами и скриптами). Вместе они образуют MAS — комплексный фреймворк верификации безопасности мобильных приложений.

Что такое MAS Testing Profiles и какие профили существуют?
?
Профили заменили уровни L1/L2 из MASVS 1.x. Четыре профиля: L1 (базовое тестирование для всех приложений), L2 (глубокое тестирование для high-risk), R (тестирование защиты от реверс-инжиниринга), P (тестирование приватности). Комбинируются по результатам threat modeling.

Почему MASVS-AUTH-1 требует криптографической привязки биометрии?
?
UI-based биометрия возвращает лишь boolean, который можно подменить через Frida. Криптографическая биометрия привязывает аутентификацию к операции с ключом в TEE/Secure Enclave — без реальной биометрии Cipher/SecKey не выполнит операцию, что делает bypass бесполезным.

Какие инструменты составляют основу среды тестирования MASTG?
?
Frida — динамическая инструментация (hook функций, перехват вызовов). Objection — высокоуровневая обёртка над Frida (disable pinning, root bypass). MobSF — автоматизированный статический и динамический анализ. Jadx — декомпиляция APK. Burp Suite/mitmproxy — перехват трафика. APKTool — манипуляция APK.

В чём суть MASVS-STORAGE-1 и какие платформенные механизмы его реализуют?
?
Требование: приложение хранит sensitive данные в зашифрованном или secure storage. Android: EncryptedSharedPreferences с MasterKey (AES256_GCM), Android Keystore для ключей. iOS: Keychain Services с kSecAttrAccessibleWhenUnlockedThisDeviceOnly. Запрещено хранить токены, пароли, PII в plaintext SharedPreferences/NSUserDefaults.

Почему root/jailbreak detection не является надёжным барьером?
?
Любой root detection обходится: Magisk Hide/Shamiko (Android), специализированные bypass (iOS), Frida-скрипты, кастомные ядра. Root detection — это deterrent, повышающий стоимость атаки, но не барьер. Правильный подход: defense in depth с server-side валидацией, device attestation и risk-based authentication.

Как MASVS-PRIVACY-4 ограничивает использование идентификаторов?
?
Запрещены hardware-идентификаторы (IMEI, MAC-адрес). Advertising ID только с согласия пользователя. Требуются resettable и scoped идентификаторы. Цель — минимизировать отслеживание и обеспечить privacy by design в соответствии с GDPR/CCPA.

Какие автоматические MASVS-проверки можно встроить в CI/CD пайплайн?
?
MobSF static analysis (hardcoded secrets, weak crypto, manifest проблемы), dependency vulnerability scan (OWASP Dependency-Check), Android Lint security rules, trufflehog для поиска секретов, генерация MASVS compliance отчёта. Автоматизация покрывает MASVS-CODE, частично STORAGE, CRYPTO, NETWORK, PLATFORM — но ручное тестирование (L2+) остаётся необходимым.
...
|-------------|------|-------|
| OWASP Mobile Top 10 | [[mobile-security-owasp]] | Изучить ландшафт мобильных угроз, которые систематизирует MASVS |
| Защита приложений | [[mobile-app-protection]] | Освоить практическую реализацию требований MASVS-RESILIENCE и MASVS-STORAGE |
| Веб-безопасность OWASP | [[web-security-owasp]] | Покрыть серверную часть мобильных приложений — API и бэкенд-уязвимости |
| Основы безопасности | [[security-fundamentals]] | Укрепить теоретический фундамент (CIA Triad, Defense in Depth) под всеми категориями MASVS |
| Моделирование угроз | [[threat-modeling]] | Научиться выбирать правильный MAS Testing Profile через risk assessment |
