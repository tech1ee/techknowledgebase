...
> **Определение.** Аутентификация на основе токенов — это механизм сохранения состояния идентификации поверх протокола без состояния (HTTP), при котором клиент предъявляет серверу криптографически верифицируемый артефакт вместо повторной передачи учётных данных при каждом запросе.

### Историческая хронология

| Год | Событие | Значение |
|-----|---------|----------|
| 1994 | Lou Montulli (Netscape) изобретает cookies | Первый механизм сохранения состояния в HTTP |
| 1997 | RFC 2109 — первая спецификация cookies | Формализация «HTTP State Management Mechanism» |
| 2000 | Появление sticky sessions в load balancers | Попытка масштабирования stateful-подхода |
| 2011 | RFC 6265 (Barth A.) — современная спецификация cookies | Определение атрибутов Secure, HttpOnly, Domain, Path |
| 2012 | RFC 6749 — OAuth 2.0 Authorization Framework | Token-based авторизация для API |
| 2015 | RFC 7519 (Jones M., Bradley J., Sakimura N.) — JSON Web Token | Стандартизация self-contained токенов |
| 2020 | RFC 6265bis — атрибут SameSite по умолчанию | Защита от CSRF на уровне протокола |
| 2023 | RFC 9449 — DPoP (Demonstration of Proof-of-Possession) | Привязка bearer-токенов к криптографическому ключу клиента |

### Структура JWT (RFC 7519)

JWT состоит из трёх частей, разделённых точками, каждая — Base64URL-кодированная строка:
- **Header** — метаданные: алгоритм подписи (`alg`) и тип токена (`typ`)
- **Payload** — claims (утверждения): registered (`iss`, `sub`, `aud`, `exp`, `iat`), public и private
- **Signature** — `HMAC-SHA256(base64url(header) + "." + base64url(payload), secret)` для HS256, или RSA/ECDSA-подпись для асимметричных алгоритмов

> **Ключевой принцип.** Stateful (sessions) vs stateless (JWT) — это не выбор «лучше/хуже», а фундаментальный архитектурный trade-off: sessions перекладывают хранение состояния на сервер (централизация, мгновенный отзыв), JWT — на клиента (децентрализация, масштабируемость, но невозможность мгновенного отзыва без дополнительной инфраструктуры).

### Атрибуты безопасности cookies

| Атрибут | Назначение | Защита от |
|---------|-----------|-----------|
| `Secure` | Передача только по HTTPS | Перехват по сети |
| `HttpOnly` | Недоступность из JavaScript | XSS-кража |
| `SameSite=Strict` | Отправка только с того же сайта | CSRF-атаки |
| `SameSite=Lax` | Отправка при top-level навигации | CSRF с POST |
| `Domain` / `Path` | Ограничение области видимости | Утечка на поддомены |

**См. также:** [[security-cryptography-fundamentals]] — криптографические примитивы (HMAC, RSA, ECDSA), лежащие в основе подписей JWT; [[auth-oauth2-oidc]] — использование JWT в OAuth 2.0 потоках.
...
- **Fett, D., Küsters, R. & Schmitz, G. (2016).** "A Comprehensive Formal Security Analysis of OAuth 2.0". ACM CCS. -- Формальный анализ безопасности OAuth 2.0, включая token-модели. Выявил уязвимости в спецификации.
