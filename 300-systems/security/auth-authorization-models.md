...
> **Определение.** Авторизация — формальный процесс определения, имеет ли аутентифицированный субъект (subject) право выполнить запрошенное действие (action) над конкретным ресурсом (resource) в заданном контексте (environment). В отличие от аутентификации («кто ты?»), авторизация отвечает на вопрос «что тебе разрешено?».

### Эволюция моделей авторизации

| Год | Модель | Авторы / Организация | Ключевая идея |
|-----|--------|---------------------|---------------|
| 1971 | Access Matrix | Lampson B. | Матрица «субъект × ресурс → права»; теоретический фундамент всех моделей |
| 1973 | Bell-LaPadula (MAC) | Bell D., LaPadula L. | «No read up, no write down» — модель конфиденциальности для DoD |
| 1977 | Biba Integrity Model | Biba K. | Инверсия Bell-LaPadula: «No read down, no write up» — модель целостности |
| 1983 | DAC (TCSEC) | DoD, Orange Book | Дискреционный контроль: владелец решает, кому давать доступ |
| 1992 | RBAC (формализация) | Ferraiolo D., Kuhn R. (NIST) | Роли как абстракция между пользователями и permissions |
| 1996 | RBAC0–RBAC3 | Sandhu R., Coyne E., Feinstein H., Youman C. | Иерархии ролей, ограничения, Separation of Duties |
| 2005 | ABAC | NIST SP 800-162 (Jin X., Krishnan R., Sandhu R.) | Атрибуты субъекта, ресурса, действия и окружения |
| 2019 | ReBAC (Zanzibar) | Pang R. et al. (Google) | Граф отношений: авторизация через обход tuple-графа |

### Сравнение формальных моделей

| Критерий | DAC | MAC | RBAC | ABAC | ReBAC |
|----------|-----|-----|------|------|-------|
| Кто принимает решение | Владелец ресурса | Центральная политика | Администратор ролей | Policy engine | Граф отношений |
| Гранулярность | Ресурс | Уровень секретности | Роль → permission | Атрибуты (4 категории) | Tuple (subject, relation, object) |
| Масштабируемость | Низкая | Средняя | Средняя (role explosion) | Высокая | Очень высокая |
| Сложность отладки | Низкая | Низкая | Низкая | Высокая | Средняя |
| Пример системы | UNIX chmod | SELinux, MLS | AWS IAM Roles | XACML, Cedar | Google Zanzibar, SpiceDB |

> **Формальная связь.** Все модели авторизации — это различные способы представления одной и той же абстракции: **матрицы доступа** (Lampson, 1971). ACL — столбец матрицы (для каждого ресурса). Capability — строка матрицы (для каждого субъекта). RBAC группирует строки по ролям. ABAC вычисляет ячейки динамически. ReBAC заменяет матрицу графом.

**См. также:** [[authentication-authorization]] — разграничение аутентификации и авторизации; [[web-security-owasp]] — Broken Access Control как уязвимость #1 в OWASP Top 10.
...
- **Bell, D. & LaPadula, L. (1973). "Secure Computer Systems: Mathematical Foundations"** -- модель конфиденциальности для Министерства обороны США, определившая MAC. Историческое значение для понимания военных моделей доступа.
...
- **OpenFGA Documentation (openfga.dev).** -- Практическое руководство по реализации ReBAC на основе модели Zanzibar. Содержит примеры моделирования отношений, DSL для описания типов и тесты авторизации.

- **Cedar Policy Language (docs.cedarpolicy.com).** -- Язык политик от AWS для выражения ABAC/RBAC-правил. Формально верифицируемый, с инструментами анализа и simulation.
