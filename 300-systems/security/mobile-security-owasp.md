...
> [!question]- Почему OWASP в 2024 году объединил Code Tampering и Reverse Engineering в одну категорию (M7), а не оставил их отдельными?
> Потому что обе угрозы направлены на бинарник приложения и связаны единым вектором атаки: атакующий сначала декомпилирует приложение (reverse engineering), а затем модифицирует его (code tampering). Защитные меры тоже пересекаются — obfuscation, integrity checks, anti-debugging одновременно усложняют и анализ, и модификацию. Разделение на две категории создавало ложное впечатление, что это независимые проблемы, тогда как на практике это этапы одной цепочки атаки.

> [!question]- Вы проектируете Android-приложение для мобильного банка. Архитектура использует MVVM с Repository pattern. В каких слоях архитектуры должны быть реализованы меры защиты от категорий M1, M3 и M9 — и почему нельзя ограничиться только одним слоем?
> M1 (Credential Usage) — слой Data/Repository: EncryptedSharedPreferences и Android Keystore для хранения токенов, CredentialProvider за интерфейсом репозитория. M3 (Auth/AuthZ) — слой Network + ViewModel: interceptor в OkHttp для токенов, проверка авторизации на сервере, BiometricPrompt с CryptoObject во ViewModel для критичных операций. M9 (Data Storage) — слой Data: шифрование БД через SQLCipher, EncryptedFile для файлов. Нельзя ограничиться одним слоем, потому что Defense in Depth требует защиты на каждом уровне — компрометация одного слоя не должна раскрывать все данные. Кроме того, client-side проверки без серверной авторизации бесполезны (M3).

> [!question]- Разработчик настроил certificate pinning через OkHttp CertificatePinner и считает, что приложение полностью защищено от MITM-атак. Примените знания из M5 и M7 — какие дополнительные атаки возможны и какие меры нужны?
> Certificate pinning через OkHttp можно обойти на rooted устройстве с помощью Frida или Xposed — они перехватывают вызовы CertificatePinner и подставляют любой сертификат. Поэтому нужна комплексная защита: (1) root/jailbreak detection из M7 для обнаружения скомпрометированного окружения; (2) integrity checks — проверка подписи APK для защиты от repackaging; (3) Network Security Config как дополнительный слой к programmatic pinning; (4) backup pin для ротации сертификатов; (5) RASP для обнаружения runtime hooks. Один pinning без защиты бинарника (M7) — неполноценная защита.

> [!question]- Приложение использует стороннюю analytics SDK, которая запрашивает ACCESS_FINE_LOCATION и READ_CONTACTS. К каким категориям OWASP Mobile Top 10 2024 это относится и какие конкретные действия необходимо предпринять?
> Это затрагивает сразу три категории: M2 (Supply Chain) — SDK не прошёл security review и запрашивает excessive permissions; M6 (Privacy Controls) — SDK может собирать PII без consent пользователя; M8 (Misconfiguration) — избыточные permissions расширяют attack surface. Действия: (1) провести security review SDK — проверить сетевой трафик, запрашиваемые permissions, privacy policy; (2) изолировать SDK за internal API, чтобы ограничить доступ к данным; (3) проверить, действительно ли нужны эти permissions для аналитики (скорее всего, нет); (4) добавить SDK в SBOM для отслеживания уязвимостей; (5) рассмотреть альтернативную SDK с минимальными permissions.

---

## Ключевые карточки

M1 Improper Credential Usage — какие 3 основных правила хранения credentials в мобильном приложении?
?
(1) Никогда не хардкодить credentials в коде — извлекаются через reverse engineering. (2) Использовать платформенное шифрованное хранилище: EncryptedSharedPreferences / Keychain. (3) Передавать credentials в HTTP headers, а не в URL query parameters.

Почему OWASP добавил Supply Chain Security (M2) в Mobile Top 10 2024?
?
Более 90% мобильных приложений используют сторонние SDK, атаки через supply chain стали массовыми (SolarWinds, Log4Shell). Каждый SDK — потенциальный вектор: устаревшие версии с CVE, вредоносный код, избыточные permissions.

Чем отличается аутентификация от авторизации в контексте M3?
?
Аутентификация (AuthN) проверяет личность пользователя (кто ты?). Авторизация (AuthZ) проверяет права доступа (что тебе разрешено?). OWASP объединил их в M3, потому что аутентификация без авторизации недостаточна — например, IDOR позволяет аутентифицированному пользователю получить чужие данные.

Что такое IDOR и почему client-side авторизация не защищает от него?
?
IDOR (Insecure Direct Object Reference) — атака, при которой пользователь меняет ID в запросе (например, /orders/123 на /orders/124) и получает чужие данные. Client-side проверки бесполезны, потому что атакующий может отправлять запросы напрямую к API, минуя клиентский код.

Почему BiometricPrompt без CryptoObject небезопасен на Android?
?
Без CryptoObject биометрическая аутентификация — просто callback (true/false), который можно подменить на rooted устройстве через Frida. С CryptoObject биометрия привязана к криптографической операции в Android Keystore — без реальной биометрии невозможно получить ключ.

Какие 3 основных вектора утечки PII из мобильного приложения (M6)?
?
(1) Логи — PII в logcat/console и crash reports. (2) Clipboard — скопированные пароли доступны другим приложениям. (3) Backups — sensitive данные включаются в device backup и доступны через adb backup или iCloud.

Почему obfuscation (M7) не является достаточной защитой от reverse engineering?
?
Obfuscation усложняет анализ, но не делает его невозможным. Инструменты вроде jadx, Ghidra и Frida позволяют декомпилировать и анализировать даже обфусцированный код. Obfuscation — один слой защиты (defense in depth), его нужно дополнять integrity checks, anti-debugging и серверным хранением secrets.

Какие алгоритмы шифрования рекомендует OWASP (M10) и какие запрещает?
?
Рекомендуется: AES-256-GCM (symmetric), RSA-2048+/ECDSA (asymmetric), SHA-256+ (hashing), Argon2/bcrypt (password hashing), SecureRandom. Запрещено: DES, 3DES, RC4, MD5, SHA-1, ECB mode, hardcoded ключи, java.util.Random.

Почему Insecure Data Storage понизилось с M2 (2016) до M9 (2024)?
?
Платформы значительно улучшили механизмы защиты данных: Android добавил EncryptedSharedPreferences и Scoped Storage, iOS усилил Data Protection API. Базовый уровень защиты вырос, хотя проблема остаётся при неправильном использовании API.

Что такое SBOM и зачем он нужен для защиты от M2 (Supply Chain)?
?
SBOM (Software Bill of Materials) — полный список всех компонентов и зависимостей приложения с их версиями. Позволяет быстро определить, затронуто ли приложение при обнаружении новой CVE в библиотеке, и автоматизировать сканирование на уязвимости.
...
|-------------|------|-------|
| Стандарт верификации | [[mobile-security-masvs]] | Систематический чеклист для проверки каждой категории Mobile Top 10 |
| Практическая защита | [[mobile-app-protection]] | Root detection, RASP, attestation — конкретные реализации контрмер |
| Android-безопасность | [[android-permissions-security]] | Permissions model, exported components, secure storage на Android |
| Аутентификация | [[auth-passwordless-mfa]] | MFA и passwordless — глубже в тему M3 (Insecure Authentication) |
| Криптография | [[security-cryptography-fundamentals]] | Фундамент для понимания M10 (Insufficient Cryptography) |
| TLS и сетевая защита | [[security-https-tls]] | TLS handshake, certificate pinning — глубже в тему M5 |
| Веб-угрозы | [[web-security-owasp]] | OWASP Web Top 10 — связь с мобильными API и бэкендами |
