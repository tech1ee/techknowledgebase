...
> [!question]- Почему для верификации webhook используется HMAC-подпись, а не Bearer Token?
> Bearer token предполагает, что получатель предоставляет токен отправителю. Но в webhook-модели инициатор запроса -- отправитель (Stripe, GitHub), а не получатель. Получатель не контролирует, когда придёт запрос, и не может "выдать" токен отправителю в рамках стандартного OAuth-flow. HMAC решает эту проблему через shared secret: обе стороны знают секрет заранее, отправитель подписывает тело запроса, получатель проверяет подпись. Секрет при этом не передаётся по сети -- передаётся только производная (подпись).

> [!question]- Компания переносит монолит на микросервисную архитектуру в Kubernetes. Сервис заказов вызывает сервис платежей. Какую комбинацию паттернов аутентификации вы бы применили и почему?
> Оптимальная комбинация: Service Mesh (Istio/Linkerd) для автоматического mTLS на транспортном уровне + JWT с audience restriction на уровне приложения. mTLS обеспечивает идентификацию сервисов без изменения кода (sidecar proxy обрабатывает прозрачно). JWT с `aud: payment-service` предотвращает confused deputy attack -- токен, предназначенный сервису заказов, не будет принят сервисом платежей. Для Kubernetes-среды дополнительно стоит использовать Service Accounts (IRSA/Workload Identity) для доступа к облачным ресурсам без static secrets.

> [!question]- Как связаны механизм DPoP (RFC 9449) и концепция App Attestation на мобильных устройствах? Можно ли одно заменить другим?
> DPoP и App Attestation решают разные задачи и работают на разных уровнях. DPoP привязывает access token к криптографическому ключу клиента -- украденный токен бесполезен без приватного ключа. App Attestation подтверждает, что запрос исходит от легитимного приложения на неповреждённом устройстве. Одно не заменяет другое: DPoP защищает от кражи токена, а Attestation -- от модифицированного клиента или бота. Максимальная защита мобильного API требует обоих механизмов одновременно (OAuth 2.0 + PKCE + DPoP + App Attestation).

> [!question]- Сервис использует API key для публичного геокодирования API. Ключ утёк в публичный GitHub-репозиторий. Какие конкретные шаги нужно предпринять, используя паттерн dual-key ротации, и какие сигналы мониторинга проверить?
> Шаги: (1) немедленно создать новый API key (secondary) на стороне провайдера, (2) развернуть приложение с новым ключом, (3) через мониторинг убедиться, что трафик идёт по новому ключу, (4) отозвать скомпрометированный ключ. Параллельно проверить сигналы аномалий: появились ли запросы из нетипичных географических регионов, был ли аномальный всплеск объёма запросов, обращался ли ключ к неожиданным endpoints (например, billing вместо geocoding). Rate limiting per key ограничивает ущерб до момента отзыва.

---

## Ключевые карточки

API Key -- это идентификатор чего?
?
API Key идентифицирует приложение или проект, но не конкретного пользователя. Это статическая строка без встроенного expiration и scope. Аналогия -- номер читательского билета: библиотекарь знает отдел, но не знает, кто именно пришёл.

Что такое DPoP и какую проблему он решает?
?
DPoP (Demonstration of Proof-of-Possession, RFC 9449) привязывает bearer token к криптографическому ключу клиента. Украденный токен бесполезен без приватного ключа. Клиент при каждом запросе подписывает proof своим приватным ключом, а сервер проверяет соответствие.

Как работает dual-key ротация API key без даунтайма?
?
Приложение хранит два ключа (primary и secondary), оба одновременно валидны на сервере. При ротации: создать новый ключ, переключить приложение, убедиться в переключении трафика, удалить старый. В любой момент хотя бы один ключ валиден.

Почему при проверке HMAC-подписи webhook нужно constant-time сравнение?
?
Обычное `==` прерывается при первом несовпадающем символе. Злоумышленник может замерять время ответа и побайтово подбирать подпись (timing attack). Constant-time сравнение (`MessageDigest.isEqual`) выполняется одинаковое время независимо от совпадения.

Что такое confused deputy attack и как audience restriction её предотвращает?
?
Confused deputy -- атака, при которой токен, предназначенный сервису A, используется для доступа к сервису B. Audience restriction (`aud` claim в JWT) заставляет каждый сервис принимать только токены, адресованные именно ему, разрывая цепочку несанкционированной передачи.

Чем mTLS отличается от обычного TLS?
?
В обычном TLS только клиент проверяет сертификат сервера. В mTLS (mutual TLS) обе стороны предъявляют и проверяют сертификаты друг друга. Идентичность встроена в транспортный уровень -- приложению не нужно обрабатывать токены.

Какие три вердикта возвращает Android Play Integrity API?
?
Device integrity (устройство не рутовано, bootloader заблокирован), app integrity (приложение установлено из Play Store, не модифицировано), account integrity (аккаунт Google в хорошем состоянии). Сервер получает подписанный Google verdict и решает, доверять ли запросу.

Что означает Zero Trust и чем отличается от "отсутствия доверия"?
?
Zero Trust означает отсутствие неявного (implicit) доверия, а не полное отсутствие доверия. Доверие есть, но оно явное, проверяемое и ограниченное по времени. Каждый запрос верифицируется независимо от сетевого расположения -- принцип "never trust, always verify" (NIST SP 800-207).

Почему Service Mesh не заменяет application-level авторизацию?
?
Service Mesh обеспечивает transport-level security (mTLS, идентификация сервисов), но не знает бизнес-логику. Mesh не может решить, "может ли Service A вызвать DELETE /users/42". Application-level авторизация (RBAC, ABAC) по-прежнему необходима поверх mesh.
...
|-------------|------|-------|
| Фундамент аутентификации | [[authentication-authorization]] | Разобраться в базовых понятиях AuthN vs AuthZ, ролях и scopes перед углублением в API-паттерны |
| Механика токенов | [[auth-sessions-jwt-tokens]] | Понять внутреннее устройство JWT, sessions и refresh tokens, на которые опираются bearer-паттерны |
| OAuth 2.0 потоки | [[auth-oauth2-oidc]] | Изучить Authorization Code, Client Credentials и PKCE -- flows, используемые в API-аутентификации |
| Транспортная безопасность | [[security-https-tls]] | Углубить понимание TLS, сертификатов и PKI -- фундамент для mTLS |
| Защита API от злоупотреблений | [[security-api-protection]] | Дополнить аутентификацию защитой: rate limiting, input validation, API gateway patterns |
| Модели авторизации | [[auth-authorization-models]] | После аутентификации (кто вызывает?) перейти к авторизации (что разрешено?) -- RBAC, ABAC для API endpoints |
