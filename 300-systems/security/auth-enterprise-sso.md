...
> [!question]- Почему в Kerberos пароль пользователя никогда не передаётся по сети, и какую конкретную проблему это решает по сравнению с SAML, где пароль вводится на странице IdP?
> В Kerberos клиент шифрует timestamp ключом, производным от пароля (pre-authentication), и отправляет только зашифрованный timestamp. KDC проверяет его, используя копию ключа из своей базы. Это исключает перехват пароля даже при компрометации сетевого канала. В SAML пароль вводится на веб-странице IdP и передаётся по TLS -- безопасность зависит от целостности TLS-соединения. Kerberos был спроектирован для эпохи, когда шифрование канала не было гарантировано (LAN 1988 года), поэтому защита встроена на уровне протокола, а не транспорта.

> [!question]- Компания мигрирует с on-premise Active Directory на облачный IdP (Okta). Внутренние сервисы используют Kerberos, а SaaS-приложения -- SAML. Какую стратегию вы выберете и почему нельзя просто "выключить" AD?
> Нельзя выключить AD одномоментно, потому что Kerberos требует KDC в локальной сети -- облачный IdP не может выдавать Kerberos-тикеты для on-premise ресурсов (файловые серверы, принтеры, SharePoint on-prem). Стратегия: настроить Okta как федеративный IdP для всех SaaS (SAML/OIDC), сохранить AD как KDC для внутренних ресурсов, связать Okta с AD через агент синхронизации (Okta AD Agent). Постепенно переводить внутренние сервисы на веб-интерфейсы с OIDC, и только после полного отказа от Kerberos-зависимых сервисов можно деcommission AD.

> [!question]- Как XML Signature Wrapping атака связана с общей проблемой разделения парсинга и валидации в системах безопасности? Приведите аналогию из другого домена.
> XML Signature Wrapping эксплуатирует расхождение между тем, какой XML-элемент покрыт подписью (по XPath), и тем, какой элемент SP фактически обрабатывает. Подпись валидна, но применяется к "старому" элементу, а SP читает подменённый. Это частный случай TOCTOU (Time of Check / Time of Use) -- проверяется одно, используется другое. Аналогия из веб-безопасности: HTTP Request Smuggling, где фронтенд и бэкенд по-разному парсят границы HTTP-запросов. Общий принцип: если валидация и обработка используют разные парсеры или разные пути к данным -- возникает вектор атаки.

> [!question]- При проектировании мобильного приложения с поддержкой корпоративного SSO вы выбираете между SAML и OIDC. Какой протокол предпочтительнее и какие технические ограничения мобильной платформы определяют выбор?
> OIDC предпочтительнее для мобильных приложений по трём причинам. Во-первых, формат: JSON/JWT компактнее XML -- меньше трафик и быстрее парсинг на устройстве. Во-вторых, flow: OIDC использует Authorization Code + PKCE, адаптированный для native apps (RFC 8252), тогда как SAML HTTP-POST binding требует WebView для обработки HTML auto-submit form -- это создаёт UX-проблемы и риски безопасности (WebView может утечь cookies). В-третьих, экосистема: AppAuth SDK для iOS/Android поддерживает OIDC нативно, а зрелых мобильных SAML-библиотек практически нет.

## Ключевые карточки

Что такое SSO и какую главную проблему он решает?
?
SSO (Single Sign-On) -- архитектурный паттерн, при котором пользователь аутентифицируется один раз и получает доступ ко всем сервисам без повторного ввода пароля. Решает проблему password fatigue в корпорациях с десятками внутренних систем.

Какие три типа утверждений содержит SAML Assertion?
?
Authentication Statement (кто, когда, каким методом аутентифицирован), Attribute Statement (email, группы, роли пользователя), Authorization Decision Statement (разрешение на действие -- используется крайне редко). На практике SP используют первые два типа.

В чём разница между SP-Initiated и IdP-Initiated SAML Flow?
?
SP-Initiated: пользователь обращается к SP, SP генерирует AuthnRequest и перенаправляет к IdP. IdP-Initiated: пользователь начинает с портала IdP и кликает на приложение. IdP-Initiated менее безопасен -- нет InResponseTo для привязки ответа к запросу.

Из каких компонентов состоит Kerberos KDC и какова роль каждого?
?
KDC состоит из Authentication Server (AS) -- проверяет начальную аутентификацию и выдаёт TGT, и Ticket-Granting Server (TGS) -- выдаёт Service Tickets к конкретным сервисам на основе предъявленного TGT. Оба компонента работают на одном сервере (Domain Controller в AD).

Что такое Golden Ticket атака в Kerberos и почему она критична?
?
Атакующий, получивший хеш учётной записи krbtgt из Active Directory, может генерировать TGT для любого пользователя с любыми привилегиями. Это полная компрометация домена. Защита: регулярная двойная ротация пароля krbtgt и мониторинг аномальных TGT.

Чем LDAP отличается от Active Directory?
?
LDAP -- протокол доступа к директории (древовидной базе данных). Active Directory -- продукт Microsoft, который использует LDAP как один из протоколов наряду с Kerberos, DNS, NTLM и GPO. В Linux-мире LDAP (OpenLDAP) используется отдельно, без экосистемы AD.

Почему LDAP-аутентификация двухэтапная?
?
Пользователь вводит email или username, а не DN. Поэтому сначала сервисный аккаунт ищет DN пользователя по атрибуту (mail, uid) через LDAP search, затем выполняется re-bind найденным DN с паролем пользователя. Успешный bind означает верный пароль.

Что такое SCIM и какую задачу он решает поверх SSO?
?
SCIM (System for Cross-domain Identity Management) -- REST API для автоматического создания и удаления аккаунтов пользователей во всех сервисах. SSO решает "как войти", SCIM решает "как создать/удалить аккаунт". При найме HR добавляет сотрудника в IdP -- SCIM создаёт аккаунты во всех SP.

Почему SAML Response передаётся через HTTP-POST, а не HTTP-Redirect?
?
SAML Response содержит XML Assertion с цифровой подписью и атрибутами -- это десятки килобайт. URL имеет ограничение длины около 8 KB, поэтому HTTP-Redirect не подходит. HTTP-POST использует auto-submitting HTML-форму для передачи Response в теле запроса.

Что такое федерация и чем она отличается от обычного SSO?
?
SSO -- один логин для всех сервисов одной организации. Федерация -- один логин для сервисов разных организаций, установивших взаимное доверие (Trust Relationship) через обмен метаданными. Пример: eduroam позволяет студенту одного вуза подключаться к Wi-Fi другого.

## Куда дальше

| Направление | Файл | Зачем |
|-------------|------|-------|
| Основы аутентификации | [[authentication-authorization]] | Hub-статья: AuthN vs AuthZ, обзор всех методов -- prerequisite для глубокого понимания SSO |
| OAuth 2.0 и OIDC | [[auth-oauth2-oidc]] | Детальная механика OIDC -- современной альтернативы SAML для SSO |
| MFA и Passwordless | [[auth-passwordless-mfa]] | FIDO2/passkeys как phishing-resistant дополнение к enterprise SSO |
| API-аутентификация | [[auth-api-service-patterns]] | Machine-to-machine auth, SSO tokens для API -- то, что SAML не покрывает |
| Криптографические основы | [[security-cryptography-fundamentals]] | Цифровые подписи, X.509 сертификаты -- основа доверия в SAML и Kerberos |
| Модели авторизации | [[auth-authorization-models]] | RBAC, ABAC, ReBAC -- что происходит после аутентификации через SSO |
