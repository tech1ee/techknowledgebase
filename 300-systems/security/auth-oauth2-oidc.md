...
> [!question]- Почему PKCE стал обязательным для ВСЕХ клиентов в OAuth 2.1, включая серверные приложения, которые уже имеют client_secret?
> Потому что PKCE и client_secret защищают от разных атак. Client_secret подтверждает идентичность клиента, но не защищает от authorization code injection -- атаки, при которой злоумышленник подставляет чужой код в callback легитимного клиента. PKCE криптографически привязывает конкретный authorization code к конкретному запросу авторизации через пару code_verifier/code_challenge. Даже если атакующий получит и code, и client_secret, без оригинального code_verifier обмен невозможен. Это принцип defense in depth -- несколько слоёв защиты закрывают разные векторы атак.

> [!question]- Вы проектируете мобильное приложение, которое поддерживает вход через Google и GitHub. Пользователь зарегистрировался через Google (alex@mail.com), а через месяц нажимает "Войти через GitHub" с тем же email. Как безопасно обработать эту ситуацию?
> Нельзя автоматически связывать аккаунты по email. Нужно: (1) проверить claim email_verified -- если GitHub не подтвердил email, игнорировать совпадение; (2) если email подтверждён обоими провайдерами -- запросить у пользователя подтверждение через уже подключённый провайдер (Google), чтобы доказать владение аккаунтом; (3) использовать sub (уникальный идентификатор провайдера) как primary key для связки, а не email. Автоматическая привязка по email создаёт уязвимость: злоумышленник может добавить чужой email в GitHub без верификации и получить доступ к аккаунту жертвы.

> [!question]- Как механизм DPoP (RFC 9449) решает проблему утечки bearer-токенов, и чем он концептуально схож с TLS client certificates из [[security-https-tls]]?
> Bearer token работает как наличные: кто украл -- тот и использует. DPoP привязывает токен к криптографической паре ключей клиента: при каждом API-запросе клиент создаёт подписанный proof, и resource server проверяет, что proof подписан тем же ключом, к которому привязан токен. Без приватного ключа украденный токен бесполезен. Это концептуально аналогично mTLS (mutual TLS), где клиент тоже доказывает владение приватным ключом при каждом соединении. Разница: mTLS работает на транспортном уровне (TLS handshake), а DPoP -- на уровне приложения (HTTP-заголовки), что делает его проще для развёртывания без инфраструктурных изменений.

> [!question]- Проанализируйте: в каких случаях Token Introspection (RFC 7662) предпочтительнее JWT-based access tokens, а в каких -- наоборот?
> JWT-based access tokens предпочтительнее при высоких нагрузках: resource server проверяет подпись локально без сетевого вызова (латентность ~0 мс vs 1-100 мс у introspection). Но у JWT есть фундаментальный недостаток -- невозможность мгновенного отзыва: пока токен не истёк, он валиден. Token Introspection предпочтительнее, когда: (1) нужен гарантированный real-time отзыв -- финансовые системы, медицинские данные; (2) access token -- opaque string, и resource server не может проверить его самостоятельно; (3) нужна централизованная аудит-логика проверки. Практический компромисс: короткоживущие JWT (5-15 мин) + отзыв refresh token покрывают большинство сценариев без introspection.

---

## Ключевые карточки

OAuth 2.0 решает проблему "password anti-pattern". Что это значит?
?
Приложение никогда не должно знать пароль пользователя от стороннего сервиса. Вместо передачи полных учётных данных OAuth создаёт систему ограниченных, отзываемых, привязанных к конкретному действию разрешений (токенов).

Зачем в Authorization Code Flow промежуточный authorization code, если можно сразу вернуть access token?
?
Front-channel (браузер) небезопасен: URL виден в истории, логах, Referer, расширениям. Authorization code -- одноразовая строка, бесполезная без client_secret. Обмен на токены идёт по back-channel (сервер-сервер), где ни code, ни токены не проходят через браузер.

Что такое PKCE и какую проблему он решает?
?
PKCE (Proof Key for Code Exchange) защищает от перехвата authorization code у public clients (мобильные приложения, SPA), которые не могут хранить client_secret. Клиент генерирует одноразовый code_verifier, отправляет его SHA-256 хеш (code_challenge) при авторизации, а оригинальный verifier -- при обмене кода на токен.

Чем ID Token (OIDC) отличается от access token (OAuth) по назначению и получателю?
?
Access token предназначен для Resource Server (API) -- это пропуск к ресурсам. ID Token предназначен для Client (вашего приложения) -- это подписанное утверждение о том, кто пользователь. Отправлять ID Token как Bearer token в API-запросах -- нарушение протокола и уязвимость.

Для чего нужен параметр nonce в OIDC?
?
Nonce -- одноразовое значение, защищающее от replay-атак. Клиент генерирует nonce, отправляет с запросом авторизации, а authorization server включает его в ID Token. При получении токена клиент сверяет nonce с сохранённым в сессии. Подставить ID Token из чужой сессии невозможно -- nonce не совпадёт.

Какие три ключевых изменения вносит OAuth 2.1 по сравнению с OAuth 2.0?
?
(1) PKCE обязателен для всех клиентов, включая серверные. (2) Удалены Implicit и Password flows как небезопасные. (3) Redirect URI требует только exact match -- никаких wildcards или частичного совпадения.

Что такое DPoP и чем он отличается от bearer token?
?
DPoP (Demonstrating Proof of Possession) привязывает access token к криптографической паре ключей клиента. При каждом API-запросе клиент создаёт подписанный proof. Если токен украден, без приватного ключа он бесполезен. Bearer token -- как наличные (кто нашёл, тот тратит), DPoP -- как карта с PIN.

Почему автоматическая привязка аккаунтов по email при social login опасна?
?
Провайдер может не верифицировать email (claim email_verified = false). Злоумышленник может добавить чужой email в свой аккаунт на стороннем провайдере и через social login получить доступ к аккаунту жертвы. Безопасный подход: проверять email_verified, использовать sub как primary key, требовать подтверждение через уже подключённый провайдер.

Когда OAuth НЕ нужен?
?
(1) Простые веб-приложения с серверными сессиями -- session-based auth проще и надёжнее. (2) Внутренние микросервисы -- лучше mTLS или service mesh. (3) Нет интеграции со сторонним API и не предоставляете свой API сторонним клиентам.
...
|-------------|------|-------|
| Базовые концепции | [[authentication-authorization]] | Закрепить разницу AuthN vs AuthZ перед углублением в OAuth/OIDC |
| JWT и сессии | [[auth-sessions-jwt-tokens]] | Понять формат JWT, стратегии хранения токенов и ограничения, которые напрямую влияют на OAuth-реализации |
| Enterprise SSO | [[auth-enterprise-sso]] | Изучить SAML, ADFS, Okta и сравнить с OIDC в корпоративном контексте |
| API-паттерны | [[auth-api-service-patterns]] | Применить OAuth на практике: Client Credentials для M2M, API gateways, стратегии валидации токенов |
| TLS и транспорт | [[security-https-tls]] | Понять, почему TLS обязателен для всех OAuth flows и как работает mTLS как альтернатива bearer tokens |