...
> [!question]- Почему ProGuard/R8 obfuscation недостаточна для защиты Android-приложения от реверс-инжиниринга, даже если включены максимальные настройки оптимизации?
> ProGuard/R8 выполняет только name obfuscation — переименование классов, методов и полей. Он не шифрует строки (API URL, ключи остаются в открытом виде), не изменяет control flow (логика алгоритмов читаема), не защищает от динамического анализа (Frida перехватывает runtime-значения). Опытный реверсер восстановит бизнес-логику, ориентируясь на структуру вызовов и строковые константы. Для реальной защиты необходимы string encryption, control flow obfuscation и RASP — что даёт, например, DexGuard.

> [!question]- Приложение использует Play Integrity API и получает вердикт MEETS_DEVICE_INTEGRITY. Атакующий запускает Frida на том же устройстве и перехватывает API-ответы. Какой слой защиты отсутствует и как это исправить?
> Play Integrity подтверждает целостность устройства и бинарника, но не защищает от runtime-атак. Frida внедряется в процесс после прохождения attestation и свободно хукает функции. Отсутствует слой RASP — runtime application self-protection. Необходимо добавить: Frida detection (проверка портов 27042/27043, /proc/self/maps на артефакты), debugger detection (TracerPid, ptrace), hooking detection. Также критично вынести чувствительную логику на сервер, чтобы даже при успешном hooking атакующий не получил доступ к бизнес-данным.

> [!question]- Команда решает хранить API-ключ для платёжной системы в native (.so) библиотеке, считая это достаточно безопасным. Примените принцип Defense in Depth: какие дополнительные слои нужно реализовать на уровне build-системы, runtime и серверной архитектуры?
> На уровне build-системы: включить R8/DexGuard obfuscation для Java/Kotlin-слоя, strip symbols из .so, использовать string encryption для native-констант. На уровне runtime: добавить anti-debugging (ptrace check в native), Frida detection, проверку integrity APK-подписи перед расшифровкой ключа. На уровне серверной архитектуры: перейти на BFF-паттерн, где API-ключ платёжной системы хранится только на бэкенде, а мобильное приложение взаимодействует через свой сервер с attestation-токеном. Ключ в native-коде — лишь один слой, без остальных он извлекается за часы.

> [!question]- Сравните подходы к jailbreak detection на iOS (fork check, dylib injection check, sandbox violation check) и root detection на Android (file checks, package checks, Magisk detection). Какой из подходов сложнее обойти и почему?
> iOS-подход сложнее обойти по нескольким причинам. Fork check опирается на ограничение ядра (sandbox запрещает fork()), и его обход требует патча ядра. Dylib injection check сканирует _dyld_image_count() — системный вызов, который сложно перехватить без substrate/hooking framework (который сам является индикатором). Sandbox violation — попытка записи вне контейнера — проверяет kernel-level ограничение. На Android root detection опирается на user-space артефакты: файлы в /system (Magisk Hide скрывает), пакеты (переименовываются), build tags (подменяются через props). Magisk + Shamiko + Zygisk обходят большинство Android-проверок на уровне user-space. Однако оба подхода обходимы при достаточной мотивации — поэтому они должны быть частью Defense in Depth, а не единственной защитой.

---

## Ключевые карточки

Defense in Depth в контексте мобильной безопасности — что это?
?
Стратегия многослойной защиты: code hardening (build time) + integrity verification (launch time) + environment checks (runtime) + continuous monitoring + response strategy. Bypass одного слоя не компрометирует всю систему, а стоимость атаки растёт экспоненциально.

Какие четыре уровня obfuscation существуют и чем они различаются?
?
1) Name obfuscation — переименование символов (ProGuard/R8). 2) String encryption — шифрование строковых констант. 3) Control flow obfuscation — изменение структуры кода, fake branches. 4) Code virtualization — код исполняется custom VM. Каждый следующий уровень экспоненциально усложняет анализ.

Что проверяет Play Integrity API и каковы его ограничения?
?
Проверяет: genuine device (MEETS_DEVICE_INTEGRITY), базовую целостность (MEETS_BASIC_INTEGRITY), hardware-backed security (MEETS_STRONG_INTEGRITY), лицензию приложения. Ограничения: 10 000 запросов/день, не защищает от runtime-атак (Frida), требует server-side verification, обходим на rooted устройствах.

Чем App Attest (iOS) отличается от DeviceCheck?
?
App Attest подтверждает целостность бинарника через Secure Enclave (key pair + attestation от Apple), защищает от тампирования. DeviceCheck хранит 2 бита на комбинацию device+app для бизнес-логики (trial tracking, fraud prevention). App Attest — про integrity, DeviceCheck — про device-level flags.

Что такое BFF-паттерн и зачем он нужен для secrets protection?
?
Backend-for-Frontend: мобильное приложение не обращается к third-party API напрямую, а через свой бэкенд, который хранит API-ключи. Приложение не содержит secrets. Бонусы: rate limiting, audit logging, лёгкая ротация ключей, валидация запросов.

Какие основные методы Frida detection существуют?
?
1) Проверка default-портов (27042, 27043). 2) Поиск процесса frida-server. 3) Анализ /proc/self/maps на frida-библиотеки. 4) D-Bus проверка. 5) Поиск memory-артефактов (LIBFRIDA, GumJS). 6) Проверка named pipes в /data/local/tmp.

Почему root/jailbreak detection нельзя использовать как единственную меру защиты?
?
Все методы обходимы: Magisk Hide/Shamiko/Zygisk скрывают root на Android за минуты, iOS-тулинг патчит jailbreak-индикаторы. Root detection — это deterrent (повышение планки), а не барьер. Требуется комбинация с RASP, attestation, server-side validation и BFF-паттерном.

Какие три проверки выполняет IntegrityVerifier для Android APK?
?
1) Верификация подписи APK — сравнение SHA-256 хеша сертификата с ожидаемым. 2) Проверка источника установки — installer должен быть из списка разрешённых (Play Store, Galaxy Store и т.д.). 3) Проверка контрольной суммы APK — SHA-256 бинарника совпадает с эталоном.

Какие стратегии реагирования на security-угрозы должны быть в приложении?
?
Четыре уровня: LOW — только логирование. MEDIUM — отключение некритичных функций + предупреждение. HIGH — блокировка чувствительных операций + очистка данных. CRITICAL — полная очистка данных, уведомление сервера, завершение приложения.
...
|-------------|------|-------|
| Каталог мобильных угроз | [[mobile-security-owasp]] | Понять ландшафт угроз (OWASP Mobile Top 10), от которых защищают техники из этого материала |
| Стандарт верификации | [[mobile-security-masvs]] | Изучить формальные требования MASVS-RESILIENCE к anti-tampering и reverse engineering protection |
| Безопасность Android | [[android-permissions-security]] | Углубиться в Android Keystore, permission model и EncryptedSharedPreferences как основу для RASP |
| Безопасность iOS | [[ios-permissions-security]] | Освоить Keychain, Secure Enclave, Data Protection API и App Attest для iOS-реализации защиты |
| Моделирование угроз | [[threat-modeling]] | Научиться приоритизировать техники защиты через систематический threat modeling для конкретного приложения |
| Основы безопасности | [[security-fundamentals]] | Закрепить базовые принципы (CIA triad, Defense in Depth), на которых строится вся архитектура защиты |
