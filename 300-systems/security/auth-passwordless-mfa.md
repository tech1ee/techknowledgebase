...
> **Определение.** Многофакторная аутентификация (MFA) — метод подтверждения личности, требующий предъявления доказательств из двух или более независимых категорий (факторов): знание (knowledge), владение (possession), биометрия (inherence). Беспарольная аутентификация (passwordless) исключает фактор знания, заменяя shared secret на асимметричную криптографию.

### Таксономия факторов аутентификации

| Фактор | Категория | Примеры | Уязвимости |
|--------|-----------|---------|------------|
| Знание (knowledge) | Что ты знаешь | Пароль, PIN, security question | Фишинг, brute-force, утечка БД |
| Владение (possession) | Что ты имеешь | TOTP-приложение, YubiKey, телефон (SMS) | SIM-swap, кража устройства |
| Биометрия (inherence) | Кто ты есть | Отпечаток пальца, Face ID, голос | Spoofing, невозможность замены |
| Местоположение | Где ты находишься | GPS, IP-геолокация | VPN/proxy, GPS spoofing |
| Поведение | Как ты действуешь | Динамика нажатий, pattern of life | Имитация, low accuracy |

### Историческая хронология

| Год | Событие | Значение |
|-----|---------|----------|
| 1961 | CTSS (MIT) — первая парольная система | Фернандо Корбато, shared secret для многопользовательского доступа |
| 1998 | RFC 2289 — OTP (One-Time Password) | S/KEY: одноразовые пароли на основе хеш-цепочки |
| 2005 | RFC 4226 — HOTP (HMAC-based OTP) | HMAC-SHA1 с counter; основа для аппаратных токенов (RSA SecurID) |
| 2011 | RFC 6238 — TOTP (Time-based OTP) | HMAC-SHA1 с `floor(unix_time / 30)`; Google Authenticator |
| 2019 | W3C WebAuthn (Brand C. et al.) | Асимметричная криптография в браузере; credential привязан к origin |
| 2022 | FIDO Alliance — passkeys | Discoverable WebAuthn credentials с облачной синхронизацией |
| 2025 | NIST SP 800-63-4 (Grassi P. et al.) | Argon2id, отказ от complexity requirements, phishing-resistant MFA для AAL2+ |

### Уровни аутентификации по NIST SP 800-63-4

| Уровень | Требования | Phishing-resistant | Примеры |
|---------|-----------|-------------------|---------|
| AAL1 | Один фактор (любой) | Нет | Пароль, SMS OTP |
| AAL2 | Два фактора, рекомендуется phishing-resistant | Рекомендуется | Пароль + TOTP, passkey |
| AAL3 | Два фактора, hardware-bound, обязательно phishing-resistant | Обязательно | YubiKey + PIN, Secure Enclave passkey |

> **Фундаментальный принцип WebAuthn.** Credential криптографически привязан к origin (домену). Authenticator создаёт уникальную пару ключей для каждого `(rpId, user)`, и приватный ключ никогда не покидает устройство. Фишинговый сайт с другим origin не сможет получить подпись для чужого credential — защита встроена в протокол, а не делегирована пользователю.

**См. также:** [[security-cryptography-fundamentals]] — HMAC, ECDSA, Ed25519, лежащие в основе TOTP и WebAuthn; [[auth-enterprise-sso]] — интеграция FIDO2/passkeys с корпоративным SSO.
...
**Bonneau, J. et al. (2012). "The Quest to Replace Passwords: A Framework for Comparative Evaluation of Web Authentication Schemes"** — фундаментальная работа от Cambridge и Microsoft Research. Авторы определили 25 свойств (usability, deployability, security), по которым оцениваются все альтернативы паролям, и проанализировали 35 схем. Ни одна не превосходила пароли по всем критериям одновременно -- что объясняет, почему замена паролей заняла десятилетия. **Почему читать:** даёт framework для оценки любого метода аутентификации -- включая passkeys.

**Grassi, P. et al. (2025). NIST SP 800-63-4 "Digital Identity Guidelines"** — обновлённый национальный стандарт: рекомендации по password hashing (Argon2id), отказ от complexity requirements и принудительной ротации, уровни аутентификации (AAL1-3), требования к phishing-resistant MFA, запрет security questions. **Почему читать:** де-факто стандарт для всей индустрии.

**Brand, C. et al. (2019). W3C Web Authentication: An API for accessing Public Key Credentials** — W3C Recommendation, определяющая WebAuthn API -- протокол, на котором построены passkeys. **Почему читать:** техническая спецификация на уровне отдельных полей (`PublicKeyCredentialCreationOptions`, `AuthenticatorAttestationResponse`, origin validation).

**Provos, N. & Mazieres, D. (1999). "A Future-Adaptable Password Scheme"** — оригинальная статья о bcrypt. 8 страниц, одна ключевая идея (adaptive cost factor), 27 лет релевантности. **Почему читать:** образец элегантного инженерного мышления.

**Biryukov, A., Dinu, D., Khovratovich, D. (2015). "Argon2: the memory-hard function for password hashing and other applications"** — описание победителя Password Hashing Competition. Объясняет, почему memory-hardness критична, и как Argon2 достигает настраиваемого баланса между временем, памятью и параллелизмом.

**M'Raihi, D. et al. (2005). RFC 4226: HOTP — An HMAC-Based One-Time Password Algorithm.** IETF. — формальная спецификация HOTP, фундамент для TOTP (RFC 6238) и аппаратных токенов.

### Практические руководства

**FIDO Alliance. (2024). "Passkeys Developer Guide".** — практическое руководство по реализации passkeys: регистрация, аутентификация, conditional UI, cross-device flows. Содержит примеры для веб и мобильных платформ.

**OWASP. (2025). "Multi-Factor Authentication Cheat Sheet".** — рекомендации по выбору и реализации MFA: какие факторы комбинировать, как обрабатывать recovery, rate limiting для OTP.

**Yubico. (2024). "WebAuthn Developer Guide".** — пошаговое руководство по реализации серверной и клиентской части WebAuthn с примерами кода и обработкой edge cases.
...
> [!question]- Почему memory-hardness Argon2id критичнее, чем увеличение числа итераций PBKDF2, при защите от GPU-атак?
> GPU имеет тысячи ядер, но ограниченный объём памяти на каждое ядро. PBKDF2 не требует значительной RAM, поэтому GPU может атаковать с полной параллельностью (миллиарды хешей в секунду). Argon2id требует 64+ MB на каждое вычисление, что ограничивает параллелизм GPU до сотен потоков вместо тысяч -- разница в эффективности атаки составляет шесть порядков. Увеличение итераций PBKDF2 линейно замедляет атаку, но не устраняет преимущество GPU в параллелизме.

> [!question]- Компания использует TOTP как второй фактор и считает систему защищённой от фишинга. Проанализируйте, почему это убеждение ошибочно, и предложите альтернативу.
> TOTP-код -- это 6-значное число, не привязанное к домену. Пользователь введёт его на фишинговом сайте так же легко, как на настоящем. Real-time phishing proxy (Evilginx2, Modlishka) перехватывает и пароль, и TOTP одновременно, ретранслируя их на настоящий сервер. Для phishing resistance необходимы WebAuthn/passkeys, где credential криптографически привязан к origin -- authenticator откажется подписывать challenge для чужого домена.

> [!question]- Как связаны концепции из криптографии (HMAC, асимметричное шифрование, цифровые подписи) с тремя разными механизмами аутентификации: TOTP, password hashing и passkeys?
> TOTP использует HMAC-SHA1: общий секрет + текущее время дают одноразовый код. Password hashing (Argon2id, bcrypt) использует KDF -- однонаправленные функции для безопасного хранения паролей. Passkeys используют асимметричную криптографию и цифровые подписи (ECDSA/Ed25519): приватный ключ подписывает challenge, сервер проверяет подпись публичным ключом. Каждый механизм опирается на разные криптографические примитивы, обеспечивая разный уровень безопасности.

> [!question]- Вы проектируете систему аутентификации для мобильного банковского приложения. Какой уровень AAL по NIST SP 800-63-4 вы выберете и какие конкретные методы реализуете?
> Для банковского приложения необходим минимум AAL2, желательно AAL3. Реализация: passkeys как основной метод (phishing-resistant, удовлетворяет AAL2-AAL3), биометрия (Face ID / BiometricPrompt) для разблокировки приватного ключа на устройстве, hardware-backed ключи через Secure Enclave (iOS) или StrongBox (Android). SMS OTP недопустим для AAL2+ по NIST. TOTP допустим для AAL2, но не обеспечивает phishing resistance. Recovery flow: recovery codes + trusted contacts.

---

## Ключевые карточки

Почему MD5 и SHA-256 непригодны для хеширования паролей?
?
Они проектировались для скорости. GPU вычисляет ~70 миллиардов MD5 или ~22 миллиарда SHA-256 хешей в секунду, что позволяет перебрать 8-символьный пароль за часы. KDF (bcrypt, Argon2id) намеренно замедляют каждую попытку.

Что такое memory-hardness и почему она важна для password hashing?
?
Memory-hardness означает, что для вычисления одного хеша требуется значительный объём RAM (например, 64 MB для Argon2id). GPU имеет тысячи ядер, но ограниченную память, поэтому параллельные атаки становятся непрактично дорогими.

Чем passkey отличается от обычного WebAuthn credential?
?
Passkey -- это discoverable WebAuthn credential, синхронизируемый через облачное хранилище (iCloud Keychain, Google Password Manager). Обычный WebAuthn credential привязан к одному устройству -- потеря устройства означает потерю доступа.

Как WebAuthn обеспечивает phishing resistance?
?
Credential криптографически привязан к origin (домену). Authenticator проверяет origin и отказывается подписывать challenge для чужого домена. Фишинговый сайт examp1e.com не получит подпись для credential, зарегистрированного на example.com.

Какие три параметра определяют стоимость Argon2id?
?
Memory (объём RAM, рекомендация OWASP: 64 MB+), time (число проходов по памяти, рекомендация: 3+), parallelism (число потоков, обычно 1-4). Memory -- самый эффективный рычаг против GPU-атак.

Почему NIST SP 800-63-4 отказался от complexity requirements и принудительной ротации паролей?
?
Complexity requirements ведут к предсказуемым паттернам (P@ssw0rd!, Summer2025!). Принудительная ротация порождает последовательности (Password1, Password2). Оба подхода создают иллюзию безопасности, но на практике ослабляют пароли.

Почему биометрия (Face ID, Touch ID) -- это НЕ передача пароля серверу?
?
Биометрические данные никогда не покидают устройство. Они используются локально для разблокировки криптографического ключа в Secure Enclave (iOS) или TEE (Android). Сервер получает только подпись, созданную этим ключом, а не биометрию.

Почему SMS OTP не рекомендован NIST для AAL2 и выше?
?
SMS уязвим к SIM-swapping (перенос номера на SIM атакующего) и перехвату через SS7 (протокол 1970-х без защиты от несанкционированного доступа). FBI зарегистрировало более 68,000 случаев SIM swap в 2024 году.

Что такое TOTP и из каких компонентов вычисляется одноразовый код?
?
TOTP (Time-Based One-Time Password) -- код, вычисляемый по формуле HMAC-SHA1(shared_secret, floor(unix_time / 30)). Из результата HMAC через dynamic truncation извлекается 6-значный код. Код меняется каждые 30 секунд.

В чём ключевое отличие AAL2 от AAL3 по NIST SP 800-63-4?
?
AAL2 требует MFA и рекомендует phishing-resistant методы. AAL3 требует MFA с обязательным hardware-bound authenticator (YubiKey, Secure Enclave) и обязательной phishing resistance. AAL3 применяется для гос. систем, обороны, здравоохранения.
...
|-------------|------|-------|
| Общая картина AuthN и AuthZ | [[authentication-authorization]] | Вернуться к hub-файлу и увидеть, как password hashing, MFA и passkeys вписываются в общую архитектуру аутентификации и авторизации |
| Сессии и токены после аутентификации | [[auth-sessions-jwt-tokens]] | Понять, что происходит после успешной аутентификации: как сервер запоминает пользователя через sessions, JWT и opaque tokens |
| Enterprise SSO и FIDO2 в корпорациях | [[auth-enterprise-sso]] | Разобраться, как passkeys и MFA интегрируются с SAML, OIDC, MDM и compliance-требованиями крупных компаний |
| Криптографический фундамент | [[security-cryptography-fundamentals]] | Углубить понимание HMAC, ECDSA, хеш-функций и асимметричной криптографии, на которых построены все механизмы из этого файла |
| Биометрия и безопасность на Android | [[android-permissions-security]] | Изучить платформенную реализацию: BiometricPrompt, KeyStore, Credential Manager API и классы биометрии |
| Биометрия и безопасность на iOS | [[ios-permissions-security]] | Изучить платформенную реализацию: LAContext, Secure Enclave, AuthenticationServices и особенности Face ID / Touch ID |
| Machine-to-machine аутентификация | [[auth-api-service-patterns]] | Разобраться с API keys, mutual TLS и OAuth client credentials -- методами аутентификации для сценариев без участия человека |