...
> [!question]- Почему монолитная кодовая база становится нечитаемой с ростом, если рассматривать это через призму element interactivity?
> С ростом кодовой базы увеличивается количество взаимодействующих элементов (классы, зависимости, побочные эффекты). Element interactivity определяет intrinsic load: чем больше элементов нужно удерживать одновременно для понимания, тем выше нагрузка на рабочую память. Монолит превышает предел рабочей памяти (~7 элементов), и понимание становится невозможным без декомпозиции.

> [!question]- Как отличить intrinsic load от extraneous load при проектировании API? Приведите пример каждого типа.
> Intrinsic load -- сложность самой предметной области (количество параметров, взаимосвязи между endpoints). Его нельзя убрать, но можно управлять через декомпозицию. Extraneous load -- плохой дизайн API (непоследовательные naming conventions, избыточные поля, запутанная документация). Extraneous load можно устранить без потери функциональности.

> [!question]- Разработчик перегружен при code review файла на 500 строк. Какие принципы CLT помогут снизить нагрузку?
> (1) Chunking -- разбить на логические блоки для поэтапного review. (2) Снижение extraneous load -- форматирование, именование, комментарии к нетривиальной логике. (3) Schema formation -- использовать знакомые паттерны, которые ревьюер может распознать как единый chunk. (4) Fading -- начать с общей архитектуры, затем детали.

---

## Ключевые карточки

Какие три типа когнитивной нагрузки выделяет теория Свеллера?
?
Intrinsic (внутренняя) -- определяется сложностью материала и element interactivity. Extraneous (внешняя) -- вызвана плохим дизайном, можно убрать. Germane (полезная) -- направлена на формирование схем, нужно максимизировать.

Что такое element interactivity?
?
Количество элементов, которые нужно обрабатывать одновременно для понимания. Высокая интерактивность = высокая intrinsic load. Пример: простая формула -- низкая, система взаимосвязанных микросервисов -- высокая.

Почему рабочая память ограничена ~7 элементами?
?
Модель Миллера (7 +/- 2): рабочая память может удерживать ограниченное количество независимых элементов. Современные исследования уточняют до 4 chunks. Это фундаментальное ограничение, которое нельзя обойти, но можно компенсировать через chunking и schema formation.

Как germane load отличается от extraneous?
?
Germane load -- когнитивные усилия на формирование схем и глубокое понимание (полезная нагрузка). Extraneous -- усилия на навигацию, расшифровку плохого дизайна (бесполезная). Цель: минимизировать extraneous, максимизировать germane в пределах общего лимита.

Что такое expertise reversal effect?
?
Инструкции, полезные новичкам, мешают экспертам. Подробные пошаговые объяснения создают extraneous load для тех, кто уже имеет схемы. Пример: подробные комментарии в коде полезны джуниорам, но раздражают сеньоров.
...
| Направление | Ссылка |
|-------------|--------|
| Следующий шаг | [[context-switching]] |
| Углубиться | [[how-brain-learns]] |
| Смежная тема | [[clean-code-solid]] |
