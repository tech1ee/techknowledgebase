...
> **File Transfer** — процесс передачи данных между системами через сеть. В контексте мобильных устройств ключевые проблемы: ненадёжная сеть, ограниченная батарея, фоновые ограничения ОС и необходимость resumable downloads.

**Ключевые теоретические концепции:**

| Концепция | Источник | Суть | Применение к File Downloader |
|-----------|----------|------|------------------------------|
| HTTP Range Requests | RFC 7233 (2014) | Запрос части ресурса по byte range | Resumable downloads (докачка с места обрыва) |
| Idempotent Operations | HTTP spec | Повторный запрос даёт тот же результат | Safe retry при network failure |
| Backpressure | Reactive Streams | Механизм управления потоком данных | Контроль скорости записи на диск |
| Circuit Breaker | Michael Nygard (2007) | Прекращение запросов к failing service | Прекращение retry при persistent failure |
| Producer-Consumer | Dijkstra (1965) | Паттерн разделения производства и потребления | Download queue: scheduler → downloader → disk writer |

**Resumable Download Protocol (HTTP Range):**

```
Первый запрос:    GET /file HTTP/1.1
Ответ:            200 OK, Content-Length: 100MB, Accept-Ranges: bytes

Обрыв на 40MB.

Повторный запрос: GET /file HTTP/1.1
                  Range: bytes=40000000-
Ответ:            206 Partial Content, Content-Range: bytes 40000000-99999999/100000000
```

Без поддержки Range сервером — resume невозможен; library должен проверять `Accept-Ranges` header и fallback к полной перезакачке.

> **Nygard (2007), *Release It!*:** Circuit Breaker pattern предотвращает каскадные failures. В File Downloader: после N неудачных retry переводим загрузку в "waiting for network" состояние вместо бесконечного retry, экономя battery и network.

→ Связано: [[system-design-android]], [[android-background-work]], [[architecture-resilience-patterns]]
...
- RFC 7233 (2014). *Hypertext Transfer Protocol (HTTP/1.1): Range Requests*. — Спецификация resumable downloads через byte range requests.

- Nygard M. (2007). *Release It! Design and Deploy Production-Ready Software*. — Circuit Breaker pattern для предотвращения каскадных failures.
