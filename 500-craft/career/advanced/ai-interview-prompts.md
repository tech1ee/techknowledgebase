...
- [[ai-interview-preparation]] — Полный гайд по AI-подготовке к интервью, где эти промпты встроены в систему. Текущий материал — это reference-карта: бери нужный промпт и используй. Гайд объясняет КОГДА и ЗАЧЕМ использовать каждую категорию промптов, а также anti-паттерны и этику использования AI.

- [[ai-era-job-search]] — Обзор AI-инструментов для поиска работы в целом. Промпты из текущего материала работают в связке с инструментами оттуда: Jobscan для ATS-оптимизации, Teal для трекинга, Final Round AI для mock-интервью. Вместе они создают полный AI-assisted workflow.
...
> [!question]- Почему промпт "Write me a resume" неэффективен, а структурированный промпт с контекстом даёт качественный результат?
> AI без контекста генерирует generic контент, который рекрутеры распознают как AI-written. Структурированный промпт включает: конкретную роль, компанию, реальные метрики, ограничения формата. Это направляет AI на персонализированный результат, который можно проверить и доработать. Принцип: garbage in -- garbage out.

> [!question]- Ты получил feedback от рекрутера: "Your answers felt rehearsed". Какие промпты из библиотеки помогут исправить эту проблему?
> 1) Behavioral Mock Interviewer с probing follow-ups -- тренирует реакцию на неожиданные вопросы. 2) Story Improvement промпт -- добавляет specifics и личный стиль. 3) Post-Interview Analysis -- помогает выявить паттерн "заученности". Ключевое: использовать промпты для ПРАКТИКИ, а не заучивания скриптов.

> [!question]- Как правильно комбинировать промпты из разных категорий для подготовки к конкретной компании за 48 часов?
> 48h план: Day 1 -- Company Deep Dive + Predict Interview Questions + LP Alignment (если Amazon). Day 2 -- DSA Mock по предсказанным топикам + SD Design Review для likely problems + Behavioral Mock по предсказанным dimensions. Утро дня интервью -- Light warm-up + Story review. Промпты работают как pipeline: output одного становится input следующего.

> [!question]- Назови три случая, когда использование промпта из библиотеки может навредить подготовке.
> 1) Промпт для Solution Explanation ДО попытки решить задачу самостоятельно -- создаёт иллюзию понимания. 2) Промпт Counter-Offer Script при отсутствии реального leverage -- может привести к отзыву оффера. 3) Промпт Interviewer Research с домыслами AI -- ошибочная "общая тема" на интервью разрушает доверие.

---

## Ключевые карточки

Структура эффективного промпта для AI (шаблон)?
?
Четыре блока: Context (фон ситуации), Task (что сделать), Format (структура ответа), Constraints (ограничения и требования). Опционально: Examples (примеры желаемого output).

Промпт для STAR Story Builder -- что он должен включать?
?
Input: вопрос интервью + raw experience. Output: S (контекст, 2-3 предложения), T (личная ответственность), A (3-5 конкретных действий), R (количественный результат). Плюс: усиление результата, likely follow-ups, сокращённая 1-min версия.

Разница между промптами для Claude, GPT-4o и Gemini?
?
Claude: длинный контекст, нюансированный анализ -- быть детальным, просить reasoning. GPT-4o: быстрая итерация, код -- структурировать чёткими секциями. Gemini: research, суммаризация -- хорош для company research.

Промпт для Mock DSA Interviewer -- ключевые правила?
?
Пять правил: 1) Чёткая постановка задачи. 2) Ждать решения кандидата. 3) Задавать follow-up вопросы. 4) Давать hints, но не решать. 5) После решения -- детальный feedback. Указать: компанию, сложность, топик.

Промпт для Offer Analysis -- какие данные нужны?
?
Offer: base + equity + bonus + benefits + other. Context: текущая компенсация, market rate, локация, другие офферы. Output: total comp calculation, сравнение с рынком, red flags, leverage points, стратегия counter-offer.
...
|------------|------|--------|
| Следующий шаг | Полный гайд AI-подготовки к интервью | [[ai-interview-preparation]] |
| Углубиться | Фундамент интервью: DSA, SD, Behavioral | [[se-interview-foundation]] |
| Смежная тема | Стратегия переговоров по офферу | [[negotiation]] |
| Обзор | AI-инструменты для поиска работы | [[ai-era-job-search]] |
