...
> **Language-Specific Assessment** — оценка глубины владения конкретным языком программирования: от синтаксиса и идиом до понимания компиляции, runtime behaviour и design decisions авторов языка.

**Уровни знания Kotlin (для interview assessment):**

| Уровень | Что знаешь | Что спрашивают | Позиция |
|---------|-----------|----------------|---------|
| **Syntax** | val/var, when, data class | "Чем val отличается от var?" | Junior |
| **Idioms** | Extension functions, scope functions, sealed classes | "Когда let vs apply vs run?" | Mid |
| **Internals** | inline, reified, contracts, delegation | "Что делает inline под капотом?" | Senior |
| **Design** | Coroutines internals, Flow operators, KMP | "Как работает structured concurrency?" | Senior+ |
| **Language Design** | Kotlin compiler, type system, context receivers | "Почему Kotlin не поддерживает X?" | Staff |

**Kotlin в контексте programming language theory:**

| Концепция | Kotlin Feature | Теоретическая основа |
|-----------|---------------|---------------------|
| Null Safety | `String?` vs `String` | Option types (ML, 1973) |
| Algebraic Data Types | sealed class/interface | Sum types (type theory) |
| Coroutines | suspend, structured concurrency | CPS transformation, cooperative scheduling |
| Extension Functions | `fun String.foo()` | Open classes (CLOS, 1988); type-safe |
| Delegation | `by` keyword | Delegation pattern (GoF, 1994); language-level |

Kotlin-вопросы на интервью проверяют не знание API, а **understanding of language design decisions**: почему sealed class, а не enum? Почему inline, а не обычная функция? Ответ уровня Senior всегда включает trade-off analysis.

→ Связано: [[android-questions]], [[architecture-questions]], [[technical-interview]]
...
- Jemerov D., Isakova S. (2017). *Kotlin in Action*. — Авторитетный источник по Kotlin от разработчиков JetBrains; design decisions языка.

- Gamma E. et al. (1994). *Design Patterns: Elements of Reusable Object-Oriented Software*. — GoF patterns, реализованные на уровне языка в Kotlin (delegation, observer).

- McDowell G.L. (2015). *Cracking the Coding Interview*. — Методология решения задач; trade-off analysis применим к Kotlin.
