...
> [!question]- Почему распределение веса между DSA и System Design меняется с ростом уровня (70% DSA для Junior vs 15% для Staff+)?
> Junior оценивается по базовым coding skills -- его задача писать работающий код. Staff+ оценивается по ability to design systems и influence decisions. AI автоматизирует coding, но не заменяет architectural judgment. Чем выше уровень, тем больше ценится "какую проблему решать" вместо "как написать код".

> [!question]- На интервью тебе дали задачу и AI-assistant (Meta AI-enabled format). Какую стратегию использования AI покажет Senior-level thinking?
> Senior использует AI для scaffolding и boilerplate, но: 1) проверяет каждый output, 2) ловит edge cases которые AI пропустил, 3) объясняет вслух свой reasoning, 4) модифицирует AI-код под специфику задачи. НЕ делает: копирует без проверки, просит полное решение, показывает непонимание сгенерированного кода.

> [!question]- Как RESHADED framework для System Design помогает структурировать 45-минутный ответ?
> R (Requirements) -- 5 мин, E (Estimations) -- 3 мин, S (Storage Schema) -- 5 мин, H (High-Level Design) -- 10 мин, A (API Design) -- 5 мин, D (Detailed Design) -- 12 мин, E (Evaluate trade-offs) -- 3 мин, D (Distinctive Additions) -- 2 мин. Framework не даёт "забыть" критические аспекты и показывает structured thinking.

> [!question]- Почему behavioral история без метрик в Result -- провал, даже если Action описан детально?
> Без метрик Result субъективен: "улучшил" -- насколько? "ускорил" -- на сколько? Метрики превращают claim в evidence. Hiring manager не может сравнить "сделал быстрее" vs "сократил с 15 до 3 минут". STAR без R с цифрами = 50% истории потеряно.

---

## Ключевые карточки

12 паттернов DSA -- какие и зачем?
?
Two Pointers, Sliding Window, Fast&Slow, Merge Intervals, Cyclic Sort, In-place Reversal, Tree BFS, Tree DFS, Two Heaps, Subsets/Backtracking, Binary Search, Top K Elements. Покрывают 87% задач на интервью. Pattern-based подход эффективнее grinding.

STAR method -- формула?
?
S (Situation: контекст, 2-3 предложения) + T (Task: ТВОЯ задача) + A (Action: что ТЫ сделал, "I" не "we") + R (Result: измеримый результат с цифрами). Длительность: 2-3 минуты. 7 заготовленных историй покрывают все dimensions.

AI-Enabled Interview -- правила?
?
DO: использовать AI для scaffolding, проверять каждый output, объяснять вслух, модифицировать AI-код. DON'T: копировать без проверки, просить полное решение, показывать непонимание. Verification -- ключевой навык.

Complexity Analysis -- ключевые значения?
?
O(1) hash lookup, O(log n) binary search, O(n) linear scan, O(n log n) efficient sort, O(n^2) nested loops, O(2^n) subsets, O(n!) permutations. На интервью: назвать complexity ДО кодирования, обсудить time vs space trade-offs.

8 dimensions Behavioral Interview?
?
Leadership, Conflict Resolution, Failure/Learning, Teamwork, Ambiguity, Impact, Growth/Mentoring, Customer Focus. Подготовить 7 STAR-историй, каждая покрывает 2-3 dimensions. Company-specific: Google (Googleyness), Amazon (16 LPs), Meta (Move Fast).
...
|------------|------|--------|
| Следующий шаг | DSA паттерны с примерами кода | [[coding-challenges]] |
| Углубиться | Mobile System Design для Android | [[system-design-android]] |
| Смежная тема | Архитектура распределённых систем | [[architecture-distributed-systems]] |
| Обзор | Полный процесс интервью от recruiter до оффера | [[interview-process]] |
