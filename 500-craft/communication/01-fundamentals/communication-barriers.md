...
> **Коммуникационный барьер** — любой фактор, препятствующий точной передаче или интерпретации сообщения между отправителем и получателем. В модели Шеннона—Уивера (1949) барьеры обобщённо описываются как «шум» (noise), однако современная организационная психология выделяет не менее семи самостоятельных категорий.

### Эволюция классификации барьеров

| Период | Автор/школа | Вклад |
|--------|-------------|-------|
| 1948 | Claude Shannon | Введение понятия *noise* как технической помехи в канале связи |
| 1950-е | Wilbur Schramm | Расширение до «semantic noise» — различия в опыте кодирования/декодирования |
| 1966 | Paul Watzlawick et al. | Пять аксиом коммуникации; барьер возникает при нарушении любой из аксиом |
| 1976 | Edward T. Hall | Высококонтекстные vs. низкоконтекстные культуры — культурный барьер как системное явление |
| 1991 | Robbins & Judge | Систематизация организационных барьеров: filtering, selective perception, information overload |
| 2024 | Grammarly State of Business Communication | Количественная оценка: $1.2 трлн ежегодных потерь от miscommunication в США |

### Теория шума Шеннона

Клод Шеннон формализовал шум как случайную величину, добавляемую к сигналу в канале передачи. В контексте человеческой коммуникации Уоррен Уивер предложил расширить это понятие:

> «Проблема коммуникации состоит в воспроизведении в одной точке сообщения, отправленного из другой точки, — точно или приблизительно. Часто сообщения имеют значение, но это не релевантно инженерной проблеме.» — Shannon, 1948

Уивер, однако, показал, что *семантический* и *прагматический* уровни шума не менее критичны. В организационном контексте Robbins & Judge (Organizational Behavior, 17th ed., 2017) системно описали **filtering** как намеренное искажение информации отправителем и **selective perception** как ненамеренное искажение получателем — два фундаментально разных механизма с разными стратегиями преодоления.

### Пять аксиом Watzlawick

Пол Вацлавик, Джанет Бивин и Дон Джексон в работе «Pragmatics of Human Communication» (1967) сформулировали пять аксиом, нарушение каждой из которых создаёт коммуникационный барьер:

| Аксиома | Суть | Барьер при нарушении |
|---------|------|---------------------|
| Невозможность не коммуницировать | Любое поведение — сообщение | Молчание интерпретируется как «согласие» или «игнорирование» |
| Контент и отношения | Каждое сообщение несёт два уровня | Конфликт уровней: слова говорят одно, тон — другое |
| Пунктуация | Участники по-разному «начинают» цепочку | «Ты начал первый» — конфликт из-за разной точки отсчёта |
| Цифровой и аналоговый | Вербальное (точное) + невербальное (многозначное) | Противоречие между словами и жестами |
| Симметрия и комплементарность | Равноправие vs. иерархия | Борьба за статус блокирует содержание |

Связь с другими темами vault'а: [[communication-models]] описывает саму механику передачи, а данный файл — что именно эту механику нарушает. [[cultural-dimensions]] углубляет тему культурных барьеров через модели Хофстеде и Холла.
...
|---|----------|-----|-----------|
| 1 | Shannon C.E., Weaver W. The Mathematical Theory of Communication. University of Illinois Press, 1949 | Монография | Понятие шума, три уровня коммуникационных проблем |
| 2 | Watzlawick P., Beavin J., Jackson D. Pragmatics of Human Communication. W.W. Norton, 1967 | Монография | Пять аксиом коммуникации, типы нарушений |
| 3 | Hall E.T. Beyond Culture. Anchor Books, 1976 | Монография | High/low context, культурные барьеры |
| 4 | Robbins S.P., Judge T.A. Organizational Behavior, 17th ed. Pearson, 2017 | Учебник | Filtering, selective perception, information overload |
...
> [!question]- Почему иерархический filtering опаснее остальных барьеров для принятия решений на уровне CEO?
> Потому что filtering накапливается на каждом уровне передачи: dev смягчает для lead, lead — для manager, manager — для CEO. В итоге CEO получает искажённую картину ("проект on track" вместо "отстаём на 3 дня") и принимает решения на неверных данных. Остальные барьеры (semantic, physical) обычно локальны — между двумя людьми, а filtering пронизывает всю цепочку и масштабируется с ростом организации. Именно поэтому skip-level 1-on-1 и метрики вместо субъективных отчётов — ключевые контрмеры.

> [!question]- Ты проводишь code review для коллеги из Японии. Он молчит после твоих комментариев. Какие барьеры могут действовать одновременно и как бы ты адаптировал свой подход?
> Одновременно могут действовать: (1) cultural barrier — в высоко-контекстных культурах молчание может означать обдумывание или несогласие, выраженное косвенно; (2) psychological barrier — defensiveness, если feedback воспринят как атака на лицо; (3) selective perception — ты можешь интерпретировать молчание как согласие, хотя это не так. Адаптация: смягчить формулировки ("может быть, стоит рассмотреть..."), дать время на ответ асинхронно, спросить предпочтения по формату feedback, использовать assume positive intent.

> [!question]- Представь: ты внедряешь cognitive load theory из области обучения в практику написания Slack-сообщений. Какие конкретные принципы снижения cognitive load помогут преодолеть information overload?
> Из cognitive load theory применимы: (1) chunking — разбивать сообщение на блоки (WHAT / SINCE / IMPACT / NEED); (2) снижение extraneous load — убирать лишние детали, оставлять только релевантное для получателя; (3) signaling — использовать визуальные маркеры (emoji severity, bold ключевых слов, @mention нужных людей) для направления внимания; (4) BLUF (Bottom Line Up Front) работает как advance organizer — даёт schema для восприятия остальной информации. Это прямое пересечение науки об обучении и рабочей коммуникации.

> [!question]- В чём разница между filtering и selective perception, и почему для борьбы с ними нужны разные стратегии?
> Filtering — это намеренное искажение на стороне отправителя (sender выбирает что сказать и как смягчить). Selective perception — ненамеренное искажение на стороне получателя (receiver "слышит" через свои biases и ожидания). Стратегии разные, потому что причины разные: для filtering нужно создать psychological safety и культуру, где честность не наказывается (системное решение). Для selective perception нужно тренировать навыки: paraphrasing, assume positive intent, seek disconfirming evidence (индивидуальное решение). Одно лечится культурой, другое — осознанностью.

---

## Ключевые карточки

7 категорий коммуникационных барьеров?
?
Physical (физические помехи), Semantic (языковые/терминологические), Psychological (ментальные состояния), Cultural (культурные различия), Organizational (структурные), Technological (технические), Perceptual (восприятия — filtering, selective perception).

Что такое filtering в коммуникации и чем он опасен?
?
Filtering — намеренное изменение информации отправителем перед передачей. Опасен тем, что накапливается на каждом уровне иерархии: dev → lead → manager → CEO, в итоге руководство принимает решения на искажённых данных. Контрмеры: psychological safety, skip-level 1-on-1, метрики вместо субъективных отчётов.

Чем selective perception отличается от filtering?
?
Filtering — намеренное искажение отправителем (sender решает что сказать). Selective perception — ненамеренное искажение получателем (receiver "слышит" через свои biases и ожидания). Filtering лечится культурой безопасности, selective perception — осознанностью и техниками: paraphrase, assume positive intent, seek disconfirming evidence.

Какой канал выбрать: async text, sync chat, или video call?
?
High complexity/emotion — video/voice call (нужен tone of voice). Medium — sync chat с follow-up записью. Low complexity/emotion — async text достаточно. Правило: чем выше эмоциональная нагрузка или сложность темы, тем богаче канал.

Что такое Curse of Knowledge и как его преодолеть?
?
Curse of Knowledge — автоматическое предположение, что другие знают то же, что и ты. Проявляется в использовании jargon, пропуске контекста, фразах "это очевидно". Преодоление: объясняй аббревиатуры, не предполагай контекст, спрашивай "Знаком ли ты с X?", используй аналогии из повседневной жизни.

BLUF-шаблон для urgent Slack-сообщения?
?
[EMOJI + SEVERITY]: One-line summary, затем блоки: WHAT (конкретная проблема), SINCE (когда началось), IMPACT (бизнес-эффект), NEED (@кто — что нужно), Links (dashboard, logs). Bottom Line Up Front — главное в первой строке.

Шаблон honest reporting без filtering?
?
ФАКТ: что происходит без sugar-coating. ПРИЧИНА: почему — факты, не оправдания. ВАРИАНТЫ: 2-3 решения с trade-offs. РЕКОМЕНДАЦИЯ: что предлагаешь. ВОПРОС: что нужно от получателя. Ключ — структура снимает страх "как сказать плохое".

Как адаптировать feedback для коллеги из высоко-контекстной культуры?
?
Смягчить формулировки ("может быть, стоит рассмотреть..."), дать пример вместо директивы, спросить мнение ("Что думаешь?"), дать время на асинхронный ответ, спросить предпочтения по формату feedback. Избегать прямолинейного "это плохо, переделай".
...
|-------------|------|-------|
| Модели коммуникации | [[communication-models]] | Понять механику передачи информации — где именно возникают барьеры |
| Активное слушание | [[active-listening]] | Главная техника преодоления perceptual barriers на стороне получателя |
| Культурные различия | [[cultural-dimensions]] | Глубже разобраться в cultural barriers через модели Хофстеде и Мейер |
| Обратная связь | [[giving-feedback]] | Применить знания о барьерах при выдаче feedback — избежать defensiveness |
| Async-коммуникация | [[async-communication]] | Преодоление physical и technological barriers в distributed teams |
| Трудные разговоры | [[conflict-resolution]] | Что делать когда psychological barriers уже привели к конфликту |
| Cognitive load | [[cognitive-load-theory]] | Научная основа information overload — как мозг обрабатывает информацию |
