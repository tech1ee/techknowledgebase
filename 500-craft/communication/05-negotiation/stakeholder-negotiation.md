...
> [!question]- Почему один и тот же запрос на рефакторинг может быть отклонён CTO, но принят после переформулировки?
> Потому что первая формулировка говорит на языке инженера (технический долг, старый код), а вторая — на языке executive (ROI, risk, cost savings). CTO принимает решения через призму бизнес-метрик: стоимость инцидентов, time-to-market, competitive advantage. Переформулировка не меняет суть предложения — она адаптирует message под decision framework получателя, как API contract адаптирует interface под разных consumers.

> [!question]- PM настаивает на 10 features за квартал, а capacity команды — 6 quality features. Какую стратегию negotiation ты выберешь и почему?
> Оптимальная стратегия — scope negotiation через options. Шаг 1: попросить PM проранжировать features по user value. Шаг 2: показать gap (10 vs 6). Шаг 3: предложить варианты — top 6 quality, или top 8 в MVP, или top 6 + 2 с contractor. Ключевое: не говорить "нет" (adversarial), а вместе выбрать лучший trade-off для бизнеса. Это переводит разговор из "engineering vs product" в совместное решение.

> [!question]- Ты Tech Lead и хочешь обосновать investment в автоматизацию перед Engineering Director. Какие элементы из stakeholder negotiation и [[delegation]] / [[engineering-metrics]] помогут построить убедительный case?
> Из stakeholder negotiation: структура PROBLEM → PROPOSAL → ROI → INVESTMENT → RISKS → ASK. Из [[engineering-metrics]] — конкретные цифры: deploy time, test time, hours of toil per week, чтобы quantify проблему. Из [[delegation]] — показать что automation освободит senior engineers для более strategic работы. Вместе это даёт: quantified pain + structured proposal + показ leverage effect на всю команду. Director видит ROI в часах и деньгах, а не абстрактную "нужна автоматизация".

> [!question]- Клиент просит добавить new feature без увеличения бюджета. Проанализируй: какой из трёх вариантов (reduce scope, phased delivery, alternative solution) несёт наибольший risk для отношений с клиентом и почему?
> Наибольший risk — reduce scope, потому что клиент может воспринять это как "забираете то, что обещали". Даже если формально scope не менялся, клиент часто уже mentally committed к полному набору. Phased delivery сохраняет всё обещанное, просто распределяет по времени. Alternative solution может разочаровать качеством. Ключ: при reduce scope критично показать что решение принимает клиент ("Какой approach лучше aligns с вашими priorities?"), а не навязывается вами.

---

## Ключевые карточки

Stakeholder negotiation: главный принцип?
?
Speak Their Language — адаптируй message под stakeholder, не заставляй их адаптироваться к тебе. Executives = ROI/risk, PM = user value/timelines, clients = outcomes/budget.

Что такое Political Capital в контексте stakeholders?
?
Накопленное доверие и влияние. Расходуется при escalations и "крупных просьбах", пополняется через delivered commitments и полезные инициативы. Не трать на мелочи — save для truly important issues.

Как структурировать pitch для CTO (5 элементов)?
?
1) Lead with Business Impact (не с техническим решением). 2) Be Concise — problem, solution, ROI, ask, risks. 3) Anticipate Questions. 4) Offer Options (3 варианта с trade-offs). 5) Clear Ask — конкретное решение + timeline.

PM vs Engineer: что PM хочет от engineering (4 вещи)?
?
1) Predictability — можно ли plan around estimates. 2) Options — не "нет", а "вот что можем". 3) Partnership — совместная работа, не adversarial. 4) Early warning — проблемы рано, не в последний момент.

Scope creep у клиента: три варианта ответа?
?
1) Reduce scope — оставить highest value items в budget. 2) Phased delivery — всё включено, но разнесено по времени. 3) Alternative solution — simpler implementation для дополнительных requests в рамках budget.

Power/Interest Grid: как работать с High Power + High Interest stakeholder?
?
Manage Closely — активно engage и involve в процесс принятия решений. Это ключевые sponsors (CTO, key decision makers). Не просто информировать, а вовлекать и запрашивать input.

Negotiation с PM: как обосновать рефакторинг?
?
Перевести в язык PM: показать что рефакторинг ускоряет feature delivery. Пример: "Сейчас каждая feature = 2 недели. После рефакторинга (1 sprint) = 1 неделя. За квартал: 6 features без рефакторинга vs 9 features с ним."

Ошибка "Complaining без Solutions" — как исправить?
?
Вместо "PM опять нереалистичные deadlines!" сказать: "Вот gap между expectation и reality. Вот три варианта как bridge." Всегда приходи с options, не с жалобами.
...
|-------------|------|-------|
| Основы переговоров | [[negotiation-fundamentals]] | BATNA, ZOPA и базовые концепции — фундамент для stakeholder переговоров |
| Фреймворки переговоров | [[negotiation-frameworks]] | Harvard method, позиционный vs интересный подход — структурирует negotiation |
| Разрешение конфликтов | [[conflict-resolution]] | Когда stakeholders конфликтуют между собой или с командой |
| Executive communication | [[executive-communication]] | Углублённые техники общения с C-level, дополняет раздел про CTO |
| Управление tech debt | [[tech-debt-management]] | Стратегия обоснования и планирования tech debt — частый предмет negotiation |
| Карьерный рост (Staff+) | [[staff-plus-engineering]] | Influence without authority — ключевой навык для Staff/Principal ролей |
| Salary negotiation | [[negotiation]] | Переговоры о compensation — прямое применение stakeholder skills к собственной карьере |
