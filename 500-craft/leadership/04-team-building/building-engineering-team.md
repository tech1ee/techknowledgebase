...
> [!question]- Первый найм определяет культуру
> Основатель стартапа нанял 5 инженеров с одинаковым background — все из FAANG, все backend, все senior. Через год команда страдает от groupthink. Какие конкретные изменения в hiring strategy вы бы предложили, используя Team Composition Framework? Почему diversity of perspectives важнее 'culture fit'?

> [!question]- Two-pizza rule и communication overhead
> Ваша команда выросла до 12 человек. Delivery замедлился, meetings занимают полдня, а code review backlog растёт. Используя stage model из файла, определите на какой стадии находится команда и какие конкретные шаги нужно предпринять для restructuring.

> [!question]- Team Topologies в реальности
> Компания с 30 инженерами решает создать platform team. Но stream-aligned teams жалуются, что platform team не понимает их потребности. Как Team Topologies framework объясняет правильное взаимодействие между этими типами команд? Какие interaction modes должны использоваться?

## Ключевые карточки

Какой оптимальный размер инженерной команды и почему?
?
5-8 человек (Two-pizza rule). Менее 5 — недостаточно capacity, single points of failure. Более 8 — communication overhead растёт экспоненциально (n*(n-1)/2 связей).

Какой recommended skill mix в команде по уровням?
?
1-2 Senior (tech lead, architecture), 3-4 Mid (core execution), 1-2 Junior (fresh perspectives, growth). Ratio rule: 1 senior на 2-3 mid/junior максимум.

Назови 4 типа команд по Team Topologies.
?
Stream-aligned (продуктовые, ~80%), Enabling (помогают другим командам), Complicated-subsystem (сложные подсистемы), Platform (внутренние сервисы).

Что показывает Project Aristotle как #1 фактор эффективности команды?
?
Psychological Safety — уверенность, что можно рисковать, задавать вопросы и признавать ошибки без наказания или унижения.

На сколько лет первые 5 hires определяют культуру команды?
?
На 3-5 лет (по данным First Round Review). Поэтому первый найм важнее любого culture document.

## Куда дальше

| Тип | Ссылка | Описание |
|-----|--------|----------|
| Следующий шаг | [[team-culture]] | Культура как emergent property команды |
| Углубиться | [[team-dynamics]] | Психология команды: Tuckman, конфликты |
| Смежная тема | [[microservices-vs-monolith]] | Архитектура отражает структуру команды |
