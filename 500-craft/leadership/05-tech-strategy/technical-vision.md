...
> [!question]- Vision vs Wishlist
> CTO представляет 'Tech Vision': 'Будем использовать AI, перейдём на микросервисы, внедрим Kubernetes'. Почему это wishlist, а не vision? Используя template из файла, трансформируйте это в proper tech vision с business context, current/target state, principles и non-goals.

> [!question]- Communicating vision разным аудиториям
> Вам нужно представить tech vision о миграции на микросервисы трём аудиториям: CEO, product team и инженерам. Как отличается формат и содержание для каждой аудитории? Какой 'So what?' для каждой группы?

> [!question]- Vision -> Roadmap -> Execution
> Ваш tech vision: 'Независимые deploy cycles для каждой команды через 2 года'. Используя цепочку Vision -> Strategy -> Roadmap -> Execution, разложите этот vision на конкретные yearly themes, quarterly milestones и sprint-level tasks.

## Ключевые карточки

Назови 5 компонентов tech vision.
?
1. Current state (where we are). 2. Target state (where we're going, 2-3 years). 3. Principles (how we decide). 4. Priorities (what first). 5. Non-goals (what we won't do).

Какая цепочка от vision к execution?
?
Vision (2-3 года) -> Strategy (yearly themes) -> Roadmap (quarterly milestones) -> Execution (sprint level). Vision без roadmap — мечты. Roadmap без vision — суета.

Приведи пример guiding principle для tech vision.
?
'Boring technology' — выбираем проверенные, хорошо понятные технологии. Innovation в продукте, не в инфраструктуре. Или 'Buy before build' — при 70%+ fit предпочитаем покупку.

Как часто нужно пересматривать tech vision?
?
Review cadence: quarterly. Vision определяет направление на 2-3 года, но environment changes, поэтому нужно revisit и при необходимости adjust стратегию и priorities.

## Куда дальше

| Тип | Ссылка | Описание |
|-----|--------|----------|
| Следующий шаг | [[architecture-decisions]] | ADR как реализация vision |
| Углубиться | [[strategic-thinking]] | Стратегическое мышление для vision |
| Смежная тема | [[api-design]] | Архитектурные решения в API |
