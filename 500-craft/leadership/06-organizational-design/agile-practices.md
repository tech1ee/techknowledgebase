...
> [!question]- Zombie Scrum диагностика
> Команда проводит все Scrum ceremonies (standup, planning, review, retro), но ничего не улучшается: те же баги, тот же pace, ретро заканчивается без action items. Это 'Zombie Scrum'. Используя список anti-patterns, определите root cause и предложите revitalization plan.

> [!question]- Scrum vs Kanban выбор
> Команда поддержки (bug fixes, customer requests, ops) пытается работать по Scrum со спринтами. Sprint commitment невозможен, потому что приоритеты меняются ежедневно. Почему Kanban лучше подходит для этого контекста?

## Ключевые карточки

Назови 4 ценности Agile Manifesto.
?
...
?
Scrum: fixed sprints, defined roles (PO/SM), changes next sprint, velocity metrics. Kanban: continuous flow, flexible roles, changes anytime, lead time/WIP metrics. Scrum для new products, Kanban для operations/support.

Какие agile practices имеют наивысшую ценность?
?
Retrospectives (самая ценная), daily standups (short!), visual boards, WIP limits, demo/review with stakeholders, regular planning.

## Куда дальше

| Тип | Ссылка | Описание |
|-----|--------|----------|
| Следующий шаг | [[development-process]] | Конкретные процессы разработки |
| Углубиться | [[team-dynamics]] | Динамика команды в agile |
| Смежная тема | [[context-switching]] | WIP limits и cognitive load |
