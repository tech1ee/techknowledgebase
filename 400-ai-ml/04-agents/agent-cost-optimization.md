...
Оптимизация стоимости агентов неразрывно связана с их production deployment. Архитектурные решения при деплое — выбор между stateless и stateful паттернами, конфигурация auto-scaling, использование task queues — напрямую влияют на итоговые расходы. Понимание инфраструктурных затрат (compute, networking, storage) дополняет оптимизацию на уровне LLM API. Без грамотного deployment даже идеально оптимизированные промпты и модели будут работать с избыточными расходами на инфраструктуру.

### [[ai-api-integration]]

Знание особенностей различных LLM API — ценовых моделей, лимитов, batch endpoints — является фундаментом для cost optimization. Каждый провайдер предлагает разные pricing tiers, скидки на batch processing и кэширование промптов. Умение правильно интегрировать API с учётом retry-логики, rate limiting и fallback стратегий помогает избежать непредвиденных расходов. Детальное понимание token counting и billing модели каждого провайдера позволяет точнее прогнозировать costs.

### [[caching-strategies]]

Кэширование — один из самых мощных инструментов снижения стоимости AI-агентов, способный дать экономию до 88% на input tokens. Паттерны prompt caching, semantic caching и tool result caching напрямую заимствуют принципы из классических стратегий кэширования: TTL, LRU eviction, cache invalidation. Понимание общих подходов к кэшированию (write-through, write-behind, cache-aside) помогает выбрать правильную стратегию для конкретного типа агентных запросов.
...
> [!question]- Как правильно комбинировать маршрутизацию моделей и prompt-кэширование для снижения стоимости агента?
> Маршрутизация направляет простые запросы на дешёвые модели (GPT-4o-mini, Claude Haiku), а сложные --- на мощные. Prompt-кэширование сохраняет системные промпты и часто используемые контексты, чтобы не платить за повторную обработку одних и тех же токенов. Вместе они могут снизить стоимость на 60-80%.

> [!question]- Какие метрики нужно отслеживать для оценки cost-efficiency агента?
> Ключевые метрики: стоимость за задачу (cost-per-task), стоимость за токен по каждой модели, hit rate кэша, процент запросов к дешёвым vs дорогим моделям, и качество ответов (чтобы экономия не ухудшила результаты).

> [!question]- Почему наивный подход "всегда использовать самую дешёвую модель" не работает для агентов?
> Агенты выполняют цепочки действий, и ошибка на раннем шаге каскадно ухудшает все последующие. Дешёвая модель может сэкономить на одном вызове, но привести к дополнительным итерациям, retry и исправлениям, что в итоге стоит дороже.

> [!question]- Как batch processing помогает снизить стоимость агентных систем?
> Batch API (например, OpenAI Batch) обрабатывает запросы асинхронно со скидкой 50%. Для задач без требования real-time (аналитика, обработка документов, генерация отчётов) это прямая экономия без потери качества.

---

## Ключевые карточки

Что такое model routing в контексте оптимизации стоимости агентов?
?
Автоматическое направление запросов на разные модели в зависимости от сложности задачи. Простые запросы идут на дешёвые модели (Haiku, GPT-4o-mini), сложные --- на мощные (Opus, GPT-4o).

Какие основные стратегии prompt-кэширования существуют?
?
Кэширование системных промптов (одинаковых для всех запросов), кэширование few-shot примеров, и кэширование контекста разговора. Anthropic и OpenAI предоставляют встроенные механизмы prompt caching.

Как рассчитать ROI от оптимизации стоимости агента?
?
ROI = (стоимость_до - стоимость_после - затраты_на_оптимизацию) / затраты_на_оптимизацию. Учитывать нужно не только прямые затраты на API, но и время разработки, поддержку инфраструктуры кэширования и мониторинга.

Что такое token budgeting для агентов?
?
Установка лимитов на количество токенов, которые агент может использовать для решения одной задачи. Включает ограничения на длину промпта, количество tool calls, и размер контекстного окна.

В чём разница между cost optimization и cost reduction?
?
Cost reduction --- простое сокращение расходов (дешевле модель, меньше вызовов). Cost optimization --- максимизация ценности на единицу затрат. Оптимизация может даже увеличить расходы, если это пропорционально больше увеличивает качество.
...
|-------------|------|-------|
| Следующий шаг | [[agent-production-deployment]] | Применить оптимизации в production-среде |
| Углубиться | [[ai-cost-optimization]] | Общие стратегии оптимизации стоимости AI |
| Смежная тема | [[caching-strategies]] | Паттерны кэширования за пределами AI |
