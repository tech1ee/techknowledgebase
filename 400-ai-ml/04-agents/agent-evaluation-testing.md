...
Тестирование и debugging образуют замкнутый цикл улучшения качества агента. Когда evaluation обнаруживает regression или низкий pass rate, debugging помогает локализовать причину — будь то проблема в промпте, tool selection или state management. Систематические debugging playbooks порождают новые test cases, а CI/CD pipeline с автоматическими eval'ами предотвращает попадание багов в production. Без debugging навыков невозможно интерпретировать результаты тестов и понять, почему агент проваливает определённые сценарии.
...
> [!question]- Чем evaluation агента отличается от тестирования обычного ПО?
> Агенты недетерминированы --- одинаковый вход может дать разные результаты. Нужны вероятностные метрики (pass rate по N прогонам), оценка качества рассуждений (не только финального ответа), и тестирование tool selection. Вместо assert используют scoring functions и LLM-as-judge.

> [!question]- Какие уровни тестирования существуют для агентной системы?
> Unit-уровень: тестирование отдельных tool functions и парсинга ответов. Component-уровень: тестирование prompt + model (правильный ли tool выбирается). Integration-уровень: тестирование полной цепочки с реальными инструментами. System-уровень: end-to-end тестирование пользовательских сценариев.

> [!question]- Как использовать LLM-as-judge для оценки качества агента?
> Отдельная LLM оценивает ответы агента по заданным критериям (correctness, completeness, relevance). Нужен чёткий rubric (шкала оценки), reference answers для калибровки, и мониторинг agreement rate между LLM-judge и human evaluators.

> [!question]- Как построить regression test suite для агента?
> Собирать golden dataset из production traces --- успешные кейсы с подтверждённым качеством. При изменении промпта или модели прогонять dataset и сравнивать метрики. Автоматизировать через CI/CD с порогом допустимой деградации.

---

## Ключевые карточки

Что такое eval dataset для агентов?
?
Набор тестовых сценариев с входными данными, ожидаемыми действиями и эталонными результатами. Включает edge cases, multi-step задачи и adversarial примеры. Обычно 50-200 примеров для базового покрытия.

Какие метрики используются для оценки агентов?
?
Task success rate, tool selection accuracy, average steps to completion, cost per task, latency P50/P95, и factual correctness. Для RAG-агентов добавляют context relevance и faithfulness.

Что такое LLM-as-judge и когда его применять?
?
Использование отдельной LLM для автоматической оценки качества ответов агента. Применяют когда human evaluation слишком дорого для масштабирования. Требует калибровки на 50-100 human-labeled примерах.

Как тестировать multi-step агентов?
?
Проверять каждый промежуточный шаг (не только финальный результат), тестировать recovery от ошибок на промежуточных шагах, и измерять оптимальность пути (количество шагов vs минимально необходимое).

В чём разница между online и offline evaluation?
?
Offline evaluation: прогон на фиксированном dataset до деплоя (regression testing). Online evaluation: мониторинг метрик на реальном трафике после деплоя. Оба необходимы --- offline ловит регрессии, online ловит distribution shift.
