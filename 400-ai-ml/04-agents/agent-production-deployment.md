...
Production deployment и cost optimization находятся в постоянном балансе. Архитектурные решения при деплое — количество реплик, выбор instance types, стратегия auto-scaling — определяют инфраструктурные расходы. Одновременно, паттерны cost optimization (model routing, prompt caching, batch processing) влияют на требования к infrastructure: кэширование требует Redis, batch processing — queue system, model routing — classifier service. Грамотный production deployment минимизирует total cost of ownership, объединяя оптимизацию на уровне LLM API и инфраструктуры.

### [[ai-observability-monitoring]]

Observability — необходимый компонент любого production deployment агентов. Без метрик (latency, error rate, token usage), трейсинга (полные trajectory логи) и структурированного логирования невозможно понять состояние системы, диагностировать проблемы и принимать решения о scaling. Health checks, readiness probes и alerting, описанные в deployment, опираются на observability pipeline. В свою очередь, observability данные используются для HPA (Horizontal Pod Autoscaler) через custom metrics.

### [[kubernetes-basics]]

Kubernetes — основная платформа для production deployment AI-агентов, предоставляющая auto-scaling, self-healing, rolling updates и secret management. Понимание концепций K8s (Deployments, Services, HPA, ConfigMaps, Secrets, Ingress) критично для конфигурирования production-grade агентной системы. Специфика агентов — длительные сессии, непредсказуемое потребление ресурсов, зависимость от внешних API — требует особого подхода к resource limits, timeout настройкам и pod disruption budgets.
...
> [!question]- Какие компоненты инфраструктуры необходимы для production-деплоя агента?
> API gateway (rate limiting, auth), queue система для асинхронных задач, persistent state storage, observability stack (tracing, metrics, logging), fallback/circuit breaker, и autoscaling. Для stateful агентов --- также session storage и checkpointing.

> [!question]- Как обеспечить reliability агента при нестабильности LLM API?
> Retry с exponential backoff, fallback на альтернативную модель/провайдер, circuit breaker для предотвращения cascade failures, timeout на каждый LLM-вызов, кэширование частых запросов, и graceful degradation с partial results.

> [!question]- Какие стратегии масштабирования применяются для агентных систем?
> Horizontal scaling: несколько реплик за load balancer. Queue-based: задачи в очереди, workers обрабатывают параллельно. Autoscaling по метрикам (queue depth, latency). Для long-running агентов --- checkpointing + resumable execution.

> [!question]- Как организовать безопасный rollout обновлений агента?
> Canary deployment: 5-10% трафика на новую версию, мониторинг метрик. A/B testing: сравнение метрик качества между версиями. Shadow mode: новая версия работает параллельно, но результаты не отдаются пользователю. Rollback при деградации метрик.

---

## Ключевые карточки

Какие паттерны деплоя подходят для AI агентов?
?
Synchronous API (для быстрых задач <30 сек), async queue-based (для long-running задач), streaming (для real-time feedback), и webhook-based (для событийных агентов). Выбор зависит от латентности задачи и UX требований.

Зачем нужен checkpointing для агентов?
?
Сохранение промежуточного состояния агента позволяет возобновить работу после сбоя, не начиная с начала. Критично для long-running агентов (минуты-часы), где потеря прогресса --- потеря денег и времени.

Как реализовать rate limiting для агентной системы?
?
Per-user лимиты (запросы/минуту), per-agent лимиты (предотвращение runaway agents), global лимиты на LLM API calls, и token budget per request. Реализуется через API gateway (Kong, Envoy) или middleware.

Что такое circuit breaker в контексте агентов?
?
Паттерн, который отключает вызовы к сервису после N последовательных ошибок. Для агентов: если LLM API или инструмент постоянно fail, circuit breaker переключает на fallback вместо бесконечных retry.

Как мониторить health агентной системы?
?
Liveness (агент запущен), readiness (может принимать задачи), метрики quality (success rate, accuracy), метрики performance (latency, throughput), и метрики cost (tokens/task, API spend). Dashboard + alerts на аномалии.
...
|-------------|------|-------|
| Следующий шаг | [[agent-cost-optimization]] | Оптимизация стоимости в production |
| Углубиться | [[ai-devops-deployment]] | DevOps-практики для AI-систем |
| Смежная тема | [[kubernetes-advanced]] | Оркестрация контейнеров для масштабирования |
