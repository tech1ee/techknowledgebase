...
> **Agentic RAG** — архитектурный паттерн, в котором автономный агент управляет процессом Retrieval-Augmented Generation, принимая решения о необходимости, стратегии и итеративности поиска. Формально: агент реализует политику $\pi(a_t | s_t)$, где действия $a_t \in \{retrieve, refine\_query, evaluate, generate, stop\}$, а состояние $s_t$ включает текущий контекст, историю поисков и оценку качества.

Теоретическая база Agentic RAG объединяет два направления: **информационный поиск (IR)** и **теорию агентов**:

| Направление | Ключевая работа | Вклад в Agentic RAG |
|-------------|----------------|---------------------|
| RAG | Lewis et al. (2020), arXiv:2005.11401 | Формализация retrieval-augmented generation: $P(y|x) = \sum_z P(z|x) \cdot P(y|z,x)$ |
| **Self-RAG** | Asai et al. (2023), arXiv:2310.11511 | Reflection tokens для самооценки: нужен ли retrieval, релевантен ли контекст |
| **CRAG** | Yan et al. (2024), arXiv:2401.15884 | Corrective RAG: оценка и коррекция нерелевантных результатов |
| **Adaptive RAG** | Jeong et al. (2024), arXiv:2403.14403 | Классификация сложности запроса для выбора стратегии |
| **Active RAG** | Jiang et al. (2023), arXiv:2305.06983 | Агент активно решает когда и что искать |
| **HyDE** | Gao et al. (2022), arXiv:2212.10496 | Hypothetical Document Embeddings — генерация гипотезы для поиска |

> **Ключевой сдвиг**: в классическом RAG retrieval — фиксированный шаг pipeline. В Agentic RAG retrieval — **осознанное действие агента**, подчинённое цели максимизации качества ответа. Это переход от **imperative** к **goal-directed** retrieval.

**Эволюция архитектур RAG:**

| Поколение | Паттерн | Принятие решений | Качество (относительно) |
|-----------|---------|-------------------|------------------------|
| Naive RAG | Query → Retrieve → Generate | Нет | Baseline |
| Advanced RAG | + Rewriting + Reranking | Минимальное (rule-based) | +15-20% |
| Modular RAG | Компонентная архитектура | Конфигурируемое | +25-30% |
| **Agentic RAG** | **Агент управляет всем циклом** | **Автономное (LLM-based)** | **+30-50%** |

Теоретическая проблема Agentic RAG — **баланс между exploration и exploitation**: агент должен решить, продолжать ли поиск (exploration) или генерировать ответ на основе имеющегося контекста (exploitation). Это прямая аналогия с multi-armed bandit problem (Robbins, 1952).

См. также: [[rag-advanced-techniques|RAG Advanced]] — техники улучшения retrieval, [[ai-agents-advanced|AI Agents]] — архитектура агентов (ReAct, BDI), [[vector-databases-guide|Vector DBs]] — similarity search.
...
Agentic RAG — это следующая ступень эволюции после Advanced RAG. Если Advanced RAG добавляет query rewriting, reranking и hybrid search к базовому pipeline, то Agentic RAG делает сам pipeline адаптивным — агент решает, нужен ли retrieval, какой query использовать и достаточно ли результатов. Понимание техник Advanced RAG (HyDE, step-back prompting, multi-query) является фундаментом: Agentic RAG использует эти же техники, но выбирает между ними динамически. Без знания базовых RAG паттернов невозможно оценить, когда agentic подход оправдан.

### [[ai-agents-advanced]]

Agentic RAG — это частный случай AI-агента, где основная задача — intelligent information retrieval и synthesis. Архитектурные паттерны агентов (ReAct loop, tool use, planning, self-correction) полностью применимы: retrieval становится tool, grading — reasoning step, query rewriting — planning. Multi-agent RAG с ролями Researcher, Critic и Writer демонстрирует, как multi-agent архитектуры повышают качество retrieval за счёт specialization и проверки на полноту.

### [[vector-databases-guide]]

Vector databases — ключевой компонент инфраструктуры для Agentic RAG, обеспечивающий быстрый similarity search по embeddings. Выбор vector database (Pinecone, Weaviate, Qdrant, Chroma) влияет на latency retrieval шага, а значит и на общую скорость агента. Понимание HNSW алгоритмов, metadata filtering и hybrid search позволяет оптимизировать retrieval quality — что критично, поскольку в Agentic RAG каждый неудачный retrieval trigger'ит дополнительные LLM вызовы для query rewriting и повторного поиска.
...
| # | Источник | Вклад |
|---|----------|-------|
| 1 | Lewis P. et al. (2020). *Retrieval-Augmented Generation for Knowledge-Intensive NLP Tasks*. arXiv:2005.11401 | Оригинальная формализация RAG |
| 2 | Asai A. et al. (2023). *Self-RAG: Learning to Retrieve, Generate, and Critique through Self-Reflection*. arXiv:2310.11511 | Reflection tokens для адаптивного retrieval |
| 3 | Yan S. et al. (2024). *Corrective Retrieval Augmented Generation*. arXiv:2401.15884 | Коррекция нерелевантных результатов |
| 4 | Jeong S. et al. (2024). *Adaptive-RAG: Learning to Adapt Retrieval-Augmented LLMs through Question Complexity*. arXiv:2403.14403 | Классификация запросов для выбора стратегии |
| 5 | Gao L. et al. (2022). *Precise Zero-Shot Dense Retrieval without Relevance Labels*. arXiv:2212.10496 | HyDE — Hypothetical Document Embeddings |
| 6 | Robbins H. (1952). *Some Aspects of the Sequential Design of Experiments*. Bulletin of AMS | Multi-armed bandit — exploration vs exploitation |

### Практические руководства

| # | Источник | Вклад |
|---|----------|-------|
| 1 | [LangGraph — Agentic RAG Tutorial](https://langchain-ai.github.io/langgraph/tutorials/rag/langgraph_agentic_rag/) | Реализация на LangGraph |
| 2 | [LlamaIndex — Agentic RAG](https://docs.llamaindex.ai/en/stable/examples/agent/agentic_rag/) | Реализация на LlamaIndex |
| 3 | [Anthropic — Building Effective Agents](https://www.anthropic.com/engineering/building-effective-agents) | Best practices для агентов |
| 4 | [DeepLearning.AI — Building Agentic RAG with LlamaIndex](https://www.deeplearning.ai/short-courses/building-agentic-rag-with-llamaindex/) | Практический курс |

---

## Проверь себя

> [!question]- Чем agentic RAG отличается от классического RAG pipeline?
> Классический RAG: фиксированный pipeline (query -> retrieve -> generate). Agentic RAG: агент решает когда и как делать retrieval, может переформулировать запрос, обращаться к нескольким источникам, проверять релевантность результатов и делать iterative retrieval. Агент управляет процессом, а не следует фиксированной цепочке.

> [!question]- Что такое Self-RAG и как он улучшает качество ответов?
> Self-RAG --- модель, которая генерирует reflection tokens для самооценки: нужен ли retrieval, релевантен ли контекст, поддержан ли ответ фактами. Это позволяет адаптивно решать когда обращаться к базе знаний и фильтровать нерелевантные результаты, повышая factual accuracy.

> [!question]- Как реализовать corrective RAG для обработки некачественных результатов поиска?
> Corrective RAG оценивает каждый retrieved документ на relevance. Нерелевантные документы отбрасываются, запрос переформулируется и поиск повторяется. При полном отсутствии релевантных результатов агент переключается на web search или сообщает о невозможности ответить.

> [!question]- Какие стратегии query decomposition применяются в agentic RAG?
> Разбиение сложного вопроса на подзапросы (sub-question decomposition), параллельный поиск по каждому подзапросу, step-back prompting (абстрагирование вопроса для широкого поиска), и HyDE (hypothetical document embeddings --- генерация гипотетического ответа для поиска похожих документов).
...
?
Подход, где AI-агент управляет процессом retrieval-augmented generation: решает когда искать, как формулировать запрос, оценивает результаты и может делать несколько итераций поиска. В отличие от классического RAG, процесс адаптивный, а не фиксированный.

Какие типы agentic RAG существуют?
?
Self-RAG (самооценка необходимости поиска), Corrective RAG (коррекция нерелевантных результатов), Adaptive RAG (выбор стратегии под тип запроса), и Multi-source RAG (агент выбирает между несколькими базами знаний и API).

Что такое iterative retrieval в agentic RAG?
?
Агент делает несколько раундов поиска, уточняя запрос на каждой итерации. Первый поиск даёт общий контекст, второй --- детали, третий --- проверку фактов. Останавливается когда качество контекста достаточно для ответа.

Как агент решает, нужен ли retrieval для данного запроса?
?
Классификация запроса: фактический вопрос (нужен retrieval), creative/opinion (не нужен), math/logic (не нужен). Self-RAG использует специальный token для этого решения. Простой подход --- threshold на confidence модели.

В чём преимущества multi-source agentic RAG?
?
Агент выбирает оптимальный источник для каждого подзапроса: vector store для семантического поиска, SQL для структурированных данных, web для актуальной информации, API для real-time данных. Комбинация источников даёт более полные ответы.
...
|-------------|------|-------|
| Следующий шаг | [[rag-advanced-techniques]] | Продвинутые техники RAG |
| Углубиться | [[ai-agents-advanced]] | Архитектура AI-агентов в деталях |
| Смежная тема | [[vector-databases-guide]] | Хранилища для retrieval |
