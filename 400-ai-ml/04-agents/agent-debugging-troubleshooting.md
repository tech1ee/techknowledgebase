...
Debugging и observability — две стороны одной медали. Observability предоставляет данные (traces, metrics, logs), а debugging использует эти данные для нахождения root cause. Без выстроенной observability pipeline отладка агента превращается в гадание — невозможно понять, что именно пошло не так в многошаговом выполнении. Инструменты вроде LangSmith, Arize Phoenix и Datadog LLM, описанные в observability-разделе, являются основными рабочими инструментами для agent debugging.

### [[agent-evaluation-testing]]

Тестирование и отладка агентов тесно переплетены: хорошие тесты предотвращают баги, а при обнаружении бага первый шаг — написать воспроизводящий тест. Trajectory evaluation помогает выявить систематические проблемы в reasoning агента, а regression тесты гарантируют, что исправленный баг не вернётся. Debugging playbooks из этого гайда становятся основой для создания targeted test cases, закрывающих обнаруженные failure modes.

### [[observability]]

Общие принципы observability (structured logging, distributed tracing, metrics exporters) полностью применимы к AI-агентам, но с дополнительной спецификой. Классический observability stack (Prometheus + Grafana + ELK) расширяется специфичными для LLM метриками: token usage, hallucination rate, tool selection accuracy. Понимание фундаментальных концепций observability — correlation IDs, span context propagation, alerting thresholds — необходимо для построения debugging infrastructure для агентов.
...
> [!question]- Как отличить проблему в промпте агента от проблемы в tool execution?
> Если агент выбирает неправильные инструменты или передаёт неверные параметры --- проблема в промпте или reasoning. Если правильный инструмент вызван с правильными параметрами, но результат некорректный --- проблема в tool execution. Tracing помогает разделить эти случаи, показывая каждый шаг решения.

> [!question]- Какие инструменты используются для tracing агентных цепочек?
> LangSmith (для LangChain/LangGraph), Arize Phoenix (open-source), Weights & Biases (ML-ориентированный), и OpenTelemetry (универсальный). Все они визуализируют цепочку вызовов, токены, латентность и ошибки на каждом шаге.

> [!question]- Почему стандартные подходы к дебагу (breakpoints, step-through) плохо работают с агентами?
> Агенты недетерминированы --- один и тот же вход может дать разные цепочки действий. Поведение зависит от стохастического вывода LLM, состояния внешних инструментов и контекста. Вместо breakpoints нужны traces, replay и snapshot-based debugging.

> [!question]- Как организовать систему алертов для агента в production?
> Мониторить: процент failed tool calls, среднюю длину цепочки (loop detection), стоимость за задачу (cost anomaly), латентность P95, и качество ответов (через автоматические eval). Алерты при отклонении от baseline на 2+ стандартных отклонения.

---

## Ключевые карточки

Что такое agent trace и зачем он нужен?
?
Полная запись цепочки действий агента: каждый LLM-вызов, tool call, промежуточные результаты, токены и латентность. Позволяет воспроизвести и проанализировать поведение агента постфактум.

Как обнаружить infinite loop в агенте?
?
Мониторить количество шагов в цепочке и устанавливать max_iterations. Анализировать повторяющиеся tool calls с одинаковыми параметрами. Использовать state comparison между итерациями --- если состояние не меняется, агент зациклился.

Что такое replay debugging для агентов?
?
Сохранение полного состояния агента (промпты, ответы LLM, результаты tool calls) и возможность воспроизвести выполнение детерминированно. Позволяет тестировать изменения промпта на тех же входных данных.

Какие типичные ошибки агентов на уровне reasoning?
?
Hallucination фактов, неправильный выбор инструмента, пропуск необходимых шагов, преждевременное завершение задачи, и чрезмерная декомпозиция (over-planning). Диагностируются через анализ промежуточных рассуждений в trace.

Как настроить fallback стратегию для tool failures?
?
Retry с экспоненциальным backoff для transient ошибок, альтернативные инструменты для функциональных failures, graceful degradation с partial results, и уведомление пользователя при критических сбоях.
...
|-------------|------|-------|
| Следующий шаг | [[agent-evaluation-testing]] | Систематическое тестирование агентов |
| Углубиться | [[ai-observability-monitoring]] | Глубокий мониторинг AI-систем |
| Смежная тема | [[observability]] | Общие практики наблюдаемости систем |
