...
> **AI Engineering** (инженерия искусственного интеллекта) — прикладная инженерная дисциплина, занимающаяся проектированием, разработкой и эксплуатацией программных систем, использующих предобученные модели искусственного интеллекта (foundation models) в качестве ключевого вычислительного компонента (Huyen, 2024).

Термин был формализован Чип Хуен в книге *"AI Engineering: Building Applications with Foundation Models"* (O'Reilly, 2024), где она провела чёткое разграничение между **ML Engineering** (создание моделей) и **AI Engineering** (интеграция моделей в приложения).

### Историческая таксономия ролей

| Период | Доминирующая роль | Основная задача | Ключевой навык |
|--------|-------------------|-----------------|----------------|
| 2000-2010 | Data Analyst | Отчёты, SQL-запросы | Excel, SQL |
| 2010-2015 | Data Scientist | Предиктивные модели | Статистика, R/Python |
| 2015-2020 | ML Engineer | Training pipelines | PyTorch, MLflow |
| 2020-2022 | MLOps Engineer | Model serving | Kubernetes, CI/CD |
| 2022-н.в. | AI Engineer | LLM integration | Prompting, RAG, Agents |

Эта эволюция отражает сдвиг парадигмы, описанный Андреем Карпатым как переход от **Software 1.0** (детерминированный код) к **Software 2.0** (обученные нейросети) и далее к **Software 3.0** (программирование на естественном языке через промпты).

### Формальное разграничение ролей

> **ML Engineer** решает задачу вида: дан датасет D, найти функцию f(x) → y с минимальной ошибкой на валидации. **AI Engineer** решает задачу вида: дана foundation model M, построить систему S, которая решает бизнес-задачу B с приемлемым качеством, стоимостью и латентностью (Huyen, 2024).

Согласно анализу Swyx (Shawn Wang, 2023), предложившего термин "AI Engineer" в статье *"The Rise of the AI Engineer"*, эта роль возникла благодаря двум факторам:
1. **Commoditization inference** — доступ к мощным моделям через API снизил барьер входа
2. **Emergence of prompt engineering** — появление нового интерфейса взаимодействия с моделями

### Теоретическая модель компетенций

По данным исследования IntuitionLabs (2025), анализ 12,000+ вакансий AI/ML выявил следующее распределение требуемых навыков:

| Категория | Доля вакансий | Примеры |
|-----------|---------------|---------|
| NLP / Prompting | 19.7% | Prompt engineering, text generation |
| Computer Vision | 14.2% | Object detection, image classification |
| Cloud Platforms | 33% (Azure), 26% (AWS) | Deployment, scaling |
| Frameworks | 10%+ | [[prompt-engineering-masterclass\|LangChain]], LlamaIndex |
| MLOps | ~15% | CI/CD, monitoring, containerization |

### Связь с Computer Science

AI Engineering опирается на несколько фундаментальных областей CS:
- **Distributed Systems** — оркестрация вызовов к LLM API, обработка отказов, [[api-design|API design]]
- **Information Retrieval** — RAG-системы используют принципы из теории поиска (Robertson & Zaragoza, 2009)
- **Software Engineering** — паттерны проектирования, тестирование, мониторинг
- **Human-Computer Interaction** — проектирование промптов как нового вида UI
...
- Karpathy, A. (2017). *Software 2.0*. Blog post. — Концепция программирования через обучение нейросетей
- Wang, S. "Swyx" (2023). *The Rise of the AI Engineer*. Latent.Space. — Статья, формализовавшая роль AI Engineer
...
> [!question]- Почему AI Engineer не обучает модели с нуля, а использует готовые foundation models?
> Потому что стоимость обучения frontier-модели составляет $100M+, требует тысяч GPU и месяцев работы. AI Engineer фокусируется на интеграции готовых моделей через API, что позволяет создать AI-продукт за дни, а не за год. Это разделение труда аналогично тому, как веб-разработчик не создает браузер, а использует его возможности.

> [!question]- Вы строите корпоративный чат-бот, который должен отвечать на вопросы по внутренней документации компании. LLM не знает этих документов. Какой подход выберете и почему?
> RAG (Retrieval-Augmented Generation). LLM обучена на публичных данных и не знает корпоративную документацию. RAG позволяет искать релевантные документы в vector database и подставлять их в контекст промпта, чтобы модель генерировала ответы на основе реальных данных компании, а не галлюцинировала.

> [!question]- Чем отличаются роли AI Engineer, ML Engineer и Data Scientist на практике?
> AI Engineer интегрирует готовые модели (LLM) в продукты через API, строит RAG и агентов. ML Engineer обучает и деплоит task-specific модели, строит ML-пайплайны. Data Scientist анализирует данные для извлечения инсайтов и предсказаний. Главное различие: AI Engineer использует готовые модели, ML Engineer их создает, Data Scientist интерпретирует данные.

> [!question]- Какие навыки наиболее востребованы для AI Engineer по данным анализа вакансий 2025 года?
> NLP и prompting (19.7% вакансий), Python (90%+ инструментов), LangChain/LlamaIndex (10%+ вакансий), облачные платформы (Azure 33%, AWS 26%), RAG и vector databases. Математика глубокого уровня и PhD не требуются, в отличие от ML Engineer.
...
AI Engineering -- это...?
?
Дисциплина на стыке software engineering и AI, фокусирующаяся на интеграции готовых foundation models (GPT-4, Claude, Llama) в production-приложения. AI Engineer не создает модели с нуля, а строит RAG-системы, чат-ботов и AI-агентов.

Что такое RAG и зачем он нужен?
?
Retrieval-Augmented Generation -- техника, при которой LLM получает доступ к внешним данным через поиск в vector database. Нужен потому, что LLM не знает корпоративную документацию и актуальные данные, а RAG позволяет генерировать ответы на основе найденных документов.

Чем AI-агент отличается от обычного чат-бота?
?
AI-агент может выполнять действия: вызывать API, искать в интернете, выполнять код, отправлять email. Обычный чат-бот только генерирует текст на основе контекста. Агент использует tools и принимает решения о следующем шаге.

Какие три главных ограничения LLM?
?
Галлюцинации (уверенная генерация неверной информации), зависимость от провайдеров (pricing, rate limits, политики), быстрое устаревание знаний (knowledge cutoff). Решения: RAG, multi-provider архитектура, непрерывное обучение.

Какова средняя зарплата AI Engineer в США (2025)?
?
$138,986 по Glassdoor, до $217,654 на 90-м перцентиле. В AI-стартапах (OpenAI, Anthropic) -- $530,000-$690,000. Рост вакансий 41.8% год к году.

Какие основные фреймворки для RAG и агентов?
?
LangChain (rapid prototyping, 50K+ интеграций), LlamaIndex (complex data ingestion, 150+ коннекторов), LangGraph (stateful агенты), Haystack (enterprise NLP), CrewAI (multi-agent), DSPy (prompt optimization).

Какой оптимальный roadmap для перехода в AI Engineering?
?
Месяцы 1-2: Python и основы ML. Месяцы 3-4: LLM API и Prompt Engineering. Месяцы 5-6: RAG и Vector Databases. Месяцы 7-8: Агенты, LangChain, deployment. Итого 6-8 месяцев.
...
|-------------|------|-------|
| Следующий шаг | [[llm-fundamentals]] | Понять как работают LLM изнутри -- фундамент для осознанной работы с API |
| Углубиться | [[models-landscape-2025]] | Выбор конкретной модели под задачу -- от GPT-4o до DeepSeek R1 |
| Смежная тема | [[api-design]] | REST API паттерны -- база для интеграции с LLM провайдерами |
