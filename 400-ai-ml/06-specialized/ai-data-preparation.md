...
Качество данных — фундамент RAG pipeline. Стратегия chunking напрямую определяет retrieval accuracy: слишком крупные чанки снижают precision, слишком мелкие — теряют контекст. Semantic chunking, описанный в этом гайде, даёт до +70% accuracy по сравнению с наивным разбиением по символам. Advanced RAG техники (query rewriting, reranking) компенсируют некоторые проблемы с данными, но не могут полностью заменить качественную подготовку.

### [[ai-fine-tuning-guide]]

Подготовка данных для fine-tuning требует иного подхода, чем для RAG: нужны instruction-response пары определённого формата, а не документные чанки. Синтетические данные, генерируемые через LLM, позволяют масштабировать создание training set при ограниченном количестве реальных примеров. Data quality метрики и валидация, описанные в этом гайде, применимы к обоим сценариям — и для RAG chunking, и для fine-tuning datasets.

### [[ai-testing-evaluation]]

Оценка качества данных неразрывна с evaluation AI систем. Synthetic QA pairs, создаваемые в этом гайде, становятся golden dataset для evaluation pipeline. Метрики RAGAS (faithfulness, answer relevancy, context precision) напрямую зависят от качества chunking и data preparation. Cycle "prepare data → evaluate → improve data" — основной цикл итерационного улучшения AI-приложений.

### [[tutorial-rag-chatbot]]

Практический tutorial по RAG-чатботу использует все концепции data preparation из этого гайда: chunking стратегии для обработки документов, embedding модели для векторизации чанков, metadata extraction для filtering. Этот гайд даёт теорию и best practices, а tutorial показывает их применение в реальном проекте от загрузки документов до production-ready системы.
...
> [!question]- Какие стратегии chunking существуют и когда использовать каждую?
> Fixed-size: простейший, хорош для однородного текста. Recursive: рекурсивное разделение по структуре (абзацы, предложения), универсальный выбор. Semantic: группировка по смыслу через embeddings, лучший для RAG. Document-aware: учитывает структуру документа (заголовки, секции). Для большинства RAG --- semantic chunking.

> [!question]- Как генерировать качественные синтетические данные для fine-tuning?
> Seed examples (10-50 ручных), LLM генерирует вариации с заданными constraints. Фильтрация: deduplication, quality scoring через отдельную LLM, human review выборки (10-20%). Diversity: variation в длине, стиле, сложности. Важно: не fine-tune на данных той же модели, которую обучаешь.

> [!question]- Как измерить качество chunking для RAG-системы?
> Метрики: retrieval precision/recall на eval dataset, context relevance score (оценивает LLM), chunk coherence (standalone understandability), и end-to-end answer quality. A/B тестирование разных стратегий на одинаковых вопросах. Оптимальный chunk size: 256-512 токенов с overlap 50-100.
...
?
Разделение текста на фрагменты по смысловым границам, а не по фиксированному размеру. Используются embeddings для определения семантических переходов: если cosine similarity соседних предложений падает ниже threshold --- это граница chunk. Даёт лучший retrieval для RAG.

Какой оптимальный размер chunk для RAG?
?
256-512 токенов для большинства задач. Overlap: 50-100 токенов (10-20% от chunk). Меньшие chunks (128) --- лучше precision, но хуже context. Большие (1024) --- больше context, но хуже precision. Нужно тестировать на своих данных.

Что такое синтетические данные для AI и когда их использовать?
?
Данные, сгенерированные LLM для обучения других моделей. Используют когда: реальных данных мало, нужны специфические форматы, privacy constraints. Методы: instruction generation, data augmentation, persona-based generation. Качество зависит от seed examples и фильтрации.

Как оценить качество данных для AI-приложений?
?
Dimensions: accuracy (корректность), completeness (полнота), consistency (нет противоречий), freshness (актуальность), diversity (покрытие случаев). Автоматические checks + human review выборки. Data quality метрики должны быть в CI/CD pipeline.

Что такое document parsing и какие инструменты существуют?
?
Извлечение структурированного текста из документов (PDF, DOCX, HTML). Инструменты: Unstructured.io (универсальный), PyMuPDF (PDF), Docling (IBM), LlamaParse (LlamaIndex). Сложности: таблицы, изображения, multi-column layouts. OCR для сканированных документов.
...
|-------------|------|-------|
| Следующий шаг | [[rag-advanced-techniques]] | Применение подготовленных данных в RAG |
| Углубиться | [[embeddings-complete-guide]] | Глубокое понимание embeddings для chunking |
| Смежная тема | [[database-design-optimization]] | Паттерны структурирования данных |
