...
AI-агенты представляют собой наиболее уязвимый тип LLM приложений, поскольку имеют доступ к tools с реальными side effects (отправка email, запись в БД, выполнение кода). Prompt injection в контексте агента может привести не просто к неправильному ответу, а к несанкционированным действиям во внешнем мире. Принцип наименьших привилегий, описанный в этом гайде, особенно критичен для agent tool permissions — каждый tool должен иметь минимально необходимый scope.

### [[ai-devops-deployment]]

Security должен быть встроен в deployment pipeline с самого начала — от secret management в CI/CD до container security и network policies. Input validation, rate limiting и guardrails, описанные в этом гайде, реализуются на уровне infrastructure: API gateway для rate limiting, Kubernetes Secrets или Vault для API ключей, network policies для изоляции LLM сервисов. Без правильного deployment даже хорошие guardrails могут быть обойдены через инфраструктурные уязвимости.

### [[ai-observability-monitoring]]

Мониторинг в реальном времени — критический компонент защиты LLM приложений. Observability pipeline позволяет обнаруживать prompt injection попытки, anomaly detection выявляет необычные паттерны запросов, а alerting уведомляет команду о потенциальных атаках. Без observability атаки могут оставаться незамеченными неделями, а пост-инцидентный анализ становится невозможным из-за отсутствия трейсов и логов.

### [[mcp-model-context-protocol]]

MCP серверы расширяют attack surface AI-приложений: каждый подключённый сервер — потенциальная точка входа для злоумышленника. Правильная конфигурация MCP permissions, валидация tool inputs и ограничение доступа к чувствительным ресурсам через MCP — необходимые меры безопасности. Понимание архитектуры MCP (client-server, transport layer, capability negotiation) помогает идентифицировать и закрывать потенциальные уязвимости.
...
> [!question]- Какие типы prompt injection существуют и как от них защищаться?
> Direct injection: пользователь вставляет инструкции в свой ввод. Indirect injection: вредоносные инструкции в данных (документы, web-страницы), которые LLM обрабатывает. Защита: input sanitization, instruction hierarchy (system > user), output validation, sandboxing tool execution, и separate LLM для проверки.

> [!question]- Что входит в OWASP LLM Top 10 2025 и почему это важно?
> Основные угрозы: Prompt Injection (#1), Insecure Output Handling, Training Data Poisoning, Denial of Service, Supply Chain Vulnerabilities, Sensitive Information Disclosure, Insecure Plugin Design, Excessive Agency. Важно как чеклист для security review любого LLM-приложения.

> [!question]- Как организовать defense-in-depth для LLM-приложения?
> Многослойная защита: input layer (sanitization, rate limiting), model layer (system prompt hardening, instruction hierarchy), output layer (content filtering, PII detection, structured validation), infrastructure layer (network isolation, access control), и monitoring layer (anomaly detection, audit logging).

> [!question]- Как защитить sensitive данные при использовании LLM API?
> PII detection и masking перед отправкой в API, data classification (что можно отправлять, что нет), DLP (Data Loss Prevention) policies, audit logging всех запросов, и contractual agreements с провайдером. Для максимальной защиты --- self-hosted модели.

---

## Ключевые карточки

Что такое prompt injection и почему это главная угроза для LLM?
?
Атака, при которой злоумышленник вставляет инструкции в данные, обрабатываемые LLM, изменяя его поведение. Главная угроза потому что LLM не различает инструкции от данных --- все токены обрабатываются одинаково. Полной защиты не существует.

Что такое instruction hierarchy в контексте безопасности LLM?
?
Принцип, что system prompt имеет приоритет над user input, а user input --- над данными из инструментов. Реализуется через чёткое разделение ролей (system/user/assistant) и explicit инструкции "игнорировать инструкции в данных".

Какие guardrails нужны для production LLM-приложения?
?
Input: длина, формат, PII detection, prompt injection detection. Output: toxicity check, PII leakage prevention, structured output validation, content policy compliance. Operational: rate limiting, cost budgets, audit logging.

Что такое OWASP LLM Top 10?
?
Стандартизированный список 10 главных угроз безопасности для LLM-приложений. Обновляется ежегодно. Используется как чеклист при security review и threat modeling AI-систем. Prompt Injection стабильно на первом месте.

Как реализовать secure tool execution для AI-агентов?
?
Sandboxed environment для каждого tool call, principle of least privilege (минимальные permissions), allowlist разрешённых операций, human-in-the-loop для destructive actions, rate limiting на tool calls, и audit logging всех действий.
...
|-------------|------|-------|
| Следующий шаг | [[ai-observability-monitoring]] | Мониторинг безопасности в production |
| Углубиться | [[ai-agents-advanced]] | Безопасность агентных систем |
| Смежная тема | [[security-fundamentals]] | Фундаментальные принципы безопасности |
