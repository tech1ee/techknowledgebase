...
Что такое AI Agent и из каких компонентов он состоит?
?
AI Agent — это AI-система, способная планировать и действовать автономно. Состоит из четырёх компонентов: LLM (мозг), Tools (инструменты), Memory (память/контекст), Planning (стратегия).

Как работает паттерн ReAct?
?
ReAct (Reasoning + Acting) чередует три шага: Thought (рассуждение) -> Action (вызов инструмента) -> Observation (интерпретация результата). Цикл повторяется, пока агент не получит достаточно информации для финального ответа.

В чём разница между Plan-and-Execute и ReAct?
?
Plan-and-Execute сначала создаёт полный план, затем выполняет шаги — предсказуемый, но негибкий. ReAct итеративно чередует мышление и действие, адаптируясь к результатам — гибкий, но может «застрять» в цикле.

Что такое MCP и какую проблему он решает?
?
MCP (Model Context Protocol) — стандарт от Anthropic для интеграции инструментов с LLM. Решает проблему фрагментации: до MCP каждый фреймворк имел свой формат описания tools. Аналогия: MCP для AI — как USB для устройств.

Какие три паттерна мульти-агентных систем существуют?
?
Sequential (пайплайн: Agent A -> B -> C), Hierarchical (менеджер делегирует подагентам), Collaborative (агенты дебатируют, судья выбирает лучший результат).

Какие меры безопасности обязательны для production-агентов?
?
Iteration limit (защита от бесконечных циклов), cost limit (контроль расходов), tool whitelist (только разрешённые инструменты), sandboxing (изоляция выполнения), human-in-the-loop (подтверждение критичных действий), audit logging.

Чем отличается Function Calling у OpenAI и Anthropic?
?
Оба используют JSON-схему для описания tools, но отличаются форматом: OpenAI использует `tool_calls` в ответе и роль `tool` для результатов. Anthropic использует блоки `tool_use` в content и `tool_result` для результатов.

Какие типы памяти используются в AI-агентах?
?
Buffer Memory (полная история, но растёт без ограничений), Summary Memory (сжатая, но теряет детали), Vector Memory (семантический поиск по релевантности). Лучший подход — гибридный: short-term + long-term + summary.

Когда использовать single agent vs multi-agent?
?
Single agent — для простых задач, ограниченного бюджета и предсказуемости. Multi-agent — когда нужны разные «экспертизы», верификация (агент проверяет агента), параллельная обработка или сложные workflow с ветвлением.
...
|-------------|------|-------|
| Следующий шаг | [[ai-agents-advanced]] | Углублённое изучение агентных архитектур и продвинутых паттернов |
| Углубиться | [[mcp-model-context-protocol]] | Детальное руководство по Model Context Protocol |
| Углубиться | [[agent-frameworks-comparison]] | Сравнение фреймворков: LangChain, CrewAI, AutoGen и др. |
| Смежная тема | [[ai-production-systems]] | Деплой агентов в продакшен: serving, cost, monitoring |
| Смежная тема | [[rag-and-prompt-engineering]] | RAG и промпты — фундамент для агентных систем |
