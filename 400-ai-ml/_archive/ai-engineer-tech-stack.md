...
> [!question]- Почему опытные инженеры в 2024-2025 уходят от LangChain к написанию собственного кода оркестрации, несмотря на 600+ готовых интеграций?
> LangChain создаёт избыточные абстракции, которые скрывают внутреннюю логику вызовов. В production это приводит к трём проблемам: (1) сложно дебажить — ошибка прячется глубоко в стеке, (2) API часто меняется и ломает код при обновлениях, (3) трудно понять, какой именно промпт и контекст отправляется в модель. Свой код даёт полный контроль, прозрачность и отсутствие vendor lock-in — что критично для надёжных production-систем.

> [!question]- У тебя есть RAG-система на Chroma, которая работает в прототипе. Бизнес хочет выкатить в production. Какие конкретные шаги нужны для перехода стека?
> 1. Заменить Chroma на managed-решение (Pinecone) или self-hosted с высокой производительностью (Qdrant/Weaviate) — Chroma не рассчитана на production-нагрузки. 2. Добавить observability (Langfuse или LangSmith) — без логов невозможно диагностировать проблемы пользователей. 3. Обернуть в FastAPI + Docker для деплоя. 4. Перенести API-ключи в секрет-менеджер, не хранить в .env на сервере. 5. Настроить мониторинг стоимости API-вызовов.

> [!question]- Компания обрабатывает медицинские данные пациентов. Почему нельзя просто использовать OpenAI API, и какие компоненты стека придётся заменить?
> Медицинские данные подпадают под регуляторные требования (HIPAA, GDPR и аналоги) — данные не должны покидать контролируемый контур. Нужно заменить: (1) LLM-провайдер — перейти на self-hosted open-source модель (Llama 3.1 через vLLM), (2) векторную БД — self-hosted Qdrant или Milvus вместо облачного Pinecone, (3) inference — развернуть на собственных серверах или в private cloud (AWS с Bedrock может быть вариантом при наличии BAA-соглашения). Суть: весь data path должен оставаться внутри инфраструктуры компании.

> [!question]- В каких случаях fine-tuning оправдан, а в каких это преждевременная оптимизация? Приведи критерий принятия решения.
> Fine-tuning оправдан, когда: модель должна говорить на специфическом языке домена (юридический, медицинский), нужна стабильно высокая точность на узкой задаче, и RAG + prompt engineering уже попробованы и не дают нужного качества. Это преждевременная оптимизация, когда: вы ещё не оптимизировали промпты, не пробовали RAG, или задача достаточно общая. Критерий: сначала RAG + промпт-инжиниринг, измерить качество на тестовых данных, и только если метрики не дотягивают — рассматривать fine-tuning (LoRA/QLoRA как стандартный выбор).

---

## Ключевые карточки

Какие 4 уровня составляют типичный AI-стек?
?
Приложение (UI/API) -> Оркестрация (LangChain/LlamaIndex/свой код) -> Векторные БД + LLM-провайдеры -> Инфраструктура (AWS/GCP/Azure/локальный сервер).

Чем семантический поиск в векторной БД отличается от поиска в обычной БД?
?
Обычная БД ищет по точному совпадению (WHERE title = 'cats'). Векторная БД превращает текст в числовые векторы (embeddings) и ищет по близости в векторном пространстве — запрос "домашние питомцы" найдёт документы про кошек и собак по смысловой близости.

Что означает "8B", "70B", "405B" в названиях open-source моделей, и какие ресурсы нужны для каждого?
?
B = миллиарды параметров. 8B — запускается на одной GPU (RTX 3090, 24GB VRAM). 70B — нужно несколько GPU или облако. 405B — только облако с серьёзными ресурсами. Больше параметров = выше качество, но экспоненциально растут требования к железу.

Когда выбрать open-source модель вместо закрытого API (OpenAI/Anthropic)?
?
Четыре основных причины: (1) критична приватность данных — данные не покидают контур, (2) нужен fine-tuning, (3) большой объём запросов — closed API становится дорого, (4) регуляторные требования. Для большинства задач начинать с закрытых API — проще и быстрее.

Чем LoRA отличается от QLoRA и full fine-tuning?
?
LoRA — обучает только небольшую часть параметров модели (low-rank adapters), стандартный баланс качества и ресурсов. QLoRA — LoRA + квантизация весов, позволяет обучать на меньшем количестве GPU-памяти. Full fine-tuning — обновляет все параметры, требует огромных ресурсов и используется редко.

Зачем нужна LLM-observability и что происходит без неё?
?
Observability — возможность видеть, какие промпты отправляются, какой контекст из RAG подставляется, какие ответы приходят. Без неё при жалобе пользователя "ответ неправильный" невозможно определить причину: плохой промпт, нерелевантный контекст из RAG или галлюцинация модели. Инструменты: LangSmith, Langfuse (open-source), Helicone.

Какой минимальный стек нужен для первого AI-проекта?
?
Python 3.10+ с четырьмя пакетами: openai (API к GPT), chromadb (векторная БД), jupyter (эксперименты), python-dotenv (хранение API-ключей). Бюджет: $5-20 на API-вызовы. Для production добавить FastAPI, Docker и managed векторную БД (Pinecone).
...
|-------------|------|-------|
| Основы профессии | [[ai-engineering-intro]] | Понять роль AI-инженера и отличия от ML-инженера |
| Векторные БД глубже | [[vector-databases-guide]] | Детальное сравнение БД, индексы, оптимизация поиска |
| RAG на практике | [[rag-advanced-techniques]] | Продвинутые техники retrieval: гибридный поиск, re-ranking |
| Агенты | [[ai-agents-advanced]] | Архитектура AI-агентов, инструменты, multi-agent системы |
| Production-деплой | [[ai-devops-deployment]] | CI/CD, контейнеризация, масштабирование AI-сервисов |
| Docker | [[docker-for-developers]] | Контейнеризация для деплоя AI-приложений (кросс-домен: DevOps) |
| Облачные платформы | [[cloud-platforms-essentials]] | AWS/GCP/Azure — инфраструктура для AI-стека (кросс-домен: Cloud) |
