...
В каком порядке выбирать метод адаптации модели?
?
Prompting (быстро, дёшево) -> RAG (актуальные данные, прозрачность) -> Fine-tuning (только если первые два не работают). Fine-tuning — последний resort, не первый выбор.

Что такое LoRA и почему она эффективнее полного fine-tuning?
?
LoRA (Low-Rank Adaptation) добавляет малые обучаемые матрицы (rank decomposition) к замороженным весам модели. Обучается 0.1-1% параметров вместо 100%. Требует в 10-100 раз меньше GPU-памяти, быстрее обучается, адаптер занимает ~100 МБ вместо ~16 ГБ.

Чем QLoRA отличается от LoRA?
?
QLoRA = LoRA + Quantization. Базовая модель квантизируется до 4 бит (NF4), а LoRA-адаптеры обучаются в bfloat16. Позволяет обучать 7B-модель на одной RTX 4090 (24 ГБ), а 70B — на двух A100 (48 ГБ). Качество близко к полному fine-tuning.

Что такое catastrophic forgetting и как его избежать?
?
Catastrophic forgetting — модель забывает оригинальные способности после fine-tuning на узких данных. Как избежать: использовать LoRA (оригинальные веса не меняются), добавлять general data в обучение, регуляризация (weight decay, dropout), меньше эпох, тестировать на общих задачах.

Как выбрать rank (r) для LoRA?
?
Простая задача (формат) — r=8, средняя (стиль) — r=16-32, сложная (домен) — r=64+. Начинай с r=16, увеличивай при underfitting, уменьшай при overfitting. Alpha обычно = 2r (например, r=16, alpha=32).

Сколько примеров нужно для fine-tuning?
?
Минимум 100 примеров для заметного эффекта, 500-1000 — хороший баланс качество/стоимость, 10000+ — для серьёзной адаптации. С двумя примерами будет overfitting. Всегда делать train/validation split.

Когда fine-tuning НЕ поможет?
?
Когда данные часто обновляются (нужен RAG), мало примеров (<100, нужен few-shot prompting), нужна прозрачность (RAG показывает источники), задача требует reasoning (нужна лучшая модель), нужны разные «личности» (prompting гибче).

Fine-tuning меняет ЧТО модель знает или КАК отвечает?
?
Fine-tuning в первую очередь меняет КАК модель отвечает (стиль, формат, тон), а не ЧТО она знает. Для добавления новых знаний лучше подходит RAG, где данные обновляемы и прозрачны.
...
|-------------|------|-------|
| Следующий шаг | [[ai-fine-tuning-guide]] | Расширенное руководство по fine-tuning в production |
| Углубиться | [[llm-inference-optimization]] | Оптимизация инференса fine-tuned моделей |
| Смежная тема | [[rag-and-prompt-engineering]] | RAG — основная альтернатива fine-tuning |
| Смежная тема | [[ai-evaluation-metrics]] | Метрики для оценки результатов дообучения |
| Практика | [[local-llms-self-hosting]] | Self-hosting fine-tuned моделей |
...
**[[ai-ml-overview]]** — Обзор AI/ML-раздела показывает место fine-tuning в общей иерархии AI Engineering: промпт-инженерия → RAG → fine-tuning → pre-training. Fine-tuning — это «последний resort», когда более простые методы не дают нужного результата. Понимание этой иерархии помогает принять правильное решение о необходимости дообучения и избежать преждевременной оптимизации.
