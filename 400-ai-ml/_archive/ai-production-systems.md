...
Что такое Model Routing и зачем он нужен?
?
Model Routing — маршрутизация запросов к разным моделям в зависимости от сложности. Простые запросы -> дешёвая модель (GPT-4o-mini), сложные -> мощная (Claude Opus). Экономит 5-10x на стоимости без заметной потери качества.

Как работает Semantic Caching?
?
Новый запрос -> вычисляем embedding -> ищем похожие запросы в кэше (cosine similarity) -> если similarity > threshold (0.90-0.95), возвращаем кэшированный ответ. Преимущество: «What is Python?» и «Tell me about Python» дают cache hit. Типичный hit rate: 10-30%.

Когда self-hosted выгоднее API providers?
?
Self-hosted лучше при высоком объёме (>1M запросов/месяц), строгих privacy requirements, кастомизации (fine-tuned models), предсказуемой latency. API providers — для быстрого старта, низкого объёма, отсутствия DevOps-экспертизы. Break-even: ~500K-1M запросов/месяц.

Какие метрики критичны для production AI системы?
?
Latency (TTFT <500ms, total <5s), Cost (per query, per user/day), Quality (faithfulness, relevance, user satisfaction), System (error rate <1%, throughput, cache hit rate). Алерты на: P95 latency, error rate spike, quality score drop, cost anomalies.

Что такое TTFT и TPS?
?
TTFT (Time to First Token) — время до первого токена ответа, критично для UX потоковой генерации. TPS (Tokens per Second) — скорость генерации, влияет на общее время ответа. Оба зависят от модели, инфраструктуры и текущей нагрузки.

Как организовать A/B тестирование LLM-систем?
?
Consistent assignment (один пользователь всегда в одном варианте), достаточный размер (минимум 1000 запросов на вариант), статистическая значимость (p-value <0.05). Тестировать: разные модели, промпты, RAG-стратегии, temperature. Логировать всё для post-hoc анализа.

Что такое vLLM и зачем он нужен?
?
vLLM — open-source движок для high-throughput LLM serving. Автоматический batching, PagedAttention для эффективного использования GPU-памяти, поддержка tensor parallelism. Позволяет деплоить модели как OpenAI-совместимый API.

Какие компоненты входят в production AI архитектуру?
?
API Gateway (auth, rate limiting), AI Orchestration Layer (model router, semantic cache, RAG pipeline, prompt manager), LLM backends (API providers + self-hosted), Observability (logging, metrics, traces, evaluation). Каждый слой решает свою задачу надёжности и масштабирования.

Как работает Token Bucket Rate Limiter для LLM API?
?
У каждого пользователя «ведро» с токенами и запросами, которое постепенно пополняется (refill по времени). Запрос проходит, если в ведре достаточно токенов. Защищает от перерасхода и обеспечивает fair use. При переполнении — очередь (RequestQueue) или retry-after.
...
|-------------|------|-------|
| Следующий шаг | [[ai-devops-deployment]] | DevOps-практики для AI-систем |
| Углубиться | [[ai-cost-optimization]] | Детальные стратегии оптимизации стоимости |
| Углубиться | [[llm-inference-optimization]] | Оптимизация инференса LLM |
| Смежная тема | [[ai-observability-monitoring]] | Мониторинг и observability AI |
| Смежная тема | [[ai-evaluation-metrics]] | Метрики качества для production |
| Практика | [[local-llms-self-hosting]] | Self-hosting моделей на своей инфраструктуре |
