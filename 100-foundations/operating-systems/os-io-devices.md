...
> **Подсистема ввода/вывода** — компонент ОС, абстрагирующий разнообразие аппаратных устройств через унифицированный интерфейс (device driver model) и оптимизирующий передачу данных между CPU, памятью и периферией.

### Три модели I/O

| Модель | Механизм | Кто ждёт? | Использование |
|--------|----------|-----------|---------------|
| **Programmed I/O (Polling)** | CPU опрашивает устройство в цикле | CPU занят ожиданием | Простые embedded-системы |
| **Interrupt-driven I/O** | Устройство прерывает CPU по готовности | CPU свободен между прерываниями | Клавиатура, мышь |
| **DMA (Direct Memory Access)** | Контроллер DMA передаёт данные напрямую в RAM | CPU полностью свободен | Диск, сеть, GPU |

### Формальная иерархия I/O

```
User process → System call → VFS → File System → Block layer → Device driver → Hardware
```

Каждый уровень добавляет абстракцию и снижает зависимость от конкретного оборудования.

### «Everything is a file» (Unix philosophy)

Unix формализовал I/O через **файловый интерфейс**: open/read/write/close/ioctl. Устройства доступны через /dev/*. Это позволяет использовать одни и те же системные вызовы для файлов, сокетов, pipe-ов и устройств.

> **См. также**: [[os-file-systems]] — файловые системы, [[os-memory-management]] — memory-mapped I/O, [[os-processes-threads]] — блокировка при I/O
