...
> **Синхронизация** — координация параллельных процессов/потоков для обеспечения корректности при доступе к разделяемым ресурсам. Формализована через задачу **взаимного исключения** (mutual exclusion, Dijkstra 1965).

### Фундаментальные проблемы

| Проблема | Автор | Формулировка |
|----------|-------|-------------|
| **Critical Section** | Dijkstra (1965) | Только один поток в критической секции одновременно |
| **Dining Philosophers** | Dijkstra (1965) | Модель deadlock: 5 философов, 5 вилок |
| **Producer-Consumer** | Dijkstra (1965) | Bounded buffer: синхронизация записи и чтения |
| **Readers-Writers** | Courtois et al. (1971) | Множество читателей ИЛИ один писатель |

### Примитивы синхронизации

| Примитив | Автор | Семантика |
|----------|-------|-----------|
| **Semaphore** | Dijkstra (1965) | Счётчик + wait/signal (P/V операции) |
| **Mutex** | — | Бинарный семафор: lock/unlock |
| **Condition Variable** | Hoare (1974) | wait/signal/broadcast + mutex |
| **Monitor** | Hoare (1974) | Mutex + condition variables в одном объекте |

### Четыре условия deadlock (Coffman et al., 1971)

Deadlock возникает тогда и только тогда, когда **все четыре** условия выполняются одновременно:
1. **Mutual exclusion** — ресурс занят эксклюзивно
2. **Hold and wait** — процесс удерживает ресурс и ждёт другой
3. **No preemption** — ресурс нельзя отнять принудительно
4. **Circular wait** — цикл в графе ожидания

> **См. также**: [[os-processes-threads]] — процессы и потоки, [[os-scheduling]] — планирование, [[concurrency-vs-parallelism]] — модели конкурентности
...
> [!question]- Сервис использует ConcurrentHashMap с 200 потоками, но профилирование показывает высокий lock contention и 40% CPU в BLOCKED состоянии. Какие 3 подхода решат проблему, и какой выберешь первым?
> **1. Шардирование данных:** Разделить данные по independent ключам, каждый шард с отдельным lock — снижает contention в N раз. **2. Read-Write Lock:** Если workload read-heavy (>80% reads), заменить на ReadWriteLock — readers не блокируют друг друга. **3. Lock-free структуры:** Для простых операций (counters, flags) использовать AtomicReference/CAS — нет lock вообще. **Первый шаг:** профилировать с JFR чтобы понять read/write ratio. Если read-heavy → ReadWriteLock. Если write-heavy → шардирование. 40% BLOCKED при 200 потоках часто означает, что потоков слишком много для данной гранулярности lock — снизить до 20-40 потоков тоже поможет.

> [!question]- Объясни, почему futex эффективнее обычного mutex и в каком сценарии разница наиболее заметна.
> Futex (Fast Userspace muTEX) разделяет fast path и slow path. **Fast path (нет contention):** атомарная CAS-операция в userspace (~10-20 CPU cycles, ~5-10 ns) — **никакого syscall**. Mutex всегда требует syscall. **Slow path (есть contention):** syscall futex(FUTEX_WAIT), поток засыпает в kernel wait queue. Разница максимальна при **low contention** — если 95% захватов lock происходят без конкуренции, futex экономит ~100-200 ns на каждый захват. При 1M lock/unlock операций в секунду это 100-200 ms CPU time. В высоконагруженном сценарии (high contention) разница минимальна — оба деградируют к syscall + context switch.

> [!question]- В задаче Dining Philosophers 5 философов берут левую вилку, затем правую — deadlock. Решение "чётные берут левую первой, нечётные — правую" работает. Какое из 4 условий Coffman оно нарушает и почему?
> Нарушает **Circular Wait**. При стандартном подходе образуется цикл: P0→вилка0→P1→вилка1→P2→вилка2→P3→вилка3→P4→вилка4→P0. Асимметричный подход ломает цикл: P0(чёт) берёт left→right, P1(нечёт) берёт right→left. Это эквивалентно lock ordering — вилки получают глобальный порядок (по номеру), и каждый философ захватывает в порядке возрастания номеров. P0 берёт fork0→fork1, P1 берёт fork1→fork2... P4 берёт fork0→fork4 (а не fork4→fork0). Цикл невозможен, потому что всегда есть хотя бы один философ, который ждёт "меньшую" вилку, а она свободна.

> [!question]- Therac-25 убил пациентов из-за race condition. Почему тестирование не обнаружило баг, и какие современные инструменты помогли бы?
> Therac-25 race condition возникал только при быстром вводе оператора (менее 8 секунд между командами) — UI-поток и аппаратный поток конфликтовали при доступе к shared state. Тестирование не обнаружило: (1) race conditions нон-детерминистичны — могут не проявиться в тысячах запусков; (2) тестировщики не вводили команды достаточно быстро; (3) не было stress testing многопоточности. **Современные инструменты:** ThreadSanitizer (TSan) — инструментирует каждый memory access, детектирует data races; Helgrind (Valgrind) — анализирует happens-before отношения; `go test -race` в Go. Также: formal verification (TLA+, SPIN) доказывает корректность concurrent алгоритмов математически, а не тестированием.

---

## Ключевые карточки

В чём разница между mutex и semaphore?
?
**Mutex:** бинарный (locked/unlocked), имеет владельца — только захвативший поток может освободить. Для взаимоисключения (1 поток в критической секции). **Semaphore:** счётчик (0..N), нет владельца — любой поток может сделать signal. Для ограничения параллелизма (N потоков одновременно). Пример: connection pool на 10 соединений → semaphore(10).

Назови 4 условия Coffman для deadlock.
?
Deadlock возможен только при ВСЕХ четырёх одновременно: **1. Mutual Exclusion** — ресурс может использовать только один поток. **2. Hold and Wait** — поток держит ресурс и ждёт другой. **3. No Preemption** — ресурс нельзя отобрать силой. **4. Circular Wait** — цикл ожидания (A→B→C→A). Убери любое одно — deadlock невозможен. Lock ordering нарушает Circular Wait, trylock нарушает Hold and Wait.

Почему condition variable требует проверки в while, а не if?
?
Три причины: **1. Spurious wakeup** — реализация может разбудить поток без signal (особенность pthread/OS). **2. Lost wakeup** — signal мог прийти до wait. **3. Изменившееся условие** — пока поток просыпался, другой поток мог изменить данные. `while(!ready) { wait(); }` — после пробуждения повторно проверяет условие. `if(!ready) { wait(); }` — опасно, продолжит с невалидными данными.

Что такое futex и почему он быстрее syscall-based mutex?
?
Futex (Fast Userspace muTEX) — механизм Linux. **Fast path:** атомарный CAS в userspace (~10 ns), БЕЗ syscall. **Slow path:** syscall futex(FUTEX_WAIT) только при contention. Обычный mutex ВСЕГДА syscall (~100-200 ns). При low contention (95%+ захватов без конкуренции) futex в 10-20x быстрее. В Java synchronized → biased lock → thin lock (CAS) → fat lock (futex) — та же идея escalation.

Когда spinlock лучше mutex?
?
**Spinlock лучше** когда ожидаемое время ожидания < стоимости context switch (~2 µs). Spinlock не вызывает syscall, не усыпляет поток — просто крутится в цикле. Идеален для: очень коротких критических секций (десятки ns), ядра ОС (нельзя sleep в interrupt handler), многоядерных систем (другое ядро быстро освободит lock). **Mutex лучше** для длительных секций: spinlock сжигает CPU cycles впустую.

Что такое priority inversion и как priority inheritance решает проблему?
?
Low-priority T3 держит lock, high-priority T1 ждёт этот lock, medium-priority T2 вытесняет T3 → T1 блокируется неопределённо (инверсия: T2 фактически имеет приоритет выше T1). **Priority inheritance:** когда T1 ждёт lock, ядро временно повышает приоритет T3 до уровня T1. T2 не может вытеснить T3, T3 быстро завершает и отпускает lock, T1 продолжает. Пример: Mars Pathfinder (1997) — решено remote patch с priority inheritance.

Lock-free (CAS) vs Lock-based: когда что использовать?
?
**CAS (Compare-And-Swap):** для простых операций (счётчики, указатели). AtomicInteger, AtomicReference. ~10-30 ns. Нет блокировки, нет deadlock. Но: retry storms при high contention, сложный retry loop для complex state. **Lock-based:** для сложных операций (multi-field updates, invariants). Надёжнее, проще reasoning. ~20-100 ns. При low contention разница минимальна. При high contention оба деградируют — пересмотреть дизайн (шардирование, immutable data).

Readers-Writers problem: почему rwlock не всегда лучше mutex?
?
RWLock позволяет concurrent readers, но: (1) writer starvation — если readers постоянно приходят, writer может ждать вечно; (2) overhead — RWLock сложнее mutex (атомарный счётчик readers + state machine), при коротких критических секциях overhead нивелирует выигрыш; (3) write-heavy workloads — если >30% writes, rwlock не даёт преимущества. RWLock выгоден только при: read-heavy (>90% reads) + длинные критические секции.
...
|-------------|------|-------|
| Следующий шаг | [[os-scheduling]] | Priority inversion и выбор spinlock vs mutex зависят от scheduling |
| Следующий шаг | [[os-processes-threads]] | Потоки разделяют адресное пространство — причина race conditions |
| Углубиться | [[jvm-synchronization]] | synchronized, ReentrantLock, volatile — построены на futex и CAS |
| Углубиться | [[kotlin-coroutines]] | Mutex в корутинах, structured concurrency как альтернатива locks |
| Смежная тема | [[os-memory-management]] | Memory barriers, cache coherency — низкоуровневая основа синхронизации |
| Смежная тема | [[concurrency-vs-parallelism]] | Фундаментальная разница между concurrency и parallelism |
