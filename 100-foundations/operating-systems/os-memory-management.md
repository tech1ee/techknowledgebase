...
> **Виртуальная память** — абстракция, предоставляющая каждому процессу иллюзию собственного непрерывного адресного пространства, отображаемого на физическую память через аппаратную трансляцию адресов (MMU).

### Историческая хронология

| Год | Событие | Значение |
|-----|---------|----------|
| 1961 | **Atlas** (Kilburn, Manchester) | Первая реализация виртуальной памяти (demand paging) |
| 1962 | **Atlas One-Level Store** | Автоматическое перемещение страниц между RAM и drum |
| 1968 | **Denning — Working Set Model** | Формальная модель locality of reference |
| 1969 | **Multics** | Сегментация + paging |
| 1970 | **Belady's Anomaly** | Больше фреймов → больше page faults (для FIFO) |

### Формальная модель paging
...
**TLB (Translation Lookaside Buffer)** — кэш page table в CPU. TLB hit ~1ns, TLB miss ~10-100ns (обход page table).

### Алгоритмы замены страниц

| Алгоритм | Принцип | Оптимальность |
|----------|---------|---------------|
| **OPT** (Belady) | Заменить страницу, которая не понадобится дольше всего | Оптимальный (теоретический) |
| **LRU** | Заменить наименее недавно использованную | Близок к OPT, дорогой |
| **Clock (Second Chance)** | Аппроксимация LRU через reference bit | Практичный, используется в Linux |
| **FIFO** | Заменить самую старую | Прост, но Belady's anomaly |

> **См. также**: [[os-processes-threads]] — процессы и адресные пространства, [[os-file-systems]] — файловая система и swap, [[memory-safety-ownership]] — безопасность памяти
