...
> **Виртуализация** — создание программной абстракции аппаратной платформы, позволяющей запускать несколько изолированных операционных систем на одном физическом сервере. Формализована Popek & Goldberg (1974) через требования к виртуализируемой архитектуре.

### Теорема Popek-Goldberg (1974)

Архитектура эффективно виртуализируема, если:
- **Все чувствительные инструкции (sensitive) являются привилегированными (privileged)**
- Sensitive = инструкции, влияющие на конфигурацию ресурсов или зависящие от неё
- Privileged = инструкции, вызывающие trap в user mode

x86 до VT-x (2005) **не соответствовала** этому требованию → binary translation (VMware) как workaround.

### Два подхода к изоляции

| Аспект | Virtual Machine | Container |
|--------|----------------|-----------|
| Изоляция | Полная (собственное ядро) | Уровень процессов (общее ядро) |
| Overhead | 1-5% CPU, ~256MB+ RAM per VM | ~1-2% CPU, ~10MB per container |
| Startup | Минуты | Секунды |
| Безопасность | Сильная (hypervisor boundary) | Слабее (kernel exploit = escape) |
| Механизм | Hypervisor (VMM) | Namespaces + cgroups (Linux) |

### Историческая хронология

| Год | Событие | Значение |
|-----|---------|----------|
| 1967 | **IBM CP-67** | Первая ОС с виртуальными машинами |
| 1974 | **Popek & Goldberg** | Формальные требования к виртуализации |
| 1999 | **VMware Workstation** | Binary translation для x86 |
| 2003 | **Xen** | Паравиртуализация (open source) |
| 2005 | **Intel VT-x / AMD-V** | Аппаратная поддержка виртуализации |
| 2007 | **KVM** | Виртуализация в ядре Linux |
| 2008 | **Linux namespaces + cgroups** | Основа контейнеризации |
| 2013 | **Docker** | Контейнеры для разработчиков |

> **См. также**: [[docker-for-developers]] — Docker на практике, [[kubernetes-basics]] — оркестрация контейнеров, [[os-processes-threads]] — процессы и изоляция
