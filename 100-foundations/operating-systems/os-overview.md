...
> **Операционная система** — системное ПО, управляющее аппаратными ресурсами компьютера и предоставляющее сервисы для прикладных программ через стандартизированные абстракции (процессы, файлы, виртуальная память).

### Три фундаментальные абстракции ОС

| Абстракция | Что скрывает | Механизм |
|-----------|-------------|----------|
| **Процесс** | Мультипрограммность: один CPU → иллюзия параллельности | Time-sharing, context switch |
| **Виртуальная память** | Ограниченность и фрагментацию RAM | Paging, MMU, page table |
| **Файл** | Физическую организацию диска (блоки, секторы) | Файловая система, inode |

### Ключевые принципы

| Принцип | Автор | Формулировка |
|---------|-------|-------------|
| **Mechanism vs Policy** | Lampson (1969) | Ядро предоставляет механизмы, политика определяется выше |
| **Least Privilege** | Saltzer & Schroeder (1975) | Каждый компонент получает минимум привилегий |
| **Separation of concerns** | Dijkstra (1968) | Каждый слой решает одну задачу |

> **См. также**: [[os-processes-threads]] — процессы и потоки, [[os-memory-management]] — виртуальная память, [[os-fundamentals-for-devs]] — ОС глазами разработчика
