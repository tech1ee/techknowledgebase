...
> **Resilience (устойчивость)** — способность системы продолжать функционировать при наличии сбоев, деградируя gracefully вместо полного отказа. Формализована через паттерны Michael Nygard «Release It!» (2007, 2018).

### Фундаментальные законы

| Закон | Формулировка | Следствие |
|-------|-------------|----------|
| **Fallacies of Distributed Computing** (Deutsch, 1994) | Сеть ненадёжна, латентность ненулевая, пропускная способность конечна | Каждый внешний вызов может сбоить |
| **Murphy's Law** | Всё, что может сломаться — сломается | Проектировать для сбоя, не для успеха |
| **Cascade failure** | Один сбоивший компонент перегружает остальные | Isolation и Circuit Breaker обязательны |

### Формальная модель Circuit Breaker
...
- **Closed**: все запросы проходят, считаются ошибки
- **Open**: все запросы отклоняются немедленно (fast fail)
- **Half-Open**: пропускается один пробный запрос

### Retry с Exponential Backoff

Формула: `delay = base × 2^attempt + random_jitter`

Без jitter: все клиенты повторяют одновременно → **thundering herd**. С jitter: повторы распределяются во времени.

> **См. также**: [[error-handling]] — обработка ошибок, [[architecture-rate-limiting]] — защита от перегрузки, [[event-driven-architecture]] — асинхронная коммуникация как resilience
...
### Теоретические основы
- **Nygard M. (2007/2018). Release It! Pragmatic Bookshelf.** — формализация resilience patterns: Circuit Breaker, Bulkhead, Steady State, Fail Fast
- **Hanmer R. (2007). Patterns for Fault Tolerant Software. Wiley.** — академическая основа fault tolerance паттернов
- **Deutsch P. (1994). The Eight Fallacies of Distributed Computing.** — 8 ложных предположений о сети, мотивация для resilience
