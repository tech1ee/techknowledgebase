...
> [!question]- Почему слепое следование принципу "одна вещь на класс" может навредить проекту?
> Потому что SRP говорит не "одна вещь", а "одна причина для изменения" (один актор/стейкхолдер). Интерпретация "одна вещь" ведёт к explosion of classes — проект из 10 классов превращается в 100 микроклассов, каждый по 5 строк. Их совокупная сложность (навигация, связи, именование) превышает сложность исходного кода. Правильный вопрос: "Сколько разных людей/команд попросят изменить этот класс?"

> [!question]- Чем отличается Dependency Inversion от Dependency Injection, и когда DIP не нужен?
> DIP — это принцип: высокоуровневые модули зависят от абстракций, а не от конкретных реализаций. DI — это механизм передачи зависимостей (через конструктор, фабрику, контейнер). DIP не нужен, когда реализация единственная навсегда (Math.random, String), или когда тестирование не требует подмены зависимости. Создание интерфейса для каждого класса без реальной необходимости только усложняет навигацию и понимание кода.

> [!question]- Представьте: вы делаете MVP за неделю. Какие из принципов SOLID вы применили бы, а какие отложили? Почему?
> На MVP стоит применить SRP на уровне функций (чтобы код хотя бы читался) и DIP для внешних зависимостей (API, БД) — это упростит тестирование и смену провайдеров. OCP, ISP и LSP можно осознанно отложить, зафиксировав технический долг. Причина: MVP проверяет гипотезу, а не архитектуру. Если гипотеза провалится, идеальная архитектура не спасёт проект. Ключ — осознанный долг с планом рефакторинга.

> [!question]- Почему комментарий "что делает код" — обычно признак плохого кода, а "почему так сделано" — нужный комментарий?
> Комментарий "что" сигнализирует, что код непонятен сам по себе — его нужно переписать с понятными именами. Комментарий "почему" объясняет контекст, который нельзя выразить в коде: бизнес-решение, ссылку на баг, неочевидное ограничение. Код отвечает на "как", имена — на "что", и только комментарии могут ответить на "почему именно так".

---

## Ключевые карточки

Что такое SRP в точной формулировке Robert C. Martin?
?
Класс должен иметь одну и только одну причину для изменения. "Причина" = актор (стейкхолдер), чьи требования вызывают изменение. Не "одна вещь", а "один актор".

Чем отличается Open/Closed Principle Мейера от версии Мартина?
?
Meyer (1988) предлагал расширение через наследование реализации. Martin (1996) — через абстракции и полиморфизм (интерфейсы). Современный подход Мартина предпочтительнее, т.к. наследование создаёт tight coupling.

Что проверяет LSP — синтаксис или семантику?
?
Семантику (поведение). Компилятор проверяет синтаксическую совместимость типов, но LSP требует behavioral subtyping: подтип не должен нарушать preconditions, postconditions и invariants базового типа.

Почему ISP говорит не о размере интерфейса, а о связях между клиентами?
?
Fat interface создаёт inadvertent coupling: изменение метода для одного клиента вынуждает перекомпилировать (или реализовывать) метод для другого клиента, который этот метод не использует. Правильное дробление — по ролям клиентов, а не по количеству методов.

В чём разница между DIP (принцип) и DI (механизм)?
?
DIP — принцип проектирования: зависимости должны указывать на абстракции, а не на конкретные реализации. DI — конкретный механизм передачи зависимостей через конструктор, метод или контейнер. DI реализует DIP, но DIP можно соблюдать и без DI-контейнера.

Когда God Class допустим?
?
В early-stage prototyping при маленькой команде, быстро меняющихся требованиях и приоритете проверки идеи. Условие: признать это временным решением и запланировать рефакторинг после стабилизации продукта.

Что случилось с Knight Capital в 2012 году и какой принцип чистого кода был нарушен?
?
Компания потеряла $440M за 45 минут. Нарушения: мёртвый код не был удалён, флаг переиспользовали для новой функции, отсутствовал code review при деплое. Это пример накопленного технического долга и нарушений Clean Code.

Какой порядок приоритетов при написании кода?
?
1) Работает. 2) Правильно. 3) Быстро. 4) Красиво. Сначала рабочий код, потом корректность, затем производительность, и только потом эстетика.
...
|-------------|------|-------|
| Следующий шаг | [[design-patterns]] | Паттерны GoF — практические реализации принципов SOLID |
| Углубиться | [[refactoring-techniques]] | Техники безопасного улучшения существующего кода |
| Смежная тема | [[android-architecture]] | Как SOLID применяется в архитектуре Android-приложений |
