...
> **Build system** — инструмент, автоматизирующий трансформацию исходного кода в исполняемые артефакты. Формально: build = применение DAG (directed acyclic graph) зависимостей между задачами с минимальным пересчётом при инкрементальных изменениях.

### Формальная модель сборки

Build system оперирует **DAG зависимостей**:
- **Узлы** = задачи (compile, link, test, package)
- **Рёбра** = зависимости (task B зависит от task A)
- **Инвариант**: DAG = нет циклов → существует топологический порядок выполнения

### Эволюция build-систем

| Год | Система | Ключевая инновация |
|-----|---------|-------------------|
| 1976 | **Make** (Feldman, Bell Labs) | Правила на основе файлов + timestamps |
| 2000 | **Ant** (Apache) | XML-описание, кроссплатформенность |
| 2004 | **Maven** | Convention over configuration + dependency management |
| 2007 | **Bazel** (Google, внутр. как Blaze) | Герметичные сборки, distributed cache |
| 2012 | **Gradle** | Декларативный DSL + инкрементальная сборка |
| 2023 | **Gradle Build Cache** | Remote cache, configuration cache |

### Три модели инвалидации

| Модель | Как определяет «устарело» | Пример |
|--------|--------------------------|--------|
| **Timestamp** | Время модификации файла | Make |
| **Content hash** | SHA-256 содержимого | Bazel, Gradle |
| **Task input/output** | Хэш всех входов задачи | Gradle (up-to-date checks) |

> **См. также**: [[dependency-resolution]] — разрешение зависимостей, [[module-systems]] — модульность, [[ci-cd-pipelines]] — CI/CD
...
Что такое incremental build и зачем он нужен?
?
Incremental build — стратегия сборки, при которой пересобираются ТОЛЬКО изменившиеся части проекта. Gradle отслеживает inputs/outputs каждого task: если inputs не изменились, task помечается UP-TO-DATE и пропускается. Это сокращает время сборки с минут до секунд для локальных изменений.

Чем Configuration phase отличается от Execution phase в Gradle?
?
Configuration phase — выполняет build.gradle.kts, строит DAG tasks, НЕ выполняет task actions. Execution phase — выполняет запрошенные tasks в порядке зависимостей. Тяжёлые операции (чтение файлов, сетевые вызовы) в Configuration phase замедляют КАЖДЫЙ запуск Gradle, даже `./gradlew help`.

Почему task без объявленных inputs/outputs не кэшируется?
?
Gradle определяет UP-TO-DATE/FROM-CACHE статус по хешам inputs и outputs. Без объявленных outputs Gradle не знает, что считать результатом работы, и не может определить, нужно ли перевыполнять task. Такой task выполняется КАЖДЫЙ раз, что убивает производительность сборки.

Что такое remote build cache и какую проблему он решает?
?
Remote build cache — общий кэш результатов tasks, доступный всей команде и CI. Когда один разработчик (или CI) собирает модуль, результат сохраняется на сервере. Другие разработчики получают готовый результат из кэша вместо повторной сборки. Это ускоряет CI и локальную разработку в разы.

Что такое Version Catalog в Gradle и зачем он нужен?
?
Version Catalog (libs.versions.toml) — централизованное место для объявления версий всех зависимостей. Решает проблему дублирования: вместо повторения "kotlinx-coroutines:1.8.0" в каждом модуле, версия объявляется один раз в каталоге и используется как `libs.coroutines.core`. Предотвращает конфликты версий между модулями.

Чем Make принципиально отличается от Gradle?
?
Make (1976) — императивный, работает с файлами и timestamps, нет dependency management, разработчик описывает "как" собирать. Gradle (2007) — декларативный DSL с Kotlin/Groovy, smart caching (content hashing вместо timestamps), incremental build, встроенный dependency management (Maven Central), daemon для тёплой JVM, параллельное выполнение tasks.

Что такое convention plugin и зачем выносить конфигурацию в buildSrc?
?
Convention plugin — переиспользуемый набор настроек Gradle в buildSrc или отдельном проекте. Решает проблему монолитного build.gradle.kts (1000+ строк): общая конфигурация (plugins, dependencies, tasks) выносится в именованный плагин (`id("my-kotlin-library")`), который применяется одной строкой. Обеспечивает консистентность между модулями.
...
|-------------|------|-------|
| Следующий шаг | [[dependency-resolution]] | Как build system разрешает конфликты транзитивных зависимостей |
| Углубиться | [[android-gradle-fundamentals]] | Gradle в контексте Android — плагины, build variants, signing |
| Смежная тема | [[ci-cd-pipelines]] | Build system как ядро CI/CD pipeline: автоматизация сборки и деплоя |
