...
> **Dependency resolution** — процесс определения совместимого набора версий пакетов, удовлетворяющего всем объявленным ограничениям. Формально: это задача **constraint satisfaction** (SAT), которая в общем случае NP-полна.

### Формальная модель

Задача разрешения зависимостей:
- **Дано**: граф зависимостей G = (V, E), где V = пакеты, E = зависимости с version constraints
- **Найти**: подмножество V' ⊂ V (по одной версии каждого пакета), удовлетворяющее всем constraints
- **Сложность**: NP-complete в общем случае (сводится к SAT)

### Стратегии разрешения конфликтов

| Стратегия | Описание | Кто использует |
|-----------|----------|---------------|
| **Newest wins** | При конфликте выбирается новейшая версия | Gradle, Maven |
| **Nested** | Каждый пакет получает свою копию зависимости | npm (node_modules) |
| **Flat/Hoisting** | Дедупликация: общая версия поднимается на верхний уровень | npm (с hoisting), pnpm |
| **Lock file** | Точные версии фиксируются для воспроизводимости | Gradle (.lock), npm (package-lock.json) |

### Semantic Versioning (SemVer)

Tom Preston-Werner (2010) формализовал схему версионирования:
```
MAJOR.MINOR.PATCH
  ↑      ↑      ↑
...
**Diamond dependency problem**: A зависит от B и C, оба зависят от D (разные версии) → конфликт, решаемый стратегией.

> **См. также**: [[build-systems-theory]] — системы сборки, [[module-systems]] — модульность
