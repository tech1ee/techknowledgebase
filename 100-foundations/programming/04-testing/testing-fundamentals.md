...
> **Тестирование программного обеспечения** — процесс верификации и валидации: проверка, что программа соответствует спецификации (verification: «build the thing right») и удовлетворяет потребностям пользователя (validation: «build the right thing»).

### Фундаментальная теорема тестирования

Edsger Dijkstra (1970): «Testing shows the presence, not the absence of bugs». Тестирование может доказать наличие дефектов, но не может доказать их отсутствие — это фундаментальное ограничение.

### Test Pyramid (Cohn, 2009)

Mike Cohn формализовал иерархию тестов:

```
        /  E2E  \         Медленно, дорого, хрупко (5-10%)
       / Integration \     Средне (15-25%)
      /    Unit Tests  \   Быстро, дёшево, стабильно (65-80%)
```

| Уровень | Что проверяет | Скорость | Хрупкость |
|---------|--------------|----------|-----------|
| **Unit** | Одна функция / класс в изоляции | ms | Низкая |
| **Integration** | Взаимодействие модулей | s | Средняя |
| **E2E** | Полный путь пользователя | min | Высокая |

### Формальные свойства хорошего теста (FIRST)

| Свойство | Расшифровка | Следствие |
|----------|-------------|----------|
| **F**ast | Быстрый | Тесты запускаются после каждого изменения |
| **I**ndependent | Независимый | Порядок запуска не влияет на результат |
| **R**epeatable | Воспроизводимый | Одинаковый результат на любом окружении |
| **S**elf-validating | Самопроверяемый | Результат: pass/fail, без ручной интерпретации |
| **T**imely | Своевременный | Написан вовремя (до или с кодом) |

> **См. также**: [[tdd-practice]] — TDD как метод разработки, [[mocking-strategies]] — подмена зависимостей, [[code-smells]] — код, трудный для тестирования
...
### Теоретические основы
- **Dijkstra E. (1970). Notes On Structured Programming.** — «Testing shows the presence, not the absence of bugs» — фундаментальная теорема тестирования
- **Cohn M. (2009). Succeeding with Agile. Addison-Wesley.** — Test Pyramid: Unit → Integration → E2E (оригинальная формулировка)
- **Myers G. (1979). The Art of Software Testing. Wiley.** — классическая книга: «Testing is the process of executing a program with the intent of finding errors»
