...
> **SOLID** — пять принципов проектирования классов, направленных на создание систем, устойчивых к изменениям. Каждый принцип формализует конкретный аспект управления зависимостями и связанностью в объектно-ориентированных системах.

### Формальные определения

| Принцип | Формальное определение | Математическая аналогия |
|---------|----------------------|------------------------|
| **SRP** | Модуль должен иметь одного и только одного актора (stakeholder), ответственного за изменения | Функциональная декомпозиция: один модуль = одна функция |
| **OCP** | Модуль открыт для расширения (новое поведение) и закрыт для модификации (существующий код) | Полиморфизм подтипов: новые реализации без изменения абстракции |
| **LSP** | Если S — подтип T, то объекты T можно заменить объектами S без нарушения свойств программы | Поведенческая совместимость: pre(T) ⊇ pre(S), post(S) ⊇ post(T) |
| **ISP** | Клиент не должен зависеть от методов, которые не использует | Минимизация интерфейса: interface = minimal cohesive set |
| **DIP** | Модули высокого уровня не зависят от модулей низкого уровня; оба зависят от абстракций | Инверсия направления зависимости в графе |

### LSP: формальная модель Liskov-Wing (1994)

Барбара Лисков и Джанетт Винг формализовали принцип подстановки через **контрактное программирование**:

```
Для подтипа S <: T должно выполняться:
1. Contravariance of preconditions:  pre(T.m) ⊇ pre(S.m)   — подтип принимает ≥ того, что базовый тип
2. Covariance of postconditions:     post(S.m) ⊇ post(T.m)  — подтип гарантирует ≥ того, что базовый тип
3. History constraint:               S не нарушает инвариантов T
```

Это связано с [[variance-covariance|ковариантностью и контравариантностью]] в системах типов.

### DIP как инверсия графа зависимостей

Без DIP: зависимости идут сверху вниз → высокоуровневая политика зависит от низкоуровневых деталей.
С DIP: вводится абстракция → обе стороны зависят от неё → **направление зависимости инвертируется**.

```
Без DIP:          Business Logic ──────▸ Database
С DIP:            Business Logic ──▸ Repository ◂── DatabaseImpl
                                   (абстракция)
```

### Связь SOLID с другими принципами

| SOLID | Связь | Другой принцип |
|-------|-------|---------------|
| SRP | ≈ | High Cohesion (GRASP) |
| OCP | реализуется через | Strategy / Template Method (GoF) |
| LSP | формализует | Design by Contract (Meyer 1988) |
| ISP | следствие | Interface Segregation ≈ минимальное сцепление |
| DIP | основа | Dependency Injection, IoC |

> **См. также**: [[oop-fundamentals]] — основы ООП, [[coupling-cohesion]] — связанность и сцепление, [[design-patterns-overview]] — паттерны как реализация SOLID
...
### Теоретические основы
- **Liskov B., Wing J. (1994). A Behavioral Notion of Subtyping. ACM TOPLAS.** — формализация LSP через пред/постусловия и инварианты (контрактное программирование)
- **Martin R.C. (2000). Design Principles and Design Patterns.** — оригинальная статья, объединяющая все 5 принципов
- **Meyer B. (1988). Object-Oriented Software Construction. Prentice Hall.** — Design by Contract, оригинальная формулировка OCP
- **Cardelli L. (1984). A Semantics of Multiple Inheritance.** — формальная модель подтипов, на которой основан LSP
- **Parnas D. (1972). On the Criteria To Be Used in Decomposing Systems into Modules. CACM.** — information hiding как основа SRP
