...
> **Чистый код** — код, оптимизированный для человеческого восприятия: минимизирующий когнитивную нагрузку при чтении, модификации и отладке. Формально связан с метриками **сложности кода** (McCabe, 1976; Halstead, 1977) и исследованиями **когнитивной психологии программирования** (Weinberg, 1971).

### Метрики сложности кода

| Метрика | Автор | Формула / Определение | Что измеряет |
|---------|-------|-----------------------|-------------|
| **Cyclomatic Complexity** | McCabe (1976) | M = E - N + 2P (рёбра - узлы + 2×компоненты графа потока) | Число линейно независимых путей выполнения |
| **Cognitive Complexity** | SonarSource (2017) | Штрафы за вложенность, прерывание потока, рекурсию | Сложность для человеческого восприятия |
| **Halstead Metrics** | Halstead (1977) | Volume = N × log₂(η), Effort = D × V | Размер и усилие чтения программы |
| **Lines of Code (LOC)** | — | Подсчёт строк | Грубый proxy размера |

### Закон Боёма и экономика качества кода

Barry Boehm (1981) показал:
- Стоимость исправления дефекта растёт **экспоненциально** с фазой обнаружения
- Дефект на этапе требований → ×1, дизайна → ×5, кодирования → ×10, тестирования → ×50, production → ×200
- **Чистый код** — это инвестиция в раннее обнаружение, не эстетика

### Когнитивные ограничения программиста

| Ограничение | Исследование | Следствие для кода |
|-------------|-------------|-------------------|
| **Working memory: 7±2 элементов** | Miller (1956) | Функция не должна оперировать > 7 переменными одновременно |
| **80% времени — чтение** | Индустриальные замеры (Martin 2008) | Оптимизировать для чтения, не для написания |
| **Экспоненциальный рост ошибок при вложенности > 3** | Soloway & Ehrlich (1984) | Ограничивать nesting depth |

### Эволюция идей о качестве кода

| Год | Автор / Работа | Вклад |
|-----|---------------|-------|
| 1971 | Weinberg — *The Psychology of Computer Programming* | Первое исследование когнитивных аспектов программирования |
| 1972 | Parnas — *Information Hiding* | Основа инкапсуляции и модульности |
| 1976 | McCabe — *Cyclomatic Complexity* | Формальная метрика сложности |
| 1999 | Fowler — *Refactoring* | Каталог рефакторингов (72 паттерна) |
| 2003 | Beck — *Test-Driven Development* | Тесты как specification + safety net |
| 2008 | Martin — *Clean Code* | Систематизация практик (с оговорками) |
| 2017 | SonarSource — Cognitive Complexity | Метрика, учитывающая человеческое восприятие |

> **См. также**: [[code-smells]] — признаки плохого кода, [[refactoring-catalog]] — техники улучшения, [[solid-principles]] — принципы проектирования
...
### Теоретические основы
- **McCabe T. (1976). A Complexity Measure. IEEE TSE.** — cyclomatic complexity: формальная метрика сложности через граф потока управления
- **Halstead M. (1977). Elements of Software Science. Elsevier.** — метрики объёма, сложности и усилия чтения программы
- **Weinberg G. (1971). The Psychology of Computer Programming. Van Nostrand.** — первое исследование когнитивных аспектов программирования
- **Boehm B. (1981). Software Engineering Economics. Prentice Hall.** — экономика дефектов: стоимость исправления растёт экспоненциально с фазой обнаружения
- **Campbell G.A. (2018). Cognitive Complexity: A New Way of Measuring Understandability. SonarSource.** — метрика, учитывающая восприятие кода человеком (vs McCabe)
