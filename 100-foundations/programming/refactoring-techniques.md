...
?
Рефакторинг — изменение внутренней структуры кода без изменения внешнего поведения, мелкими безопасными шагами с тестами. Переписывание — создание нового кода с нуля, часто с изменением поведения. Рефакторинг безопаснее: каждый шаг проверяется тестами.

Что такое Rule of Three для рефакторинга?
?
Первый раз — просто сделай. Второй раз — отметь дублирование, но сделай. Третий раз — рефактори. Правило защищает от преждевременной абстракции: два повторения ещё не паттерн, три — уже закономерность.

Что такое characterization test и зачем он нужен?
?
Тест, фиксирующий ТЕКУЩЕЕ поведение legacy-кода (даже если оно неправильное). Цель — создать safety net перед рефакторингом: запустить код, посмотреть результат, записать как expected. Без characterization tests рефакторинг legacy опасен.

Что такое Seam в legacy-коде?
?
Место, где можно изменить поведение без редактирования существующего кода. Object seam: подмена зависимости через конструктор. Link seam: подмена через import/require. Preprocessing seam: feature flags. Seams позволяют тестировать и постепенно менять legacy.

Что такое Sprout Method/Class?
?
Техника работы с legacy: вместо изменения существующего (опасного) кода, новая логика пишется в новом методе/классе, который полностью покрыт тестами. Старый код просто вызывает новый. Постепенно legacy "обрастает" чистым кодом.

Чем Extract Method отличается от Extract Class?
?
Extract Method: выделение кода из длинного метода в отдельный метод (Low effort, решает Long Method). Extract Class: выделение группы полей и методов в новый класс (Medium effort, решает Large Class, Divergent Change). Часто Extract Method → заметили паттерн → Extract Class.

Какой workflow безопасного рефакторинга?
?
1) Убедиться, что тесты есть (нет → characterization tests). 2) Одно маленькое изменение. 3) Запустить тесты. 4) Тесты зелёные → commit. 5) Тесты красные → undo. Повторять. Ключ: маленькие шаги + частые тесты = безопасность.
...
|-------------|------|-------|
| Следующий шаг | [[technical-debt]] | Стратегическое управление долгом, который рефакторинг сокращает |
| Углубиться | [[testing-strategies]] | Тесты — prerequisite для безопасного рефакторинга |
| Смежная тема | [[android-modularization]] | Модуляризация Android-проекта как масштабный рефакторинг |
