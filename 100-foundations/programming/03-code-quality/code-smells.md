...
> **Code smell** — поверхностный индикатор глубокой проблемы в дизайне программы. Термин введён Кентом Беком для книги Мартина Фаулера «Refactoring» (1999). Формально: smell = эвристика, указывающая на вероятное нарушение принципов проектирования (SOLID, coupling/cohesion, DRY).

### Формальная классификация

Фаулер и Бек (1999) определили 22 запаха. Wake (2003) систематизировал их в 5 категорий по типу проблемы:

| Категория | Проблема | Нарушаемый принцип |
|-----------|---------|-------------------|
| **Bloaters** | Код стал слишком большим | SRP, KISS |
| **OO Abusers** | Неправильное использование ООП-механизмов | LSP, OCP |
| **Change Preventers** | Изменение одного требования → правки в N местах | SRP, DRY |
| **Couplers** | Чрезмерная связь между классами | Low Coupling |
| **Dispensables** | Код, который можно удалить | YAGNI |

### Связь smell → refactoring

Каждый запах имеет **каталог рефакторингов** для его устранения (Fowler, 1999):
- Long Method → Extract Method
- Feature Envy → Move Method
- Data Clumps → Introduce Parameter Object / Extract Class
- Switch Statements → Replace with Polymorphism (sealed class в Kotlin)

> **См. также**: [[refactoring-catalog]] — техники устранения, [[clean-code]] — метрики сложности, [[legacy-code-strategies]] — работа с запахливым кодом
...
### Теоретические основы
- **Fowler M. (1999/2018). Refactoring: Improving the Design of Existing Code. Addison-Wesley.** — оригинальный каталог 22 code smells + рефакторинги для их устранения
- **Beck K. (in Fowler 1999). Code Smells.** — Кент Бек ввёл термин «code smell» как эвристику для обнаружения проблем дизайна
- **Wake W. (2003). Refactoring Workbook. Addison-Wesley.** — систематизация смеллов в 5 категорий (Bloaters, OO Abusers, Change Preventers, Couplers, Dispensables)
