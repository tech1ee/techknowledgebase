...
> **Legacy code** — код без тестов (Feathers, 2004). Определение не связано с возрастом кода: код, написанный вчера без тестов — уже legacy; 20-летний код с хорошим покрытием — просто код.

### Формальная модель Фезерса

Michael Feathers в «Working Effectively with Legacy Code» (2004) формализовал проблему:
- **Legacy Dilemma**: чтобы изменить код, нужны тесты; чтобы написать тесты, нужно изменить код (разорвать зависимости)
- **Seam** (шов) — место в программе, где можно изменить поведение без редактирования исходного кода
- **Enabling point** — место, через которое активируется шов (конструктор, параметр, подкласс)

Типы швов:
| Тип | Механизм | Пример |
|-----|----------|--------|
| **Object seam** | Полиморфизм / интерфейс | Передать mock через конструктор |
| **Preprocessing seam** | Макросы / ifdef | C/C++ conditional compilation |
| **Link seam** | Замена модуля при линковке | Тестовая реализация библиотеки |

### Strangler Fig Pattern (Fowler, 2004)

Martin Fowler предложил метафору удушающего фикуса: новый код обвивает старый, постепенно перехватывая функциональность, пока оригинал не отомрёт. Формализовано как **постепенная миграция** без «big bang rewrite».

> **См. также**: [[code-smells]] — признаки проблемного кода, [[refactoring-catalog]] — техники улучшения, [[testing-fundamentals]] — тесты как safety net
...
### Теоретические основы
- **Feathers M. (2004). Working Effectively with Legacy Code. Prentice Hall.** — определение legacy code, seam model, characterization tests, 25 dependency-breaking techniques
- **Fowler M. (2004). Strangler Fig Application.** — метафора постепенной миграции через обёртывание legacy-системы
- **Opdyke W. (1992). Refactoring Object-Oriented Frameworks. PhD Dissertation, UIUC.** — первый формальный каталог рефакторингов
