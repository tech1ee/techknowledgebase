...
Когда использовать Exceptions, а когда Result types?
?
Exceptions — для неожиданных ситуаций (программистские ошибки, инфраструктурные проблемы), обрабатываются централизованно. Result types — для ожидаемых вариантов (пользователь не найден, невалидный ввод), caller обязан обработать оба случая явно.

Что такое Fail Fast и почему это важно?
?
Обнаружить и сигнализировать об ошибке как можно раньше (валидация на входе функций, guard clauses). Важно: быстрая обратная связь, меньше side effects до момента ошибки, stack trace ближе к источнику, система не попадает в неопределённое состояние.

Что такое Railway-Oriented Programming?
?
Метафора двух путей: Success Track (зелёный) и Failure Track (красный). Каждый шаг возвращает Result[Output, Error]. Если шаг возвращает Error — остальные шаги пропускаются, ошибка проходит по "красному пути". Реализуется через map/flatMap на Result типе.

Чем отличается Retry от Circuit Breaker?
?
Retry: повторяет единичный запрос при transient error (с exponential backoff). Circuit Breaker: отслеживает общее число failures, при достижении порога "размыкает цепь" — сразу возвращает fallback без попыток, давая сервису время восстановиться. Часто используются вместе.

Что такое Guard Clause и как он улучшает читаемость?
?
Guard Clause — ранний return при невалидных условиях вместо вложенных if-else. Вместо 4 уровней вложенности: 4 последовательных `if bad → return 0`. Код становится линейным, основная логика — в конце, без глубокой индентации.

Что такое Null Object Pattern?
?
Специальный объект для "отсутствия значения" с нейтральным поведением вместо null-проверок. Пример: NullCustomer с name="Guest" и loyalty_program=NullLoyaltyProgram(discount=0). Код работает единообразно без проверок на null.

Какие три категории ошибок существуют и как с ними поступать?
?
1) Recoverable (сеть, валидация) — retry, fallback, показать пользователю. 2) Unrecoverable (out of memory, corrupted data) — crash, alert. 3) Panic/Fatal (security breach, data integrity violation) — немедленный shutdown, не допустить дальнейшей порчи данных.
...
|-------------|------|-------|
| Следующий шаг | [[architecture-resilience-patterns]] | Паттерны устойчивости на уровне архитектуры (Circuit Breaker, Bulkhead) |
| Углубиться | [[functional-programming]] | Result/Either типы и Railway-Oriented Programming из FP |
| Смежная тема | [[observability]] | Мониторинг и логирование ошибок в production |
