...
> **Observer (Publish-Subscribe)** — поведенческий паттерн, определяющий зависимость «один-ко-многим» между объектами так, что при изменении состояния одного объекта все зависимые уведомляются автоматически (GoF, 1994).

### Формальная модель

```
Subject (Observable)                    Observer
  - observers: List<Observer>             - update(state)
  - attach(observer)
  - detach(observer)
  - notify() → для всех o в observers: o.update(state)
```

### Связь с реактивным программированием

Observer — предшественник реактивных потоков. Эволюция:

| Поколение | Модель | Пример | Проблема |
|-----------|--------|--------|----------|
| **GoF (1994)** | Subject/Observer с callbacks | Java Observable, Android OnClickListener | Memory leaks (lapsed listener problem), thread-unsafe |
| **Rx (2009)** | Observable streams | RxJava, RxKotlin | Сложная модель ошибок, обратное давление (backpressure) |
| **Flow (2019)** | Cold/Hot coroutine streams | Kotlin Flow, StateFlow, SharedFlow | Structured concurrency решает lifecycle |

### Lapsed Listener Problem

Формально: если Observer зарегистрирован в Subject, но не отписан при уничтожении → Subject удерживает ссылку → **memory leak**. Это фундаментальная проблема паттерна Observer, решаемая в Kotlin через:
- `lifecycleScope` / `repeatOnLifecycle` (Android)
- `StateFlow` + structured concurrency (автоматическая отписка при отмене корутины)

> **См. также**: [[design-patterns-overview]] — каталог паттернов, [[strategy-pattern]] — другой поведенческий паттерн, [[state-pattern]] — управление состоянием
...
### Теоретические основы
- **Gamma E. et al. (1994). Design Patterns. Addison-Wesley.** — оригинальный Observer: Subject/Observer, pull vs push model, change propagation
- **Eugster P. et al. (2003). The Many Faces of Publish/Subscribe. ACM Computing Surveys.** — формальная таксономия Pub/Sub систем: topic-based, content-based, type-based
- **Bainomugisha E. et al. (2013). A Survey on Reactive Programming. ACM Computing Surveys.** — от Observer к Reactive Streams: FRP, Rx, формальные модели
