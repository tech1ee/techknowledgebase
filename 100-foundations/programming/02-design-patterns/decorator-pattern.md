...
> **Decorator (Wrapper)** — структурный паттерн проектирования, динамически добавляющий объекту новые обязанности. Предоставляет гибкую альтернативу наследованию для расширения функциональности (GoF, 1994).
...
└── Decorator            — содержит ссылку на Component
    ├── DecoratorA       — добавляет поведение A
    └── DecoratorB       — добавляет поведение B
```

Ключевое свойство: Decorator реализует **тот же интерфейс**, что и декорируемый объект → можно оборачивать декоратор в декоратор (stacking).

### Математическая модель: композиция функций

Decorator формально эквивалентен **композиции функций**:
```
f(g(h(x)))  =  Logging(Caching(Metrics(repository)))
```

Каждый декоратор — функция, принимающая Component и возвращающая Component с дополнительным поведением. Порядок применения важен (как и в математике: f∘g ≠ g∘f).

### Проблема комбинаторного взрыва

Без Decorator: N аспектов × все комбинации = **2^N - 1** классов.
С Decorator: N аспектов = **N** классов, комбинируемых произвольно.

| Аспекты | Без Decorator (наследование) | С Decorator (композиция) |
|---------|---------------------------|------------------------|
| 2 | 3 класса | 2 класса |
| 3 | 7 классов | 3 класса |
| 5 | 31 класс | 5 классов |
| 10 | 1023 класса | 10 классов |

> **См. также**: [[design-patterns-overview]] — каталог паттернов, [[adapter-pattern]] — похожий, но другой интерфейс, [[composition-vs-inheritance]] — теоретическое обоснование
...
### Теоретические основы
- **Gamma E. et al. (1994). Design Patterns. Addison-Wesley.** — оригинальное описание Decorator: композиция вместо наследования, решение проблемы комбинаторного взрыва
- **Bloch J. (2018). Effective Java, Item 18.** — «Favor composition over inheritance» с формальным обоснованием через Fragile Base Class Problem
