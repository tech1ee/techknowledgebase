...
> **Монотонный стек** — стек, поддерживающий инвариант: элементы в стеке всегда упорядочены (строго возрастают или строго убывают). При добавлении нового элемента, нарушающего монотонность, все «лишние» элементы извлекаются — именно в этот момент для них вычисляется ответ.

### Инвариант стека

- **Возрастающий стек (снизу вверх):** при push нового элемента `x` — pop все элементы `>= x`. Решает задачу *Next Smaller Element*.
- **Убывающий стек (снизу вверх):** при push нового элемента `x` — pop все элементы `<= x`. Решает задачу *Next Greater Element*.

### Amortized-анализ: почему O(n)?

Каждый из `n` элементов выполняет ровно один push и не более одного pop. Суммарно: не более `2n` операций со стеком. Следовательно, амортизированная сложность — `O(n)`, несмотря на то что отдельный push может вызвать цепочку pop'ов.

### Классические применения

| Задача | Тип стека | Что вычисляется при pop |
|--------|-----------|------------------------|
| Next Greater Element | Убывающий | Ближайший правый больший элемент |
| Largest Rectangle in Histogram | Возрастающий | Площадь с popped bar как высотой |
| Stock Span | Убывающий | Количество дней с ценой <= текущей |
| Trapping Rain Water | Убывающий | Объём воды во впадине |

См. также: [[stacks-queues]], [[sliding-window-pattern]] (Monotonic Deque).
...
| # | Источник | Вклад |
|---|----------|-------|
| 1 | Cormen et al. (2009). *CLRS* — глава 17 (Amortized Analysis) | Метод потенциалов для доказательства O(n) амортизированной сложности стековых операций |
| 2 | Skiena (2020). *The Algorithm Design Manual* | Стековые алгоритмы и их применение к задачам гистограмм |
...
> [!question]- Почему Monotonic Stack решает Next Greater Element за O(n), а не O(n^2)?
> Каждый элемент push и pop ровно один раз. Amortized: n push + n pop = 2n операций = O(n). Без стека: для каждого элемента сканируем вправо -> O(n^2). Стек хранит 'кандидатов' — элементы ещё без ответа. Когда приходит больший — все меньшие в стеке получают ответ.

> [!question]- Задача: Largest Rectangle in Histogram. Как Monotonic Stack даёт O(n)?
> Поддерживаем возрастающий стек индексов. Для каждого bar: если ниже top стека — pop и вычисляем площадь с popped bar как высотой. Ширина = от текущей позиции до нового top стека. Каждый bar push/pop один раз -> O(n). Это 'contribution technique': каждый bar вносит вклад как максимальная высота в своём диапазоне.

> [!question]- Когда использовать убывающий Monotonic Stack, а когда возрастающий?
> Возрастающий (снизу вверх): Next Greater Element, Largest Rectangle in Histogram. Убывающий: Next Smaller Element, Trapping Rain Water. Правило: если ищем 'больший' — возрастающий стек (pop когда пришёл больший). Если 'меньший' — убывающий (pop когда пришёл меньший).

## Ключевые карточки

Что такое Monotonic Stack?
?
Стек где элементы всегда монотонно возрастают или убывают. При push нового: pop все нарушающие монотонность. Каждый element push/pop один раз -> amortized O(n). Решает: Next Greater/Smaller Element, Largest Rectangle, Stock Span, Trapping Rain Water.

Как решить Next Greater Element?
?
Проходим массив справа налево (или слева направо). Стек хранит 'кандидатов'. Для текущего: pop все <= current (они не могут быть NGE для следующих). NGE = top стека (или -1 если пуст). Push current. O(n).

Что такое Stock Span Problem?
?
Для каждого дня: сколько последовательных дней цена была <= текущей. Monotonic Decreasing Stack хранит индексы. Span[i] = i - stack.top (индекс ближайшего дня с большей ценой). O(n).

Как Monotonic Stack решает Trapping Rain Water?
?
Decreasing stack: при pop (нашли 'впадину') вычисляем trapped water. Height = min(current, stack.top) - popped height. Width = current_index - stack.top - 1. Суммируем. Альтернатива: Two Pointers (проще, тоже O(n)).

Что такое Contribution Technique?
?
Вместо 'какой ответ для элемента i?' спрашиваем 'в скольких подмассивах элемент i является min/max?'. Monotonic Stack находит границы (left_smaller, right_smaller). Contribution = value * count_of_subarrays. Применение: Sum of Subarray Minimums, Sum of Subarray Ranges.

## Куда дальше

| Тип | Ссылка | Зачем |
|-----|--------|-------|
| Следующий шаг | [[patterns/topological-sort-pattern]] | Topological Sort для зависимостей |
| Углубиться | [[data-structures/stacks-queues]] | Стеки и очереди — основа паттерна |
| Смежная тема | [[patterns/sliding-window-pattern]] | Monotonic Deque для Sliding Window |
