...
> **Задача поиска подстроки (string matching)** — формально: дан текст `T[1..n]` и паттерн `P[1..m]`, найти все вхождения `P` в `T`. Наивный алгоритм — `O(nm)`, оптимальные — `O(n + m)`.

### Формальная модель строкового поиска

Строка — последовательность символов из конечного алфавита `Σ`. Поиск подстроки эквивалентен нахождению всех сдвигов `s`, таких что `T[s+1..s+m] = P[1..m]`. Все эффективные алгоритмы основаны на **предобработке** паттерна или текста, исключающей повторные сравнения.

### Ключевые алгоритмы

| Алгоритм | Идея | Предобработка | Поиск | Автор |
|----------|------|---------------|-------|-------|
| **KMP** | Prefix function — максимальный префикс = суффикс | `O(m)` | `O(n)` | Knuth, Morris, Pratt (1977) |
| **Rabin-Karp** | Rolling hash: `h(s[i..j]) = Σ s[k] * p^k mod q` | `O(m)` | `O(n)` ожид., `O(nm)` worst | Rabin & Karp (1987) |
| **Z-function** | `Z[i]` = длина наибольшего подстроки от `i`, совпадающей с префиксом | `O(n)` | `O(n)` | — |
| **Aho-Corasick** | Trie + failure links — множественный поиск | `O(Σm_i)` | `O(n + z)` | Aho & Corasick (1975) |

### Rolling Hash (Rabin-Karp)

Ключевая идея: хеш окна длины `m` пересчитывается за `O(1)` при сдвиге на один символ: `h(i+1) = (h(i) - T[i] * p^(m-1)) * p + T[i+m]`. Коллизии хешей требуют верификации, но на практике редки при правильном выборе базы и модуля — см. [[hash-tables]], [[sliding-window-pattern]].
...
- Gusfield (1997). *Algorithms on Strings, Trees, and Sequences.* — наиболее полное академическое изложение строковых алгоритмов: suffix trees, suffix arrays, Aho-Corasick
