...
> **Meet in the Middle** — техника декомпозиции пространства поиска на две половины с последующим комбинированием результатов. Снижает экспоненциальную сложность `O(2^n)` до `O(2^(n/2) * n)` — квадратный корень от исходного пространства.

### Связь с парадоксом дней рождения

Эффективность Meet in the Middle аналогична парадоксу дней рождения (birthday paradox): вероятность коллизии среди `k` элементов из пространства размера `N` становится значимой при `k ≈ √N`. Аналогично, MITM ищет «коллизию» (совпадение) между двумя множествами размера `√(2^n) = 2^(n/2)`.

### Декомпозиция пространства поиска

| Этап | Действие | Сложность |
|------|----------|-----------|
| 1. Разбиение | Делим `n` элементов на две половины по `n/2` | — |
| 2. Перебор | Генерируем все `2^(n/2)` подмножеств каждой половины | `O(2^(n/2))` |
| 3. Комбинирование | Для каждого результата первой половины ищем дополнение во второй (sort + binary search или HashMap) | `O(2^(n/2) * n)` |

### Область применимости

Работает для `n ~ 20-40`: при `n ≤ 20` достаточно полного перебора `2^20 ≈ 10^6`, при `n > 40` даже `2^(n/2)` слишком велико. Альтернативы: [[dp-patterns]] (полиномиальное состояние), [[backtracking]] (с pruning) — см. также [[binary-search-pattern]] для комбинирования.
...
| # | Источник | Вклад |
|---|----------|-------|
| 1 | Horowitz & Sahni (1974). *Computing partitions with applications to the knapsack problem* | Оригинальная формализация MITM для задачи о рюкзаке |
| 2 | Cormen et al. (2009). *CLRS* — глава 35 (Approximation Algorithms) | Subset Sum и экспоненциальные алгоритмы |
...
> [!question]- Почему Meet in the Middle снижает O(2^n) до O(2^(n/2) * n), а не до O(2^(n/2))?
> Разбиваем на две половины по n/2 элементов. Перебираем все подмножества каждой: 2^(n/2) для каждой. Комбинирование: для каждого результата первой ищем подходящий во второй (бинарный поиск O(log 2^(n/2)) = O(n/2)). Итого: 2 * 2^(n/2) + 2^(n/2) * n/2 = O(n * 2^(n/2)).

> [!question]- Задача 4SUM: найти четвёрку a+b+c+d = target. Как Meet in the Middle даёт O(n^2 log n)?
> Разделяем: все пары (a+b) и все пары (c+d). Для каждой пары a+b ищем target-(a+b) среди пар c+d. HashMap: O(n^2) для генерации, O(1) lookup = O(n^2). С сортировкой + binary search: O(n^2 log n). Наивный O(n^4) -> O(n^2).

> [!question]- Когда Meet in the Middle применим, а когда лучше DP?
> MITM: n от 20 до 40 (2^20 = 10^6 - допустимо, 2^40 = 10^12 - нет). DP: n до нескольких тысяч с полиномиальным состоянием. MITM: задачи на подмножества с большими значениями (не помещаются в DP-таблицу). DP: задачи с optimal substructure и умеренными constraints.

## Ключевые карточки

Что такое Meet in the Middle?
?
Разбиение задачи на две половины: 1) Перебрать все варианты первой половины. 2) Перебрать все варианты второй. 3) Комбинировать (sort + binary search или HashMap). Снижает O(2^n) до O(2^(n/2) * n). Работает для n ~ 20-40.

Как MITM решает Subset Sum?
?
Массив n элементов, target S. Разбиваем на A (n/2) и B (n/2). Генерируем все суммы подмножеств A и B. Сортируем B. Для каждой суммы a из A ищем S-a в B бинарным поиском. O(2^(n/2) * n). Для n=40: 2^20 * 40 ~ 40M операций.

Какие задачи решает MITM?
?
1) Subset Sum с большими значениями. 2) 4SUM (разбиение на пары). 3) Bidirectional BFS (кратчайший путь). 4) Closest Subset Sum. 5) Задачи с n ~ 20-40 где brute force O(2^n) слишком медленный, а DP невозможен.

Что такое Bidirectional BFS?
?
MITM для поиска кратчайшего пути: BFS одновременно от start и target. Встреча в середине. Обычный BFS: O(b^d), Bidirectional: O(b^(d/2) + b^(d/2)) = O(b^(d/2)). Где b = branching factor, d = depth. Экспоненциальное ускорение.

Как оценить, когда MITM подходит?
?
Constraints: n <= 40 и значения элементов большие (не помещаются в DP). 2^20 ~ 10^6 (допустимо). 2^40 ~ 10^12 (не допустимо). 2^(40/2) ~ 10^6 (допустимо). Если n <= 20 — обычный brute force. Если n > 40 — нужен DP или другой подход.

## Куда дальше

| Тип | Ссылка | Зачем |
|-----|--------|-------|
| Следующий шаг | [[patterns/string-algorithms-advanced]] | Продвинутые строковые алгоритмы |
| Углубиться | [[algorithms/backtracking]] | Backtracking для перебора |
| Смежная тема | [[patterns/binary-search-pattern]] | Binary Search для комбинирования |
