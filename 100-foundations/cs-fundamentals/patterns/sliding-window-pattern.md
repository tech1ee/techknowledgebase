---
title: "Паттерн скользящего окна (Sliding Window)"
created: 2025-12-29
modified: 2026-02-13
type: deep-dive
status: published
...
> **Скользящее окно (Sliding Window)** — техника инкрементальных вычислений над непрерывным подмассивом (подстрокой), позволяющая избежать повторных вычислений за счёт поддержания состояния окна при его сдвиге. Амортизированная сложность — `O(n)`.

### Amortized O(n) анализ

Каждый элемент входит в окно (expand right) ровно один раз и покидает его (shrink left) не более одного раза. Суммарно: `n` расширений + не более `n` сужений = `2n` операций = `O(n)`. Ключевое условие: операции добавления и удаления элемента из состояния окна должны выполняться за `O(1)` или амортизированный `O(1)`.

### Два типа окна

| Тип | Размер окна | Стратегия | Пример |
|-----|-------------|-----------|--------|
| **Fixed-size** | Фиксирован (`k`) | Инициализация первых `k`, затем сдвиг: `+right, -left` | Maximum Sum Subarray of Size K |
| **Variable-size** | Динамический | Expand right пока условие позволяет, shrink left когда нарушается | Minimum Window Substring, Longest Substring Without Repeating Characters |

### Ограничения применимости

Sliding Window требует **монотонности**: расширение окна должно монотонно изменять отслеживаемую величину. При наличии отрицательных чисел (сумма подмассива) монотонность нарушается — нужен prefix sum + HashMap. Подробнее: [[two-pointers-pattern]], [[monotonic-stack-pattern]].
...
| # | Источник | Вклад |
|---|----------|-------|
| 1 | Cormen et al. (2009). *CLRS* — глава 17 (Amortized Analysis) | Амортизированный анализ: метод агрегации для доказательства O(n) |
| 2 | Sedgewick & Wayne (2011). *Algorithms* | Инкрементальные вычисления и оптимизация повторных операций |
...
> [!question]- Почему Sliding Window не работает для поиска подмассива с суммой K при отрицательных числах?
> Sliding Window полагается на монотонность: расширение окна увеличивает сумму, сужение — уменьшает. С отрицательными числами это свойство нарушается: добавление элемента может уменьшить сумму. Нужен prefix sum + HashMap: для каждого prefix[i] ищем prefix[i]-K в HashMap.

> [!question]- Задача: найти наименьшее окно в строке S, содержащее все символы строки T. Какой инвариант окна?
> Variable-size Sliding Window. Инвариант: окно содержит все символы T с правильными частотами. HashMap для подсчёта нужных символов. Expand right пока не собрали все. Shrink left пока инвариант выполняется. Обновляем минимум на каждом шаге shrink. O(n).

> [!question]- Как Monotonic Deque решает задачу максимума в скользящем окне за O(n)?
> Deque хранит индексы в убывающем порядке значений. При добавлении нового элемента: удаляем из конца deque все меньшие (они никогда не будут максимумом). При сдвиге окна: удаляем из начала если индекс вышел за окно. Максимум окна = front of deque. Каждый элемент добавляется и удаляется ровно один раз -> amortized O(n).

## Ключевые карточки

Какие два типа Sliding Window?
?
1) Fixed-size: окно фиксированного размера K. Инициализация первого окна, затем сдвиг (добавляем справа, убираем слева). Пример: Maximum Sum Subarray of Size K. 2) Variable-size: размер меняется по условию. Два указателя: expand right, shrink left. Пример: Minimum Window Substring.

Как определить, что задача решается Sliding Window?
?
Признаки: 1) Непрерывный подмассив/подстрока. 2) Нужен оптимум (мин/макс длина, сумма). 3) Есть условие (constraint) для окна. 4) Данные — массив или строка. Если 'subsequence' (не непрерывная) — НЕ Sliding Window.

Что такое Sliding Window + HashMap?
?
Комбинация для строковых задач. HashMap отслеживает частоту символов в текущем окне. Expand: увеличиваем counter символа. Shrink: уменьшаем counter. Применение: Longest Substring Without Repeating Characters, Permutation in String, Minimum Window Substring.

Чем отличается Sliding Window от Two Pointers?
?
Sliding Window — подтип Two Pointers (Same Direction), но с состоянием окна. Two Pointers: указатели могут двигаться независимо. Sliding Window: окно непрерывно, каждый сдвиг — инкрементальное обновление. Ключевое отличие: SW поддерживает инвариант окна.

Как Sliding Window применяется к задачам на подстроку?
?
Substring = непрерывная часть строки = идеальная задача для SW. Паттерн: HashMap для char frequency в окне, expand right добавляя char, shrink left когда условие нарушается. Все задачи 'найти подстроку с условием X' решаются этим шаблоном.

## Куда дальше

| Тип | Ссылка | Зачем |
|-----|--------|-------|
| Следующий шаг | [[patterns/binary-search-pattern]] | Бинарный поиск — ещё один фундаментальный паттерн |
| Углубиться | [[patterns/monotonic-stack-pattern]] | Монотонный стек для max/min в окне |
| Смежная тема | [[data-structures/hash-tables]] | HashMap для подсчёта в окне |
