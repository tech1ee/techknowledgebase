...
> **Cyclic Sort** — алгоритм сортировки на месте для массивов, содержащих значения из ограниченного диапазона `[1, n]` (или `[0, n-1]`). Основан на идее: каждый элемент помещается на «своё место» — значение `v` ставится на индекс `v - 1`.

### Связь с теорией перестановок

Массив значений `[1, n]` — это перестановка. Каждая перестановка разлагается на циклы: элемент `a[i]` указывает на позицию, куда он должен встать, образуя цепочку swap'ов. Алгоритм следует по циклам перестановки, ставя каждый элемент на место за один swap.

### Сравнение с Counting Sort

| Свойство | Cyclic Sort | Counting Sort |
|----------|-------------|---------------|
| Диапазон значений | Строго `[1, n]` или `[0, n-1]` | Произвольный `[min, max]` |
| Доп. память | `O(1)` — in-place | `O(k)`, где k = диапазон |
| Сложность | `O(n)` — каждый элемент перемещается не более одного раза | `O(n + k)` |
| Бонус | После сортировки легко найти пропущенные/дублирующиеся элементы | Только сортировка |

Cyclic Sort оптимален для задач типа *Find Missing Number*, *Find All Duplicates*, *First Missing Positive* — см. [[sorting-algorithms]], [[arrays-strings]].
...
### [[sorting-algorithms]]
Cyclic Sort — это специализированный алгоритм сортировки, работающий за O(n) для массивов с элементами в ограниченном диапазоне. В отличие от comparison-based алгоритмов (Merge Sort, Quick Sort) с нижней границей O(n log n), Cyclic Sort использует свойство index-value mapping и не выполняет сравнений. Понимание классических сортировок помогает оценить, когда Cyclic Sort применим и почему он быстрее: он эксплуатирует конкретное свойство данных, недоступное общим алгоритмам. Связь с Counting Sort — оба достигают линейного времени за счёт ограничений на входные данные.

### [[arrays-strings]]
Cyclic Sort работает исключительно с массивами и требует понимания индексации, swap-операций и in-place модификации. Ключевая идея паттерна — маппинг значения на индекс (число i на позицию i-1) — напрямую опирается на свойство random access массивов за O(1). Без уверенного владения массивами невозможно понять, почему swap `nums[i]` с `nums[nums[i]-1]` корректно ставит элемент на своё место. Arrays & Strings формирует фундамент для всех паттернов, работающих с позиционным доступом.

### [[hash-tables]]
Hash-таблицы — это альтернативный подход к задачам, которые решает Cyclic Sort: поиск дубликатов, пропущенных элементов, подсчёт частот. HashSet решает Find Missing Number за O(n) времени и O(n) памяти, тогда как Cyclic Sort — за O(n) времени и O(1) памяти. Понимание обоих подходов позволяет выбирать оптимальный инструмент: hash-таблицы универсальнее, но Cyclic Sort выигрывает по памяти для массивов с числами в фиксированном диапазоне.
...
> [!question]- Почему Cyclic Sort работает за O(n), хотя внутри while-цикла есть swap?
> Каждый swap ставит ровно один элемент на своё место. Максимум n-1 swap за всё время работы. Amortized analysis: хотя inner while может сделать несколько swaps, суммарно по всем итерациям outer for — не более n. Каждый элемент перемещается максимум один раз.

> [!question]- Задача: найти все пропущенные числа в массиве [1,n]. Почему Cyclic Sort лучше HashSet?
> HashSet: O(n) время, O(n) память. Cyclic Sort: O(n) время, O(1) память. После сортировки 'на месте' — один проход: если nums[i] != i+1, то i+1 пропущен. In-place решение не требует дополнительной памяти. Паттерн: число i должно быть на позиции i-1.

> [!question]- Как модифицировать Cyclic Sort для нахождения дубликатов?
> При сортировке: если nums[i] != i+1 и nums[nums[i]-1] == nums[i], это дубликат (число уже на своём месте, а мы видим его копию). Не swap, а фиксируем дубликат и идём дальше. Для Find All Duplicates: собираем все такие случаи. O(n) время, O(1) память.

## Ключевые карточки

Что такое Cyclic Sort?
?
Паттерн для массивов с числами в диапазоне [1,n] или [0,n-1]. Идея: число i ставим на позицию i-1. Цикл: while nums[i] != i+1: swap(nums[i], nums[nums[i]-1]). O(n) время, O(1) память. После сортировки легко найти missing/duplicate.

Какие задачи решает Cyclic Sort?
?
1) Find Missing Number (268). 2) Find All Missing Numbers (448). 3) Find Duplicate (287). 4) Find All Duplicates (442). 5) First Missing Positive (41). 6) Set Mismatch (645). Все — O(n) время, O(1) доп. память.

Как решить First Missing Positive за O(n)?
?
Cyclic Sort модификация: для значений в [1,n] ставим на место. Игнорируем числа <= 0 и > n. После сортировки: первый индекс i где nums[i] != i+1 — ответ i+1. Если все на месте — ответ n+1.

Чем Cyclic Sort отличается от обычной сортировки?
?
Обычная сортировка: O(n log n), работает с любыми данными. Cyclic Sort: O(n), только для чисел в известном диапазоне [1,n]. Не comparison-based, а index-based: число i -> позиция i-1. Это pigeonhole principle в действии.

Когда Cyclic Sort НЕ применим?
?
1) Числа не в диапазоне [1,n] или [0,n-1]. 2) Массив содержит отрицательные числа. 3) Числа с большим диапазоном (нельзя использовать как индекс). 4) Нужен порядок, а не missing/duplicate. В этих случаях: HashSet или сортировка.

## Куда дальше

| Тип | Ссылка | Зачем |
|-----|--------|-------|
| Следующий шаг | [[patterns/top-k-elements-pattern]] | Top K Elements через Heap |
| Углубиться | [[algorithms/sorting-algorithms]] | Теория сортировок подробно |
| Смежная тема | [[data-structures/arrays-strings]] | Массивы — основа паттерна |
...
- Cormen, Leiserson, Rivest & Stein (2009). *Introduction to Algorithms (CLRS).* — глава 8 (Sorting in Linear Time): формальное доказательство нижней границы O(n log n) для comparison-based сортировок и описание линейных алгоритмов, к которым концептуально относится Cyclic Sort
- Skiena (2020). *The Algorithm Design Manual.* — раздел о permutation cycles: теоретическая основа Cyclic Sort

### Практические руководства

- Sedgewick & Wayne (2011). *Algorithms.* — визуализации сортировок и анализ in-place алгоритмов; помогает понять, почему swap-based подходы корректны и эффективны
