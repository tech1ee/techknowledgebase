...
> **Топологическая сортировка** — линейное упорядочение вершин ориентированного ациклического графа (DAG) `G = (V, E)` такое, что для каждого ребра `(u, v) ∈ E` вершина `u` предшествует `v` в порядке.

### Теорема о DAG

Граф допускает топологическую сортировку тогда и только тогда, когда он является DAG (directed acyclic graph). Доказательство: если граф содержит цикл `v₁ → v₂ → ... → v₁`, то `v₁` должна быть одновременно до и после себя — противоречие.

### Два алгоритма

| Свойство | Алгоритм Кана (Kahn, 1962) | DFS-based |
|----------|---------------------------|-----------|
| Подход | BFS + indegree | DFS reverse postorder |
| Структура | Queue с вершинами indegree = 0 | Рекурсивный стек |
| Обнаружение цикла | Обработано < V вершин → цикл | Back edge (GRAY → GRAY) → цикл |
| Лексикографический порядок | Заменить Queue на Min-Heap | Сложнее адаптировать |
| Сложность | `O(V + E)` | `O(V + E)` |

### Применения

Топологическая сортировка — основа для: разрешения зависимостей (build-системы, package managers), планирования задач (course scheduling), DP на DAG (longest path за полиномиальное время) — см. [[dfs-bfs-patterns]], [[dp-patterns]].
...
| # | Источник | Вклад |
|---|----------|-------|
| 1 | Kahn (1962). *Topological sorting of large networks* | Оригинальная статья: BFS-based алгоритм с indegree |
| 2 | Cormen et al. (2009). *CLRS* — глава 22.4 | DFS-based топологическая сортировка, доказательство корректности |
...
> [!question]- Почему Topological Sort возможен только для DAG и как он обнаруживает циклы?
> Topological ordering требует: для каждого ребра u->v, u идёт перед v. В цикле a->b->c->a это невозможно (a должен быть и до, и после c). Kahn's BFS: если обработано < V вершин — цикл есть (оставшиеся вершины в цикле, indegree никогда не станет 0). DFS: back edge (GRAY -> GRAY) = цикл.

> [!question]- Задача: определить порядок курсов с prerequisites. Как Kahn's BFS решает задачу?
> Граф: course -> prerequisite edges. Kahn's: 1) Подсчёт indegree для каждого курса. 2) Очередь: все с indegree=0 (нет prerequisites). 3) Извлекаем, уменьшаем indegree соседей. Если стал 0 — в очередь. 4) Если обработано == V — порядок найден. Иначе — цикл (impossible).

> [!question]- Когда существует несколько валидных топологических порядков и как выбрать лексикографически наименьший?
> Несколько порядков когда есть вершины без зависимостей между собой (несравнимые). Для лексикографически наименьшего: заменяем Queue на Priority Queue (Min-Heap) в Kahn's algorithm. На каждом шаге выбираем наименьшую вершину с indegree=0.

## Ключевые карточки

Какие два алгоритма Topological Sort?
?
1) Kahn's BFS: подсчёт indegree, очередь вершин с indegree=0, уменьшение indegree при обработке. O(V+E). 2) DFS + reverse postorder: DFS с push в стек после обработки всех потомков. Reverse стека = топологический порядок. O(V+E).

Что такое indegree и зачем он нужен?
?
Indegree вершины = количество входящих рёбер = количество зависимостей. Вершина с indegree=0 не зависит ни от кого — можно обрабатывать первой. Kahn's algorithm уменьшает indegree при обработке, создавая новые 'свободные' вершины.

Какие задачи решает Topological Sort?
?
1) Course Schedule (207, 210). 2) Build System (зависимости компиляции). 3) Alien Dictionary (порядок букв). 4) Task Scheduling с зависимостями. 5) Longest Path в DAG. 6) Minimum Height Trees (связано). Везде есть DAG + зависимости.

Как найти Longest Path в DAG?
?
Topological Sort + DP: dp[v] = максимальный путь до v. Обрабатываем в топологическом порядке: dp[v] = max(dp[u] + weight(u,v)) для всех предшественников u. O(V+E). В общем графе Longest Path — NP-hard, но в DAG — полиномиален.

Чем Kahn's отличается от DFS подхода?
?
Kahn's (BFS): итеративный, явный indegree, Queue. Проще для обнаружения цикла (count < V). DFS: рекурсивный, reverse postorder. Естественнее для path-related задач. Оба O(V+E). Kahn's легче модифицировать (Min-Heap для лексикографического порядка).

## Куда дальше

| Тип | Ссылка | Зачем |
|-----|--------|-------|
| Следующий шаг | [[patterns/union-find-pattern]] | Union-Find для связных компонент |
| Углубиться | [[algorithms/graph-algorithms]] | Алгоритмы на графах |
| Смежная тема | [[patterns/dfs-bfs-patterns]] | DFS/BFS — основа обхода |
