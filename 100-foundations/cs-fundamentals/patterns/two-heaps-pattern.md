...
> **Задача поддержания медианы (Median Maintenance)** — в потоке данных эффективно отвечать на запрос «какова текущая медиана?» после каждого нового элемента. Наивный подход (пересортировка) — `O(n log n)` на вставку, Two Heaps — `O(log n)`.

### Инвариант двух куч

Структура состоит из двух куч, поддерживающих следующий инвариант:

- **Max-Heap (нижняя половина):** хранит меньшую половину элементов; вершина = максимум нижней половины
- **Min-Heap (верхняя половина):** хранит большую половину; вершина = минимум верхней половины
- **Балансировка:** `|maxHeap.size - minHeap.size| <= 1`

### Операции

| Операция | Действие | Сложность |
|----------|----------|-----------|
| **Добавление** | Вставить в нужную кучу, перебалансировать если нужно | `O(log n)` |
| **Получение медианы** | Нечётное кол-во: вершина большей кучи; чётное: среднее двух вершин | `O(1)` |
| **Удаление** (Sliding Window Median) | Lazy deletion + перебалансировка | `O(log n)` |

### Обобщение

Паттерн Two Heaps применим не только к медиане, но к любой задаче, где нужно эффективно отслеживать разделение данных на две группы с доступом к границе: IPO (maximize capital), Sliding Window Median — см. [[heaps-priority-queues]], [[top-k-elements-pattern]].
...
- Munro & Paterson (1980). *Selection and sorting with limited storage* — streaming алгоритмы для задач выбора с ограниченной памятью
...
- Skiena (2020). *The Algorithm Design Manual.* — практические применения median maintenance в streaming-системах; раздел о выборе структуры данных под задачу
