...
> **Two Pointers** — техника линейного сканирования с двумя индексами, поддерживающими инвариант относительно текущего состояния поиска. Снижает вложенные циклы `O(n²)` до одного прохода `O(n)` за счёт использования свойств входных данных (отсортированность, монотонность).

### Формальный инвариант

Для задачи поиска пары `(a[i], a[j])` с заданным свойством в отсортированном массиве: указатели `left` и `right` (начинающие с концов) поддерживают инвариант — **ответ, если существует, находится в диапазоне `[left, right]`**. Каждое перемещение указателя гарантированно исключает элемент, не принадлежащий ни одному решению.

### Три вариации

| Вариация | Движение | Применение |
|----------|----------|-----------|
| **Opposite Direction** | `left →`, `← right` (навстречу) | Two Sum (sorted), Container With Most Water, палиндром |
| **Same Direction** | `slow →`, `fast →` (однонаправленно) | Remove Duplicates, partition, Sliding Window |
| **Fast/Slow** | `slow: +1`, `fast: +2` (разная скорость) | Cycle Detection (Floyd), середина списка |

### Условия применимости

Two Pointers требует предсказуемости движения: перемещение указателя должно монотонно изменять отслеживаемую величину. Для отсортированных массивов — это гарантируется. Для неотсортированных данных без возможности сортировки — нужен HashMap. Расширение: [[sliding-window-pattern]] (Same Direction + состояние окна), [[fast-slow-pointers-pattern]] (специализация Fast/Slow).
...
| # | Источник | Вклад |
|---|----------|-------|
| 1 | Cormen et al. (2009). *CLRS* — глава 2 (Getting Started) | Формальный анализ линейного сканирования, инварианты циклов |
| 2 | Sedgewick & Wayne (2011). *Algorithms* | Паттерны работы с указателями на массивах и связных списках |
...
> [!question]- Почему Two Pointers на отсортированном массиве снижает сложность с O(n^2) до O(n)?
> В отсортированном массиве перемещение указателя гарантирует монотонное изменение суммы. При sum < target двигаем left вправо (увеличиваем сумму), при sum > target двигаем right влево (уменьшаем). Каждый указатель проходит массив максимум один раз -> O(n). Без сортировки нет этого свойства, нужен вложенный цикл.

> [!question]- Задача: удалить дубликаты из отсортированного массива in-place. Какой тип Two Pointers и почему?
> Same Direction: slow (позиция записи) и fast (позиция чтения). Fast сканирует массив, при nums[fast] != nums[slow] — записываем и сдвигаем slow. Invariant: всё до slow включительно — уникальные элементы. O(n) время, O(1) память.

> [!question]- Когда Two Pointers НЕ применим и нужен другой подход?
> 1) Неотсортированные данные без возможности сортировки (нужен HashMap). 2) Задачи на подмассив с суммой при отрицательных числах (Sliding Window не работает, нужен prefix sum). 3) Задачи на subsequence, а не subarray. 4) Когда нужны ВСЕ пары, а не одна.

## Ключевые карточки

Какие три типа Two Pointers?
?
1) Opposite Direction — указатели с разных концов, сходятся к центру. Пример: Two Sum в отсортированном массиве. 2) Same Direction — оба от начала, разная скорость. Пример: Remove Duplicates. 3) Fast/Slow — разная скорость шага. Пример: Cycle Detection (Floyd).

Как решить 3Sum за O(n^2)?
?
Сортировка O(n log n). Фиксируем первый элемент (цикл i). Для оставшихся — Two Pointers Opposite Direction: left=i+1, right=n-1. Если sum < 0, left++; если sum > 0, right--. Пропускаем дубликаты. Итого: O(n^2).

Что такое Dutch National Flag и когда применять?
?
Три указателя: low, mid, high. Сортировка массива из 3 типов элементов (0,1,2) за один проход O(n). mid сканирует: 0 -> swap с low и оба вперёд; 2 -> swap с high, high назад; 1 -> mid вперёд. Применение: partition, sort colors.

Как найти середину linked list за один проход?
?
Fast/Slow pointers: slow шагает на 1, fast на 2. Когда fast достигает конца, slow на середине. Один проход, O(1) память. Используется в: Merge Sort для списков, Palindrome check, Reorder List.

Что такое Container With Most Water и почему Two Pointers оптимален?
?
Массив высот, найти пару линий с максимальной площадью. Two Pointers: left=0, right=n-1. Площадь = min(h[left], h[right]) * (right-left). Двигаем указатель с меньшей высотой (большая высота не поможет при уменьшении ширины). O(n).

## Куда дальше

| Тип | Ссылка | Зачем |
|-----|--------|-------|
| Следующий шаг | [[patterns/sliding-window-pattern]] | Скользящее окно — развитие Two Pointers |
| Углубиться | [[patterns/fast-slow-pointers-pattern]] | Алгоритм Флойда подробно |
| Смежная тема | [[data-structures/arrays-strings]] | Массивы и строки — основа паттерна |
