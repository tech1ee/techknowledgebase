...
> **Битовые манипуляции** — операции над двоичным представлением чисел, основанные на булевой алгебре (George Boole, 1854). Каждая побитовая операция выполняется за `O(1)` на аппаратном уровне, что делает их самыми быстрыми примитивами вычислений.

### Таблица побитовых операций

| Операция | Символ | Правило | Пример (5 = 101, 3 = 011) |
|----------|--------|---------|---------------------------|
| AND | `&` | Оба бита = 1 | `101 & 011 = 001` (1) |
| OR | `\|` | Хотя бы один = 1 | `101 \| 011 = 111` (7) |
| XOR | `^` | Биты различны | `101 ^ 011 = 110` (6) |
| NOT | `~` | Инверсия всех битов | `~101 = ...010` |
| Left Shift | `<<` | Умножение на 2^k | `101 << 1 = 1010` (10) |
| Right Shift | `>>` | Деление на 2^k | `101 >> 1 = 010` (2) |

### Ключевые битовые трюки

- **XOR-свойства:** `a ^ a = 0`, `a ^ 0 = a`, коммутативность — основа поиска уникальных элементов
- **Очистка младшего бита:** `n & (n - 1)` — удаляет rightmost set bit (Brian Kernighan); используется для проверки степени двойки и подсчёта единичных битов
- **Bitmask-подмножества:** число `mask` кодирует подмножество из `n` элементов, всего `2^n` подмножеств — см. [[dp-patterns]]
...
| # | Источник | Вклад |
|---|----------|-------|
| 1 | Warren (2012). *Hacker's Delight* | Исчерпывающий справочник битовых трюков с доказательствами корректности |
| 2 | Knuth (2011). *TAOCP, Vol. 4A: Combinatorial Algorithms* | Формализация перебора подмножеств через bitmask |
