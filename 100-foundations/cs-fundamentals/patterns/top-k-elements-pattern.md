...
> **Задача выбора (Selection Problem)** — нахождение `k`-го по порядку элемента в неотсортированном массиве. Фундаментальный результат: Blum, Floyd, Pratt, Rivest & Tarjan (1973) доказали существование детерминированного алгоритма со сложностью `O(n)`.

### Три подхода к Top K

| Подход | Время | Память | Когда использовать |
|--------|-------|--------|--------------------|
| **Сортировка** | `O(n log n)` | `O(1)`–`O(n)` | Нужен полный порядок; малые `n` |
| **Min-Heap размера K** | `O(n log k)` | `O(k)` | Streaming данные; `k << n` |
| **QuickSelect** | `O(n)` average, `O(n²)` worst | `O(1)` | Одноразовый запрос; модификация массива допустима |

### Ключевой инсайт: Min-Heap для Top K наибольших

Для `K` наибольших элементов используется **Min-Heap** размера `K` (не Max-Heap!). Логика: min-heap хранит `K` текущих кандидатов, и при добавлении нового элемента `> heap.peek()` минимальный кандидат вытесняется. После обработки всех `n` элементов в heap остаются ровно `K` наибольших.

### Median of Medians (Blum et al., 1973)

Детерминированный `O(n)` алгоритм выбора: разбивает массив на группы по 5, находит медиану каждой группы, рекурсивно находит медиану медиан, использует её как pivot для partition. Гарантирует, что не менее 30% элементов отсекается на каждом шаге — см. [[heaps-priority-queues]], [[sorting-algorithms]].
...
> [!question]- Почему для Top K наибольших используется Min-Heap, а не Max-Heap?
> Min-Heap размера K: вершина = наименьший из K наибольших. При добавлении нового элемента: если > вершины — удаляем вершину, добавляем новый. Так поддерживаем K наибольших. Max-Heap потребовал бы хранить все n элементов. Min-Heap: O(n log k) время, O(k) память.

> [!question]- Когда QuickSelect лучше Heap для Top K и когда хуже?
> QuickSelect: O(n) average, O(n^2) worst case, модифицирует массив. Heap: O(n log k) всегда, не модифицирует. QuickSelect лучше: k близко к n/2, все данные в памяти. Heap лучше: streaming данные, k << n, нужна стабильность. Introselect: QuickSelect + fallback на median-of-medians = O(n) worst case.

> [!question]- Задача: Top K Frequent Elements. Почему Bucket Sort даёт O(n)?
> Максимальная частота = n. Создаём массив buckets[0..n], где buckets[f] = список элементов с частотой f. HashMap для подсчёта частот O(n). Заполняем buckets O(n). Собираем от конца до K элементов O(n). Итого O(n), без log. Работает только когда диапазон частот ограничен.

## Ключевые карточки

Какие три подхода к Top K?
?
1) Sort: O(n log n), простейший. 2) Heap размера K: O(n log k), потоковый. 3) QuickSelect: O(n) average, in-place. Выбор: streaming -> Heap, все данные в памяти + k ~ n/2 -> QuickSelect, простота -> Sort.

Как работает QuickSelect?
?
Partition (как в QuickSort): pivot на своё место, меньшие слева, большие справа. Если pivot на позиции k — нашли. Если позиция > k — ищем слева. Если < k — справа. Average O(n), worst O(n^2). Randomized pivot для защиты от worst case.

Что такое Kth Largest Element?
?
Найти K-й по величине элемент. Approaches: Sort + index O(n log n). Min-Heap размера K + проход O(n log k). QuickSelect на n-k позицию O(n). Heap лучше для streaming, QuickSelect — для статических данных.

Как решить K Closest Points to Origin?
?
Для каждой точки: distance^2 = x^2 + y^2. Top K ближайших = K наименьших distance. Max-Heap размера K: если новая точка ближе вершины — replace. Или QuickSelect по distance. O(n log k) или O(n).

Когда использовать Bucket Sort для Top K Frequent?
?
Когда максимальная частота ограничена (обычно <= n). Создаём n+1 bucket, элемент с частотой f идёт в bucket[f]. Собираем K элементов от bucket[n] к bucket[0]. O(n). Не работает если частоты могут быть произвольно большими.

## Куда дальше

| Тип | Ссылка | Зачем |
|-----|--------|-------|
| Следующий шаг | [[patterns/k-way-merge-pattern]] | K-way Merge через Heap |
| Углубиться | [[data-structures/heaps-priority-queues]] | Heap — основа паттерна |
| Смежная тема | [[algorithms/sorting-algorithms]] | QuickSelect и QuickSort |
...
- Cormen, Leiserson, Rivest & Stein (2009). *Introduction to Algorithms (CLRS).* — глава 9 (Medians and Order Statistics): формальный анализ selection algorithms, включая QuickSelect с доказательством ожидаемого O(n)
- Blum, Floyd, Pratt, Rivest & Tarjan (1973). *Time bounds for selection.* — оригинальная статья, доказывающая существование детерминированного O(n) алгоритма выбора (Median of Medians)
