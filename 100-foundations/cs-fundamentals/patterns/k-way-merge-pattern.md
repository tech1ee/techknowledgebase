...
> **K-way Merge** — алгоритм слияния `K` отсортированных последовательностей в одну отсортированную за `O(N log K)`, где `N` — общее число элементов. Использует min-heap (приоритетную очередь) для эффективного извлечения минимального элемента среди `K` текущих кандидатов.

### Алгоритм

1. Поместить первый элемент каждого из `K` списков в min-heap — `O(K log K)`
2. Извлечь минимум из heap, добавить в результат — `O(log K)`
3. Из того же списка, откуда был минимум, добавить следующий элемент в heap
4. Повторять шаги 2-3 до исчерпания всех списков — всего `N` итераций

### Связь с External Sorting

K-way Merge — ключевой примитив **внешней сортировки** (external sort): когда данные не помещаются в оперативную память, файл разбивается на блоки, каждый сортируется отдельно, затем `K` отсортированных блоков сливаются через K-way Merge. Этот же принцип используется в MapReduce и merge-фазе базы данных.

### Сложность

| Операция | Время | Память |
|----------|-------|--------|
| Построение heap | `O(K)` | `O(K)` |
| Извлечение N элементов | `O(N log K)` | `O(K)` |
| Итого | `O(N log K)` | `O(K)` |

См. также: [[heaps-priority-queues]], [[sorting-algorithms]], [[top-k-elements-pattern]].
...
### [[heaps-priority-queues]]
Min-Heap — центральная структура данных паттерна K-way Merge. Heap поддерживает K элементов (по одному из каждого списка) и позволяет извлекать минимальный за O(log K) вместо O(K) при линейном поиске. Без понимания операций insert, extractMin и свойства heap order невозможно реализовать K-way Merge эффективно. Heaps & Priority Queues объясняют, почему heap гарантирует O(log K) на операцию и как sift-up/sift-down поддерживают инвариант.

### [[top-k-elements-pattern]]
Top K Elements и K-way Merge — "зеркальные" паттерны: оба используют heap размера K, но для разных целей. Top K ищет K экстремальных элементов в одном неупорядоченном наборе, а K-way Merge объединяет K упорядоченных наборов. Понимание одного паттерна ускоряет освоение другого, так как механика работы с heap идентична. На интервью важно различать, когда применять каждый из них.

### [[two-heaps-pattern]]
Two Heaps и K-way Merge объединяет общая идея — использование heap для эффективной работы с упорядоченными данными. Two Heaps разделяет один поток на две половины для поддержания медианы, тогда как K-way Merge объединяет K потоков в один. Оба паттерна демонстрируют мощь heap как инструмента для streaming-задач, где данные поступают порционно.

### [[sorting-algorithms]]
K-way Merge — прямое обобщение фазы merge из Merge Sort: вместо слияния двух отсортированных массивов сливаются K. Понимание двухпутевого слияния (merge двух sorted arrays за O(n+m)) является основой для K-way Merge. Паттерн также критичен для external sorting — алгоритма сортировки данных, не помещающихся в оперативную память, где диск делится на K отсортированных частей, которые затем сливаются.
...
> [!question]- Почему для K-way Merge используется Min-Heap размера K, а не последовательное слияние пар?
> Min-Heap: O(N log K) — каждый из N элементов проходит через heap высоты log K. Последовательное слияние пар: первое слияние N/K + N/K = 2N/K, второе 2N/K + N/K = 3N/K... Итого O(N*K). Heap на порядок эффективнее когда K велико.

> [!question]- Задача: Kth Smallest Element in Sorted Matrix. Почему Binary Search лучше Heap?
> Heap: O(K log K) — может быть O(n^2 log n) для большого K. Binary Search по значению: O(n * log(max-min)). Для каждого mid подсчитываем сколько элементов <= mid за O(n) (staircase traversal). Если count >= K, ответ <= mid. Эффективнее для больших K.

> [!question]- Как K-way Merge масштабируется в external sorting, когда данные не помещаются в память?
> Данные разбиваются на chunks, каждый сортируется в RAM. K-way merge chunks: heap размера K (по одному элементу из каждого chunk). Читаем следующий элемент из chunk когда предыдущий извлечён. RAM = K буферов. Оптимальный K = RAM / buffer_size. MapReduce использует этот принцип.

## Ключевые карточки

Как работает K-way Merge с Min-Heap?
?
1) Кладём первый элемент каждого из K списков в Min-Heap (с индексом списка). 2) Извлекаем минимум. 3) Если в этом списке есть следующий — добавляем. 4) Повторяем пока heap не пуст. O(N log K) время, O(K) память.

Как решить Merge K Sorted Lists?
?
Min-Heap хранит головы K списков. Извлекаем минимум, добавляем next этого node. Повторяем. O(N log K). Альтернатива: Divide & Conquer — попарное слияние, log K раундов по O(N). Тоже O(N log K), но без heap.

Что такое Smallest Range Covering K Lists?
?
Найти наименьший диапазон [a,b] содержащий хотя бы один элемент из каждого списка. K-way Merge + Sliding Window: heap для минимума, отслеживаем максимум. Range = max - heap.top. Двигаем указатель списка с минимумом. O(N log K).

Чем K-way Merge отличается от обычного Merge Sort?
?
Merge Sort: 2-way merge, O(N log N). K-way Merge: K > 2 отсортированных источников, O(N log K). K-way эффективнее когда данные уже разбиты на K отсортированных частей. External sorting: K-way merge chunks с диска.

Какие задачи решает K-way Merge?
?
1) Merge K Sorted Lists. 2) Kth Smallest in M Sorted Arrays. 3) Smallest Range Covering K Lists. 4) External Sorting. 5) Find K Pairs with Smallest Sums. Общий паттерн: несколько отсортированных источников + нужен глобальный порядок.

## Куда дальше

| Тип | Ссылка | Зачем |
|-----|--------|-------|
| Следующий шаг | [[patterns/two-heaps-pattern]] | Two Heaps для медианы |
| Углубиться | [[algorithms/sorting-algorithms]] | External sorting подробно |
| Смежная тема | [[data-structures/heaps-priority-queues]] | Heap операции |
...
- Cormen, Leiserson, Rivest & Stein (2009). *Introduction to Algorithms (CLRS).* — глава 6 (Heapsort) и глава 2 (Merge procedure): формальная основа для heap operations и merge; задачи к главе 6 включают K-way merge как обобщение
- Knuth (1998). *The Art of Computer Programming, Vol. 3: Sorting and Searching.* — детальный анализ external sorting и multiway merging; исторический контекст и оптимальные стратегии слияния K потоков

### Практические руководства

- Sedgewick & Wayne (2011). *Algorithms.* — реализация priority queue на Java; раздел Multiway Merge с практическими примерами и визуализациями работы heap при слиянии
