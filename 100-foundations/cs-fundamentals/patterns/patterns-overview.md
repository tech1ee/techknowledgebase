...
> **Алгоритмический паттерн** — обобщённая стратегия решения, применимая к целому классу задач со схожей структурой. Паттерн описывает не конкретный алгоритм, а *подход*: как декомпозировать задачу и какие инварианты поддерживать.

### Таксономия паттернов

| Мета-стратегия | Суть | Паттерны |
|---------------|------|----------|
| **Сведение к известной задаче** | Трансформировать условие так, чтобы применить знакомый алгоритм | [[binary-search-pattern]], [[two-pointers-pattern]], [[sliding-window-pattern]] |
| **Разделяй и властвуй** | Разбить задачу на независимые подзадачи, решить рекурсивно, объединить | [[k-way-merge-pattern]], [[meet-in-the-middle]] |
| **Запоминай и переиспользуй** | Кэшировать результаты подзадач, избегая повторных вычислений | [[dp-patterns]] |
| **Жадный выбор** | На каждом шаге принимать локально оптимальное решение | [[intervals-pattern]], [[topological-sort-pattern]] |
| **Поддержание инварианта структуры** | Использовать структуру данных с гарантированными свойствами | [[monotonic-stack-pattern]], [[two-heaps-pattern]], [[union-find-pattern]] |

### Принцип выбора паттерна

Распознавание паттерна основано на **сигнатурах задачи**: тип входных данных (отсортированный массив, граф, интервалы), тип вопроса (оптимум, подсчёт, существование) и ограничения (размер входа, допустимая сложность). Метод Полья (1945) формализует этот процесс: *«Знаешь ли ты родственную задачу? Можешь ли ты использовать её результат?»* — см. [[problem-solving-framework]].
