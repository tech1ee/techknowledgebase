...
> **Принцип оптимальности Беллмана (1957):** оптимальная стратегия обладает свойством, что, каковы бы ни были начальное состояние и начальное решение, оставшиеся решения должны составлять оптимальную стратегию относительно состояния, порождённого первым решением.

### Два необходимых условия для DP

| Условие | Определение | Пример |
|---------|-------------|--------|
| **Оптимальная подструктура** | Оптимальное решение задачи содержит оптимальные решения подзадач | Кратчайший путь A→C через B содержит кратчайший A→B |
| **Перекрывающиеся подзадачи** | Одни и те же подзадачи решаются многократно | `fib(5)` вызывает `fib(3)` дважды в наивной рекурсии |

### Два подхода реализации

- **Top-down (мемоизация):** рекурсия с кэшированием; вычисляет только нужные подзадачи; естественнее для написания
- **Bottom-up (табуляция):** итеративное заполнение таблицы от базовых случаев; нет overhead рекурсии; легче оптимизировать пространство

### Формальная структура DP-решения

Любая DP-задача определяется тройкой: (1) **состояние** — что описывает подзадачу, (2) **рекуррентное соотношение** — как связаны подзадачи, (3) **базовый случай** — начальные значения. Контрпример (не DP): longest simple path не имеет оптимальной подструктуры — см. [[backtracking]], [[greedy-algorithms]].
...
| # | Источник | Вклад |
|---|----------|-------|
| 1 | Bellman (1957). *Dynamic Programming* | Оригинальная монография; принцип оптимальности, формализация DP |
| 2 | Cormen et al. (2009). *CLRS* — глава 15 | Optimal substructure, overlapping subproblems, rod cutting, LCS |
...
> [!question]- Почему Fibonacci DP — это O(n), а наивная рекурсия O(2^n), хотя обе вычисляют одно и то же?
> Наивная рекурсия пересчитывает fib(k) экспоненциально много раз: fib(5) вызывает fib(3) дважды, fib(2) трижды и т.д. DP (memo/tabulation) вычисляет каждое fib(k) ровно один раз и кэширует результат. n уникальных подзадач x O(1) на каждую = O(n). Space optimization: достаточно хранить только 2 предыдущих значения.

> [!question]- Как определить состояние DP для задачи 0/1 Knapsack и почему нужны два измерения?
> Состояние dp[i][w] = максимальная ценность из первых i предметов с ограничением веса w. Два измерения: i (какие предметы рассмотрели) и w (оставшаяся вместимость). Переход: dp[i][w] = max(dp[i-1][w], dp[i-1][w-weight[i]] + value[i]). Одного измерения недостаточно — нужно знать И какие предметы доступны, И текущий лимит.

> [!question]- Задача Longest Common Subsequence: почему DP, а не Greedy?
> Greedy (выбрать первый общий символ) не гарантирует оптимум. Пример: 'ABCBDAB' и 'BDCAB' — greedy может пойти по неоптимальному пути. LCS имеет optimal substructure: LCS(i,j) зависит от LCS(i-1,j-1), LCS(i-1,j), LCS(i,j-1). Overlapping subproblems: LCS(i,j) вычисляется многократно в наивной рекурсии.

## Ключевые карточки

Какие 5 основных паттернов DP?
?
1) Fibonacci: dp[i] = f(dp[i-1], dp[i-2]). 2) 0/1 Knapsack: выбрать/не выбрать, dp[i][w]. 3) LCS: две последовательности, dp[i][j]. 4) LIS: возрастающая подпоследовательность, O(n log n). 5) Grid DP: пути в матрице, dp[i][j] = f(dp[i-1][j], dp[i][j-1]).

Чем Top-down отличается от Bottom-up DP?
?
Top-down: рекурсия + мемоизация. Естественнее для написания, вычисляет только нужные подзадачи. Bottom-up: итеративное заполнение таблицы. Нет overhead рекурсии, легче оптимизировать пространство. Оба O(n) по времени. Bottom-up обычно быстрее на практике.

Как оптимизировать пространство в 2D DP?
?
Если dp[i] зависит только от dp[i-1], достаточно хранить два ряда (или один с reverse iteration для 0/1 Knapsack). Пример: LCS O(nm) -> O(min(n,m)). Grid DP: два ряда вместо полной матрицы. Fibonacci: 2 переменные вместо массива.

Что такое optimal substructure?
?
Оптимальное решение задачи содержит оптимальные решения подзадач. Пример: кратчайший путь A->C через B содержит кратчайший путь A->B. Необходимое условие для DP. Контрпример: longest simple path НЕ имеет optimal substructure.

Как распознать DP задачу?
?
Признаки: 1) 'Minimum/Maximum/Count ways'. 2) Overlapping subproblems (пересчёт одного и того же). 3) Optimal substructure. 4) Decision at each step (выбрать/не выбрать). 5) Ограничения позволяют O(n^2) или O(n*W). Если greedy не доказуем — пробуй DP.

## Куда дальше

| Тип | Ссылка | Зачем |
|-----|--------|-------|
| Следующий шаг | [[patterns/intervals-pattern]] | Интервалы — частый DP/Greedy паттерн |
| Углубиться | [[algorithms/dp-optimization]] | Оптимизации DP (Knuth, D&C, CHT) |
| Смежная тема | [[algorithms/dynamic-programming]] | Теория DP подробно |
