...
> **Задача планирования интервалов (Interval Scheduling)** — класс задач комбинаторной оптимизации, где необходимо выбрать максимальное число непересекающихся интервалов или минимизировать ресурсы для обслуживания всех интервалов.

### Ключевые теоремы

- **Activity Selection (жадный алгоритм):** сортировка по правому концу и жадный выбор интервала с наименьшим `end` даёт максимальное число непересекающихся интервалов. Доказательство: exchange argument (замена любого оптимального интервала на «ранний» не ухудшает решение).
- **Теорема Дилворта:** минимальное число ресурсов (переговорных, процессоров) = максимальное число одновременно активных интервалов.

### Техника Sweep Line

Sweep Line (метод заметающей прямой) преобразует интервалы в **события**: `(time, +1)` для начала и `(time, -1)` для конца. Сортировка событий по времени и проход со счётчиком определяют максимальное перекрытие за `O(n log n)`.

### Типы задач на интервалы

| Тип | Подход | Сортировка |
|-----|--------|-----------|
| Merge Intervals | Слияние пересекающихся | По началу (`start`) |
| Activity Selection | Максимум непересекающихся | По концу (`end`) |
| Meeting Rooms | Минимум ресурсов | Sweep Line / Min-Heap |
| Intersection | Общая часть двух списков | Two Pointers |

См. также: [[greedy-algorithms]], [[sorting-algorithms]], [[two-pointers-pattern]].
...
| # | Источник | Вклад |
|---|----------|-------|
| 1 | Cormen et al. (2009). *CLRS* — глава 16 (Greedy Algorithms) | Activity Selection: доказательство оптимальности жадного подхода |
| 2 | de Berg et al. (2008). *Computational Geometry* — глава 2 | Sweep Line: формализация метода заметающей прямой для геометрических задач |
...
> [!question]- Почему для merge intervals достаточно сортировки по началу, а не по обоим концам?
> После сортировки по началу: если intervals[i].start <= intervals[i-1].end, интервалы пересекаются. Сортировка по концу не нужна, потому что при пересечении мы берём max(end). Если start1 <= start2 и start2 <= end1, они точно пересекаются. O(n log n) на сортировку + O(n) на merge.

> [!question]- Задача: минимальное количество переговорных для набора встреч. Почему Sweep Line лучше Merge?
> Merge даёт объединённые интервалы, но не максимальное перекрытие. Sweep Line: создаём события +1 (start) и -1 (end), сортируем. Проходим и отслеживаем текущее количество одновременных встреч. Максимум = ответ. Альтернатива: Min-Heap (отслеживает конец самой ранней встречи).

> [!question]- Когда для интервалов нужна сортировка по концу, а не по началу?
> Activity Selection (максимум непересекающихся интервалов): сортировка по концу. Greedy: выбираем интервал с наименьшим end, удаляем все пересекающиеся. Это оптимально (доказано exchange argument). Сортировка по началу не даёт оптимум — длинный ранний интервал может 'заблокировать' несколько коротких.

## Ключевые карточки

Как работает Merge Intervals?
?
1) Сортировка по start. 2) Инициализация: result = [intervals[0]]. 3) Для каждого следующего: если start <= result[-1].end, merge (update end = max). Иначе — добавить новый. O(n log n). Ключ: после сортировки достаточно одного прохода.

Что такое Insert Interval?
?
Вставить новый интервал в отсортированный список, merge при необходимости. Три фазы: 1) все интервалы до нового (end < new.start). 2) все пересекающиеся (start <= new.end) — merge. 3) все после. O(n).

Какие типы задач на интервалы?
?
1) Merge: объединение пересекающихся. 2) Insert: вставка и merge. 3) Scheduling: минимум ресурсов для всех интервалов. 4) Selection: максимум непересекающихся. 5) Gap finding: пропуски между интервалами. 6) Intersection: общие части двух списков.

Что такое Sweep Line для интервалов?
?
Преобразуем интервалы в события: (time, +1) для start, (time, -1) для end. Сортируем. Проходим, отслеживая counter. Максимум counter = макс. одновременных интервалов. Работает для: meeting rooms, overlapping, max concurrent.

Как найти пересечение двух отсортированных списков интервалов?
?
Two Pointers: по одному указателю на каждый список. Пересечение: [max(start1, start2), min(end1, end2)] если max(start) <= min(end). Двигаем указатель с меньшим end. O(n + m).

## Куда дальше

| Тип | Ссылка | Зачем |
|-----|--------|-------|
| Следующий шаг | [[patterns/top-k-elements-pattern]] | Top K Elements паттерн |
| Углубиться | [[algorithms/greedy-algorithms]] | Greedy для interval scheduling |
| Смежная тема | [[patterns/two-pointers-pattern]] | Two Pointers для intersection |
