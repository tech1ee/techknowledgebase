...
> **Бинарный поиск** — алгоритм поиска на монотонном предикате, сужающий пространство поиска вдвое на каждой итерации. Корректность основана на поддержании инварианта: искомый элемент всегда находится в текущем диапазоне `[lo, hi]`.

### Формальное определение

Пусть задан монотонный предикат `P(x)`: `P(x) = false` для `x < t` и `P(x) = true` для `x >= t`. Бинарный поиск находит граничную точку `t` за `O(log n)` шагов.

### Три варианта поиска

| Вариант | Цель | Инвариант | Результат |
|---------|------|-----------|-----------|
| **Exact match** | Найти элемент `== target` | `arr[mid] == target` → return | Индекс или -1 |
| **Lower bound** | Первый элемент `>= target` | `right = mid` при `arr[mid] >= target` | Позиция вставки (bisect_left) |
| **Upper bound** | Первый элемент `> target` | `right = mid` при `arr[mid] > target` | Позиция после последнего равного (bisect_right) |

### Сложность и применимость

Временная сложность — `O(log n)`, пространственная — `O(1)`. Применим не только к отсортированным массивам, но к любой задаче с монотонным свойством: поиск на ответе (binary search on answer), минимизация/максимизация при монотонной проверке — см. [[searching-algorithms]], [[divide-and-conquer]].
...
| Источник | Вклад |
|----------|-------|
| Knuth (1998). *The Art of Computer Programming, Vol. 3: Sorting and Searching* | Формальный анализ бинарного поиска, доказательство корректности инвариантов |
| Cormen et al. (2009). *Introduction to Algorithms (CLRS)* — глава 2, 12 | Анализ O(log n), бинарный поиск в контексте деревьев поиска и сортировки |
...
> [!question]- Почему Binary Search применим не только к отсортированным массивам, а к любой монотонной функции?
> Binary Search работает когда есть монотонный предикат: f(x) = false для x < threshold, f(x) = true для x >= threshold. Отсортированный массив — частный случай. Другие примеры: поиск минимальной скорости (Koko Eating Bananas), минимальной capacity (Ship Packages), корня числа.

> [!question]- Как избежать бесконечного цикла в Binary Search и какой шаблон самый безопасный?
> Безопасный шаблон: while left < right, mid = left + (right - left) / 2. Для lower bound: if condition -> right = mid, else left = mid + 1. Для upper bound: mid = left + (right - left + 1) / 2, if condition -> left = mid, else right = mid - 1. Ключ: mid всегда смещается к одной стороне, предотвращая зацикливание.

> [!question]- Задача: найти пиковый элемент в массиве (peak element). Почему Binary Search работает на несортированном массиве?
> Peak: nums[i] > nums[i-1] и nums[i] > nums[i+1]. Binary Search работает потому что есть gradient property: если nums[mid] < nums[mid+1], пик справа; иначе — пик слева (или mid сам пик). Гарантия: nums[-1] = nums[n] = -inf, поэтому пик всегда существует. O(log n).

## Ключевые карточки

Что такое Binary Search на ответе?
?
Вместо поиска в массиве — поиск значения ответа. Если можно проверить 'возможен ли ответ X?' за O(n) и ответ монотонен (если X возможен, то X+1 тоже), используем BS по X. Пример: минимальная скорость, максимальная сумма подмассива при разбиении на K частей.

Чем lower_bound отличается от upper_bound?
?
lower_bound: первый элемент >= target. upper_bound: первый элемент > target. Количество элементов == target: upper_bound - lower_bound. В C++: std::lower_bound, std::upper_bound. В Python: bisect_left, bisect_right.

Как Binary Search работает на rotated sorted array?
?
Массив [4,5,6,7,0,1,2]. На каждом шаге одна половина отсортирована. Определяем какая: if nums[left] <= nums[mid] — левая отсортирована. Проверяем, target в отсортированной половине или нет. O(log n).

Когда Binary Search лучше Two Pointers?
?
BS: O(log n) для одного поиска, требует сортировку/монотонность. Two Pointers: O(n) для одного прохода. BS лучше: множественные запросы к статичным данным, поиск на ответе, задачи с monotonic predicate. TP лучше: один проход по данным, пары с условием.

Как избежать integer overflow в mid = (left + right) / 2?
?
Вместо mid = (left + right) / 2 используй mid = left + (right - left) / 2. Это эквивалентно математически, но left + right может overflow для больших значений. В Python overflow нет (arbitrary precision), но в Java/C++ — критично.

## Куда дальше

| Тип | Ссылка | Зачем |
|-----|--------|-------|
| Следующий шаг | [[patterns/dfs-bfs-patterns]] | DFS/BFS — обход графов и деревьев |
| Углубиться | [[algorithms/searching-algorithms]] | Теория поиска подробно |
| Смежная тема | [[patterns/two-pointers-pattern]] | Two Pointers на отсортированных данных |
