...
> [!question]- Как ограничение N <= 20 определяет алгоритм и почему именно 20?
> 2^20 = 1,048,576 ~ 10^6 операций — допустимо за 1-2 секунды. Значит: полный перебор подмножеств (bitmask), bitmask DP, backtracking с pruning. N <= 10 -> permutation brute force (10! = 3.6M). N <= 15 -> bitmask DP с переходами. N <= 20 -> meet in the middle (2^10 * 2^10).

> [!question]- Задача: N = 10^5, Q = 10^5 запросов на подотрезок. Какие структуры данных подходят?
> Offline: Prefix Sum (sum), Sparse Table (min/max). Online update: Segment Tree (любая ассоциативная операция) O(log n) update/query, BIT/Fenwick (prefix sums) O(log n). Sqrt Decomposition O(sqrt n). Выбор: если нет update -> Sparse Table O(1) query. С update -> Segment Tree или BIT.

> [!question]- Почему ключевые слова в условии задачи подсказывают алгоритм?
> 'Shortest path' -> BFS/Dijkstra. 'Minimum/Maximum' + choices -> DP/Greedy. 'All permutations' -> Backtracking. 'Connected components' -> DFS/BFS/Union-Find. 'Substring/Subsequence' -> Sliding Window/DP. 'Schedule/Intervals' -> Sort + Greedy. Pattern matching экономит время на анализ.

## Ключевые карточки

Как constraints определяют сложность алгоритма?
?
N <= 10: O(N!). N <= 20: O(2^N). N <= 500: O(N^3). N <= 5000: O(N^2). N <= 10^5: O(N log N). N <= 10^6: O(N). N <= 10^9: O(log N) или O(sqrt N). N <= 10^18: O(log N) (binary exponentiation).

Какие ключевые слова указывают на DP?
?
'Minimum cost', 'Maximum profit', 'Number of ways', 'Can we achieve?', 'Longest/Shortest subsequence'. Если есть выбор на каждом шаге (take/skip) и optimal substructure — это DP. Если можно доказать greedy choice property — Greedy.

Когда использовать Segment Tree vs Fenwick?
?
Fenwick: prefix sums, point update, проще в реализации, меньше памяти. Segment Tree: любая ассоциативная операция (min, max, GCD), range update (lazy propagation), более гибкий. Правило: если достаточно Fenwick — используй Fenwick.

Как определить задачу на графы?
?
Признаки: 'cities and roads', 'network', 'connections', 'dependencies', 'reachability'. Матрица 0/1 = implicit graph. Constraints: V, E указаны явно. Тип: connected -> DFS/BFS, shortest path -> BFS/Dijkstra, MST -> Kruskal/Prim.

Что делать если не можешь классифицировать задачу?
?
1) Перечитать conditions. 2) Проверить constraints (N определяет сложность). 3) Попробовать brute force для маленьких N и найти паттерн. 4) Рисовать примеры. 5) Думать от обратного (что если ответ X — можно ли проверить?). 6) Binary Search на ответе.

## Куда дальше

| Тип | Ссылка | Зачем |
|-----|--------|-------|
| Следующий шаг | [[interview-prep/leetcode-roadmap]] | LeetCode Roadmap для практики |
| Углубиться | [[patterns/patterns-overview]] | Все паттерны подробно |
| Смежная тема | [[big-o-complexity]] | Сложность алгоритмов |
