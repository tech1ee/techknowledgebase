...
> **Implementation** в контексте CP — навык быстрого и безошибочного перевода алгоритмической идеи в работающий код. Включает оптимизацию ввода/вывода, использование шаблонов, систематическую отладку и знание языковых особенностей.

### Типичные ловушки реализации

| Ловушка | Причина | Решение |
|---------|---------|---------|
| Integer overflow | int32 переполняется при n > 46340 (sqrt(2^31)) | Использовать long long / Int64 по умолчанию |
| Off-by-one | 0-indexed vs 1-indexed, < vs <= | Зафиксировать конвенцию, тесты на n=1 |
| Не инициализированные переменные | Глобальные vs локальные, множественные тесты | memset / fill перед каждым тестом |
| TLE из-за I/O | Scanner (Java), input() (Python) | BufferedReader, sys.stdin.readline |
| WA из-за модулярной арифметики | (a - b) % MOD может быть отрицательным | ((a - b) % MOD + MOD) % MOD |

### Оптимизация ввода/вывода

| Язык | Медленный I/O | Быстрый I/O | Ускорение |
|------|-------------|-------------|-----------|
| C++ | cin/cout (с sync) | scanf/printf или cin без sync | 2-5x |
| Java | Scanner | BufferedReader + StringTokenizer | 5-10x |
| Python | input() | sys.stdin.readline() | 3-5x |
| Kotlin | readLine() | BufferedReader(System.\`in\`.reader()) | 5-10x |

### Языковые особенности для CP

> **Правило простоты:** на контесте код должен быть максимально простым и читаемым. Элегантность и DRY-принципы вторичны. Цель — корректное решение за минимальное время написания и отладки.

| Приём | Описание | Экономия |
|-------|----------|----------|
| Template (шаблон) | Готовый код с Fast I/O, typedefs, макросами | 5-10 мин на контест |
| Debug macro | `#ifdef LOCAL` для вывода промежуточных значений | Быстрая отладка |
| Prewritten structures | DSU, SegTree, BIT в template | 10-15 мин на задачу |

**Связанные темы:** [[competitive-programming-overview]] (обзор CP), [[contest-strategy]] (стратегия контеста), [[common-mistakes]] (пересечение с ошибками на интервью)
...
### Теоретические основы
- **Bentley, J. (1986). "Programming Pearls."** — Классика о ловушках реализации: Column 4 "Writing Correct Programs" показывает, что даже binary search содержит неочевидные баги. Основа культуры тщательного тестирования.
- **Sedgewick, Wayne (2011). "Algorithms."** — Образцовые реализации всех основных алгоритмов на Java с подробным анализом производительности. Код адаптируется как competitive template.

### Практические руководства
- **Halim, Halim (2013). "Competitive Programming 3."** — Обширная коллекция шаблонов кода, приёмов Fast I/O и реализаций стандартных структур данных (DSU, Segment Tree, BIT). Практические советы проверены на тысячах контестов.
- **Skiena (2008). "The Algorithm Design Manual."** — Практические аспекты реализации: как избежать типичных ошибок, как тестировать решения, как выбирать структуры данных. War Stories показывают реальные ловушки.
