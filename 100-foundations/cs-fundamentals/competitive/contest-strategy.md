...
> **Стратегия контеста** — системный подход к распределению времени, выбору задач и управлению состоянием во время соревнования. Основана на принципах теории принятия решений, тайм-менеджмента и спортивной психологии.

### Тайм-менеджмент: формальная модель

Контест длится T минут, содержит N задач возрастающей сложности. Оптимальная стратегия максимизирует `score(solved, penalty)`:

| Фаза | Время | Действия |
|------|-------|----------|
| Разведка | 0-5 мин | Прочитать все задачи, определить тип каждой |
| Лёгкие задачи (A-B) | 5-25 мин | Решить гарантированные задачи, минимизировать penalty |
| Средние задачи (C-D) | 25-90 мин | Выбрать наиболее знакомую, решать с pivot-стратегией |
| Сложные/дебаг | 90-120 мин | Дорешивание, отладка, hacking |

### Стратегия выбора задач

> **Pivot-стратегия:** если задача не поддаётся > 15 минут или получено > 3 WA — переключиться на другую задачу. Стоимость переключения (context switch) ниже стоимости застревания.

### Методология stress testing

Stress testing — автоматический поиск контрпримера:

1. Написать brute force решение (гарантированно корректное, но медленное)
2. Написать генератор случайных тестов (с малыми constraints)
3. Цикл: генерировать тест -> запустить оба решения -> сравнить output
4. При расхождении — найден контрпример для отладки

### Психологические факторы

| Фактор | Влияние | Контрмера |
|--------|---------|-----------|
| Tilt (фрустрация после WA) | Снижение точности на 20-30% | Pause, дыхательная техника, переключение задачи |
| Anchoring (привязка к первому подходу) | Невидение альтернативных решений | Сознательный reframe: "а если это не DP?" |
| Sunk cost fallacy | Продолжение работы над безнадёжной задачей | Жёсткий таймер: 15 мин pivot rule |
| Overconfidence | Не тестирование "очевидного" решения | Чеклист edge cases перед submit |

**Связанные темы:** [[competitive-programming-overview]] (обзор CP), [[implementation-tips]] (реализация), [[problem-classification]] (распознавание типа задачи), [[mock-interview-guide]] (аналогичные навыки для интервью)
...
**[[competitive-programming-overview]]** — Стратегия контеста является одной из ключевых подтем обзора соревновательного программирования. Если обзор даёт общее понимание экосистемы CP (платформы, форматы, рейтинги), то стратегия контеста углубляется в тактику поведения во время конкретного раунда. Без понимания общей картины из overview сложно понять, зачем вообще нужны описанные здесь приёмы тайм-менеджмента и психологической устойчивости. Overview задаёт контекст, стратегия — конкретные действия.

**[[implementation-tips]]** — Стратегия и реализация — две стороны одной медали. Стратегия отвечает на вопрос «какую задачу решать и когда переключаться», а implementation tips — на вопрос «как решать быстро и без багов». Быстрый I/O, готовые шаблоны и навыки debugging из implementation-tips напрямую влияют на скорость реализации, которую стратегия учитывает при распределении времени. Невозможно следовать правилу «A+B за 15 минут», если нет отлаженного template.

**[[problem-classification]]** — Классификация задач по ограничениям — это фундамент для стратегического выбора задачи на контесте. Когда стратегия говорит «просканируй все задачи в первые 10 минут», именно навык из problem-classification позволяет быстро определить тип задачи по N и ключевым словам. Без умения мгновенно классифицировать задачи стратегия greedy-выбора (ROI = очки / время) не работает — невозможно оценить ожидаемое время решения.
...
### Теоретические основы
- **Kahneman, D. (2011). "Thinking, Fast and Slow."** — Когнитивные искажения (anchoring, sunk cost), напрямую влияющие на принятие решений на контесте: выбор задачи, момент переключения, интерпретация WA.
- **Ericsson, K.A. et al. (1993). "The Role of Deliberate Practice in the Acquisition of Expert Performance."** Psychological Review. — Теоретическая основа upsolving и структурированной практики: рост происходит на границе текущих способностей.

### Практические руководства
- **Halim, Halim (2013). "Competitive Programming 3."** — Наиболее практичное руководство по CP. Содержит не только алгоритмы, но и отдельные главы по стратегии контеста, тайм-менеджменту и психологии соревнований.
- **Skiena (2008). "The Algorithm Design Manual."** — «War Stories» — реальные истории решения задач, развивающие стратегическое мышление в условиях неопределённости.
- **Laaksonen (2017). "Guide to Competitive Programming."** — Современное руководство на основе CSES problem set с прогрессией от базовых тем к продвинутым.
...
> [!question]- Почему стратегия 'решить A-B быстро, потом думать над C' обычно лучше, чем 'начать с C'?
> A-B дают гарантированные баллы с минимальным временем. Penalty time растёт за каждую минуту. Быстрые A-B = низкий penalty + время на C-D. Начать с C: риск потратить 30+ мин и не решить, упустив A-B. Исключение: если ты уверен в C и видишь подход сразу — можно начать с неё.

> [!question]- Как определить момент, когда нужно переключиться с задачи на другую (pivot)?
> Правило 3 WA или 15 мин без прогресса: если не видишь подход 15 мин — переключайся. Если 3 Wrong Answer подряд — переключайся (вероятно фундаментальная ошибка в подходе). Возвращайся позже свежим взглядом. Лучше решить 3 задачи из 6, чем застрять на одной.

> [!question]- Почему pre-contest routine влияет на результат и что она включает?
> Физиология: сон >= 7 часов, еда за 1-2 часа, вода. Разогрев: решить 1-2 лёгкие задачи за 15 мин до contest. Подготовка: template открыт, IDE настроена, вкладки с reference. Психология: 'contest — это практика, не экзамен'. Stressed mind делает на 20-30% больше ошибок.

## Ключевые карточки

Какая оптимальная стратегия для Div.2 contest?
?
1) Прочитать A, решить (5-10 мин). 2) Прочитать B, решить (10-15 мин). 3) Прочитать C-D-E, выбрать наиболее знакомую. 4) Решать C (15-25 мин). 5) Если stuck — перейти к D. 6) Последние 15 мин — debug/hack. Цель: стабильно A+B+C.

Что такое penalty time и как его минимизировать?
?
Penalty = суммарное время до каждого accepted submission. Каждый WA = +10 мин penalty. Минимизация: 1) Решай быстрые задачи первыми. 2) Тестируй перед submit (edge cases). 3) Не submit наугад. Penalty различает участников с одинаковым числом решённых.

Как эффективно дебажить на контесте?
?
1) Print debugging (cerr <<). 2) Маленькие тесты вручную. 3) Stress testing: brute force + random tests. 4) Binary search по коду: комментируй половину. 5) Не менять подход после 1 WA — часто ошибка в реализации, не в идее.

Что делать после contest?
?
1) Upsolving: решить нерешённые задачи. 2) Прочитать editorial для всех задач. 3) Изучить решения топ-10 участников. 4) Записать новые паттерны/трюки. 5) Если была ошибка — понять причину (bugs vs wrong approach). Пост-анализ = 50% обучения.

Как справиться со стрессом на контесте?
?
1) Дыхание 4-7-8 (вдох 4с, задержка 7с, выдох 8с). 2) Reframe: 'это тренировка, не экзамен'. 3) Физическая пауза: встать, потянуться. 4) Прочитать другую задачу (переключение контекста). 5) Помнить: один contest не определяет уровень.

## Куда дальше

| Тип | Ссылка | Зачем |
|-----|--------|-------|
| Следующий шаг | [[competitive/implementation-tips]] | Советы по реализации |
| Углубиться | [[competitive/problem-classification]] | Классификация задач по constraints |
| Смежная тема | [[interview-prep/mock-interview-guide]] | Mock интервью — схожие навыки |
