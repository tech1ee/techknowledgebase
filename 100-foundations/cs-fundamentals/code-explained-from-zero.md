...
> **Программирование** — процесс создания инструкций (программ) для компьютера, описывающих последовательность операций над данными. Любая программа сводится к трём базовым конструкциям: **последовательность**, **ветвление**, **цикл** (Бём и Якопини, 1966).

### Таксономия фундаментальных концепций

| Концепция | Что делает | Аналогия |
|-----------|-----------|----------|
| **Переменная** | Хранит данные (число, текст, логическое значение) | Подписанная коробка |
| **Условие (if/else)** | Выбирает одну из ветвей выполнения | Развилка на дороге |
| **Цикл (for/while)** | Повторяет действие заданное число раз или пока условие истинно | Конвейер |
| **Функция** | Именованный блок кода, принимающий входные данные и возвращающий результат | Рецепт |
| **Массив** | Упорядоченная коллекция элементов с доступом по индексу | Ряд пронумерованных ячеек |
| **Рекурсия** | Функция вызывает сама себя для решения подзадачи | Матрёшка |

### Путь изучения

Документ строит знания послойно: (1) переменные и типы данных, (2) условия и циклы, (3) функции и массивы, (4) рекурсия и сортировки, (5) деревья и графы. Каждый слой опирается на предыдущий. Для продвинутых тем — см. [[code-explained-advanced]], [[arrays-strings]], [[recursion-fundamentals]].
