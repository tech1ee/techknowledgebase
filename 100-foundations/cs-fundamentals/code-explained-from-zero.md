...
> [!question]- Почему бинарный поиск работает только с отсортированным массивом?
> Бинарный поиск отбрасывает половину элементов на каждом шаге, сравнивая средний элемент с целевым. Это возможно только если элементы упорядочены — иначе нельзя определить, в какой половине искать. Без сортировки нужен линейный поиск O(n).

> [!question]- Тебе нужно проверить, является ли строка скобок сбалансированной. Почему стек — идеальная структура данных для этой задачи?
> Каждая закрывающая скобка должна соответствовать последней открывающей (LIFO принцип). Стек естественно реализует это: открывающие скобки кладём на вершину, при закрывающей — проверяем и снимаем с вершины. В конце стек должен быть пуст.

> [!question]- Почему наивная реализация Фибоначчи без мемоизации имеет экспоненциальную сложность?
> Каждый вызов fibonacci(n) порождает два рекурсивных вызова, образуя дерево вызовов. При этом одни и те же значения вычисляются многократно: fibonacci(3) вызывается десятки раз при вычислении fibonacci(10). Мемоизация сохраняет результаты и снижает сложность с O(2^n) до O(n).

> [!question]- В чём преимущество связного списка перед массивом для вставки элементов в начало?
> Вставка в начало связного списка — O(1): создаём новый узел и перенаправляем указатель head. В массиве вставка в начало — O(n), потому что все элементы нужно сдвинуть на одну позицию вправо.

---

## Ключевые карточки

Чем отличаются val и var в Kotlin?
?
val — неизменяемая переменная (нельзя переприсвоить после создания). var — изменяемая переменная (можно изменять значение). По умолчанию предпочитайте val.

Что такое стек (Stack)?
?
Структура данных LIFO (последний вошёл — первый вышел). Операции: push (добавить на вершину), pop (удалить с вершины), peek (посмотреть вершину). Пример: стопка тарелок.

Что такое очередь (Queue)?
?
Структура данных FIFO (первый вошёл — первый вышел). Операции: enqueue (добавить в конец), dequeue (удалить с начала). Пример: очередь в магазине.

Какова сложность бинарного поиска?
?
O(log n) по времени, O(1) по памяти. Каждая итерация уменьшает пространство поиска вдвое. Для 1 миллиона элементов нужно около 20 сравнений.

В чём идея Dynamic Programming (bottom-up)?
?
Решение задачи через заполнение таблицы от базовых случаев к целевому. Каждая ячейка использует ранее вычисленные значения. Избегает повторных вычислений, характерных для наивной рекурсии.

Чем DFS отличается от BFS?
?
DFS (обход в глубину) идёт максимально глубоко, затем возвращается — использует стек. BFS (обход в ширину) посещает всех соседей по уровням — использует очередь. BFS находит кратчайший путь в невзвешенном графе.

Какова сложность Merge Sort?
?
O(n log n) по времени всегда (best, average, worst). O(n) по памяти (для временного массива при слиянии). Стабильная сортировка, основанная на принципе Divide & Conquer.
...
|-------------|------|-------|
| Следующий шаг | [[code-explained-advanced]] | Продвинутые алгоритмы с разбором кода |
| Углубиться | [[data-structures/arrays-strings]] | Детальное изучение массивов и строк |
| Смежная тема | [[big-o-complexity]] | Формальный анализ сложности алгоритмов |
