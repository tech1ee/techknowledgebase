...
> [!question]- Почему алгоритм Тарьяна для SCC работает за один DFS, а Косарайю требует два?
> Тарьян использует low-link values и стек: если low[v] == disc[v], все вершины на стеке до v образуют SCC. Косарайю проще, но требует два DFS: первый для порядка finish time, второй по транспонированному графу в обратном порядке. Тарьян экономит один проход.

> [!question]- Как найти мосты в графе и зачем они нужны?
> Мост -- ребро, удаление которого увеличивает число компонент связности. Алгоритм: DFS + low-link. Ребро (u,v) -- мост, если low[v] > disc[u]. Применение: анализ надёжности сетей, critical connections, 2-edge-connected components.

> [!question]- Почему Floyd-Warshall имеет O(V^3), а Dijkstra из каждой вершины -- O(V*(V+E)*log V)?
> Floyd-Warshall: три вложенных цикла по V, всегда V^3. V*Dijkstra: V * (V+E)*log V. Для разреженных графов (E=O(V)) V*Dijkstra = O(V^2 log V) -- быстрее. Для плотных (E=O(V^2)): V*Dijkstra = O(V^3 log V) -- медленнее. Floyd-Warshall также обрабатывает отрицательные веса.

---

## Ключевые карточки

Что такое SCC (Strongly Connected Components)?
?
Максимальное подмножество вершин, где каждая вершина достижима из каждой другой. Алгоритмы: Косарайю (два DFS, O(V+E)), Тарьян (один DFS, O(V+E)). Применение: анализ зависимостей, 2-SAT.

Что такое мосты и точки сочленения?
?
Мост: ребро, удаление которого разрывает граф. Точка сочленения: вершина, удаление которой разрывает граф. Находятся через DFS + low-link values за O(V+E). Критичны для анализа надёжности сетей.

Что такое Floyd-Warshall?
?
All-pairs shortest paths за O(V^3). dp[i][j][k] = кратчайший путь от i до j через промежуточные вершины {1..k}. Работает с отрицательными весами (но не с отрицательными циклами). Прост в реализации: три вложенных цикла.

Что такое Bellman-Ford?
?
Single-source shortest paths за O(V*E). Работает с отрицательными весами. Обнаруживает отрицательные циклы (если после V-1 итераций есть relaxation). Медленнее Dijkstra для неотрицательных весов.

Что такое алгоритм A*?
?
Оптимизация Dijkstra с эвристикой: f(n) = g(n) + h(n), где g -- стоимость пути, h -- эвристическая оценка до цели. При допустимой эвристике (h <= реального расстояния) гарантирует оптимальный путь. Используется в навигации и играх.
...
|-------------|------|-------|
| Следующий шаг | [[algorithms/shortest-paths]] | Все алгоритмы кратчайших путей |
| Углубиться | [[algorithms/network-flow]] | Потоки в сетях |
| Смежная тема | [[algorithms/minimum-spanning-tree]] | Минимальные остовные деревья |
