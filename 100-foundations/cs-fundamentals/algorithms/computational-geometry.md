...
> [!question]- Почему cross product -- основа большинства алгоритмов вычислительной геометрии?
> Cross product определяет ориентацию тройки точек (CCW/CW/collinear) за O(1). Это базовая операция для: convex hull (отсечение неправильных поворотов), пересечение отрезков (точки по разные стороны), площадь многоугольника (формула шнурков), point-in-polygon.

> [!question]- Как Ray Casting определяет, находится ли точка внутри многоугольника?
> Выпускаем луч из точки вправо, считаем пересечения с рёбрами. Нечётное число -- внутри, чётное -- снаружи. Работает для любых многоугольников (даже невыпуклых). Каждый вход/выход из многоугольника добавляет одно пересечение.

> [!question]- Почему алгоритм выпуклой оболочки работает за O(n log n), а не O(n)?
> Нижняя граница -- O(n log n) из-за сортировки точек. Сам обход Graham Scan или Andrew Monotone Chain -- O(n). Для предварительно отсортированных точек convex hull строится за O(n).

---

## Ключевые карточки

Что такое cross product в 2D?
?
cross(a, b) = a.x * b.y - a.y * b.x. Знак определяет ориентацию: > 0 CCW (b слева от a), < 0 CW (b справа), = 0 коллинеарны. Модуль = площадь параллелограмма. Основа всей вычислительной геометрии.

Что такое Convex Hull?
?
Минимальный выпуклый многоугольник, содержащий все точки. Алгоритмы: Graham Scan O(n log n), Andrew Monotone Chain O(n log n), Jarvis March O(nh). Применение: closest pair, rotating calipers, collision detection.

Как проверить пересечение отрезков?
?
Два отрезка AB и CD пересекаются, если: ориентации (A,B,C) и (A,B,D) разные, И ориентации (C,D,A) и (C,D,B) разные. Особый случай: коллинеарные отрезки. Всё через cross product за O(1).

Что такое формула шнурков (Shoelace)?
?
Площадь многоугольника = (1/2) * |sum(x[i]*y[i+1] - x[i+1]*y[i])|. Сумма cross products соседних точек. O(n) для n вершин. Знаковая площадь определяет ориентацию (CW/CCW).

Что такое closest pair of points?
?
Нахождение двух ближайших точек из множества. Наивно O(n^2). Divide and Conquer: делим по x, решаем рекурсивно, проверяем strip шириной 2*delta. O(n log n). Sweep line тоже O(n log n).
...
|-------------|------|-------|
| Следующий шаг | [[algorithms/dp-optimization]] | Оптимизации DP |
| Углубиться | [[code-explained-advanced]] | Реализация всех геометрических алгоритмов |
| Смежная тема | [[algorithms/divide-and-conquer]] | D&C для closest pair |
