...
> [!question]- Почему max flow = min cut (теорема Форда-Фалкерсона)?
> Поток ограничен любым разрезом (все единицы потока должны пересечь разрез). Минимальный разрез -- bottleneck. Max-flow min-cut theorem доказывает, что максимальный поток равен пропускной способности минимального разреза. Алгоритм находит и то, и другое.

> [!question]- Почему алгоритм Диница быстрее Ford-Fulkerson?
> Ford-Fulkerson ищет любой augmenting path (DFS), что может дать O(V*E) путей. Диниц строит layered graph через BFS и ищет blocking flows -- после каждого BFS расстояние до sink увеличивается. Максимум V фаз, итого O(V^2 * E).

> [!question]- Как свести задачу bipartite matching к max flow?
> Добавляем source S и sink T. S соединяем со всеми вершинами левой доли (capacity 1), все вершины правой доли -- с T (capacity 1). Рёбра между долями capacity 1. Max flow = maximum matching. Hopcroft-Karp быстрее для этой конкретной задачи.

---

## Ключевые карточки

Что такое Network Flow?
?
Задача: найти максимальный поток от source (s) до sink (t) в сети с пропускными способностями рёбер. Max-flow min-cut theorem: максимальный поток = минимальный разрез. Применение: matching, routing, scheduling.

Какие алгоритмы max flow существуют?
?
Ford-Fulkerson: DFS augmenting paths, O(V*E^2). Edmonds-Karp: BFS augmenting paths, O(V*E^2). Dinic: layered graph + blocking flow, O(V^2*E). Push-Relabel: O(V^2*E) или O(V^3). Dinic -- наиболее практичный.

Что такое residual graph?
?
Граф остаточных пропускных способностей: для каждого ребра (u,v) с потоком f и capacity c, residual capacity = c-f (прямое) и f (обратное). Обратные рёбра позволяют отменять ранее направленный поток.

Что такое min cut?
?
Минимальный разрез: разделение вершин на S (содержит source) и T (содержит sink), минимизирующее суммарную capacity рёбер из S в T. Равен max flow. Находится через BFS в residual graph после max flow.

Где применяется Network Flow?
?
Bipartite matching, project selection, image segmentation, airline scheduling, network routing, baseball elimination. Многие задачи сводятся к max flow через конструирование графа.
...
|-------------|------|-------|
| Следующий шаг | [[algorithms/string-algorithms]] | Строковые алгоритмы |
| Углубиться | [[algorithms/graph-advanced]] | Другие продвинутые графовые алгоритмы |
| Смежная тема | [[algorithms/minimum-spanning-tree]] | MST как Greedy на графах |
