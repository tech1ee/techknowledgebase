...
> [!question]- Почему KMP работает за O(n+m), а наивный поиск за O(n*m)?
> KMP использует prefix function для пропуска уже совпавших символов. При несовпадении KMP не возвращает указатель текста назад, а сдвигает паттерн на основе таблицы prefix function. Каждый символ текста обрабатывается максимум дважды, давая O(n+m).

> [!question]- Как Rabin-Karp использует хеширование для поиска подстроки?
> Вычисляем хеш паттерна и хеш каждого окна текста длины m через rolling hash (O(1) на сдвиг). При совпадении хешей проверяем символы. Среднее O(n+m), но worst case O(nm) при множестве коллизий. Хорош для multiple pattern search.

> [!question]- Что такое Z-функция и чем она полезна?
> Z[i] = длина максимального общего префикса строки s и подстроки s[i..]. Вычисляется за O(n). Поиск паттерна: конкатенируем pattern + separator + text, ищем Z[i] == len(pattern). Также: нахождение периода строки, количество различных подстрок.
...
?
Алгоритм поиска подстроки за O(n+m). Prefix function: для каждой позиции паттерна -- длина наибольшего proper prefix, который также suffix. При несовпадении сдвигаем паттерн по таблице, не возвращая указатель текста.

Что такое Rabin-Karp?
?
Поиск подстроки через rolling hash. Хеш окна пересчитывается за O(1). Средняя O(n+m), worst O(nm). Преимущество: легко расширяется на multiple pattern search. Используется для plagiarism detection, DNA matching.

Что такое prefix function?
?
pi[i] = длина наибольшего proper prefix строки s[0..i], который также suffix. Вычисляется за O(n). Основа KMP. Также: нахождение периода строки (n - pi[n-1]).

Что такое Z-функция?
?
Z[i] = длина наибольшего общего префикса s и s[i..]. Вычисляется за O(n) с Z-boxes. Альтернатива KMP для pattern matching. Концептуально проще prefix function.

Когда какой строковый алгоритм выбрать?
?
Один паттерн: KMP или Z-функция, O(n+m). Несколько паттернов: Rabin-Karp или Aho-Corasick. Все подстроки: Suffix Array + LCP. Approximate matching: DP (edit distance). Для интервью обычно достаточно KMP/Z.
...
|-------------|------|-------|
| Следующий шаг | [[algorithms/string-advanced]] | Суффиксные структуры и Aho-Corasick |
| Углубиться | [[data-structures/tries]] | Trie для prefix matching |
| Смежная тема | [[algorithms/dynamic-programming]] | DP для строковых задач (LCS, edit distance) |
