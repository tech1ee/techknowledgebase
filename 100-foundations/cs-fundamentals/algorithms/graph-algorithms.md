...
> [!question]- Почему BFS гарантирует кратчайший путь в невзвешенном графе?
> BFS обходит вершины по уровням: сначала все на расстоянии 1, затем 2 и т.д. Первое посещение вершины происходит по кратчайшему пути, потому что все пути длины k обрабатываются до путей длины k+1. В взвешенном графе это не работает -- нужен Dijkstra.

> [!question]- Как обнаружить цикл в ненаправленном графе?
> DFS: если встречаем посещённую вершину, не являющуюся родителем текущей -- цикл. Union-Find: для каждого ребра (u,v), если find(u) == find(v) -- добавление этого ребра создаёт цикл. Union-Find проще для этой конкретной задачи.

> [!question]- В каких случаях DFS предпочтительнее BFS и наоборот?
> DFS: поиск циклов, топологическая сортировка, поиск всех путей, SCC, проверка двудольности. BFS: кратчайший путь (невзвешенный), уровни в дереве, ближайший объект. BFS использует больше памяти (O(width)) vs DFS (O(height)).

---

## Ключевые карточки

Какова сложность BFS и DFS?
?
Оба: O(V + E) по времени, где V -- вершины, E -- рёбра. BFS: O(V) память (очередь). DFS: O(V) память (рекурсия/стек). Для dense графа E = O(V^2), для sparse E = O(V).

Что такое топологическая сортировка?
?
Линейный порядок вершин DAG: для каждого ребра u->v, u перед v. Два алгоритма: DFS (обратный postorder) и Kahn (BFS с in-degree). Применение: сборка зависимостей, расписание, порядок курсов.

Как BFS находит кратчайший путь?
?
Помимо visited массива, храним parent[v] -- откуда пришли в v. После нахождения target восстанавливаем путь: target -> parent[target] -> ... -> start. BFS гарантирует кратчайший путь в невзвешенном графе.

Что такое bipartite graph и как его проверить?
?
Граф, вершины которого можно разделить на два множества так, что все рёбра идут между множествами. Проверка: BFS/DFS раскраска в 2 цвета. Если можно раскрасить без конфликтов -- bipartite.

Как найти connected components?
?
Запускаем DFS/BFS из каждой непосещённой вершины. Каждый запуск находит одну компоненту связности. Для ориентированного графа (SCC): алгоритм Косарайю (два DFS) или Тарьяна.
...
|-------------|------|-------|
| Следующий шаг | [[algorithms/shortest-paths]] | Алгоритмы кратчайших путей |
| Углубиться | [[algorithms/graph-advanced]] | SCC, мосты, точки сочленения |
| Смежная тема | [[patterns/dfs-bfs-patterns]] | Паттерны DFS/BFS для интервью |
