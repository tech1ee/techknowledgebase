---
title: "Теория чисел для алгоритмов"
created: 2026-02-09
modified: 2026-02-13
type: deep-dive
status: published
...
> [!question]- Почему алгоритм Евклида для GCD работает за O(log(min(a,b)))?
> На каждом шаге gcd(a,b) = gcd(b, a%b). Остаток a%b < a/2 (доказывается для обоих случаев: b > a/2 и b <= a/2). Значит за два шага числа уменьшаются минимум вдвое. Итого O(log(min(a,b))) шагов.

> [!question]- Зачем нужна модулярная арифметика в competitive programming?
> Числа в ответах могут быть огромными (например, число путей в графе). Модулярная арифметика (обычно mod 10^9+7 или 998244353) позволяет работать с числами фиксированного размера. (a+b)%m = ((a%m)+(b%m))%m, аналогично для умножения.

> [!question]- Как быстрое возведение в степень работает за O(log n)?
> a^n через binary exponentiation: если n чётное, a^n = (a^(n/2))^2. Если нечётное, a^n = a * a^(n-1). Каждый шаг уменьшает n вдвое. Вместо n умножений -- log n. Работает и с модулем: (a*b) % m.

---

## Ключевые карточки

Что такое GCD и как вычислить?
?
Greatest Common Divisor: наибольший общий делитель. Алгоритм Евклида: gcd(a,b) = gcd(b, a%b), базовый случай gcd(a,0) = a. O(log(min(a,b))). Расширенный Евклид: находит x,y что ax + by = gcd(a,b).

Что такое модулярная арифметика?
?
Арифметика по модулю m: (a+b)%m, (a*b)%m. Вычитание: (a-b+m)%m. Деление: a/b = a * b^(-1) mod m, где b^(-1) = b^(m-2) mod m (малая теорема Ферма, m -- простое). Модуль 10^9+7 -- стандарт CP.

Что такое решето Эратосфена?
?
Нахождение всех простых до n за O(n log log n). Начинаем с 2, вычёркиваем все кратные. Переходим к следующему невычеркнутому. Оптимизация: начинаем вычёркивание с p^2, идём до sqrt(n).

Что такое binary exponentiation?
?
Быстрое возведение a^n за O(log n). Если n чётное: a^n = (a^(n/2))^2. Нечётное: a^n = a * a^(n-1). С модулем: pow(a, n, m). Применение: модулярная инверсия, матричное возведение в степень.

Что такое Китайская теорема об остатках (CRT)?
?
Если m1, m2, ..., mk попарно взаимно просты, система x = a_i (mod m_i) имеет единственное решение mod M = m1*m2*...*mk. Позволяет разбить задачу по модулям и объединить результаты.
...
|-------------|------|-------|
| Следующий шаг | [[algorithms/combinatorics]] | Комбинаторика для подсчёта |
| Углубиться | [[algorithms/dp-optimization]] | Матричное возведение для линейных рекуррентностей |
| Смежная тема | [[algorithms/string-algorithms]] | Хеширование строк использует модулярную арифметику |
