...
> [!question]- Почему Suffix Array + LCP может заменить Suffix Tree для большинства задач?
> Suffix Array занимает O(n) памяти (массив индексов), Suffix Tree -- O(n) но с большой константой (указатели). С LCP array можно решить все те же задачи: longest repeated substring, number of distinct substrings, pattern matching. Suffix Array cache-friendlier.

> [!question]- Как Aho-Corasick решает multi-pattern matching за O(n + m + z)?
> Строим Trie из всех паттернов, добавляем failure links (аналог prefix function KMP для Trie). При обходе текста: при несовпадении переходим по failure link вместо возврата. Каждый символ текста обрабатывается O(1) раз. z -- число найденных совпадений.

> [!question]- Зачем нужен LCP array и как он строится?
> LCP[i] = длина наибольшего общего префикса между SA[i] и SA[i-1] (соседние суффиксы в отсортированном порядке). Строится за O(n) алгоритмом Касаи. Применение: число различных подстрок = n*(n+1)/2 - sum(LCP), longest repeated substring = max(LCP).
...
?
Отсортированный массив всех суффиксов строки (хранятся индексы начала). Построение: O(n log n) или O(n). Занимает O(n) памяти. С LCP array решает большинство строковых задач.

Что такое Aho-Corasick?
?
Multi-pattern matching: ищет все вхождения множества паттернов в тексте за O(n + m + z). Строит автомат из Trie + failure links. Аналог KMP для множества паттернов. Используется в antivirus, spam filters, DNA analysis.

Что такое Suffix Automaton?
?
Минимальный DAG, принимающий все подстроки строки. O(n) состояний, O(n) построение. Решает: число различных подстрок, longest common substring, shortest non-occurring string. Мощнее Suffix Array, но сложнее в реализации.

Что такое Manacher algorithm?
?
Нахождение всех палиндромных подстрок за O(n). Использует симметрию палиндромов для пропуска проверок. Находит longest palindromic substring. Альтернатива DP-подходу O(n^2).

Как построить Suffix Array за O(n log n)?
?
Двоичный подъём: сортируем суффиксы по первым 1, 2, 4, 8, ... символам. На каждом шаге используем ранги предыдущего шага для сравнения. log n шагов, каждый O(n log n) с обычной сортировкой или O(n) с radix sort.
...
|-------------|------|-------|
| Следующий шаг | [[algorithms/number-theory]] | Теория чисел для алгоритмов |
| Углубиться | [[data-structures/tries]] | Trie как основа Aho-Corasick |
| Смежная тема | [[algorithms/string-algorithms]] | Базовые строковые алгоритмы |
