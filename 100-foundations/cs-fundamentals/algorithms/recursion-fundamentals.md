...
> [!question]- Почему каждая рекурсия должна иметь базовый случай?
> Без базового случая рекурсия никогда не остановится, вызывая бесконечную цепочку вызовов до переполнения стека (StackOverflow). Базовый случай -- это условие, при котором функция возвращает результат без рекурсивного вызова, останавливая цепочку.

> [!question]- У тебя рекурсия глубиной 100000. Почему это проблема и как решить?
> Call stack имеет ограниченный размер (обычно 1-8 MB). 100000 уровней рекурсии переполнят стек. Решения: итеративная реализация с явным стеком, tail call optimization (если язык поддерживает), или memoization для уменьшения глубины.

> [!question]- Как мемоизация превращает O(2^n) Fibonacci в O(n)?
> Без мемоизации каждый вызов fib(n) порождает два подвызова, образуя дерево 2^n узлов. С мемоизацией каждое уникальное значение fib(k) вычисляется только один раз и кэшируется. Всего n уникальных подзадач, каждая решается за O(1).

---

## Ключевые карточки

Какие два обязательных компонента рекурсии?
?
1) Базовый случай -- условие остановки, возвращающее результат без рекурсии. 2) Рекурсивный случай -- вызов самой себя с упрощённой подзадачей. Без базового случая -- бесконечная рекурсия, без упрощения -- нет продвижения к решению.

Что такое call stack и как он связан с рекурсией?
?
Call stack хранит состояние каждого вызова функции (параметры, локальные переменные, адрес возврата). При рекурсии каждый вызов добавляет frame на стек. Глубина рекурсии = глубина стека. Переполнение стека = StackOverflow.

Что такое tail recursion?
?
Рекурсия, где рекурсивный вызов -- последняя операция функции. Компилятор может оптимизировать в цикл (tail call optimization), используя O(1) памяти вместо O(n). Kotlin: tailrec keyword. Java/Python: не поддерживают TCO.

Чем top-down отличается от bottom-up подхода?
?
Top-down: начинаем с большой задачи, разбиваем на подзадачи рекурсивно + мемоизация. Bottom-up: решаем от простых подзадач к сложным итеративно (таблица DP). Bottom-up обычно эффективнее по памяти, top-down проще для понимания.

Какие задачи естественно решаются рекурсией?
?
Деревья (обход, высота, поиск), Divide and Conquer (Merge Sort, Quick Sort), backtracking (permutations, subsets), графы (DFS), математические (factorial, Fibonacci, power). Всё, что имеет рекурсивную структуру.
...
|-------------|------|-------|
| Следующий шаг | [[algorithms/dynamic-programming]] | DP как оптимизация рекурсии |
| Углубиться | [[algorithms/backtracking]] | Рекурсивный перебор с отсечением |
| Смежная тема | [[data-structures/trees-binary]] | Рекурсия в деревьях |
