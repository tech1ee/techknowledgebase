...
> [!question]- Почему Binary Search требует отсортированных данных, а Linear Search -- нет?
> Binary Search отбрасывает половину на каждом шаге, сравнивая средний элемент с target. Это возможно только если данные упорядочены -- иначе нельзя определить, в какой половине искать. Linear Search проверяет каждый элемент последовательно, порядок не важен.

> [!question]- Тебе нужно найти минимальную скорость для доставки посылок за D дней. Как Binary Search применим к этой задаче?
> Binary Search on Answer: ищем не элемент в массиве, а ответ в диапазоне [1, max_weight]. Для каждой скорости mid проверяем, успеваем ли за D дней. Если да -- ищем меньшую скорость (right = mid), если нет -- большую (left = mid + 1). O(n log M) где M -- диапазон ответа.

> [!question]- Почему mid = left + (right - left) / 2, а не mid = (left + right) / 2?
> Предотвращение integer overflow. Если left и right близки к MAX_INT, их сумма может переполнить тип. Формула left + (right - left) / 2 математически эквивалентна, но безопасна, потому что right - left не переполняется.

---

## Ключевые карточки

Какова сложность Binary Search?
?
O(log n) по времени, O(1) по памяти (итеративный). Каждая итерация уменьшает пространство поиска вдвое. Для 10^9 элементов -- около 30 сравнений.

Что такое Binary Search on Answer?
?
Применение бинарного поиска не к массиву, а к пространству ответов. Если можно проверить check(x) за O(f(n)) и ответ монотонен, бинарный поиск по ответу даёт O(f(n) * log(range)).

Какие вариации Binary Search существуют?
?
Lower bound (первый >= target), upper bound (первый > target), leftmost/rightmost equal, search in rotated array, peak element, search on answer. Все основаны на одном принципе: сужение диапазона.

Когда использовать Linear Search вместо Binary Search?
?
Когда данные не отсортированы и сортировка не оправдана (один запрос). Когда n мало (< 10-20). Когда структура данных не поддерживает random access (linked list). Linear Search: O(n), не требует предусловий.

Что такое interpolation search?
?
Улучшение Binary Search для равномерно распределённых данных. Вместо mid = (l+r)/2 используем mid = l + (target - arr[l]) * (r-l) / (arr[r] - arr[l]). O(log log n) в среднем, O(n) в худшем.
...
|-------------|------|-------|
| Следующий шаг | [[algorithms/recursion-fundamentals]] | Основы рекурсии |
| Углубиться | [[patterns/binary-search-pattern]] | Паттерн Binary Search для интервью |
| Смежная тема | [[algorithms/sorting-algorithms]] | Сортировка как prerequisite для Binary Search |
