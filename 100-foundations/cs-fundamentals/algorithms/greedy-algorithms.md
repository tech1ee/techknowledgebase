...
> [!question]- Почему Greedy не всегда даёт оптимальный ответ и как это проверить?
> Greedy делает локально оптимальный выбор, но локальный оптимум не всегда ведёт к глобальному. Проверка: exchange argument (если заменить часть жадного решения, станет хуже) или контрпример. Задача о монетах: greedy работает для стандартных номиналов, но не для произвольных.

> [!question]- Тебе нужно выбрать максимум непересекающихся интервалов. Почему жадный выбор по правой границе оптимален?
> Сортируя по правой границе и выбирая интервалы с наименьшим концом, мы освобождаем максимум места для последующих интервалов. Это доказывается exchange argument: любое другое решение можно заменить жадным без потери количества интервалов.

> [!question]- Как отличить задачу для Greedy от задачи для DP?
> Greedy: есть greedy choice property (локальный оптимум ведёт к глобальному) + оптимальная подструктура. DP: есть оптимальная подструктура + перекрывающиеся подзадачи, но нет greedy choice property. Если сомневаешься -- начни с DP, он всегда корректен.

---

## Ключевые карточки

Что такое Greedy алгоритм?
?
Алгоритм, который на каждом шаге делает локально оптимальный выбор, надеясь получить глобальный оптимум. Работает только для задач с greedy choice property. Обычно быстрее DP, но применим не всегда.

Какие классические Greedy задачи существуют?
?
Activity Selection (max non-overlapping intervals), Huffman Coding (оптимальное кодирование), Fractional Knapsack (дробный рюкзак), Minimum Spanning Tree (Kruskal, Prim), Dijkstra (shortest paths с неотрицательными весами).

Что такое exchange argument?
?
Метод доказательства корректности Greedy: берём оптимальное решение и показываем, что замена любого элемента на жадный выбор не ухудшает решение. Если для каждого шага можно провести такой exchange -- Greedy корректен.

Чем Greedy отличается от DP?
?
Greedy: один локальный выбор на шаге, O(n log n) обычно, нет backtracking. DP: рассматривает все варианты, O(n^2) или больше, гарантирует оптимум. Greedy быстрее, но применим только при greedy choice property.

Как доказать, что Greedy даёт оптимальный ответ?
?
Два метода: 1) Exchange argument: показать, что любое решение можно трансформировать в жадное без ухудшения. 2) Matroid theory: задача имеет matroid structure. На практике чаще используют exchange argument.
...
|-------------|------|-------|
| Следующий шаг | [[algorithms/backtracking]] | Полный перебор когда Greedy не работает |
| Углубиться | [[algorithms/minimum-spanning-tree]] | MST как классическое применение Greedy |
| Смежная тема | [[patterns/intervals-pattern]] | Паттерн интервалов (часто Greedy) |
