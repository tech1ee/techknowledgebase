...
> [!question]- Как Master Theorem помогает определить сложность Divide and Conquer?
> Master Theorem решает T(n) = aT(n/b) + f(n): a подзадач размера n/b с f(n) на разделение/слияние. Три случая определяют, доминирует ли работа на рекурсии (case 1), сбалансирована (case 2), или на merge (case 3). Позволяет быстро определить O() без раскрытия рекурсии.

> [!question]- Почему Merge Sort всегда O(n log n), а Quick Sort может деградировать до O(n^2)?
> Merge Sort всегда делит массив ровно пополам -- гарантированно log n уровней. Quick Sort делит по pivot, который может быть неудачным (крайний элемент) -- одна часть n-1, другая 0, что даёт n уровней. Randomized pivot решает это в среднем.

> [!question]- Как Karatsuba ускоряет умножение длинных чисел?
> Наивное умножение n-значных чисел: 4 подумножения размера n/2 -> O(n^2). Karatsuba: 3 подумножения за счёт алгебраического трюка (ac, bd, (a+b)(c+d)-ac-bd). T(n) = 3T(n/2) + O(n) -> O(n^1.585).

---

## Ключевые карточки

Что такое Divide and Conquer?
?
Парадигма: 1) Divide -- разбить задачу на подзадачи. 2) Conquer -- решить подзадачи рекурсивно. 3) Combine -- объединить решения. Примеры: Merge Sort, Quick Sort, Binary Search, Karatsuba multiplication.

Что такое Master Theorem?
?
Формула для T(n) = aT(n/b) + f(n). Case 1: f(n) = O(n^(log_b(a)-e)) -> T = O(n^log_b(a)). Case 2: f(n) = Theta(n^log_b(a)) -> T = O(n^log_b(a) * log n). Case 3: f(n) = Omega(...) -> T = O(f(n)).

Какие алгоритмы используют Divide and Conquer?
?
Merge Sort (T=2T(n/2)+O(n) -> O(n log n)), Binary Search (T=T(n/2)+O(1) -> O(log n)), Quick Sort (avg O(n log n)), Karatsuba (O(n^1.585)), Strassen matrix mult (O(n^2.807)), Closest pair of points (O(n log n)).

Чем D&C отличается от DP?
?
D&C: подзадачи не перекрываются, каждая решается один раз. DP: подзадачи перекрываются, результаты кэшируются. Merge Sort -- D&C (части массива не пересекаются). Fibonacci -- DP (fib(3) вычисляется многократно).

Что такое рекурсивная декомпозиция?
?
Разбиение задачи на подзадачи того же типа, но меньшего размера. Ключевое свойство D&C: подзадачи независимы. Это позволяет параллелизацию (Fork-Join framework, MapReduce).
...
|-------------|------|-------|
| Следующий шаг | [[algorithms/graph-algorithms]] | Графовые алгоритмы |
| Углубиться | [[algorithms/dp-optimization]] | Divide and Conquer оптимизация DP |
| Смежная тема | [[algorithms/sorting-algorithms]] | Merge Sort и Quick Sort как D&C |
