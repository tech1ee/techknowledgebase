...
### Проблема: DP слишком медленное

Динамическое программирование -- один из самых мощных алгоритмических инструментов. Но у него есть фундаментальная проблема: для вычисления каждого состояния часто нужно перебрать все предыдущие состояния. Если состояний n, то для каждого мы перебираем O(n) предыдущих, получая O(n^2). Для задач разбиения на k частей -- O(kn^2) или даже O(n^3).

При n = 10^5 (типичный constraint в олимпиадных задачах) O(n^2) = 10^10 операций -- это timeout. Нужно O(n log n) или O(n).

Аналогия: представьте, что вы ищете лучшего поставщика для каждого из n магазинов. Наивный подход -- для каждого магазина обзвонить ВСЕХ поставщиков. Но если вы знаете, что поставщики "специализируются" по регионам (поставщик А лучше для западных магазинов, Б -- для восточных), вы можете заранее построить "карту лучших поставщиков" и для каждого магазина сразу найти ответ.

Оптимизации DP работают по тому же принципу: они обнаруживают СТРУКТУРУ в переходах DP и используют её для ускорения.
...
Наивно: O(n^2) — проверяем все j для каждого i
...
### Две ключевые идеи: Knuth и Convex Hull Trick

Все оптимизации DP сводятся к одной фундаментальной идее: **не перебирать все предыдущие состояния, а использовать структуру задачи, чтобы сразу найти оптимальное**.

**Оптимизация Кнута** основана на наблюдении, что оптимальная точка разбиения МОНОТОННА: если для отрезка длины 20 лучшая точка разреза -- позиция 8, то для отрезка длины 21 она будет >= 8. Это позволяет сузить диапазон поиска. Кнут доказал это свойство для задачи Optimal Binary Search Tree в 1971 году, но условие применимо ко многим задачам разбиения на подотрезки.

Почему монотонность работает? Представьте разрезание верёвки на части минимальной стоимости. Если верёвка стала длиннее, оптимальная точка первого разреза сдвинется вправо (или останется на месте), но не сдвинется влево. Это интуитивно: дополнительная длина "тянет" оптимум правее. Формальное условие для этого -- quadrangle inequality на функции стоимости: cost(a,c) + cost(b,d) <= cost(a,d) + cost(b,c) для a <= b <= c <= d.

**Convex Hull Trick** основан на геометрической интерпретации. Если переход DP можно записать как dp[i] = min(dp[j] + b[j] * a[i]), то каждое состояние j определяет ЛИНЕЙНУЮ ФУНКЦИЮ y = b[j] * x + dp[j]. Для каждого i мы ищем минимум этих функций в точке x = a[i]. Минимум линейных функций -- это нижняя выпуклая оболочка (convex hull), и для неё можно эффективно поддерживать структуру данных.

Почему это работает? Вспомните тарифы такси (аналогия из раздела выше). Каждый тариф -- прямая. Нижняя огибающая показывает лучший тариф для каждого расстояния. Если добавлять тарифы с монотонно убывающим наклоном и запрашивать расстояния в порядке возрастания, оболочку можно поддерживать за амортизированный O(1) на операцию.
...
**[[dynamic-programming]]** -- Базовое DP (memoization, tabulation, optimal substructure, overlapping subproblems) -- абсолютный prerequisite. Оптимизации DP не меняют саму идею DP, они ускоряют ПЕРЕХОД между состояниями. Без глубокого понимания базового DP оптимизации будут казаться магией.

**[[segment-tree]]** -- Segment Tree часто используется КАК оптимизация DP: вместо перебора всех предыдущих состояний, мы храним их в Segment Tree и находим минимум/максимум за O(log n). Это "универсальная" оптимизация, которая работает для многих видов DP.

**[[divide-and-conquer]]** -- D&C Optimization DP напрямую использует принцип Divide and Conquer: найти opt для середины, рекурсивно решить левую и правую половины с сужённым диапазоном поиска. Понимание мастер-теоремы помогает оценить сложность.

**[[convex-hull]]** -- Convex Hull Trick использует геометрическую идею выпуклой оболочки. Понимание, что нижняя огибающая линейных функций -- это нижняя часть convex hull в пространстве "наклон-отсечение", связывает алгебраическую оптимизацию с геометрической интуицией.
...
> [!question]- Почему Convex Hull Trick оптимизирует DP с переходами dp[i] = min(dp[j] + b[j]*a[i])?
> Переход эквивалентен запросу минимума набора линейных функций y = b[j]*x + dp[j] в точке x = a[i]. CHT поддерживает lower convex hull этих линий. Если a[i] монотонны, запрос O(1) (pointer movement). Иначе O(log n) binary search. Вместо O(n^2) получаем O(n log n) или O(n).

> [!question]- Когда Divide and Conquer оптимизация DP применима?
> Когда opt(i) <= opt(i+1) -- оптимальная точка перехода монотонна. dp[i][j] = min(dp[i-1][k] + cost(k+1,j)). D&C: решаем для среднего j, рекурсивно для половин с суженным диапазоном k. O(n * m * log n) вместо O(n * m * n).

> [!question]- Что такое Knuth optimization и когда она работает?
> Для dp[i][j] = min(dp[i][k] + dp[k][j] + cost(i,j)), если cost удовлетворяет quadrangle inequality и opt[i][j-1] <= opt[i][j] <= opt[i+1][j], то можно ограничить диапазон поиска. O(n^2) вместо O(n^3). Применяется к optimal BST, matrix chain.

---

## Ключевые карточки

Что такое Convex Hull Trick (CHT)?
?
Оптимизация DP: поддерживает набор линейных функций y = kx + b, отвечает на запрос min/max в точке x. Lower/upper convex hull линий. Добавление O(1) amortized, запрос O(1) (monotone) или O(log n). Снижает O(n^2) DP до O(n) или O(n log n).

Что такое Li Chao Tree?
?
Дерево отрезков для CHT: хранит линейные функции, поддерживает add line O(log C) и query O(log C), где C -- диапазон x. Проще CHT в реализации, работает для произвольных запросов (не требует монотонности). Популярен в CP.

Что такое D&C DP optimization?
?
Divide and Conquer оптимизация для DP с монотонным opt: dp[i][j] = min(dp[i-1][k] + cost(k+1,j)). Если opt(j) монотонно, решаем рекурсивно: средний j, сужаем диапазон k для половин. O(n*m*log n) вместо O(n*m*n).

Что такое Knuth optimization?
?
Для interval DP вида dp[i][j] = min(dp[i][k] + dp[k][j] + cost(i,j)) с quadrangle inequality. opt[i][j-1] <= opt[i][j] <= opt[i+1][j]. Позволяет перебирать k в ограниченном диапазоне. O(n^2) вместо O(n^3).

Что такое SOS DP (Sum over Subsets)?
?
DP по подмножествам: для каждой маски m вычислить сумму f(s) по всем подмножествам s. Наивно O(3^n), SOS DP за O(n * 2^n) через итерацию по битам. Применение: maximum AND/OR pair, counting divisors в bitmask.
...
|-------------|------|-------|
| Следующий шаг | [[competitive/competitive-programming-overview]] | Обзор competitive programming |
| Углубиться | [[algorithms/dynamic-programming]] | Основы DP |
| Смежная тема | [[algorithms/computational-geometry]] | CHT для геометрических задач |
| Обзор | [[cs-fundamentals-overview]] | Вернуться к карте раздела |

*Обновлено: 2026-02-10 -- Добавлена глубокая теория: почему DP нуждается в оптимизациях (проблема O(n^2)), две ключевые идеи (монотонность Кнута и геометрическая интерпретация CHT), интуиция за quadrangle inequality, связь CHT с геометрией выпуклой оболочки*
