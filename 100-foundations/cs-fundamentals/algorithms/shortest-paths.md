...
> [!question]- Почему Dijkstra не работает с отрицательными весами?
> Dijkstra помечает вершину как обработанную после извлечения из priority queue. С отрицательным ребром может существовать более дешёвый путь через ещё не обработанную вершину. Bellman-Ford решает это, делая V-1 проходов по всем рёбрам.

> [!question]- Когда выбрать Dijkstra, а когда Bellman-Ford или Floyd-Warshall?
> Dijkstra: один источник, неотрицательные веса, O((V+E)log V). Bellman-Ford: один источник, отрицательные веса, O(VE). Floyd-Warshall: все пары, O(V^3). Для разреженных графов Dijkstra быстрее всех.

> [!question]- Как 0-1 BFS ускоряет Dijkstra для графов с весами 0 и 1?
> Вместо priority queue используем deque: рёбра весом 0 добавляем в начало, весом 1 -- в конец. Это даёт O(V+E) вместо O((V+E)log V). Работает потому что в deque всегда не более двух различных расстояний.

---

## Ключевые карточки

Какие алгоритмы кратчайших путей существуют?
?
BFS: невзвешенный граф, O(V+E). Dijkstra: неотрицательные веса, O((V+E)log V). Bellman-Ford: отрицательные веса, O(VE). Floyd-Warshall: все пары, O(V^3). 0-1 BFS: веса 0/1, O(V+E). A*: с эвристикой.

Как работает Dijkstra?
?
Priority queue с (distance, vertex). Извлекаем вершину с минимальным расстоянием, релаксируем её соседей. Помечаем как обработанную. Гарантирует оптимальность при неотрицательных весах. O((V+E)log V) с binary heap.

Что такое relaxation?
?
Обновление расстояния: если dist[u] + weight(u,v) < dist[v], то dist[v] = dist[u] + weight(u,v). Ключевая операция всех алгоритмов кратчайших путей. Bellman-Ford делает V-1 раундов relaxation по всем рёбрам.

Как обнаружить отрицательный цикл?
?
Bellman-Ford: после V-1 итераций, если ещё возможна relaxation -- есть отрицательный цикл. Floyd-Warshall: если dist[i][i] < 0. SPFA: если вершина попадает в очередь >= V раз.

Что такое SPFA?
?
Shortest Path Faster Algorithm: оптимизация Bellman-Ford с очередью. Релаксируем только соседей обновлённых вершин. В среднем O(E), в худшем O(VE). Популярен в competitive programming, но Dijkstra надёжнее.
...
|-------------|------|-------|
| Следующий шаг | [[algorithms/minimum-spanning-tree]] | MST алгоритмы |
| Углубиться | [[algorithms/network-flow]] | Потоки и минимальные разрезы |
| Смежная тема | [[data-structures/heaps-priority-queues]] | Priority Queue для Dijkstra |
