...
> [!question]- Почему Kruskal и Prim дают одинаковый результат, но работают по-разному?
> Оба используют Greedy и дают MST (или один из MST при равных весах). Kruskal: сортирует рёбра глобально, добавляет минимальное не создающее цикл (DSU). Prim: растит дерево от вершины, добавляя минимальное ребро к не-дереву (priority queue). Kruskal лучше для разреженных, Prim для плотных графов.

> [!question]- Как Cut Property доказывает корректность MST алгоритмов?
> Cut Property: для любого разреза графа (разделения вершин на два множества) минимальное ребро, пересекающее разрез, принадлежит MST. И Kruskal, и Prim на каждом шаге выбирают такое ребро, что гарантирует корректность.

> [!question]- Когда MST единственно, а когда их несколько?
> MST единственно, если все веса рёбер различны. При наличии рёбер с одинаковыми весами может быть несколько MST. Все MST имеют одинаковый суммарный вес. Количество MST может быть экспоненциальным.
...
Что такое MST?
?
Minimum Spanning Tree: подграф, соединяющий все вершины с минимальным суммарным весом рёбер, без циклов. Содержит ровно V-1 рёбро. Применение: кабельные сети, кластеризация, приближённый TSP.

Как работает алгоритм Крускала?
?
1) Сортируем все рёбра по весу. 2) Перебираем в порядке возрастания. 3) Добавляем ребро, если не создаёт цикл (проверка через DSU). O(E log E) из-за сортировки. Лучше для разреженных графов.

Как работает алгоритм Прима?
?
1) Начинаем с произвольной вершины. 2) Добавляем минимальное ребро, ведущее из дерева наружу (priority queue). 3) Повторяем V-1 раз. O((V+E) log V) с binary heap. Лучше для плотных графов.

Что такое Boruvka algorithm?
?
Каждая компонента параллельно выбирает минимальное выходящее ребро. Число компонент уменьшается вдвое за итерацию. O(E log V). Хорошо параллелизуется. Используется как шаг в алгоритмах для distributed MST.

Какая связь MST и кластеризации?
?
Удаляя k-1 самых тяжёлых рёбер из MST, получаем k кластеров с максимальным минимальным расстоянием между кластерами. Это single-linkage clustering. Простой и эффективный метод для euclidean данных.
...
|-------------|------|-------|
| Следующий шаг | [[algorithms/network-flow]] | Потоки в сетях |
| Углубиться | [[algorithms/graph-advanced]] | Продвинутые графовые алгоритмы |
| Смежная тема | [[patterns/union-find-pattern]] | Union-Find для алгоритма Крускала |
