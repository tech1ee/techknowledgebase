...
## Теоретические основы: формальный базис DP

### Происхождение термина

> **Атрибуция:** Термин "Dynamic Programming" введён **Richard Bellman** (1953) в RAND Corporation. Слово "dynamic" выбрано для впечатления на руководство (Bellman писал: "programming" звучало научно, а "dynamic" — впечатляюще). С "динамическим" в физическом смысле метод не связан.

### Два необходимых условия для DP

> **Определение:** Задача решается методом DP ⟺ она обладает:
...
Если есть только (1) без (2) — это Divide and Conquer (MergeSort). Если есть (2) без (1) — задача не оптимизационная.

### Принцип оптимальности Беллмана

> **Принцип (Bellman, 1957):** "Оптимальная политика обладает свойством: каков бы ни был начальный стат и начальное решение, оставшиеся решения должны составлять оптимальную политику относительно состояния, возникающего в результате первого решения."

Формально: если P* = (d₁*, d₂*, ..., dₙ*) — оптимальная последовательность решений, то (d₂*, ..., dₙ*) — оптимальная для подзадачи, начинающейся с состояния после d₁*.

### Memoization vs Tabulation: два подхода

| | Memoization (Top-Down) | Tabulation (Bottom-Up) |
|---|------------------------|----------------------|
| **Направление** | От задачи к подзадачам | От подзадач к задаче |
| **Реализация** | Рекурсия + кэш | Итерация + таблица |
| **Вычисляет** | Только нужные подзадачи (lazy) | Все подзадачи (eager) |
| **Stack overflow** | Возможен при глубокой рекурсии | Нет |
| **Формально** | Кэширование функции f: State → Value | Заполнение массива dp[state] |

### Связь с теорией графов

Любая DP-задача — кратчайший (или длиннейший) путь в **DAG подзадач** (Directed Acyclic Graph). Каждая подзадача — вершина, зависимости — рёбра. Topological sort этого DAG даёт порядок вычисления для tabulation.

### Связи

- [[recursion-fundamentals]] — рекурсия как основа memoization
- [[greedy-algorithms]] — greedy = DP без перебора всех вариантов
- [[graph-algorithms]] — DP на графах (Bellman-Ford, Floyd-Warshall)
