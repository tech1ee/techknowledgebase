...
## Теоретические основы: формальный базис деревьев

### Формальное определение

> **Определение:** Дерево (tree) — связный ациклический граф T = (V, E), где |E| = |V| - 1. **Корневое дерево** (rooted tree) — дерево с выделенной вершиной r (root). **Бинарное дерево** — корневое дерево, где каждый узел имеет не более 2 детей.

### BST Property: формальный инвариант

> **Инвариант BST:** Для каждого узла v: все ключи в left(v) < key(v) < все ключи в right(v).

Этот инвариант обеспечивает **бинарный поиск** на дереве: O(h) для поиска, вставки, удаления, где h — высота.

| Тип BST | Высота h | Поиск worst case |
|---------|---------|-----------------|
| **Несбалансированное** | O(n) — вырождение в список | O(n) |
| **Случайное** | O(log n) в среднем | O(log n) avg |
| **AVL** | ≤ 1.44 log n | O(log n) |
| **Red-Black** | ≤ 2 log n | O(log n) |

### Обходы дерева: формальные определения

Для дерева с корнем r, левым поддеревом L и правым поддеревом R:
- **Inorder** (LNR): inorder(L), visit(r), inorder(R) → **сортированный порядок** для BST
- **Preorder** (NLR): visit(r), preorder(L), preorder(R) → **сериализация** дерева
- **Postorder** (LRN): postorder(L), postorder(R), visit(r) → **удаление** дерева
- **Level-order** (BFS): по уровням слева направо → **минимальная глубина**

**Теорема:** Inorder + Preorder (или Inorder + Postorder) **однозначно** определяют бинарное дерево. Только Preorder + Postorder — нет (неоднозначность для узлов с одним ребёнком).

### Связи

- [[trees-advanced]] — AVL, Red-Black как self-balancing BST
- [[recursion-fundamentals]] — деревья = рекурсивная структура данных
- [[graph-algorithms]] — дерево = частный случай графа
...
**[[trees-advanced]]** — после освоения базовых BST логичный следующий шаг --- самобалансирующиеся деревья. AVL-деревья гарантируют строгий баланс с разницей высот поддеревьев не более 1, Red-Black деревья ослабляют инвариант ради меньшего числа вращений при вставке/удалении, а B-деревья оптимизированы под блочное чтение с диска. Все три структуры решают главную проблему наивного BST --- вырождение в связный список при отсортированных данных, обеспечивая гарантированное O(log n) для всех операций.

**[[graph-algorithms]]** --- деревья являются частным случаем графов: связный ациклический неориентированный граф. DFS и BFS, изученные на деревьях, напрямую переносятся на графы --- с добавлением массива visited для обработки циклов. Многие графовые алгоритмы (Kruskal, Prim) строят остовное дерево графа, а топологическая сортировка DAG --- обобщение postorder-обхода дерева.

**[[dfs-bfs-patterns]]** --- паттерны обхода деревьев (preorder, inorder, postorder) являются частными случаями DFS на графах. Preorder соответствует обработке вершины при входе, postorder --- при выходе, а inorder специфичен для бинарных деревьев и даёт отсортированный порядок в BST. Level-order обход дерева --- это BFS, который на графах используется для нахождения кратчайших путей в невзвешенных графах.
...
- **Cormen, Leiserson, Rivest & Stein (2009). "Introduction to Algorithms."** --- главы 12--13: BST, Red-Black trees, детальный анализ операций вставки, удаления и поиска с доказательствами сложности
- **Sedgewick & Wayne (2011). "Algorithms."** --- раздел Binary Search Trees с превосходными визуализациями, пошаговыми примерами и реализацией на Java
- **Skiena (2008). "The Algorithm Design Manual."** --- практические применения деревьев в реальных системах, рекомендации по выбору структуры данных
...
> [!question]- Почему BST может деградировать до O(n) операций и как это предотвратить?
> Если элементы вставляются в отсортированном порядке, BST превращается в связный список с высотой n. Решение -- самобалансирующиеся деревья (AVL, Red-Black), которые гарантируют высоту O(log n) через ротации после каждой вставки/удаления.

> [!question]- Тебе дано бинарное дерево и нужно найти максимальную глубину. Какой обход выбрать -- DFS или BFS, и почему?
> DFS (рекурсивный) естественнее: max_depth(node) = 1 + max(max_depth(left), max_depth(right)). Код в 3 строки. BFS тоже работает (считаем уровни), но требует очередь и больше кода. Оба O(n) по времени, но DFS использует O(h) памяти (h -- высота), а BFS -- O(w) (w -- ширина).

> [!question]- Какие три основных порядка обхода дерева и когда каждый полезен?
> Inorder (L-Root-R): даёт отсортированные элементы BST. Preorder (Root-L-R): для сериализации/копирования дерева. Postorder (L-R-Root): для удаления дерева, вычисления размеров поддеревьев. Level-order (BFS): для обхода по уровням.

> [!question]- Почему inorder обход BST даёт элементы в отсортированном порядке?
> Свойство BST: все элементы левого поддерева < корня < все элементы правого поддерева. Inorder сначала обходит левое поддерево (меньшие), затем корень, затем правое (большие). Рекурсивно это гарантирует возрастающий порядок.

---

## Ключевые карточки

Какое главное свойство BST?
?
Для каждого узла: все значения в левом поддереве < значение узла < все значения в правом поддереве. Это свойство обеспечивает O(log n) поиск в сбалансированном дереве.

Какова сложность операций сбалансированного BST?
?
Search, Insert, Delete: всё O(log n). Inorder traversal: O(n). Min/Max: O(log n) -- идём до крайнего левого/правого листа. Несбалансированное BST может деградировать до O(n).

Какие три порядка обхода дерева (DFS)?
?
Inorder (L-Root-R) -- отсортированный порядок для BST. Preorder (Root-L-R) -- сериализация. Postorder (L-R-Root) -- удаление, вычисление размеров. Все три реализуются рекурсивно или через стек.

Как удалить узел из BST?
?
Три случая: 1) лист -- просто удаляем. 2) один ребёнок -- заменяем узел ребёнком. 3) два ребёнка -- заменяем inorder successor (минимум в правом поддереве) или inorder predecessor, затем удаляем successor.

Что такое level-order traversal?
?
BFS обход дерева по уровням: сначала корень, потом все узлы глубины 1, потом глубины 2 и т.д. Реализуется через очередь. Используется для поиска минимальной глубины, serialization.

Какая разница между binary tree и BST?
?
Binary tree -- каждый узел имеет максимум двух детей, без ограничений на значения. BST -- binary tree с упорядоченностью: left < root < right. BST позволяет O(log n) поиск, обычное дерево -- нет.
...
| Следующий шаг | [[data-structures/graphs]] | Графы как обобщение деревьев |

| Углубиться | [[data-structures/trees-advanced]] | Самобалансирующиеся деревья AVL и Red-Black |

| Смежная тема | [[data-structures/heaps-priority-queues]] | Другое бинарное дерево со свойством кучи |
