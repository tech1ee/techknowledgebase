...
> **Задача RMQ (Range Minimum Query)** — для статического массива A[0..n-1] ответить на запрос: "каков минимальный элемент на подотрезке A[l..r]?" Sparse Table решает эту задачу с предобработкой O(n log n) и ответом на запрос за O(1).

### Формальная модель

Sparse Table хранит двумерный массив `st[i][j]`, где `st[i][j]` = результат операции на подотрезке A[i..i+2^j-1] длины 2^j:

- **Построение:** `st[i][0] = A[i]`; `st[i][j] = op(st[i][j-1], st[i + 2^(j-1)][j-1])` — динамическое программирование
- **Запрос:** для отрезка [l, r] находим k = floor(log2(r-l+1)), ответ = `op(st[l][k], st[r-2^k+1][k])`

| Параметр | Значение | Пояснение |
|----------|----------|-----------|
| Предобработка | O(n log n) | Заполнение таблицы по уровням |
| Память | O(n log n) | n строк, log n столбцов |
| Запрос (idempotent) | **O(1)** | Два перекрывающихся диапазона |
| Запрос (non-idempotent) | O(log n) | Разбиение без перекрытий |

### Идемпотентность как ключевое свойство

> **Идемпотентная операция** — операция f, для которой f(a, a) = a. Примеры: min, max, GCD, AND, OR. Контрпримеры: sum (sum(a, a) = 2a ≠ a), XOR.

Идемпотентность позволяет перекрывать диапазоны при запросе без потери корректности: `min(A[l..l+2^k-1], A[r-2^k+1..r])` = `min(A[l..r])`, даже если диапазоны пересекаются. Именно это обеспечивает O(1) ответ.

### Связь с задачей LCA

Задача LCA (Lowest Common Ancestor) на деревьях сводится к RMQ через Euler Tour (Bender, Farach-Colton, 2000): LCA(u, v) = вершина с минимальной глубиной между первым появлением u и v в Euler Tour. Это даёт O(1) LCA с O(n log n) предобработкой через Sparse Table.

**Связанные темы:** [[segment-tree]] (динамическая альтернатива), [[fenwick-tree]] (prefix sums), [[divide-and-conquer]] (теоретическая основа декомпозиции)
...
### Теоретические основы
- **Bender, M. & Farach-Colton, M. (2000). "The LCA Problem Revisited."** LATIN 2000. — Оригинальная статья, показавшая сведение LCA к RMQ и алгоритм O(n) построения для +-1 RMQ. Фундаментальная работа в области алгоритмов на деревьях.
- **Gabow, H., Bentley, J. & Tarjan, R. (1984). "Scaling and Related Techniques for Geometry Problems."** STOC. — Ранние результаты по задаче RMQ, предшествовавшие Sparse Table.
- **Cormen, T. et al. (2009). "Introduction to Algorithms (CLRS)."** — Главы о динамическом программировании и amortized analysis дают теоретическую базу для понимания препроцессинга Sparse Table.
...
- [CP-Algorithms — Sparse Table](https://cp-algorithms.com/data_structures/sparse-table.html) — полное руководство с реализациями на C++ и разбором всех вариантов (idempotent/non-idempotent, 2D, LCA).
...
> [!question]- Почему Sparse Table даёт O(1) для min/max, но не для sum?
> Min/max -- идемпотентные операции: min(a, a) = a. Можно перекрыть два диапазона без потери корректности: min([l, l+2^k-1], [r-2^k+1, r]). Для суммы перекрытие даёт двойной подсчёт, поэтому нужно разбиение без перекрытий, что требует O(log n).

> [!question]- Тебе дан статический массив из 10 миллионов элементов и 10 миллионов запросов на минимум отрезка. Sparse Table или Segment Tree?
> Sparse Table: preprocessing O(n log n), каждый запрос O(1). Итого: 10M*log(10M) + 10M = ~233M + 10M. Segment Tree: build O(n), каждый запрос O(log n). Итого: 10M + 10M*23 = ~240M. Sparse Table быстрее из-за O(1) queries при большом количестве запросов.

> [!question]- Почему Sparse Table не подходит для динамических данных?
> Любое изменение элемента требует пересчёта всех перекрывающих диапазонов -- до O(n log n) обновлений. Для динамических данных Segment Tree даёт O(log n) update. Sparse Table оптимален только для immutable массивов.
...
?
Структура для статических Range Minimum/Maximum Query за O(1). Preprocessing O(n log n), память O(n log n). Хранит ответы для диапазонов длины 2^k. Работает только для идемпотентных операций.

Что такое идемпотентная операция?
?
Операция f, где f(a, a) = a. Примеры: min, max, GCD, AND, OR. Не идемпотентны: sum, XOR. Идемпотентность позволяет перекрывать диапазоны без ошибок, давая O(1) queries.

Как Sparse Table отвечает на запрос min(l, r)?
?
Находим k = floor(log2(r-l+1)). Ответ = min(table[l][k], table[r-2^k+1][k]). Два диапазона длины 2^k покрывают [l,r] с возможным перекрытием. Для min перекрытие не влияет на ответ.

Какое применение Sparse Table для LCA?
?
LCA через Euler tour + RMQ: записываем Euler tour дерева с глубинами, LCA(u,v) = узел с минимальной глубиной между первым появлением u и v. Sparse Table даёт O(1) RMQ, итого LCA за O(1) с O(n log n) preprocessing.

Когда выбрать Sparse Table, а когда Segment Tree?
?
Sparse Table: статические данные, много queries, нужен O(1). Segment Tree: динамические данные, updates нужны, не-идемпотентные операции (sum). Fenwick Tree: prefix sums + point updates, проще Segment Tree.
...
| Следующий шаг | [[data-structures/persistent-structures]] | Персистентные версии структур данных |

| Углубиться | [[data-structures/segment-tree]] | Динамическая альтернатива для range queries |

| Смежная тема | [[algorithms/divide-and-conquer]] | Теоретическая основа Sparse Table |
