...
## Теоретические основы: формальный базис стеков и очередей

### Абстрактные типы данных (ADT)

> **Определение:** Stack и Queue — **абстрактные типы данных** (ADT), определяемые только через операции и их свойства, без указания реализации.

**Stack ADT (LIFO — Last In, First Out):**
- push(x) — добавить элемент на вершину
- pop() → x — удалить и вернуть верхний элемент
- peek() → x — вернуть верхний без удаления
- **Аксиома:** pop(push(S, x)) = (S, x) — последний положенный = первый взятый

**Queue ADT (FIFO — First In, First Out):**
- enqueue(x) — добавить в конец
- dequeue() → x — удалить и вернуть из начала
- **Аксиома:** dequeue(enqueue(Q, x₁, x₂, ..., xₙ)) = (Q', x₁) — первый пришёл = первый ушёл

### Историческая атрибуция

- **Stack:** Формализован Фридрихом Бауэром и Клаусом Заммельсоном (1957, Германия). Позднее Alan Turing использовал "burial" и "unburial" (запись и извлечение) в своих ранних работах. Эдсгер Дейкстра применил stack для разбора арифметических выражений (**Shunting-yard algorithm**, 1961)
- **Queue:** Формализована в контексте **теории массового обслуживания** (Agner Krarup Erlang, 1909 — queueing theory для телефонных сетей)

### Deque: обобщение

> **Определение:** **Deque** (Double-Ended Queue) — ADT, позволяющий вставку и удаление с обоих концов. Deque обобщает и Stack, и Queue.

### Связи

- [[linked-lists]] — реализация stack/queue через linked list
- [[graph-algorithms]] — Queue для BFS, Stack для DFS
- [[recursion-fundamentals]] — call stack = аппаратный stack
...
> [!question]- Почему стек идеально подходит для проверки сбалансированности скобок, а очередь -- нет?
> Скобки должны закрываться в обратном порядке открытия (LIFO). Стек естественно реализует это: последняя открывающая скобка -- первая для проверки. Очередь (FIFO) проверяла бы первую открывающую, что нарушает порядок.

> [!question]- У тебя есть только две очереди. Как реализовать стек?
> Push: добавляем элемент в queue2, затем переливаем все элементы из queue1 в queue2, меняем queue1 и queue2 местами. Pop: просто dequeue из queue1. Push стоит O(n), Pop -- O(1). Альтернативно: Push O(1), Pop O(n).

> [!question]- Что такое монотонный стек и какие задачи он решает?
> Стек, в котором элементы строго возрастают или убывают. Решает задачи вида Next Greater Element, Maximum Rectangle in Histogram за O(n). При добавлении элемента удаляем с вершины все нарушающие монотонность.

> [!question]- Почему deque (double-ended queue) универсальнее стека и очереди?
> Deque поддерживает добавление и удаление с обоих концов за O(1). Может работать и как стек (LIFO с одного конца), и как очередь (FIFO с двух концов). Используется в sliding window maximum и BFS с приоритетами.
...
?
Абстрактный тип данных LIFO: последний добавленный элемент извлекается первым. Операции: push O(1), pop O(1), peek O(1). Применение: скобки, DFS, undo, call stack.

Что такое очередь (Queue)?
?
Абстрактный тип данных FIFO: первый добавленный элемент извлекается первым. Операции: enqueue O(1), dequeue O(1). Применение: BFS, буферизация, планировщик задач.

Как реализовать очередь на двух стеках?
?
Стек input для enqueue, стек output для dequeue. При dequeue: если output пуст, переливаем все из input в output (порядок инвертируется). Амортизированно O(1) на операцию.

Что такое circular queue?
?
Очередь на массиве фиксированного размера с wrap-around. Указатели head и tail двигаются по кругу: next = (current + 1) % size. Эффективнее обычной очереди на массиве, где нужен сдвиг элементов.

Какие реальные применения стека?
?
Call stack программы (вызовы функций), undo/redo в редакторах, парсинг выражений, DFS обход графов, обратная польская нотация, backtracking алгоритмы.

Какие реальные применения очереди?
?
BFS обход графов, буферизация I/O, планировщик процессов ОС, print queue, message queue (Kafka, RabbitMQ), кэш LRU (с doubly linked list).
...
| Следующий шаг | [[data-structures/hash-tables]] | Следующая ключевая структура для интервью |

| Углубиться | [[patterns/monotonic-stack-pattern]] | Продвинутый паттерн на основе стека |

| Смежная тема | [[data-structures/trees-binary]] | Деревья используют стек (DFS) и очередь (BFS) |
