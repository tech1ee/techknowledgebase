...
> **Trie (префиксное дерево)** — корневое дерево, в котором каждое ребро помечено символом алфавита, а путь от корня до узла образует префикс хранимых строк. Название происходит от слова "re**trie**val" (Fredkin, 1960).

### Формальное определение

Trie для множества строк S над алфавитом Σ — это корневое дерево T, где:

- Корень — пустая строка ε
- Каждое ребро помечено символом c ∈ Σ
- Путь от корня до узла v кодирует строку-префикс
- Узел помечен как "терминальный", если соответствующая строка ∈ S

| Операция | Сложность | Пояснение |
|----------|-----------|-----------|
| Insert(word) | O(m) | m = длина слова, проход по символам |
| Search(word) | O(m) | Спуск по рёбрам, проверка терминальности |
| StartsWith(prefix) | O(p) | p = длина префикса |
| Delete(word) | O(m) | Очистка терминальной метки + pruning |
| Autocomplete(prefix) | O(p + k) | p = длина префикса, k = число результатов |

### Варианты реализации и сжатия

| Вариант | Память | Скорость | Когда использовать |
|---------|--------|----------|-------------------|
| Array-based (children[26]) | O(Σ * N * L) | Максимальная | Фиксированный алфавит, критична скорость |
| HashMap-based | O(N * L) в среднем | Средняя | Большой/переменный алфавит |
| **Patricia/Radix Tree** | O(N * L) сжатая | Средняя | Длинные общие префиксы (IP routing) |
| **DAWG** (directed acyclic word graph) | Минимальная | Средняя | Sharing суффиксов (словари) |

### Таксономия применений

- **Поиск и автодополнение:** поисковые системы, IDE, T9
- **Проверка орфографии:** словари с edit distance
- **Маршрутизация:** IP longest prefix match (Patricia/Radix в Linux kernel)
- **Сжатие данных:** Huffman coding (бинарный Trie)
- **Биоинформатика:** поиск паттернов в ДНК-последовательностях

### Историческая справка

| Год | Событие | Значение |
|-----|---------|----------|
| 1959 | de la Briandais R., "File Searching Using Variable Length Keys" | Первое описание Trie-подобной структуры |
| 1960 | Fredkin E., "Trie Memory" | Введение термина "Trie" (от retrieval) |
| 1968 | Morrison D., PATRICIA tree | Compressed Trie для IP-маршрутизации |
| 1988 | Blumer et al., DAWG | Минимальное представление множества строк |

**Связанные темы:** [[hash-tables]] (альтернатива для exact match), [[string-algorithms]] (строковые алгоритмы), [[graphs]] (Trie как частный случай дерева/графа)
...
### Теоретические основы
- **Fredkin, E. (1960). "Trie Memory."** Communications of the ACM. — Оригинальная статья, вводящая термин "Trie" (от retrieval) и описывающая префиксное дерево как структуру для эффективного поиска строк.
- **de la Briandais, R. (1959). "File Searching Using Variable Length Keys."** Proc. Western Joint Computer Conference. — Первое описание Trie-подобной структуры, предшествовавшее работе Фредкина.
- **Morrison, D. (1968). "PATRICIA — Practical Algorithm To Retrieve Information Coded in Alphanumeric."** JACM. — Compressed Trie (Patricia Tree), применяемый в IP-маршрутизации и ядре Linux.
- **Cormen, T. et al. (2009). "Introduction to Algorithms (CLRS)."** — Строковые структуры данных и алгоритмы, теоретическая база для Trie.

### Практические руководства
- [LeetCode 208 — Implement Trie](https://leetcode.com/problems/implement-trie-prefix-tree/) — базовая задача на реализацию
- [Tech Interview Handbook — Trie](https://www.techinterviewhandbook.org/algorithms/trie/) — cheatsheet для интервью
- [GeeksforGeeks — Trie](https://www.geeksforgeeks.org/dsa/trie-insert-and-search/) — tutorial с реализацией
- [Baeldung — Hash Table vs Trie](https://www.baeldung.com/cs/hash-table-vs-trie-prefix-tree) — сравнение подходов
- [Kodeco — Tries in Kotlin](https://www.kodeco.com/books/data-structures-algorithms-in-kotlin/v1.0/chapters/10-tries) — реализация на Kotlin
- [Codecademy — Complete Guide to Prefix Trees](https://www.codecademy.com/article/trie-data-structure-complete-guide-to-prefix-trees) — полное руководство
- [AlgoCademy — Trie in Coding Interviews](https://algocademy.com/blog/how-often-does-the-trie-data-structure-appear-in-coding-interviews/) — частота Trie на интервью
...
> [!question]- Почему Trie эффективнее HashMap для autocomplete, хотя оба дают быстрый поиск?
> Trie позволяет найти ВСЕ слова с данным префиксом за O(prefix + results) одним обходом поддерева. HashMap потребовал бы перебора всех ключей O(n) для каждого запроса, потому что не знает о префиксных связях между ключами.

> [!question]- Тебе нужно реализовать спеллчекер. Какое преимущество Trie даёт перед HashSet для проверки слов?
> Trie позволяет не только проверить существование слова за O(m), но и найти похожие слова (с одной опечаткой) через обход соседних ветвей. HashSet проверяет точное совпадение, но для fuzzy search нужно генерировать все возможные варианты и проверять каждый.

> [!question]- Какова пространственная сложность Trie и как её оптимизировать?
> В worst case O(ALPHABET * N * L), где N -- число слов, L -- средняя длина. Каждый узел хранит массив из ALPHABET указателей. Оптимизации: HashMap вместо массива (экономит на редких символах), compressed Trie (сжатие цепочек одиночных детей), DAWG (sharing суффиксов).
...
?
Древовидная структура для хранения строк, где каждый узел представляет один символ. Путь от корня до узла образует префикс. Позволяет O(m) поиск, вставку и prefix matching, где m -- длина строки.

Какие операции поддерживает Trie?
?
Insert(word): O(m). Search(word): O(m). StartsWith(prefix): O(m). Delete(word): O(m). Autocomplete: O(prefix + results). Все операции зависят от длины строки, не от количества слов.

Где применяется Trie?
?
Autocomplete (поисковые системы), спеллчекер, IP routing (longest prefix match), T9 предиктивный ввод, word games (Boggle), DNA sequence matching, prefix-based filtering.

Что такое compressed Trie (Patricia/Radix)?
?
Trie, где цепочки узлов с одним ребёнком сжимаются в один узел с подстрокой на ребре. Экономит память и ускоряет обход. Используется в Linux kernel (radix tree), routing tables.

Чем Trie лучше и хуже HashMap для хранения строк?
?
Лучше: prefix search, autocomplete, sorted iteration, no hash collisions. Хуже: больше памяти (ALPHABET указателей на узел), медленнее для exact match (cache misses при обходе узлов). Для exact match HashMap обычно быстрее.
...
| Следующий шаг | [[algorithms/string-algorithms]] | Алгоритмы обработки строк |

| Углубиться | [[data-structures/segment-tree]] | Другая древовидная структура для range queries |

| Смежная тема | [[data-structures/hash-tables]] | Сравнение Trie vs HashMap для строковых задач |
