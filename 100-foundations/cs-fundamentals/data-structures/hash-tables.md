...
## Теоретические основы: формальный базис хеш-таблиц

### Хеширование: формальное определение

> **Определение:** Хеш-функция h: U → {0, 1, ..., m-1} отображает ключи из вселенной U в индексы таблицы размера m. Идеальная хеш-функция — инъективна (без коллизий), но для |U| >> m это невозможно (**pigeonhole principle**).

### Birthday Paradox и коллизии

> **Парадокс дней рождения:** В группе из 23 человек вероятность совпадения дней рождения > 50%. Аналогично: при вставке √m элементов в таблицу размера m вероятность коллизии > 50%.

Следствие: коллизии **неизбежны** и нужна стратегия их разрешения.

### Стратегии разрешения коллизий

| Метод | Идея | Worst case | Avg case | Cache |
|-------|------|-----------|----------|-------|
| **Chaining** (списки) | Каждый slot → связный список | O(n) | O(1+α) | Плохой |
| **Open Addressing** (linear probing) | Ищем следующий свободный slot | O(n) | O(1/(1-α)) | Хороший |
| **Cuckoo Hashing** (Pagh & Rodler, 2001) | 2 хеш-функции, "выселение" | O(1) worst! | O(1) | Хороший |
| **Robin Hood Hashing** | Перемещаем "богатых" (близких) ради "бедных" (далёких) | O(log n) | O(1) | Хороший |

α = n/m — **load factor**. При α > 0.75 производительность деградирует → **rehashing** (удвоение таблицы).

### Universal Hashing (Carter & Wegman, 1979)

> **Определение:** Семейство хеш-функций H является **универсальным**, если для любых двух различных ключей x ≠ y: Pr[h(x) = h(y)] ≤ 1/m, где h выбрана случайно из H.

Это гарантирует, что **ни один набор данных** не вызовет worst-case поведение (защита от adversarial input).

### Perfect Hashing (Fredman, Komlós, Szemerédi, 1984)

> **Теорема (FKS, 1984):** Для статического множества из n ключей можно построить хеш-функцию с **O(1) worst-case** lookup и O(n) памяти.

Идея: двухуровневое хеширование. Первый уровень → bucket. Второй уровень → perfect hash внутри bucket.

### Связи

- [[arrays-strings]] — массив как основа hash table
- [[caching-strategies]] — hash table как основа кэша
- [[sorting-algorithms]] — counting sort использует хеширование по значению
...
> [!question]- Почему HashMap имеет O(1) среднюю, но O(n) худшую сложность поиска?
> В среднем хеш-функция распределяет ключи равномерно по бакетам, давая O(1). В худшем случае все ключи попадают в один бакет (коллизии), образуя цепочку длиной n. Java 8+ смягчает это, переключая длинные цепочки на Red-Black Tree (O(log n) вместо O(n)).

> [!question]- Тебе нужно подсчитать частоту каждого символа в строке длиной 10 миллионов. HashMap или массив из 128 элементов -- что выбрать?
> Массив из 128 элементов (ASCII): O(1) доступ по индексу символа, никакого overhead на хеширование, идеальная cache locality. HashMap имеет overhead на boxing, хеширование, коллизии. Для фиксированного небольшого алфавита массив всегда эффективнее.

> [!question]- Почему load factor 0.75 считается хорошим компромиссом?
> При load factor < 0.75 вероятность коллизий достаточно низка для O(1) операций. Выше 0.75 -- резкий рост коллизий. Ниже 0.5 -- слишком много пустых бакетов (расход памяти). 0.75 балансирует скорость и память.

> [!question]- В чём разница между open addressing и separate chaining для разрешения коллизий?
> Separate chaining: каждый бакет -- список коллизий, проще реализовать, работает при high load factor. Open addressing: ищем свободный слот в самом массиве (linear probing, quadratic), лучше cache locality, но деградирует при load factor > 0.7.
...
?
Преобразует ключ произвольного размера в целое число (хеш-код), которое используется как индекс в массиве бакетов. Хорошая хеш-функция равномерно распределяет ключи, минимизируя коллизии.

Что такое load factor?
?
Отношение количества элементов к числу бакетов: n/capacity. При достижении порога (обычно 0.75) таблица расширяется вдвое и все элементы перехешируются (rehash). Это амортизированная O(1) операция.

Какие два основных способа разрешения коллизий?
?
Separate Chaining: список элементов в каждом бакете. Open Addressing: поиск свободного слота (linear probing, quadratic probing, double hashing). Java использует chaining с переходом на Red-Black Tree.

Почему ключи HashMap должны быть иммутабельными?
?
Если ключ изменится после вставки, его хеш-код изменится, и элемент окажется в неправильном бакете. Поиск по старому хешу вернёт null. Поэтому String, Integer -- хорошие ключи, а mutable объекты -- нет.

Какие задачи чаще всего решаются через HashMap?
?
Two Sum (complement lookup), подсчёт частот, группировка анаграмм, LRU Cache, обнаружение дубликатов, O(1) lookup по ключу. HashMap -- самая частая структура на интервью.

Чем HashSet отличается от HashMap?
?
HashSet хранит только ключи (без значений). Внутри реализован через HashMap<K, dummy>. Используется для проверки уникальности и membership test за O(1). Операции: add, remove, contains.
...
| Следующий шаг | [[data-structures/trees-binary]] | Деревья как альтернатива с упорядоченностью |

| Углубиться | [[patterns/two-pointers-pattern]] | HashMap + Two Pointers -- частая комбинация |

| Смежная тема | [[data-structures/tries]] | Специализированная хеш-структура для строк |
