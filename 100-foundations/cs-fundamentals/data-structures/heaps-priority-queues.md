...
### Аналогия 1: Больничная сортировка (triage)

```
ПРИЁМНЫЙ ПОКОЙ БОЛЬНИЦЫ:

Пациенты обслуживаются НЕ по порядку прихода,
а по срочности состояния — это и есть приоритетная очередь.

Обычная очередь (FIFO):
  Пришёл первым → обслужен первым
  [Насморк] → [Перелом] → [Инфаркт]
  Инфаркт ждёт 2 часа? Недопустимо!

Приоритетная очередь (Heap):
  Срочность определяет порядок
  [Инфаркт (10)] → [Перелом (5)] → [Насморк (1)]

  Приходит новый пациент с ожогом (7)?
  → offer(7) — вставляем в кучу
  → Он встаёт МЕЖДУ инфарктом и переломом

  Врач освободился?
  → poll() — извлекаем самого срочного
  → O(log n), даже если в очереди 1000 пациентов

HEAP = система triage в коде.
```

### Аналогия 2: Очередь на посадку в аэропорту

```
ПОСАДКА В САМОЛЁТ:

Не все пассажиры равны — есть приоритеты:

Группа 1: Бизнес-класс, инвалиды     (приоритет 1)
Группа 2: Семьи с маленькими детьми   (приоритет 2)
Группа 3: Часто летающие (Gold/Silver) (приоритет 3)
Группа 4: Все остальные               (приоритет 4)

Это MIN-HEAP по номеру группы:

            [1]  ← Бизнес-класс идёт первым
           /   \
         [2]   [3]
         /
       [4]

Новый пассажир? offer() — встаёт на своё место.
Открыли посадку? poll() — группа 1 идёт первой.

ВАЖНО: Внутри одной группы — обычная очередь (FIFO).
Но группа 1 ВСЕГДА раньше группы 4,
даже если группа 4 пришла на час раньше.

Heap не гарантирует порядок ВНУТРИ уровня —
только между parent и children.
```
...
**[[sorting-algorithms]]** — Куча и сортировка связаны через Heap Sort — алгоритм сортировки, который строит max-heap из массива за O(n), а затем последовательно извлекает максимум, формируя отсортированный массив. Heap Sort гарантирует O(n log n) в худшем случае (в отличие от Quick Sort) и работает in-place (в отличие от Merge Sort). Однако на практике Heap Sort проигрывает Quick Sort из-за худшей cache locality — heap «прыгает» по массиву (parent -> child), тогда как Quick Sort работает с последовательными участками памяти.

**[[graph-algorithms]]** — Куча — ключевая структура данных для эффективных алгоритмов на графах. Алгоритм Dijkstra использует min-heap для выбора ближайшей нерассмотренной вершины: без кучи сложность O(V^2), с бинарной кучей — O((V+E) log V), а с кучей Фибоначчи — O(E + V log V). Алгоритм Prim для построения минимального остовного дерева (MST) использует аналогичный подход. В обоих случаях операция decrease-key критична — именно поэтому для плотных графов иногда используют Fibonacci Heap вместо бинарной кучи.

**[[greedy-algorithms]]** — Жадные алгоритмы часто используют приоритетную очередь для выбора «лучшего» элемента на каждом шаге. Классический пример — алгоритм Хаффмана для построения оптимального префиксного кода: на каждом шаге из min-heap извлекаются два узла с наименьшей частотой и объединяются. Activity Selection, Fractional Knapsack, задачи на scheduling — все эти greedy-задачи естественно решаются через heap. Паттерн «выбери лучшее из доступного» = poll() из приоритетной очереди.
...
> [!question]- Почему buildHeap работает за O(n), а не за O(n log n), хотя каждый sift-down стоит O(log n)?
> Не все узлы находятся на максимальной глубине. Половина узлов -- листья (0 работы), четверть -- глубина 1, и т.д. Суммарно: n/2*0 + n/4*1 + n/8*2 + ... = O(n). Это математически доказывается через сумму геометрического ряда.

> [!question]- У тебя поток из 10 миллионов чисел, нужно в любой момент знать медиану. Какую структуру данных выберешь?
> Two Heaps: max-heap для нижней половины, min-heap для верхней. Медиана -- вершина max-heap (или среднее двух вершин). Вставка O(log n), получение медианы O(1). Это оптимальнее сортировки или balanced BST для этой конкретной задачи.

> [!question]- Почему heap реализуют на массиве, а не на узлах с указателями?
> Complete binary tree идеально ложится на массив: дети узла i находятся по индексам 2i+1 и 2i+2, родитель -- (i-1)/2. Нет overhead на указатели, отличная cache locality. Это делает heap на массиве в разы быстрее на практике.

> [!question]- В чём разница между heap и BST, если оба -- бинарные деревья?
> BST упорядочен горизонтально (left < root < right), позволяя O(log n) поиск любого элемента. Heap упорядочен вертикально (parent <= children для min-heap), гарантируя только O(1) доступ к min/max. Heap не поддерживает эффективный поиск произвольного элемента.
...
Что такое heap (куча)?
?
Complete binary tree со свойством: каждый родитель <= (min-heap) или >= (max-heap) своих детей. Гарантирует O(1) доступ к min/max, O(log n) insert и extract. Реализуется на массиве.

Как работают sift-up и sift-down?
?
Sift-up: после вставки элемент поднимается вверх, меняясь с родителем, пока свойство кучи не восстановлено. Sift-down: после извлечения корня последний элемент ставится на место корня и опускается вниз. Оба O(log n).

Что такое Priority Queue?
?
Абстрактный тип данных: элементы извлекаются в порядке приоритета, а не порядке вставки. Обычно реализуется через heap. Используется в Dijkstra, Huffman coding, планировщиках задач, merge k sorted lists.

Как решать задачи Top-K через heap?
?
Для K наибольших: min-heap размера K. Для K наименьших: max-heap размера K. При добавлении нового элемента сравниваем с вершиной, при необходимости заменяем. O(n log K) вместо O(n log n) сортировки.

Что такое паттерн Two Heaps?
?
Max-heap для нижней половины данных, min-heap для верхней. Позволяет находить медиану за O(1), вставлять за O(log n). Используется для задач sliding window median и stream median.

Какова сложность основных операций heap?
?
Insert: O(log n). Extract min/max: O(log n). Peek min/max: O(1). Build heap: O(n). Поиск произвольного элемента: O(n). Heap sort: O(n log n).
...
| Следующий шаг | [[data-structures/tries]] | Следующая структура для интервью |

| Углубиться | [[patterns/top-k-elements-pattern]] | Ключевой паттерн с использованием heap |

| Смежная тема | [[algorithms/shortest-paths]] | Dijkstra использует priority queue |
