...
## Теоретические основы: формальный базис сбалансированных деревьев

### Зачем нужна балансировка: вырождение BST

> **Теорема:** Несбалансированное BST в худшем случае вырождается в связный список с высотой O(n). Среднее число сравнений при поиске: O(n/2) вместо O(log n).

Цель self-balancing BST: гарантировать h = O(log n), т.е. все операции (поиск, вставка, удаление) за O(log n) в **худшем** случае.

### AVL-дерево: Адельсон-Вельский и Ландис (1962)

> **Определение (Adelson-Velsky & Landis, 1962):** AVL-дерево — BST, в котором для каждого узла v: |height(left(v)) - height(right(v))| ≤ 1.

**Теорема о высоте AVL:**
Высота AVL-дерева с n узлами: h ≤ 1.4404 · log₂(n+2) - 0.328

**Доказательство:** Пусть N(h) — минимальное число узлов в AVL высоты h.
- N(0) = 1, N(1) = 2
- N(h) = N(h-1) + N(h-2) + 1 (рекуррентность, аналогичная числам Фибоначчи)
- N(h) = F(h+2) - 1, где F — числа Фибоначчи
- По формуле Бине: N(h) ≈ φʰ⁺² / √5, где φ = (1+√5)/2 ≈ 1.618
- Следовательно: h ≤ log_φ(n) ≈ 1.44 · log₂(n)

AVL — **первое в истории самобалансирующееся дерево** поиска.

### Red-Black дерево: Guibas & Sedgewick (1978)

> **Определение (Guibas & Sedgewick, 1978):** Red-Black дерево — BST с 5 инвариантами:
> 1. Каждый узел красный или чёрный
> 2. Корень чёрный
> 3. Все NIL-листья чёрные
> 4. Красный узел имеет только чёрных детей
> 5. Все пути от узла к NIL-потомкам содержат одинаковое число чёрных узлов (**black-height**)

**Теорема о высоте Red-Black:** h ≤ 2 · log₂(n+1)

**Доказательство:** Свойства 4+5 гарантируют, что самый длинный путь не более чем вдвое длиннее самого короткого (чередование красных и чёрных). Если black-height = bh, то поддерево содержит ≥ 2^bh - 1 узлов → n ≥ 2^(h/2) - 1 → h ≤ 2·log₂(n+1).

### Изоморфизм RB ↔ 2-3-4 деревья

Ключевой инсайт: Red-Black дерево — **бинарное представление** 2-3-4 дерева (B-tree порядка 4):
- Чёрный узел = 2-node
- Чёрный узел + 1 красный ребёнок = 3-node
- Чёрный узел + 2 красных ребёнка = 4-node

Это объясняет, почему RB-дерево "работает" — инварианты RB эквивалентны инвариантам B-tree. Sedgewick использовал это для создания **Left-Leaning Red-Black Trees** (2008) — упрощённой реализации.

### Сравнительная таблица

| Свойство | AVL | Red-Black | B-Tree |
|----------|-----|-----------|--------|
| **Высота** | ≤ 1.44 log n | ≤ 2 log n | ≤ log_t n |
| **Строгость баланса** | Высокая | Умеренная | Зависит от порядка |
| **Ротации при insert** | O(log n) | ≤ 2 | O(1) amortized |
| **Ротации при delete** | O(log n) | ≤ 3 | O(1) amortized |
| **Оптимально для** | Read-heavy | Write-heavy | Disk I/O |
| **Использование** | Базы данных (in-memory) | std::map, TreeMap, Linux | Файловые системы, БД |

### Связи

- [[trees-binary]] — основы BST
- [[database-design-optimization]] — B-Tree в индексах СУБД
- [[sorting-algorithms]] — HeapSort использует heap (не BST)
...
- **Adelson-Velsky, G. & Landis, E. (1962). "An algorithm for the organization of information." Soviet Mathematics Doklady.** — Оригинальная статья AVL-деревьев. Первое самобалансирующееся дерево в истории CS. Доказательство высоты ≤ 1.44 log n
- **Guibas, L.J. & Sedgewick, R. (1978). "A Dichromatic Framework for Balanced Trees." FOCS.** — Изобретение Red-Black деревьев. Доказательство связи с 2-3-4 деревьями. Унифицированный подход к балансировке
- **Bayer, R. (1972). "Symmetric Binary B-Trees: Data Structure and Maintenance Algorithms."** — Предшественник Red-Black деревьев (symmetric binary B-trees ≡ RB-trees)

### Практические руководства

- **Cormen, T. et al. (2009). CLRS, Chapter 13: Red-Black Trees.** — Каноническое изложение с полными доказательствами
- **Sedgewick, R. (2008). Left-Leaning Red-Black Trees.** — Упрощённая реализация
- **Sedgewick, R. & Wayne, K. (2011). Algorithms, 4th Edition.** — Связь 2-3 и RB деревьев с визуализациями
