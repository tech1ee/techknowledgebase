...
> [!question]- Почему Segment Tree использует O(4n) памяти, а не O(2n)?
> При хранении в массиве не все поддеревья имеют одинаковую глубину. Если n не степень двойки, некоторые листья на последнем уровне оставляют пустые слоты. Множитель 4 гарантирует достаточно места для любого n.

> [!question]- Тебе нужно отвечать на range sum queries и делать range updates на массиве из 200000 элементов. Почему без lazy propagation не обойтись?
> Без lazy propagation range update стоит O(n) -- обновляем каждый элемент отдельно. С lazy propagation range update O(log n): откладываем обновление до момента запроса. Для 200000 элементов и множества запросов lazy propagation необходим.

> [!question]- Почему Segment Tree работает для любой ассоциативной операции?
> Структура зависит только от Divide and Conquer: результат на [l,r] = merge(результат на [l,mid], результат на [mid+1,r]). Это работает для sum, min, max, gcd, xor. Нужна только функция merge и нейтральный элемент.
...
?
Бинарное дерево для range queries и point/range updates. Каждый узел хранит агрегат для отрезка массива. Build O(n), query O(log n), update O(log n). Память O(4n).

Что такое lazy propagation?
?
Техника отложенного обновления: при range update помечаем узел, но не обновляем детей. Push down происходит только при запросе. Позволяет range update за O(log n) вместо O(n).

Какие три случая при query(l, r)?
?
1) Отрезок узла не пересекается с [l,r] -- нейтральный элемент. 2) Полностью внутри [l,r] -- значение узла. 3) Частичное пересечение -- merge ответов детей.

Когда Segment Tree, а когда Fenwick?
?
Fenwick: проще, меньше памяти, только prefix queries + point updates. Segment Tree: универсальнее (range update, lazy, persistent), но сложнее.

Какие операции поддерживает Segment Tree?
?
Point update, range query, range update (lazy). Любая ассоциативная операция: sum, min, max, gcd, count. Можно хранить несколько агрегатов в одном узле.
...
| Следующий шаг | [[data-structures/fenwick-tree]] | Более простая альтернатива для prefix queries |

| Углубиться | [[data-structures/persistent-structures]] | Персистентный Segment Tree |

| Смежная тема | [[data-structures/sparse-table]] | O(1) queries для статических данных |
