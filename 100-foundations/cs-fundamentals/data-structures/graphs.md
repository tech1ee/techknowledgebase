...
> [!question]- Почему BFS находит кратчайший путь в невзвешенном графе, а DFS -- нет?
> BFS обходит вершины по уровням (расстоянию от старта): сначала все на расстоянии 1, потом 2 и т.д. Первое посещение вершины гарантирует кратчайший путь. DFS идёт вглубь, может найти длинный путь раньше короткого, без гарантий оптимальности.

> [!question]- Тебе дан граф из 100000 вершин и 200000 рёбер. Adjacency list или adjacency matrix -- что выбрать?
> Adjacency list: O(V+E) = 300000 памяти. Adjacency matrix: O(V^2) = 10^10 памяти -- не влезет. Для разреженных графов (E << V^2) всегда выбираем adjacency list. Matrix оправдана только для плотных графов или когда нужна O(1) проверка ребра.

> [!question]- Как обнаружить цикл в направленном графе с помощью DFS?
> Используем три цвета: белый (не посещён), серый (в обработке), чёрный (завершён). Если DFS встречает серую вершину -- найден цикл (back edge). Серая вершина означает, что мы ещё в процессе обхода её поддерева, и возврат к ней образует цикл.

> [!question]- В каких реальных системах графы используются как основная модель данных?
> Социальные сети (дружба, подписки), навигация (карты, GPS), компиляторы (граф зависимостей), сети (маршрутизация пакетов), рекомендательные системы, базы данных (Neo4j), package managers (npm, gradle).

---

## Ключевые карточки

Какие два основных способа представления графа?
?
Adjacency List: массив списков соседей. O(V+E) память, эффективен для разреженных графов. Adjacency Matrix: матрица V*V. O(V^2) память, O(1) проверка ребра, для плотных графов.

Чем DFS отличается от BFS?
?
DFS (стек/рекурсия): идёт вглубь, находит циклы, топологическую сортировку, компоненты связности. BFS (очередь): обходит по уровням, находит кратчайший путь в невзвешенном графе. Оба O(V+E).

Что такое топологическая сортировка?
?
Линейный порядок вершин DAG (направленного ациклического графа): для каждого ребра u->v, u стоит перед v. Реализуется через DFS (обратный postorder) или алгоритм Кана (BFS с in-degree). Применение: порядок сборки, расписание.

Как найти компоненты связности?
?
Для неориентированного графа: запускаем DFS/BFS из каждой непосещённой вершины, каждый запуск -- новая компонента. Для ориентированного (SCC): алгоритм Косарайю (два DFS) или Тарьяна (один DFS).

Какие типы графов существуют?
?
Направленный/ненаправленный, взвешенный/невзвешенный, циклический/ациклический (DAG), связный/несвязный, полный, двудольный (bipartite). Каждый тип имеет свои алгоритмы и свойства.

Что такое back edge и зачем он нужен?
?
Back edge -- ребро от потомка к предку в DFS-дереве. Наличие back edge в направленном графе означает цикл. В ненаправленном графе любое ребро к посещённому узлу (кроме родителя) образует цикл.
...
| Следующий шаг | [[algorithms/graph-algorithms]] | Алгоритмы на графах: BFS, DFS, Dijkstra |

| Углубиться | [[algorithms/graph-advanced]] | Продвинутые графовые алгоритмы |

| Смежная тема | [[patterns/topological-sort-pattern]] | Паттерн топологической сортировки |
