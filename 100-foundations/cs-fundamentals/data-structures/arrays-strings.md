...
> **Массив (Array)** — абстрактный тип данных, представляющий конечную упорядоченную коллекцию элементов одного типа, хранящихся в непрерывной области памяти и доступных по целочисленному индексу за O(1).

### Модель непрерывной памяти

Массив занимает непрерывный блок памяти размером `n * sizeof(element)`. Адрес i-го элемента вычисляется арифметически: `base_address + i * sizeof(element)`. Это обеспечивает:

- **O(1) произвольный доступ** — адрес любого элемента вычисляется за константное время
- **Высокую cache locality** — последовательные обращения попадают в одну cache line (64 байта на x86)
- **Предсказуемость hardware prefetcher** — CPU загружает следующие элементы заранее

### Амортизированный анализ динамических массивов

Динамический массив (ArrayList, vector, list) решает проблему фиксированного размера через стратегию удвоения буфера. При переполнении выделяется новый буфер размером 2n, и все элементы копируются.

| Операция | Worst case | Амортизированно | Пояснение |
|----------|-----------|-----------------|-----------|
| Доступ по индексу | O(1) | O(1) | Адресная арифметика |
| Вставка в конец | O(n) | **O(1)** | Удвоение буфера: суммарная стоимость n вставок = O(n) |
| Вставка в начало | O(n) | O(n) | Сдвиг всех элементов |
| Удаление из конца | O(1) | O(1) | Уменьшение счётчика |
| Поиск | O(n) | O(n) | Линейный обход |

> **Строка (String)** — неизменяемая (в большинстве языков) последовательность символов, реализованная как массив символов с дополнительными операциями (конкатенация, поиск подстрок, сравнение).

### Хронология ключевых результатов

| Год | Результат | Значение |
|-----|-----------|----------|
| 1960-е | Массивы в Fortran/ALGOL | Первые реализации непрерывных массивов в языках программирования |
| 1970 | Knuth, "The Art of Computer Programming" | Формализация массива как фундаментальной структуры данных |
| 1977 | Knuth-Morris-Pratt алгоритм | O(n) поиск подстроки, основа строковых алгоритмов |
| 1987 | Rabin-Karp алгоритм | Вероятностный поиск подстроки через хеширование |
| 1993 | Amortized analysis (Tarjan) | Формальное обоснование O(1) амортизированной вставки |

**Связанные темы:** [[big-o-complexity]] (анализ сложности), [[two-pointers-pattern]] (основной паттерн для массивов), [[sliding-window-pattern]] (подмассивы и подстроки), [[hash-tables]] (ускорение поиска на основе массивов)
...
- **Knuth, D. (1997). "The Art of Computer Programming, Vol. 1: Fundamental Algorithms."** — формализация массива как фундаментальной структуры данных, анализ адресной арифметики и моделей памяти
...
- **Tarjan, R. (1985). "Amortized Computational Complexity."** — формальное обоснование амортизированного анализа, применяемого к динамическим массивам (стратегия удвоения буфера)
...
> [!question]- Почему доступ к элементу массива по индексу занимает O(1), а вставка в начало -- O(n)?
> Массив хранит элементы в непрерывной памяти. Зная адрес начала и размер элемента, адрес любого элемента вычисляется арифметически за O(1). Вставка в начало требует сдвига всех n элементов вправо, отсюда O(n).

> [!question]- У тебя строка длиной 100000 символов и нужно добавить символ в середину. Массив символов или связный список -- что выбрать и почему?
> Для единичной вставки массив потребует O(n) на сдвиг, но на практике он быстрее из-за cache locality. Связный список даёт O(1) вставку при известной позиции, но O(n) на поиск позиции. Для частых вставок в середину -- связный список, для единичных -- массив.

> [!question]- Почему конкатенация строк в цикле через += даёт O(n^2), а StringBuilder -- O(n)?
> Строки в большинстве языков иммутабельны. Каждое += создаёт новую строку, копируя предыдущую: 1+2+...+n = O(n^2). StringBuilder использует изменяемый буфер с амортизированным O(1) на append.

> [!question]- В чём преимущество cache locality массивов перед связными списками?
> Элементы массива расположены в непрерывной памяти, что позволяет CPU загружать целые cache lines. Узлы связного списка разбросаны по памяти, каждое обращение -- потенциальный cache miss. На современном железе это может давать разницу в 10-100 раз.

---

## Ключевые карточки

Какова сложность основных операций массива?
?
Access по индексу: O(1). Search: O(n). Insert в конец: амортизированно O(1). Insert в начало/середину: O(n). Delete: O(n).

Что такое contiguous memory и почему она важна?
?
Элементы массива расположены в непрерывной области памяти. Это обеспечивает O(1) доступ по индексу через адресную арифметику и эффективное использование CPU cache.

Почему строки иммутабельны в Java/Kotlin/Python?
?
Иммутабельность обеспечивает безопасность в многопоточных программах, позволяет кэшировать hash-код, и используется как ключ в HashMap. Для изменяемых строк используют StringBuilder.

Какие два паттерна чаще всего применяются с массивами?
?
Two Pointers (два указателя) для sorted arrays и пар, и Sliding Window (скользящее окно) для subarray/substring задач. Оба дают O(n) вместо O(n^2).

Чем отличается ArrayList от обычного массива?
?
ArrayList -- динамический массив, автоматически увеличивающий размер при переполнении. Вставка в конец амортизированно O(1) за счёт удвоения буфера. Обычный массив имеет фиксированный размер.

Что такое prefix sum массив?
?
Массив, где prefix[i] = сумма элементов от 0 до i-1. Позволяет вычислять сумму любого подотрезка за O(1): sum(l,r) = prefix[r+1] - prefix[l]. Построение за O(n).
...
| Следующий шаг | [[data-structures/linked-lists]] | Альтернативная структура хранения последовательностей |

| Углубиться | [[patterns/two-pointers-pattern]] | Главный паттерн работы с массивами |

| Смежная тема | [[data-structures/hash-tables]] | Быстрый поиск на основе массивов |
