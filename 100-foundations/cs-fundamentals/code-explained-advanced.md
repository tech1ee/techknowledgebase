...
> **Анализ продвинутого кода** — методология пошагового разбора алгоритмов повышенной сложности, где каждая конструкция объясняется через формальные свойства (инварианты, рекуррентные соотношения, амортизированный анализ).

### Методология разбора

Документ использует три уровня объяснения для каждого алгоритма:

| Уровень | Что объясняется | Цель |
|---------|----------------|------|
| **Интуиция** | Аналогия из реальной жизни, визуализация | Понять *зачем* работает |
| **Формальная модель** | Математическое определение, инвариант, рекуррентное соотношение | Понять *почему* работает |
| **Код с комментариями** | Каждая строка кода с пояснением | Понять *как* реализовать |

### Покрываемые темы

Документ охватывает три группы продвинутых алгоритмов: (1) **строковые** — хеширование, rolling hash, KMP/Z-function (см. [[string-algorithms-advanced]]), (2) **динамическое программирование** — knapsack, LCS, LIS с оптимизациями пространства (см. [[dp-patterns]]), (3) **оптимизации** — amortized analysis, bit tricks, space-time tradeoffs. Предварительное чтение: [[code-explained-from-zero]], [[arrays-strings]], [[recursion-fundamentals]].
...
> [!question]- Почему NTT предпочтительнее FFT в competitive programming?
> NTT работает в поле по модулю и даёт точные целочисленные результаты без ошибок округления. В CP ответы обычно требуются по модулю (часто 998244353), что делает NTT естественным выбором. FFT с комплексными числами может давать ошибки для больших коэффициентов.

> [!question]- Тебе дана задача: на дереве из 100000 вершин нужно отвечать на запросы суммы на пути. Какую технику ты выберешь и почему?
> Heavy-Light Decomposition (HLD) + Segment Tree. HLD разбивает дерево на цепи так, что любой путь пересекает O(log n) цепей. На каждой цепи запрос к Segment Tree за O(log n). Итого: O(log^2 n) на запрос, что достаточно быстро для n = 100000.

> [!question]- В чём ключевое отличие алгоритма Диница от Форда-Фалкерсона?
> Форд-Фалкерсон ищет любой увеличивающий путь (DFS), что может дать O(V*E^2). Диниц строит слоистую сеть через BFS и ищет блокирующие потоки, что гарантирует O(V^2 * E). Слоистая сеть ограничивает количество фаз числом вершин.

> [!question]- Почему Convex Hull Trick оптимизирует DP с переходами вида dp[i] = min(dp[j] + b[j]*a[i])?
> Такой переход эквивалентен запросу минимума набора линейных функций y = b[j]*x + dp[j] в точке x = a[i]. CHT поддерживает выпуклую оболочку этих линий, позволяя отвечать на запрос за O(1) амортизированно вместо перебора всех j.

---

## Ключевые карточки

Что делает хеширование строк?
?
Преобразует строку в число для быстрого сравнения за O(1) вместо O(n). hash(s) = s[0]*BASE^(n-1) + ... + s[n-1]*BASE^0. Возможны коллизии, которые нужно проверять.

Что такое Z-функция?
?
Z[i] = длина наибольшего общего префикса строки s и подстроки s[i..]. Вычисляется за O(n). Применяется для поиска паттерна, нахождения периода строки.

Что такое Bitmask DP?
?
DP, где состояние включает битовую маску подмножества элементов. Подходит для n <= 20 (2^20 состояний). Позволяет перебирать все подмножества эффективно.

Какова сложность алгоритма Диница?
?
O(V^2 * E) для общего графа. O(E * sqrt(V)) для unit capacity графов. Использует BFS для построения слоистой сети и DFS для нахождения блокирующих потоков.

Что такое Centroid Decomposition?
?
Разбиение дерева через последовательное удаление центроидов. Центроид — вершина, при удалении которой все поддеревья <= n/2. Глубина рекурсии O(log n), каждая вершина обрабатывается O(log n) раз.

Зачем нужно векторное произведение (cross product)?
?
Определяет ориентацию тройки точек: > 0 — против часовой, < 0 — по часовой, = 0 — коллинеарны. Основа для выпуклой оболочки, пересечения отрезков, площади многоугольника.
...
|-------------|------|-------|
| Следующий шаг | [[algorithms/string-algorithms]] | Теория строковых алгоритмов |
| Углубиться | [[algorithms/network-flow]] | Формальное изучение потоков в сетях |
| Смежная тема | [[algorithms/computational-geometry]] | Теория вычислительной геометрии |
...
### [[code-explained-from-zero]]
Базовый документ с разбором кода для начинающих является прямым предшественником данного материала. Без понимания основ — как работают циклы, условия, базовые структуры данных — невозможно перейти к продвинутым алгоритмам. Code Explained from Zero формирует навык чтения кода построчно с полным пониманием каждой операции, что критически важно при разборе сложных алгоритмов вроде KMP или Segment Tree. Рекомендуется пройти базовый документ полностью перед переходом к продвинутым темам.

### [[string-algorithms]]
Строковые алгоритмы составляют значительную часть данного документа — хеширование строк, Rabin-Karp, KMP разбираются с объяснением каждой строки кода. Понимание базовых строковых операций (сравнение, конкатенация, подстрока) необходимо для осознания того, зачем нужны продвинутые алгоритмы поиска подстрок. Связь двусторонняя: данный документ показывает реализацию, а String Algorithms — теоретическую основу и доказательства корректности.

### [[dynamic-programming]]
Продвинутое динамическое программирование занимает вторую крупную часть документа — разбираются задачи на DP с bitmask, DP на деревьях, оптимизации состояний. Знание основ DP (memoization, tabulation, определение состояний и переходов) является обязательным для понимания продвинутых примеров. Без теоретической базы из Dynamic Programming невозможно понять, почему выбрано конкретное состояние или почему переход формулируется именно так.
...
- Cormen, Leiserson, Rivest & Stein (2009). *Introduction to Algorithms (CLRS).* — каноническое изложение всех алгоритмов, разобранных в документе: строковое хеширование, KMP, DP; формальные доказательства корректности и анализ сложности

### Практические руководства

- Halim & Halim (2013). *Competitive Programming 3.* — практический подход к алгоритмам competitive programming с сотнями задач и разбором решений; покрывает все темы данного документа
- Laaksonen (2017). *Competitive Programmer's Handbook.* — компактное изложение алгоритмов для соревнований: строки, DP, графы; свободно доступна онлайн и идеально дополняет разбор кода
