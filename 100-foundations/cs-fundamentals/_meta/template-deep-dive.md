...
> **Instructional design** (проектирование обучения) — системный подход к созданию учебных материалов, основанный на теориях обучения, когнитивной психологии и эмпирических исследованиях эффективности.

**Теоретические основы шаблона:**

| Теория | Автор | Влияние на шаблон |
|--------|-------|-------------------|
| **Feynman Technique** | Richard Feynman | 4 принципа: объясни просто → найди пробелы → аналогии → от простого к сложному |
| **Cognitive Load Theory** | Sweller, 1988 | Порядок секций: сначала интуиция, потом формализация — снижение extraneous load |
| **Multimedia Learning** | Mayer, 2001 | ASCII-визуализации + текст: dual coding (Paivio 1971) |
| **Scaffolding** | Vygotsky, 1978 | 3 уровня объяснения: 5-летний → абстракция → формальное определение |
| **Desirable Difficulties** | Bjork, 1994 | Секция "Распространённые ошибки" — learning from errors |
| **Constructive Alignment** | Biggs, 1996 | Цели обучения → содержание → практика → оценка выровнены |

**Структура шаблона как модель ADDIE** (Branson et al. 1975):
- **A**nalysis → секция "Зачем это нужно?" (проблема)
- **D**esign → секция "Что это такое?" (концепция)
- **D**evelopment → секция "Код" (реализация)
- **I**mplementation → секция "Практика" (задачи)
- **E**valuation → секция "Проверь себя" + чеклист качества

Связанные материалы: [[learning-complex-things]], [[cognitive-load-theory]], [[deliberate-practice]], [[cs-fundamentals-overview]]
...
> [!question]- Почему в шаблоне секция «Код» стоит на 7-й позиции, а не на 1-й? Какой педагогический принцип это отражает?
> Это отражает принцип Feynman Technique и подход «от понимания к реализации». Код — это формализация уже понятой концепции, а не способ её изучения. Если начать с кода, студент запоминает синтаксис, но не понимает «почему». Сначала нужно: 1) понять проблему (секция «Зачем»), 2) объяснить простым языком (аналогии), 3) визуализировать механику (ASCII-диаграммы), 4) разобрать по шагам. Только после этого код становится естественным выражением уже сформированной ментальной модели, а не «магией».

> [!question]- Зачем в шаблоне требуется раздел «Распространённые ошибки», если уже есть «Как это работает»? Разве правильного объяснения недостаточно?
> Недостаточно, потому что знание «как правильно» не защищает от типичных заблуждений. Мозг строит ментальные модели по аналогии с уже известным, и эти модели часто ошибочны. Раздел ошибок явно показывает: «вот так думают многие — и вот почему это неправильно». Это использует принцип desirable difficulties: столкновение с ошибкой и её разбор укрепляет правильное понимание сильнее, чем просто чтение верного объяснения. Кроме того, на собеседованиях именно ошибки (off-by-one, неправильный base case, мутирование при итерации) — главная причина провалов.

> [!question]- Почему шаблон требует три уровня объяснения («для 5-летнего» → «абстракция» → «формальное определение»), а не сразу формальное определение?
> Потому что формальное определение работает только для тех, кто уже понимает концепцию. Для тех, кто учит впервые, абстрактные определения — это «шум». Три уровня реализуют scaffolding: аналогия создаёт интуитивную модель → абстракция связывает её с программированием → формальное определение фиксирует точный смысл. На каждом уровне мозг «закрепляет крюк», на который вешается следующий уровень. Это также учитывает разные стили обучения и позволяет опытным разработчикам сразу перейти к нужному уровню.

---

## Ключевые карточки

Из каких 12 обязательных секций состоит шаблон deep-dive (в фиксированном порядке)?
?
1) TL;DR, 2) Зачем это нужно (мотивация), 3) Что это такое (аналогии), 4) Терминология, 5) Как это работает (визуализация), 6) Сложность операций, 7) Код (реализация), 8) Распространённые ошибки, 9) Когда использовать, 10) Практические задачи, 11) Связанные темы, 12) Источники.

Какие четыре принципа Feynman Technique заложены в основу шаблона?
?
1) Объясни как 5-летнему — если не можешь, значит сам не понял. 2) Найди пробелы — где объяснение «магия», разбери глубже. 3) Используй аналогии — связь с реальным миром = понимание. 4) От простого к сложному — каждый шаг опирается на предыдущий.

Какой формат комментариев в коде требует шаблон и почему?
?
Комментарии должны объяснять «ПОЧЕМУ» (причину решения), а не «ЧТО» (описание действия). Например, вместо `// создаём массив` нужно `// Почему массив: O(1) доступ по индексу`. Это заставляет автора и читателя понимать обоснование каждого решения, а не просто следовать коду.

Каков workflow создания материала по шаблону (9 шагов)?
?
1) Запустить deep-researcher по теме → 2) Прочитать отчёт, выписать ключевые концепции → 3) Для каждой концепции придумать аналогию → 4) Создать ASCII-визуализации → 5) Написать пошаговые разборы без кода → 6) Написать код с комментариями → 7) Добавить ошибки, практику, связи → 8) Проверить по чеклисту → 9) Обновить MOC и связанные материалы.

На каких трёх языках программирования должен быть представлен код в каждом материале?
?
Kotlin, Java и Python. Логика во всех трёх реализациях одинаковая — меняется только синтаксис. Сначала пишется базовая версия (для понимания), затем оптимизированная (с объяснением каждой оптимизации).
...
|-------------|------|-------|
| План обучения | [[learning-path]] | Увидеть порядок изучения всех тем и зависимости между ними |
| Обзор области | [[cs-fundamentals-overview]] | MOC с навигацией по всем материалам CS Fundamentals |
| Пример материала | [[arrays-strings]] | Реальный deep-dive по шаблону — массивы и строки |
| Паттерны задач | [[patterns-overview]] | Каталог всех паттернов решения алгоритмических задач |
| Типичные ошибки | [[common-mistakes]] | Разбор частых ошибок на интервью — применение секции 8 шаблона |
