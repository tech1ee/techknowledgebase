...
> **Type Erasure** — трансформация компилятора, удаляющая информацию о параметрах типов (generics) из скомпилированного кода. Формально: `erase(C<T₁,...,Tₙ>) = C<Object,...,Object>` (с bridge methods для сохранения type safety).

> **Reification** — сохранение полной информации о типах (включая generic параметры) в runtime-представлении.

### Историческая атрибуция

| Событие | Автор/Система | Год | Решение |
|---------|--------------|-----|---------|
| **Java Generics (erasure)** | Bracha et al. | 2004 | Erasure для backward compatibility с Java 1.0-1.4 bytecode |
| **C# Generics (reified)** | Kennedy & Syme | 2001 | Reified generics: CLR хранит типы в runtime |
| **Scala (erasure)** | Odersky | 2003 | Erasure на JVM + ClassTag/TypeTag для частичного reification |
| **Kotlin reified** | JetBrains | 2016 | `inline fun <reified T>` — compile-time подстановка обходит erasure |

### Erasure vs Reification: теоретический trade-off

| Свойство | Erasure (JVM) | Reification (CLR) |
|----------|--------------|-------------------|
| **Backward compatibility** | Полная (старый bytecode работает) | Нет (новый runtime) |
| **Runtime type check** | `is List<*>` только (не `is List<Int>`) | `is List<int>` возможно |
| **Specialization** | Один класс для всех `T` | Отдельный код для value types |
| **Размер binary** | Компактный | Больше (specialization) |
| **Reflection** | `List<Int>` → `List` (тип потерян) | `List<int>` → `List<int>` |

> **Теорема (Viroli & Natali, 2000):** Полная reification и backward binary compatibility — взаимоисключающие свойства для языка с generic extension. Java выбрала compatibility, C# — reification (сломав совместимость .NET 1.x → 2.0).

### Kotlin reified: механизм

`reified` в Kotlin — не настоящая reification, а **compile-time инлайнинг**:

```
// Компилятор видит:
inline fun <reified T> isType(x: Any): Boolean = x is T

// При вызове isType<String>("hello") подставляет:
fun isType_String(x: Any): Boolean = x is String  // конкретный тип!
```

Ограничение: работает только с `inline` функциями (тело встраивается в call site, где тип известен).
...
### Теоретические основы
- Bracha, G. et al. (1998). "Making the Future Safe for the Past: Adding Genericity to the Java Programming Language" — OOPSLA; design rationale для erasure
- Kennedy, A. & Syme, D. (2001). "Design and Implementation of Generics for the .NET CLR" — PLDI; reification approach
- Viroli, M. & Natali, A. (2000). "Parametric Polymorphism in Java: an Approach to Translation Based on Reflective Features" — OOPSLA; analysis of trade-offs
- Pierce, B. (2002). *TAPL* — erasure semantics и parametricity

### Практические руководства
- Bloch, J. (2018). *Effective Java*, 3rd ed. — Items 26-33: последствия type erasure
- [Android Developers: Reification of the Erased](https://medium.com/androiddevelopers/reification-of-the-erased-41e246725d2c) — объяснение
