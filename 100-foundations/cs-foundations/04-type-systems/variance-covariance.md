...
> [!question]- Почему List<Dog> можно присвоить переменной List<Animal> в Kotlin, но MutableList<Dog> нельзя присвоить MutableList<Animal>?
> List<T> в Kotlin объявлен как List<out T> — ковариантный, только производит T (read-only). Если Dog : Animal, то List<Dog> является подтипом List<Animal> — безопасно, потому что из списка можно только читать Animal. MutableList<T> — инвариантный, и производит, и потребляет T. Если бы MutableList<Dog> был подтипом MutableList<Animal>, можно было бы добавить Cat через ссылку MutableList<Animal> — нарушение type safety.

> [!question]- Что означает правило PECS (Producer Extends, Consumer Super) и как оно выражается в Kotlin?
> PECS из Java: если generic-тип только производит значения — используй `? extends T` (covariance), если только потребляет — `? super T` (contravariance). В Kotlin: `out T` = producer (ковариантность, можно читать T), `in T` = consumer (контравариантность, можно записывать T). Мнемоника: out = "выходит из класса" (return), in = "входит в класс" (parameter).

> [!question]- Функция принимает Comparator<Animal>. Можно ли передать Comparator<Any>? Почему?
> Да, потому что Comparator<T> объявлен как Comparator<in T> — контравариантный. Comparator<Any> умеет сравнивать любые объекты, значит умеет сравнивать и Animal. При contravariance направление подтипирования инвертируется: если Animal : Any, то Comparator<Any> является подтипом Comparator<Animal>. Это безопасно: Comparator только потребляет T (принимает в compare()), не производит.

---

## Ключевые карточки

Что такое ковариантность (covariance) и как она обозначается в Kotlin?
?
Ковариантность (out): если Dog : Animal, то Producer<Dog> : Producer<Animal>. Направление подтипирования сохраняется. В Kotlin: `class Box<out T>`. T может быть только в out-позициях (return type). List<out T> — ковариантный, поэтому List<Dog> присваивается List<Animal>.

---

Что такое контравариантность (contravariance) и как она обозначается?
?
Контравариантность (in): если Dog : Animal, то Consumer<Animal> : Consumer<Dog>. Направление инвертируется. В Kotlin: `class Handler<in T>`. T может быть только в in-позициях (parameter type). Comparator<in T> — контравариантный, Comparator<Any> можно передать как Comparator<Animal>.

---

Что такое инвариантность и когда она необходима?
?
Инвариантность: MutableBox<Dog> НЕ является подтипом MutableBox<Animal> и наоборот. Нет отношения подтипирования. Необходима когда тип используется и как producer, и как consumer (read+write). MutableList<T> инвариантен — если бы был ковариантен, можно было бы добавить Cat в List<Dog> через ссылку List<Animal>.

---

Чем declaration-site variance отличается от use-site variance?
?
Declaration-site (Kotlin): variance указывается при объявлении класса: `class List<out T>`. Все использования автоматически ковариантны. Use-site (Java wildcards): variance указывается при каждом использовании: `List<? extends Animal>`. Kotlin поддерживает оба: declaration-site (out/in) и use-site (type projection: `Array<out Animal>`).

---

Что такое @UnsafeVariance в Kotlin и зачем оно нужно?
?
@UnsafeVariance позволяет использовать out-параметр в in-позиции (или наоборот), подавляя ошибку компилятора. Пример: List<out T> имеет contains(element: @UnsafeVariance T) — T в in-позиции, но безопасно, потому что contains только сравнивает, не модифицирует. Используется когда разработчик гарантирует безопасность, а компилятор не может это доказать.
...
|-------------|------|-------|
| Следующий шаг | [[type-erasure-reification]] | Понять, что происходит с generic-типами и variance в runtime |
| Углубиться | [[kotlin-type-system]] | Практическое применение variance в Kotlin |
