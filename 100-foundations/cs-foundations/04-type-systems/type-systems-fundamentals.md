...
> [!question]- Kotlin — статически типизированный язык с type inference. Чем это отличается от динамической типизации Python, если в обоих языках можно не писать типы?
> В Kotlin type inference работает compile-time: компилятор выводит типы из контекста (val x = 42 -> x: Int) и проверяет все типы до запуска. После вывода тип фиксирован — x = "hello" вызовет ошибку компиляции. В Python типы определяются runtime: x = 42 присваивает тип объекту, а x = "hello" следующей строкой — валидно, тип меняется. Ошибка типа в Python обнаружится только при выполнении.

> [!question]- Почему Kotlin имеет null-safe систему типов и как это связано с концепцией soundness?
> Sound type system гарантирует: если программа прошла type checking, определённые ошибки невозможны в runtime. NullPointerException в Java нарушает soundness: компилятор разрешает код, который падает в runtime. Kotlin разделяет String (non-null) и String? (nullable) — компилятор гарантирует, что non-null переменная никогда не будет null. Это делает систему типов более sound (хотя абсолютная soundness недостижима на практике).

> [!question]- В KMP один Kotlin-код работает на JVM (type erasure) и Native (сохранение типов). Какие практические проблемы это создаёт?
> На JVM generics подвергаются type erasure: List<Int> и List<String> в runtime одинаковы — нельзя сделать `is List<Int>` проверку. На Native типы сохраняются. Это значит: (1) Код с `is T` check работает на Native, но не на JVM (без reified). (2) Reflection ведёт себя по-разному. (3) Оптимизации на основе типов различаются. Решение: использовать inline reified для кросс-платформенных type checks.

---

## Ключевые карточки

Чем static typing отличается от dynamic typing?
?
Static typing — типы проверяются compile-time, ошибки обнаруживаются до запуска (Kotlin, Java, Swift). Dynamic typing — типы проверяются runtime, ошибки обнаруживаются при выполнении (Python, JavaScript, Ruby). Static даёт безопасность и IDE-поддержку, dynamic — гибкость и скорость прототипирования.

---

Что такое type inference и как он работает в Kotlin?
?
Type inference — автоматический вывод типов компилятором из контекста. В Kotlin: val x = 42 (компилятор выводит Int), val list = listOf("a", "b") (выводит List<String>). K2 frontend использует FIR для более мощного inference. Типы выводятся compile-time — безопасность static typing + лаконичность dynamic.

---

Что такое soundness системы типов?
?
Soundness — гарантия: если программа прошла type checking, определённые категории ошибок невозможны в runtime. Sound система: "well-typed programs don't go wrong". На практике абсолютная soundness жертвуется ради удобства (Java array covariance unsound, Kotlin @UnsafeVariance). Более sound = больше ошибок пойманных compile-time.

---

Какие виды полиморфизма поддерживает Kotlin?
?
1) Subtype (наследование): open class, interface — переменная базового типа хранит подтип. 2) Parametric (generics): List<T> — один код для разных типов. 3) Ad-hoc (перегрузка): fun process(x: Int), fun process(x: String) — разный код для разных типов. Kotlin также поддерживает extension functions как форму ad-hoc полиморфизма.

---

Что такое duck typing и чем оно отличается от structural typing?
?
Duck typing (Python) — runtime: "если объект имеет метод quack(), вызываем его" — ошибка при отсутствии обнаружится в runtime. Structural typing (TypeScript, Go) — compile-time: компилятор проверяет, что тип имеет нужные поля/методы, без explicit implements. Оба не требуют explicit наследования, но structural typing безопаснее.
...
|-------------|------|-------|
| Следующий шаг | [[generics-parametric-polymorphism]] | Глубокое погружение в generics — основу типобезопасных коллекций |
| Углубиться | [[variance-covariance]] | Понять ковариантность и контравариантность в Kotlin |
| Смежная тема | [[compilation-pipeline]] | Как система типов используется на этапе семантического анализа |
