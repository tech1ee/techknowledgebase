...
> [!question]- Почему Kotlin coroutines называют "легковесными потоками" и чем они фундаментально отличаются от OS-потоков?
> OS-поток создаётся ядром ОС, имеет свой стек (~1-8 MB), требует context switch через ядро (~1-10 мкс) с переключением TLB и кэшей. Coroutine — объект в user-space (~несколько KB), переключение через suspend/resume без участия ядра (~100 нс). Можно создать миллион coroutines, но не миллион потоков. "Легковесность" — в отсутствии OS overhead: нет системного вызова, нет переключения контекста ядра.

> [!question]- На Android один процесс, но много потоков. Почему Android не использует многопроцессную архитектуру для компонентов приложения?
> Потоки внутри одного процесса разделяют виртуальное адресное пространство — обмен данными мгновенный (общая heap-память). Межпроцессное взаимодействие (IPC) требует копирования данных через ядро (Binder), что на порядки медленнее. Приложение активно обменивается данными между UI, networking, DB — разделение на процессы создало бы огромный overhead. Однако Service может работать в отдельном процессе (android:process) для изоляции критичных компонентов.

> [!question]- Два потока одновременно записывают в одну переменную без синхронизации. Какие проблемы могут возникнуть и почему?
> Race condition: результат зависит от порядка исполнения потоков. Без синхронизации: (1) Lost update — одно значение перезатрёт другое. (2) Torn read/write — на некоторых архитектурах 64-bit запись неатомарна, и читатель увидит "половину" значения. (3) Visibility — из-за кэшей CPU один поток может не увидеть записи другого. Решения: mutex, atomic операции, volatile.
...
?
Процесс — изолированная единица исполнения с собственным адресным пространством, файловыми дескрипторами, PID. Поток (thread) — единица исполнения внутри процесса, разделяющая адресное пространство с другими потоками. У потока свой стек и регистры, но общая heap, code segment, данные. Создание потока ~10x быстрее процесса.

---

Что такое context switch и почему он дорогой?
?
Context switch — переключение CPU с одного потока/процесса на другой. Шаги: сохранить регистры текущего потока, восстановить регистры нового, обновить page tables (для процессов). Стоимость: ~1-10 мкс + невидимые потери: промахи TLB, инвалидация кэшей L1/L2. Для процессов дороже чем для потоков из-за смены адресного пространства.

---

Какие модели потоков существуют (M:N mapping)?
?
1:1 (kernel threads) — каждый user thread = один kernel thread (Linux, Windows). M:1 (green threads) — много user threads на один kernel thread (ранний Java, Erlang BEAM). M:N (hybrid) — M user threads на N kernel threads (Go goroutines, Java Virtual Threads). Kotlin coroutines — M:N через dispatcher'ы.

---

Что такое thread safety и какие уровни существуют?
?
Thread safety — корректная работа кода при одновременном доступе из нескольких потоков. Уровни: (1) Immutable — данные только для чтения, безопасны всегда. (2) Thread-safe — корректная работа через внутреннюю синхронизацию. (3) Conditionally safe — безопасно при внешней синхронизации вызывающим кодом. (4) Not thread-safe — требует полной внешней синхронизации.

---

Почему 1MB стек потока ограничивает их количество?
?
Каждый OS-поток получает стек (~1 MB по умолчанию в Linux). 1000 потоков = 1 GB только стековой памяти, плюс overhead ядра на каждый поток. На 32-bit системах адресное пространство 4 GB — максимум ~4000 потоков. Coroutines используют ~несколько KB объектов на heap, поэтому можно создать миллион.
...
|-------------|------|-------|
| Следующий шаг | [[concurrency-vs-parallelism]] | Понять разницу между конкурентностью и параллелизмом |
| Углубиться | [[synchronization-primitives]] | Изучить mutex, semaphore и механизмы координации |
| Смежная тема | [[os-fundamentals-for-devs]] | Как ОС управляет процессами на уровне ядра |
