...
> [!question]- Два потока пытаются захватить два мьютекса (A и B) в разном порядке. Почему возникает deadlock и как его предотвратить?
> Deadlock возникает при circular wait: поток 1 захватил A и ждёт B, поток 2 захватил B и ждёт A. Ни один не может продвинуться. Предотвращение: (1) Lock ordering — всегда захватывать мьютексы в одном порядке (сначала A, потом B). (2) tryLock с timeout — отпустить все захваченные, если не удалось захватить следующий. (3) Избегать вложенных блокировок. (4) Использовать lock-free структуры данных (atomic operations).

> [!question]- Почему volatile в Java/Kotlin не заменяет mutex для операции counter++?
> volatile гарантирует visibility (все потоки видят актуальное значение) и запрещает reordering, но не обеспечивает atomicity составных операций. counter++ — это три операции: read, increment, write. Между read и write другой поток может прочитать старое значение. Результат: lost updates. Для атомарного инкремента нужен AtomicInteger.incrementAndGet() или mutex. volatile подходит только для простых read/write флагов.

> [!question]- На Mars Pathfinder произошёл сбой из-за priority inversion. Что это такое и как решается?
> Priority inversion: высокоприоритетный поток блокируется ожиданием mutex'а, который удерживает низкоприоритетный поток. Средний поток вытесняет низкоприоритетный, косвенно блокируя высокоприоритетный. На Mars Pathfinder: высокоприоритетная задача (bus management) ждала mutex от низкоприоритетной (meteo data), которую вытесняла средняя. Решение: priority inheritance — временно поднять приоритет владельца mutex'а до уровня ожидающего потока.
...
?
Mutex (mutual exclusion) — бинарный: один поток владеет, только он может освободить. Semaphore — счётчик: позволяет N потокам одновременно (N определяется при создании). Бинарный семафор похож на mutex, но любой поток может сделать signal (release). Mutex имеет ownership, semaphore — нет. Mutex предназначен для защиты critical section, semaphore — для ограничения количества одновременных доступов.

---

Какие четыре условия необходимы для deadlock (условия Коффмана)?
?
1) Mutual exclusion — ресурс может быть захвачен только одним потоком. 2) Hold and wait — поток удерживает ресурс и ждёт другой. 3) No preemption — ресурс нельзя отобрать. 4) Circular wait — цепочка потоков, каждый ждёт ресурс следующего. Для deadlock нужны все четыре одновременно. Устранение любого предотвращает deadlock.

---

Что такое CAS (Compare-And-Swap) и зачем он нужен?
?
CAS — атомарная операция процессора: "если значение == expected, заменить на new, иначе вернуть текущее". Выполняется за одну hardware-инструкцию (CMPXCHG на x86). Основа lock-free алгоритмов: вместо mutex'а поток пытается CAS в цикле. Без блокировок, без deadlock'ов, но возможен livelock (бесконечные повторы при высокой конкуренции).

---

Что делает volatile в Java/Kotlin (JVM)?
?
volatile гарантирует: (1) Visibility — запись одного потока видна всем другим (flush cache -> main memory). (2) Ordering — запрещает компилятору и процессору переупорядочивать операции с volatile переменной. НЕ гарантирует atomicity составных операций (i++ — не атомарна). Подходит для флагов и simple read/write.

---

Чем ReadWriteLock лучше обычного mutex?
?
ReadWriteLock разделяет доступ: множество reader'ов могут работать одновременно (shared lock), writer получает эксклюзивный доступ. При преобладании чтений (read-heavy workload) — значительный выигрыш по сравнению с mutex, который блокирует все операции. Пример: кэш, который часто читается и редко обновляется.
...
|-------------|------|-------|
| Следующий шаг | [[async-models-overview]] | Понять модели async-программирования от callbacks до coroutines |
| Углубиться | [[kotlin-coroutines]] | Как Kotlin coroutines решают проблемы синхронизации |
| Смежная тема | [[concurrency-vs-parallelism]] | Вернуться к концептуальной разнице concurrency и parallelism |
