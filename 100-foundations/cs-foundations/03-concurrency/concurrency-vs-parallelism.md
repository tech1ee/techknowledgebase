...
> [!question]- Kotlin coroutines могут работать на одном потоке (Dispatchers.Main). Это concurrency или parallelism и почему?
> Это concurrency без parallelism. Concurrency — управление несколькими задачами одновременно (они могут чередоваться). Parallelism — физическое одновременное исполнение на нескольких ядрах. Coroutines на одном потоке чередуются в точках suspension (suspend-функции), создавая иллюзию одновременности, но физически выполняется только одна корутина в каждый момент. Это как один повар, переключающийся между блюдами.

> [!question]- Почему закон Амдала ограничивает выигрыш от добавления ядер, даже если 95% кода параллелизуемо?
> По закону Амдала: speedup = 1 / (S + (1-S)/N), где S — доля sequential кода, N — число ядер. Даже при 95% параллельного кода (S=0.05) и бесконечном числе ядер максимальный speedup = 1/0.05 = 20x. Sequential 5% становятся bottleneck, потому что их нельзя ускорить добавлением ядер. Это фундаментальный предел параллелизма.

> [!question]- На Android-устройстве 8 ядер, но приложение использует Dispatchers.Default с 4 потоками. Это ограничивает parallelism? Почему?
> Dispatchers.Default создаёт пул размером max(2, число ядер) потоков, то есть на 8-ядерном устройстве будет 8 потоков. Если бы было только 4 потока — да, это ограничивает parallelism до 4 параллельных вычислений. Concurrency при этом не ограничена: тысячи coroutines могут быть запущены, но одновременно на CPU будут работать только 4/8. Это различие и есть суть concurrency vs parallelism.
...
?
Concurrency — управление несколькими задачами одновременно (структура программы). Parallelism — физическое одновременное исполнение (свойство runtime). Concurrency возможна на одном ядре (чередование задач), parallelism требует нескольких ядер. Rob Pike: "Concurrency is about dealing with lots of things at once. Parallelism is about doing lots of things at once."

---

Что такое закон Амдала и какова его формула?
?
Закон Амдала: speedup = 1 / (S + (1-S)/N), где S — доля sequential кода, N — число процессоров. Определяет теоретический предел ускорения от параллелизации. При S=0.1 (10% sequential) и бесконечном N максимальный speedup = 10x. Sequential часть — bottleneck.

---

Чем закон Густавсона отличается от закона Амдала?
?
Амдал: фиксированный размер задачи, добавляем ядра. Густавсон: фиксированное время, увеличиваем задачу. При масштабировании данных доля sequential кода уменьшается, и speedup растёт почти линейно. Формула Густавсона: speedup = N - S*(N-1). Это более оптимистичный взгляд на параллелизм для больших данных.

---

Как coroutines сочетают concurrency и parallelism?
?
Coroutines предоставляют concurrency (структурное управление задачами через suspend/resume). Parallelism зависит от dispatcher'а: Dispatchers.Main — один поток (только concurrency), Dispatchers.Default — пул потоков (concurrency + parallelism), Dispatchers.IO — расширенный пул для блокирующих операций.

---

Какие три модели конкурентности существуют?
?
1) Shared Memory (потоки + мьютексы) — потоки разделяют память, синхронизация через примитивы. 2) Message Passing (CSP, Actor model) — изолированные процессы общаются сообщениями, нет shared state. 3) Data Parallelism (SIMD, GPU) — одна операция над массивом данных параллельно. Kotlin coroutines ближе к CSP через channels.
...
|-------------|------|-------|
| Следующий шаг | [[synchronization-primitives]] | Изучить mutex, semaphore и другие механизмы координации |
| Углубиться | [[async-models-overview]] | Понять модели async-программирования от callbacks до coroutines |
| Смежная тема | [[processes-threads-fundamentals]] | Вернуться к основам процессов и потоков |
