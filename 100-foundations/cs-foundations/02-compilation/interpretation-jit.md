...
> [!question]- Ты замеряешь производительность Kotlin-кода на JVM, и первые 1000 итераций в 10 раз медленнее последующих. Почему это происходит и как правильно делать бенчмарки?
> Это warmup-эффект JIT-компиляции. Первые итерации выполняются интерпретатором (Level 0), затем C1 компилирует горячие методы с базовыми оптимизациями (Level 3), и только после ~10000 вызовов C2 применяет агрессивные оптимизации (Level 4). Правильный бенчмарк включает фазу прогрева — JMH (Java Microbenchmark Harness) делает это автоматически, запуская warmup-итерации перед измерением.

> [!question]- Почему inline cache переходит в megamorphic-состояние и как это влияет на производительность горячего цикла?
> Inline cache отслеживает типы на каждом call site. Если на одном call site встречается 1 тип — monomorphic (прямой вызов), 2-4 типа — polymorphic (цепочка проверок), 5+ типов — megamorphic (медленный vtable lookup). В megamorphic-состоянии JIT не может кэшировать конкретные реализации, и каждый вызов требует полного lookup. Разница может составлять 5-10x для горячих циклов. Решение: группировать объекты по типу или использовать конкретные типы вместо интерфейсов.

> [!question]- Один и тот же Kotlin-код на JVM показывает пиковую производительность через 30 секунд, а на iOS (Kotlin/Native) — сразу при запуске, но чуть ниже пика JVM. Почему?
> JVM использует JIT-компиляцию: начинает с интерпретации, собирает профиль, компилирует горячий код с агрессивными оптимизациями (C2). Через 30 секунд пик достигнут, потому что C2 использовал runtime-данные для спекулятивных оптимизаций. Kotlin/Native использует AOT через LLVM: код скомпилирован до запуска — мгновенный старт, предсказуемая производительность, но без runtime-профилирования. JIT может обогнать AOT по пиковой скорости, потому что знает реальные типы и горячие пути.

> [!question]- Что такое деоптимизация и почему она не является катастрофой для JIT?
> Деоптимизация — откат от скомпилированного native-кода к интерпретации, когда спекулятивные предположения JIT оказались неверными (например, предполагал тип Int, а пришёл Double). JIT вставляет guard-проверки перед спекулятивным кодом. При срабатывании guard'а происходит bailout. Это не катастрофа, потому что JIT перекомпилирует метод с учётом новой информации — теперь для более широкого набора типов. Обычно после re-JIT код снова становится быстрым.

---

## Ключевые карточки

Какие 5 уровней tiered compilation использует HotSpot JVM?
?
Level 0 — Interpreter (полное профилирование). Level 1 — C1 без профилирования (для trivial-методов). Level 2 — C1 с лёгким профилированием (когда C2 занят). Level 3 — C1 с полным профилированием (основной путь). Level 4 — C2 с агрессивными оптимизациями (максимальная производительность). Типичный путь: 0 -> 3 -> 4.

---

Чем отличается C1 от C2 компилятора в HotSpot JVM?
?
C1 (Client) — быстрая компиляция за 1-10 мс, базовые оптимизации (inlining маленьких методов, constant folding, DCE). C2 (Server) — медленная компиляция за 50-500 мс, агрессивные оптимизации (escape analysis, loop unrolling, vectorization, speculative inlining). C2 использует sea-of-nodes IR для глубокого анализа.

---

Какие три состояния имеет inline cache?
?
Monomorphic (1 тип) — одна проверка + прямой вызов, самый быстрый. Polymorphic (2-4 типа) — цепочка проверок + прямые вызовы, быстрый. Megamorphic (5+ типов) — vtable/hashtable lookup, медленный. Эмпирические данные: ~90% call sites monomorphic, ~9% polymorphic, ~1% megamorphic.

---

Что такое OSR (On-Stack Replacement) и зачем он нужен?
?
OSR позволяет заменить интерпретируемый цикл на скомпилированный JIT-код прямо посреди выполнения. Без OSR метод с циклом на миллион итераций компилируется только после завершения метода. С OSR — JIT может подменить код "на ходу" после достижения порога итераций. Это как замена двигателя во время движения.

---

Чем method JIT отличается от tracing JIT?
?
Method JIT (JVM, V8) компилирует целые методы/функции. Tracing JIT (LuaJIT, PyPy) записывает реальный путь исполнения одной итерации горячего цикла и компилирует этот trace. Tracing автоматически инлайнит все вызовы на пути, но страдает от performance cliffs при непредсказуемых ветвлениях.

---

Какие четыре уровня компиляции использует V8?
?
Ignition (интерпретатор bytecode), Sparkplug (baseline native без оптимизаций, почти мгновенная компиляция), Maglev (mid-tier с SSA, быстрая компиляция), TurboFan (fully optimized, sea-of-nodes, максимум). Sparkplug и Maglev добавлены для заполнения разрыва между интерпретацией и полной оптимизацией.
...
|-------------|------|-------|
| Следующий шаг | [[native-compilation-llvm]] | Понять AOT-альтернативу JIT — как Kotlin/Native компилирует через LLVM |
| Углубиться | [[jvm-profiling]] | Практика профилирования JVM-приложений и анализ JIT-поведения |
| Смежная тема | [[bytecode-virtual-machines]] | Вернуться к основам — что именно JIT компилирует |
