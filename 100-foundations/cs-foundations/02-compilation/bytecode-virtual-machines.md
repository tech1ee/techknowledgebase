...
> [!question]- Android-приложение тормозит при первом запуске, но потом работает плавно. Как архитектура ART (Android 7.0+) объясняет это поведение?
> ART 7.0+ использует гибридную модель AOT+JIT. При первом запуске код интерпретируется и JIT-компилируется по мере обнаружения горячих методов — отсюда начальные тормоза. JIT собирает профиль горячего кода и сохраняет его. При зарядке ночью происходит AOT-компиляция по профилю. Каждый следующий запуск быстрее предыдущего, потому что горячий код уже скомпилирован в native.

> [!question]- Почему Google выбрал register-based архитектуру для Dalvik VM вместо stack-based, как в JVM?
> Register-based архитектура генерирует на 47% меньше инструкций (Shi et al., 2008). Для мобильных процессоров 2007 года (~400 MHz ARM11) JIT-компиляция была слишком тяжёлой, и Dalvik работал преимущественно в режиме интерпретации. При интерпретации каждая инструкция — это dispatch (самая дорогая операция). Меньше инструкций = меньше dispatch'ей = быстрее. Кроме того, register-based bytecode ближе к реальным ARM-регистрам.

> [!question]- Один и тот же Kotlin-код ведёт себя по-разному на Android и iOS. Какие различия на уровне VM/runtime могут это объяснять?
> На Android код исполняется через ART (bytecode -> JIT/AOT native), на iOS — через Kotlin/Native (AOT via LLVM, нет VM). Различия: (1) integer overflow для больших чисел может отличаться, (2) coroutines работают через ContinuationInterceptor на JVM и через native threading на Native, (3) memory management — GC на ART vs tracing GC на K/N, (4) interop границы разные — JNI на Android vs Objective-C bridge на iOS.

---

## Ключевые карточки

Чем отличается stack-based VM от register-based VM?
?
Stack-based VM (JVM) хранит операнды на стеке — инструкции берут данные с вершины стека. Register-based VM (Dalvik) хранит операнды в виртуальных регистрах — каждая инструкция явно указывает номера регистров. Stack-based компактнее (инструкции 1-3 байта), register-based быстрее при интерпретации (меньше инструкций). После JIT разница минимальна.

---

Что такое Bytecode Verification в JVM и зачем она нужна?
?
JVM проверяет bytecode перед исполнением: корректность формата (magic number CAFEBABE), совместимость типов, отсутствие переполнения стека, легальность доступа (private/public). Нужна потому, что bytecode может быть создан вручную или злонамеренно модифицирован — верификатор предотвращает чтение произвольной памяти и обход системы типов.

---

Какие три этапа проходит класс в JVM при загрузке?
?
1) Loading — ClassLoader находит .class файл и читает bytecode в память. 2) Linking — verification (проверка корректности), preparation (выделение памяти для static полей), resolution (разрешение символьных ссылок). 3) Initialization — выполнение static блоков и инициализаторов. Загрузка ленивая — класс загружается только при реальной необходимости.

---

Чем DEX формат отличается от .class и зачем он нужен?
?
DEX (Dalvik Executable) оптимизирован для мобильных устройств: объединяет тысячи классов в один файл (меньше обращений к ФС), дедуплицирует константы (строка "Hello" хранится один раз вместо 50), использует register-based инструкции (быстрее интерпретация). Toolchain: .java -> .class -> d8 -> .dex -> APK.

---

Что такое WasmGC и почему он важен для Kotlin/Wasm?
?
WasmGC — расширение WebAssembly, добавляющее garbage collection на уровне runtime браузера. Без WasmGC Kotlin/Wasm должен был нести свой GC внутри WASM-модуля, увеличивая его размер. С WasmGC — GC предоставляется браузером. С декабря 2024 все major браузеры поддерживают WasmGC, и Compose Multiplatform работает на Kotlin/Wasm.

---

В какие форматы bytecode компилируется один Kotlin-файл в KMP?
?
JVM bytecode (.class) для JVM/серверов, DEX (через Android toolchain) для Android, Native binary (через LLVM) для iOS/macOS, JavaScript (.js) для Web, WebAssembly (.wasm) для браузеров. Один и тот же fun greet() = "Hello" превращается в 4-5 совершенно разных представлений.
...
|-------------|------|-------|
| Следующий шаг | [[native-compilation-llvm]] | Понять альтернативу VM — AOT-компиляцию через LLVM для iOS |
| Углубиться | [[interpretation-jit]] | Разобраться, как VM превращает bytecode в быстрый native-код через JIT |
| Смежная тема | [[compilation-pipeline]] | Вернуться к общей картине — откуда берётся bytecode |
