...
## Теоретические основы: формальный базис компиляции

### Компилятор: формальное определение

> **Определение:** Компилятор — программа, реализующая **отображение** f: L_source → L_target, где L_source и L_target — формальные языки, причём семантика программы сохраняется: ∀p ∈ L_source: sem(f(p)) = sem(p).

Это задача **трансляции** между формальными языками, которая решается через цепочку промежуточных представлений.

### Теоретический фундамент каждого этапа

| Этап | Теоретическая основа | Ключевой результат |
|------|---------------------|--------------------|
| **Лексический анализ** | Регулярные языки, конечные автоматы (Kleene, 1956) | Regex → NFA → DFA (Thompson, 1968) |
| **Синтаксический анализ** | Контекстно-свободные грамматики (Chomsky, 1956) | LL/LR-парсеры (Knuth, 1965 — LR) |
| **Семантический анализ** | Теория типов (Church, 1940), type inference (Hindley-Milner, 1969/1978) | Decidable type checking для большинства PL |
| **IR** | SSA form (Cytron et al., 1991) | Упрощение анализа потока данных |
| **Оптимизация** | Теория потоков данных (Kildall, 1973) | Dataflow analysis framework |
| **Кодогенерация** | Register allocation = Graph coloring (Chaitin, 1981) | NP-complete, но heuristics работают |

### Иерархия Хомского: почему этапы разделены

> **Иерархия Хомского (1956):** Формальные языки классифицируются по сложности грамматик:

```
Тип 3: Регулярные     — распознаются конечным автоматом  → Lexer
Тип 2: КС-грамматики  — распознаются стековым автоматом → Parser
Тип 1: КЗ-грамматики  — недецидируемые в общем случае
Тип 0: Рекурсивно перечислимые — машина Тьюринга
```

**Почему lexer и parser разделены:** Лексемы (идентификаторы, числа, ключевые слова) описываются **регулярными** выражениями (Тип 3), а структура программы — **контекстно-свободными** грамматиками (Тип 2). Регулярные языки проще и быстрее обрабатывать → отдельный быстрый этап.

### Frontend/Backend разделение: теорема M+N

> **Архитектурный принцип:** Для M языков и N целевых платформ:
> - Без IR: M × N компиляторов
> - С общим IR: M frontend'ов + N backend'ов = M + N компонентов

LLVM (Lattner & Adve, 2004) и GCC используют этот принцип. Kotlin K2 — тоже: один frontend, четыре backend'а (JVM, Native/LLVM, JS, WASM).

### Историческая атрибуция

| Веха | Автор | Год |
|------|-------|-----|
| Первый компилятор (A-0) | Grace Hopper | 1952 |
| Fortran (первый optimizing compiler) | John Backus et al. | 1957 |
| BNF (формальная нотация грамматик) | Backus & Naur | 1959 |
| LR-parsing | Donald Knuth | 1965 |
| YACC (compiler compiler) | Stephen Johnson | 1975 |
| SSA form | Cytron et al. | 1991 |
| LLVM | Chris Lattner | 2004 |

### Связи

- [[bytecode-virtual-machines]] — VM как целевая платформа компиляции
- [[interpretation-jit]] — JIT = compilation at runtime
- [[native-compilation-llvm]] — LLVM как backend
...
- **Chomsky, N. (1956). "Three Models for the Description of Language." IRE Transactions on Information Theory.** — Иерархия Хомского: формальная классификация языков и грамматик, основа разделения lexer/parser
- **Knuth, D.E. (1965). "On the Translation of Languages from Left to Right." Information and Control.** — LR-parsing: самый мощный класс детерминированных парсеров для КС-грамматик
- **Cytron, R. et al. (1991). "Efficiently Computing Static Single Assignment Form and the Control Dependence Graph." ACM TOPLAS.** — SSA form: промежуточное представление, упрощающее оптимизации
- **Lattner, C. & Adve, V. (2004). "LLVM: A Compilation Framework for Lifelong Program Analysis & Transformation." CGO.** — LLVM: универсальный backend для множества языков

### Практические руководства

- **Aho, A. et al. (2006). Compilers: Principles, Techniques, and Tools ("Dragon Book").** — Каноническая книга по компиляторам
- **Appel, A. (1998). Modern Compiler Implementation.** — Практический подход с реализацией
- **Nystrom, R. (2021). Crafting Interpreters.** — Лучший ресурс для начинающих
- [JetBrains: K2 Compiler](https://blog.jetbrains.com/kotlin/2024/05/celebrating-kotlin-2-0-fast-smart-and-multiplatform/) — Архитектура K2
