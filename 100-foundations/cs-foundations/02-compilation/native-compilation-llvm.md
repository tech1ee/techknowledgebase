...
> **AOT-компиляция (Ahead-Of-Time)** — трансляция исходного кода (или промежуточного представления) в машинный код **до** запуска программы. Результат — native binary, исполняемый непосредственно процессором.

> **LLVM** — модульная инфраструктура компиляторов, основанная на типизированном промежуточном представлении (LLVM IR) в форме SSA (Static Single Assignment).

### Историческая атрибуция

| Событие | Автор | Год | Вклад |
|---------|-------|-----|-------|
| **SSA Form** | Cytron et al. | 1991 | Static Single Assignment — каждая переменная присваивается ровно раз |
| **LLVM** | Lattner, C. (University of Illinois) | 2004 | "LLVM: A Compilation Framework for Lifelong Program Analysis & Transformation" |
| **Clang** | Apple / LLVM community | 2007 | C/C++/ObjC frontend для LLVM, замена GCC |
| **Swift** | Lattner, C. (Apple) | 2014 | Язык, спроектированный для LLVM с SIL (Swift IL) |
| **Kotlin/Native** | JetBrains | 2017 | Kotlin → LLVM IR → native binary (через Konan) |

### Теорема M+N (формально)

> **Теорема:** При наличии общего IR компиляция `M` языков для `N` платформ требует `M + N` компонентов вместо `M × N`.

```
Без LLVM:  M языков × N платформ = M·N компиляторов
           (3 × 4 = 12)

С LLVM:    M frontends + N backends = M+N компонентов
           (3 + 4 = 7)
...
### LLVM IR: ключевые свойства

LLVM IR — типизированное представление в **SSA-форме**:

| Свойство | Значение | Зачем |
|----------|----------|-------|
| **SSA** | Каждая переменная присваивается ровно один раз | Упрощает анализ и оптимизации |
| **Typed** | Каждое значение имеет тип (`i32`, `float`, `ptr`) | Type checking на уровне IR |
| **Three-address** | `%result = add i32 %a, %b` | Простой формат для оптимизаций |
| **Phi-nodes** | `%x = phi [%a, %bb1], [%b, %bb2]` | Слияние значений из разных путей (SSA) |
| **3 формы** | In-memory IR, Bitcode (.bc), Human-readable (.ll) | Flexibility |

### AOT vs JIT: формальные trade-offs

| Свойство | AOT (LLVM) | JIT (HotSpot) |
|----------|-----------|---------------|
| **Доступная информация** | Только статическая (типы, CFG) | Статическая + runtime profile |
| **Speculative optimization** | Нет (нет runtime info) | Да (deoptimization при неудаче) |
| **Startup time** | Мгновенный (код уже скомпилирован) | Медленный (interpret → compile) |
| **Peak performance** | Ниже (нет PGO runtime) | Выше (после warmup) |
| **Предсказуемость** | Высокая (всегда одна скорость) | Низкая (warmup, deopt) |
| **Profile-Guided Opt** | Offline PGO (отдельный прогон) | Online PGO (автоматически) |
...
### Теоретические основы
- Cytron, R. et al. (1991). "Efficiently Computing Static Single Assignment Form and the Control Dependence Graph" — TOPLAS; SSA form
- Lattner, C. & Adve, V. (2004). "LLVM: A Compilation Framework for Lifelong Program Analysis & Transformation" — CGO; LLVM architecture
- Aho, A. et al. (2006). *Compilers: Principles, Techniques, and Tools* (Dragon Book) — фундамент компиляторов
- Appel, A. (1998). *Modern Compiler Implementation* — SSA и промежуточные представления

### Практические руководства
- [LLVM Official](https://llvm.org/) — официальный сайт
- [AOSA Book: LLVM](https://aosabook.org/en/v1/llvm.html) — архитектура от создателей
