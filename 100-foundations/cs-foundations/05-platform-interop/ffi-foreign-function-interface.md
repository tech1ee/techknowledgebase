...
> [!question]- Почему JNI считается сложным и error-prone, и какие проблемы решает Java Foreign Function & Memory API (Project Panama)?
> JNI требует: написать C/C++ boilerplate, вручную управлять JNI references (Global/Local), конвертировать типы через JNIEnv, обрабатывать исключения через CheckException. Ошибка в любом месте — crash без stack trace. Panama (JEP 454) решает: вызов C-функций напрямую из Java (без C-кода), автоматический marshalling типов, безопасное управление off-heap памятью через Arena, type-safe API вместо void* указателей.

> [!question]- Как Kotlin/Native взаимодействует с iOS frameworks через Objective-C interop? Какой механизм используется?
> Kotlin/Native использует cinterop: (1) Читает Objective-C headers (.h файлы) из framework. (2) Генерирует Kotlin-обёртки для ObjC-классов/протоколов. (3) В runtime вызовы идут через objc_msgSend (dynamic dispatch Objective-C). Kotlin-классы наследующие NSObject регистрируются в ObjC runtime и доступны из Swift/ObjC. Маппинг: ObjC protocols -> Kotlin interfaces, ObjC blocks -> Kotlin lambdas, NSString <-> kotlin.String.

> [!question]- Почему передача строки из Kotlin/JVM в C через JNI требует копирования, а из Kotlin/Native — может не требовать?
> На JVM строки хранятся в managed heap как UTF-16 char arrays. GC может перемещать объекты — нативный код не может безопасно держать указатель на JVM-строку. Поэтому GetStringUTFChars копирует строку в native-память (UTF-8). На Kotlin/Native строки уже в native-памяти, memory layout фиксирован (нет компacting GC) — можно передать указатель напрямую через cinterop без копирования. Это фундаментальное различие managed vs native heap.
...
?
FFI — механизм вызова функций, написанных на одном языке, из другого языка. Решает проблему: разные языки имеют разные calling conventions, типы данных, memory management. FFI обеспечивает маппинг типов, маршаллинг данных и управление жизненным циклом. Примеры: JNI (Java->C), cinterop (Kotlin/Native->C/ObjC), ctypes (Python->C).

---

Как работает JNI (Java Native Interface)?
?
1) Объявить native метод в Java/Kotlin: `external fun nativeMethod(): Int`. 2) Компилятор генерирует JNI header. 3) Написать C/C++ реализацию с JNIEnv* для доступа к JVM. 4) Скомпилировать в .so/.dylib. 5) Загрузить через System.loadLibrary(). JNIEnv предоставляет функции: FindClass, GetMethodID, CallVoidMethod, NewStringUTF и т.д.

---

Как работает cinterop в Kotlin/Native?
?
1) Создать .def файл с описанием заголовков C/ObjC. 2) cinterop tool генерирует Kotlin bindings (.klib). 3) В Kotlin-коде импортировать и вызывать как обычные функции. Для Objective-C: автоматический маппинг классов, протоколов, блоков. Поддержка pointer arithmetic через CPointer<T>, автоматическое управление памятью через memScoped {}.

---

Чем Objective-C interop отличается от C interop в Kotlin/Native?
?
C interop: вызов C-функций, маппинг примитивных типов, работа с указателями (CPointer). ObjC interop: полный маппинг объектной модели — классы, протоколы, наследование, blocks, categories. Kotlin-класс может наследовать NSObject и быть доступным из Swift/ObjC. ObjC interop значительно богаче и сложнее, но критичен для iOS-разработки.

---

Что такое pinning в контексте FFI?
?
Pinning — фиксация объекта в памяти, запрет GC на перемещение. Нужен когда native-код держит указатель на managed-объект: если GC переместит объект, указатель станет dangling. В JNI: GetPrimitiveArrayCritical пинит массив. В .NET: fixed statement. В Kotlin/Native: объекты уже не перемещаются (нет compacting GC), поэтому pinning не нужен.
...
|-------------|------|-------|
| Следующий шаг | [[bridges-bindings-overview]] | Автоматическая генерация bindings: SWIG, cinterop, SKIE |
| Углубиться | [[abi-calling-conventions]] | Низкоуровневые детали: как бинарный код вызывает функции |
| Смежная тема | [[memory-layout-marshalling]] | Как данные конвертируются между форматами при FFI-вызовах |
