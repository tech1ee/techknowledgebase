...
> [!question]- Структура C с полями char (1 байт) и int (4 байта) занимает 8 байт вместо 5. Почему и как это влияет на interop?
> Из-за alignment padding: процессор требует, чтобы int был выровнен на 4-байтовую границу. После char (offset 0, 1 байт) компилятор вставляет 3 байта padding, чтобы int начинался с offset 4. Итого: 1 (char) + 3 (padding) + 4 (int) = 8 байт. При interop Kotlin/Native должен знать точный layout: если маршаллинг не учитывает padding, данные будут прочитаны со сдвигом — corrupted values.

> [!question]- Почему передача данных между Kotlin/JVM и C через JNI требует маршаллинга, а между Kotlin/Native и C — не всегда?
> JVM хранит данные в managed heap с объектными заголовками (mark word, klass pointer), GC может перемещать объекты. C ожидает простые struct'ы в contiguous memory без заголовков. Маршаллинг: копирование данных из JVM-формата в C-формат и обратно. Kotlin/Native компилирует в native binary — данные уже в native-памяти, layout может совпадать с C struct layout (зависит от типа). Примитивы и структуры передаются без копирования через указатели.

> [!question]- Что такое endianness и когда оно создаёт проблемы при передаче данных между платформами?
> Endianness — порядок байтов в многобайтовом значении. Little-endian (x86, ARM): младший байт первым (0x12345678 хранится как 78 56 34 12). Big-endian (network byte order, SPARC): старший байт первым (12 34 56 78). Проблема при: сетевых протоколах (network = big-endian, host может быть little-endian), чтении бинарных файлов с другой платформы, serialization для кросс-платформенного обмена. Решение: явное преобразование через htonl/ntohl.

---

## Ключевые карточки

Что такое alignment и зачем он нужен?
?
Alignment — требование, чтобы адрес данных в памяти был кратен определённому числу (обычно размеру типа). Int (4 байта) выравнивается на 4, long (8 байт) на 8. Причина: hardware — шина памяти читает выровненные блоки за один цикл. Unaligned access: на x86 — медленнее (2 чтения вместо 1), на ARM — может вызвать hardware exception.

---

Что такое padding в структурах и как его минимизировать?
?
Padding — неиспользуемые байты, вставляемые компилятором для alignment. struct {char a; int b;} = 8 байт (3 байта padding). Минимизация: упорядочить поля по убыванию размера: struct {int b; char a;} = 5 байт + 3 tail padding = 8 байт (или 5 с __attribute__((packed))). Packed structures убирают padding, но теряют performance из-за unaligned access.

---

Что такое маршаллинг (marshalling)?
?
Маршаллинг — преобразование данных из формата одного языка/runtime в формат другого. JVM объект -> C struct: удалить object header, сконвертировать String (UTF-16 -> UTF-8), разложить поля по C layout. Обратно: создать JVM-объект, заполнить поля. Маршаллинг имеет стоимость (копирование, конвертация) и является bottleneck FFI-вызовов.

---

Чем отличается layout JVM-объекта от C-структуры?
?
JVM-объект: object header (mark word 8 байт + klass pointer 4/8 байт) + поля (с JVM-специфичным padding). C-структура: только поля (с platform-специфичным padding), без заголовков. JVM может переупорядочивать поля для минимизации padding. Размер JVM-объекта обычно больше аналогичного C struct из-за object header (12-16 байт overhead).

---

Что такое serialization и чем она отличается от маршаллинга?
?
Маршаллинг — конвертация между in-memory представлениями (JVM -> native). Serialization — конвертация из in-memory в byte stream (для файлов, сети). Serialization = маршаллинг + encoding в формат (JSON, Protobuf, CBOR). Маршаллинг обычно между языками в одном процессе, serialization — между процессами/машинами.
...
|-------------|------|-------|
| Следующий шаг | [[bridges-bindings-overview]] | Как автоматическая генерация bindings абстрагирует маршаллинг |
| Углубиться | [[abi-calling-conventions]] | Как ABI определяет layout и передачу данных |
| Смежная тема | [[memory-model-fundamentals]] | Вернуться к основам организации памяти |
