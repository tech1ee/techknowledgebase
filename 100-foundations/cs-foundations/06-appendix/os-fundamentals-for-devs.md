...
> [!question]- Приложение вызывает File.readText() в Kotlin. Сколько переключений между user mode и kernel mode происходит и почему?
> Минимум 2 переключения на каждый системный вызов: (1) user -> kernel (trap/syscall instruction) для open(), (2) kernel -> user (возврат file descriptor), (3) user -> kernel для read(), (4) kernel -> user (возврат данных). На практике больше: open + stat + read + close = ~8 переключений. Каждое переключение = сохранение/восстановление контекста, flush TLB, ~1-10 мкс. Поэтому buffered I/O читает большими блоками — минимизирует syscalls.

> [!question]- Почему Android-приложение видит 4 GB памяти, хотя физической RAM может быть 6 или 8 GB?
> Виртуальная память: каждый процесс имеет своё виртуальное адресное пространство, изолированное от других процессов. Размер определяется архитектурой (32-bit = 4 GB, 64-bit = теоретически 16 EB). MMU (Memory Management Unit) транслирует виртуальные адреса в физические через page tables. Процесс не знает о физической RAM — ОС мапит виртуальные страницы на физические по мере необходимости (demand paging). Изоляция: один процесс не может читать память другого.

> [!question]- Чем system call отличается от обычного вызова функции и почему он дороже?
> Обычный вызов функции (call/ret) — в user mode, ~1-5 нс, только push/pop на стек. System call: (1) Переход user -> kernel mode через специальную инструкцию (syscall на x86-64). (2) Сохранение регистров user-процесса. (3) Проверка прав и валидация параметров. (4) Выполнение в kernel mode. (5) Возврат в user mode. Стоимость: ~100-1000 нс (в 100-200x дороже обычного вызова). Дороже из-за: mode switch, TLB flush, security checks.
...
?
Виртуальная память — абстракция, дающая каждому процессу иллюзию собственного непрерывного адресного пространства. MMU транслирует виртуальные адреса в физические через page tables. Три задачи: (1) Изоляция — процессы не видят память друг друга. (2) Абстракция — программисту не нужно знать физические адреса. (3) Overcommit — можно выделить больше виртуальной памяти, чем есть RAM (swap).

---

Что такое system call (syscall) и какие категории существуют?
?
Syscall — запрос от user-процесса к ядру ОС для привилегированной операции. Категории: (1) File I/O: open, read, write, close. (2) Process: fork, exec, exit, wait. (3) Memory: mmap, munmap, brk. (4) Network: socket, bind, connect, send, recv. (5) Time: clock_gettime, nanosleep. Каждый syscall = переключение user -> kernel mode.

---

Чем user mode отличается от kernel mode?
?
User mode — ограниченные привилегии: нет прямого доступа к hardware, памяти других процессов, привилегированным инструкциям. Kernel mode — полные привилегии: доступ ко всему hardware, всей памяти, всем инструкциям CPU. Переключение через syscall/trap. Разделение обеспечивает безопасность: баг в приложении не может повредить ОС или другие процессы.

---

Что такое page fault и какие типы бывают?
?
Page fault — прерывание при обращении к виртуальной странице, которой нет в физической RAM. Minor fault: страница в памяти, но не замаплена (нужно обновить page table) — быстро. Major fault: страница на диске (swap) — нужно загрузить с диска — медленно (~10 мс, в ~100000x дороже доступа к RAM). Segfault: обращение к невалидному адресу — процесс завершается.

---

Что такое scheduler ОС и какие алгоритмы он использует?
?
Scheduler распределяет CPU-время между процессами/потоками. Алгоритмы: CFS (Completely Fair Scheduler, Linux) — виртуальное время, каждый процесс получает fair share. Priority-based — потоки с высоким приоритетом выполняются первыми. Round-robin — квант времени по кругу. Real-time: FIFO/RR для задач с дедлайнами. Android: CFS + cgroups для foreground/background разделения.
...
|-------------|------|-------|
| Следующий шаг | [[processes-threads-fundamentals]] | Как ОС создаёт и управляет процессами и потоками |
| Углубиться | [[cpu-architecture-basics]] | Понять hardware, на котором работает ОС |
| Смежная тема | [[memory-model-fundamentals]] | Как программист работает с памятью поверх виртуальной памяти ОС |
