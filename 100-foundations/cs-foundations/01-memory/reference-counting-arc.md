...
> **Reference Counting (RC)** — метод управления памятью, при котором каждый объект хранит счётчик количества ссылок на него. Когда счётчик достигает нуля, объект освобождается.

Формально: для объекта `o`, `rc(o) = |{p : p → o}|` — количество указателей, ссылающихся на `o`. Инвариант: `rc(o) = 0 ⟹ o` недостижим и может быть освобождён.

### Историческая атрибуция

| Событие | Автор | Год | Вклад |
|---------|-------|-----|-------|
| **Первая RC-система** | Collins, G.E. | 1960 | "A Method for Overlapping and Erasure of Lists" — первая реализация RC |
| **Proof: RC ≠ cycles** | McBeth, J.H. | 1963 | Доказательство невозможности обнаружения циклов чистым RC |
| **Deferred RC** | Deutsch & Bobrow | 1976 | Оптимизация: не считать стековые ссылки, периодический scan |
| **Cycle collection** | Bacon & Rajan | 2001 | "Concurrent Cycle Collection" — практическое решение проблемы циклов |
| **ARC в Objective-C** | Apple (Lattner) | 2011 | Compile-time вставка retain/release вместо manual |
| **Swift ARC** | Apple | 2014 | ARC как единственная модель управления памятью |

### Теоретические свойства RC vs Tracing GC

| Свойство | Reference Counting | Tracing GC |
|----------|-------------------|------------|
| **Определение "мусора"** | `rc(o) = 0` | `o ∉ reach(roots)` |
| **Когда освобождает** | Немедленно (deterministic) | При запуске GC (non-deterministic) |
| **Циклы** | НЕ обнаруживает (теорема McBeth) | Обнаруживает все |
| **Overhead per operation** | Каждое присваивание: +1/-1 | Нулевой (до GC pause) |
| **Throughput** | Ниже (20-30% overhead) | Выше (amortized) |
| **Latency** | Предсказуемая (нет пауз) | Паузы при GC (ms-сотни ms) |
| **Дуальность** | Bacon et al. (2004): RC и tracing — дуальны |

> **Теорема (McBeth, 1963):** Чистый reference counting не может обнаружить циклические ссылки. Для обнаружения циклов необходим алгоритм с глобальным обходом графа (tracing), что делает RC+cycle collector гибридом RC и tracing.

### ARC: compile-time vs runtime RC

**ARC (Automatic Reference Counting)** — не runtime GC, а **compile-time** вставка операций retain/release:

```
                    Manual RC              ARC
Кто вставляет       Программист            Компилятор
retain/release:
Когда:              Runtime                Compile-time анализ
Ошибки:             Забыл release → leak   Циклы → leak (weak решает)
                    Лишний release → crash  Компилятор гарантирует баланс
```
...
### Теоретические основы
- Collins, G.E. (1960). "A Method for Overlapping and Erasure of Lists" — CACM; первая реализация RC
- McBeth, J.H. (1963). "On the Reference Counter Method" — CACM; доказательство невозможности обнаружения циклов
- Deutsch, L.P. & Bobrow, D.G. (1976). "An Efficient, Incremental, Automatic Garbage Collector" — CACM; deferred RC
- Bacon, D.F. & Rajan, V.T. (2001). "Concurrent Cycle Collection in Reference Counted Systems" — ECOOP
- Jones, R. & Lins, R. (1996). *Garbage Collection* — главы 2-3: RC и его варианты

### Практические руководства
- Apple Documentation: Automatic Reference Counting — точные правила strong, weak, unowned
...
> [!question]- Почему retain cycles не обнаруживаются стандартным reference counting?
> Стандартный RC уменьшает счётчик, когда ссылка удаляется. При retain cycle два объекта ссылаются друг на друга: A->B, B->A. Когда внешние ссылки на A и B пропадают, count каждого остаётся >= 1 (из-за взаимных ссылок). RC никогда не уменьшит count до 0, хотя оба объекта недостижимы. Для обнаружения циклов нужен полный обход графа — а это уже tracing GC.

> [!question]- Ты разрабатываешь KMP-модуль. Kotlin-объект передаётся в Swift-код. Кто управляет его памятью?
> При передаче Kotlin-объекта в Swift через Objective-C bridge, объект получает retain от Swift-стороны (ARC). Kotlin/Native GC видит этот retain как внешнюю ссылку и не собирает объект. Когда Swift-сторона освобождает объект (ARC уменьшает count), Kotlin/Native может его собрать. Проблема: если Swift хранит strong reference, а Kotlin — strong reference на Swift-объект, возникает cross-language retain cycle.

> [!question]- В каком сценарии ты бы использовал unowned вместо weak в Swift?
> Unowned — когда ты уверен, что referenced object всегда живёт дольше, чем referencing object. Типичный случай: child ссылается на parent (child не существует без parent). Unowned не требует optional unwrapping и не имеет overhead обнуления. Но если referenced object освобождён раньше — crash. Weak безопаснее (автоматически становится nil), но требует optional и имеет overhead.
...
Что такое Reference Counting?
?
Техника управления памятью, где каждый объект хранит счётчик ссылок. При создании ссылки — count++, при удалении — count--. Когда count = 0, объект освобождается. Преимущество: детерминированное освобождение. Недостаток: не видит циклические ссылки.

---

Чем ARC отличается от ручного Reference Counting?
?
ARC (Automatic Reference Counting) — компилятор автоматически вставляет retain/release в нужных местах. Программист не вызывает retain/release вручную. Результат тот же (подсчёт ссылок), но без человеческих ошибок. ARC — compile-time, не runtime.

---

Что такое Retain Cycle и как его разрывать?
?
Retain Cycle — два объекта ссылаются друг на друга strong-ссылками. Count никогда не станет 0 — утечка памяти. Разрыв: одну ссылку сделать weak (обнуляется при освобождении) или unowned (crash при доступе после освобождения).

---

Чем weak отличается от unowned в Swift?
?
weak: optional, автоматически становится nil при освобождении объекта. Безопасно. unowned: не optional, crash если объект уже освобождён. Быстрее, но опаснее. Правило: weak для "может не существовать", unowned для "всегда существует пока я жив".

---

Как Kotlin/Native взаимодействует с ARC при iOS interop?
?
Kotlin/Native объекты, переданные в Swift, получают retain от ARC. K/N GC видит этот retain как внешнюю ссылку и не собирает объект. При release со стороны Swift K/N GC может собрать объект. Риск: cross-language retain cycles между Kotlin и Swift объектами.
...
|-------------|------|-------|
| Следующий шаг | [[memory-safety-ownership]] | Изучить ownership как третий подход к memory management |
| Углубиться | [[garbage-collection-explained]] | Сравнить RC с tracing GC в деталях |
| Смежная тема | [[ffi-foreign-function-interface]] | Понять, как ARC влияет на FFI между Kotlin и Swift |
