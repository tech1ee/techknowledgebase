...
> [!question]- Почему ownership system в Rust предотвращает data races на уровне компилятора?
> В Rust действует правило: либо одна мутабельная ссылка (&mut T), либо любое количество иммутабельных (&T), но не оба одновременно. Data race требует: (1) общий доступ + (2) мутабельность + (3) отсутствие синхронизации. Ownership system исключает комбинацию (1)+(2) на уровне компилятора — если есть мутабельный доступ, он единственный. Программа с data race просто не скомпилируется.

> [!question]- Как менялась модель потокобезопасности в Kotlin/Native и почему?
> Исходная модель (до 1.7.20): freeze model — объект "замораживался" перед передачей между потоками, становясь полностью иммутабельным. Проблема: сложно работать с мутабельным состоянием. Новая модель: tracing GC с полной поддержкой многопоточности, как на JVM. Объекты можно передавать между потоками без freeze. JetBrains отказались от freeze, потому что он создавал слишком много friction для разработчиков.

> [!question]- Ты проектируешь библиотеку для KMP. Какой подход к memory safety ты бы выбрал и почему?
> Для KMP-библиотеки: использовать иммутабельные data-структуры (Kotlin immutable collections), structured concurrency через coroutines, избегать глобального мутабельного состояния. Причина: код работает на JVM (GC), Native (tracing GC) и JS (GC). Иммутабельность безопасна на всех платформах. Для мутабельного состояния — Mutex или AtomicReference из kotlinx.atomicfu.
...
Что такое Memory Safety?
?
Гарантия отсутствия определённых классов ошибок: use-after-free (доступ к освобождённой памяти), buffer overflow (выход за границы), double free (двойное освобождение), dangling pointer (указатель на освобождённую память). Достигается через GC, ARC, ownership или runtime checks.

---

Что такое Ownership в Rust?
?
Каждое значение имеет ровно одного владельца. Когда владелец выходит из scope — значение уничтожается. Владение можно передать (move) или одолжить (borrow). Компилятор проверяет правила на этапе компиляции без runtime overhead.

---

Что такое RAII?
?
Resource Acquisition Is Initialization — паттерн из C++, где ресурс привязан к времени жизни объекта. Конструктор захватывает ресурс, деструктор освобождает. Гарантирует освобождение при выходе из scope. Kotlin use{}, Java try-with-resources — аналоги.

---

Что такое Sendable в Swift?
?
Протокол, маркирующий типы как безопасные для передачи между actors/потоками. Компилятор проверяет: value types, final immutable classes, actors — Sendable. Mutable classes без синхронизации — не Sendable. Аналог Send/Sync в Rust.

---

Чем freeze модель K/N отличалась от новой модели?
?
Freeze (до 1.7.20): объект замораживался — становился полностью иммутабельным, но мог передаваться между потоками. Новая модель: объекты свободно передаются между потоками, tracing GC работает concurrently. Ограничения freeze сняты, модель аналогична JVM.
...
|-------------|------|-------|
| Следующий шаг | [[compilation-pipeline]] | Перейти к изучению компиляции |
| Углубиться | [[synchronization-primitives]] | Изучить примитивы синхронизации для concurrent-кода |
| Смежная тема | [[reference-counting-arc]] | Сравнить ownership с ARC |
