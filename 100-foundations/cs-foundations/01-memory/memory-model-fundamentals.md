...
> [!question]- Почему stack быстрее heap, и в каких сценариях это преимущество не играет роли?
> Stack быстрее, потому что аллокация — просто сдвиг указателя (одна инструкция CPU), а данные лежат последовательно (cache-friendly). Heap требует поиска свободного блока и работы аллокатора. Преимущество не играет роли, когда: (1) размер данных неизвестен при компиляции, (2) данные должны жить дольше функции, (3) данные должны быть доступны из нескольких потоков.

> [!question]- Ты написал рекурсивную функцию, которая крашится на глубине 10000 вызовов. Какой механизм вызвал crash и как это связано со stack?
> Crash вызван Stack Overflow. Каждый рекурсивный вызов создаёт новый stack frame (локальные переменные, return address, saved registers). Stack имеет фиксированный размер (обычно 1-8 MB). При 10000 вызовах стек переполняется, и ОС генерирует SIGSEGV/StackOverflowError. Решения: tail recursion (если язык поддерживает), итеративный алгоритм, или увеличение размера стека.

> [!question]- Чем отличается модель памяти Kotlin на JVM, Native и JavaScript?
> JVM: объекты на managed heap, GC с поколениями, object header 12-16 байт. Native: собственный heap с tracing GC (ранее был freeze model), нет JVM overhead. JavaScript: V8 heap с generational GC, все объекты — JS objects. Практическое следствие: один и тот же Kotlin-код может потреблять разное количество памяти на разных платформах из-за разницы в object layout и GC-стратегиях.

> [!question]- Почему Escape Analysis важен для производительности, и как он связан с решением stack vs heap?
> Escape Analysis — анализ компилятора, определяющий, "убегает" ли объект из scope функции. Если объект не убегает (используется только внутри функции), компилятор может аллоцировать его на stack вместо heap. Это устраняет: аллокацию на heap, нагрузку на GC, indirection при доступе. JVM HotSpot и Kotlin/Native используют escape analysis для оптимизации.

---

## Ключевые карточки

Что такое Stack и чем он отличается от Heap?
?
Stack — область памяти с LIFO-порядком для локальных переменных и вызовов функций. Аллокация мгновенная (сдвиг указателя). Heap — область для динамически аллоцируемых объектов, живущих произвольное время. Аллокация дороже (поиск свободного блока).

---

Что такое Stack Frame?
?
Область стека, выделяемая для одного вызова функции. Содержит: return address, saved registers, локальные переменные, аргументы (если не в регистрах). Создаётся при вызове, уничтожается при возврате.

---

Что такое Escape Analysis?
?
Анализ компилятора, определяющий, используется ли объект только внутри функции (не "убегает"). Если не убегает — можно аллоцировать на stack вместо heap, что быстрее и не нагружает GC.

---

Почему объекты на JVM имеют overhead 12-16 байт?
?
Каждый JVM-объект содержит Object Header: Mark Word (8 байт, хранит hashcode, GC info, lock state) + Class Pointer (4 байта с compressed oops). Итого 12 байт минимум + padding до кратности 8.

---

Что такое Memory-Mapped Files?
?
Файл проецируется в виртуальное адресное пространство процесса. Чтение/запись файла выглядит как работа с памятью. ОС автоматически загружает страницы с диска по требованию (demand paging). Эффективно для больших файлов.

---

Как аллокаторы находят свободный блок на heap?
?
Основные стратегии: First Fit (первый подходящий), Best Fit (наименьший подходящий), Worst Fit (наибольший). Free list хранит свободные блоки. Buddy System делит память на степени двойки. Slab Allocator использует пулы фиксированного размера.
...
|-------------|------|-------|
| Следующий шаг | [[garbage-collection-explained]] | Узнать, как автоматически освобождается heap-память |
| Углубиться | [[memory-safety-ownership]] | Изучить ownership модели и гарантии безопасности |
| Смежная тема | [[cpu-architecture-basics]] | Понять cache hierarchy и почему locality важна |
