...
## Связность через inline wiki-links

Стандарт `related:` в frontmatter фиксирует 2–5 ключевых связей на уровне метаданных. Но читатель, который встретил незнакомый термин в теле текста, не должен покидать Obsidian, чтобы разобраться. Отсюда отдельное правило связности внутри текста.

### Правило

Каждый термин, концепция, инструмент, библиотека или файл формата, имеющий собственную страницу в vault, связывается `[[wiki-link]]` **при первом упоминании в теле материала**. Повторные упоминания — без ссылки, чтобы не захламлять текст.

Это включает:
- API и спецификации (например, первое упоминание `Vulkan` в файле про Android-графику → `[[vulkan-on-android-fundamentals]]`)
- Связанные концепции (первое упоминание `gimbal lock` в файле про вращения → `[[quaternions-and-rotations]]`)
- Инструменты и библиотеки (первое упоминание `Filament` → `[[filament-architecture-deep]]`)
- Форматы данных (`glTF`, `SPIR-V`, `KTX2`)
- Математические объекты (`quaternion`, `BRDF`, `Jacobian`)

### Что делать, если страницы ещё нет

Термин всё равно требует действия:
1. Сохранить wiki-link в тексте (будет «красной» ссылкой в Obsidian).
2. Добавить запись в `000-inbox/_inbox-queue.md` с описанием, какой файл надо создать.
3. В текущем файле кратко объяснить термин (1 предложение), чтобы читатель не завис.

### Исключения

- H-заголовки внутри того же файла — не требуют ссылок.
- Термины из повседневного программистского словаря, не имеющие собственной страницы (`if`, `thread`, `log`) — не ссылаются.
- Тема соседнего модуля, которая и так в `related:` — ссылается один раз при первом появлении, без повторов.

### Зачем это работает

Эффект сети Obsidian ценен только при плотности связей. Файл с 2–3 ссылками — это статья. Файл с 15–20 ссылками — узел сети, через который читатель движется нелинейно, сам выбирая глубину погружения. Принцип соответствует модели hypertext (Nelson, 1965), а также современному знанию об эффективном рекалле через контекстное связывание (Kosslyn, Koenig, 1992).

---

## Паттерн «Три уровня погружения»

Приём, применённый в [[big-o-complexity]] (УРОВЕНЬ 1 для бабушки → УРОВЕНЬ 2 для студента → УРОВЕНЬ 3 для профессионала). Формализуется как обязательный для определённого класса материалов.

### Когда применять

Обязательно — для deep-dive по фундаментальной теме, которая читается одновременно:
- новичком (нулевой опыт с темой),
- разработчиком mid-уровня (видел, но не копал),
- профессионалом (ищет edge cases и точные формулировки).

Признак применимости: файл имеет в тегах одновременно базовые concept-тематики и advanced-атрибуты. Если один уровень доминирует (например, узкий advanced-топик с полным списком prerequisites), приём необязателен — читатель уже подготовлен.

### Структура трёх уровней

```
## Уровень 1 — для бабушки
{Ноль кода. Только аналогии из реальной жизни.
 Объясняем WHY и WHAT, без HOW.}

## Уровень 2 — для студента
{Формальные определения, первый рабочий код (≤10 строк),
 пошаговые алгоритмы, простая математика.
 Отвечаем на HOW.}

## Уровень 3 — для профессионала
{Edge cases, оптимизации, trade-offs, взаимодействия с
 другими системами, корнер-кейсы в production.
 Отвечаем на BUT WHAT IF.}
```

Каждый уровень замкнут: читатель, остановившийся после Уровня 1, не чувствует, что текст оборван. Читатель на Уровне 3 не перечитывает Уровень 1, но может к нему вернуться за аналогией.

### Почему работает

Модель соответствует [[cognitive-science-rules#Зона ближайшего развития]] (Vygotsky, 1978): разные читатели находятся в разных ZPD, и один и тот же файл служит им через выбранный уровень. Также реализует принцип Progressive Disclosure (Nielsen, 2006): сложность открывается по мере готовности читателя.

### Эталоны в vault

- [[big-o-complexity]] — три уровня явно помечены H2-заголовками.
- [[solid-principles]] — формально без пометки «уровень», но функционально разнесено: обзор → каждый принцип по отдельности → система целиком.
- Новые материалы по 3D-графике (например, [[matrices-for-transformations]], [[quaternions-and-rotations]], [[pbr-physically-based-rendering]]) применяют три уровня.
...
- [[anti-ai-boilerplate]] — запрещённые шаблоны и заполнители

---

*Создано: 2026-02-10*
*Обновлено: 2026-04-20 — добавлены секции «Связность через inline wiki-links» и «Паттерн "Три уровня погружения"».*
