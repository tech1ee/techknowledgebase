...
| `primary_sources` | list | нет | Структурированный список первоисточников (url + title + accessed) — см. раздел ниже |
...
## Поле `primary_sources`

Для быстро меняющихся тем (AI, Android APIs, GPU libraries, frontend-экосистема) раздел «Источники» в тексте должен дублироваться структурированным полем в frontmatter. Это даёт три эффекта:

1. Dataview/скрипт могут автоматически проверять доступность URL и предупреждать, когда источник переехал или удалён.
2. Читателю легко извлечь список без парсинга текста.
3. При пересмотре файла видно, какие источники ещё актуальны (поле `accessed`), а какие надо перепроверить.
...
primary_sources:
  - url: "https://developer.android.com/games/develop/vulkan/overview"
    title: "Android Developers: Use Vulkan for graphics"
    accessed: 2026-04-20
  ...
  - url: "https://registry.khronos.org/vulkan/specs/1.4-extensions/html/"
    title: "Vulkan 1.4 Specification (Khronos)"
    accessed: 2026-04-20
```

### Правила

- **Минимум 2 источника** для любого deep-dive intermediate+ (совпадает с требованием [[depth-standard#Правило источников]]).
- **Приоритет источников** (в порядке убывания авторитета):
  1. Официальные спецификации (Khronos, W3C, ISO)
  2. Книги и академические статьи с DOI
  3. Официальная документация вендора (Google, Apple, Mozilla и т. п.)
  4. Доклады с крупных конференций (GDC, SIGGRAPH, Google I/O, Droidcon)
  5. Инженерные блоги с атрибуцией (Google Developers blog, Arm, Qualcomm)
  6. Medium/dev.to/Habr — только если автор подтверждаем по профилю и статья содержит уникальные данные

- **`accessed`** проставляется датой, когда ссылка была фактически открыта при написании. При `review_date` её надо обновить.
- **URL должен быть канонический** — не редирект, не shortener.
- **Полнота** важнее краткости: 8 источников лучше, чем 2, если это действительно использованные материалы.

### Связь с текстовой секцией «Источники»

`primary_sources` в frontmatter не отменяет раздел «Источники» в теле. Разница ролей:

| Где | Что | Зачем |
|---|---|---|
| frontmatter `primary_sources` | структурированный список URL | машинная проверка, быстрый экспорт |
| тело «Источники» | книги + talks + инлайн-комментарий к каждому источнику «зачем читать» | контекст для читателя, углубляющий понимание |

Оба блока обязательны. `primary_sources` — подмножество раздела «Источники»: только URL-адресуемые первичные материалы.
