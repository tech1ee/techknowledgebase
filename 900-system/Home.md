...
> **650+ материалов** в 5 зонах знаний с **системой активного обучения**.

**Для обучения:** [[study-dashboard]] — дашборд прогресса | [[learning-system-guide]] — как учиться с vault | [[recommended-plugins]] — рекомендуемые плагины

---

## Точки входа

> Одна база, много способов войти. Выбери свой формат.

| Формат | Описание |
|--------|----------|
| **Я [роль]** | Персональная карта: [[role-mobile-developer]] · [[role-tech-lead]] · [[role-backend-engineer]] · [[role-interview-candidate]] |
| **Концепция X везде** | Одна тема на всех платформах: [[concept-concurrency-everywhere]] · [[concept-memory-management]] · [[concept-architecture-patterns]] · [[concept-dependency-injection]] · [[concept-testing-strategies]] · [[concept-build-systems]] |
| **У меня проблема** | Диагностика: [[problem-app-performance]] · [[problem-interview-preparation]] · [[problem-team-not-shipping]] · [[problem-choosing-architecture]] · [[problem-scaling-codebase]] |
| **Помоги выбрать** | Деревья решений: [[decide-di-framework]] · [[decide-architecture-pattern]] · [[decide-database-approach]] · [[decide-cross-platform-strategy]] · [[decide-api-style]] |
| **Построить X** | От нуля до прода: [[build-production-android-app]] · [[build-production-ios-app]] · [[build-kmp-shared-library]] · [[build-ai-powered-feature]] |
| **К интервью** | По ролям и таймингу: [[interview-android-senior]] · [[interview-system-design]] · [[interview-behavioral-leadership]] |
| **Мой уровень** | Оцени себя: [[maturity-ladder]] |
| **Топ файлы** | 30 самых полезных: [[quick-reference]] |
| **Что нового** | Последние обновления: [[whats-new]] |
| **Глоссарий** | Найти термин: [[glossary-index]] |
...
> Каждый путь содержит чек-листы, время чтения, дни повторения и рекомендации по пропуску.

### По областям (фокус на одной теме)
...
### Интерливинг-пути (чередование тем для лучшего усвоения)

| Роль / Цель | Путь обучения | Области | Недель |
|-------------|---------------|---------|--------|
| Backend Engineer | [[interleaved-backend-engineer]] | Architecture, DB, Security, DevOps, Cloud, Networking | ~9 |
| Mobile Expert | [[interleaved-mobile-expert]] | Android, iOS, Cross-Platform, KMP | ~12 |
| Interview Prep | [[interleaved-interview-prep]] | CS Fundamentals, Career, Architecture | ~10 |
| Tech Lead | [[interleaved-tech-lead]] | Leadership, Communication, Architecture, Career | ~10 |
| Full Stack Foundation | [[interleaved-full-stack-foundation]] | Programming, DB, Networking, Security, OS, Cloud | ~10 |
...
> Где твой код работает. Мобильные платформы, JVM, кроссплатформа.
...
#### CS Foundations
> 61 материал: низкоуровневые основы — память, компиляция, типы, interop
...
> Как проектировать, строить и поддерживать production-системы.
...
> Слой интеллекта: от основ LLM до production AI-систем.
...
> Человеческий фактор: лидерство, коммуникация, мышление, карьера.
...
100-foundations/   → Фундамент (144 файла)
  ├── cs-fundamentals/    → 63: Алгоритмы и паттерны
  ├── cs-foundations/      → 61: Память, компиляция, типы
  ├── operating-systems/   → 8: Процессы, планировщики
  ├── jvm/                 → 37: JVM, Java, Kotlin
  └── programming/         → 12: Код, SOLID, паттерны

200-platforms/     → Платформы (246 файлов)
  ├── android/             → 70: Android Development
  ├── ios/                 → 45: iOS Development
  ├── kotlin-multiplatform/ → 70: KMP, Compose MP
  ├── cross-platform/      → 24: iOS vs Android
  └── cs-foundations/ (→ в 100-foundations)

300-systems/       → Проектирование и эксплуатация (91 файл)
  ├── architecture/        → 16: System Design, API
  ├── databases/           → 16: SQL, NoSQL
  ├── networking/          → 23: Сети и протоколы
  ├── security/            → 19: OWASP, AuthN/AuthZ
  ├── devops/              → 10: Docker, K8s, CI/CD
  └── cloud/               → 7: AWS, GCP, Azure

400-ai-ml/         → Интеллект (43 файла)
...
500-craft/         → Мастерство (128 файлов)
  ├── leadership/          → 44: Лидерство и менеджмент
  ├── career/              → 36: Карьера, поиск работы
  ├── communication/       → 26: Коммуникация
  └── thinking/            → 22: Мышление и обучение

800-resources/     → Справочники
900-system/        → Навигация, индексы, мета
  ├── _MOCs/               → 21 карта навигации
  ├── _views/              → 31 точка входа
...
```
      ...
      ▼                      ▼                      ▼
┌───────────┐      ┌──────────────┐       ┌──────────────┐
│ 900-system│      │  5 зон знаний │       │ 800-resources│
│  /_MOCs/  │      │  (контент)   │       │ (справочники)│
├───────────┤      └──────────────┘       └──────────────┘
│21 карта   │
│навигации  │
├───────────┤      Каждая область:
│ _views/   │      ├── *-overview.md (вход)
│31 точка   │      └── *-moc.md     (карта)
│входа      │
└───────────┘
```
